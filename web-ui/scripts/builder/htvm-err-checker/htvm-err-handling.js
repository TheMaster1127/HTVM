...
function handleErrorHelper2(multiLineText) {
    ...
    for (let A_Index4 = 0; A_Index4 < items4.length + 0; A_Index4++) {
        ...
        if (A_Index4 <= 142) {
            items5 = LoopParseFunc(multiLineText, "\n", "\r")
...
function handleError(htvmInstrText) {
    var fixTrim = "";
    items6 = LoopParseFunc(htvmInstrText, "\n", "\r")
    ...
    items10 = LoopParseFunc(htvmInstrText, "\n", "\r")
    for (let A_Index10 = 0; A_Index10 < items10.length + 0; A_Index10++) {
        const A_LoopField10 = items10[A_Index10 - 0];
        AIndex = A_Index10;
        ALoopField = A_LoopField10;
        for (let A_Index11 = 0; A_Index11 < HTVM_Size(lineThatCANNOThaveSpacses) + 0; A_Index11++) {
            if (AIndex == lineThatCANNOThaveSpacses[A_Index11] && InStr(ALoopField, " ")) {
        ...
        // index|error msg
        if (A_Index10 <= 148) {
            items12 = LoopParseFunc(htvmInstrText, "\n", "\r")
            for (let A_Index12 = 0; A_Index12 < items12.length + 0; A_Index12++) {
                const A_LoopField12 = items12[A_Index12 - 0];
                if (A_Index12 <= 148) {
                    if (ALoopField == A_LoopField12 && A_Index12 != AIndex) {
                        if (AIndex <= 148) {
                            return STR(AIndex) + "|You can't use the same keyword more than once. The problem is whit " + allNameARR[AIndex] + " and " + allNameARR[A_Index12] + "!!!";
                        }
                    }
                }
            }
        ...
        if (A_Index10 == 0) {
            ...
            for (let A_Index13 = 0; A_Index13 < HTVM_Size(onlyAllowed_0) + 0; A_Index13++) {
                if (A_LoopField10 == onlyAllowed_0[A_Index13]) {
                    for0 = 1;
                }
            }
            if (for0 == 0) {
                return STR(A_Index10) + "|You can only use: " + "" + Chr(34) + "cpp" + Chr(34) + ", " + Chr(34) + "py" + Chr(34) + ", " + Chr(34) + "js" + Chr(34) + ", " + Chr(34) + "go" + Chr(34) + ", " + Chr(34) + "lua" + Chr(34) + ", " + Chr(34) + "cs" + Chr(34) + ", " + Chr(34) + "java" + Chr(34) + ", " + Chr(34) + "kt" + Chr(34) + ", " + Chr(34) + "rb" + Chr(34) + ", " + Chr(34) + "nim" + Chr(34) + ", " + Chr(34) + "ahk" + Chr(34) + ", " + Chr(34) + "swift" + Chr(34) + ", " + Chr(34) + "dart" + Chr(34) + ", " + Chr(34) + "ts" + Chr(34) + ", " + Chr(34) + "groovy" + Chr(34);
            }
        }
        else
        */
        if (A_Index10 == 0 || A_Index10 == 1) {
            // Code for condition 0 and 1
            if (RegExMatch(A_LoopField10, "^[a-z][a-z0-9]{1,}$") == false) {
                return STR(A_Index10) + "|The file name must start with a lowercase letter and can only contain lowercase letters or numbers afterward.";
            }
        }
        else if (A_Index10 == 2) {
            // Code for condition 2
            if (InStr(A_LoopField10, Chr(13)) || InStr(A_LoopField10, Chr(10))) {
                return STR(A_Index10) + "|Dont use a newline or a carriage retrun";
            }
        }
        else if (A_Index10 >= 3 && A_Index10 <= 9) {
            // Code for conditions 3 - 9 (OSP and include)
            if (handleErrorHelper(disallowedChars_Globaly, A_LoopField10) || handleErrorHelper(disallowedChars_BeginEndLine, A_LoopField10, 1) || handleErrorHelper(disallowedChars_AT_THE_END, A_LoopField10, 2)) {
                return STR(A_Index10) + "|You can't use the folowing characters: " + disallowedChars_Globaly_EXPLAIN + ", " + disallowedChars_BeginEndLine_EXPLAIN + ", " + disallowedChars_AT_THE_END_EXPLAIN;
            }
        }
        else if (A_Index10 >= 10 && A_Index10 <= 45) {
            if (handleErrorHelper(disallowedChars_Globaly, A_LoopField10) || handleErrorHelper(disallowedChars_BeginEndLine, A_LoopField10, 1) || handleErrorHelper(disallowedChars_AT_THE_END, A_LoopField10, 2)) {
                return STR(A_Index10) + "|You can't use the following characters: " + disallowedChars_Globaly_EXPLAIN + ", " + disallowedChars_BeginEndLine_EXPLAIN + ", " + disallowedChars_AT_THE_END_EXPLAIN;
            }
        }
        else if (A_Index10 >= 46 && A_Index10 <= 47) {
            if (handleErrorHelper(disallowedChars_For_46_47, A_LoopField10) || handleErrorHelper(disallowedChars_Globaly, A_LoopField10)) {
                return STR(A_Index10) + "|You can't use the following characters: " + disallowedChars_For_46_47_EXPLAIN + ", " + disallowedChars_Globaly_EXPLAIN;
            }
        }
        else if (A_Index10 >= 48 && A_Index10 <= 83) {
            if (handleErrorHelper(disallowedChars_Globaly, A_LoopField10) || handleErrorHelper(disallowedChars_BeginEndLine, A_LoopField10, 1) || handleErrorHelper(disallowedChars_AT_THE_END, A_LoopField10, 2)) {
                return STR(A_Index10) + "|You can't use the following characters: " + disallowedChars_Globaly_EXPLAIN + ", " + disallowedChars_BeginEndLine_EXPLAIN + ", " + disallowedChars_AT_THE_END_EXPLAIN;
            }
        }
        else if (A_Index10 >= 84 && A_Index10 <= 89) {
            if (RegExMatch(A_LoopField10, "^\\.[A-Za-z0-9_]+$") == false) {
                return STR(A_Index10) + "|Array methods must start with a dot and contain only alphanumeric characters or underscores.";
            }
        }
        else if (A_Index10 >= 90 && A_Index10 <= 102) {
            if (handleErrorHelper(disallowedChars_Globaly, A_LoopField10) || handleErrorHelper(disallowedChars_BeginEndLine_FOR_ARRAY_KEYWORD, A_LoopField10, 1) || handleErrorHelper(disallowedChars_AT_THE_END_FOR_ARRAY_KEYWORD, A_LoopField10, 2)) {
                return STR(A_Index10) + "|You can't use the following characters: " + disallowedChars_Globaly_EXPLAIN + ", " + disallowedChars_BeginEndLine_FOR_ARRAY_KEYWORD_EXPLAIN + ", " + disallowedChars_AT_THE_END_FOR_ARRAY_KEYWORD_EXPLAIN;
            }
        }
        else if (A_Index10 == 103) {
            if (StrLen(A_LoopField10) != 1 || handleErrorHelper(disallowedChars_Globaly, A_LoopField10) || handleErrorHelper(disallowedChars_For_103, A_LoopField10)) {
                return STR(A_Index10) + "|Escape character must be a single valid character.";
            }
        }
        else if (A_Index10 >= 104 && A_Index10 <= 128) {
            if (handleErrorHelper(disallowedChars_Globaly, A_LoopField10) || handleErrorHelper(disallowedChars_BeginEndLine, A_LoopField10, 1) || handleErrorHelper(disallowedChars_AT_THE_END, A_LoopField10, 2)) {
                return STR(A_Index10) + "|You can't use the following characters: " + disallowedChars_Globaly_EXPLAIN + ", " + disallowedChars_BeginEndLine_EXPLAIN + ", " + disallowedChars_AT_THE_END_EXPLAIN;
            }
        }
        else if (A_Index10 >= 129 && A_Index10 <= 141) {
            if (handleErrorHelper(disallowedChars_Globaly, A_LoopField10) || handleErrorHelper(disallowedChars_BeginEndLine_FOR_ARRAY_KEYWORD, A_LoopField10, 1) || handleErrorHelper(disallowedChars_AT_THE_END_FOR_ARRAY_KEYWORD, A_LoopField10, 2) || InStr(A_LoopField10, " ")) {
                return STR(A_Index10) + "|Assignment operators can't contain spaces and can't use the following characters: " + disallowedChars_Globaly_EXPLAIN + ", " + disallowedChars_BeginEndLine_FOR_ARRAY_KEYWORD_EXPLAIN + ", " + disallowedChars_AT_THE_END_FOR_ARRAY_KEYWORD_EXPLAIN;
            }
        }
        else if (A_Index10 >= 142 && A_Index10 <= 143) {
            if (handleErrorHelper(disallowedChars_Globaly, A_LoopField10) || handleErrorHelper(disallowedChars_BeginEndLine_FOR_ARRAY_KEYWORD, A_LoopField10, 1)) {
                return STR(A_Index10) + "|Ternary operators can't use the following characters: " + disallowedChars_Globaly_EXPLAIN + ", " + disallowedChars_BeginEndLine_FOR_ARRAY_KEYWORD_EXPLAIN;
            }
        }
        else if (A_Index10 >= 144 && A_Index10 <= 145) {
            if (handleErrorHelper(disallowedChars_Globaly, A_LoopField10) || handleErrorHelper(disallowedChars_BeginEndLine, A_LoopField10, 1) || handleErrorHelper(disallowedChars_AT_THE_END, A_LoopField10, 2)) {
                return STR(A_Index10) + "|You can't use the following characters: " + disallowedChars_Globaly_EXPLAIN + ", " + disallowedChars_BeginEndLine_EXPLAIN + ", " + disallowedChars_AT_THE_END_EXPLAIN;
            }
        }
        else if (A_Index10 == 146) {
            if (RegExMatch(A_LoopField10, "^-?\\d+$") == false) {
                return STR(A_Index10) + "|Value must be a valid integer (optionally negative).";
            }
        }
        else if (A_Index10 >= 147 && A_Index10 <= 148) {
            if (RegExMatch(A_LoopField10, "^[a-zA-Z_][a-zA-Z0-9_]*$") == false) {
                return STR(A_Index10) + "|Variable names must start with a letter or underscore, and contain only alphanumeric characters and underscores.";
            }
        }
        else if (A_Index10 >= 149 && A_Index10 <= 161) {
            if (A_LoopField10 != "on" && A_LoopField10 != "off") {
                return STR(A_Index10) + "|Settings can only be 'on' or 'off'.";
            }
        }
        //;;;;;;;;;;;;;;;;;;;;;;;;;
        if (A_Index10 == 161) {
