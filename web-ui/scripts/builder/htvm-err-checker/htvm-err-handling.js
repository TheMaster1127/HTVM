...
function handleError(htvmInstrText) {
    ...
    if (handleErrorHelper2OUT != "NONE") {
        ...
        if ((INT(str3) >= 51 && INT(str3) <= 66) || (INT(str3) >= 90 && INT(str3) <= 94) || (INT(str3) >= 10 && INT(str3) <= 45)) {
            ...
            for (let A_Index10 = 0; A_Index10 < HTVM_Size(keywordIndices) + 0; A_Index10++) {
    ...
    for (let A_Index11 = 0; A_Index11 < items11.length + 0; A_Index11++) {
        ...
        for (let A_Index12 = 0; A_Index12 < HTVM_Size(lineThatCANNOThaveSpacses) + 0; A_Index12++) {
        ...
        // index|error msg
        if (A_Index11 <= 148) {
            items13 = LoopParseFunc(htvmInstrText, "\n", "\r")
