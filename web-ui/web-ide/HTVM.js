            ...
            if (InStr(A_LoopField173, "->")) {
                ...
                if (RegExMatch(temp, "\\b" + Trim(keyWordElse) + "\\b")) {
                    ...
                    if (RegExMatch(Trim(StrSplit(temp, "->", 1)), "(?<!\\s)\\.\\.(?!\\s)") && InStr(Trim(StrSplit(temp, "->", 1)), " in ")) {
                        ...
                        // keyWordOr
                        temp3 = "";
                        let items174 = LoopParseFunc(temp1, ",");
                        ...
                        // keyWordOr
                        temp3 = "";
                        let items175 = LoopParseFunc(temp1, ",");
...
function formatHTVMtoHTVMsubout_HELP(theCode, padding) {
    var out = "";
    for (let A_Index177 = 0; A_Index177 < padding * 4 + 0; A_Index177++) {
        out += " ";
    }
    return out + theCode;
}
...
function formatHTVMtoHTVMsubout(theCode) {
    var out = "";
    var inside = 0;
    let items178 = LoopParseFunc(theCode, "\n", "\r");
...
function fixIfElseIfMultiLineStatementHELP(aloopfield) {
    var out = "";
    var count = 0;
    let items179 = LoopParseFunc(aloopfield);
...
function fixIfElseIfMultiLineStatement(someHTVMcode) {
    var doWeExitInsta = 1;
    let items180 = LoopParseFunc(someHTVMcode, "\n", "\r");
    ...
    for (let A_Index181 = 0; A_Index181 < 10000 + 0; A_Index181++) {
        out2 = out;
        out = "";
        doWeExit = false;
        let items182 = LoopParseFunc(out2, "\n", "\r");
...
function doseHaveInclude(TheCodeThatMightHaveInclude) {
    let items183 = LoopParseFunc(TheCodeThatMightHaveInclude, "\n", "\r");
...
function compiler(htCode, allInstructionFile, mode, langToConvertToParam = "") {
    ...
    if (HTVM_Size(argHTVMinstrMORE) > 0) {
        ...
        let items185 = LoopParseFunc(allInstructionFile_2, "\n", "\r");
    ...
    */
    ...
    let items186 = LoopParseFunc(allInstructionFile, "\n", "\r");
    ...
    if (isNotHTVMfile == 0) {
        if (langToConvertTo != langFileExtension_2) {
            ...
            if (doseHaveInclude(code)) {
                ...
                for (let A_Index188 = 0; A_Index188 < 10000 + 0; A_Index188++) {
                    ...
                    for (let A_Index189 = 0; A_Index189 < items189.length + 0; A_Index189++) {
                        ...
                        if (SubStr(StrLower(Trim(currentLine)), 1, StrLen(keyWordInclude + " ")) == StrLower(keyWordInclude + " ")) {
                            ...
                            for (let A_Index190 = 0; A_Index190 < HTVM_Size(includedFilePaths) + 0; A_Index190++) {
    ...
    if (isNotHTVMfile == 0) {
        ...
        for (let A_Index199 = 0; A_Index199 < theIdNumOfThe34 + 0; A_Index199++) {
        ...
        if (langToConvertTo == "dart" || langToConvertTo == "groovy") {
            for (let A_Index200 = 0; A_Index200 < HTVM_Size(theIdNumOfThe34theVar) + 0; A_Index200++) {
        ...
        for (let A_Index201 = 0; A_Index201 < items201.length + 0; A_Index201++) {
            ...
            if (InStr(A_LoopField201, " " + keyWordComment) != false && SubStr(Trim(A_LoopField201), 1, StrLen(keyWordComment)) != keyWordComment) {
                ...
                for (let A_Index202 = 0; A_Index202 < HTVM_Size(slidingWinFixCommentsRm) + 0; A_Index202++) {
                    slidingWinFixCommentsRmI = A_Index202;
                    slidingWinFixCommentsRmHold = "";
                    for (let A_Index203 = 0; A_Index203 < StrLen(" " + keyWordComment) + 0; A_Index203++) {
        ...
        if (useCurlyBraces == "on") {
            ...
            if (keyWordCurlyBraceClose != "}") {
                ...
                for (let A_Index210 = 0; A_Index210 < items210.length + 0; A_Index210++) {
                    ...
                    let items211 = LoopParseFunc(A_LoopField210, " ");
        ...
        for (let A_Index217 = 0; A_Index217 < 2 + 0; A_Index217++) {
            outCodeFixBraces = "";
        ...
        for (let A_Index222 = 0; A_Index222 < items222.length + 0; A_Index222++) {
            ...
            if (SubStr(A_LoopField222, 1, StrLen(keyWordComment)) == keyWordComment) {
                ...
                int1 = 0;
                str17 = "";
                let items223 = LoopParseFunc(str16, " ");
                ...
                if (Trim(str1) == "") {
                    ...
                    if (Trim(str5) != "" && Trim(str10) != "") {
                        let items226 = LoopParseFunc(str5, "\n", "\r");
                        ...
                        let items227 = LoopParseFunc(str10, "\n", "\r");
                        ...
                        if (langToConvertTo == "lua") {
                            let items228 = LoopParseFunc(str12, ",");
                        ...
                        if (langToConvertTo == "java") {
                            ...
                            for (let A_Index232 = 0; A_Index232 < items232.length + 0; A_Index232++) {
                                ...
                                str20 = "";
                                let items233 = LoopParseFunc(str19, " ");
                            ...
                            let items234 = LoopParseFunc(str12, ",");
                    ...
                    if (Trim(str5) != "" && Trim(str10) == "") {
                        let items235 = LoopParseFunc(str5, "\n", "\r");
                    ...
                    if (Trim(str5) == "" && Trim(str10) != "") {
                        let items236 = LoopParseFunc(str10, "\n", "\r");
                        ...
                        if (langToConvertTo == "lua") {
                            let items237 = LoopParseFunc(str11, ",");
                        ...
                        if (langToConvertTo == "java") {
                            ...
                            for (let A_Index241 = 0; A_Index241 < items241.length + 0; A_Index241++) {
                                ...
                                str20 = "";
                                let items242 = LoopParseFunc(str19, " ");
                            ...
                            let items243 = LoopParseFunc(str11, ",");
        ...
        if (langToConvertTo != "ahk" && langToConvertTo != langFileExtension_2) {
            ...
            if (haveWeEverUsedAloop == 1) {
                ...
                let items251 = LoopParseFunc(htCodeLoopfixa, "\n", "\r");
    ...
    if (COUNT_programmingBlock_HTVMsyntax != 0) {
        ...
        for (let A_Index256 = 0; A_Index256 < COUNT_programmingBlock_HTVMsyntax + 0; A_Index256++) {
    ...
    if (langToConvertTo == "go") {
        ...
        for (let A_Index273 = 0; A_Index273 < items273.length + 0; A_Index273++) {
            ...
            if (Trim(A_LoopField273) == "}" && SubStr(Trim(fixGoManGoIsSoAnnoyingBroooFurure[A_Index273 + 1]), 1, StrLen("else if ")) == "else if ") {
                ...
                let items274 = LoopParseFunc(fixGoManGoIsSoAnnoyingBroooFurure[A_Index273 + 1]);
                ...
                for (let A_Index275 = 0; A_Index275 < fixGoManGoIsSoAnnoyingBroooCount + 0; A_Index275++) {
                    fixGoManGoIsSoAnnoyingBroooCountSpaceses += " ";
                }
    ...
    if (COUNT_programmingBlock_InTheTranspiledLang != 0) {
        for (let A_Index276 = 0; A_Index276 < COUNT_programmingBlock_InTheTranspiledLang + 0; A_Index276++) {
    ...
    if (langToConvertTo == "cpp") {
        if (COUNT_programmingBlock_CPP != 0) {
            for (let A_Index277 = 0; A_Index277 < COUNT_programmingBlock_CPP + 0; A_Index277++) {
    ...
    if (langToConvertTo == "py") {
        if (COUNT_programmingBlock_PY != 0) {
            for (let A_Index279 = 0; A_Index279 < COUNT_programmingBlock_PY + 0; A_Index279++) {
    ...
    if (langToConvertTo == "js") {
        if (COUNT_programmingBlock_JS != 0) {
            for (let A_Index281 = 0; A_Index281 < COUNT_programmingBlock_JS + 0; A_Index281++) {
    ...
    if (langToConvertTo == "go") {
        if (COUNT_programmingBlock_GO != 0) {
            for (let A_Index283 = 0; A_Index283 < COUNT_programmingBlock_GO + 0; A_Index283++) {
    ...
    if (langToConvertTo == "lua") {
        if (COUNT_programmingBlock_LUA != 0) {
            for (let A_Index285 = 0; A_Index285 < COUNT_programmingBlock_LUA + 0; A_Index285++) {
    ...
    if (langToConvertTo == "cs") {
        if (COUNT_programmingBlock_CS != 0) {
            for (let A_Index287 = 0; A_Index287 < COUNT_programmingBlock_CS + 0; A_Index287++) {
    ...
    if (langToConvertTo == "java") {
        if (COUNT_programmingBlock_JAVA != 0) {
            for (let A_Index289 = 0; A_Index289 < COUNT_programmingBlock_JAVA + 0; A_Index289++) {
    ...
    if (langToConvertTo == "kt") {
        if (COUNT_programmingBlock_KT != 0) {
            for (let A_Index291 = 0; A_Index291 < COUNT_programmingBlock_KT + 0; A_Index291++) {
    ...
    if (langToConvertTo == "rb") {
        if (COUNT_programmingBlock_RB != 0) {
            for (let A_Index293 = 0; A_Index293 < COUNT_programmingBlock_RB + 0; A_Index293++) {
    ...
    if (langToConvertTo == "nim") {
        if (COUNT_programmingBlock_NIM != 0) {
            for (let A_Index295 = 0; A_Index295 < COUNT_programmingBlock_NIM + 0; A_Index295++) {
    ...
    if (langToConvertTo == "ahk") {
        if (COUNT_programmingBlock_AHK != 0) {
            for (let A_Index297 = 0; A_Index297 < COUNT_programmingBlock_AHK + 0; A_Index297++) {
    ...
    if (langToConvertTo == "swift") {
        if (COUNT_programmingBlock_SWIFT != 0) {
            for (let A_Index299 = 0; A_Index299 < COUNT_programmingBlock_SWIFT + 0; A_Index299++) {
    ...
    if (langToConvertTo == "dart") {
        if (COUNT_programmingBlock_DART != 0) {
            for (let A_Index301 = 0; A_Index301 < COUNT_programmingBlock_DART + 0; A_Index301++) {
    ...
    if (langToConvertTo == "ts") {
        if (COUNT_programmingBlock_TS != 0) {
            for (let A_Index303 = 0; A_Index303 < COUNT_programmingBlock_TS + 0; A_Index303++) {
    ...
    if (langToConvertTo == "groovy") {
        if (COUNT_programmingBlock_GROOVY != 0) {
            for (let A_Index305 = 0; A_Index305 < COUNT_programmingBlock_GROOVY + 0; A_Index305++) {
    ...
    if (langToConvertTo == langFileExtension_2) {
        if (COUNT_programmingBlock_HTVM != 0) {
            for (let A_Index307 = 0; A_Index307 < COUNT_programmingBlock_HTVM + 0; A_Index307++) {
    ...
    if (langToConvertTo != langFileExtension_2) {
        ...
        ALoopField = "";
        str23 = "";
        let items309 = LoopParseFunc(htCode, "\n", "\r");
    ...
    var correctLang = 0;
    let items311 = LoopParseFunc(instructions, "\n", "\r");
    ...
    if (isNotHTVMfile2 == 0) {
        ...
        for (let A_Index312 = 0; A_Index312 < HTVM_Size(commentsBugFix) + 0; A_Index312++) {
    ...
    if (!(HTVM_Size(allFuncNames) <= 0)) {
        for (let A_Index313 = 0; A_Index313 < HTVM_Size(allFuncNames) + 0; A_Index313++) {
        ...
        let items314 = LoopParseFunc(allLibsToPutAtTop, "|");
    ...
    if (langToConvertTo != langFileExtension_2) {
        for (let A_Index315 = 0; A_Index315 < HTVM_Size(ospDic1) + 0; A_Index315++) {
    ...
    if (size_ospDic != 0) {
        for (let A_Index316 = 0; A_Index316 < size_ospDic + 0; A_Index316++) {
            HTVM_Pop(ospDic);
        }
    }
    //;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;
    ...
    if (size_ospDic1 != 0) {
        for (let A_Index317 = 0; A_Index317 < size_ospDic1 + 0; A_Index317++) {
            HTVM_Pop(ospDic1);
        }
    }
    //;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;
    ...
    if (size_ospDic2 != 0) {
        for (let A_Index318 = 0; A_Index318 < size_ospDic2 + 0; A_Index318++) {
            HTVM_Pop(ospDic2);
        }
    }
    //;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;
    ...
    if (size_htCodeFixRubyGlobalVars_EXPOSED != 0) {
        for (let A_Index319 = 0; A_Index319 < size_htCodeFixRubyGlobalVars_EXPOSED + 0; A_Index319++) {
    ...
    for (let A_Index320 = 0; A_Index320 < theIdNumOfThe34 + 0; A_Index320++) {
    ...
    if (isNotHTVMfile2 == 0) {
        ...
        if (langToConvertTo == langFileExtension_2) {
            for (let A_Index321 = 0; A_Index321 < HTVM_Size(weAreInMLSarr) + 0; A_Index321++) {
    ...
    if (size_weAreInMLSarr != 0) {
        for (let A_Index322 = 0; A_Index322 < size_weAreInMLSarr + 0; A_Index322++) {
...
function HTVMv2() {
    ...
    let items323 = LoopParseFunc(str0);
    for (let A_Index323 = 0; A_Index323 < items323.length + 0; A_Index323++) {
    ...
    if (HTVM_getLang_HTVM() == "cpp" || HTVM_getLang_HTVM() == "py") {
        ...
        let items324 = LoopParseFunc(allArgs, "\n", "\r");
        ...
        for (let A_Index325 = 0; A_Index325 < HTVM_Size(argHTVMinstrMORE) + 0; A_Index325++) {
