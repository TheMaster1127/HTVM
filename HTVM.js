...
function guiParserStep3(guiCode) {
    ...
    for (let A_Index138 = 0; A_Index138 < items138.length + 0; A_Index138++) {
        ...
        if (SubStr(Trim(StrLower(A_LoopField138)), 1, 4) == "gui " || RegExMatch(StrLower(Trim(A_LoopField138)), "^gui[-_][A-Za-z0-9_]+(?:[-_][A-Za-z0-9_]+)*(?=[, ]|$)")) {
            isElement = 0;
            whatElement = "";
            items139 = LoopParseFunc(A_LoopField138, " ")
            ...
            for (let A_Index140 = 0; A_Index140 < items140.length + 0; A_Index140++) {
                ...
                else if (StrSplit(temp2, ":", 1) == "value") {
                    found_value = 1;
                }
                else if (StrSplit(temp2, ":", 1) == "placeholder") {
                    found_placeholder = 1;
                }
                else if (StrSplit(temp2, ":", 1) == "callback") {
                    found_callback = 1;
                }
                else if (StrSplit(temp2, ":", 1) == "backgroundColor") {
                    found_backgroundColor = 1;
                }
                else if (StrSplit(temp2, ":", 1) == "color") {
                    found_color = 1;
                }
                else if (StrSplit(temp2, ":", 1) == "rounding") {
                    found_rounding = 1;
                }
                else if (StrSplit(temp2, ":", 1) == "border") {
                    found_border = 1;
                }
                else if (StrSplit(temp2, ":", 1) == "rawCss") {
                    found_rawCss = 1;
                }
                else if (StrSplit(temp2, ":", 1) == "size") {
                    found_size = 1;
                }
                else if (StrSplit(temp2, ":", 1) == "isEnabled") {
                    found_isEnabled = 1;
                }
                else if (StrSplit(temp2, ":", 1) == "isVisible") {
                    found_isVisible = 1;
                }
                else if (StrSplit(temp2, ":", 1) == "isDeleting") {
                    found_isDeleting = 1;
                }
            }
            ...
            isElement = 0;
            whatElement = "";
            items141 = LoopParseFunc(A_LoopField138, " ")
...
function guiParserStep2(guiCode) {
    ...
    var out = "";
    var temp = "";
    items142 = LoopParseFunc(guiCode, "\n", "\r")
...
function guiParser0(guiCode) {
    ...
    var begin = 0;
    let lookAhead = [];
    items143 = LoopParseFunc(guiCode, "\n", "\r")
    ...
    HTVM_Append(lookAhead, " ");
    items144 = LoopParseFunc(guiCode, "\n", "\r")
    for (let A_Index144 = 0; A_Index144 < items144.length + 0; A_Index144++) {
        ...
        //print("DEBIG123: " . A_LoopField144 . "|" . STR(begin))
    }
    out = StringTrimRight(out, 1);
...
function guiParser(guiCode) {
    ...
    items145 = LoopParseFunc(guiCode, "\n", "\r")
    ...
    var begin = 0;
    let lookAhead = [];
    items146 = LoopParseFunc(guiCode, "\n", "\r")
    ...
    HTVM_Append(lookAhead, " ");
    items147 = LoopParseFunc(guiCode, "\n", "\r")
    for (let A_Index147 = 0; A_Index147 < items147.length + 0; A_Index147++) {
        ...
        //print("DEBIG123: " . A_LoopField147 . "|" . STR(begin))
    }
...
function handleMultiLineStrings(code) {
    ...
    for (let A_Index148 = 0; A_Index148 < items148.length + 0; A_Index148++) {
        ...
        if (SubStrLastChars(Trim(A_LoopField148), 4) == " mls" || SubStrLastChars(Trim(A_LoopField148), 5) == " fmls") {
            didWeFindOrWeOut = 1;
        }
    }
    ...
    for (let A_Index149 = 0; A_Index149 < items149.length + 0; A_Index149++) {
        ...
        HTVM_Append(lookAhead, Trim(A_LoopField149));
    }
    ...
    for (let A_Index150 = 0; A_Index150 < items150.length + 0; A_Index150++) {
        ...
        if (langToConvertTo == langFileExtension_2) {
            ...
            if (weAreIn == 1) {
                ...
                weAreInMLSarrTemp += A_LoopField150;
            }
            if (SubStrLastChars(Trim(A_LoopField150), 4) == " mls" || SubStrLastChars(Trim(A_LoopField150), 5) == " fmls") {
                weAreIn = 1;
                out += A_LoopField150 + Chr(10);
            }
            if (weAreIn == 0) {
                out += A_LoopField150 + Chr(10);
            }
            ...
            if (weAreIn == 1 && Trim(A_LoopField150) == "HTVM-newLineAHw4y9w730q8237w4e8srhgesw0s8rgw04e5g08sgr0e8srdgt--YOU-BETERNOTUSETHISAH--aesdfssz") {
                if (lookAhead[A_Index150 + 1] == "HTVM-newLineAHw4y9w730q8237w4e8srhgesw0s8rgw04e5g08sgr0e8srdgt--YOU-BETERNOTUSETHISAH--aesdfssz") {
                    out += keyWordEscpaeChar + "n";
                }
            }
            if (weAreIn == 1 && Trim(A_LoopField150) != "HTVM-newLineAHw4y9w730q8237w4e8srhgesw0s8rgw04e5g08sgr0e8srdgt--YOU-BETERNOTUSETHISAH--aesdfssz") {
                out += A_LoopField150 + keyWordEscpaeChar + "n";
            }
            if (SubStrLastChars(Trim(A_LoopField150), 4) == " mls" || SubStrLastChars(Trim(A_LoopField150), 5) == " fmls") {
                weAreIn = 1;
                if (SubStrLastChars(Trim(A_LoopField150), 5) == " fmls") {
                    out += TrimR(StringTrimRight(TrimR(A_LoopField150), 3)) + Chr(34);
                } else {
                    out += TrimR(StringTrimRight(TrimR(A_LoopField150), 3)) + " " + Chr(34);
                }
            }
            if (weAreIn == 0) {
                if (Trim(A_LoopField150) != "mle") {
                    out += A_LoopField150 + Chr(10);
                }
            }
            //;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;
...
function slidingWinFixCommentsRmFUNC(line) {
    ...
    items151 = LoopParseFunc(line)
    for (let A_Index151 = 0; A_Index151 < items151.length + 0; A_Index151++) {
        const A_LoopField151 = items151[A_Index151 - 0];
        HTVM_Append(slidingWinFixCommentsRmFFFF, A_LoopField151);
    }
    for (let A_Index152 = 0; A_Index152 < StrLen(keyWordComment + keyWordComment) + 2 + 0; A_Index152++) {
...
function fixIfElseIfMultiLineStatementHELP(aloopfield) {
    var out = "";
    var count = 0;
    items153 = LoopParseFunc(aloopfield)
...
function fixIfElseIfMultiLineStatement(someHTVMcode) {
    var doWeExitInsta = 1;
    items154 = LoopParseFunc(someHTVMcode, "\n", "\r")
    for (let A_Index154 = 0; A_Index154 < items154.length + 0; A_Index154++) {
        const A_LoopField154 = items154[A_Index154 - 0];
        if (SubStr(StrLower(Trim(A_LoopField154)), 1, StrLen(keyWordIF)) == keyWordIF) {
            doWeExitInsta = 0;
        }
        else if (SubStr(StrLower(Trim(A_LoopField154)), 1, StrLen(keyWordWhileLoop)) == keyWordWhileLoop) {
            doWeExitInsta = 0;
        }
        else if (SubStr(StrLower(Trim(A_LoopField154)), 1, StrLen(keyWordElseIf)) == keyWordElseIf) {
            doWeExitInsta = 0;
        }
    }
    if (doWeExitInsta == 1) {
    ...
    for (let A_Index155 = 0; A_Index155 < 10000 + 0; A_Index155++) {
        out2 = out;
        out = "";
        doWeExit = false;
        items156 = LoopParseFunc(out2, "\n", "\r")
        for (let A_Index156 = 0; A_Index156 < items156.length + 0; A_Index156++) {
            const A_LoopField156 = items156[A_Index156 - 0];
            if (SubStr(StrLower(Trim(A_LoopField156)), 1, StrLen(keyWordIF)) == keyWordIF) {
                doWeJustExit = 1;
                if (fixIfElseIfMultiLineStatementHELP(A_LoopField156) == "") {
                    out += Trim(A_LoopField156) + " ";
                    doWeExit = false;
                    doWeExitHELP--;
                } else {
                    doWeExit = true;
                    out += Trim(A_LoopField156) + Chr(10);
                }
            }
            else if (SubStr(StrLower(Trim(A_LoopField156)), 1, StrLen(keyWordWhileLoop)) == keyWordWhileLoop) {
                doWeJustExit = 1;
                if (fixIfElseIfMultiLineStatementHELP(A_LoopField156) == "") {
                    out += Trim(A_LoopField156) + " ";
                    doWeExit = false;
            ...
            else if (SubStr(StrLower(Trim(A_LoopField156)), 1, StrLen(keyWordElseIf)) == keyWordElseIf) {
                doWeJustExit = 1;
                if (fixIfElseIfMultiLineStatementHELP(A_LoopField156) == "") {
                    out += Trim(A_LoopField156) + " ";
                    doWeExit = false;
...
function compiler(htCode, allInstructionFile, mode, langToConvertToParam = "") {
    ...
    items157 = LoopParseFunc(allInstructionFile, "\n", "\r")
    for (let A_Index157 = 0; A_Index157 < items157.length + 0; A_Index157++) {
        const A_LoopField157 = items157[A_Index157 - 0];
        if (A_Index157 == 0) {
            langToConvertTo = Trim(A_LoopField157);
        }
        if (A_Index157 == 1) {
            langFileExtension = Trim(A_LoopField157);
        }
        if (A_Index157 == 2) {
            commands = Trim(A_LoopField157);
        }
        if (A_Index157 == 3) {
            keyWordAlliance = Trim(A_LoopField157) + " ";
        }
        if (A_Index157 == 4) {
            keyWordCrew = Trim(A_LoopField157) + " ";
        }
        if (A_Index157 == 5) {
            keyWordProc = Trim(A_LoopField157) + " ";
        }
        if (A_Index157 == 6) {
            keyWordStruct = Trim(A_LoopField157) + " ";
        }
        if (A_Index157 == 7) {
            keyWordProp = Trim(A_LoopField157) + " ";
        }
        if (A_Index157 == 8) {
            keyWordThis = Trim(A_LoopField157);
        }
        if (A_Index157 == 9) {
            keyWordInclude = Trim(A_LoopField157);
        }
        if (A_Index157 == 10) {
            keyWordCodeInTheTranspiledLangStart = Trim(A_LoopField157);
        }
        if (A_Index157 == 11) {
            keyWordCodeInTheTranspiledLangEnd = Trim(A_LoopField157);
        }
        if (A_Index157 == 12) {
            keyWordCodeInTheTranspiledLangStartCPP = Trim(A_LoopField157);
        }
        if (A_Index157 == 13) {
            keyWordCodeInTheTranspiledLangEndCPP = Trim(A_LoopField157);
        }
        if (A_Index157 == 14) {
            keyWordCodeInTheTranspiledLangStartPY = Trim(A_LoopField157);
        }
        if (A_Index157 == 15) {
            keyWordCodeInTheTranspiledLangEndPY = Trim(A_LoopField157);
        }
        if (A_Index157 == 16) {
            keyWordCodeInTheTranspiledLangStartJS = Trim(A_LoopField157);
        }
        if (A_Index157 == 17) {
            keyWordCodeInTheTranspiledLangEndJS = Trim(A_LoopField157);
        }
        if (A_Index157 == 18) {
            keyWordCodeInTheTranspiledLangStartGO = Trim(A_LoopField157);
        }
        if (A_Index157 == 19) {
            keyWordCodeInTheTranspiledLangEndGO = Trim(A_LoopField157);
        }
        if (A_Index157 == 20) {
            keyWordCodeInTheTranspiledLangStartLUA = Trim(A_LoopField157);
        }
        if (A_Index157 == 21) {
            keyWordCodeInTheTranspiledLangEndLUA = Trim(A_LoopField157);
        }
        if (A_Index157 == 22) {
            keyWordCodeInTheTranspiledLangStartCS = Trim(A_LoopField157);
        }
        if (A_Index157 == 23) {
            keyWordCodeInTheTranspiledLangEndCS = Trim(A_LoopField157);
        }
        if (A_Index157 == 24) {
            keyWordCodeInTheTranspiledLangStartJAVA = Trim(A_LoopField157);
        }
        if (A_Index157 == 25) {
            keyWordCodeInTheTranspiledLangEndJAVA = Trim(A_LoopField157);
        }
        if (A_Index157 == 26) {
            keyWordCodeInTheTranspiledLangStartKT = Trim(A_LoopField157);
        }
        if (A_Index157 == 27) {
            keyWordCodeInTheTranspiledLangEndKT = Trim(A_LoopField157);
        }
        if (A_Index157 == 28) {
            keyWordCodeInTheTranspiledLangStartRB = Trim(A_LoopField157);
        }
        if (A_Index157 == 29) {
            keyWordCodeInTheTranspiledLangEndRB = Trim(A_LoopField157);
        }
        if (A_Index157 == 30) {
            keyWordCodeInTheTranspiledLangStartNIM = Trim(A_LoopField157);
        }
        if (A_Index157 == 31) {
            keyWordCodeInTheTranspiledLangEndNIM = Trim(A_LoopField157);
        }
        if (A_Index157 == 32) {
            keyWordCodeInTheTranspiledLangStartAHK = Trim(A_LoopField157);
        }
        if (A_Index157 == 33) {
            keyWordCodeInTheTranspiledLangEndAHK = Trim(A_LoopField157);
        }
        if (A_Index157 == 34) {
            keyWordCodeInTheTranspiledLangStartSWIFT = Trim(A_LoopField157);
        }
        if (A_Index157 == 35) {
            keyWordCodeInTheTranspiledLangEndSWIFT = Trim(A_LoopField157);
        }
        if (A_Index157 == 36) {
            keyWordCodeInTheTranspiledLangStartDART = Trim(A_LoopField157);
        }
        if (A_Index157 == 37) {
            keyWordCodeInTheTranspiledLangEndDART = Trim(A_LoopField157);
        }
        if (A_Index157 == 38) {
            keyWordCodeInTheTranspiledLangStartTS = Trim(A_LoopField157);
        }
        if (A_Index157 == 39) {
            keyWordCodeInTheTranspiledLangEndTS = Trim(A_LoopField157);
        }
        if (A_Index157 == 40) {
            keyWordCodeInTheTranspiledLangStartGROOVY = Trim(A_LoopField157);
        }
        if (A_Index157 == 41) {
            keyWordCodeInTheTranspiledLangEndGROOVY = Trim(A_LoopField157);
        }
        if (A_Index157 == 42) {
            keyWordCodeInTheTranspiledLangStartHTVM = Trim(A_LoopField157);
        }
        if (A_Index157 == 43) {
            keyWordCodeInTheTranspiledLangEndHTVM = Trim(A_LoopField157);
        }
        if (A_Index157 == 44) {
            keyWordCodeInHTVMstart = Trim(A_LoopField157);
        }
        if (A_Index157 == 45) {
            keyWordCodeInHTVMend = Trim(A_LoopField157);
        }
        if (A_Index157 == 46) {
            keyWordCurlyBraceOpen = Trim(A_LoopField157);
        }
        if (A_Index157 == 47) {
            keyWordCurlyBraceClose = Trim(A_LoopField157);
        }
        if (A_Index157 == 48) {
            keyWordNull = Trim(A_LoopField157);
        }
        if (A_Index157 == 49) {
            keyWordTrue = Trim(A_LoopField157);
        }
        if (A_Index157 == 50) {
            keyWordFalse = Trim(A_LoopField157);
        }
        if (A_Index157 == 51) {
            keyWordVoid = Trim(A_LoopField157);
        }
        if (A_Index157 == 52) {
            keyWordDouble = Trim(A_LoopField157);
        }
        if (A_Index157 == 53) {
            keyWordChar = Trim(A_LoopField157);
        }
        if (A_Index157 == 54) {
            keyWordUint8 = Trim(A_LoopField157);
        }
        if (A_Index157 == 55) {
            keyWordUint16 = Trim(A_LoopField157);
        }
        if (A_Index157 == 56) {
            keyWordUint32 = Trim(A_LoopField157);
        }
        if (A_Index157 == 57) {
            keyWordUint64 = Trim(A_LoopField157);
        }
        if (A_Index157 == 58) {
            keyWordINT = Trim(A_LoopField157);
        }
        if (A_Index157 == 59) {
            keyWordSTR = Trim(A_LoopField157);
        }
        if (A_Index157 == 60) {
            keyWordBOOL = Trim(A_LoopField157);
        }
        if (A_Index157 == 61) {
            keyWordFLOAT = Trim(A_LoopField157);
        }
        if (A_Index157 == 62) {
            keyWordINT8 = Trim(A_LoopField157);
        }
        if (A_Index157 == 63) {
            keyWordINT16 = Trim(A_LoopField157);
        }
        if (A_Index157 == 64) {
            keyWordINT32 = Trim(A_LoopField157);
        }
        if (A_Index157 == 65) {
            keyWordINT64 = Trim(A_LoopField157);
        }
        if (A_Index157 == 66) {
            keyWordIF = Trim(A_LoopField157) + " ";
        }
        if (A_Index157 == 67) {
            keyWordElseIf = Trim(A_LoopField157) + " ";
        }
        if (A_Index157 == 68) {
            keyWordElse = Trim(A_LoopField157);
        }
        if (A_Index157 == 69) {
            keyWordWhileLoop = Trim(A_LoopField157) + " ";
        }
        if (A_Index157 == 70) {
            keyWordLoopInfinite = Trim(A_LoopField157);
        }
        if (A_Index157 == 71) {
            keyWordLoop = Trim(A_LoopField157) + " ";
        }
        if (A_Index157 == 72) {
            keyWordLoopParse = Trim(A_LoopField157) + " ";
        }
        if (A_Index157 == 73) {
            keyWordContinue = Trim(A_LoopField157);
        }
        if (A_Index157 == 74) {
            keyWordBreak = Trim(A_LoopField157);
        }
        if (A_Index157 == 75) {
            keyWordFunc = Trim(A_LoopField157) + " ";
        }
        if (A_Index157 == 76) {
            keyWordAwait = Trim(A_LoopField157) + " ";
        }
        if (A_Index157 == 77) {
            keyWordAsync = Trim(A_LoopField157) + " ";
        }
        if (A_Index157 == 78) {
            keyWordThrow = Trim(A_LoopField157) + " ";
        }
        if (A_Index157 == 79) {
            keyWordErrorMsg = Trim(A_LoopField157);
        }
        if (A_Index157 == 80) {
            keyWordTry = Trim(A_LoopField157);
        }
        if (A_Index157 == 81) {
            keyWordCatch = Trim(A_LoopField157) + " ";
        }
        if (A_Index157 == 82) {
            keyWordFinally = Trim(A_LoopField157);
        }
        if (A_Index157 == 83) {
            keyWordReturnStatement = Trim(A_LoopField157) + " ";
        }
        if (A_Index157 == 84) {
            keyWordArrayAppend = Trim(A_LoopField157);
        }
        if (A_Index157 == 85) {
            keyWordArrayPop = Trim(A_LoopField157);
        }
        if (A_Index157 == 86) {
            keyWordArraySize = Trim(A_LoopField157);
        }
        if (A_Index157 == 87) {
            keyWordArrayInsert = Trim(A_LoopField157);
        }
        if (A_Index157 == 88) {
            keyWordArrayRemove = Trim(A_LoopField157);
        }
        if (A_Index157 == 89) {
            keyWordArrayIndexOf = Trim(A_LoopField157);
        }
        if (A_Index157 == 90) {
            keyWordArrayDefinition = Trim(A_LoopField157);
        }
        if (A_Index157 == 91) {
            keyWordArrayOfIntegersDefinition = Trim(A_LoopField157);
        }
        if (A_Index157 == 92) {
            keyWordArrayOfStringsDefinition = Trim(A_LoopField157);
        }
        if (A_Index157 == 93) {
            keyWordArrayOfFloatingPointNumbersDefinition = Trim(A_LoopField157);
        }
        if (A_Index157 == 94) {
            keyWordArrayOfBooleansDefinition = Trim(A_LoopField157);
        }
        if (A_Index157 == 95) {
            keyWordVar = Trim(A_LoopField157) + " ";
        }
        if (A_Index157 == 96) {
            keyWordLet = Trim(A_LoopField157) + " ";
        }
        if (A_Index157 == 97) {
            keyWordConst = Trim(A_LoopField157) + " ";
        }
        if (A_Index157 == 98) {
            keyWordEnd = Trim(A_LoopField157);
        }
        if (A_Index157 == 99) {
            keyWordGlobal = Trim(A_LoopField157) + " ";
        }
        if (A_Index157 == 100) {
            keyWordComment = Trim(A_LoopField157);
        }
        if (A_Index157 == 101) {
            keyWordCommentOpenMultiLine = Trim(A_LoopField157);
        }
        if (A_Index157 == 102) {
            keyWordCommentCloseMultiLine = Trim(A_LoopField157);
        }
        if (A_Index157 == 103) {
            keyWordEscpaeChar = Trim(A_LoopField157);
        }
        if (A_Index157 == 104) {
            keyWordMainLabel = Trim(A_LoopField157);
        }
        if (A_Index157 == 105) {
            keyWordConcat = Trim(A_LoopField157);
        }
        if (A_Index157 == 106) {
            keyWordAdd = Trim(A_LoopField157);
        }
        if (A_Index157 == 107) {
            keyWordSub = Trim(A_LoopField157);
        }
        if (A_Index157 == 108) {
            keyWordMul = Trim(A_LoopField157);
        }
        if (A_Index157 == 109) {
            keyWordDiv = Trim(A_LoopField157);
        }
        if (A_Index157 == 110) {
            keyWordMod = Trim(A_LoopField157);
        }
        if (A_Index157 == 111) {
            keyWordExp = Trim(A_LoopField157);
        }
        if (A_Index157 == 112) {
            keyWordEqual = Trim(A_LoopField157);
        }
        if (A_Index157 == 113) {
            keyWordStrictEqual = Trim(A_LoopField157);
        }
        if (A_Index157 == 114) {
            keyWordNotEqual = Trim(A_LoopField157);
        }
        if (A_Index157 == 115) {
            keyWordGreater = Trim(A_LoopField157);
        }
        if (A_Index157 == 116) {
            keyWordLess = Trim(A_LoopField157);
        }
        if (A_Index157 == 117) {
            keyWordGreaterEqual = Trim(A_LoopField157);
        }
        if (A_Index157 == 118) {
            keyWordLessEqual = Trim(A_LoopField157);
        }
        if (A_Index157 == 119) {
            keyWordAnd = Trim(A_LoopField157);
        }
        if (A_Index157 == 120) {
            keyWordOr = Trim(A_LoopField157);
        }
        if (A_Index157 == 121) {
            keyWordNot = Trim(A_LoopField157);
        }
        if (A_Index157 == 122) {
            keyWordBitAnd = Trim(A_LoopField157);
        }
        if (A_Index157 == 123) {
            keyWordBitOr = Trim(A_LoopField157);
        }
        if (A_Index157 == 124) {
            keyWordBitXor = Trim(A_LoopField157);
        }
        if (A_Index157 == 125) {
            keyWordBitNot = Trim(A_LoopField157);
        }
        if (A_Index157 == 126) {
            keyWordShiftLeft = Trim(A_LoopField157);
        }
        if (A_Index157 == 127) {
            keyWordShiftRight = Trim(A_LoopField157);
        }
        if (A_Index157 == 128) {
            keyWordShiftUnsignedRight = Trim(A_LoopField157);
        }
        if (A_Index157 == 129) {
            keyWordAssign = Trim(A_LoopField157);
        }
        if (A_Index157 == 130) {
            keyWordAssignAdd = Trim(A_LoopField157);
        }
        if (A_Index157 == 131) {
            keyWordAssignConcat = Trim(A_LoopField157);
        }
        if (A_Index157 == 132) {
            keyWordAssignSub = Trim(A_LoopField157);
        }
        if (A_Index157 == 133) {
            keyWordAssignMul = Trim(A_LoopField157);
        }
        if (A_Index157 == 134) {
            keyWordAssignDiv = Trim(A_LoopField157);
        }
        if (A_Index157 == 135) {
            keyWordAssignMod = Trim(A_LoopField157);
        }
        if (A_Index157 == 136) {
            keyWordAssignShiftLeft = Trim(A_LoopField157);
        }
        if (A_Index157 == 137) {
            keyWordAssignShiftRight = Trim(A_LoopField157);
        }
        if (A_Index157 == 138) {
            keyWordLogicalAssignShiftRight = Trim(A_LoopField157);
        }
        if (A_Index157 == 139) {
            keyWordAssignBitAnd = Trim(A_LoopField157);
        }
        if (A_Index157 == 140) {
            keyWordAssignBitOr = Trim(A_LoopField157);
        }
        if (A_Index157 == 141) {
            keyWordAssignBitXor = Trim(A_LoopField157);
        }
        if (A_Index157 == 142) {
            keyWordTernary1 = Trim(A_LoopField157);
        }
        if (A_Index157 == 143) {
            keyWordTernary2 = Trim(A_LoopField157);
        }
        if (A_Index157 == 144) {
            keyWordInc = Trim(A_LoopField157);
        }
        if (A_Index157 == 145) {
            keyWordDec = Trim(A_LoopField157);
        }
        if (A_Index157 == 146) {
            AHKlikeLoopsIndexedAt = Trim(A_LoopField157);
        }
        if (A_Index157 == 147) {
            keyWordAIndex = Trim(A_LoopField157);
        }
        if (A_Index157 == 148) {
            keyWordALoopField = Trim(A_LoopField157);
        }
        if (A_Index157 == 149) {
            useCurlyBraces = Trim(A_LoopField157);
        }
        if (A_Index157 == 150) {
            useEnd = Trim(A_LoopField157);
        }
        if (A_Index157 == 151) {
            useSemicolon = Trim(A_LoopField157);
        }
        if (A_Index157 == 152) {
            useParentheses = Trim(A_LoopField157);
        }
        if (A_Index157 == 153) {
            usePrefixTypeForTypeDefinition = Trim(A_LoopField157);
        }
        if (A_Index157 == 154) {
            usePostfixTypeForTypeDefinition = Trim(A_LoopField157);
        }
        if (A_Index157 == 155) {
            usePythonicColonSyntax = Trim(A_LoopField157);
        }
        if (A_Index157 == 156) {
            useCurlyBracesSyntaxForArrayDef = Trim(A_LoopField157);
        }
        if (A_Index157 == 157) {
            useInJavaScriptAlwaysUseVar = Trim(A_LoopField157);
        }
        if (A_Index157 == 158) {
            useJavaScriptInAfullHTMLfile = Trim(A_LoopField157);
        }
        if (A_Index157 == 159) {
            useJavaScriptAmainFuncDef = Trim(A_LoopField157);
        }
        if (A_Index157 == 160) {
            useJavaScriptAllFuncsAreAsync = Trim(A_LoopField157);
        }
        if (A_Index157 == 161) {
            useJavaScriptAlwaysTripleEqual = Trim(A_LoopField157);
        }
    }
    if (HTVM_Size(argHTVMinstrMORE) > 0) {
        ...
        items158 = LoopParseFunc(allInstructionFile_2, "\n", "\r")
        for (let A_Index158 = 0; A_Index158 < items158.length + 0; A_Index158++) {
            const A_LoopField158 = items158[A_Index158 - 0];
            if (A_Index158 == 0) {
                langToConvertTo_2 = Trim(A_LoopField158);
            }
            if (A_Index158 == 1) {
                langFileExtension_2 = Trim(A_LoopField158);
            }
            if (A_Index158 == 2) {
                commands_2 = Trim(A_LoopField158);
            }
            if (A_Index158 == 3) {
                keyWordAlliance_2 = Trim(A_LoopField158) + " ";
            }
            if (A_Index158 == 4) {
                keyWordCrew_2 = Trim(A_LoopField158) + " ";
            }
            if (A_Index158 == 5) {
                keyWordProc_2 = Trim(A_LoopField158) + " ";
            }
            if (A_Index158 == 6) {
                keyWordStruct_2 = Trim(A_LoopField158) + " ";
            }
            if (A_Index158 == 7) {
                keyWordProp_2 = Trim(A_LoopField158) + " ";
            }
            if (A_Index158 == 8) {
                keyWordThis_2 = Trim(A_LoopField158);
            }
            if (A_Index158 == 9) {
                keyWordInclude_2 = Trim(A_LoopField158);
            }
            if (A_Index158 == 10) {
                keyWordCodeInTheTranspiledLangStart_2 = Trim(A_LoopField158);
            }
            if (A_Index158 == 11) {
                keyWordCodeInTheTranspiledLangEnd_2 = Trim(A_LoopField158);
            }
            if (A_Index158 == 12) {
                keyWordCodeInTheTranspiledLangStartCPP_2 = Trim(A_LoopField158);
            }
            if (A_Index158 == 13) {
                keyWordCodeInTheTranspiledLangEndCPP_2 = Trim(A_LoopField158);
            }
            if (A_Index158 == 14) {
                keyWordCodeInTheTranspiledLangStartPY_2 = Trim(A_LoopField158);
            }
            if (A_Index158 == 15) {
                keyWordCodeInTheTranspiledLangEndPY_2 = Trim(A_LoopField158);
            }
            if (A_Index158 == 16) {
                keyWordCodeInTheTranspiledLangStartJS_2 = Trim(A_LoopField158);
            }
            if (A_Index158 == 17) {
                keyWordCodeInTheTranspiledLangEndJS_2 = Trim(A_LoopField158);
            }
            if (A_Index158 == 18) {
                keyWordCodeInTheTranspiledLangStartGO_2 = Trim(A_LoopField158);
            }
            if (A_Index158 == 19) {
                keyWordCodeInTheTranspiledLangEndGO_2 = Trim(A_LoopField158);
            }
            if (A_Index158 == 20) {
                keyWordCodeInTheTranspiledLangStartLUA_2 = Trim(A_LoopField158);
            }
            if (A_Index158 == 21) {
                keyWordCodeInTheTranspiledLangEndLUA_2 = Trim(A_LoopField158);
            }
            if (A_Index158 == 22) {
                keyWordCodeInTheTranspiledLangStartCS_2 = Trim(A_LoopField158);
            }
            if (A_Index158 == 23) {
                keyWordCodeInTheTranspiledLangEndCS_2 = Trim(A_LoopField158);
            }
            if (A_Index158 == 24) {
                keyWordCodeInTheTranspiledLangStartJAVA_2 = Trim(A_LoopField158);
            }
            if (A_Index158 == 25) {
                keyWordCodeInTheTranspiledLangEndJAVA_2 = Trim(A_LoopField158);
            }
            if (A_Index158 == 26) {
                keyWordCodeInTheTranspiledLangStartKT_2 = Trim(A_LoopField158);
            }
            if (A_Index158 == 27) {
                keyWordCodeInTheTranspiledLangEndKT_2 = Trim(A_LoopField158);
            }
            if (A_Index158 == 28) {
                keyWordCodeInTheTranspiledLangStartRB_2 = Trim(A_LoopField158);
            }
            if (A_Index158 == 29) {
                keyWordCodeInTheTranspiledLangEndRB_2 = Trim(A_LoopField158);
            }
            if (A_Index158 == 30) {
                keyWordCodeInTheTranspiledLangStartNIM_2 = Trim(A_LoopField158);
            }
            if (A_Index158 == 31) {
                keyWordCodeInTheTranspiledLangEndNIM_2 = Trim(A_LoopField158);
            }
            if (A_Index158 == 32) {
                keyWordCodeInTheTranspiledLangStartAHK_2 = Trim(A_LoopField158);
            }
            if (A_Index158 == 33) {
                keyWordCodeInTheTranspiledLangEndAHK_2 = Trim(A_LoopField158);
            }
            if (A_Index158 == 34) {
                keyWordCodeInTheTranspiledLangStartSWIFT_2 = Trim(A_LoopField158);
            }
            if (A_Index158 == 35) {
                keyWordCodeInTheTranspiledLangEndSWIFT_2 = Trim(A_LoopField158);
            }
            if (A_Index158 == 36) {
                keyWordCodeInTheTranspiledLangStartDART_2 = Trim(A_LoopField158);
            }
            if (A_Index158 == 37) {
                keyWordCodeInTheTranspiledLangEndDART_2 = Trim(A_LoopField158);
            }
            if (A_Index158 == 38) {
                keyWordCodeInTheTranspiledLangStartTS_2 = Trim(A_LoopField158);
            }
            if (A_Index158 == 39) {
                keyWordCodeInTheTranspiledLangEndTS_2 = Trim(A_LoopField158);
            }
            if (A_Index158 == 40) {
                keyWordCodeInTheTranspiledLangStartGROOVY_2 = Trim(A_LoopField158);
            }
            if (A_Index158 == 41) {
                keyWordCodeInTheTranspiledLangEndGROOVY_2 = Trim(A_LoopField158);
            }
            if (A_Index158 == 42) {
                keyWordCodeInTheTranspiledLangStartHTVM_2 = Trim(A_LoopField158);
            }
            if (A_Index158 == 43) {
                keyWordCodeInTheTranspiledLangEndHTVM_2 = Trim(A_LoopField158);
            }
            if (A_Index158 == 44) {
                keyWordCodeInHTVMstart_2 = Trim(A_LoopField158);
            }
            if (A_Index158 == 45) {
                keyWordCodeInHTVMend_2 = Trim(A_LoopField158);
            }
            if (A_Index158 == 46) {
                keyWordCurlyBraceOpen_2 = Trim(A_LoopField158);
            }
            if (A_Index158 == 47) {
                keyWordCurlyBraceClose_2 = Trim(A_LoopField158);
            }
            if (A_Index158 == 48) {
                keyWordNull_2 = Trim(A_LoopField158);
            }
            if (A_Index158 == 49) {
                keyWordTrue_2 = Trim(A_LoopField158);
            }
            if (A_Index158 == 50) {
                keyWordFalse_2 = Trim(A_LoopField158);
            }
            if (A_Index158 == 51) {
                keyWordVoid_2 = Trim(A_LoopField158);
            }
            if (A_Index158 == 52) {
                keyWordDouble_2 = Trim(A_LoopField158);
            }
            if (A_Index158 == 53) {
                keyWordChar_2 = Trim(A_LoopField158);
            }
            if (A_Index158 == 54) {
                keyWordUint8_2 = Trim(A_LoopField158);
            }
            if (A_Index158 == 55) {
                keyWordUint16_2 = Trim(A_LoopField158);
            }
            if (A_Index158 == 56) {
                keyWordUint32_2 = Trim(A_LoopField158);
            }
            if (A_Index158 == 57) {
                keyWordUint64_2 = Trim(A_LoopField158);
            }
            if (A_Index158 == 58) {
                keyWordINT_2 = Trim(A_LoopField158);
            }
            if (A_Index158 == 59) {
                keyWordSTR_2 = Trim(A_LoopField158);
            }
            if (A_Index158 == 60) {
                keyWordBOOL_2 = Trim(A_LoopField158);
            }
            if (A_Index158 == 61) {
                keyWordFLOAT_2 = Trim(A_LoopField158);
            }
            if (A_Index158 == 62) {
                keyWordINT8_2 = Trim(A_LoopField158);
            }
            if (A_Index158 == 63) {
                keyWordINT16_2 = Trim(A_LoopField158);
            }
            if (A_Index158 == 64) {
                keyWordINT32_2 = Trim(A_LoopField158);
            }
            if (A_Index158 == 65) {
                keyWordINT64_2 = Trim(A_LoopField158);
            }
            if (A_Index158 == 66) {
                keyWordIF_2 = Trim(A_LoopField158) + " ";
            }
            if (A_Index158 == 67) {
                keyWordElseIf_2 = Trim(A_LoopField158) + " ";
            }
            if (A_Index158 == 68) {
                keyWordElse_2 = Trim(A_LoopField158);
            }
            if (A_Index158 == 69) {
                keyWordWhileLoop_2 = Trim(A_LoopField158) + " ";
            }
            if (A_Index158 == 70) {
                keyWordLoopInfinite_2 = Trim(A_LoopField158);
            }
            if (A_Index158 == 71) {
                keyWordLoop_2 = Trim(A_LoopField158) + " ";
            }
            if (A_Index158 == 72) {
                keyWordLoopParse_2 = Trim(A_LoopField158) + " ";
            }
            if (A_Index158 == 73) {
                keyWordContinue_2 = Trim(A_LoopField158);
            }
            if (A_Index158 == 74) {
                keyWordBreak_2 = Trim(A_LoopField158);
            }
            if (A_Index158 == 75) {
                keyWordFunc_2 = Trim(A_LoopField158) + " ";
            }
            if (A_Index158 == 76) {
                keyWordAwait_2 = Trim(A_LoopField158) + " ";
            }
            if (A_Index158 == 77) {
                keyWordAsync_2 = Trim(A_LoopField158) + " ";
            }
            if (A_Index158 == 78) {
                keyWordThrow_2 = Trim(A_LoopField158) + " ";
            }
            if (A_Index158 == 79) {
                keyWordErrorMsg_2 = Trim(A_LoopField158);
            }
            if (A_Index158 == 80) {
                keyWordTry_2 = Trim(A_LoopField158);
            }
            if (A_Index158 == 81) {
                keyWordCatch_2 = Trim(A_LoopField158) + " ";
            }
            if (A_Index158 == 82) {
                keyWordFinally_2 = Trim(A_LoopField158);
            }
            if (A_Index158 == 83) {
                keyWordReturnStatement_2 = Trim(A_LoopField158) + " ";
            }
            if (A_Index158 == 84) {
                keyWordArrayAppend_2 = Trim(A_LoopField158);
            }
            if (A_Index158 == 85) {
                keyWordArrayPop_2 = Trim(A_LoopField158);
            }
            if (A_Index158 == 86) {
                keyWordArraySize_2 = Trim(A_LoopField158);
            }
            if (A_Index158 == 87) {
                keyWordArrayInsert_2 = Trim(A_LoopField158);
            }
            if (A_Index158 == 88) {
                keyWordArrayRemove_2 = Trim(A_LoopField158);
            }
            if (A_Index158 == 89) {
                keyWordArrayIndexOf_2 = Trim(A_LoopField158);
            }
            if (A_Index158 == 90) {
                keyWordArrayDefinition_2 = Trim(A_LoopField158);
            }
            if (A_Index158 == 91) {
                keyWordArrayOfIntegersDefinition_2 = Trim(A_LoopField158);
            }
            if (A_Index158 == 92) {
                keyWordArrayOfStringsDefinition_2 = Trim(A_LoopField158);
            }
            if (A_Index158 == 93) {
                keyWordArrayOfFloatingPointNumbersDefinition_2 = Trim(A_LoopField158);
            }
            if (A_Index158 == 94) {
                keyWordArrayOfBooleansDefinition_2 = Trim(A_LoopField158);
            }
            if (A_Index158 == 95) {
                keyWordVar_2 = Trim(A_LoopField158);
            }
            if (A_Index158 == 96) {
                keyWordLet_2 = Trim(A_LoopField158);
            }
            if (A_Index158 == 97) {
                keyWordConst_2 = Trim(A_LoopField158);
            }
            if (A_Index158 == 98) {
                keyWordEnd_2 = Trim(A_LoopField158);
            }
            if (A_Index158 == 99) {
                keyWordGlobal_2 = Trim(A_LoopField158) + " ";
            }
            if (A_Index158 == 100) {
                keyWordComment_2 = Trim(A_LoopField158);
            }
            if (A_Index158 == 101) {
                keyWordCommentOpenMultiLine_2 = Trim(A_LoopField158);
            }
            if (A_Index158 == 102) {
                keyWordCommentCloseMultiLine_2 = Trim(A_LoopField158);
            }
            if (A_Index158 == 103) {
                keyWordEscpaeChar_2 = Trim(A_LoopField158);
            }
            if (A_Index158 == 104) {
                keyWordMainLabel_2 = Trim(A_LoopField158);
            }
            if (A_Index158 == 105) {
                keyWordConcat_2 = Trim(A_LoopField158);
            }
            if (A_Index158 == 106) {
                keyWordAdd_2 = Trim(A_LoopField158);
            }
            if (A_Index158 == 107) {
                keyWordSub_2 = Trim(A_LoopField158);
            }
            if (A_Index158 == 108) {
                keyWordMul_2 = Trim(A_LoopField158);
            }
            if (A_Index158 == 109) {
                keyWordDiv_2 = Trim(A_LoopField158);
            }
            if (A_Index158 == 110) {
                keyWordMod_2 = Trim(A_LoopField158);
            }
            if (A_Index158 == 111) {
                keyWordExp_2 = Trim(A_LoopField158);
            }
            if (A_Index158 == 112) {
                keyWordEqual_2 = Trim(A_LoopField158);
            }
            if (A_Index158 == 113) {
                keyWordStrictEqual_2 = Trim(A_LoopField158);
            }
            if (A_Index158 == 114) {
                keyWordNotEqual_2 = Trim(A_LoopField158);
            }
            if (A_Index158 == 115) {
                keyWordGreater_2 = Trim(A_LoopField158);
            }
            if (A_Index158 == 116) {
                keyWordLess_2 = Trim(A_LoopField158);
            }
            if (A_Index158 == 117) {
                keyWordGreaterEqual_2 = Trim(A_LoopField158);
            }
            if (A_Index158 == 118) {
                keyWordLessEqual_2 = Trim(A_LoopField158);
            }
            if (A_Index158 == 119) {
                keyWordAnd_2 = Trim(A_LoopField158);
            }
            if (A_Index158 == 120) {
                keyWordOr_2 = Trim(A_LoopField158);
            }
            if (A_Index158 == 121) {
                keyWordNot_2 = Trim(A_LoopField158);
            }
            if (A_Index158 == 122) {
                keyWordBitAnd_2 = Trim(A_LoopField158);
            }
            if (A_Index158 == 123) {
                keyWordBitOr_2 = Trim(A_LoopField158);
            }
            if (A_Index158 == 124) {
                keyWordBitXor_2 = Trim(A_LoopField158);
            }
            if (A_Index158 == 125) {
                keyWordBitNot_2 = Trim(A_LoopField158);
            }
            if (A_Index158 == 126) {
                keyWordShiftLeft_2 = Trim(A_LoopField158);
            }
            if (A_Index158 == 127) {
                keyWordShiftRight_2 = Trim(A_LoopField158);
            }
            if (A_Index158 == 128) {
                keyWordShiftUnsignedRight_2 = Trim(A_LoopField158);
            }
            if (A_Index158 == 129) {
                keyWordAssign_2 = Trim(A_LoopField158);
            }
            if (A_Index158 == 130) {
                keyWordAssignAdd_2 = Trim(A_LoopField158);
            }
            if (A_Index158 == 131) {
                keyWordAssignConcat_2 = Trim(A_LoopField158);
            }
            if (A_Index158 == 132) {
                keyWordAssignSub_2 = Trim(A_LoopField158);
            }
            if (A_Index158 == 133) {
                keyWordAssignMul_2 = Trim(A_LoopField158);
            }
            if (A_Index158 == 134) {
                keyWordAssignDiv_2 = Trim(A_LoopField158);
            }
            if (A_Index158 == 135) {
                keyWordAssignMod_2 = Trim(A_LoopField158);
            }
            if (A_Index158 == 136) {
                keyWordAssignShiftLeft_2 = Trim(A_LoopField158);
            }
            if (A_Index158 == 137) {
                keyWordAssignShiftRight_2 = Trim(A_LoopField158);
            }
            if (A_Index158 == 138) {
                keyWordLogicalAssignShiftRight_2 = Trim(A_LoopField158);
            }
            if (A_Index158 == 139) {
                keyWordAssignBitAnd_2 = Trim(A_LoopField158);
            }
            if (A_Index158 == 140) {
                keyWordAssignBitOr_2 = Trim(A_LoopField158);
            }
            if (A_Index158 == 141) {
                keyWordAssignBitXor_2 = Trim(A_LoopField158);
            }
            if (A_Index158 == 142) {
                keyWordTernary1_2 = Trim(A_LoopField158);
            }
            if (A_Index158 == 143) {
                keyWordTernary2_2 = Trim(A_LoopField158);
            }
            if (A_Index158 == 144) {
                keyWordInc_2 = Trim(A_LoopField158);
            }
            if (A_Index158 == 145) {
                keyWordDec_2 = Trim(A_LoopField158);
            }
            if (A_Index158 == 146) {
                AHKlikeLoopsIndexedAt_2 = Trim(A_LoopField158);
            }
            if (A_Index158 == 147) {
                keyWordAIndex_2 = Trim(A_LoopField158);
            }
            if (A_Index158 == 148) {
                keyWordALoopField_2 = Trim(A_LoopField158);
            }
            if (A_Index158 == 149) {
                useCurlyBraces_2 = Trim(A_LoopField158);
            }
            if (A_Index158 == 150) {
                useEnd_2 = Trim(A_LoopField158);
            }
            if (A_Index158 == 151) {
                useSemicolon_2 = Trim(A_LoopField158);
            }
            if (A_Index158 == 152) {
                useParentheses_2 = Trim(A_LoopField158);
            }
            if (A_Index158 == 153) {
                usePrefixTypeForTypeDefinition_2 = Trim(A_LoopField158);
            }
            if (A_Index158 == 154) {
                usePostfixTypeForTypeDefinition_2 = Trim(A_LoopField158);
            }
            if (A_Index158 == 155) {
                usePythonicColonSyntax_2 = Trim(A_LoopField158);
            }
            if (A_Index158 == 156) {
                useCurlyBracesSyntaxForArrayDef_2 = Trim(A_LoopField158);
            }
            if (A_Index158 == 157) {
                useInJavaScriptAlwaysUseVar_2 = Trim(A_LoopField158);
            }
            if (A_Index158 == 158) {
                useJavaScriptInAfullHTMLfile_2 = Trim(A_LoopField158);
            }
            if (A_Index158 == 159) {
                useJavaScriptAmainFuncDef_2 = Trim(A_LoopField158);
            }
            if (A_Index158 == 160) {
                useJavaScriptAllFuncsAreAsync_2 = Trim(A_LoopField158);
            }
            if (A_Index158 == 161) {
                useJavaScriptAlwaysTripleEqual_2 = Trim(A_LoopField158);
            }
        }
    }
    ...
    */
    ...
    items159 = LoopParseFunc(allInstructionFile, "\n", "\r")
    for (let A_Index159 = 0; A_Index159 < items159.length + 0; A_Index159++) {
        const A_LoopField159 = items159[A_Index159 - 0];
        if (RegExMatch(Trim(A_LoopField159), "^name: [A-Za-z0-9_]+")) {
            availableFuncsInHTVMInHTVM += Trim(StrSplit(Trim(A_LoopField159), "name: ", 2)) + Chr(10);
        }
    }
    ...
    for (let A_Index160 = 0; A_Index160 < items160.length + 0; A_Index160++) {
        ...
        if (Trim(StrLower(A_LoopField160)) == StrLower(keyWordCodeInTheTranspiledLangEnd)) {
        ...
        else if (Trim(StrLower(A_LoopField160)) == StrLower(keyWordCodeInTheTranspiledLangEndCPP)) {
        ...
        else if (Trim(StrLower(A_LoopField160)) == StrLower(keyWordCodeInTheTranspiledLangEndPY)) {
        ...
        else if (Trim(StrLower(A_LoopField160)) == StrLower(keyWordCodeInTheTranspiledLangEndJS)) {
        ...
        else if (Trim(StrLower(A_LoopField160)) == StrLower(keyWordCodeInTheTranspiledLangEndGO)) {
        ...
        else if (Trim(StrLower(A_LoopField160)) == StrLower(keyWordCodeInTheTranspiledLangEndLUA)) {
        ...
        else if (Trim(StrLower(A_LoopField160)) == StrLower(keyWordCodeInTheTranspiledLangEndCS)) {
        ...
        else if (Trim(StrLower(A_LoopField160)) == StrLower(keyWordCodeInTheTranspiledLangEndJAVA)) {
        ...
        else if (Trim(StrLower(A_LoopField160)) == StrLower(keyWordCodeInTheTranspiledLangEndKT)) {
        ...
        else if (Trim(StrLower(A_LoopField160)) == StrLower(keyWordCodeInTheTranspiledLangEndRB)) {
        ...
        else if (Trim(StrLower(A_LoopField160)) == StrLower(keyWordCodeInTheTranspiledLangEndNIM)) {
        ...
        else if (Trim(StrLower(A_LoopField160)) == StrLower(keyWordCodeInTheTranspiledLangEndAHK)) {
        ...
        else if (Trim(StrLower(A_LoopField160)) == StrLower(keyWordCodeInTheTranspiledLangEndSWIFT)) {
        ...
        else if (Trim(StrLower(A_LoopField160)) == StrLower(keyWordCodeInTheTranspiledLangEndDART)) {
        ...
        else if (Trim(StrLower(A_LoopField160)) == StrLower(keyWordCodeInTheTranspiledLangEndTS)) {
        ...
        else if (Trim(StrLower(A_LoopField160)) == StrLower(keyWordCodeInTheTranspiledLangEndGROOVY)) {
        ...
        else if (Trim(StrLower(A_LoopField160)) == StrLower(keyWordCodeInTheTranspiledLangEndHTVM)) {
        ...
        else if (Trim(StrLower(A_LoopField160)) == StrLower(keyWordCodeInHTVMend)) {
            //;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;
            //;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;
            ...
            holdTempDataProgrammingBlockThenPutInArr += A_LoopField160 + Chr(10);
        }
        else if (Trim(StrLower(A_LoopField160)) == StrLower(keyWordCodeInTheTranspiledLangStart)) {
            holdTempDataProgrammingBlockThenPutInArr = "";
            inProgarmmingBlock = 1;
        }
        else if (Trim(StrLower(A_LoopField160)) == StrLower(keyWordCodeInTheTranspiledLangStartCPP)) {
            holdTempDataProgrammingBlockThenPutInArr = "";
            inProgarmmingBlock = 1;
        }
        else if (Trim(StrLower(A_LoopField160)) == StrLower(keyWordCodeInTheTranspiledLangStartPY)) {
            holdTempDataProgrammingBlockThenPutInArr = "";
            inProgarmmingBlock = 1;
        }
        else if (Trim(StrLower(A_LoopField160)) == StrLower(keyWordCodeInTheTranspiledLangStartJS)) {
            holdTempDataProgrammingBlockThenPutInArr = "";
            inProgarmmingBlock = 1;
        }
        else if (Trim(StrLower(A_LoopField160)) == StrLower(keyWordCodeInTheTranspiledLangStartGO)) {
            holdTempDataProgrammingBlockThenPutInArr = "";
            inProgarmmingBlock = 1;
        }
        else if (Trim(StrLower(A_LoopField160)) == StrLower(keyWordCodeInTheTranspiledLangStartLUA)) {
            holdTempDataProgrammingBlockThenPutInArr = "";
            inProgarmmingBlock = 1;
        }
        else if (Trim(StrLower(A_LoopField160)) == StrLower(keyWordCodeInTheTranspiledLangStartCS)) {
            holdTempDataProgrammingBlockThenPutInArr = "";
            inProgarmmingBlock = 1;
        }
        else if (Trim(StrLower(A_LoopField160)) == StrLower(keyWordCodeInTheTranspiledLangStartJAVA)) {
            holdTempDataProgrammingBlockThenPutInArr = "";
            inProgarmmingBlock = 1;
        }
        else if (Trim(StrLower(A_LoopField160)) == StrLower(keyWordCodeInTheTranspiledLangStartKT)) {
            holdTempDataProgrammingBlockThenPutInArr = "";
            inProgarmmingBlock = 1;
        }
        else if (Trim(StrLower(A_LoopField160)) == StrLower(keyWordCodeInTheTranspiledLangStartRB)) {
            holdTempDataProgrammingBlockThenPutInArr = "";
            inProgarmmingBlock = 1;
        }
        else if (Trim(StrLower(A_LoopField160)) == StrLower(keyWordCodeInTheTranspiledLangStartNIM)) {
            holdTempDataProgrammingBlockThenPutInArr = "";
            inProgarmmingBlock = 1;
        }
        else if (Trim(StrLower(A_LoopField160)) == StrLower(keyWordCodeInTheTranspiledLangStartAHK)) {
            holdTempDataProgrammingBlockThenPutInArr = "";
            inProgarmmingBlock = 1;
        }
        else if (Trim(StrLower(A_LoopField160)) == StrLower(keyWordCodeInTheTranspiledLangStartSWIFT)) {
            holdTempDataProgrammingBlockThenPutInArr = "";
            inProgarmmingBlock = 1;
        }
        else if (Trim(StrLower(A_LoopField160)) == StrLower(keyWordCodeInTheTranspiledLangStartDART)) {
            holdTempDataProgrammingBlockThenPutInArr = "";
            inProgarmmingBlock = 1;
        }
        else if (Trim(StrLower(A_LoopField160)) == StrLower(keyWordCodeInTheTranspiledLangStartTS)) {
            holdTempDataProgrammingBlockThenPutInArr = "";
            inProgarmmingBlock = 1;
        }
        else if (Trim(StrLower(A_LoopField160)) == StrLower(keyWordCodeInTheTranspiledLangStartGROOVY)) {
            holdTempDataProgrammingBlockThenPutInArr = "";
            inProgarmmingBlock = 1;
        }
        else if (Trim(StrLower(A_LoopField160)) == StrLower(keyWordCodeInTheTranspiledLangStartHTVM)) {
            holdTempDataProgrammingBlockThenPutInArr = "";
            inProgarmmingBlock = 1;
        }
        else if (Trim(StrLower(A_LoopField160)) == StrLower(keyWordCodeInHTVMstart)) {
            //;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;
            //;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;
            holdTempDataProgrammingBlockThenPutInArr = "";
            inProgarmmingBlock = 1;
            ...
            programmingBlocksTemp += A_LoopField160 + Chr(10);
        }
    }
    ...
    if (isNotHTVMfile == 0) {
        if (langToConvertTo != langFileExtension_2) {
            if (InStr(StrLower(code), StrLower(keyWordInclude + " "))) {
                for (let A_Index161 = 0; A_Index161 < 10000 + 0; A_Index161++) {
                    doseCodeHaveLibs = 0;
                    allLibCODE = "";
                    if (A_Index161 == 0) {
                        items162 = LoopParseFunc(code, "\n", "\r")
                        for (let A_Index162 = 0; A_Index162 < items162.length + 0; A_Index162++) {
                            ...
                            if (SubStr(StrLower(Trim(A_LoopField162)), 1, StrLen(StrLower(keyWordInclude + " "))) == StrLower(keyWordInclude + " ")) {
                                HTVM_Append(allLibsWeSee, StrReplace(StringTrimLeft(Trim(A_LoopField162), StrLen(StrLower(keyWordInclude) + " ")), Chr(34), ""));
                                HTVM_Append(allLibsWeSeeSEE, StrReplace(StringTrimLeft(Trim(A_LoopField162), StrLen(StrLower(keyWordInclude) + " ")), Chr(34), ""));
                                doseCodeHaveLibs = 1;
                            }
                        }
                    } else {
                        items163 = LoopParseFunc(code, "\n", "\r")
                        for (let A_Index163 = 0; A_Index163 < items163.length + 0; A_Index163++) {
                            const A_LoopField163 = items163[A_Index163 - 0];
                            ALoopField = A_LoopField163;
                            if (SubStr(StrLower(Trim(A_LoopField163)), 1, StrLen(StrLower(keyWordInclude + " "))) == StrLower(keyWordInclude + " ")) {
                                ...
                                for (let A_Index164 = 0; A_Index164 < HTVM_Size(allLibsWeSee) + 0; A_Index164++) {
                                    if (StrReplace(StringTrimLeft(Trim(ALoopField), StrLen(StrLower(keyWordInclude) + " ")), Chr(34), "") == allLibsWeSee[A_Index164]) {
                                        doseCodeHaveLibs_HAS_SEEN = 1;
                                    }
                                }
                                if (doseCodeHaveLibs_HAS_SEEN == 0) {
                                    HTVM_Append(allLibsWeSee, StrReplace(StringTrimLeft(Trim(A_LoopField163), StrLen(StrLower(keyWordInclude) + " ")), Chr(34), ""));
                                    HTVM_Append(allLibsWeSeeSEE, StrReplace(StringTrimLeft(Trim(A_LoopField163), StrLen(StrLower(keyWordInclude) + " ")), Chr(34), ""));
                                    doseCodeHaveLibs = 1;
                                }
                            }
                        }
                    }
                    for (let A_Index165 = 0; A_Index165 < HTVM_Size(allLibsWeSee) + 0; A_Index165++) {
                        allLibsWeSeeHOLD = allLibsWeSee[A_Index165];
                        doseCodeHaveLibs_HAS_SEEN2 = 0;
                        for (let A_Index166 = 0; A_Index166 < HTVM_Size(allLibsWeSeeSEE) - HTVM_Size(allLibsWeSee) + 0; A_Index166++) {
                            if (allLibsWeSeeHOLD == allLibsWeSeeSEE[A_Index166]) {
                                doseCodeHaveLibs_HAS_SEEN2 = 1;
                            }
                        }
                        if (doseCodeHaveLibs_HAS_SEEN2 == 0) {
                            allLibCODE += FileRead(Trim(allLibsWeSee[HTVM_Size(allLibsWeSee) - A_Index165 - 1])) + Chr(10);
                        }
                        HTVM_Pop(allLibsWeSee);
                    }
                    allLibCODEfix = "";
                    items167 = LoopParseFunc(code, "\n", "\r")
                    for (let A_Index167 = 0; A_Index167 < items167.length + 0; A_Index167++) {
                        ...
                        if (SubStr(StrLower(Trim(A_LoopField167)), 1, StrLen(StrLower(keyWordInclude + " "))) != StrLower(keyWordInclude + " ")) {
                            allLibCODEfix += A_LoopField167 + Chr(10);
                        }
                    }
    ...
    if (isNotHTVMfile == 0) {
        ...
        items169 = LoopParseFunc(code)
        for (let A_Index169 = 0; A_Index169 < items169.length + 0; A_Index169++) {
            const A_LoopField169 = items169[A_Index169 - 0];
            theIdNumOfThe34theVar[A_Index169] = theIdNumOfThe34theVar[A_Index169] + Chr(34);
            HTVM_Append(getAllCharForTheFurtureSoIcanAddEscapeChar, A_LoopField169);
        }
        ...
        items170 = LoopParseFunc(code)
        for (let A_Index170 = 0; A_Index170 < items170.length + 0; A_Index170++) {
            const A_LoopField170 = items170[A_Index170 - 0];
            if (A_LoopField170 == keyWordEscpaeChar && getAllCharForTheFurtureSoIcanAddEscapeChar[A_Index170 + 1] == Chr(34)) {
                ...
                if (fixOutFixDoubleQuotesInsideDoubleQuotesFIXok != 1) {
                    OutFixDoubleQuotesInsideDoubleQuotes += A_LoopField170;
                } else {
        ...
        if (keyWordEscpaeChar == Chr(92) && langToConvertTo != langFileExtension_2) {
            items171 = LoopParseFunc(code)
            for (let A_Index171 = 0; A_Index171 < items171.length + 0; A_Index171++) {
                const A_LoopField171 = items171[A_Index171 - 0];
                if (A_LoopField171 == Chr(34)) {
                    areWEinSome34sNum++;
                }
                if (areWEinSome34sNum == 1) {
                    if (A_LoopField171 != Chr(34)) {
                        if (A_LoopField171 == keyWordEscpaeChar && langToConvertTo != langFileExtension_2) {
                            theIdNumOfThe34theVar[theIdNumOfThe34] = theIdNumOfThe34theVar[theIdNumOfThe34] + Chr(92);
                        } else {
                            theIdNumOfThe34theVar[theIdNumOfThe34] = theIdNumOfThe34theVar[theIdNumOfThe34] + A_LoopField171;
                        }
                    } else {
                        theIdNumOfThe34++;
                ...
                if (areWEinSome34sNum == 2 || areWEinSome34sNum == 0) {
                    if (A_LoopField171 != Chr(34)) {
                        htCodeOUT754754 += A_LoopField171;
                    }
                    areWEinSome34sNum = 0;
                }
            }
        } else {
            items172 = LoopParseFunc(code)
            for (let A_Index172 = 0; A_Index172 < items172.length + 0; A_Index172++) {
                const A_LoopField172 = items172[A_Index172 - 0];
                if (A_LoopField172 == Chr(34)) {
                    areWEinSome34sNum++;
                }
                if (areWEinSome34sNum == 1) {
                    if (A_LoopField172 != Chr(34)) {
                        if (A_LoopField172 == keyWordEscpaeChar && keyWordEscpaeChar == getAllCharForTheFurtureSoIcanAddEscapeChar[A_Index172 + 1] && langToConvertTo != langFileExtension_2) {
                        ...
                        else if (A_LoopField172 == keyWordEscpaeChar && langToConvertTo != langFileExtension_2) {
                            ...
                            theIdNumOfThe34theVar[theIdNumOfThe34] = theIdNumOfThe34theVar[theIdNumOfThe34] + A_LoopField172;
                        }
                    } else {
                        theIdNumOfThe34++;
                        htCodeOUT754754 += "ihuiuuhuuhtheidFor--asds" + str21 + "as--" + str21 + "theuhturtyphoutr--" + Chr(65) + Chr(65) + STR(theIdNumOfThe34) + Chr(65) + Chr(65);
                ...
                if (areWEinSome34sNum == 2 || areWEinSome34sNum == 0) {
                    if (A_LoopField172 != Chr(34)) {
                        htCodeOUT754754 += A_LoopField172;
                    }
                    areWEinSome34sNum = 0;
                }
            }
        ...
        for (let A_Index173 = 0; A_Index173 < theIdNumOfThe34 + 0; A_Index173++) {
            theIdNumOfThe34theVar[A_Index173] = theIdNumOfThe34theVar[A_Index173] + Chr(34);
        }
        HTVM_Append(theIdNumOfThe34theVar, Chr(34));
        code = StrReplace(code, Chr(13), "");
        slidingWinFixCommentsRmI = 0;
        newStringOutCode = "";
        items174 = LoopParseFunc(code, "\n", "\r")
        for (let A_Index174 = 0; A_Index174 < items174.length + 0; A_Index174++) {
            const A_LoopField174 = items174[A_Index174 - 0];
            if (InStr(A_LoopField174, " " + keyWordComment) != false && SubStr(Trim(A_LoopField174), 1, StrLen(keyWordComment)) != keyWordComment) {
                posForRemoveCommentsOnTheEndOfTheLine = 0;
                slidingWinFixCommentsRmI = 0;
                slidingWinFixCommentsRm = slidingWinFixCommentsRmFUNC(A_LoopField174);
                for (let A_Index175 = 0; A_Index175 < HTVM_Size(slidingWinFixCommentsRm) + 0; A_Index175++) {
                    slidingWinFixCommentsRmI = A_Index175;
                    slidingWinFixCommentsRmHold = "";
                    for (let A_Index176 = 0; A_Index176 < StrLen(" " + keyWordComment) + 0; A_Index176++) {
                        slidingWinFixCommentsRmHold += slidingWinFixCommentsRm[slidingWinFixCommentsRmI + A_Index176];
                    }
                    if (Trim(slidingWinFixCommentsRmHold) == keyWordComment) {
                        posForRemoveCommentsOnTheEndOfTheLine = A_Index175;
                        break;
                    }
                }
                if (posForRemoveCommentsOnTheEndOfTheLine > 0) {
                    newStringOutCode += SubStr(A_LoopField174, 1, posForRemoveCommentsOnTheEndOfTheLine) + Chr(10);
                } else {
                    newStringOutCode += A_LoopField174 + Chr(10);
                }
            } else {
                newStringOutCode += A_LoopField174 + Chr(10);
            }
        ...
        if (langToConvertTo != langFileExtension_2) {
            ...
            items177 = LoopParseFunc(code, "\n", "\r")
            for (let A_Index177 = 0; A_Index177 < items177.length + 0; A_Index177++) {
                const A_LoopField177 = items177[A_Index177 - 0];
                if (SubStr(Trim(A_LoopField177), 1, 1) != ";") {
                    semiColonForOneLinerOut += StrReplace(A_LoopField177, ";", Chr(10)) + Chr(10);
                } else {
                    semiColonForOneLinerOut += A_LoopField177 + Chr(10);
                }
            }
        ...
        if (useCurlyBraces == "on") {
            ...
            str1 = "";
            items178 = LoopParseFunc(code, "\n", "\r")
            for (let A_Index178 = 0; A_Index178 < items178.length + 0; A_Index178++) {
                const A_LoopField178 = items178[A_Index178 - 0];
                str1 = "";
                if (InStr(A_LoopField178, keyWordCurlyBraceOpen) || InStr(A_LoopField178, keyWordCurlyBraceClose)) {
                    if (SubStr(Trim(A_LoopField178), 1, StrLen(Trim(keyWordComment))) == Trim(keyWordComment) && isLineAconstruct(Trim(A_LoopField178)) == false) {
                        fixSomeCurlyBraceForExratFlexability += A_LoopField178 + Chr(10);
                    } else {
                        if (InStr(A_LoopField178, Trim(keyWordArrayDefinition) + " " + keyWordAssign + " ") || InStr(A_LoopField178, Trim(keyWordArrayOfIntegersDefinition) + " " + keyWordAssign + " ") || InStr(A_LoopField178, Trim(keyWordArrayOfStringsDefinition) + " " + keyWordAssign + " ") || InStr(A_LoopField178, Trim(keyWordArrayOfFloatingPointNumbersDefinition) + " " + keyWordAssign + " ") || InStr(A_LoopField178, Trim(keyWordArrayOfBooleansDefinition) + " " + keyWordAssign + " ") && useCurlyBracesSyntaxForArrayDef == "on") {
                            fixSomeCurlyBraceForExratFlexability += A_LoopField178 + Chr(10);
                        } else {
                            str1 = StrReplace(A_LoopField178, keyWordCurlyBraceOpen, "{");
            ...
            if (keyWordCurlyBraceClose != "}") {
                items183 = LoopParseFunc(code, "\n", "\r")
                for (let A_Index183 = 0; A_Index183 < items183.length + 0; A_Index183++) {
                    const A_LoopField183 = items183[A_Index183 - 0];
                    htCodeOutFixINT2 = 0;
                    if (isLineAconstruct(Trim(A_LoopField183)) == true || Trim(A_LoopField183) == keyWordCurlyBraceClose) {
                        htCodeOutFixINT2 = 1;
                    }
                    items184 = LoopParseFunc(A_LoopField183, " ")
                    for (let A_Index184 = 0; A_Index184 < items184.length + 0; A_Index184++) {
                        const A_LoopField184 = items184[A_Index184 - 0];
                        if (htCodeOutFixINT2 == 1 || Trim(A_LoopField184) == keyWordCurlyBraceClose) {
                            htCodeOutFixINT2 = 0;
                            htCodeOutFixEnd += HTVMmatchStrRrplace(A_LoopField184, keyWordCurlyBraceClose, "}") + " ";
                        } else {
                            htCodeOutFixEnd += A_LoopField184 + " ";
                        }
                    }
                    htCodeOutFixEnd = StringTrimRight(htCodeOutFixEnd, 1);
                    htCodeOutFixEnd += Chr(10);
        ...
        if (useCurlyBraces == "on") {
            ...
            int5 = 0;
            items185 = LoopParseFunc(code, "\n", "\r")
            for (let A_Index185 = 0; A_Index185 < items185.length + 0; A_Index185++) {
                const A_LoopField185 = items185[A_Index185 - 0];
                HTVM_Append(addNoCurlyFixWhenNextLineAfterConstructLookAhead, Trim(A_LoopField185));
            }
            HTVM_Append(addNoCurlyFixWhenNextLineAfterConstructLookAhead, " ");
            //;;;
            addNoCurlyFixWhenNextLineAfterConstructOut = "";
            items186 = LoopParseFunc(code, "\n", "\r")
            for (let A_Index186 = 0; A_Index186 < items186.length + 0; A_Index186++) {
                const A_LoopField186 = items186[A_Index186 - 0];
                if (isLineAconstruct(Trim(A_LoopField186)) && addNoCurlyFixWhenNextLineAfterConstructLookAhead[A_Index186 + 1] != "{") {
                    int5 = 1;
                    addNoCurlyFixWhenNextLineAfterConstructOut += A_LoopField186 + Chr(10) + "{" + Chr(10);
                } else {
                    if (int5 == 1) {
                        int5 = 0;
                        addNoCurlyFixWhenNextLineAfterConstructOut += A_LoopField186 + Chr(10) + "}" + Chr(10);
                    } else {
                        addNoCurlyFixWhenNextLineAfterConstructOut += A_LoopField186 + Chr(10);
                    }
                }
            }
        ...
        htCodeOutFixEnd = "";
        if (useEnd == "on") {
            items187 = LoopParseFunc(code, "\n", "\r")
            for (let A_Index187 = 0; A_Index187 < items187.length + 0; A_Index187++) {
                const A_LoopField187 = items187[A_Index187 - 0];
                if (StrLower(Trim(A_LoopField187)) == StrLower(keyWordEnd)) {
                    htCodeOutFixEnd += "}\n";
                }
                else if (SubStr(StrLower(Trim(A_LoopField187)), 1, StrLen(StrLower(keyWordIF))) == StrLower(keyWordIF) && isLineAconstruct(Trim(A_LoopField187)) == true) {
                    htCodeOutFixEnd += A_LoopField187 + "\n{\n";
                }
                else if (SubStr(StrLower(Trim(A_LoopField187)), 1, StrLen(StrLower(keyWordWhileLoop))) == StrLower(keyWordWhileLoop) && isLineAconstruct(Trim(A_LoopField187)) == true) {
                    htCodeOutFixEnd += A_LoopField187 + "\n{\n";
                }
                else if (SubStr(StrLower(Trim(A_LoopField187)), 1, StrLen(StrLower(keyWordElseIf))) == StrLower(keyWordElseIf) && isLineAconstruct(Trim(A_LoopField187)) == true) {
                    htCodeOutFixEnd += "\n}\n" + A_LoopField187 + "\n{\n";
                }
                else if (SubStr(StrLower(Trim(A_LoopField187)), 1, StrLen(StrLower(keyWordElse))) == StrLower(keyWordElse) && isLineAconstruct(Trim(A_LoopField187)) == true) {
                    htCodeOutFixEnd += "\n}\n" + A_LoopField187 + "\n{\n";
                }
                else if (SubStr(StrLower(Trim(A_LoopField187)), 1, StrLen(StrLower(keyWordCatch))) == StrLower(keyWordCatch) && isLineAconstruct(Trim(A_LoopField187)) == true) {
                    htCodeOutFixEnd += "\n}\n" + A_LoopField187 + "\n{\n";
                }
                else if (SubStr(StrLower(Trim(A_LoopField187)), 1, StrLen(StrLower(keyWordFinally))) == StrLower(keyWordFinally) && isLineAconstruct(Trim(A_LoopField187)) == true) {
                    htCodeOutFixEnd += "\n}\n" + A_LoopField187 + "\n{\n";
                }
                else if (isLineAconstruct(Trim(A_LoopField187)) == true) {
                    htCodeOutFixEnd += A_LoopField187 + "\n{\n";
                } else {
                    htCodeOutFixEnd += A_LoopField187 + Chr(10);
                }
            }
            code = StringTrimRight(htCodeOutFixEnd, 1);
        }
        ...
        for (let A_Index190 = 0; A_Index190 < 2 + 0; A_Index190++) {
            ...
            for (let A_Index191 = 0; A_Index191 < items191.length + 0; A_Index191++) {
                ...
                if (InStr(A_LoopField191, Trim(keyWordArrayDefinition) + " " + keyWordAssign + " ") || InStr(A_LoopField191, Trim(keyWordArrayOfIntegersDefinition) + " " + keyWordAssign + " ") || InStr(A_LoopField191, Trim(keyWordArrayOfStringsDefinition) + " " + keyWordAssign + " ") || InStr(A_LoopField191, Trim(keyWordArrayOfFloatingPointNumbersDefinition) + " " + keyWordAssign + " ") || InStr(A_LoopField191, Trim(keyWordArrayOfBooleansDefinition) + " " + keyWordAssign + " ") && useCurlyBracesSyntaxForArrayDef == "on") {
                    if (SubStr(StrLower(A_LoopField191), 1, StrLen(StrLower(keyWordFunc))) == StrLower(keyWordFunc) || SubStr(StrLower(A_LoopField191), 1, StrLen(StrLower(keyWordAsync) + StrLower(keyWordFunc))) == StrLower(keyWordAsync) + StrLower(keyWordFunc)) {
                        if (InStr(Trim(A_LoopField191), "{") && Trim(A_LoopField191) != "{" && SubStr(Trim(A_LoopField191), 1, StrLen(Trim(keyWordComment))) != Trim(keyWordComment)) {
                            outCodeFixBraces += Trim(StrReplace(Trim(A_LoopField191), "{", "")) + "\n{\n";
                        }
                        else if (InStr(Trim(A_LoopField191), "}") && Trim(A_LoopField191) != "}" && SubStr(Trim(A_LoopField191), 1, StrLen(Trim(keyWordComment))) != Trim(keyWordComment)) {
                            outCodeFixBraces += "}\n" + Trim(StrReplace(Trim(A_LoopField191), "}", "")) + Chr(10);
                        } else {
                            outCodeFixBraces += Trim(A_LoopField191) + Chr(10);
                        }
                    } else {
                        outCodeFixBraces += Trim(A_LoopField191) + Chr(10);
                    }
        ...
        if (usePythonicColonSyntax == "on" || useCurlyBraces == "off" && useEnd == "off") {
            items192 = LoopParseFunc(code, "\n", "\r")
            for (let A_Index192 = 0; A_Index192 < items192.length + 0; A_Index192++) {
                const A_LoopField192 = items192[A_Index192 - 0];
                if (isLineAconstruct(Trim(A_LoopField192)) == true) {
                    htCodeOutFixEnd += StringTrimRight(A_LoopField192, 1) + Chr(10);
                } else {
        ...
        if (isNotHTVMfile2 == 0) {
            ...
            for (let A_Index193 = 0; A_Index193 < items193.length + 0; A_Index193++) {
                ...
                if (StrLower(Trim(A_LoopField193)) == StrLower(keyWordMainLabel)) {
                    ...
                    codeOutFixAndAddMainFunc += A_LoopField193 + Chr(10);
                }
            }
        ...
        //print(code)
        //print("=======DOWN==========")
        items195 = LoopParseFunc(code, "\n", "\r")
        for (let A_Index195 = 0; A_Index195 < items195.length + 0; A_Index195++) {
            const A_LoopField195 = items195[A_Index195 - 0];
            lineDone = 0;
            if (InStr(A_LoopField195, "guiInit(") || InStr(A_LoopField195, "guiAdd(") || InStr(A_LoopField195, "guiAddElement(") || InStr(A_LoopField195, "guiControl(")) {
                lineDone = 1;
                htCode += Trim(A_LoopField195) + Chr(10);
            }
            if (SubStr(A_LoopField195, 1, StrLen(keyWordComment)) == keyWordComment) {
                lineDone = 1;
                str1 = StringTrimLeft(A_LoopField195, StrLen(keyWordComment));
                if (langToConvertTo == langFileExtension_2) {
            ...
            else if (Trim(A_LoopField195) == keyWordCommentOpenMultiLine) {
                lineDone = 1;
                if (langToConvertTo == langFileExtension_2) {
            ...
            else if (Trim(A_LoopField195) == keyWordCommentCloseMultiLine) {
                lineDone = 1;
                if (langToConvertTo == langFileExtension_2) {
            ...
            else if (InStr(A_LoopField195, "[][][][][] ")) {
                lineDone = 1;
                str1 = StrSplit(A_LoopField195, " ", 2);
            ...
            else if (SubStr(StrLower(Trim(A_LoopField195)), 1, StrLen(StrLower(keyWordStruct))) == StrLower(keyWordStruct)) {
                lineDone = 1;
                str0 = Trim(StringTrimLeft(A_LoopField195, StrLen(keyWordStruct)));
                if (langToConvertTo == langFileExtension_2) {
                    htCode += Trim(keyWordStruct_2) + " " + str0 + Chr(10);
                }
            }
            else if (SubStr(StrLower(Trim(A_LoopField195)), 1, StrLen(StrLower(keyWordAlliance))) == StrLower(keyWordAlliance)) {
                lineDone = 1;
                str0 = Trim(StringTrimLeft(A_LoopField195, StrLen(keyWordAlliance)));
                if (langToConvertTo == langFileExtension_2) {
                    htCode += Trim(keyWordAlliance_2) + " " + str0 + Chr(10);
                }
            }
            else if (SubStr(StrLower(Trim(A_LoopField195)), 1, StrLen(StrLower(keyWordCrew))) == StrLower(keyWordCrew)) {
                lineDone = 1;
                str0 = Trim(StringTrimLeft(A_LoopField195, StrLen(keyWordCrew)));
                if (langToConvertTo == langFileExtension_2) {
                    htCode += Trim(keyWordCrew_2) + " " + str0 + Chr(10);
                }
            }
            else if (InStr(A_LoopField195, "[end][end][end][end][end] ")) {
                lineDone = 1;
                if (langToConvertTo == langFileExtension_2) {
                    htCode += "}" + Chr(10);
                }
            }
            else if (SubStr(StrLower(Trim(A_LoopField195)), 1, StrLen(StrLower(keyWordProp))) == StrLower(keyWordProp)) {
                lineDone = 1;
                str0 = Trim(StringTrimLeft(A_LoopField195, StrLen(keyWordProp)));
            ...
            else if (SubStr(StrLower(Trim(A_LoopField195)), 1, StrLen(StrLower(keyWordProc))) == StrLower(keyWordProc)) {
                lineDone = 1;
                str0 = Trim(StringTrimLeft(A_LoopField195, StrLen(keyWordProc)));
                str1 = Trim(StrSplit(str0, "-----------------------HTVM-OSP-HTVM-OBJ--------------", 1));
                str20 = Trim(StrSplit(str0, "-----------------------HTVM-OSP-HTVM-OBJ--------------", 2));
            ...
            else if (SubStr(StrLower(Trim(A_LoopField195)), 1, StrLen(StrLower(keyWordIF))) == StrLower(keyWordIF) && SubStr(StrLower(Trim(A_LoopField195)), 1, StrLen(StrLower(keyWordElseIf))) != StrLower(keyWordElseIf)) {
                lineDone = 1;
                str1 = Trim(StringTrimLeft(A_LoopField195, StrLen(keyWordIF)));
            ...
            else if (SubStr(StrLower(Trim(A_LoopField195)), 1, StrLen(StrLower(keyWordElseIf))) == StrLower(keyWordElseIf) && SubStr(StrLower(Trim(A_LoopField195)), 1, StrLen(StrLower(keyWordIF))) != StrLower(keyWordIF)) {
                lineDone = 1;
                str1 = Trim(StringTrimLeft(A_LoopField195, StrLen(keyWordElseIf)));
                str1 = Trim(expressionParserTranspiler("(" + str1 + ")"));
                str1 = StringTrimRight(str1, 1);
                str1 = StringTrimLeft(str1, 1);
                if (langToConvertTo == langFileExtension_2) {
            ...
            else if (StrLower(Trim(A_LoopField195)) == StrLower(keyWordElse) || StrLower(Trim(A_LoopField195)) == StrLower(keyWordElse + ":")) {
                lineDone = 1;
                if (langToConvertTo == langFileExtension_2) {
                    if (usePythonicColonSyntax_2 == "on") {
            ...
            else if (SubStr(StrLower(Trim(A_LoopField195)), 1, StrLen(StrLower(keyWordWhileLoop))) == StrLower(keyWordWhileLoop)) {
                lineDone = 1;
                str1 = Trim(StringTrimLeft(A_LoopField195, StrLen(keyWordWhileLoop)));
                str1 = Trim(expressionParserTranspiler("(" + str1 + ")"));
                str1 = StringTrimRight(str1, 1);
                str1 = StringTrimLeft(str1, 1);
                if (langToConvertTo == langFileExtension_2) {
            ...
            else if (StrLower(Trim(A_LoopField195)) == StrLower(keyWordTry) || StrLower(Trim(A_LoopField195)) == StrLower(keyWordTry + ":")) {
                lineDone = 1;
                if (langToConvertTo == langFileExtension_2) {
                    if (usePythonicColonSyntax_2 == "on") {
                        htCode += keyWordTry_2 + ":\n";
            ...
            else if (SubStr(StrLower(Trim(A_LoopField195)), 1, StrLen(StrLower(keyWordCatch))) == StrLower(keyWordCatch)) {
                lineDone = 1;
                str1 = Trim(StrReplace(StrReplace(StringTrimLeft(A_LoopField195, StrLen(keyWordCatch)), "(", ""), ")", ""));
            ...
            else if (StrLower(Trim(A_LoopField195)) == StrLower(keyWordFinally) || StrLower(Trim(A_LoopField195)) == StrLower(keyWordFinally + ":")) {
                lineDone = 1;
                if (langToConvertTo == langFileExtension_2) {
                    if (usePythonicColonSyntax_2 == "on") {
            ...
            else if (SubStr(StrLower(Trim(A_LoopField195)), 1, StrLen(StrLower(keyWordThrow))) == StrLower(keyWordThrow)) {
                lineDone = 1;
                str1 = Trim(A_LoopField195);
                if (SubStrLastChars(str1, 1) == ";") {
                    str1 = StringTrimRight(str1, 1);
                }
            ...
            else if (StrLower(A_LoopField195) == StrLower(keyWordBreak) || StrLower(A_LoopField195) == StrLower(keyWordBreak + ";")) {
                lineDone = 1;
                if (langToConvertTo == langFileExtension_2) {
                    if (useSemicolon_2 == "on") {
            ...
            else if (StrLower(A_LoopField195) == StrLower(Trim(keyWordReturnStatement)) || StrLower(A_LoopField195) == StrLower(Trim(keyWordReturnStatement) + ";")) {
                lineDone = 1;
                if (langToConvertTo == langFileExtension_2) {
                    if (useSemicolon_2 == "on") {
            ...
            else if (SubStr(Trim(StrLower(A_LoopField195)), 1, StrLen(StrLower(keyWordReturnStatement))) == StrLower(keyWordReturnStatement)) {
                lineDone = 1;
                str1 = Trim(expressionParserTranspiler(Trim(StringTrimLeft(A_LoopField195, StrLen(keyWordReturnStatement)))));
                if (SubStrLastChars(str1, 1) == ";") {
                    str1 = StringTrimRight(str1, 1);
                }
                if (langToConvertTo == langFileExtension_2) {
            ...
            else if (StrLower(A_LoopField195) == StrLower(keyWordContinue) || StrLower(A_LoopField195) == StrLower(keyWordContinue + ";")) {
                lineDone = 1;
                if (langToConvertTo == langFileExtension_2) {
                    if (useSemicolon_2 == "on") {
            ...
            else if (StrLower(A_LoopField195) == "::" + keyWordContinue || StrLower(A_LoopField195) == "::" + keyWordContinue + ";") {
                lineDone = 1;
                if (langToConvertTo == langFileExtension_2) {
                    htCode += "::" + keyWordContinue_2 + Chr(10);
                }
            ...
            else if (StrLower(A_LoopField195) == StrLower(keyWordLoopInfinite) || StrLower(A_LoopField195) == StrLower(keyWordLoopInfinite + ":")) {
                // infinity loops
            ...
            else if (SubStr(Trim(StrLower(A_LoopField195)), 1, StrLen(StrLower(keyWordLoop))) == StrLower(keyWordLoop) && SubStr(Trim(StrLower(A_LoopField195)), 1, StrLen(StrLower(keyWordLoop)) + 2) != StrLower(keyWordLoop) + "% " && SubStr(Trim(StrLower(A_LoopField195)), 1, StrLen(StrLower(keyWordLoopParse))) != StrLower(keyWordLoopParse)) {
                out2 = StringTrimLeft(A_LoopField195, StrLen(keyWordLoop));
            ...
            else if (SubStr(Trim(StrLower(A_LoopField195)), 1, StrLen(StrLower(keyWordLoop)) + 2) == StrLower(keyWordLoop) + "% ") {
                howMany_fixAindexInGoUnused++;
                out2 = StringTrimLeft(A_LoopField195, StrLen(keyWordLoop) + 2);
                out2 = StrReplace(out2, ":", "");
                //MsgBox % out2
                //MsgBox, % out2
                out2 = Trim(out2);
            ...
            else if (SubStr(StrLower(A_LoopField195), 1, StrLen(StrLower(keyWordLoopParse))) == StrLower(keyWordLoopParse)) {
                howMany_fixAindexInGoUnused++;
                var1 = StringTrimLeft(A_LoopField195, StrLen(keyWordLoopParse));
                out2 = StrReplace(out2, ":", "");
                lineDone = 1;
            ...
            else if (SubStrLastChars(Trim(A_LoopField195), StrLen(keyWordInc)) == keyWordInc || SubStrLastChars(Trim(A_LoopField195), StrLen(keyWordInc + ";")) == keyWordInc + ";" && lineDone == 0) {
                if (useSemicolon == "on" || SubStrLastChars(Trim(A_LoopField195), 1) == ";") {
                    str1 = Trim(StringTrimRight(Trim(A_LoopField195), StrLen(keyWordInc + ";")));
                } else {
                    str1 = Trim(StringTrimRight(Trim(A_LoopField195), StrLen(keyWordInc)));
                }
                str1 = Trim(expressionParserTranspiler(str1));
            ...
            else if (SubStrLastChars(Trim(A_LoopField195), StrLen(keyWordDec)) == keyWordDec || SubStrLastChars(Trim(A_LoopField195), StrLen(keyWordDec + ";")) == keyWordDec + ";" && lineDone == 0) {
                if (useSemicolon == "on" || SubStrLastChars(Trim(A_LoopField195), 1) == ";") {
                    str1 = Trim(StringTrimRight(Trim(A_LoopField195), StrLen(keyWordDec + ";")));
                } else {
                    str1 = Trim(StringTrimRight(Trim(A_LoopField195), StrLen(keyWordDec)));
                }
                str1 = Trim(expressionParserTranspiler(str1));
            ...
            else if (SubStr(StrLower(Trim(A_LoopField195)), 1, StrLen(StrLower(keyWordInclude + " "))) == StrLower(keyWordInclude + " ") && langToConvertTo == langFileExtension_2) {
                htCode += keyWordInclude_2 + " " + Trim(StringTrimLeft(Trim(A_LoopField195), StrLen(StrLower(keyWordInclude) + " "))) + Chr(10);
            }
            else if (StrLower(A_LoopField195) == StrLower(keyWordGlobal) || StrLower(A_LoopField195) == StrLower(keyWordGlobal + ";")) {
                if (langToConvertTo == "ahk") {
                    htCode += "global\n";
                }
            }
            else if (SubStr(StrLower(A_LoopField195), 1, StrLen(StrLower(keyWordAwait))) == StrLower(keyWordAwait)) {
                str1 = StringTrimLeft(A_LoopField195, StrLen(keyWordAwait));
                //MsgBox, % A_LoopField195
                lineDone = 1;
            ...
            else if (SubStr(StrLower(A_LoopField195), 1, StrLen(StrLower(keyWordGlobal))) == StrLower(keyWordGlobal)) {
                str1 = StringTrimLeft(A_LoopField195, StrLen(keyWordGlobal));
                //MsgBox, % A_LoopField195
            ...
            else if (SubStr(StrLower(A_LoopField195), 1, StrLen(keyWordFunc)) == StrLower(keyWordFunc) || SubStr(StrLower(A_LoopField195), 1, StrLen(StrLower(keyWordAsync) + StrLower(keyWordFunc))) == StrLower(keyWordAsync) + StrLower(keyWordFunc) && InStr(StrLower(A_LoopField195), " main(") == false && lineDone == 0) {
                lineDone = 1;
                if (SubStr(StrLower(A_LoopField195), 1, StrLen(StrLower(keyWordAsync) + StrLower(keyWordFunc))) == StrLower(keyWordAsync) + StrLower(keyWordFunc)) {
                    // is async
                    int1 = 1;
                    str1 = Trim(StringTrimLeft(A_LoopField195, StrLen(StrLower(keyWordAsync + keyWordFunc))));
                } else {
                    // is async
                    int1 = 0;
                    str1 = Trim(StringTrimLeft(A_LoopField195, StrLen(StrLower(keyWordFunc))));
                }
                if (langToConvertTo != langFileExtension_2 && langToConvertTo != "cpp" && langToConvertTo != "go" && langToConvertTo != "cs" && langToConvertTo != "java" && langToConvertTo != "kt" && langToConvertTo != "nim" && langToConvertTo != "swift" && langToConvertTo != "dart" && langToConvertTo != "ts") {
                    if (str1 != "") {
                        str4 = "";
                        items197 = LoopParseFunc(str1, ",");
                        for (let A_Index197 = 0; A_Index197 < items197.length + 0; A_Index197++) {
                            const A_LoopField197 = items197[A_Index197 - 0];
                            if (InStr(A_LoopField197, " := ")) {
                                if (InStr(Trim(StrSplit(A_LoopField197, " := ", 1)), " ") != true) {
                                    str4 += "optanal-HTVM-opt-parmsNOtWorking-theirewIs-a-bug-Iusfbudfbuoeshfuisbav=-fwaegs-awedsf-dd-sfgc " + A_LoopField197 + ", ";
                                } else {
                                    str4 += A_LoopField197 + ", ";
                                }
                            }
                            else if (InStr(A_LoopField197, " = ")) {
                                if (InStr(Trim(StrSplit(A_LoopField197, " = ", 1)), " ") != true) {
                                    str4 += "optanal-HTVM-opt-parmsNOtWorking-theirewIs-a-bug-Iusfbudfbuoeshfuisbav=-fwaegs-awedsf-dd-sfgc " + A_LoopField197 + ", ";
                                } else {
                                    str4 += A_LoopField197 + ", ";
                            ...
                            else if (InStr(A_LoopField197, " " + Trim(keyWordAssign) + " ")) {
                                if (InStr(Trim(StrSplit(A_LoopField197, " " + Trim(keyWordAssign) + " ", 1)), " ") != true) {
                                    str4 += "optanal-HTVM-opt-parmsNOtWorking-theirewIs-a-bug-Iusfbudfbuoeshfuisbav=-fwaegs-awedsf-dd-sfgc " + A_LoopField197 + ", ";
                                } else {
                                    str4 += A_LoopField197 + ", ";
                                }
                ...
                if (Trim(str1) == "") {
                    ...
                    str19 = "";
                    str20 = "";
                    items198 = LoopParseFunc(str1, ",");
                    for (let A_Index198 = 0; A_Index198 < items198.length + 0; A_Index198++) {
                        const A_LoopField198 = items198[A_Index198 - 0];
                        //print("|" . Trim(A_LoopField198) . "|")
                        if (InStr(Trim(A_LoopField198), " ")) {
                            // there is space aka there is a type
                            if (InStr(A_LoopField198, " " + Trim(keyWordAssign) + " ") == false) {
                                if (usePrefixTypeForTypeDefinition == "on") {
                                    // normal
                                    str6 = Trim(A_LoopField198);
                                ...
                                // optanal param
                                if (InStr(Trim(StrSplit(Trim(A_LoopField198), " " + Trim(keyWordAssign) + " ", 1)), " ")) {
                                    // there is a type
                                    if (usePrefixTypeForTypeDefinition == "on") {
                                        // normal
                                        str6 = Trim(StrSplit(Trim(A_LoopField198), " " + Trim(keyWordAssign) + " ", 1));
                                        str9 = Trim(expressionParserTranspiler(Trim(StrSplit(Trim(A_LoopField198), " " + Trim(keyWordAssign) + " ", 2))));
                                        str7 = getTheLastWord(str6);
                                        str8 = getFuncTypeConvert(Trim(StringTrimRight(str6, StrLen(Trim(str7)) + 1)));
                                        //;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;
                                        //;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;
                                    ...
                                    // there is no type
                                    str8 = Trim(StrSplit(Trim(A_LoopField198), " " + Trim(keyWordAssign) + " ", 1));
                                    str9 = Trim(expressionParserTranspiler(Trim(StrSplit(Trim(A_LoopField198), " " + Trim(keyWordAssign) + " ", 2))));
                                    // str10
                                    // skipLeftCuleyForFuncPLS := 1
                                    //;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;
                            ...
                            str5 += Trim(A_LoopField198) + Chr(10);
                        }
                    }
                    //print(str5)
                    //print(str10)
                    ...
                    if (Trim(str5) != "" && Trim(str10) != "") {
                        ...
                        if (langToConvertTo == "go") {
                            ...
                            str13 += "{" + Chr(10) + " ";
                            items203 = LoopParseFunc(str12, ",");
                            for (let A_Index203 = 0; A_Index203 < items203.length + 0; A_Index203++) {
                                const A_LoopField203 = items203[A_Index203 - 0];
                                str16 = Trim(StrSplit(Trim(StrSplit(Trim(A_LoopField203), " = ", 1)), " ", 1));
                                str17 = Trim(StrSplit(Trim(A_LoopField203), " = ", 2));
                                str13 += str16 + " := " + str17 + Chr(10);
                            }
                            items204 = LoopParseFunc(str12, ",");
                            for (let A_Index204 = 0; A_Index204 < items204.length + 0; A_Index204++) {
                                const A_LoopField204 = items204[A_Index204 - 0];
                                str16 = Trim(StrSplit(Trim(StrSplit(Trim(A_LoopField204), " = ", 1)), " ", 1));
                                str18 = Trim(StrSplit(Trim(StrSplit(Trim(A_LoopField204), " = ", 1)), " ", 2));
                                // str16 = var name
                                // str18 = var type
                                str13 += "if len(__HTVM_V2_TO_GO_optionalParams__) " + Chr(62) + " " + STR(A_Index204) + Chr(10) + "{" + Chr(10) + str16 + " = __HTVM_V2_TO_GO_optionalParams__[" + STR(A_Index204) + "].(" + str18 + ")" + Chr(10) + "}" + Chr(10);
                            }
                            str12 = str14;
                        }
                        if (langToConvertTo == "java") {
                            ...
                            items207 = LoopParseFunc(str12, ",");
                            for (let A_Index207 = 0; A_Index207 < items207.length + 0; A_Index207++) {
                                const A_LoopField207 = items207[A_Index207 - 0];
                                str16 = Trim(StrSplit(Trim(StrSplit(Trim(A_LoopField207), " = ", 1)), " ", 2));
                                str17 = Trim(StrSplit(Trim(StrSplit(Trim(A_LoopField207), " = ", 1)), " ", 1));
                                // str16 = var name
                                // str17 = type name
                                str13 += "if (__HTVM_V2_TO_JAVA_optionalParams__.length " + Chr(62) + " " + STR(A_Index207) + ") " + str16 + " = (" + str17 + ") __HTVM_V2_TO_JAVA_optionalParams__[" + STR(A_Index207) + "];" + Chr(10);
                            }
                            str12 = str14;
                        }
                        str11 += str12;
            ...
            else if (InStr(StrLower(A_LoopField195), " main(") && lineDone == 0) {
                lineDone = 1;
                javaMainFuncSeen = 1;
                csMainFuncSeen = 1;
                htCode += A_LoopField195 + Chr(10);
            }
            else if (InStr(A_LoopField195, " " + keyWordAssign + " ") || InStr(A_LoopField195, " " + keyWordAssignAdd + " ") || InStr(A_LoopField195, " " + keyWordAssignConcat + " ") || InStr(A_LoopField195, " " + keyWordAssignSub + " ") || InStr(A_LoopField195, " " + keyWordAssignMul + " ") || InStr(A_LoopField195, " " + keyWordAssignDiv + " ") || InStr(A_LoopField195, " " + keyWordAssignMod + " ") || InStr(A_LoopField195, " " + keyWordAssignShiftLeft + " ") || InStr(A_LoopField195, " " + keyWordAssignShiftRight + " ") || InStr(A_LoopField195, " " + keyWordLogicalAssignShiftRight + " ") || InStr(A_LoopField195, " " + keyWordAssignBitAnd + " ") || InStr(A_LoopField195, " " + keyWordAssignBitOr + " ") || InStr(A_LoopField195, " " + keyWordAssignBitXor + " ") && lineDone == 0) {
                allVarsSoWeDontReDecVars_FIX_TOGGLE = 1;
                //keyWordAssign
                //keyWordAssignAdd
                //keyWordAssignConcat
                ...
                //allVarsSoWeDontReDecVars_FIX_int64
                //allVarsSoWeDontReDecVars_FIX_float
                if (InStr(A_LoopField195, " " + keyWordAssign + " ")) {
                    str1 = StrSplit(A_LoopField195, " " + keyWordAssign + " ", 1);
                    str6 = StrSplit(A_LoopField195, " " + keyWordAssign + " ", 2);
                    str10 = keyWordAssign;
                    str11 = keyWordAssign_2;
                    if (langToConvertTo == "ahk") {
                        str12 = ":=";
                ...
                else if (InStr(A_LoopField195, " " + keyWordAssignAdd + " ")) {
                    str1 = StrSplit(A_LoopField195, " " + keyWordAssignAdd + " ", 1);
                    str6 = StrSplit(A_LoopField195, " " + keyWordAssignAdd + " ", 2);
                    str10 = keyWordAssignAdd;
                    str11 = keyWordAssignAdd_2;
                    str12 = "+=";
                }
                else if (InStr(A_LoopField195, " " + keyWordAssignConcat + " ")) {
                    str1 = StrSplit(A_LoopField195, " " + keyWordAssignConcat + " ", 1);
                    str6 = StrSplit(A_LoopField195, " " + keyWordAssignConcat + " ", 2);
                    str10 = keyWordAssignConcat;
                    str11 = keyWordAssignConcat_2;
                    if (langToConvertTo == "ahk") {
                        str12 = ".=";
                ...
                else if (InStr(A_LoopField195, " " + keyWordAssignSub + " ")) {
                    str1 = StrSplit(A_LoopField195, " " + keyWordAssignSub + " ", 1);
                    str6 = StrSplit(A_LoopField195, " " + keyWordAssignSub + " ", 2);
                    str10 = keyWordAssignSub;
                    str11 = keyWordAssignSub_2;
                    str12 = "-=";
                }
                else if (InStr(A_LoopField195, " " + keyWordAssignMul + " ")) {
                    str1 = StrSplit(A_LoopField195, " " + keyWordAssignMul + " ", 1);
                    str6 = StrSplit(A_LoopField195, " " + keyWordAssignMul + " ", 2);
                    str10 = keyWordAssignMul;
                    str11 = keyWordAssignMul_2;
                    str12 = "*=";
                }
                else if (InStr(A_LoopField195, " " + keyWordAssignDiv + " ")) {
                    str1 = StrSplit(A_LoopField195, " " + keyWordAssignDiv + " ", 1);
                    str6 = StrSplit(A_LoopField195, " " + keyWordAssignDiv + " ", 2);
                    str10 = keyWordAssignDiv;
                    str11 = keyWordAssignDiv_2;
                    str12 = "/=";
                }
                else if (InStr(A_LoopField195, " " + keyWordAssignMod + " ")) {
                    str1 = StrSplit(A_LoopField195, " " + keyWordAssignMod + " ", 1);
                    str6 = StrSplit(A_LoopField195, " " + keyWordAssignMod + " ", 2);
                    str10 = keyWordAssignMod;
                    str11 = keyWordAssignMod_2;
                    str12 = "%=";
                }
                else if (InStr(A_LoopField195, " " + keyWordAssignShiftLeft + " ")) {
                    str1 = StrSplit(A_LoopField195, " " + keyWordAssignShiftLeft + " ", 1);
                    str6 = StrSplit(A_LoopField195, " " + keyWordAssignShiftLeft + " ", 2);
                    str10 = keyWordAssignShiftLeft;
                    str11 = keyWordAssignShiftLeft_2;
                    str12 = "<<=";
                }
                else if (InStr(A_LoopField195, " " + keyWordAssignShiftRight + " ")) {
                    str1 = StrSplit(A_LoopField195, " " + keyWordAssignShiftRight + " ", 1);
                    str6 = StrSplit(A_LoopField195, " " + keyWordAssignShiftRight + " ", 2);
                    str10 = keyWordAssignShiftRight;
                    str11 = keyWordAssignShiftRight_2;
                    str12 = ">>=";
                }
                else if (InStr(A_LoopField195, " " + keyWordLogicalAssignShiftRight + " ")) {
                    str1 = StrSplit(A_LoopField195, " " + keyWordLogicalAssignShiftRight + " ", 1);
                    str6 = StrSplit(A_LoopField195, " " + keyWordLogicalAssignShiftRight + " ", 2);
                    str10 = keyWordLogicalAssignShiftRight;
                    str11 = keyWordLogicalAssignShiftRight_2;
                    str12 = ">>>=";
                }
                else if (InStr(A_LoopField195, " " + keyWordAssignBitAnd + " ")) {
                    str1 = StrSplit(A_LoopField195, " " + keyWordAssignBitAnd + " ", 1);
                    str6 = StrSplit(A_LoopField195, " " + keyWordAssignBitAnd + " ", 2);
                    str10 = keyWordAssignBitAnd;
                    str11 = keyWordAssignBitAnd_2;
                    str12 = "&=";
                }
                else if (InStr(A_LoopField195, " " + keyWordAssignBitOr + " ")) {
                    str1 = StrSplit(A_LoopField195, " " + keyWordAssignBitOr + " ", 1);
                    str6 = StrSplit(A_LoopField195, " " + keyWordAssignBitOr + " ", 2);
                    str10 = keyWordAssignBitOr;
                    str11 = keyWordAssignBitOr_2;
                    str12 = "|=";
                }
                else if (InStr(A_LoopField195, " " + keyWordAssignBitXor + " ")) {
                    str1 = StrSplit(A_LoopField195, " " + keyWordAssignBitXor + " ", 1);
                    str6 = StrSplit(A_LoopField195, " " + keyWordAssignBitXor + " ", 2);
                    str10 = keyWordAssignBitXor;
                    str11 = keyWordAssignBitXor_2;
                    str12 = "^=";
                }
            ...
            else if (SubStr(Trim(A_LoopField195), 1, StrLen(keyWordVar)) == keyWordVar && lineDone == 0) {
                allVarsSoWeDontReDecVars_FIX_TOGGLE = 1;
                if (usePostfixTypeForTypeDefinition == "off") {
                    lineDone = 1;
                    str1 = Trim(StringTrimLeft(Trim(A_LoopField195), StrLen(keyWordVar)));
                    if (SubStrLastChars(str1, 1) == ";") {
                        str1 = StringTrimRight(str1, 1);
                    }
                    items221 = LoopParseFunc(str1, " ")
            ...
            else if (SubStr(Trim(A_LoopField195), 1, StrLen(keyWordLet)) == keyWordLet && lineDone == 0) {
                allVarsSoWeDontReDecVars_FIX_TOGGLE = 1;
                if (usePostfixTypeForTypeDefinition == "off") {
                    lineDone = 1;
                    str1 = Trim(StringTrimLeft(Trim(A_LoopField195), StrLen(keyWordLet)));
                    if (SubStrLastChars(str1, 1) == ";") {
                        str1 = StringTrimRight(str1, 1);
                    }
                    items222 = LoopParseFunc(str1, " ")
            ...
            else if (SubStr(Trim(A_LoopField195), 1, StrLen(keyWordDouble + " ")) == keyWordDouble + " " || SubStr(Trim(A_LoopField195), 1, StrLen(keyWordChar + " ")) == keyWordChar + " " || SubStr(Trim(A_LoopField195), 1, StrLen(keyWordUint8 + " ")) == keyWordUint8 + " " || SubStr(Trim(A_LoopField195), 1, StrLen(keyWordUint16 + " ")) == keyWordUint16 + " " || SubStr(Trim(A_LoopField195), 1, StrLen(keyWordUint32 + " ")) == keyWordUint32 + " " || SubStr(Trim(A_LoopField195), 1, StrLen(keyWordUint64 + " ")) == keyWordUint64 + " " || SubStr(Trim(A_LoopField195), 1, StrLen(keyWordINT + " ")) == keyWordINT + " " || SubStr(Trim(A_LoopField195), 1, StrLen(keyWordSTR + " ")) == keyWordSTR + " " || SubStr(Trim(A_LoopField195), 1, StrLen(keyWordBOOL + " ")) == keyWordBOOL + " " || SubStr(Trim(A_LoopField195), 1, StrLen(keyWordFLOAT + " ")) == keyWordFLOAT + " " || SubStr(Trim(A_LoopField195), 1, StrLen(keyWordINT8 + " ")) == keyWordINT8 + " " || SubStr(Trim(A_LoopField195), 1, StrLen(keyWordINT16 + " ")) == keyWordINT16 + " " || SubStr(Trim(A_LoopField195), 1, StrLen(keyWordINT32 + " ")) == keyWordINT32 + " " || SubStr(Trim(A_LoopField195), 1, StrLen(keyWordINT64 + " ")) == keyWordINT64 + " " || SubStr(Trim(A_LoopField195), 1, StrLen(keyWordArrayDefinition + " ")) == keyWordArrayDefinition + " " || SubStr(Trim(A_LoopField195), 1, StrLen(keyWordArrayOfIntegersDefinition + " ")) == keyWordArrayOfIntegersDefinition + " " || SubStr(Trim(A_LoopField195), 1, StrLen(keyWordArrayOfStringsDefinition + " ")) == keyWordArrayOfStringsDefinition + " " || SubStr(Trim(A_LoopField195), 1, StrLen(keyWordArrayOfFloatingPointNumbersDefinition + " ")) == keyWordArrayOfFloatingPointNumbersDefinition + " " || SubStr(Trim(A_LoopField195), 1, StrLen(keyWordArrayOfBooleansDefinition + " ")) == keyWordArrayOfBooleansDefinition + " " && usePrefixTypeForTypeDefinition == "on" && lineDone == 0) {
                lineDone = 1;
                allVarsSoWeDontReDecVars_FIX_TOGGLE = 1;
                str1 = Trim(A_LoopField195);
                if (SubStrLastChars(str1, 1) == ";") {
            ...
            else if (RegExMatch(Trim(A_LoopField195), "^[a-zA-Z_][a-zA-Z0-9_]*: [^:]*;?$") && usePostfixTypeForTypeDefinition == "on" && lineDone == 0) {
                lineDone = 1;
                allVarsSoWeDontReDecVars_FIX_TOGGLE = 1;
                str1 = Trim(A_LoopField195);
                if (SubStrLastChars(str1, 1) == ";") {
            ...
            else if (RegExMatch(Trim(A_LoopField195), "^[a-zA-Z0-9_\\.]+\\(") && !(InStr(StrLower(A_LoopField195), " main(")) && lineDone == 0) {
                lineDone = 1;
                str1 = Trim(A_LoopField195);
                if (SubStrLastChars(str1, 1) == ";") {
                    str1 = StringTrimRight(str1, 1);
            ...
            else if (RegExMatch(A_LoopField195, "\\.[a-zA-Z_][a-zA-Z0-9_]*") && lineDone == 0) {
                lineDone = 1;
                str1 = Trim(A_LoopField195);
                if (SubStrLastChars(str1, 1) == ";") {
                    str1 = StringTrimRight(str1, 1);
            ...
            else if (KeyWordsCommands(A_LoopField195, "check", commands, langToConvertTo) == "true" && lineDone == 0) {
                lineDone = 1;
                if (langToConvertTo == langFileExtension_2) {
                    htCode += A_LoopField195 + Chr(10);
                } else {
                    out_KeyWordsCommands = KeyWordsCommands(A_LoopField195, "transpile", commands, langToConvertTo);
                    htCode += out_KeyWordsCommands + Chr(10);
                }
            } else {
                //print("else else else " . A_LoopField195)
                // this is THE else
                //;;;;;;;;;;;;;;;;;;
                if (lineDone != 1) {
                    if (skipLeftCuleyForFuncPLS != 1) {
                        if (SubStr(Trim(StrLower(A_LoopField195)), 1, 1) == Chr(125)) {
                            htCode += Chr(125) + Chr(10);
                        } else {
                            if (htCodeAcurlyBraceAddSomeVrasFixLP == 1 && SubStr(Trim(StrLower(A_LoopField195)), 1, 1) == Chr(123)) {
                                htCodeAcurlyBraceAddSomeVrasFixLP = 0;
                                htCode += A_LoopField195 + Chr(10) + theFixTextLoopLP + Chr(10);
                            } else {
                                if (htCodeAcurlyBraceAddSomeVrasFixNL == 1 && SubStr(Trim(StrLower(A_LoopField195)), 1, 1) == Chr(123)) {
                                    htCodeAcurlyBraceAddSomeVrasFixNL = 0;
                                    htCode += A_LoopField195 + Chr(10) + theFixTextLoopNL + Chr(10);
                                } else {
                                    htCode += A_LoopField195 + Chr(10);
                                }
                            }
        ...
        if (langToConvertTo != "ahk" && langToConvertTo != langFileExtension_2) {
            ...
            if (haveWeEverUsedAloop == 1) {
                ...
                for (let A_Index224 = 0; A_Index224 < items224.length + 0; A_Index224++) {
                    ...
                    if (out1 == "nl") {
                        ...
                        for (let A_Index225 = 0; A_Index225 < items225.length + 0; A_Index225++) {
                            ...
                            //MsgBox, |%A_LoopField225%|`n|%fixLoopLokingFor%|
                            if (InStr(A_LoopField225, fixLoopLokingFor) && insdeAnestedLoopBAD != 1) {
                                fixLoopLokingForNum = 1;
                                //MsgBox, do we came here 1
                            }
                            if (SubStr(Trim(A_LoopField225), 1, 4) == "for " && weAreDoneHereCurly != 1 && insdeAnestedLoopBAD != 1 && fixLoopLokingForNum == 1) {
                                s = StrSplit(A_LoopField225 ,"" + keyWordAIndex + "", 2);
                                out1z = s;
                                s = StrSplit(out1z, " ", 1);
                                out1z = Trim(s);
                                //MsgBox, % out1z
                                ...
                                foundTheTopLoop++;
                                inTarget = 1;
                                //MsgBox, % A_LoopField225
                                dontSaveStr = 1;
                                ALoopField = A_LoopField225;
                                DeleayOneCuzOfLoopParse = 1;
                                out4758686d86d86d86578991a += ALoopField + Chr(10);
                            }
                            if (inTarget == 1 && InStr(A_LoopField225, Chr(123)) && insdeAnestedLoopBAD != 1) {
                                insideBracket = 1;
                            }
                            if (insideBracket == 1 && InStr(A_LoopField225, Chr(123)) && insdeAnestedLoopBAD != 1) {
                                netsedCurly++;
                            }
                            if (insideBracket == 1 && InStr(A_LoopField225, Chr(125)) && insdeAnestedLoopBAD != 1) {
                                netsedCurly--;
                                readyToEnd = 1;
                            }
                            if (SubStr(Trim(A_LoopField225), 1, 4) == "for " && insdeAnestedLoopBAD != 1 && foundTheTopLoop >= 2) {
                            ...
                            if (insdeAnestedLoopBAD == 1) {
                                if (InStr(A_LoopField225, Chr(123))) {
                                    insideBracket1 = 1;
                                }
                                if (insideBracket1 == 1 && InStr(A_LoopField225, Chr(123))) {
                                    netsedCurly1++;
                                }
                                if (insideBracket1 == 1 && InStr(A_LoopField225, Chr(125))) {
                                    netsedCurly1--;
                                    readyToEnd1 = 1;
                                }
                                if (InStr(A_LoopField225, Chr(125)) && readyToEnd1 == 1 && netsedCurly1 == 0 && insideBracket == 1) {
                                    //MsgBox, % A_LoopField225
                                    eldLoopNestedBADlol = 1;
                                }
                                out4758686d86d86d86578991a += A_LoopField225 + Chr(10);
                            }
                            ...
                            if (inTarget == 1 && InStr(A_LoopField225, Chr(125)) && readyToEnd == 1 && netsedCurly == 0 && weAreDoneHereCurly == 0 && dontSaveStr != 1 && insdeAnestedLoopBAD != 1) {
                                //MsgBox, % A_LoopField225
                        ...
                        foundTheTopLoop = 0;
                        out4758686d86d86d86578991a = "";
                        items226 = LoopParseFunc(htCode, "\n", "\r")
                        for (let A_Index226 = 0; A_Index226 < items226.length + 0; A_Index226++) {
                            const A_LoopField226 = items226[A_Index226 - 0];
                            if (InStr(A_LoopField226 , fixLoopLokingFor) && insdeAnestedLoopBAD != 1) {
                                fixLoopLokingForNum = 1;
                                //MsgBox, do we came here 3
                            }
                            if (SubStr(Trim(A_LoopField226), 1, 4) == "for " && weAreDoneHereCurly != 1 && insdeAnestedLoopBAD != 1 && fixLoopLokingForNum == 1) {
                                s = StrSplit(A_LoopField226 ,"" + keyWordAIndex + "", 2);
                                out1z = s;
                                s = StrSplit(out1z, " ", 1);
                                ...
                                foundTheTopLoop++;
                                inTarget = 1;
                                //MsgBox, % A_LoopField226
                                dontSaveStr = 1;
                                ALoopField = A_LoopField226;
                                DeleayOneCuzOfLoopParse = 1;
                                out4758686d86d86d86578991a += ALoopField + Chr(10);
                            }
                            if (inTarget == 1 && InStr(A_LoopField226, Chr(123)) && insdeAnestedLoopBAD != 1) {
                                insideBracket = 1;
                            }
                            if (insideBracket == 1 && InStr(A_LoopField226, Chr(123)) && insdeAnestedLoopBAD != 1) {
                                netsedCurly++;
                            }
                            if (insideBracket == 1 && InStr(A_LoopField226, Chr(125)) && insdeAnestedLoopBAD != 1) {
                                netsedCurly--;
                                readyToEnd = 1;
                            }
                            if (SubStr(Trim(A_LoopField226), 1, 4) == "for " && insdeAnestedLoopBAD != 1 && foundTheTopLoop >= 2) {
                                insdeAnestedLoopBAD = 1;
                                insideBracket1 = 0;
                                netsedCurly1 = 0;
                            }
                            ...
                            if (insdeAnestedLoopBAD == 1) {
                                if (InStr(A_LoopField226, Chr(123))) {
                                    insideBracket1 = 1;
                                }
                                if (insideBracket1 == 1 && InStr(A_LoopField226, Chr(123))) {
                                    netsedCurly1++;
                                }
                                if (insideBracket1 == 1 && InStr(A_LoopField226, Chr(125))) {
                                    netsedCurly1--;
                                    readyToEnd1 = 1;
                                }
                                if (InStr(A_LoopField226, Chr(125)) && readyToEnd1 == 1 && netsedCurly1 == 0 && insideBracket == 1) {
                                    //MsgBox, % A_LoopField226
                                    eldLoopNestedBADlol = 1;
                                }
                                out4758686d86d86d86578991a += A_LoopField226 + Chr(10);
                            }
                            if (inTarget == 1 && dontSaveStr != 1 && fixLoopLokingForNum != 1 && insdeAnestedLoopBAD != 1) {
                                ALoopField = A_LoopField226;
                                // Replace "A_Index" with or without a following digit with "A_Index" + out1z
                            ...
                            if ((inTarget == 1 && InStr(A_LoopField226, Chr(125)) && readyToEnd == 1 && netsedCurly == 0 && weAreDoneHereCurly == 0 && dontSaveStr != 1 && insdeAnestedLoopBAD != 1)) {
                                //MsgBox, % A_LoopField226
                                weAreDoneHereCurly = 1;
                                inTarget = 0;
                                endBracketDOntPutThere = 1;
                ...
                hold = 0;
                items227 = LoopParseFunc(htCode, "\n", "\r")
                for (let A_Index227 = 0; A_Index227 < items227.length + 0; A_Index227++) {
                    const A_LoopField227 = items227[A_Index227 - 0];
                    ignore = 0;
                    if (SubStr(Trim(A_LoopField227), 1, 4) == "for ") {
                        if (hold == 1 && holdText == A_LoopField227) {
                            ignore = 1;
                        } else {
                            holdText = A_LoopField227;
                            hold = 1;
                        }
                    }
                    if (!ignore) {
                        out4758686d86dgt8r754444444 += A_LoopField227 + Chr(10);
                    }
                }
    ...
    if (COUNT_programmingBlock_HTVMsyntax != 0) {
        ...
        for (let A_Index229 = 0; A_Index229 < COUNT_programmingBlock_HTVMsyntax + 0; A_Index229++) {
            ADD_ALL_programmingBlock_HTVMsyntax += programmingBlock_HTVMsyntax[A_Index229] + Chr(10);
        }
        ...
        isNotHTVMfile2 = 1;
        for (let A_Index230 = 0; A_Index230 < COUNT_programmingBlock_HTVMsyntax + 0; A_Index230++) {
            htCode = StrReplace(htCode, "programmingBlock_HTVMsyntax-programmingBlock_HTVMsyntax-AA" + STR(A_Index230 + 1) + "AA", compiler(programmingBlock_HTVMsyntax[A_Index230], allInstructionFile, "full", langToConvertToParam));
        }
        isNotHTVMfile2 = 0;
    }
    //;;;;;;;;;;;;;;;;;;;;;;;;;;;
    ...
    if (langToConvertTo == langFileExtension_2) {
        ...
        if (useEnd_2 == "on") {
            items231 = LoopParseFunc(htCode, "\n", "\r")
            for (let A_Index231 = 0; A_Index231 < items231.length + 0; A_Index231++) {
                const A_LoopField231 = items231[A_Index231 - 0];
                if (Trim(A_LoopField231) == keyWordCurlyBraceClose_2) {
                    fixLuaAndRuby += StrReplace(A_LoopField231, keyWordCurlyBraceClose_2, Trim(keyWordEnd_2)) + Chr(10);
                }
                else if (SubStrLastChars(A_LoopField231, 2) == " " + keyWordCurlyBraceOpen_2) {
                    fixLuaAndRuby += StringTrimRight(A_LoopField231, 2) + Chr(10);
                } else {
                    fixLuaAndRuby += A_LoopField231 + Chr(10);
                }
            }
        ...
        if (langToConvertTo == "lua" || langToConvertTo == "rb" || langToConvertTo == langFileExtension_2) {
            if (langToConvertTo == langFileExtension_2 && useEnd_2 == "on") {
                ...
                items232 = LoopParseFunc(htCode, "\n", "\r")
                for (let A_Index232 = 0; A_Index232 < items232.length + 0; A_Index232++) {
                    const A_LoopField232 = items232[A_Index232 - 0];
                    if (Trim(A_LoopField232) == "}") {
                        fixLuaAndRuby += StrReplace(A_LoopField232, "}", fixLuaAndRubyHELP) + Chr(10);
                    }
                    else if (SubStrLastChars(A_LoopField232, 2) == " {") {
                        fixLuaAndRuby += StringTrimRight(A_LoopField232, 2) + Chr(10);
                    } else {
                        fixLuaAndRuby += A_LoopField232 + Chr(10);
                    }
                }
                htCode = StringTrimRight(fixLuaAndRuby, 1);
            } else {
                fixLuaAndRubyHELP = "end";
                items233 = LoopParseFunc(htCode, "\n", "\r")
                for (let A_Index233 = 0; A_Index233 < items233.length + 0; A_Index233++) {
                    const A_LoopField233 = items233[A_Index233 - 0];
                    if (Trim(A_LoopField233) == "}") {
                        fixLuaAndRuby += StrReplace(A_LoopField233, "}", fixLuaAndRubyHELP) + Chr(10);
                    }
                    else if (SubStrLastChars(A_LoopField233, 2) == " {") {
                        fixLuaAndRuby += StringTrimRight(A_LoopField233, 2) + Chr(10);
                    } else {
                        fixLuaAndRuby += A_LoopField233 + Chr(10);
                    }
                }
                htCode = StringTrimRight(fixLuaAndRuby, 1);
            }
    ...
    if (langToConvertTo == langFileExtension_2 && useEnd_2 == "on") {
        items234 = LoopParseFunc(htCode, "\n", "\r")
        for (let A_Index234 = 0; A_Index234 < items234.length + 0; A_Index234++) {
            const A_LoopField234 = items234[A_Index234 - 0];
            HTVM_Append(nextWordEndFix, A_LoopField234);
        }
        HTVM_Append(nextWordEndFix, " ");
        items235 = LoopParseFunc(htCode, "\n", "\r")
        for (let A_Index235 = 0; A_Index235 < items235.length + 0; A_Index235++) {
            const A_LoopField235 = items235[A_Index235 - 0];
            if (Trim(A_LoopField235) == Trim(keyWordEnd_2) && SubStr(Trim(nextWordEndFix[A_Index235 + 1]), 1, StrLen(Trim(keyWordElseIf_2) + " ")) == Trim(keyWordElseIf_2) + " " || Trim(A_LoopField235) == Trim(keyWordEnd_2) && (Trim(nextWordEndFix[A_Index235 + 1]) == Trim(keyWordElse_2) || Trim(nextWordEndFix[A_Index235 + 1]) == Trim(keyWordElse_2 + ":"))) {
                nextWordEndFixOut += "";
            } else {
                nextWordEndFixOut += A_LoopField235 + Chr(10);
            }
        }
        htCode = StringTrimRight(nextWordEndFixOut, 1);
    }
    if (langToConvertTo == "lua") {
        items236 = LoopParseFunc(htCode, "\n", "\r")
        for (let A_Index236 = 0; A_Index236 < items236.length + 0; A_Index236++) {
            const A_LoopField236 = items236[A_Index236 - 0];
            HTVM_Append(nextWordEndFix, A_LoopField236);
        }
        HTVM_Append(nextWordEndFix, " ");
        items237 = LoopParseFunc(htCode, "\n", "\r")
        for (let A_Index237 = 0; A_Index237 < items237.length + 0; A_Index237++) {
            const A_LoopField237 = items237[A_Index237 - 0];
            if (Trim(A_LoopField237) == "end" && SubStr(Trim(nextWordEndFix[A_Index237 + 1]), 1, StrLen("elseif ")) == "elseif " || Trim(A_LoopField237) == "end" && Trim(nextWordEndFix[A_Index237 + 1]) == "else") {
                nextWordEndFixOut += "";
            } else {
                nextWordEndFixOut += A_LoopField237 + Chr(10);
            }
        }
        htCode = StringTrimRight(nextWordEndFixOut, 1);
    }
    if (langToConvertTo == "rb") {
        items238 = LoopParseFunc(htCode, "\n", "\r")
        for (let A_Index238 = 0; A_Index238 < items238.length + 0; A_Index238++) {
            const A_LoopField238 = items238[A_Index238 - 0];
            HTVM_Append(nextWordEndFix, A_LoopField238);
        }
        HTVM_Append(nextWordEndFix, " ");
        items239 = LoopParseFunc(htCode, "\n", "\r")
        for (let A_Index239 = 0; A_Index239 < items239.length + 0; A_Index239++) {
            const A_LoopField239 = items239[A_Index239 - 0];
            if (Trim(A_LoopField239) == "end" && SubStr(Trim(nextWordEndFix[A_Index239 + 1]), 1, StrLen("elsif ")) == "elsif " || Trim(A_LoopField239) == "end" && Trim(nextWordEndFix[A_Index239 + 1]) == "else") {
                nextWordEndFixOut += "";
            } else {
                nextWordEndFixOut += A_LoopField239 + Chr(10);
            }
        }
        htCode = StringTrimRight(nextWordEndFixOut, 1);
    }
    ...
    nextWordEndFixOut = "";
    if (langToConvertTo == "rb") {
        items240 = LoopParseFunc(htCode, "\n", "\r")
        for (let A_Index240 = 0; A_Index240 < items240.length + 0; A_Index240++) {
            const A_LoopField240 = items240[A_Index240 - 0];
            HTVM_Append(nextWordEndFixRB, A_LoopField240);
        }
        HTVM_Append(nextWordEndFixRB, " ");
        items241 = LoopParseFunc(htCode, "\n", "\r")
        for (let A_Index241 = 0; A_Index241 < items241.length + 0; A_Index241++) {
            const A_LoopField241 = items241[A_Index241 - 0];
            if (Trim(A_LoopField241) == "end" && SubStr(Trim(nextWordEndFixRB[A_Index241 + 1]), 1, StrLen("rescue ")) == "rescue " || Trim(A_LoopField241) == "end" && Trim(nextWordEndFixRB[A_Index241 + 1]) == "ensure") {
                nextWordEndFixOut += "";
            } else {
    ...
    if (langToConvertTo == langFileExtension_2 && useEnd_2 == "on") {
        items242 = LoopParseFunc(htCode, "\n", "\r")
        for (let A_Index242 = 0; A_Index242 < items242.length + 0; A_Index242++) {
            const A_LoopField242 = items242[A_Index242 - 0];
            HTVM_Append(nextWordEndFixRB, A_LoopField242);
        }
        HTVM_Append(nextWordEndFixRB, " ");
        items243 = LoopParseFunc(htCode, "\n", "\r")
        for (let A_Index243 = 0; A_Index243 < items243.length + 0; A_Index243++) {
            const A_LoopField243 = items243[A_Index243 - 0];
            if (Trim(A_LoopField243) == Trim(keyWordEnd_2) && SubStr(Trim(nextWordEndFixRB[A_Index243 + 1]), 1, StrLen(Trim(keyWordCatch_2) + " ")) == Trim(keyWordCatch_2) + " " || Trim(A_LoopField243) == Trim(keyWordEnd_2) && Trim(nextWordEndFixRB[A_Index243 + 1]) == Trim(keyWordFinally_2) || Trim(nextWordEndFixRB[A_Index243 + 1]) == Trim(keyWordFinally_2 + ":")) {
                nextWordEndFixOut += "";
            } else {
    ...
    nextWordEndFixOut = "";
    if (langToConvertTo == "ahk") {
        items244 = LoopParseFunc(htCode, "\n", "\r")
        for (let A_Index244 = 0; A_Index244 < items244.length + 0; A_Index244++) {
            const A_LoopField244 = items244[A_Index244 - 0];
            if (SubStr(Trim(A_LoopField244), 1, StrLen("Loop, Parse, ")) == "Loop, Parse, ") {
                nextWordEndFixOut += StrReplace(A_LoopField244, "{", Chr(10) + "{" + Chr(10)) + Chr(10);
            } else {
    ...
    if (langToConvertTo == "go") {
        ...
        items246 = LoopParseFunc(htCode, "\n", "\r")
        for (let A_Index246 = 0; A_Index246 < items246.length + 0; A_Index246++) {
            const A_LoopField246 = items246[A_Index246 - 0];
            if (Trim(A_LoopField246) == "}" && SubStr(Trim(fixGoManGoIsSoAnnoyingBroooFurure[A_Index246 + 1]), 1, StrLen("else if ")) == "else if ") {
                fixGoManGoIsSoAnnoyingBroooSkip = 1;
                fixGoManGoIsSoAnnoyingBroooCount = 0;
                items247 = LoopParseFunc(fixGoManGoIsSoAnnoyingBroooFurure[A_Index246 + 1])
                ...
                for (let A_Index248 = 0; A_Index248 < fixGoManGoIsSoAnnoyingBroooCount + 0; A_Index248++) {
                    fixGoManGoIsSoAnnoyingBroooCountSpaceses += " ";
                }
                fixGoManGoIsSoAnnoyingBrooo += fixGoManGoIsSoAnnoyingBroooCountSpaceses + "} " + Trim(fixGoManGoIsSoAnnoyingBroooFurure[A_Index246 + 1]) + Chr(10);
            } else {
                if (fixGoManGoIsSoAnnoyingBroooSkip == 0) {
                    fixGoManGoIsSoAnnoyingBrooo += A_LoopField246 + Chr(10);
                }
    ...
    if (COUNT_programmingBlock_InTheTranspiledLang != 0) {
        for (let A_Index249 = 0; A_Index249 < COUNT_programmingBlock_InTheTranspiledLang + 0; A_Index249++) {
            htCode = StrReplace(htCode, "programmingBlock_InTheTranspiledLang-programmingBlock_InTheTranspiledLang-AA" + STR(A_Index249 + 1) + "AA", programmingBlock_InTheTranspiledLang[A_Index249]);
        }
    }
    if (langToConvertTo == "cpp") {
        if (COUNT_programmingBlock_CPP != 0) {
            for (let A_Index250 = 0; A_Index250 < COUNT_programmingBlock_CPP + 0; A_Index250++) {
                htCode = StrReplace(htCode, "programmingBlock_CPP-programmingBlock_CPP-AA" + STR(A_Index250 + 1) + "AA", programmingBlock_CPP[A_Index250]);
            }
        }
    } else {
        if (COUNT_programmingBlock_CPP != 0) {
            for (let A_Index251 = 0; A_Index251 < COUNT_programmingBlock_CPP + 0; A_Index251++) {
                if (langToConvertTo == langFileExtension_2) {
                    htCode = StrReplace(htCode, "programmingBlock_CPP-programmingBlock_CPP-AA" + STR(A_Index251 + 1) + "AA", keyWordCodeInTheTranspiledLangStartCPP_2 + Chr(10) + programmingBlock_CPP[A_Index251] + Chr(10) + keyWordCodeInTheTranspiledLangEndCPP_2);
                } else {
                    htCode = StrReplace(htCode, "programmingBlock_CPP-programmingBlock_CPP-AA" + STR(A_Index251 + 1) + "AA", Chr(10));
                }
            }
        }
    }
    if (langToConvertTo == "py") {
        if (COUNT_programmingBlock_PY != 0) {
            for (let A_Index252 = 0; A_Index252 < COUNT_programmingBlock_PY + 0; A_Index252++) {
                htCode = StrReplace(htCode, "programmingBlock_PY-programmingBlock_PY-AA" + STR(A_Index252 + 1) + "AA", programmingBlock_PY[A_Index252]);
            }
        }
    } else {
        if (COUNT_programmingBlock_PY != 0) {
            for (let A_Index253 = 0; A_Index253 < COUNT_programmingBlock_PY + 0; A_Index253++) {
                if (langToConvertTo == langFileExtension_2) {
                    htCode = StrReplace(htCode, "programmingBlock_PY-programmingBlock_PY-AA" + STR(A_Index253 + 1) + "AA", keyWordCodeInTheTranspiledLangStartPY_2 + Chr(10) + programmingBlock_PY[A_Index253] + Chr(10) + keyWordCodeInTheTranspiledLangEndPY_2);
                } else {
                    htCode = StrReplace(htCode, "programmingBlock_PY-programmingBlock_PY-AA" + STR(A_Index253 + 1) + "AA", Chr(10));
                }
            }
        }
    }
    if (langToConvertTo == "js") {
        if (COUNT_programmingBlock_JS != 0) {
            for (let A_Index254 = 0; A_Index254 < COUNT_programmingBlock_JS + 0; A_Index254++) {
                htCode = StrReplace(htCode, "programmingBlock_JS-programmingBlock_JS-AA" + STR(A_Index254 + 1) + "AA", programmingBlock_JS[A_Index254]);
            }
        }
    } else {
        if (COUNT_programmingBlock_JS != 0) {
            for (let A_Index255 = 0; A_Index255 < COUNT_programmingBlock_JS + 0; A_Index255++) {
                if (langToConvertTo == langFileExtension_2) {
                    htCode = StrReplace(htCode, "programmingBlock_JS-programmingBlock_JS-AA" + STR(A_Index255 + 1) + "AA", keyWordCodeInTheTranspiledLangStartJS_2 + Chr(10) + programmingBlock_JS[A_Index255] + Chr(10) + keyWordCodeInTheTranspiledLangEndJS_2);
                } else {
                    htCode = StrReplace(htCode, "programmingBlock_JS-programmingBlock_JS-AA" + STR(A_Index255 + 1) + "AA", Chr(10));
                }
            }
        }
    }
    if (langToConvertTo == "go") {
        if (COUNT_programmingBlock_GO != 0) {
            for (let A_Index256 = 0; A_Index256 < COUNT_programmingBlock_GO + 0; A_Index256++) {
                htCode = StrReplace(htCode, "programmingBlock_GO-programmingBlock_GO-AA" + STR(A_Index256 + 1) + "AA", programmingBlock_GO[A_Index256]);
            }
        }
    } else {
        if (COUNT_programmingBlock_GO != 0) {
            for (let A_Index257 = 0; A_Index257 < COUNT_programmingBlock_GO + 0; A_Index257++) {
                if (langToConvertTo == langFileExtension_2) {
                    htCode = StrReplace(htCode, "programmingBlock_GO-programmingBlock_GO-AA" + STR(A_Index257 + 1) + "AA", keyWordCodeInTheTranspiledLangStartGO_2 + Chr(10) + programmingBlock_GO[A_Index257] + Chr(10) + keyWordCodeInTheTranspiledLangEndGO_2);
                } else {
                    htCode = StrReplace(htCode, "programmingBlock_GO-programmingBlock_GO-AA" + STR(A_Index257 + 1) + "AA", Chr(10));
                }
            }
        }
    }
    if (langToConvertTo == "lua") {
        if (COUNT_programmingBlock_LUA != 0) {
            for (let A_Index258 = 0; A_Index258 < COUNT_programmingBlock_LUA + 0; A_Index258++) {
                htCode = StrReplace(htCode, "programmingBlock_LUA-programmingBlock_LUA-AA" + STR(A_Index258 + 1) + "AA", programmingBlock_LUA[A_Index258]);
            }
        }
    } else {
        if (COUNT_programmingBlock_LUA != 0) {
            for (let A_Index259 = 0; A_Index259 < COUNT_programmingBlock_LUA + 0; A_Index259++) {
                if (langToConvertTo == langFileExtension_2) {
                    htCode = StrReplace(htCode, "programmingBlock_LUA-programmingBlock_LUA-AA" + STR(A_Index259 + 1) + "AA", keyWordCodeInTheTranspiledLangStartLUA_2 + Chr(10) + programmingBlock_LUA[A_Index259] + Chr(10) + keyWordCodeInTheTranspiledLangEndLUA_2);
                } else {
                    htCode = StrReplace(htCode, "programmingBlock_LUA-programmingBlock_LUA-AA" + STR(A_Index259 + 1) + "AA", Chr(10));
                }
            }
        }
    }
    if (langToConvertTo == "cs") {
        if (COUNT_programmingBlock_CS != 0) {
            for (let A_Index260 = 0; A_Index260 < COUNT_programmingBlock_CS + 0; A_Index260++) {
                htCode = StrReplace(htCode, "programmingBlock_CS-programmingBlock_CS-AA" + STR(A_Index260 + 1) + "AA", programmingBlock_CS[A_Index260]);
            }
        }
    } else {
        if (COUNT_programmingBlock_CS != 0) {
            for (let A_Index261 = 0; A_Index261 < COUNT_programmingBlock_CS + 0; A_Index261++) {
                if (langToConvertTo == langFileExtension_2) {
                    htCode = StrReplace(htCode, "programmingBlock_CS-programmingBlock_CS-AA" + STR(A_Index261 + 1) + "AA", keyWordCodeInTheTranspiledLangStartCS_2 + Chr(10) + programmingBlock_CS[A_Index261] + Chr(10) + keyWordCodeInTheTranspiledLangEndCS_2);
                } else {
                    htCode = StrReplace(htCode, "programmingBlock_CS-programmingBlock_CS-AA" + STR(A_Index261 + 1) + "AA", Chr(10));
                }
            }
        }
    }
    if (langToConvertTo == "java") {
        if (COUNT_programmingBlock_JAVA != 0) {
            for (let A_Index262 = 0; A_Index262 < COUNT_programmingBlock_JAVA + 0; A_Index262++) {
                htCode = StrReplace(htCode, "programmingBlock_JAVA-programmingBlock_JAVA-AA" + STR(A_Index262 + 1) + "AA", programmingBlock_JAVA[A_Index262]);
            }
        }
    } else {
        if (COUNT_programmingBlock_JAVA != 0) {
            for (let A_Index263 = 0; A_Index263 < COUNT_programmingBlock_JAVA + 0; A_Index263++) {
                if (langToConvertTo == langFileExtension_2) {
                    htCode = StrReplace(htCode, "programmingBlock_JAVA-programmingBlock_JAVA-AA" + STR(A_Index263 + 1) + "AA", keyWordCodeInTheTranspiledLangStartJAVA_2 + Chr(10) + programmingBlock_JAVA[A_Index263] + Chr(10) + keyWordCodeInTheTranspiledLangEndJAVA_2);
                } else {
                    htCode = StrReplace(htCode, "programmingBlock_JAVA-programmingBlock_JAVA-AA" + STR(A_Index263 + 1) + "AA", Chr(10));
                }
            }
        }
    }
    if (langToConvertTo == "kt") {
        if (COUNT_programmingBlock_KT != 0) {
            for (let A_Index264 = 0; A_Index264 < COUNT_programmingBlock_KT + 0; A_Index264++) {
                htCode = StrReplace(htCode, "programmingBlock_KT-programmingBlock_KT-AA" + STR(A_Index264 + 1) + "AA", programmingBlock_KT[A_Index264]);
            }
        }
    } else {
        if (COUNT_programmingBlock_KT != 0) {
            for (let A_Index265 = 0; A_Index265 < COUNT_programmingBlock_KT + 0; A_Index265++) {
                if (langToConvertTo == langFileExtension_2) {
                    htCode = StrReplace(htCode, "programmingBlock_KT-programmingBlock_KT-AA" + STR(A_Index265 + 1) + "AA", keyWordCodeInTheTranspiledLangStartKT_2 + Chr(10) + programmingBlock_KT[A_Index265] + Chr(10) + keyWordCodeInTheTranspiledLangEndKT_2);
                } else {
                    htCode = StrReplace(htCode, "programmingBlock_KT-programmingBlock_KT-AA" + STR(A_Index265 + 1) + "AA", Chr(10));
                }
            }
        }
    }
    if (langToConvertTo == "rb") {
        if (COUNT_programmingBlock_RB != 0) {
            for (let A_Index266 = 0; A_Index266 < COUNT_programmingBlock_RB + 0; A_Index266++) {
                htCode = StrReplace(htCode, "programmingBlock_RB-programmingBlock_RB-AA" + STR(A_Index266 + 1) + "AA", programmingBlock_RB[A_Index266]);
            }
        }
    } else {
        if (COUNT_programmingBlock_RB != 0) {
            for (let A_Index267 = 0; A_Index267 < COUNT_programmingBlock_RB + 0; A_Index267++) {
                if (langToConvertTo == langFileExtension_2) {
                    htCode = StrReplace(htCode, "programmingBlock_RB-programmingBlock_RB-AA" + STR(A_Index267 + 1) + "AA", keyWordCodeInTheTranspiledLangStartRB_2 + Chr(10) + programmingBlock_RB[A_Index267] + Chr(10) + keyWordCodeInTheTranspiledLangEndRB_2);
                } else {
                    htCode = StrReplace(htCode, "programmingBlock_RB-programmingBlock_RB-AA" + STR(A_Index267 + 1) + "AA", Chr(10));
                }
            }
        }
    }
    if (langToConvertTo == "nim") {
        if (COUNT_programmingBlock_NIM != 0) {
            for (let A_Index268 = 0; A_Index268 < COUNT_programmingBlock_NIM + 0; A_Index268++) {
                htCode = StrReplace(htCode, "programmingBlock_NIM-programmingBlock_NIM-AA" + STR(A_Index268 + 1) + "AA", programmingBlock_NIM[A_Index268]);
            }
        }
    } else {
        if (COUNT_programmingBlock_NIM != 0) {
            for (let A_Index269 = 0; A_Index269 < COUNT_programmingBlock_NIM + 0; A_Index269++) {
                if (langToConvertTo == langFileExtension_2) {
                    htCode = StrReplace(htCode, "programmingBlock_NIM-programmingBlock_NIM-AA" + STR(A_Index269 + 1) + "AA", keyWordCodeInTheTranspiledLangStartNIM_2 + Chr(10) + programmingBlock_NIM[A_Index269] + Chr(10) + keyWordCodeInTheTranspiledLangEndNIM_2);
                } else {
                    htCode = StrReplace(htCode, "programmingBlock_NIM-programmingBlock_NIM-AA" + STR(A_Index269 + 1) + "AA", Chr(10));
                }
            }
        }
    }
    if (langToConvertTo == "ahk") {
        if (COUNT_programmingBlock_AHK != 0) {
            for (let A_Index270 = 0; A_Index270 < COUNT_programmingBlock_AHK + 0; A_Index270++) {
                htCode = StrReplace(htCode, "programmingBlock_AHK-programmingBlock_AHK-AA" + STR(A_Index270 + 1) + "AA", programmingBlock_AHK[A_Index270]);
            }
        }
    } else {
        if (COUNT_programmingBlock_AHK != 0) {
            for (let A_Index271 = 0; A_Index271 < COUNT_programmingBlock_AHK + 0; A_Index271++) {
                if (langToConvertTo == langFileExtension_2) {
                    htCode = StrReplace(htCode, "programmingBlock_AHK-programmingBlock_AHK-AA" + STR(A_Index271 + 1) + "AA", keyWordCodeInTheTranspiledLangStartAHK_2 + Chr(10) + programmingBlock_AHK[A_Index271] + Chr(10) + keyWordCodeInTheTranspiledLangEndAHK_2);
                } else {
                    htCode = StrReplace(htCode, "programmingBlock_AHK-programmingBlock_AHK-AA" + STR(A_Index271 + 1) + "AA", Chr(10));
                }
            }
        }
    }
    if (langToConvertTo == "swift") {
        if (COUNT_programmingBlock_SWIFT != 0) {
            for (let A_Index272 = 0; A_Index272 < COUNT_programmingBlock_SWIFT + 0; A_Index272++) {
                htCode = StrReplace(htCode, "programmingBlock_SWIFT-programmingBlock_SWIFT-AA" + STR(A_Index272 + 1) + "AA", programmingBlock_SWIFT[A_Index272]);
            }
        }
    } else {
        if (COUNT_programmingBlock_SWIFT != 0) {
            for (let A_Index273 = 0; A_Index273 < COUNT_programmingBlock_SWIFT + 0; A_Index273++) {
                if (langToConvertTo == langFileExtension_2) {
                    htCode = StrReplace(htCode, "programmingBlock_SWIFT-programmingBlock_SWIFT-AA" + STR(A_Index273 + 1) + "AA", keyWordCodeInTheTranspiledLangStartSWIFT_2 + Chr(10) + programmingBlock_SWIFT[A_Index273] + Chr(10) + keyWordCodeInTheTranspiledLangEndSWIFT_2);
                } else {
                    htCode = StrReplace(htCode, "programmingBlock_SWIFT-programmingBlock_SWIFT-AA" + STR(A_Index273 + 1) + "AA", Chr(10));
                }
            }
        }
    }
    if (langToConvertTo == "dart") {
        if (COUNT_programmingBlock_DART != 0) {
            for (let A_Index274 = 0; A_Index274 < COUNT_programmingBlock_DART + 0; A_Index274++) {
                htCode = StrReplace(htCode, "programmingBlock_DART-programmingBlock_DART-AA" + STR(A_Index274 + 1) + "AA", programmingBlock_DART[A_Index274]);
            }
        }
    } else {
        if (COUNT_programmingBlock_DART != 0) {
            for (let A_Index275 = 0; A_Index275 < COUNT_programmingBlock_DART + 0; A_Index275++) {
                if (langToConvertTo == langFileExtension_2) {
                    htCode = StrReplace(htCode, "programmingBlock_DART-programmingBlock_DART-AA" + STR(A_Index275 + 1) + "AA", keyWordCodeInTheTranspiledLangStartDART_2 + Chr(10) + programmingBlock_DART[A_Index275] + Chr(10) + keyWordCodeInTheTranspiledLangEndDART_2);
                } else {
                    htCode = StrReplace(htCode, "programmingBlock_DART-programmingBlock_DART-AA" + STR(A_Index275 + 1) + "AA", Chr(10));
                }
            }
        }
    }
    if (langToConvertTo == "ts") {
        if (COUNT_programmingBlock_TS != 0) {
            for (let A_Index276 = 0; A_Index276 < COUNT_programmingBlock_TS + 0; A_Index276++) {
                htCode = StrReplace(htCode, "programmingBlock_TS-programmingBlock_TS-AA" + STR(A_Index276 + 1) + "AA", programmingBlock_TS[A_Index276]);
            }
        }
    } else {
        if (COUNT_programmingBlock_TS != 0) {
            for (let A_Index277 = 0; A_Index277 < COUNT_programmingBlock_TS + 0; A_Index277++) {
                if (langToConvertTo == langFileExtension_2) {
                    htCode = StrReplace(htCode, "programmingBlock_TS-programmingBlock_TS-AA" + STR(A_Index277 + 1) + "AA", keyWordCodeInTheTranspiledLangStartTS_2 + Chr(10) + programmingBlock_TS[A_Index277] + Chr(10) + keyWordCodeInTheTranspiledLangEndTS_2);
                } else {
                    htCode = StrReplace(htCode, "programmingBlock_TS-programmingBlock_TS-AA" + STR(A_Index277 + 1) + "AA", Chr(10));
                }
            }
        }
    }
    if (langToConvertTo == "groovy") {
        if (COUNT_programmingBlock_GROOVY != 0) {
            for (let A_Index278 = 0; A_Index278 < COUNT_programmingBlock_GROOVY + 0; A_Index278++) {
                htCode = StrReplace(htCode, "programmingBlock_GROOVY-programmingBlock_GROOVY-AA" + STR(A_Index278 + 1) + "AA", programmingBlock_GROOVY[A_Index278]);
            }
        }
    } else {
        if (COUNT_programmingBlock_GROOVY != 0) {
            for (let A_Index279 = 0; A_Index279 < COUNT_programmingBlock_GROOVY + 0; A_Index279++) {
                if (langToConvertTo == langFileExtension_2) {
                    htCode = StrReplace(htCode, "programmingBlock_GROOVY-programmingBlock_GROOVY-AA" + STR(A_Index279 + 1) + "AA", keyWordCodeInTheTranspiledLangStartGROOVY_2 + Chr(10) + programmingBlock_GROOVY[A_Index279] + Chr(10) + keyWordCodeInTheTranspiledLangEndGROOVY_2);
                } else {
                    htCode = StrReplace(htCode, "programmingBlock_GROOVY-programmingBlock_GROOVY-AA" + STR(A_Index279 + 1) + "AA", Chr(10));
                }
            }
        }
    }
    ...
    if (langToConvertTo == langFileExtension_2) {
        if (COUNT_programmingBlock_HTVM != 0) {
            for (let A_Index280 = 0; A_Index280 < COUNT_programmingBlock_HTVM + 0; A_Index280++) {
                htCode = StrReplace(htCode, "programmingBlock_HTVM-programmingBlock_HTVM-AA" + STR(A_Index280 + 1) + "AA", programmingBlock_HTVM[A_Index280]);
            }
        }
    } else {
        if (COUNT_programmingBlock_HTVM != 0) {
            for (let A_Index281 = 0; A_Index281 < COUNT_programmingBlock_HTVM + 0; A_Index281++) {
                htCode = StrReplace(htCode, "programmingBlock_HTVM-programmingBlock_HTVM-AA" + STR(A_Index281 + 1) + "AA", Chr(10));
            }
        }
    }
    if (langToConvertTo != langFileExtension_2) {
        for (let A_Index282 = 0; A_Index282 < theIdNumOfThe34 + 0; A_Index282++) {
            if (theIdNumOfThe34 == A_Index282 + 1) {
                if (langToConvertTo == langFileExtension_2) {
                    if (keyWordEscpaeChar_2 == "\\" && keyWordEscpaeChar != "\\") {
                        htCode = StrReplace(htCode, "fihuiuuhuuhtheidFor--asds" + str21 + "as--" + str21 + "theuhturtyphoutr--" + Chr(65) + Chr(65) + STR(A_Index282 + 1) + Chr(65) + Chr(65), StrReplace(StrReplace(fString(theIdNumOfThe34theVar[A_Index282 + 1]), "\\", "\\\\"), keyWordEscpaeChar, keyWordEscpaeChar_2) + Chr(34));
                    } else {
                        htCode = StrReplace(htCode, "fihuiuuhuuhtheidFor--asds" + str21 + "as--" + str21 + "theuhturtyphoutr--" + Chr(65) + Chr(65) + STR(A_Index282 + 1) + Chr(65) + Chr(65), StrReplace(fString(theIdNumOfThe34theVar[A_Index282 + 1]), keyWordEscpaeChar, keyWordEscpaeChar_2) + Chr(34));
                    }
                } else {
                    htCode = StrReplace(htCode, "fihuiuuhuuhtheidFor--asds" + str21 + "as--" + str21 + "theuhturtyphoutr--" + Chr(65) + Chr(65) + STR(A_Index282 + 1) + Chr(65) + Chr(65), StrReplace(fString(theIdNumOfThe34theVar[A_Index282 + 1]), keyWordEscpaeChar, "\\") + Chr(34));
                }
            } else {
                if (langToConvertTo == langFileExtension_2) {
                    if (keyWordEscpaeChar_2 == "\\" && keyWordEscpaeChar != "\\") {
                        htCode = StrReplace(htCode, "fihuiuuhuuhtheidFor--asds" + str21 + "as--" + str21 + "theuhturtyphoutr--" + Chr(65) + Chr(65) + STR(A_Index282 + 1) + Chr(65) + Chr(65), StrReplace(StrReplace(fString(theIdNumOfThe34theVar[A_Index282 + 1]), "\\", "\\\\"), keyWordEscpaeChar, keyWordEscpaeChar_2));
                    } else {
                        htCode = StrReplace(htCode, "fihuiuuhuuhtheidFor--asds" + str21 + "as--" + str21 + "theuhturtyphoutr--" + Chr(65) + Chr(65) + STR(A_Index282 + 1) + Chr(65) + Chr(65), StrReplace(fString(theIdNumOfThe34theVar[A_Index282 + 1]), keyWordEscpaeChar, keyWordEscpaeChar_2));
                    }
                } else {
                    htCode = StrReplace(htCode, "fihuiuuhuuhtheidFor--asds" + str21 + "as--" + str21 + "theuhturtyphoutr--" + Chr(65) + Chr(65) + STR(A_Index282 + 1) + Chr(65) + Chr(65), StrReplace(fString(theIdNumOfThe34theVar[A_Index282 + 1]), keyWordEscpaeChar, "\\"));
                }
            }
        }
    }
    ...
    var correctLang = 0;
    items283 = LoopParseFunc(instructions, "\n", "\r")
    for (let A_Index283 = 0; A_Index283 < items283.length + 0; A_Index283++) {
        const A_LoopField283 = items283[A_Index283 - 0];
        if (Trim(A_LoopField283) == "funcEND======================funcEND==============") {
            areWeInAFuncFromInstructions = 0;
            areWeInAFuncFromInstructionsLineNum = 0;
        ...
        if (areWeInAFuncFromInstructions == 1) {
            if (areWeInAFuncFromInstructionsLineNum == 1) {
                // name of the func
                funcLangHolder = StringTrimLeft(A_LoopField283, 5);
            ...
            if (areWeInAFuncFromInstructionsLineNum == 2) {
                // name of the func
                funcNameHolder = StringTrimLeft(A_LoopField283, 5);
                if (correctLang == 1 && InStr(htCode, Trim(funcNameHolder))) {
            ...
            if (areWeInAFuncFromInstructionsLineNum == 3) {
                // all libs
                funcLibsHolder = StringTrimLeft(A_LoopField283, 5);
                if (correctLang == 1 && InStr(htCode, Trim(funcNameHolder))) {
            ...
            if (areWeInAFuncFromInstructionsLineNum == 4) {
                // func description
                funcDescriptionHolder = StringTrimLeft(A_LoopField283, 12);
                if (correctLang == 1 && InStr(htCode, Trim(funcNameHolder))) {
            ...
            if (areWeInAFuncFromInstructionsLineNum >= 5) {
                // the full func
                if (correctLang == 1 && InStr(htCode, Trim(funcNameHolder))) {
                    funcFuncHolder += A_LoopField283 + Chr(10);
                }
            }
            //MsgBox, % A_LoopField283
            areWeInAFuncFromInstructionsLineNum++;
        }
        if (Trim(A_LoopField283) == "func======================func==============") {
    ...
    if (!(HTVM_Size(allFuncNames) <= 0)) {
        for (let A_Index284 = 0; A_Index284 < HTVM_Size(allFuncNames) + 0; A_Index284++) {
            if (InStr(htCode, allFuncNames[A_Index284]) + "(") {
                //MsgBox, % allFuncNames[A_Index284]
                allFuncsToPutAtTop += allFuncs[A_Index284] + Chr(10);
                if (Trim(allFuncLibs[A_Index284]) != "null") {
                    allLibsToPutAtTop += allFuncLibs[A_Index284] + "|";
                }
            }
        }
        allLibsToPutAtTop = StringTrimRight(allLibsToPutAtTop, 1);
        var allLibsToPutAtTopTEMP = "";
        items285 = LoopParseFunc(allLibsToPutAtTop, "|")
        for (let A_Index285 = 0; A_Index285 < items285.length + 0; A_Index285++) {
            const A_LoopField285 = items285[A_Index285 - 0];
            allLibsToPutAtTopTEMP += A_LoopField285 + Chr(10);
        }
    ...
    if (langToConvertTo != langFileExtension_2) {
        for (let A_Index286 = 0; A_Index286 < HTVM_Size(ospDic1) + 0; A_Index286++) {
            str00 = ospDic1[A_Index286];
            if (langToConvertTo == "rb") {
    ...
    if (size_ospDic != 0) {
        for (let A_Index287 = 0; A_Index287 < size_ospDic + 0; A_Index287++) {
            HTVM_Pop(ospDic);
        }
    }
    //;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;
    ...
    if (size_ospDic1 != 0) {
        for (let A_Index288 = 0; A_Index288 < size_ospDic1 + 0; A_Index288++) {
            HTVM_Pop(ospDic1);
        }
    }
    //;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;
    ...
    if (size_ospDic2 != 0) {
        for (let A_Index289 = 0; A_Index289 < size_ospDic2 + 0; A_Index289++) {
            HTVM_Pop(ospDic2);
        }
    }
    availableFuncsInHTVMInHTVM = "";
    ...
    for (let A_Index290 = 0; A_Index290 < theIdNumOfThe34 + 0; A_Index290++) {
        if (theIdNumOfThe34 == A_Index290 + 1) {
            if (langToConvertTo == langFileExtension_2) {
                if (keyWordEscpaeChar_2 == "\\" && keyWordEscpaeChar != "\\") {
                    htCode = StrReplace(htCode, "ihuiuuhuuhtheidFor--asds" + str21 + "as--" + str21 + "theuhturtyphoutr--" + Chr(65) + Chr(65) + STR(A_Index290 + 1) + Chr(65) + Chr(65), StrReplace(StrReplace(theIdNumOfThe34theVar[A_Index290 + 1], "\\", "\\\\"), keyWordEscpaeChar, keyWordEscpaeChar_2) + Chr(34));
                } else {
                    htCode = StrReplace(htCode, "ihuiuuhuuhtheidFor--asds" + str21 + "as--" + str21 + "theuhturtyphoutr--" + Chr(65) + Chr(65) + STR(A_Index290 + 1) + Chr(65) + Chr(65), StrReplace(theIdNumOfThe34theVar[A_Index290 + 1], keyWordEscpaeChar, keyWordEscpaeChar_2) + Chr(34));
                }
            } else {
                htCode = StrReplace(htCode, "ihuiuuhuuhtheidFor--asds" + str21 + "as--" + str21 + "theuhturtyphoutr--" + Chr(65) + Chr(65) + STR(A_Index290 + 1) + Chr(65) + Chr(65), StrReplace(theIdNumOfThe34theVar[A_Index290 + 1], keyWordEscpaeChar, "\\") + Chr(34));
            }
        } else {
            if (langToConvertTo == langFileExtension_2) {
                if (keyWordEscpaeChar_2 == "\\" && keyWordEscpaeChar != "\\") {
                    htCode = StrReplace(htCode, "ihuiuuhuuhtheidFor--asds" + str21 + "as--" + str21 + "theuhturtyphoutr--" + Chr(65) + Chr(65) + STR(A_Index290 + 1) + Chr(65) + Chr(65), StrReplace(StrReplace(theIdNumOfThe34theVar[A_Index290 + 1], "\\", "\\\\"), keyWordEscpaeChar, keyWordEscpaeChar_2));
                } else {
                    htCode = StrReplace(htCode, "ihuiuuhuuhtheidFor--asds" + str21 + "as--" + str21 + "theuhturtyphoutr--" + Chr(65) + Chr(65) + STR(A_Index290 + 1) + Chr(65) + Chr(65), StrReplace(theIdNumOfThe34theVar[A_Index290 + 1], keyWordEscpaeChar, keyWordEscpaeChar_2));
                }
            } else {
                htCode = StrReplace(htCode, "ihuiuuhuuhtheidFor--asds" + str21 + "as--" + str21 + "theuhturtyphoutr--" + Chr(65) + Chr(65) + STR(A_Index290 + 1) + Chr(65) + Chr(65), StrReplace(theIdNumOfThe34theVar[A_Index290 + 1], keyWordEscpaeChar, "\\"));
            }
        }
    }
    if (langToConvertTo == langFileExtension_2) {
    ...
    if (isNotHTVMfile2 == 0) {
        ...
        if (langToConvertTo == langFileExtension_2) {
            for (let A_Index291 = 0; A_Index291 < HTVM_Size(weAreInMLSarr) + 0; A_Index291++) {
                htCode = StrReplace(htCode, "aejsoydoxubviycbdgsut7eoy-ihsxbvhowadesdio6376e7wuwyuau--szd--AA" + STR(A_Index291) + "AA", weAreInMLSarr[A_Index291]);
            }
    ...
    if (size_weAreInMLSarr != 0) {
        for (let A_Index292 = 0; A_Index292 < size_weAreInMLSarr + 0; A_Index292++) {
...
function HTVMv2() {
    ...
    items293 = LoopParseFunc(str0)
    for (let A_Index293 = 0; A_Index293 < items293.length + 0; A_Index293++) {
        const A_LoopField293 = items293[A_Index293 - 0];
        str00 = Trim(A_LoopField293);
    ...
    if (HTVM_getLang_HTVM() == "cpp" || HTVM_getLang_HTVM() == "py") {
        ...
        items294 = LoopParseFunc(allArgs, "\n", "\r")
        for (let A_Index294 = 0; A_Index294 < items294.length + 0; A_Index294++) {
            const A_LoopField294 = items294[A_Index294 - 0];
            if (A_Index294 == 0) {
                numOfParams++;
                argCODE = FileRead(Trim(A_LoopField294));
                argCODEfile = Trim(A_LoopField294);
            }
            else if (A_Index294 == 1) {
                numOfParams++;
                argHTVMinstr = Trim(A_LoopField294);
            }
            else if (A_Index294 == 2) {
                numOfParams++;
                argLangTo = Trim(A_LoopField294);
            } else {
                numOfParams++;
                HTVM_Append(argHTVMinstrMORE, Trim(A_LoopField294));
            }
        }
        //print("===============123431234===========start=====")
        for (let A_Index295 = 0; A_Index295 < HTVM_Size(argHTVMinstrMORE) + 0; A_Index295++) {
            //print(argHTVMinstrMORE[A_Index295])
        }
        //print("===============123431234==========end======")
