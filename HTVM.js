...
function compiler(htCode, allInstructionFile, mode, langToConvertToParam = "") {
    ...
    */
    ...
    if (isNotHTVMfile == 0) {
        ...
        if (langToConvertTo != "ahk" && langToConvertTo != langFileExtension_2) {
            ...
            if (haveWeEverUsedAloop == 1) {
                ...
                items229 = LoopParseFunc(htCodeLoopfixa, "\n", "\r")
    ...
    if (langToConvertTo == "go") {
        ...
        for (let A_Index251 = 0; A_Index251 < items251.length + 0; A_Index251++) {
            ...
            if (Trim(A_LoopField251) == "}" && SubStr(Trim(fixGoManGoIsSoAnnoyingBroooFurure[A_Index251 + 1]), 1, StrLen("else if ")) == "else if ") {
                ...
                items252 = LoopParseFunc(fixGoManGoIsSoAnnoyingBroooFurure[A_Index251 + 1])
    ...
    var correctLang = 0;
    items288 = LoopParseFunc(instructions, "\n", "\r")
    for (let A_Index288 = 0; A_Index288 < items288.length + 0; A_Index288++) {
    ...
    if (!(HTVM_Size(allFuncNames) <= 0)) {
        for (let A_Index289 = 0; A_Index289 < HTVM_Size(allFuncNames) + 0; A_Index289++) {
        ...
        items290 = LoopParseFunc(allLibsToPutAtTop, "|")
    ...
    if (langToConvertTo != langFileExtension_2) {
        for (let A_Index291 = 0; A_Index291 < HTVM_Size(ospDic1) + 0; A_Index291++) {
    ...
    if (size_ospDic != 0) {
        for (let A_Index292 = 0; A_Index292 < size_ospDic + 0; A_Index292++) {
            HTVM_Pop(ospDic);
        }
    }
    //;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;
    ...
    if (size_ospDic1 != 0) {
        for (let A_Index293 = 0; A_Index293 < size_ospDic1 + 0; A_Index293++) {
            HTVM_Pop(ospDic1);
        }
    }
    //;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;
    ...
    if (size_ospDic2 != 0) {
        for (let A_Index294 = 0; A_Index294 < size_ospDic2 + 0; A_Index294++) {
            HTVM_Pop(ospDic2);
        }
    }
    availableFuncsInHTVMInHTVM = "";
    ...
    for (let A_Index295 = 0; A_Index295 < theIdNumOfThe34 + 0; A_Index295++) {
    ...
    if (isNotHTVMfile2 == 0) {
        ...
        if (langToConvertTo == langFileExtension_2) {
            for (let A_Index296 = 0; A_Index296 < HTVM_Size(weAreInMLSarr) + 0; A_Index296++) {
    ...
    if (size_weAreInMLSarr != 0) {
        for (let A_Index297 = 0; A_Index297 < size_weAreInMLSarr + 0; A_Index297++) {
...
function HTVMv2() {
    ...
    if (HTVM_getLang_HTVM() == "cpp" || HTVM_getLang_HTVM() == "py") {
        ...
        items299 = LoopParseFunc(allArgs, "\n", "\r")
        ...
        for (let A_Index300 = 0; A_Index300 < HTVM_Size(argHTVMinstrMORE) + 0; A_Index300++) {
