...
function initializeBackendAndGUIInteractionWithLabelSubroutineLikeButConvertsToFunction(codeToConvert) {
    var out = "";
    var temp = "";
    items149 = LoopParseFunc(codeToConvert, "\n", "\r")
...
function handleMultiLineStrings(code) {
    ...
    for (let A_Index151 = 0; A_Index151 < items151.length + 0; A_Index151++) {
        ...
        HTVM_Append(lookAhead, Trim(A_LoopField151));
    }
    ...
    for (let A_Index152 = 0; A_Index152 < items152.length + 0; A_Index152++) {
        ...
        if (langToConvertTo == langFileExtension_2) {
            //;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;
            //;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;
            if (weAreIn == 1 && Trim(A_LoopField152) == "mle") {
            ...
            if (weAreIn == 1) {
                ...
                weAreInMLSarrTemp += A_LoopField152;
            }
            if (SubStrLastChars(Trim(A_LoopField152), 4) == " mls" || SubStrLastChars(Trim(A_LoopField152), 5) == " fmls") {
                weAreIn = 1;
                out += A_LoopField152 + Chr(10);
            }
            if (weAreIn == 0) {
                out += A_LoopField152 + Chr(10);
            }
            ...
            if (weAreIn == 1 && Trim(A_LoopField152) == "HTVM-newLineAHw4y9w730q8237w4e8srhgesw0s8rgw04e5g08sgr0e8srdgt--YOU-BETERNOTUSETHISAH--aesdfssz") {
                if (lookAhead[A_Index152 + 1] == "HTVM-newLineAHw4y9w730q8237w4e8srhgesw0s8rgw04e5g08sgr0e8srdgt--YOU-BETERNOTUSETHISAH--aesdfssz") {
                    out += keyWordEscpaeChar + "n";
                }
            }
            if (weAreIn == 1 && Trim(A_LoopField152) != "HTVM-newLineAHw4y9w730q8237w4e8srhgesw0s8rgw04e5g08sgr0e8srdgt--YOU-BETERNOTUSETHISAH--aesdfssz") {
                out += A_LoopField152 + keyWordEscpaeChar + "n";
            }
            if (SubStrLastChars(Trim(A_LoopField152), 4) == " mls" || SubStrLastChars(Trim(A_LoopField152), 5) == " fmls") {
                weAreIn = 1;
                if (SubStrLastChars(Trim(A_LoopField152), 5) == " fmls") {
                    out += TrimR(StringTrimRight(TrimR(A_LoopField152), 3)) + Chr(34);
                } else {
                    out += TrimR(StringTrimRight(TrimR(A_LoopField152), 3)) + " " + Chr(34);
                }
            }
            if (weAreIn == 0) {
                if (Trim(A_LoopField152) != "mle") {
                    out += A_LoopField152 + Chr(10);
                }
            }
            //;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;
...
function slidingWinFixCommentsRmFUNC(line) {
    ...
    items153 = LoopParseFunc(line)
    for (let A_Index153 = 0; A_Index153 < items153.length + 0; A_Index153++) {
        const A_LoopField153 = items153[A_Index153 - 0];
        HTVM_Append(slidingWinFixCommentsRmFFFF, A_LoopField153);
    }
    for (let A_Index154 = 0; A_Index154 < StrLen(keyWordComment + keyWordComment) + 2 + 0; A_Index154++) {
...
function fixIfElseIfMultiLineStatementHELP(aloopfield) {
    var out = "";
    var count = 0;
    items155 = LoopParseFunc(aloopfield)
...
function fixIfElseIfMultiLineStatement(someHTVMcode) {
    var doWeExitInsta = 1;
    items156 = LoopParseFunc(someHTVMcode, "\n", "\r")
    ...
    for (let A_Index157 = 0; A_Index157 < 10000 + 0; A_Index157++) {
        out2 = out;
        out = "";
        doWeExit = false;
        items158 = LoopParseFunc(out2, "\n", "\r")
        for (let A_Index158 = 0; A_Index158 < items158.length + 0; A_Index158++) {
            const A_LoopField158 = items158[A_Index158 - 0];
            if (SubStr(StrLower(Trim(A_LoopField158)), 1, StrLen(keyWordIF)) == keyWordIF) {
                doWeJustExit = 1;
                if (fixIfElseIfMultiLineStatementHELP(A_LoopField158) == "") {
                    out += Trim(A_LoopField158) + " ";
                    doWeExit = false;
                    doWeExitHELP--;
                } else {
                    doWeExit = true;
                    out += Trim(A_LoopField158) + Chr(10);
                }
            }
            else if (SubStr(StrLower(Trim(A_LoopField158)), 1, StrLen(keyWordWhileLoop)) == keyWordWhileLoop) {
                doWeJustExit = 1;
                if (fixIfElseIfMultiLineStatementHELP(A_LoopField158) == "") {
                    out += Trim(A_LoopField158) + " ";
                    doWeExit = false;
            ...
            else if (SubStr(StrLower(Trim(A_LoopField158)), 1, StrLen(keyWordElseIf)) == keyWordElseIf) {
                doWeJustExit = 1;
                if (fixIfElseIfMultiLineStatementHELP(A_LoopField158) == "") {
                    out += Trim(A_LoopField158) + " ";
                    doWeExit = false;
...
function compiler(htCode, allInstructionFile, mode, langToConvertToParam = "") {
    ...
    for (let A_Index159 = 0; A_Index159 < items159.length + 0; A_Index159++) {
        ...
        if (A_Index159 == 0) {
            langToConvertTo = Trim(A_LoopField159);
        }
        if (A_Index159 == 1) {
            langFileExtension = Trim(A_LoopField159);
        }
        if (A_Index159 == 2) {
            commands = Trim(A_LoopField159);
        }
        if (A_Index159 == 3) {
            keyWordAlliance = Trim(A_LoopField159) + " ";
        }
        if (A_Index159 == 4) {
            keyWordCrew = Trim(A_LoopField159) + " ";
        }
        if (A_Index159 == 5) {
            keyWordProc = Trim(A_LoopField159) + " ";
        }
        if (A_Index159 == 6) {
            keyWordStruct = Trim(A_LoopField159) + " ";
        }
        if (A_Index159 == 7) {
            keyWordProp = Trim(A_LoopField159) + " ";
        }
        if (A_Index159 == 8) {
            keyWordThis = Trim(A_LoopField159);
        }
        if (A_Index159 == 9) {
            keyWordInclude = Trim(A_LoopField159);
        }
        if (A_Index159 == 10) {
            keyWordCodeInTheTranspiledLangStart = Trim(A_LoopField159);
        }
        if (A_Index159 == 11) {
            keyWordCodeInTheTranspiledLangEnd = Trim(A_LoopField159);
        }
        if (A_Index159 == 12) {
            keyWordCodeInTheTranspiledLangStartCPP = Trim(A_LoopField159);
        }
        if (A_Index159 == 13) {
            keyWordCodeInTheTranspiledLangEndCPP = Trim(A_LoopField159);
        }
        if (A_Index159 == 14) {
            keyWordCodeInTheTranspiledLangStartPY = Trim(A_LoopField159);
        }
        if (A_Index159 == 15) {
            keyWordCodeInTheTranspiledLangEndPY = Trim(A_LoopField159);
        }
        if (A_Index159 == 16) {
            keyWordCodeInTheTranspiledLangStartJS = Trim(A_LoopField159);
        }
        if (A_Index159 == 17) {
            keyWordCodeInTheTranspiledLangEndJS = Trim(A_LoopField159);
        }
        if (A_Index159 == 18) {
            keyWordCodeInTheTranspiledLangStartGO = Trim(A_LoopField159);
        }
        if (A_Index159 == 19) {
            keyWordCodeInTheTranspiledLangEndGO = Trim(A_LoopField159);
        }
        if (A_Index159 == 20) {
            keyWordCodeInTheTranspiledLangStartLUA = Trim(A_LoopField159);
        }
        if (A_Index159 == 21) {
            keyWordCodeInTheTranspiledLangEndLUA = Trim(A_LoopField159);
        }
        if (A_Index159 == 22) {
            keyWordCodeInTheTranspiledLangStartCS = Trim(A_LoopField159);
        }
        if (A_Index159 == 23) {
            keyWordCodeInTheTranspiledLangEndCS = Trim(A_LoopField159);
        }
        if (A_Index159 == 24) {
            keyWordCodeInTheTranspiledLangStartJAVA = Trim(A_LoopField159);
        }
        if (A_Index159 == 25) {
            keyWordCodeInTheTranspiledLangEndJAVA = Trim(A_LoopField159);
        }
        if (A_Index159 == 26) {
            keyWordCodeInTheTranspiledLangStartKT = Trim(A_LoopField159);
        }
        if (A_Index159 == 27) {
            keyWordCodeInTheTranspiledLangEndKT = Trim(A_LoopField159);
        }
        if (A_Index159 == 28) {
            keyWordCodeInTheTranspiledLangStartRB = Trim(A_LoopField159);
        }
        if (A_Index159 == 29) {
            keyWordCodeInTheTranspiledLangEndRB = Trim(A_LoopField159);
        }
        if (A_Index159 == 30) {
            keyWordCodeInTheTranspiledLangStartNIM = Trim(A_LoopField159);
        }
        if (A_Index159 == 31) {
            keyWordCodeInTheTranspiledLangEndNIM = Trim(A_LoopField159);
        }
        if (A_Index159 == 32) {
            keyWordCodeInTheTranspiledLangStartAHK = Trim(A_LoopField159);
        }
        if (A_Index159 == 33) {
            keyWordCodeInTheTranspiledLangEndAHK = Trim(A_LoopField159);
        }
        if (A_Index159 == 34) {
            keyWordCodeInTheTranspiledLangStartSWIFT = Trim(A_LoopField159);
        }
        if (A_Index159 == 35) {
            keyWordCodeInTheTranspiledLangEndSWIFT = Trim(A_LoopField159);
        }
        if (A_Index159 == 36) {
            keyWordCodeInTheTranspiledLangStartDART = Trim(A_LoopField159);
        }
        if (A_Index159 == 37) {
            keyWordCodeInTheTranspiledLangEndDART = Trim(A_LoopField159);
        }
        if (A_Index159 == 38) {
            keyWordCodeInTheTranspiledLangStartTS = Trim(A_LoopField159);
        }
        if (A_Index159 == 39) {
            keyWordCodeInTheTranspiledLangEndTS = Trim(A_LoopField159);
        }
        if (A_Index159 == 40) {
            keyWordCodeInTheTranspiledLangStartGROOVY = Trim(A_LoopField159);
        }
        if (A_Index159 == 41) {
            keyWordCodeInTheTranspiledLangEndGROOVY = Trim(A_LoopField159);
        }
        if (A_Index159 == 42) {
            keyWordCodeInTheTranspiledLangStartHTVM = Trim(A_LoopField159);
        }
        if (A_Index159 == 43) {
            keyWordCodeInTheTranspiledLangEndHTVM = Trim(A_LoopField159);
        }
        if (A_Index159 == 44) {
            keyWordCodeInHTVMstart = Trim(A_LoopField159);
        }
        if (A_Index159 == 45) {
            keyWordCodeInHTVMend = Trim(A_LoopField159);
        }
        if (A_Index159 == 46) {
            keyWordCurlyBraceOpen = Trim(A_LoopField159);
        }
        if (A_Index159 == 47) {
            keyWordCurlyBraceClose = Trim(A_LoopField159);
        }
        if (A_Index159 == 48) {
            keyWordNull = Trim(A_LoopField159);
        }
        if (A_Index159 == 49) {
            keyWordTrue = Trim(A_LoopField159);
        }
        if (A_Index159 == 50) {
            keyWordFalse = Trim(A_LoopField159);
        }
        if (A_Index159 == 51) {
            keyWordVoid = Trim(A_LoopField159);
        }
        if (A_Index159 == 52) {
            keyWordDouble = Trim(A_LoopField159);
        }
        if (A_Index159 == 53) {
            keyWordChar = Trim(A_LoopField159);
        }
        if (A_Index159 == 54) {
            keyWordUint8 = Trim(A_LoopField159);
        }
        if (A_Index159 == 55) {
            keyWordUint16 = Trim(A_LoopField159);
        }
        if (A_Index159 == 56) {
            keyWordUint32 = Trim(A_LoopField159);
        }
        if (A_Index159 == 57) {
            keyWordUint64 = Trim(A_LoopField159);
        }
        if (A_Index159 == 58) {
            keyWordINT = Trim(A_LoopField159);
        }
        if (A_Index159 == 59) {
            keyWordSTR = Trim(A_LoopField159);
        }
        if (A_Index159 == 60) {
            keyWordBOOL = Trim(A_LoopField159);
        }
        if (A_Index159 == 61) {
            keyWordFLOAT = Trim(A_LoopField159);
        }
        if (A_Index159 == 62) {
            keyWordINT8 = Trim(A_LoopField159);
        }
        if (A_Index159 == 63) {
            keyWordINT16 = Trim(A_LoopField159);
        }
        if (A_Index159 == 64) {
            keyWordINT32 = Trim(A_LoopField159);
        }
        if (A_Index159 == 65) {
            keyWordINT64 = Trim(A_LoopField159);
        }
        if (A_Index159 == 66) {
            keyWordIF = Trim(A_LoopField159) + " ";
        }
        if (A_Index159 == 67) {
            keyWordElseIf = Trim(A_LoopField159) + " ";
        }
        if (A_Index159 == 68) {
            keyWordElse = Trim(A_LoopField159);
        }
        if (A_Index159 == 69) {
            keyWordWhileLoop = Trim(A_LoopField159) + " ";
        }
        if (A_Index159 == 70) {
            keyWordLoopInfinite = Trim(A_LoopField159);
        }
        if (A_Index159 == 71) {
            keyWordLoop = Trim(A_LoopField159) + " ";
        }
        if (A_Index159 == 72) {
            keyWordLoopParse = Trim(A_LoopField159) + " ";
        }
        if (A_Index159 == 73) {
            keyWordContinue = Trim(A_LoopField159);
        }
        if (A_Index159 == 74) {
            keyWordBreak = Trim(A_LoopField159);
        }
        if (A_Index159 == 75) {
            keyWordFunc = Trim(A_LoopField159) + " ";
        }
        if (A_Index159 == 76) {
            keyWordAwait = Trim(A_LoopField159) + " ";
        }
        if (A_Index159 == 77) {
            keyWordAsync = Trim(A_LoopField159) + " ";
        }
        if (A_Index159 == 78) {
            keyWordThrow = Trim(A_LoopField159) + " ";
        }
        if (A_Index159 == 79) {
            keyWordErrorMsg = Trim(A_LoopField159);
        }
        if (A_Index159 == 80) {
            keyWordTry = Trim(A_LoopField159);
        }
        if (A_Index159 == 81) {
            keyWordCatch = Trim(A_LoopField159) + " ";
        }
        if (A_Index159 == 82) {
            keyWordFinally = Trim(A_LoopField159);
        }
        if (A_Index159 == 83) {
            keyWordReturnStatement = Trim(A_LoopField159) + " ";
        }
        if (A_Index159 == 84) {
            keyWordArrayAppend = Trim(A_LoopField159);
        }
        if (A_Index159 == 85) {
            keyWordArrayPop = Trim(A_LoopField159);
        }
        if (A_Index159 == 86) {
            keyWordArraySize = Trim(A_LoopField159);
        }
        if (A_Index159 == 87) {
            keyWordArrayInsert = Trim(A_LoopField159);
        }
        if (A_Index159 == 88) {
            keyWordArrayRemove = Trim(A_LoopField159);
        }
        if (A_Index159 == 89) {
            keyWordArrayIndexOf = Trim(A_LoopField159);
        }
        if (A_Index159 == 90) {
            keyWordArrayDefinition = Trim(A_LoopField159);
        }
        if (A_Index159 == 91) {
            keyWordArrayOfIntegersDefinition = Trim(A_LoopField159);
        }
        if (A_Index159 == 92) {
            keyWordArrayOfStringsDefinition = Trim(A_LoopField159);
        }
        if (A_Index159 == 93) {
            keyWordArrayOfFloatingPointNumbersDefinition = Trim(A_LoopField159);
        }
        if (A_Index159 == 94) {
            keyWordArrayOfBooleansDefinition = Trim(A_LoopField159);
        }
        if (A_Index159 == 95) {
            keyWordVar = Trim(A_LoopField159) + " ";
        }
        if (A_Index159 == 96) {
            keyWordLet = Trim(A_LoopField159) + " ";
        }
        if (A_Index159 == 97) {
            keyWordConst = Trim(A_LoopField159) + " ";
        }
        if (A_Index159 == 98) {
            keyWordEnd = Trim(A_LoopField159);
        }
        if (A_Index159 == 99) {
            keyWordGlobal = Trim(A_LoopField159) + " ";
        }
        if (A_Index159 == 100) {
            keyWordComment = Trim(A_LoopField159);
        }
        if (A_Index159 == 101) {
            keyWordCommentOpenMultiLine = Trim(A_LoopField159);
        }
        if (A_Index159 == 102) {
            keyWordCommentCloseMultiLine = Trim(A_LoopField159);
        }
        if (A_Index159 == 103) {
            keyWordEscpaeChar = Trim(A_LoopField159);
        }
        if (A_Index159 == 104) {
            keyWordMainLabel = Trim(A_LoopField159);
        }
        if (A_Index159 == 105) {
            keyWordConcat = Trim(A_LoopField159);
        }
        if (A_Index159 == 106) {
            keyWordAdd = Trim(A_LoopField159);
        }
        if (A_Index159 == 107) {
            keyWordSub = Trim(A_LoopField159);
        }
        if (A_Index159 == 108) {
            keyWordMul = Trim(A_LoopField159);
        }
        if (A_Index159 == 109) {
            keyWordDiv = Trim(A_LoopField159);
        }
        if (A_Index159 == 110) {
            keyWordMod = Trim(A_LoopField159);
        }
        if (A_Index159 == 111) {
            keyWordExp = Trim(A_LoopField159);
        }
        if (A_Index159 == 112) {
            keyWordEqual = Trim(A_LoopField159);
        }
        if (A_Index159 == 113) {
            keyWordStrictEqual = Trim(A_LoopField159);
        }
        if (A_Index159 == 114) {
            keyWordNotEqual = Trim(A_LoopField159);
        }
        if (A_Index159 == 115) {
            keyWordGreater = Trim(A_LoopField159);
        }
        if (A_Index159 == 116) {
            keyWordLess = Trim(A_LoopField159);
        }
        if (A_Index159 == 117) {
            keyWordGreaterEqual = Trim(A_LoopField159);
        }
        if (A_Index159 == 118) {
            keyWordLessEqual = Trim(A_LoopField159);
        }
        if (A_Index159 == 119) {
            keyWordAnd = Trim(A_LoopField159);
        }
        if (A_Index159 == 120) {
            keyWordOr = Trim(A_LoopField159);
        }
        if (A_Index159 == 121) {
            keyWordNot = Trim(A_LoopField159);
        }
        if (A_Index159 == 122) {
            keyWordBitAnd = Trim(A_LoopField159);
        }
        if (A_Index159 == 123) {
            keyWordBitOr = Trim(A_LoopField159);
        }
        if (A_Index159 == 124) {
            keyWordBitXor = Trim(A_LoopField159);
        }
        if (A_Index159 == 125) {
            keyWordBitNot = Trim(A_LoopField159);
        }
        if (A_Index159 == 126) {
            keyWordShiftLeft = Trim(A_LoopField159);
        }
        if (A_Index159 == 127) {
            keyWordShiftRight = Trim(A_LoopField159);
        }
        if (A_Index159 == 128) {
            keyWordShiftUnsignedRight = Trim(A_LoopField159);
        }
        if (A_Index159 == 129) {
            keyWordAssign = Trim(A_LoopField159);
        }
        if (A_Index159 == 130) {
            keyWordAssignAdd = Trim(A_LoopField159);
        }
        if (A_Index159 == 131) {
            keyWordAssignConcat = Trim(A_LoopField159);
        }
        if (A_Index159 == 132) {
            keyWordAssignSub = Trim(A_LoopField159);
        }
        if (A_Index159 == 133) {
            keyWordAssignMul = Trim(A_LoopField159);
        }
        if (A_Index159 == 134) {
            keyWordAssignDiv = Trim(A_LoopField159);
        }
        if (A_Index159 == 135) {
            keyWordAssignMod = Trim(A_LoopField159);
        }
        if (A_Index159 == 136) {
            keyWordAssignShiftLeft = Trim(A_LoopField159);
        }
        if (A_Index159 == 137) {
            keyWordAssignShiftRight = Trim(A_LoopField159);
        }
        if (A_Index159 == 138) {
            keyWordLogicalAssignShiftRight = Trim(A_LoopField159);
        }
        if (A_Index159 == 139) {
            keyWordAssignBitAnd = Trim(A_LoopField159);
        }
        if (A_Index159 == 140) {
            keyWordAssignBitOr = Trim(A_LoopField159);
        }
        if (A_Index159 == 141) {
            keyWordAssignBitXor = Trim(A_LoopField159);
        }
        if (A_Index159 == 142) {
            keyWordTernary1 = Trim(A_LoopField159);
        }
        if (A_Index159 == 143) {
            keyWordTernary2 = Trim(A_LoopField159);
        }
        if (A_Index159 == 144) {
            keyWordInc = Trim(A_LoopField159);
        }
        if (A_Index159 == 145) {
            keyWordDec = Trim(A_LoopField159);
        }
        if (A_Index159 == 146) {
            AHKlikeLoopsIndexedAt = Trim(A_LoopField159);
        }
        if (A_Index159 == 147) {
            keyWordAIndex = Trim(A_LoopField159);
        }
        if (A_Index159 == 148) {
            keyWordALoopField = Trim(A_LoopField159);
        }
        if (A_Index159 == 149) {
            useCurlyBraces = Trim(A_LoopField159);
        }
        if (A_Index159 == 150) {
            useEnd = Trim(A_LoopField159);
        }
        if (A_Index159 == 151) {
            useSemicolon = Trim(A_LoopField159);
        }
        if (A_Index159 == 152) {
            useParentheses = Trim(A_LoopField159);
        }
        if (A_Index159 == 153) {
            usePrefixTypeForTypeDefinition = Trim(A_LoopField159);
        }
        if (A_Index159 == 154) {
            usePostfixTypeForTypeDefinition = Trim(A_LoopField159);
        }
        if (A_Index159 == 155) {
            usePythonicColonSyntax = Trim(A_LoopField159);
        }
        if (A_Index159 == 156) {
            useCurlyBracesSyntaxForArrayDef = Trim(A_LoopField159);
        }
        if (A_Index159 == 157) {
            useInJavaScriptAlwaysUseVar = Trim(A_LoopField159);
        }
        if (A_Index159 == 158) {
            useJavaScriptInAfullHTMLfile = Trim(A_LoopField159);
        }
        if (A_Index159 == 159) {
            useJavaScriptAmainFuncDef = Trim(A_LoopField159);
        }
        if (A_Index159 == 160) {
            useJavaScriptAllFuncsAreAsync = Trim(A_LoopField159);
        }
        if (A_Index159 == 161) {
            useJavaScriptAlwaysTripleEqual = Trim(A_LoopField159);
        }
    }
    if (HTVM_Size(argHTVMinstrMORE) > 0) {
        ...
        items160 = LoopParseFunc(allInstructionFile_2, "\n", "\r")
        for (let A_Index160 = 0; A_Index160 < items160.length + 0; A_Index160++) {
            const A_LoopField160 = items160[A_Index160 - 0];
            if (A_Index160 == 0) {
                langToConvertTo_2 = Trim(A_LoopField160);
            }
            if (A_Index160 == 1) {
                langFileExtension_2 = Trim(A_LoopField160);
            }
            if (A_Index160 == 2) {
                commands_2 = Trim(A_LoopField160);
            }
            if (A_Index160 == 3) {
                keyWordAlliance_2 = Trim(A_LoopField160) + " ";
            }
            if (A_Index160 == 4) {
                keyWordCrew_2 = Trim(A_LoopField160) + " ";
            }
            if (A_Index160 == 5) {
                keyWordProc_2 = Trim(A_LoopField160) + " ";
            }
            if (A_Index160 == 6) {
                keyWordStruct_2 = Trim(A_LoopField160) + " ";
            }
            if (A_Index160 == 7) {
                keyWordProp_2 = Trim(A_LoopField160) + " ";
            }
            if (A_Index160 == 8) {
                keyWordThis_2 = Trim(A_LoopField160);
            }
            if (A_Index160 == 9) {
                keyWordInclude_2 = Trim(A_LoopField160);
            }
            if (A_Index160 == 10) {
                keyWordCodeInTheTranspiledLangStart_2 = Trim(A_LoopField160);
            }
            if (A_Index160 == 11) {
                keyWordCodeInTheTranspiledLangEnd_2 = Trim(A_LoopField160);
            }
            if (A_Index160 == 12) {
                keyWordCodeInTheTranspiledLangStartCPP_2 = Trim(A_LoopField160);
            }
            if (A_Index160 == 13) {
                keyWordCodeInTheTranspiledLangEndCPP_2 = Trim(A_LoopField160);
            }
            if (A_Index160 == 14) {
                keyWordCodeInTheTranspiledLangStartPY_2 = Trim(A_LoopField160);
            }
            if (A_Index160 == 15) {
                keyWordCodeInTheTranspiledLangEndPY_2 = Trim(A_LoopField160);
            }
            if (A_Index160 == 16) {
                keyWordCodeInTheTranspiledLangStartJS_2 = Trim(A_LoopField160);
            }
            if (A_Index160 == 17) {
                keyWordCodeInTheTranspiledLangEndJS_2 = Trim(A_LoopField160);
            }
            if (A_Index160 == 18) {
                keyWordCodeInTheTranspiledLangStartGO_2 = Trim(A_LoopField160);
            }
            if (A_Index160 == 19) {
                keyWordCodeInTheTranspiledLangEndGO_2 = Trim(A_LoopField160);
            }
            if (A_Index160 == 20) {
                keyWordCodeInTheTranspiledLangStartLUA_2 = Trim(A_LoopField160);
            }
            if (A_Index160 == 21) {
                keyWordCodeInTheTranspiledLangEndLUA_2 = Trim(A_LoopField160);
            }
            if (A_Index160 == 22) {
                keyWordCodeInTheTranspiledLangStartCS_2 = Trim(A_LoopField160);
            }
            if (A_Index160 == 23) {
                keyWordCodeInTheTranspiledLangEndCS_2 = Trim(A_LoopField160);
            }
            if (A_Index160 == 24) {
                keyWordCodeInTheTranspiledLangStartJAVA_2 = Trim(A_LoopField160);
            }
            if (A_Index160 == 25) {
                keyWordCodeInTheTranspiledLangEndJAVA_2 = Trim(A_LoopField160);
            }
            if (A_Index160 == 26) {
                keyWordCodeInTheTranspiledLangStartKT_2 = Trim(A_LoopField160);
            }
            if (A_Index160 == 27) {
                keyWordCodeInTheTranspiledLangEndKT_2 = Trim(A_LoopField160);
            }
            if (A_Index160 == 28) {
                keyWordCodeInTheTranspiledLangStartRB_2 = Trim(A_LoopField160);
            }
            if (A_Index160 == 29) {
                keyWordCodeInTheTranspiledLangEndRB_2 = Trim(A_LoopField160);
            }
            if (A_Index160 == 30) {
                keyWordCodeInTheTranspiledLangStartNIM_2 = Trim(A_LoopField160);
            }
            if (A_Index160 == 31) {
                keyWordCodeInTheTranspiledLangEndNIM_2 = Trim(A_LoopField160);
            }
            if (A_Index160 == 32) {
                keyWordCodeInTheTranspiledLangStartAHK_2 = Trim(A_LoopField160);
            }
            if (A_Index160 == 33) {
                keyWordCodeInTheTranspiledLangEndAHK_2 = Trim(A_LoopField160);
            }
            if (A_Index160 == 34) {
                keyWordCodeInTheTranspiledLangStartSWIFT_2 = Trim(A_LoopField160);
            }
            if (A_Index160 == 35) {
                keyWordCodeInTheTranspiledLangEndSWIFT_2 = Trim(A_LoopField160);
            }
            if (A_Index160 == 36) {
                keyWordCodeInTheTranspiledLangStartDART_2 = Trim(A_LoopField160);
            }
            if (A_Index160 == 37) {
                keyWordCodeInTheTranspiledLangEndDART_2 = Trim(A_LoopField160);
            }
            if (A_Index160 == 38) {
                keyWordCodeInTheTranspiledLangStartTS_2 = Trim(A_LoopField160);
            }
            if (A_Index160 == 39) {
                keyWordCodeInTheTranspiledLangEndTS_2 = Trim(A_LoopField160);
            }
            if (A_Index160 == 40) {
                keyWordCodeInTheTranspiledLangStartGROOVY_2 = Trim(A_LoopField160);
            }
            if (A_Index160 == 41) {
                keyWordCodeInTheTranspiledLangEndGROOVY_2 = Trim(A_LoopField160);
            }
            if (A_Index160 == 42) {
                keyWordCodeInTheTranspiledLangStartHTVM_2 = Trim(A_LoopField160);
            }
            if (A_Index160 == 43) {
                keyWordCodeInTheTranspiledLangEndHTVM_2 = Trim(A_LoopField160);
            }
            if (A_Index160 == 44) {
                keyWordCodeInHTVMstart_2 = Trim(A_LoopField160);
            }
            if (A_Index160 == 45) {
                keyWordCodeInHTVMend_2 = Trim(A_LoopField160);
            }
            if (A_Index160 == 46) {
                keyWordCurlyBraceOpen_2 = Trim(A_LoopField160);
            }
            if (A_Index160 == 47) {
                keyWordCurlyBraceClose_2 = Trim(A_LoopField160);
            }
            if (A_Index160 == 48) {
                keyWordNull_2 = Trim(A_LoopField160);
            }
            if (A_Index160 == 49) {
                keyWordTrue_2 = Trim(A_LoopField160);
            }
            if (A_Index160 == 50) {
                keyWordFalse_2 = Trim(A_LoopField160);
            }
            if (A_Index160 == 51) {
                keyWordVoid_2 = Trim(A_LoopField160);
            }
            if (A_Index160 == 52) {
                keyWordDouble_2 = Trim(A_LoopField160);
            }
            if (A_Index160 == 53) {
                keyWordChar_2 = Trim(A_LoopField160);
            }
            if (A_Index160 == 54) {
                keyWordUint8_2 = Trim(A_LoopField160);
            }
            if (A_Index160 == 55) {
                keyWordUint16_2 = Trim(A_LoopField160);
            }
            if (A_Index160 == 56) {
                keyWordUint32_2 = Trim(A_LoopField160);
            }
            if (A_Index160 == 57) {
                keyWordUint64_2 = Trim(A_LoopField160);
            }
            if (A_Index160 == 58) {
                keyWordINT_2 = Trim(A_LoopField160);
            }
            if (A_Index160 == 59) {
                keyWordSTR_2 = Trim(A_LoopField160);
            }
            if (A_Index160 == 60) {
                keyWordBOOL_2 = Trim(A_LoopField160);
            }
            if (A_Index160 == 61) {
                keyWordFLOAT_2 = Trim(A_LoopField160);
            }
            if (A_Index160 == 62) {
                keyWordINT8_2 = Trim(A_LoopField160);
            }
            if (A_Index160 == 63) {
                keyWordINT16_2 = Trim(A_LoopField160);
            }
            if (A_Index160 == 64) {
                keyWordINT32_2 = Trim(A_LoopField160);
            }
            if (A_Index160 == 65) {
                keyWordINT64_2 = Trim(A_LoopField160);
            }
            if (A_Index160 == 66) {
                keyWordIF_2 = Trim(A_LoopField160) + " ";
            }
            if (A_Index160 == 67) {
                keyWordElseIf_2 = Trim(A_LoopField160) + " ";
            }
            if (A_Index160 == 68) {
                keyWordElse_2 = Trim(A_LoopField160);
            }
            if (A_Index160 == 69) {
                keyWordWhileLoop_2 = Trim(A_LoopField160) + " ";
            }
            if (A_Index160 == 70) {
                keyWordLoopInfinite_2 = Trim(A_LoopField160);
            }
            if (A_Index160 == 71) {
                keyWordLoop_2 = Trim(A_LoopField160) + " ";
            }
            if (A_Index160 == 72) {
                keyWordLoopParse_2 = Trim(A_LoopField160) + " ";
            }
            if (A_Index160 == 73) {
                keyWordContinue_2 = Trim(A_LoopField160);
            }
            if (A_Index160 == 74) {
                keyWordBreak_2 = Trim(A_LoopField160);
            }
            if (A_Index160 == 75) {
                keyWordFunc_2 = Trim(A_LoopField160) + " ";
            }
            if (A_Index160 == 76) {
                keyWordAwait_2 = Trim(A_LoopField160) + " ";
            }
            if (A_Index160 == 77) {
                keyWordAsync_2 = Trim(A_LoopField160) + " ";
            }
            if (A_Index160 == 78) {
                keyWordThrow_2 = Trim(A_LoopField160) + " ";
            }
            if (A_Index160 == 79) {
                keyWordErrorMsg_2 = Trim(A_LoopField160);
            }
            if (A_Index160 == 80) {
                keyWordTry_2 = Trim(A_LoopField160);
            }
            if (A_Index160 == 81) {
                keyWordCatch_2 = Trim(A_LoopField160) + " ";
            }
            if (A_Index160 == 82) {
                keyWordFinally_2 = Trim(A_LoopField160);
            }
            if (A_Index160 == 83) {
                keyWordReturnStatement_2 = Trim(A_LoopField160) + " ";
            }
            if (A_Index160 == 84) {
                keyWordArrayAppend_2 = Trim(A_LoopField160);
            }
            if (A_Index160 == 85) {
                keyWordArrayPop_2 = Trim(A_LoopField160);
            }
            if (A_Index160 == 86) {
                keyWordArraySize_2 = Trim(A_LoopField160);
            }
            if (A_Index160 == 87) {
                keyWordArrayInsert_2 = Trim(A_LoopField160);
            }
            if (A_Index160 == 88) {
                keyWordArrayRemove_2 = Trim(A_LoopField160);
            }
            if (A_Index160 == 89) {
                keyWordArrayIndexOf_2 = Trim(A_LoopField160);
            }
            if (A_Index160 == 90) {
                keyWordArrayDefinition_2 = Trim(A_LoopField160);
            }
            if (A_Index160 == 91) {
                keyWordArrayOfIntegersDefinition_2 = Trim(A_LoopField160);
            }
            if (A_Index160 == 92) {
                keyWordArrayOfStringsDefinition_2 = Trim(A_LoopField160);
            }
            if (A_Index160 == 93) {
                keyWordArrayOfFloatingPointNumbersDefinition_2 = Trim(A_LoopField160);
            }
            if (A_Index160 == 94) {
                keyWordArrayOfBooleansDefinition_2 = Trim(A_LoopField160);
            }
            if (A_Index160 == 95) {
                keyWordVar_2 = Trim(A_LoopField160);
            }
            if (A_Index160 == 96) {
                keyWordLet_2 = Trim(A_LoopField160);
            }
            if (A_Index160 == 97) {
                keyWordConst_2 = Trim(A_LoopField160);
            }
            if (A_Index160 == 98) {
                keyWordEnd_2 = Trim(A_LoopField160);
            }
            if (A_Index160 == 99) {
                keyWordGlobal_2 = Trim(A_LoopField160) + " ";
            }
            if (A_Index160 == 100) {
                keyWordComment_2 = Trim(A_LoopField160);
            }
            if (A_Index160 == 101) {
                keyWordCommentOpenMultiLine_2 = Trim(A_LoopField160);
            }
            if (A_Index160 == 102) {
                keyWordCommentCloseMultiLine_2 = Trim(A_LoopField160);
            }
            if (A_Index160 == 103) {
                keyWordEscpaeChar_2 = Trim(A_LoopField160);
            }
            if (A_Index160 == 104) {
                keyWordMainLabel_2 = Trim(A_LoopField160);
            }
            if (A_Index160 == 105) {
                keyWordConcat_2 = Trim(A_LoopField160);
            }
            if (A_Index160 == 106) {
                keyWordAdd_2 = Trim(A_LoopField160);
            }
            if (A_Index160 == 107) {
                keyWordSub_2 = Trim(A_LoopField160);
            }
            if (A_Index160 == 108) {
                keyWordMul_2 = Trim(A_LoopField160);
            }
            if (A_Index160 == 109) {
                keyWordDiv_2 = Trim(A_LoopField160);
            }
            if (A_Index160 == 110) {
                keyWordMod_2 = Trim(A_LoopField160);
            }
            if (A_Index160 == 111) {
                keyWordExp_2 = Trim(A_LoopField160);
            }
            if (A_Index160 == 112) {
                keyWordEqual_2 = Trim(A_LoopField160);
            }
            if (A_Index160 == 113) {
                keyWordStrictEqual_2 = Trim(A_LoopField160);
            }
            if (A_Index160 == 114) {
                keyWordNotEqual_2 = Trim(A_LoopField160);
            }
            if (A_Index160 == 115) {
                keyWordGreater_2 = Trim(A_LoopField160);
            }
            if (A_Index160 == 116) {
                keyWordLess_2 = Trim(A_LoopField160);
            }
            if (A_Index160 == 117) {
                keyWordGreaterEqual_2 = Trim(A_LoopField160);
            }
            if (A_Index160 == 118) {
                keyWordLessEqual_2 = Trim(A_LoopField160);
            }
            if (A_Index160 == 119) {
                keyWordAnd_2 = Trim(A_LoopField160);
            }
            if (A_Index160 == 120) {
                keyWordOr_2 = Trim(A_LoopField160);
            }
            if (A_Index160 == 121) {
                keyWordNot_2 = Trim(A_LoopField160);
            }
            if (A_Index160 == 122) {
                keyWordBitAnd_2 = Trim(A_LoopField160);
            }
            if (A_Index160 == 123) {
                keyWordBitOr_2 = Trim(A_LoopField160);
            }
            if (A_Index160 == 124) {
                keyWordBitXor_2 = Trim(A_LoopField160);
            }
            if (A_Index160 == 125) {
                keyWordBitNot_2 = Trim(A_LoopField160);
            }
            if (A_Index160 == 126) {
                keyWordShiftLeft_2 = Trim(A_LoopField160);
            }
            if (A_Index160 == 127) {
                keyWordShiftRight_2 = Trim(A_LoopField160);
            }
            if (A_Index160 == 128) {
                keyWordShiftUnsignedRight_2 = Trim(A_LoopField160);
            }
            if (A_Index160 == 129) {
                keyWordAssign_2 = Trim(A_LoopField160);
            }
            if (A_Index160 == 130) {
                keyWordAssignAdd_2 = Trim(A_LoopField160);
            }
            if (A_Index160 == 131) {
                keyWordAssignConcat_2 = Trim(A_LoopField160);
            }
            if (A_Index160 == 132) {
                keyWordAssignSub_2 = Trim(A_LoopField160);
            }
            if (A_Index160 == 133) {
                keyWordAssignMul_2 = Trim(A_LoopField160);
            }
            if (A_Index160 == 134) {
                keyWordAssignDiv_2 = Trim(A_LoopField160);
            }
            if (A_Index160 == 135) {
                keyWordAssignMod_2 = Trim(A_LoopField160);
            }
            if (A_Index160 == 136) {
                keyWordAssignShiftLeft_2 = Trim(A_LoopField160);
            }
            if (A_Index160 == 137) {
                keyWordAssignShiftRight_2 = Trim(A_LoopField160);
            }
            if (A_Index160 == 138) {
                keyWordLogicalAssignShiftRight_2 = Trim(A_LoopField160);
            }
            if (A_Index160 == 139) {
                keyWordAssignBitAnd_2 = Trim(A_LoopField160);
            }
            if (A_Index160 == 140) {
                keyWordAssignBitOr_2 = Trim(A_LoopField160);
            }
            if (A_Index160 == 141) {
                keyWordAssignBitXor_2 = Trim(A_LoopField160);
            }
            if (A_Index160 == 142) {
                keyWordTernary1_2 = Trim(A_LoopField160);
            }
            if (A_Index160 == 143) {
                keyWordTernary2_2 = Trim(A_LoopField160);
            }
            if (A_Index160 == 144) {
                keyWordInc_2 = Trim(A_LoopField160);
            }
            if (A_Index160 == 145) {
                keyWordDec_2 = Trim(A_LoopField160);
            }
            if (A_Index160 == 146) {
                AHKlikeLoopsIndexedAt_2 = Trim(A_LoopField160);
            }
            if (A_Index160 == 147) {
                keyWordAIndex_2 = Trim(A_LoopField160);
            }
            if (A_Index160 == 148) {
                keyWordALoopField_2 = Trim(A_LoopField160);
            }
            if (A_Index160 == 149) {
                useCurlyBraces_2 = Trim(A_LoopField160);
            }
            if (A_Index160 == 150) {
                useEnd_2 = Trim(A_LoopField160);
            }
            if (A_Index160 == 151) {
                useSemicolon_2 = Trim(A_LoopField160);
            }
            if (A_Index160 == 152) {
                useParentheses_2 = Trim(A_LoopField160);
            }
            if (A_Index160 == 153) {
                usePrefixTypeForTypeDefinition_2 = Trim(A_LoopField160);
            }
            if (A_Index160 == 154) {
                usePostfixTypeForTypeDefinition_2 = Trim(A_LoopField160);
            }
            if (A_Index160 == 155) {
                usePythonicColonSyntax_2 = Trim(A_LoopField160);
            }
            if (A_Index160 == 156) {
                useCurlyBracesSyntaxForArrayDef_2 = Trim(A_LoopField160);
            }
            if (A_Index160 == 157) {
                useInJavaScriptAlwaysUseVar_2 = Trim(A_LoopField160);
            }
            if (A_Index160 == 158) {
                useJavaScriptInAfullHTMLfile_2 = Trim(A_LoopField160);
            }
            if (A_Index160 == 159) {
                useJavaScriptAmainFuncDef_2 = Trim(A_LoopField160);
            }
            if (A_Index160 == 160) {
                useJavaScriptAllFuncsAreAsync_2 = Trim(A_LoopField160);
            }
            if (A_Index160 == 161) {
                useJavaScriptAlwaysTripleEqual_2 = Trim(A_LoopField160);
            }
        }
    }
    ...
    */
    ...
    items161 = LoopParseFunc(allInstructionFile, "\n", "\r")
    for (let A_Index161 = 0; A_Index161 < items161.length + 0; A_Index161++) {
        const A_LoopField161 = items161[A_Index161 - 0];
        if (RegExMatch(Trim(A_LoopField161), "^name: [A-Za-z0-9_]+")) {
            availableFuncsInHTVMInHTVM += Trim(StrSplit(Trim(A_LoopField161), "name: ", 2)) + Chr(10);
        }
    }
    ...
    for (let A_Index162 = 0; A_Index162 < items162.length + 0; A_Index162++) {
        ...
        if (Trim(StrLower(A_LoopField162)) == StrLower(keyWordCodeInTheTranspiledLangEnd)) {
        ...
        else if (Trim(StrLower(A_LoopField162)) == StrLower(keyWordCodeInTheTranspiledLangEndCPP)) {
        ...
        else if (Trim(StrLower(A_LoopField162)) == StrLower(keyWordCodeInTheTranspiledLangEndPY)) {
        ...
        else if (Trim(StrLower(A_LoopField162)) == StrLower(keyWordCodeInTheTranspiledLangEndJS)) {
        ...
        else if (Trim(StrLower(A_LoopField162)) == StrLower(keyWordCodeInTheTranspiledLangEndGO)) {
        ...
        else if (Trim(StrLower(A_LoopField162)) == StrLower(keyWordCodeInTheTranspiledLangEndLUA)) {
        ...
        else if (Trim(StrLower(A_LoopField162)) == StrLower(keyWordCodeInTheTranspiledLangEndCS)) {
        ...
        else if (Trim(StrLower(A_LoopField162)) == StrLower(keyWordCodeInTheTranspiledLangEndJAVA)) {
        ...
        else if (Trim(StrLower(A_LoopField162)) == StrLower(keyWordCodeInTheTranspiledLangEndKT)) {
        ...
        else if (Trim(StrLower(A_LoopField162)) == StrLower(keyWordCodeInTheTranspiledLangEndRB)) {
        ...
        else if (Trim(StrLower(A_LoopField162)) == StrLower(keyWordCodeInTheTranspiledLangEndNIM)) {
        ...
        else if (Trim(StrLower(A_LoopField162)) == StrLower(keyWordCodeInTheTranspiledLangEndAHK)) {
        ...
        else if (Trim(StrLower(A_LoopField162)) == StrLower(keyWordCodeInTheTranspiledLangEndSWIFT)) {
        ...
        else if (Trim(StrLower(A_LoopField162)) == StrLower(keyWordCodeInTheTranspiledLangEndDART)) {
        ...
        else if (Trim(StrLower(A_LoopField162)) == StrLower(keyWordCodeInTheTranspiledLangEndTS)) {
        ...
        else if (Trim(StrLower(A_LoopField162)) == StrLower(keyWordCodeInTheTranspiledLangEndGROOVY)) {
        ...
        else if (Trim(StrLower(A_LoopField162)) == StrLower(keyWordCodeInTheTranspiledLangEndHTVM)) {
        ...
        else if (Trim(StrLower(A_LoopField162)) == StrLower(keyWordCodeInHTVMend)) {
            //;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;
            //;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;
            ...
            holdTempDataProgrammingBlockThenPutInArr += A_LoopField162 + Chr(10);
        }
        else if (Trim(StrLower(A_LoopField162)) == StrLower(keyWordCodeInTheTranspiledLangStart)) {
            holdTempDataProgrammingBlockThenPutInArr = "";
            inProgarmmingBlock = 1;
        }
        else if (Trim(StrLower(A_LoopField162)) == StrLower(keyWordCodeInTheTranspiledLangStartCPP)) {
            holdTempDataProgrammingBlockThenPutInArr = "";
            inProgarmmingBlock = 1;
        }
        else if (Trim(StrLower(A_LoopField162)) == StrLower(keyWordCodeInTheTranspiledLangStartPY)) {
            holdTempDataProgrammingBlockThenPutInArr = "";
            inProgarmmingBlock = 1;
        }
        else if (Trim(StrLower(A_LoopField162)) == StrLower(keyWordCodeInTheTranspiledLangStartJS)) {
            holdTempDataProgrammingBlockThenPutInArr = "";
            inProgarmmingBlock = 1;
        }
        else if (Trim(StrLower(A_LoopField162)) == StrLower(keyWordCodeInTheTranspiledLangStartGO)) {
            holdTempDataProgrammingBlockThenPutInArr = "";
            inProgarmmingBlock = 1;
        }
        else if (Trim(StrLower(A_LoopField162)) == StrLower(keyWordCodeInTheTranspiledLangStartLUA)) {
            holdTempDataProgrammingBlockThenPutInArr = "";
            inProgarmmingBlock = 1;
        }
        else if (Trim(StrLower(A_LoopField162)) == StrLower(keyWordCodeInTheTranspiledLangStartCS)) {
            holdTempDataProgrammingBlockThenPutInArr = "";
            inProgarmmingBlock = 1;
        }
        else if (Trim(StrLower(A_LoopField162)) == StrLower(keyWordCodeInTheTranspiledLangStartJAVA)) {
            holdTempDataProgrammingBlockThenPutInArr = "";
            inProgarmmingBlock = 1;
        }
        else if (Trim(StrLower(A_LoopField162)) == StrLower(keyWordCodeInTheTranspiledLangStartKT)) {
            holdTempDataProgrammingBlockThenPutInArr = "";
            inProgarmmingBlock = 1;
        }
        else if (Trim(StrLower(A_LoopField162)) == StrLower(keyWordCodeInTheTranspiledLangStartRB)) {
            holdTempDataProgrammingBlockThenPutInArr = "";
            inProgarmmingBlock = 1;
        }
        else if (Trim(StrLower(A_LoopField162)) == StrLower(keyWordCodeInTheTranspiledLangStartNIM)) {
            holdTempDataProgrammingBlockThenPutInArr = "";
            inProgarmmingBlock = 1;
        }
        else if (Trim(StrLower(A_LoopField162)) == StrLower(keyWordCodeInTheTranspiledLangStartAHK)) {
            holdTempDataProgrammingBlockThenPutInArr = "";
            inProgarmmingBlock = 1;
        }
        else if (Trim(StrLower(A_LoopField162)) == StrLower(keyWordCodeInTheTranspiledLangStartSWIFT)) {
            holdTempDataProgrammingBlockThenPutInArr = "";
            inProgarmmingBlock = 1;
        }
        else if (Trim(StrLower(A_LoopField162)) == StrLower(keyWordCodeInTheTranspiledLangStartDART)) {
            holdTempDataProgrammingBlockThenPutInArr = "";
            inProgarmmingBlock = 1;
        }
        else if (Trim(StrLower(A_LoopField162)) == StrLower(keyWordCodeInTheTranspiledLangStartTS)) {
            holdTempDataProgrammingBlockThenPutInArr = "";
            inProgarmmingBlock = 1;
        }
        else if (Trim(StrLower(A_LoopField162)) == StrLower(keyWordCodeInTheTranspiledLangStartGROOVY)) {
            holdTempDataProgrammingBlockThenPutInArr = "";
            inProgarmmingBlock = 1;
        }
        else if (Trim(StrLower(A_LoopField162)) == StrLower(keyWordCodeInTheTranspiledLangStartHTVM)) {
            holdTempDataProgrammingBlockThenPutInArr = "";
            inProgarmmingBlock = 1;
        }
        else if (Trim(StrLower(A_LoopField162)) == StrLower(keyWordCodeInHTVMstart)) {
            //;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;
            //;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;
            holdTempDataProgrammingBlockThenPutInArr = "";
            inProgarmmingBlock = 1;
            ...
            programmingBlocksTemp += A_LoopField162 + Chr(10);
        }
    }
                ...
                for (let A_Index163 = 0; A_Index163 < 10000 + 0; A_Index163++) {
                        ...
                        for (let A_Index165 = 0; A_Index165 < items165.length + 0; A_Index165++) {
                            ...
                            if (SubStr(StrLower(Trim(A_LoopField165)), 1, StrLen(StrLower(keyWordInclude + " "))) == StrLower(keyWordInclude + " ")) {
                                ...
                                for (let A_Index166 = 0; A_Index166 < HTVM_Size(allLibsWeSee) + 0; A_Index166++) {
                                    if (StrReplace(StringTrimLeft(Trim(ALoopField), StrLen(StrLower(keyWordInclude) + " ")), Chr(34), "") == allLibsWeSee[A_Index166]) {
                                        doseCodeHaveLibs_HAS_SEEN = 1;
                                    }
                                }
                                if (doseCodeHaveLibs_HAS_SEEN == 0) {
                                    HTVM_Append(allLibsWeSee, StrReplace(StringTrimLeft(Trim(A_LoopField165), StrLen(StrLower(keyWordInclude) + " ")), Chr(34), ""));
                                    HTVM_Append(allLibsWeSeeSEE, StrReplace(StringTrimLeft(Trim(A_LoopField165), StrLen(StrLower(keyWordInclude) + " ")), Chr(34), ""));
                                    doseCodeHaveLibs = 1;
                                }
                            }
                        }
                    }
                    for (let A_Index167 = 0; A_Index167 < HTVM_Size(allLibsWeSee) + 0; A_Index167++) {
                        allLibsWeSeeHOLD = allLibsWeSee[A_Index167];
                        doseCodeHaveLibs_HAS_SEEN2 = 0;
                        for (let A_Index168 = 0; A_Index168 < HTVM_Size(allLibsWeSeeSEE) - HTVM_Size(allLibsWeSee) + 0; A_Index168++) {
                            if (allLibsWeSeeHOLD == allLibsWeSeeSEE[A_Index168]) {
                                doseCodeHaveLibs_HAS_SEEN2 = 1;
                            }
                        }
                        if (doseCodeHaveLibs_HAS_SEEN2 == 0) {
                            allLibCODE += FileRead(Trim(allLibsWeSee[HTVM_Size(allLibsWeSee) - A_Index167 - 1])) + Chr(10);
                        }
                    ...
                    for (let A_Index169 = 0; A_Index169 < items169.length + 0; A_Index169++) {
                        ...
                        if (SubStr(StrLower(Trim(A_LoopField169)), 1, StrLen(StrLower(keyWordInclude + " "))) != StrLower(keyWordInclude + " ")) {
                            allLibCODEfix += A_LoopField169 + Chr(10);
                        }
                    }
    ...
    if (isNotHTVMfile == 0) {
        ...
        for (let A_Index171 = 0; A_Index171 < items171.length + 0; A_Index171++) {
            ...
            theIdNumOfThe34theVar[A_Index171] = theIdNumOfThe34theVar[A_Index171] + Chr(34);
            HTVM_Append(getAllCharForTheFurtureSoIcanAddEscapeChar, A_LoopField171);
        }
        ...
        for (let A_Index172 = 0; A_Index172 < items172.length + 0; A_Index172++) {
            ...
            if (A_LoopField172 == keyWordEscpaeChar && getAllCharForTheFurtureSoIcanAddEscapeChar[A_Index172 + 1] == Chr(34)) {
                ...
                if (fixOutFixDoubleQuotesInsideDoubleQuotesFIXok != 1) {
                    OutFixDoubleQuotesInsideDoubleQuotes += A_LoopField172;
                } else {
        ...
        if (keyWordEscpaeChar == Chr(92) && langToConvertTo != langFileExtension_2) {
            ...
            for (let A_Index173 = 0; A_Index173 < items173.length + 0; A_Index173++) {
                ...
                if (areWEinSome34sNum == 1) {
                    if (A_LoopField173 != Chr(34)) {
                        if (A_LoopField173 == keyWordEscpaeChar && langToConvertTo != langFileExtension_2) {
            ...
            for (let A_Index174 = 0; A_Index174 < items174.length + 0; A_Index174++) {
                ...
                if (A_LoopField174 == Chr(34)) {
                    areWEinSome34sNum++;
                }
                if (areWEinSome34sNum == 1) {
                    if (A_LoopField174 != Chr(34)) {
                        if (A_LoopField174 == keyWordEscpaeChar && keyWordEscpaeChar == getAllCharForTheFurtureSoIcanAddEscapeChar[A_Index174 + 1] && langToConvertTo != langFileExtension_2) {
                        ...
                        else if (A_LoopField174 == keyWordEscpaeChar && langToConvertTo != langFileExtension_2) {
                            ...
                            theIdNumOfThe34theVar[theIdNumOfThe34] = theIdNumOfThe34theVar[theIdNumOfThe34] + A_LoopField174;
                        }
                    } else {
                        theIdNumOfThe34++;
                        htCodeOUT754754 += "ihuiuuhuuhtheidFor--asds" + str21 + "as--" + str21 + "theuhturtyphoutr--" + Chr(65) + Chr(65) + STR(theIdNumOfThe34) + Chr(65) + Chr(65);
                ...
                if (areWEinSome34sNum == 2 || areWEinSome34sNum == 0) {
                    if (A_LoopField174 != Chr(34)) {
                        htCodeOUT754754 += A_LoopField174;
                    }
                    areWEinSome34sNum = 0;
                }
            }
        ...
        for (let A_Index175 = 0; A_Index175 < theIdNumOfThe34 + 0; A_Index175++) {
        ...
        newStringOutCode = "";
        items176 = LoopParseFunc(code, "\n", "\r")
        for (let A_Index176 = 0; A_Index176 < items176.length + 0; A_Index176++) {
            const A_LoopField176 = items176[A_Index176 - 0];
            if (InStr(A_LoopField176, " " + keyWordComment) != false && SubStr(Trim(A_LoopField176), 1, StrLen(keyWordComment)) != keyWordComment) {
                posForRemoveCommentsOnTheEndOfTheLine = 0;
                slidingWinFixCommentsRmI = 0;
                slidingWinFixCommentsRm = slidingWinFixCommentsRmFUNC(A_LoopField176);
                for (let A_Index177 = 0; A_Index177 < HTVM_Size(slidingWinFixCommentsRm) + 0; A_Index177++) {
                    slidingWinFixCommentsRmI = A_Index177;
                    slidingWinFixCommentsRmHold = "";
                    for (let A_Index178 = 0; A_Index178 < StrLen(" " + keyWordComment) + 0; A_Index178++) {
                        slidingWinFixCommentsRmHold += slidingWinFixCommentsRm[slidingWinFixCommentsRmI + A_Index178];
                    }
                    if (Trim(slidingWinFixCommentsRmHold) == keyWordComment) {
                        posForRemoveCommentsOnTheEndOfTheLine = A_Index177;
                        break;
                    }
                }
                if (posForRemoveCommentsOnTheEndOfTheLine > 0) {
                    newStringOutCode += SubStr(A_LoopField176, 1, posForRemoveCommentsOnTheEndOfTheLine) + Chr(10);
                } else {
                    newStringOutCode += A_LoopField176 + Chr(10);
                }
            } else {
                newStringOutCode += A_LoopField176 + Chr(10);
            }
        ...
        if (langToConvertTo != langFileExtension_2) {
            ...
            for (let A_Index179 = 0; A_Index179 < items179.length + 0; A_Index179++) {
                ...
                if (SubStr(Trim(A_LoopField179), 1, 1) != ";") {
                    semiColonForOneLinerOut += StrReplace(A_LoopField179, ";", Chr(10)) + Chr(10);
                } else {
                    semiColonForOneLinerOut += A_LoopField179 + Chr(10);
                }
            }
        ...
        if (useCurlyBraces == "on") {
            ...
            for (let A_Index180 = 0; A_Index180 < items180.length + 0; A_Index180++) {
                ...
                str1 = "";
                if (InStr(A_LoopField180, keyWordCurlyBraceOpen) || InStr(A_LoopField180, keyWordCurlyBraceClose)) {
                    if (SubStr(Trim(A_LoopField180), 1, StrLen(Trim(keyWordComment))) == Trim(keyWordComment) && isLineAconstruct(Trim(A_LoopField180)) == false) {
                        fixSomeCurlyBraceForExratFlexability += A_LoopField180 + Chr(10);
                    } else {
                        if (InStr(A_LoopField180, Trim(keyWordArrayDefinition) + " " + keyWordAssign + " ") || InStr(A_LoopField180, Trim(keyWordArrayOfIntegersDefinition) + " " + keyWordAssign + " ") || InStr(A_LoopField180, Trim(keyWordArrayOfStringsDefinition) + " " + keyWordAssign + " ") || InStr(A_LoopField180, Trim(keyWordArrayOfFloatingPointNumbersDefinition) + " " + keyWordAssign + " ") || InStr(A_LoopField180, Trim(keyWordArrayOfBooleansDefinition) + " " + keyWordAssign + " ") && useCurlyBracesSyntaxForArrayDef == "on") {
                            fixSomeCurlyBraceForExratFlexability += A_LoopField180 + Chr(10);
                        } else {
                            str1 = StrReplace(A_LoopField180, keyWordCurlyBraceOpen, "{");
            ...
            for (let A_Index181 = 0; A_Index181 < items181.length + 0; A_Index181++) {
                ...
                fixSomeCurlyBraceForExratFlexabilityTriHELP += Trim(A_LoopField181) + Chr(10);
            }
            ...
            if (keyWordCurlyBraceOpen != "{") {
                items182 = LoopParseFunc(code, "\n", "\r")
                for (let A_Index182 = 0; A_Index182 < items182.length + 0; A_Index182++) {
                    const A_LoopField182 = items182[A_Index182 - 0];
                    htCodeOutFixINT2 = 0;
                    if (isLineAconstruct(Trim(A_LoopField182)) == true || Trim(A_LoopField182) == keyWordCurlyBraceOpen) {
                        htCodeOutFixINT2 = 1;
                    }
                    htCodeOutFixINT = 0;
                    items183 = LoopParseFunc(A_LoopField182, " ")
                    for (let A_Index183 = 0; A_Index183 < items183.length + 0; A_Index183++) {
                        const A_LoopField183 = items183[A_Index183 - 0];
                        htCodeOutFixINT++;
                    }
                    htCodeOutFixINT--;
                    items184 = LoopParseFunc(A_LoopField182, " ")
                    for (let A_Index184 = 0; A_Index184 < items184.length + 0; A_Index184++) {
                        const A_LoopField184 = items184[A_Index184 - 0];
                        if (htCodeOutFixINT == A_Index184 && htCodeOutFixINT2 == 1) {
                            htCodeOutFixINT2 = 0;
                            htCodeOutFixEnd += HTVMmatchStrRrplace(A_LoopField184, keyWordCurlyBraceOpen, "{") + " ";
                        } else {
            ...
            if (keyWordCurlyBraceClose != "}") {
                ...
                for (let A_Index185 = 0; A_Index185 < items185.length + 0; A_Index185++) {
                    ...
                    htCodeOutFixINT2 = 0;
                    if (isLineAconstruct(Trim(A_LoopField185)) == true || Trim(A_LoopField185) == keyWordCurlyBraceClose) {
                        htCodeOutFixINT2 = 1;
                    }
                    items186 = LoopParseFunc(A_LoopField185, " ")
                    for (let A_Index186 = 0; A_Index186 < items186.length + 0; A_Index186++) {
                        const A_LoopField186 = items186[A_Index186 - 0];
                        if (htCodeOutFixINT2 == 1 || Trim(A_LoopField186) == keyWordCurlyBraceClose) {
                            htCodeOutFixINT2 = 0;
                            htCodeOutFixEnd += HTVMmatchStrRrplace(A_LoopField186, keyWordCurlyBraceClose, "}") + " ";
                        } else {
                            htCodeOutFixEnd += A_LoopField186 + " ";
                        }
                    }
                    htCodeOutFixEnd = StringTrimRight(htCodeOutFixEnd, 1);
                    htCodeOutFixEnd += Chr(10);
        ...
        if (useCurlyBraces == "on") {
            ...
            for (let A_Index187 = 0; A_Index187 < items187.length + 0; A_Index187++) {
                ...
                HTVM_Append(addNoCurlyFixWhenNextLineAfterConstructLookAhead, Trim(A_LoopField187));
            }
            ...
            for (let A_Index188 = 0; A_Index188 < items188.length + 0; A_Index188++) {
                ...
                if (isLineAconstruct(Trim(A_LoopField188)) && addNoCurlyFixWhenNextLineAfterConstructLookAhead[A_Index188 + 1] != "{") {
                    int5 = 1;
                    addNoCurlyFixWhenNextLineAfterConstructOut += A_LoopField188 + Chr(10) + "{" + Chr(10);
                } else {
                    if (int5 == 1) {
                        int5 = 0;
                        addNoCurlyFixWhenNextLineAfterConstructOut += A_LoopField188 + Chr(10) + "}" + Chr(10);
                    } else {
                        addNoCurlyFixWhenNextLineAfterConstructOut += A_LoopField188 + Chr(10);
                    }
                }
            }
        ...
        if (useEnd == "on") {
            ...
            for (let A_Index189 = 0; A_Index189 < items189.length + 0; A_Index189++) {
                ...
                if (StrLower(Trim(A_LoopField189)) == StrLower(keyWordEnd)) {
                    htCodeOutFixEnd += "}\n";
                }
                else if (SubStr(StrLower(Trim(A_LoopField189)), 1, StrLen(StrLower(keyWordIF))) == StrLower(keyWordIF) && isLineAconstruct(Trim(A_LoopField189)) == true) {
                    htCodeOutFixEnd += A_LoopField189 + "\n{\n";
                }
                else if (SubStr(StrLower(Trim(A_LoopField189)), 1, StrLen(StrLower(keyWordWhileLoop))) == StrLower(keyWordWhileLoop) && isLineAconstruct(Trim(A_LoopField189)) == true) {
                    htCodeOutFixEnd += A_LoopField189 + "\n{\n";
                }
                else if (SubStr(StrLower(Trim(A_LoopField189)), 1, StrLen(StrLower(keyWordElseIf))) == StrLower(keyWordElseIf) && isLineAconstruct(Trim(A_LoopField189)) == true) {
                    htCodeOutFixEnd += "\n}\n" + A_LoopField189 + "\n{\n";
                }
                else if (SubStr(StrLower(Trim(A_LoopField189)), 1, StrLen(StrLower(keyWordElse))) == StrLower(keyWordElse) && isLineAconstruct(Trim(A_LoopField189)) == true) {
                    htCodeOutFixEnd += "\n}\n" + A_LoopField189 + "\n{\n";
                }
                else if (SubStr(StrLower(Trim(A_LoopField189)), 1, StrLen(StrLower(keyWordCatch))) == StrLower(keyWordCatch) && isLineAconstruct(Trim(A_LoopField189)) == true) {
                    htCodeOutFixEnd += "\n}\n" + A_LoopField189 + "\n{\n";
                }
                else if (SubStr(StrLower(Trim(A_LoopField189)), 1, StrLen(StrLower(keyWordFinally))) == StrLower(keyWordFinally) && isLineAconstruct(Trim(A_LoopField189)) == true) {
                    htCodeOutFixEnd += "\n}\n" + A_LoopField189 + "\n{\n";
                }
                else if (isLineAconstruct(Trim(A_LoopField189)) == true) {
                    htCodeOutFixEnd += A_LoopField189 + "\n{\n";
                } else {
        ...
        if (usePythonicColonSyntax == "off") {
            if (useEnd == "off" && useCurlyBraces == "off") {
                ...
                for (let A_Index190 = 0; A_Index190 < items190.length + 0; A_Index190++) {
                    ...
                    if (isLineAconstruct(Trim(A_LoopField190)) == true) {
                        htCodeOutFixEnd += A_LoopField190 + ":\n";
                    } else {
                        htCodeOutFixEnd += A_LoopField190 + Chr(10);
                    }
                }
                code = StringTrimRight(htCodeOutFixEnd, 1);
            }
        ...
        for (let A_Index192 = 0; A_Index192 < 2 + 0; A_Index192++) {
            ...
            for (let A_Index193 = 0; A_Index193 < items193.length + 0; A_Index193++) {
                ...
                if (InStr(A_LoopField193, Trim(keyWordArrayDefinition) + " " + keyWordAssign + " ") || InStr(A_LoopField193, Trim(keyWordArrayOfIntegersDefinition) + " " + keyWordAssign + " ") || InStr(A_LoopField193, Trim(keyWordArrayOfStringsDefinition) + " " + keyWordAssign + " ") || InStr(A_LoopField193, Trim(keyWordArrayOfFloatingPointNumbersDefinition) + " " + keyWordAssign + " ") || InStr(A_LoopField193, Trim(keyWordArrayOfBooleansDefinition) + " " + keyWordAssign + " ") && useCurlyBracesSyntaxForArrayDef == "on") {
                    if (SubStr(StrLower(A_LoopField193), 1, StrLen(StrLower(keyWordFunc))) == StrLower(keyWordFunc) || SubStr(StrLower(A_LoopField193), 1, StrLen(StrLower(keyWordAsync) + StrLower(keyWordFunc))) == StrLower(keyWordAsync) + StrLower(keyWordFunc)) {
                        if (InStr(Trim(A_LoopField193), "{") && Trim(A_LoopField193) != "{" && SubStr(Trim(A_LoopField193), 1, StrLen(Trim(keyWordComment))) != Trim(keyWordComment)) {
                            outCodeFixBraces += Trim(StrReplace(Trim(A_LoopField193), "{", "")) + "\n{\n";
                        }
                        else if (InStr(Trim(A_LoopField193), "}") && Trim(A_LoopField193) != "}" && SubStr(Trim(A_LoopField193), 1, StrLen(Trim(keyWordComment))) != Trim(keyWordComment)) {
                            outCodeFixBraces += "}\n" + Trim(StrReplace(Trim(A_LoopField193), "}", "")) + Chr(10);
                        } else {
                            outCodeFixBraces += Trim(A_LoopField193) + Chr(10);
                        }
                    } else {
                        outCodeFixBraces += Trim(A_LoopField193) + Chr(10);
                    }
        ...
        if (usePythonicColonSyntax == "on" || useCurlyBraces == "off" && useEnd == "off") {
            ...
            for (let A_Index194 = 0; A_Index194 < items194.length + 0; A_Index194++) {
                ...
                if (isLineAconstruct(Trim(A_LoopField194)) == true) {
                    htCodeOutFixEnd += StringTrimRight(A_LoopField194, 1) + Chr(10);
                } else {
        ...
        if (isNotHTVMfile2 == 0) {
            ...
            for (let A_Index195 = 0; A_Index195 < items195.length + 0; A_Index195++) {
                ...
                if (StrLower(Trim(A_LoopField195)) == StrLower(keyWordMainLabel)) {
                    didWeUseMainLabel = 1;
                    if (langToConvertTo == langFileExtension_2) {
                    ...
                    codeOutFixAndAddMainFunc += A_LoopField195 + Chr(10);
                }
            }
        ...
        for (let A_Index196 = 0; A_Index196 < items196.length + 0; A_Index196++) {
            ...
            HTVM_Append(lookIntoTheNextLineForFuncWhitNoKeyWord, A_LoopField196);
        }
        ...
        //print(code)
        //print("=======DOWN==========")
        items197 = LoopParseFunc(code, "\n", "\r")
        for (let A_Index197 = 0; A_Index197 < items197.length + 0; A_Index197++) {
            const A_LoopField197 = items197[A_Index197 - 0];
            lineDone = 0;
            if (InStr(A_LoopField197, "guiInit(") || InStr(A_LoopField197, "guiAdd(") || InStr(A_LoopField197, "guiAddElement(") || InStr(A_LoopField197, "guiControl(")) {
                lineDone = 1;
                htCode += Trim(A_LoopField197) + Chr(10);
            }
            if (SubStr(A_LoopField197, 1, StrLen(keyWordComment)) == keyWordComment) {
                lineDone = 1;
                str1 = StringTrimLeft(A_LoopField197, StrLen(keyWordComment));
                if (langToConvertTo == langFileExtension_2) {
            ...
            else if (Trim(A_LoopField197) == keyWordCommentOpenMultiLine) {
                lineDone = 1;
                if (langToConvertTo == langFileExtension_2) {
            ...
            else if (Trim(A_LoopField197) == keyWordCommentCloseMultiLine) {
                lineDone = 1;
                if (langToConvertTo == langFileExtension_2) {
                ...
                lineDone = 1;
                str1 = StrSplit(A_LoopField197, " ", 2);
            ...
            else if (SubStr(StrLower(Trim(A_LoopField197)), 1, StrLen(StrLower(keyWordStruct))) == StrLower(keyWordStruct)) {
                lineDone = 1;
                str0 = Trim(StringTrimLeft(A_LoopField197, StrLen(keyWordStruct)));
                if (langToConvertTo == langFileExtension_2) {
                    htCode += Trim(keyWordStruct_2) + " " + str0 + Chr(10);
                }
            }
            else if (SubStr(StrLower(Trim(A_LoopField197)), 1, StrLen(StrLower(keyWordAlliance))) == StrLower(keyWordAlliance)) {
                lineDone = 1;
                str0 = Trim(StringTrimLeft(A_LoopField197, StrLen(keyWordAlliance)));
                if (langToConvertTo == langFileExtension_2) {
                    htCode += Trim(keyWordAlliance_2) + " " + str0 + Chr(10);
                }
            }
            else if (SubStr(StrLower(Trim(A_LoopField197)), 1, StrLen(StrLower(keyWordCrew))) == StrLower(keyWordCrew)) {
                lineDone = 1;
                str0 = Trim(StringTrimLeft(A_LoopField197, StrLen(keyWordCrew)));
                if (langToConvertTo == langFileExtension_2) {
                    htCode += Trim(keyWordCrew_2) + " " + str0 + Chr(10);
                }
            }
            else if (InStr(A_LoopField197, "[end][end][end][end][end] ")) {
                lineDone = 1;
                if (langToConvertTo == langFileExtension_2) {
                    htCode += "}" + Chr(10);
                }
            }
            else if (SubStr(StrLower(Trim(A_LoopField197)), 1, StrLen(StrLower(keyWordProp))) == StrLower(keyWordProp)) {
                lineDone = 1;
                str0 = Trim(StringTrimLeft(A_LoopField197, StrLen(keyWordProp)));
            ...
            else if (SubStr(StrLower(Trim(A_LoopField197)), 1, StrLen(StrLower(keyWordProc))) == StrLower(keyWordProc)) {
                lineDone = 1;
                str0 = Trim(StringTrimLeft(A_LoopField197, StrLen(keyWordProc)));
                str1 = Trim(StrSplit(str0, "-----------------------HTVM-OSP-HTVM-OBJ--------------", 1));
                str20 = Trim(StrSplit(str0, "-----------------------HTVM-OSP-HTVM-OBJ--------------", 2));
            ...
            else if (SubStr(StrLower(Trim(A_LoopField197)), 1, StrLen(StrLower(keyWordIF))) == StrLower(keyWordIF) && SubStr(StrLower(Trim(A_LoopField197)), 1, StrLen(StrLower(keyWordElseIf))) != StrLower(keyWordElseIf)) {
                lineDone = 1;
                str1 = Trim(StringTrimLeft(A_LoopField197, StrLen(keyWordIF)));
            ...
            else if (SubStr(StrLower(Trim(A_LoopField197)), 1, StrLen(StrLower(keyWordElseIf))) == StrLower(keyWordElseIf) && SubStr(StrLower(Trim(A_LoopField197)), 1, StrLen(StrLower(keyWordIF))) != StrLower(keyWordIF)) {
                lineDone = 1;
                str1 = Trim(StringTrimLeft(A_LoopField197, StrLen(keyWordElseIf)));
                str1 = Trim(expressionParserTranspiler("(" + str1 + ")"));
                str1 = StringTrimRight(str1, 1);
                str1 = StringTrimLeft(str1, 1);
                if (langToConvertTo == langFileExtension_2) {
            ...
            else if (StrLower(Trim(A_LoopField197)) == StrLower(keyWordElse) || StrLower(Trim(A_LoopField197)) == StrLower(keyWordElse + ":")) {
                lineDone = 1;
                if (langToConvertTo == langFileExtension_2) {
                    if (usePythonicColonSyntax_2 == "on") {
            ...
            else if (SubStr(StrLower(Trim(A_LoopField197)), 1, StrLen(StrLower(keyWordWhileLoop))) == StrLower(keyWordWhileLoop)) {
                lineDone = 1;
                str1 = Trim(StringTrimLeft(A_LoopField197, StrLen(keyWordWhileLoop)));
                str1 = Trim(expressionParserTranspiler("(" + str1 + ")"));
                str1 = StringTrimRight(str1, 1);
                str1 = StringTrimLeft(str1, 1);
                if (langToConvertTo == langFileExtension_2) {
            ...
            else if (StrLower(Trim(A_LoopField197)) == StrLower(keyWordTry) || StrLower(Trim(A_LoopField197)) == StrLower(keyWordTry + ":")) {
                lineDone = 1;
                if (langToConvertTo == langFileExtension_2) {
                    if (usePythonicColonSyntax_2 == "on") {
                        htCode += keyWordTry_2 + ":\n";
            ...
            else if (SubStr(StrLower(Trim(A_LoopField197)), 1, StrLen(StrLower(keyWordCatch))) == StrLower(keyWordCatch)) {
                lineDone = 1;
                str1 = Trim(StrReplace(StrReplace(StringTrimLeft(A_LoopField197, StrLen(keyWordCatch)), "(", ""), ")", ""));
            ...
            else if (StrLower(Trim(A_LoopField197)) == StrLower(keyWordFinally) || StrLower(Trim(A_LoopField197)) == StrLower(keyWordFinally + ":")) {
                lineDone = 1;
                if (langToConvertTo == langFileExtension_2) {
                    if (usePythonicColonSyntax_2 == "on") {
            ...
            else if (SubStr(StrLower(Trim(A_LoopField197)), 1, StrLen(StrLower(keyWordThrow))) == StrLower(keyWordThrow)) {
                lineDone = 1;
                str1 = Trim(A_LoopField197);
                if (SubStrLastChars(str1, 1) == ";") {
                    str1 = StringTrimRight(str1, 1);
                }
            ...
            else if (StrLower(A_LoopField197) == StrLower(keyWordBreak) || StrLower(A_LoopField197) == StrLower(keyWordBreak + ";")) {
                lineDone = 1;
                if (langToConvertTo == langFileExtension_2) {
                    if (useSemicolon_2 == "on") {
            ...
            else if (StrLower(A_LoopField197) == StrLower(Trim(keyWordReturnStatement)) || StrLower(A_LoopField197) == StrLower(Trim(keyWordReturnStatement) + ";")) {
                lineDone = 1;
                if (langToConvertTo == langFileExtension_2) {
                    if (useSemicolon_2 == "on") {
            ...
            else if (SubStr(Trim(StrLower(A_LoopField197)), 1, StrLen(StrLower(keyWordReturnStatement))) == StrLower(keyWordReturnStatement)) {
                lineDone = 1;
                str1 = Trim(expressionParserTranspiler(Trim(StringTrimLeft(A_LoopField197, StrLen(keyWordReturnStatement)))));
                if (SubStrLastChars(str1, 1) == ";") {
                    str1 = StringTrimRight(str1, 1);
                }
                if (langToConvertTo == langFileExtension_2) {
            ...
            else if (StrLower(A_LoopField197) == StrLower(keyWordContinue) || StrLower(A_LoopField197) == StrLower(keyWordContinue + ";")) {
                lineDone = 1;
                if (langToConvertTo == langFileExtension_2) {
                    if (useSemicolon_2 == "on") {
            ...
            else if (StrLower(A_LoopField197) == "::" + keyWordContinue || StrLower(A_LoopField197) == "::" + keyWordContinue + ";") {
                lineDone = 1;
                if (langToConvertTo == langFileExtension_2) {
                    htCode += "::" + keyWordContinue_2 + Chr(10);
                }
            ...
            else if (StrLower(A_LoopField197) == StrLower(keyWordLoopInfinite) || StrLower(A_LoopField197) == StrLower(keyWordLoopInfinite + ":")) {
                // infinity loops
            ...
            else if (SubStr(Trim(StrLower(A_LoopField197)), 1, StrLen(StrLower(keyWordLoop))) == StrLower(keyWordLoop) && SubStr(Trim(StrLower(A_LoopField197)), 1, StrLen(StrLower(keyWordLoop)) + 2) != StrLower(keyWordLoop) + "% " && SubStr(Trim(StrLower(A_LoopField197)), 1, StrLen(StrLower(keyWordLoopParse))) != StrLower(keyWordLoopParse)) {
                out2 = StringTrimLeft(A_LoopField197, StrLen(keyWordLoop));
            ...
            else if (SubStr(Trim(StrLower(A_LoopField197)), 1, StrLen(StrLower(keyWordLoop)) + 2) == StrLower(keyWordLoop) + "% ") {
                howMany_fixAindexInGoUnused++;
                out2 = StringTrimLeft(A_LoopField197, StrLen(keyWordLoop) + 2);
                out2 = StrReplace(out2, ":", "");
                //MsgBox % out2
                //MsgBox, % out2
                out2 = Trim(out2);
            ...
            else if (SubStr(StrLower(A_LoopField197), 1, StrLen(StrLower(keyWordLoopParse))) == StrLower(keyWordLoopParse)) {
                howMany_fixAindexInGoUnused++;
                var1 = StringTrimLeft(A_LoopField197, StrLen(keyWordLoopParse));
                out2 = StrReplace(out2, ":", "");
                lineDone = 1;
            ...
            else if (SubStrLastChars(Trim(A_LoopField197), StrLen(keyWordInc)) == keyWordInc || SubStrLastChars(Trim(A_LoopField197), StrLen(keyWordInc + ";")) == keyWordInc + ";" && lineDone == 0) {
                if (useSemicolon == "on" || SubStrLastChars(Trim(A_LoopField197), 1) == ";") {
                    str1 = Trim(StringTrimRight(Trim(A_LoopField197), StrLen(keyWordInc + ";")));
                } else {
                    str1 = Trim(StringTrimRight(Trim(A_LoopField197), StrLen(keyWordInc)));
                }
                str1 = Trim(expressionParserTranspiler(str1));
            ...
            else if (SubStrLastChars(Trim(A_LoopField197), StrLen(keyWordDec)) == keyWordDec || SubStrLastChars(Trim(A_LoopField197), StrLen(keyWordDec + ";")) == keyWordDec + ";" && lineDone == 0) {
                if (useSemicolon == "on" || SubStrLastChars(Trim(A_LoopField197), 1) == ";") {
                    str1 = Trim(StringTrimRight(Trim(A_LoopField197), StrLen(keyWordDec + ";")));
                } else {
                    str1 = Trim(StringTrimRight(Trim(A_LoopField197), StrLen(keyWordDec)));
                }
                str1 = Trim(expressionParserTranspiler(str1));
            ...
            else if (SubStr(StrLower(Trim(A_LoopField197)), 1, StrLen(StrLower(keyWordInclude + " "))) == StrLower(keyWordInclude + " ") && langToConvertTo == langFileExtension_2) {
                htCode += keyWordInclude_2 + " " + Trim(StringTrimLeft(Trim(A_LoopField197), StrLen(StrLower(keyWordInclude) + " "))) + Chr(10);
            }
            else if (StrLower(A_LoopField197) == StrLower(keyWordGlobal) || StrLower(A_LoopField197) == StrLower(keyWordGlobal + ";")) {
                if (langToConvertTo == "ahk") {
                    htCode += "global\n";
                }
            }
            else if (SubStr(StrLower(A_LoopField197), 1, StrLen(StrLower(keyWordAwait))) == StrLower(keyWordAwait)) {
                str1 = StringTrimLeft(A_LoopField197, StrLen(keyWordAwait));
                //MsgBox, % A_LoopField197
                lineDone = 1;
            ...
            else if (SubStr(StrLower(A_LoopField197), 1, StrLen(StrLower(keyWordGlobal))) == StrLower(keyWordGlobal)) {
                str1 = StringTrimLeft(A_LoopField197, StrLen(keyWordGlobal));
                //MsgBox, % A_LoopField197
            ...
            else if (SubStr(StrLower(A_LoopField197), 1, StrLen(keyWordFunc)) == StrLower(keyWordFunc) || SubStr(StrLower(A_LoopField197), 1, StrLen(StrLower(keyWordAsync) + StrLower(keyWordFunc))) == StrLower(keyWordAsync) + StrLower(keyWordFunc) && InStr(StrLower(A_LoopField197), " main(") == false && lineDone == 0) {
                lineDone = 1;
                if (SubStr(StrLower(A_LoopField197), 1, StrLen(StrLower(keyWordAsync) + StrLower(keyWordFunc))) == StrLower(keyWordAsync) + StrLower(keyWordFunc)) {
                    // is async
                    int1 = 1;
                    str1 = Trim(StringTrimLeft(A_LoopField197, StrLen(StrLower(keyWordAsync + keyWordFunc))));
                } else {
                    // is async
                    int1 = 0;
                    str1 = Trim(StringTrimLeft(A_LoopField197, StrLen(StrLower(keyWordFunc))));
                }
                if (langToConvertTo != langFileExtension_2 && langToConvertTo != "cpp" && langToConvertTo != "go" && langToConvertTo != "cs" && langToConvertTo != "java" && langToConvertTo != "kt" && langToConvertTo != "nim" && langToConvertTo != "swift" && langToConvertTo != "dart" && langToConvertTo != "ts") {
                    if (str1 != "") {
                        ...
                        for (let A_Index199 = 0; A_Index199 < items199.length + 0; A_Index199++) {
                            ...
                            if (InStr(A_LoopField199, " := ")) {
                                if (InStr(Trim(StrSplit(A_LoopField199, " := ", 1)), " ") != true) {
                                    str4 += "optanal-HTVM-opt-parmsNOtWorking-theirewIs-a-bug-Iusfbudfbuoeshfuisbav=-fwaegs-awedsf-dd-sfgc " + A_LoopField199 + ", ";
                                } else {
                                    str4 += A_LoopField199 + ", ";
                                }
                            }
                            else if (InStr(A_LoopField199, " = ")) {
                                if (InStr(Trim(StrSplit(A_LoopField199, " = ", 1)), " ") != true) {
                                    str4 += "optanal-HTVM-opt-parmsNOtWorking-theirewIs-a-bug-Iusfbudfbuoeshfuisbav=-fwaegs-awedsf-dd-sfgc " + A_LoopField199 + ", ";
                                } else {
                                    str4 += A_LoopField199 + ", ";
                            ...
                            else if (InStr(A_LoopField199, " " + Trim(keyWordAssign) + " ")) {
                                if (InStr(Trim(StrSplit(A_LoopField199, " " + Trim(keyWordAssign) + " ", 1)), " ") != true) {
                                    str4 += "optanal-HTVM-opt-parmsNOtWorking-theirewIs-a-bug-Iusfbudfbuoeshfuisbav=-fwaegs-awedsf-dd-sfgc " + A_LoopField199 + ", ";
                                } else {
                                    str4 += A_LoopField199 + ", ";
                                }
                ...
                if (Trim(str1) == "") {
                    ...
                    str19 = "";
                    str20 = "";
                    items200 = LoopParseFunc(str1, ",");
                    for (let A_Index200 = 0; A_Index200 < items200.length + 0; A_Index200++) {
                        const A_LoopField200 = items200[A_Index200 - 0];
                        //print("|" . Trim(A_LoopField200) . "|")
                        if (InStr(Trim(A_LoopField200), " ")) {
                            // there is space aka there is a type
                            if (InStr(A_LoopField200, " " + Trim(keyWordAssign) + " ") == false) {
                                if (usePrefixTypeForTypeDefinition == "on") {
                                    // normal
                                    str6 = Trim(A_LoopField200);
                                ...
                                // optanal param
                                if (InStr(Trim(StrSplit(Trim(A_LoopField200), " " + Trim(keyWordAssign) + " ", 1)), " ")) {
                                    // there is a type
                                    if (usePrefixTypeForTypeDefinition == "on") {
                                        // normal
                                        str6 = Trim(StrSplit(Trim(A_LoopField200), " " + Trim(keyWordAssign) + " ", 1));
                                        str9 = Trim(expressionParserTranspiler(Trim(StrSplit(Trim(A_LoopField200), " " + Trim(keyWordAssign) + " ", 2))));
                                        str7 = getTheLastWord(str6);
                                        str8 = getFuncTypeConvert(Trim(StringTrimRight(str6, StrLen(Trim(str7)) + 1)));
                                        //;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;
                                        //;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;
                                    ...
                                    // there is no type
                                    str8 = Trim(StrSplit(Trim(A_LoopField200), " " + Trim(keyWordAssign) + " ", 1));
                                    str9 = Trim(expressionParserTranspiler(Trim(StrSplit(Trim(A_LoopField200), " " + Trim(keyWordAssign) + " ", 2))));
                                    // str10
                                    // skipLeftCuleyForFuncPLS := 1
                                    //;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;
                            ...
                            str5 += Trim(A_LoopField200) + Chr(10);
                        }
                    }
                    //print(str5)
                    //print(str10)
                    ...
                    if (Trim(str5) != "" && Trim(str10) != "") {
                        ...
                        if (langToConvertTo == "lua") {
                            ...
                            for (let A_Index203 = 0; A_Index203 < items203.length + 0; A_Index203++) {
                                ...
                                str14 += Trim(StrSplit(Trim(A_LoopField203), " = ", 1)) + ", ";
                            }
                            ...
                            for (let A_Index204 = 0; A_Index204 < items204.length + 0; A_Index204++) {
                                ...
                                str13 += Trim(StrSplit(Trim(A_LoopField204), " = ", 1)) + " = " + Trim(StrSplit(Trim(A_LoopField204), " = ", 1)) + " or " + Trim(StrSplit(Trim(A_LoopField204), " = ", 2)) + Chr(10);
                            }
                            str12 = str14;
                        }
                        if (langToConvertTo == "go") {
                            ...
                            for (let A_Index205 = 0; A_Index205 < items205.length + 0; A_Index205++) {
                                ...
                                str17 = Trim(StrSplit(Trim(A_LoopField205), " = ", 2));
                            ...
                            for (let A_Index206 = 0; A_Index206 < items206.length + 0; A_Index206++) {
                                ...
                                str16 = Trim(StrSplit(Trim(StrSplit(Trim(A_LoopField206), " = ", 1)), " ", 1));
                                str18 = Trim(StrSplit(Trim(StrSplit(Trim(A_LoopField206), " = ", 1)), " ", 2));
                                // str16 = var name
                                // str18 = var type
                                str13 += "if len(__HTVM_V2_TO_GO_optionalParams__) " + Chr(62) + " " + STR(A_Index206) + Chr(10) + "{" + Chr(10) + str16 + " = __HTVM_V2_TO_GO_optionalParams__[" + STR(A_Index206) + "].(" + str18 + ")" + Chr(10) + "}" + Chr(10);
                            }
                            str12 = str14;
                        }
                        if (langToConvertTo == "java") {
                            ...
                            for (let A_Index207 = 0; A_Index207 < items207.length + 0; A_Index207++) {
                                ...
                                str19 = Trim(A_LoopField207);
                                str20 = "";
                                items208 = LoopParseFunc(str19, " ")
                                for (let A_Index208 = 0; A_Index208 < items208.length + 0; A_Index208++) {
                                    const A_LoopField208 = items208[A_Index208 - 0];
                                    if (A_Index208 == 3) {
                                        if (RegExMatch(A_LoopField208, "^\\d+(\\.\\d+)?$")) {
                                            str20 += A_LoopField208 + "f ";
                                        } else {
                                            str20 += A_LoopField208 + " ";
                                        }
                                    } else {
                                        str20 += A_LoopField208 + " ";
                                    }
                            ...
                            items209 = LoopParseFunc(str12, ",");
                            for (let A_Index209 = 0; A_Index209 < items209.length + 0; A_Index209++) {
                                const A_LoopField209 = items209[A_Index209 - 0];
                                str16 = Trim(StrSplit(Trim(StrSplit(Trim(A_LoopField209), " = ", 1)), " ", 2));
                                str17 = Trim(StrSplit(Trim(StrSplit(Trim(A_LoopField209), " = ", 1)), " ", 1));
                                // str16 = var name
                                // str17 = type name
                                str13 += "if (__HTVM_V2_TO_JAVA_optionalParams__.length " + Chr(62) + " " + STR(A_Index209) + ") " + str16 + " = (" + str17 + ") __HTVM_V2_TO_JAVA_optionalParams__[" + STR(A_Index209) + "];" + Chr(10);
                            }
                            str12 = str14;
                        }
                        str11 += str12;
            ...
            else if (InStr(StrLower(A_LoopField197), " main(") && lineDone == 0) {
                lineDone = 1;
                javaMainFuncSeen = 1;
                csMainFuncSeen = 1;
                htCode += A_LoopField197 + Chr(10);
            }
            else if (InStr(A_LoopField197, " " + keyWordAssign + " ") || InStr(A_LoopField197, " " + keyWordAssignAdd + " ") || InStr(A_LoopField197, " " + keyWordAssignConcat + " ") || InStr(A_LoopField197, " " + keyWordAssignSub + " ") || InStr(A_LoopField197, " " + keyWordAssignMul + " ") || InStr(A_LoopField197, " " + keyWordAssignDiv + " ") || InStr(A_LoopField197, " " + keyWordAssignMod + " ") || InStr(A_LoopField197, " " + keyWordAssignShiftLeft + " ") || InStr(A_LoopField197, " " + keyWordAssignShiftRight + " ") || InStr(A_LoopField197, " " + keyWordLogicalAssignShiftRight + " ") || InStr(A_LoopField197, " " + keyWordAssignBitAnd + " ") || InStr(A_LoopField197, " " + keyWordAssignBitOr + " ") || InStr(A_LoopField197, " " + keyWordAssignBitXor + " ") && lineDone == 0) {
                allVarsSoWeDontReDecVars_FIX_TOGGLE = 1;
                //keyWordAssign
                //keyWordAssignAdd
                //keyWordAssignConcat
                ...
                //allVarsSoWeDontReDecVars_FIX_int64
                //allVarsSoWeDontReDecVars_FIX_float
                if (InStr(A_LoopField197, " " + keyWordAssign + " ")) {
                    str1 = StrSplit(A_LoopField197, " " + keyWordAssign + " ", 1);
                    str6 = StrSplit(A_LoopField197, " " + keyWordAssign + " ", 2);
                    str10 = keyWordAssign;
                    str11 = keyWordAssign_2;
                    if (langToConvertTo == "ahk") {
                        str12 = ":=";
                ...
                else if (InStr(A_LoopField197, " " + keyWordAssignAdd + " ")) {
                    str1 = StrSplit(A_LoopField197, " " + keyWordAssignAdd + " ", 1);
                    str6 = StrSplit(A_LoopField197, " " + keyWordAssignAdd + " ", 2);
                    str10 = keyWordAssignAdd;
                    str11 = keyWordAssignAdd_2;
                    str12 = "+=";
                }
                else if (InStr(A_LoopField197, " " + keyWordAssignConcat + " ")) {
                    str1 = StrSplit(A_LoopField197, " " + keyWordAssignConcat + " ", 1);
                    str6 = StrSplit(A_LoopField197, " " + keyWordAssignConcat + " ", 2);
                    str10 = keyWordAssignConcat;
                    str11 = keyWordAssignConcat_2;
                    if (langToConvertTo == "ahk") {
                        str12 = ".=";
                ...
                else if (InStr(A_LoopField197, " " + keyWordAssignSub + " ")) {
                    str1 = StrSplit(A_LoopField197, " " + keyWordAssignSub + " ", 1);
                    str6 = StrSplit(A_LoopField197, " " + keyWordAssignSub + " ", 2);
                    str10 = keyWordAssignSub;
                    str11 = keyWordAssignSub_2;
                    str12 = "-=";
                }
                else if (InStr(A_LoopField197, " " + keyWordAssignMul + " ")) {
                    str1 = StrSplit(A_LoopField197, " " + keyWordAssignMul + " ", 1);
                    str6 = StrSplit(A_LoopField197, " " + keyWordAssignMul + " ", 2);
                    str10 = keyWordAssignMul;
                    str11 = keyWordAssignMul_2;
                    str12 = "*=";
                }
                else if (InStr(A_LoopField197, " " + keyWordAssignDiv + " ")) {
                    str1 = StrSplit(A_LoopField197, " " + keyWordAssignDiv + " ", 1);
                    str6 = StrSplit(A_LoopField197, " " + keyWordAssignDiv + " ", 2);
                    str10 = keyWordAssignDiv;
                    str11 = keyWordAssignDiv_2;
                    str12 = "/=";
                }
                else if (InStr(A_LoopField197, " " + keyWordAssignMod + " ")) {
                    str1 = StrSplit(A_LoopField197, " " + keyWordAssignMod + " ", 1);
                    str6 = StrSplit(A_LoopField197, " " + keyWordAssignMod + " ", 2);
                    str10 = keyWordAssignMod;
                    str11 = keyWordAssignMod_2;
                    str12 = "%=";
                }
                else if (InStr(A_LoopField197, " " + keyWordAssignShiftLeft + " ")) {
                    str1 = StrSplit(A_LoopField197, " " + keyWordAssignShiftLeft + " ", 1);
                    str6 = StrSplit(A_LoopField197, " " + keyWordAssignShiftLeft + " ", 2);
                    str10 = keyWordAssignShiftLeft;
                    str11 = keyWordAssignShiftLeft_2;
                    str12 = "<<=";
                }
                else if (InStr(A_LoopField197, " " + keyWordAssignShiftRight + " ")) {
                    str1 = StrSplit(A_LoopField197, " " + keyWordAssignShiftRight + " ", 1);
                    str6 = StrSplit(A_LoopField197, " " + keyWordAssignShiftRight + " ", 2);
                    str10 = keyWordAssignShiftRight;
                    str11 = keyWordAssignShiftRight_2;
                    str12 = ">>=";
                }
                else if (InStr(A_LoopField197, " " + keyWordLogicalAssignShiftRight + " ")) {
                    str1 = StrSplit(A_LoopField197, " " + keyWordLogicalAssignShiftRight + " ", 1);
                    str6 = StrSplit(A_LoopField197, " " + keyWordLogicalAssignShiftRight + " ", 2);
                    str10 = keyWordLogicalAssignShiftRight;
                    str11 = keyWordLogicalAssignShiftRight_2;
                    str12 = ">>>=";
                }
                else if (InStr(A_LoopField197, " " + keyWordAssignBitAnd + " ")) {
                    str1 = StrSplit(A_LoopField197, " " + keyWordAssignBitAnd + " ", 1);
                    str6 = StrSplit(A_LoopField197, " " + keyWordAssignBitAnd + " ", 2);
                    str10 = keyWordAssignBitAnd;
                    str11 = keyWordAssignBitAnd_2;
                    str12 = "&=";
                }
                else if (InStr(A_LoopField197, " " + keyWordAssignBitOr + " ")) {
                    str1 = StrSplit(A_LoopField197, " " + keyWordAssignBitOr + " ", 1);
                    str6 = StrSplit(A_LoopField197, " " + keyWordAssignBitOr + " ", 2);
                    str10 = keyWordAssignBitOr;
                    str11 = keyWordAssignBitOr_2;
                    str12 = "|=";
                }
                else if (InStr(A_LoopField197, " " + keyWordAssignBitXor + " ")) {
                    str1 = StrSplit(A_LoopField197, " " + keyWordAssignBitXor + " ", 1);
                    str6 = StrSplit(A_LoopField197, " " + keyWordAssignBitXor + " ", 2);
                    str10 = keyWordAssignBitXor;
                    str11 = keyWordAssignBitXor_2;
                    str12 = "^=";
                }
            ...
            else if (SubStr(Trim(A_LoopField197), 1, StrLen(keyWordVar)) == keyWordVar && lineDone == 0) {
                allVarsSoWeDontReDecVars_FIX_TOGGLE = 1;
                if (usePostfixTypeForTypeDefinition == "off") {
                    lineDone = 1;
                    str1 = Trim(StringTrimLeft(Trim(A_LoopField197), StrLen(keyWordVar)));
                    if (SubStrLastChars(str1, 1) == ";") {
                        str1 = StringTrimRight(str1, 1);
                    }
            ...
            else if (SubStr(Trim(A_LoopField197), 1, StrLen(keyWordLet)) == keyWordLet && lineDone == 0) {
                allVarsSoWeDontReDecVars_FIX_TOGGLE = 1;
                if (usePostfixTypeForTypeDefinition == "off") {
                    lineDone = 1;
                    str1 = Trim(StringTrimLeft(Trim(A_LoopField197), StrLen(keyWordLet)));
                    if (SubStrLastChars(str1, 1) == ";") {
                        str1 = StringTrimRight(str1, 1);
                    }
            ...
            else if (SubStr(Trim(A_LoopField197), 1, StrLen(keyWordDouble + " ")) == keyWordDouble + " " || SubStr(Trim(A_LoopField197), 1, StrLen(keyWordChar + " ")) == keyWordChar + " " || SubStr(Trim(A_LoopField197), 1, StrLen(keyWordUint8 + " ")) == keyWordUint8 + " " || SubStr(Trim(A_LoopField197), 1, StrLen(keyWordUint16 + " ")) == keyWordUint16 + " " || SubStr(Trim(A_LoopField197), 1, StrLen(keyWordUint32 + " ")) == keyWordUint32 + " " || SubStr(Trim(A_LoopField197), 1, StrLen(keyWordUint64 + " ")) == keyWordUint64 + " " || SubStr(Trim(A_LoopField197), 1, StrLen(keyWordINT + " ")) == keyWordINT + " " || SubStr(Trim(A_LoopField197), 1, StrLen(keyWordSTR + " ")) == keyWordSTR + " " || SubStr(Trim(A_LoopField197), 1, StrLen(keyWordBOOL + " ")) == keyWordBOOL + " " || SubStr(Trim(A_LoopField197), 1, StrLen(keyWordFLOAT + " ")) == keyWordFLOAT + " " || SubStr(Trim(A_LoopField197), 1, StrLen(keyWordINT8 + " ")) == keyWordINT8 + " " || SubStr(Trim(A_LoopField197), 1, StrLen(keyWordINT16 + " ")) == keyWordINT16 + " " || SubStr(Trim(A_LoopField197), 1, StrLen(keyWordINT32 + " ")) == keyWordINT32 + " " || SubStr(Trim(A_LoopField197), 1, StrLen(keyWordINT64 + " ")) == keyWordINT64 + " " || SubStr(Trim(A_LoopField197), 1, StrLen(keyWordArrayDefinition + " ")) == keyWordArrayDefinition + " " || SubStr(Trim(A_LoopField197), 1, StrLen(keyWordArrayOfIntegersDefinition + " ")) == keyWordArrayOfIntegersDefinition + " " || SubStr(Trim(A_LoopField197), 1, StrLen(keyWordArrayOfStringsDefinition + " ")) == keyWordArrayOfStringsDefinition + " " || SubStr(Trim(A_LoopField197), 1, StrLen(keyWordArrayOfFloatingPointNumbersDefinition + " ")) == keyWordArrayOfFloatingPointNumbersDefinition + " " || SubStr(Trim(A_LoopField197), 1, StrLen(keyWordArrayOfBooleansDefinition + " ")) == keyWordArrayOfBooleansDefinition + " " && usePrefixTypeForTypeDefinition == "on" && lineDone == 0) {
                lineDone = 1;
                allVarsSoWeDontReDecVars_FIX_TOGGLE = 1;
                str1 = Trim(A_LoopField197);
                if (SubStrLastChars(str1, 1) == ";") {
            ...
            else if (RegExMatch(Trim(A_LoopField197), "^[a-zA-Z_][a-zA-Z0-9_]*: [^:]*;?$") && usePostfixTypeForTypeDefinition == "on" && lineDone == 0) {
                lineDone = 1;
                allVarsSoWeDontReDecVars_FIX_TOGGLE = 1;
                str1 = Trim(A_LoopField197);
                if (SubStrLastChars(str1, 1) == ";") {
            ...
            else if (RegExMatch(Trim(A_LoopField197), "^[a-zA-Z0-9_\\.]+\\(") && !(InStr(StrLower(A_LoopField197), " main(")) && lineDone == 0) {
                lineDone = 1;
                str1 = Trim(A_LoopField197);
                if (SubStrLastChars(str1, 1) == ";") {
                    str1 = StringTrimRight(str1, 1);
            ...
            else if (RegExMatch(A_LoopField197, "\\.[a-zA-Z_][a-zA-Z0-9_]*") && lineDone == 0) {
                lineDone = 1;
                str1 = Trim(A_LoopField197);
                if (SubStrLastChars(str1, 1) == ";") {
                    str1 = StringTrimRight(str1, 1);
            ...
            else if (KeyWordsCommands(A_LoopField197, "check", commands, langToConvertTo) == "true" && lineDone == 0) {
                lineDone = 1;
                if (langToConvertTo == langFileExtension_2) {
                    htCode += A_LoopField197 + Chr(10);
                } else {
                    out_KeyWordsCommands = KeyWordsCommands(A_LoopField197, "transpile", commands, langToConvertTo);
                    htCode += out_KeyWordsCommands + Chr(10);
                }
            } else {
                //print("else else else " . A_LoopField197)
                // this is THE else
                //;;;;;;;;;;;;;;;;;;
                if (lineDone != 1) {
                    if (skipLeftCuleyForFuncPLS != 1) {
                        if (SubStr(Trim(StrLower(A_LoopField197)), 1, 1) == Chr(125)) {
                            htCode += Chr(125) + Chr(10);
                        } else {
                            if (htCodeAcurlyBraceAddSomeVrasFixLP == 1 && SubStr(Trim(StrLower(A_LoopField197)), 1, 1) == Chr(123)) {
                                htCodeAcurlyBraceAddSomeVrasFixLP = 0;
                                htCode += A_LoopField197 + Chr(10) + theFixTextLoopLP + Chr(10);
                            } else {
                                if (htCodeAcurlyBraceAddSomeVrasFixNL == 1 && SubStr(Trim(StrLower(A_LoopField197)), 1, 1) == Chr(123)) {
                                    htCodeAcurlyBraceAddSomeVrasFixNL = 0;
                                    htCode += A_LoopField197 + Chr(10) + theFixTextLoopNL + Chr(10);
                                } else {
                                    htCode += A_LoopField197 + Chr(10);
                                }
                            }
        ...
        if (langToConvertTo != "ahk" && langToConvertTo != langFileExtension_2) {
            ...
            if (haveWeEverUsedAloop == 1) {
                ...
                for (let A_Index226 = 0; A_Index226 < items226.length + 0; A_Index226++) {
                    ...
                    if (out1 == "nl") {
                        ...
                        for (let A_Index227 = 0; A_Index227 < items227.length + 0; A_Index227++) {
                            ...
                            //MsgBox, |%A_LoopField227%|`n|%fixLoopLokingFor%|
                        ...
                        for (let A_Index228 = 0; A_Index228 < items228.length + 0; A_Index228++) {
                            ...
                            if (InStr(A_LoopField228 , fixLoopLokingFor) && insdeAnestedLoopBAD != 1) {
                                fixLoopLokingForNum = 1;
                                //MsgBox, do we came here 3
                            }
                            if (SubStr(Trim(A_LoopField228), 1, 4) == "for " && weAreDoneHereCurly != 1 && insdeAnestedLoopBAD != 1 && fixLoopLokingForNum == 1) {
                                s = StrSplit(A_LoopField228 ,"" + keyWordAIndex + "", 2);
                                out1z = s;
                                s = StrSplit(out1z, " ", 1);
                                ...
                                foundTheTopLoop++;
                                inTarget = 1;
                                //MsgBox, % A_LoopField228
                                dontSaveStr = 1;
                                ALoopField = A_LoopField228;
                                DeleayOneCuzOfLoopParse = 1;
                                out4758686d86d86d86578991a += ALoopField + Chr(10);
                            }
                            if (inTarget == 1 && InStr(A_LoopField228, Chr(123)) && insdeAnestedLoopBAD != 1) {
                                insideBracket = 1;
                            }
                            if (insideBracket == 1 && InStr(A_LoopField228, Chr(123)) && insdeAnestedLoopBAD != 1) {
                                netsedCurly++;
                            }
                            if (insideBracket == 1 && InStr(A_LoopField228, Chr(125)) && insdeAnestedLoopBAD != 1) {
                                netsedCurly--;
                                readyToEnd = 1;
                            }
                            if (SubStr(Trim(A_LoopField228), 1, 4) == "for " && insdeAnestedLoopBAD != 1 && foundTheTopLoop >= 2) {
                                insdeAnestedLoopBAD = 1;
                                insideBracket1 = 0;
                                netsedCurly1 = 0;
                            }
                            ...
                            if (insdeAnestedLoopBAD == 1) {
                                if (InStr(A_LoopField228, Chr(123))) {
                                    insideBracket1 = 1;
                                }
                                if (insideBracket1 == 1 && InStr(A_LoopField228, Chr(123))) {
                                    netsedCurly1++;
                                }
                                if (insideBracket1 == 1 && InStr(A_LoopField228, Chr(125))) {
                                    netsedCurly1--;
                                    readyToEnd1 = 1;
                                }
                                if (InStr(A_LoopField228, Chr(125)) && readyToEnd1 == 1 && netsedCurly1 == 0 && insideBracket == 1) {
                                    //MsgBox, % A_LoopField228
                                    eldLoopNestedBADlol = 1;
                                }
                                out4758686d86d86d86578991a += A_LoopField228 + Chr(10);
                            }
                            if (inTarget == 1 && dontSaveStr != 1 && fixLoopLokingForNum != 1 && insdeAnestedLoopBAD != 1) {
                                ALoopField = A_LoopField228;
                                // Replace "A_Index" with or without a following digit with "A_Index" + out1z
                            ...
                            if ((inTarget == 1 && InStr(A_LoopField228, Chr(125)) && readyToEnd == 1 && netsedCurly == 0 && weAreDoneHereCurly == 0 && dontSaveStr != 1 && insdeAnestedLoopBAD != 1)) {
                                //MsgBox, % A_LoopField228
                                weAreDoneHereCurly = 1;
                                inTarget = 0;
                                endBracketDOntPutThere = 1;
                ...
                for (let A_Index229 = 0; A_Index229 < items229.length + 0; A_Index229++) {
                    ...
                    ignore = 0;
                    if (SubStr(Trim(A_LoopField229), 1, 4) == "for ") {
                        if (hold == 1 && holdText == A_LoopField229) {
                            ignore = 1;
                        } else {
                            holdText = A_LoopField229;
                            hold = 1;
                        }
                    }
                    if (!ignore) {
                        out4758686d86dgt8r754444444 += A_LoopField229 + Chr(10);
                    }
                }
    ...
    if (COUNT_programmingBlock_HTVMsyntax != 0) {
        ...
        for (let A_Index231 = 0; A_Index231 < COUNT_programmingBlock_HTVMsyntax + 0; A_Index231++) {
            ADD_ALL_programmingBlock_HTVMsyntax += programmingBlock_HTVMsyntax[A_Index231] + Chr(10);
        }
        ...
        isNotHTVMfile2 = 1;
        for (let A_Index232 = 0; A_Index232 < COUNT_programmingBlock_HTVMsyntax + 0; A_Index232++) {
            htCode = StrReplace(htCode, "programmingBlock_HTVMsyntax-programmingBlock_HTVMsyntax-AA" + STR(A_Index232 + 1) + "AA", compiler(programmingBlock_HTVMsyntax[A_Index232], allInstructionFile, "full", langToConvertToParam));
        }
        isNotHTVMfile2 = 0;
    }
    //;;;;;;;;;;;;;;;;;;;;;;;;;;;
    ...
    if (langToConvertTo == langFileExtension_2) {
        ...
        if (useEnd_2 == "on") {
            ...
            for (let A_Index233 = 0; A_Index233 < items233.length + 0; A_Index233++) {
                ...
                if (Trim(A_LoopField233) == keyWordCurlyBraceClose_2) {
                    fixLuaAndRuby += StrReplace(A_LoopField233, keyWordCurlyBraceClose_2, Trim(keyWordEnd_2)) + Chr(10);
                }
                else if (SubStrLastChars(A_LoopField233, 2) == " " + keyWordCurlyBraceOpen_2) {
        ...
        if (langToConvertTo == "lua" || langToConvertTo == "rb" || langToConvertTo == langFileExtension_2) {
            if (langToConvertTo == langFileExtension_2 && useEnd_2 == "on") {
                ...
                for (let A_Index235 = 0; A_Index235 < items235.length + 0; A_Index235++) {
                    ...
                    if (Trim(A_LoopField235) == "}") {
                        fixLuaAndRuby += StrReplace(A_LoopField235, "}", fixLuaAndRubyHELP) + Chr(10);
                    }
                    else if (SubStrLastChars(A_LoopField235, 2) == " {") {
                        fixLuaAndRuby += StringTrimRight(A_LoopField235, 2) + Chr(10);
                    } else {
                        fixLuaAndRuby += A_LoopField235 + Chr(10);
                    }
                }
                htCode = StringTrimRight(fixLuaAndRuby, 1);
            }
    ...
    if (langToConvertTo == langFileExtension_2 && useEnd_2 == "on") {
        ...
        for (let A_Index237 = 0; A_Index237 < items237.length + 0; A_Index237++) {
            ...
            if (Trim(A_LoopField237) == Trim(keyWordEnd_2) && SubStr(Trim(nextWordEndFix[A_Index237 + 1]), 1, StrLen(Trim(keyWordElseIf_2) + " ")) == Trim(keyWordElseIf_2) + " " || Trim(A_LoopField237) == Trim(keyWordEnd_2) && (Trim(nextWordEndFix[A_Index237 + 1]) == Trim(keyWordElse_2) || Trim(nextWordEndFix[A_Index237 + 1]) == Trim(keyWordElse_2 + ":"))) {
                nextWordEndFixOut += "";
            } else {
    ...
    if (langToConvertTo == "lua") {
        ...
        for (let A_Index239 = 0; A_Index239 < items239.length + 0; A_Index239++) {
            ...
            if (Trim(A_LoopField239) == "end" && SubStr(Trim(nextWordEndFix[A_Index239 + 1]), 1, StrLen("elseif ")) == "elseif " || Trim(A_LoopField239) == "end" && Trim(nextWordEndFix[A_Index239 + 1]) == "else") {
                nextWordEndFixOut += "";
            } else {
    ...
    if (langToConvertTo == "rb") {
        ...
        for (let A_Index241 = 0; A_Index241 < items241.length + 0; A_Index241++) {
            ...
            if (Trim(A_LoopField241) == "end" && SubStr(Trim(nextWordEndFix[A_Index241 + 1]), 1, StrLen("elsif ")) == "elsif " || Trim(A_LoopField241) == "end" && Trim(nextWordEndFix[A_Index241 + 1]) == "else") {
                nextWordEndFixOut += "";
            } else {
    ...
    if (langToConvertTo == "rb") {
        ...
        for (let A_Index243 = 0; A_Index243 < items243.length + 0; A_Index243++) {
            ...
            if (Trim(A_LoopField243) == "end" && SubStr(Trim(nextWordEndFixRB[A_Index243 + 1]), 1, StrLen("rescue ")) == "rescue " || Trim(A_LoopField243) == "end" && Trim(nextWordEndFixRB[A_Index243 + 1]) == "ensure") {
                nextWordEndFixOut += "";
            } else {
    ...
    if (langToConvertTo == langFileExtension_2 && useEnd_2 == "on") {
        ...
        for (let A_Index244 = 0; A_Index244 < items244.length + 0; A_Index244++) {
            ...
            HTVM_Append(nextWordEndFixRB, A_LoopField244);
        }
        HTVM_Append(nextWordEndFixRB, " ");
        ...
        for (let A_Index245 = 0; A_Index245 < items245.length + 0; A_Index245++) {
            ...
            if (Trim(A_LoopField245) == Trim(keyWordEnd_2) && SubStr(Trim(nextWordEndFixRB[A_Index245 + 1]), 1, StrLen(Trim(keyWordCatch_2) + " ")) == Trim(keyWordCatch_2) + " " || Trim(A_LoopField245) == Trim(keyWordEnd_2) && Trim(nextWordEndFixRB[A_Index245 + 1]) == Trim(keyWordFinally_2) || Trim(nextWordEndFixRB[A_Index245 + 1]) == Trim(keyWordFinally_2 + ":")) {
                nextWordEndFixOut += "";
            } else {
    ...
    if (langToConvertTo == "ahk") {
        ...
        for (let A_Index246 = 0; A_Index246 < items246.length + 0; A_Index246++) {
            ...
            if (SubStr(Trim(A_LoopField246), 1, StrLen("Loop, Parse, ")) == "Loop, Parse, ") {
                nextWordEndFixOut += StrReplace(A_LoopField246, "{", Chr(10) + "{" + Chr(10)) + Chr(10);
            } else {
    ...
    if (langToConvertTo == "go") {
        ...
        for (let A_Index247 = 0; A_Index247 < items247.length + 0; A_Index247++) {
            ...
            HTVM_Append(fixGoManGoIsSoAnnoyingBroooFurure, A_LoopField247);
        }
        ...
        items248 = LoopParseFunc(htCode, "\n", "\r")
        for (let A_Index248 = 0; A_Index248 < items248.length + 0; A_Index248++) {
            const A_LoopField248 = items248[A_Index248 - 0];
            if (Trim(A_LoopField248) == "}" && SubStr(Trim(fixGoManGoIsSoAnnoyingBroooFurure[A_Index248 + 1]), 1, StrLen("else if ")) == "else if ") {
                fixGoManGoIsSoAnnoyingBroooSkip = 1;
                fixGoManGoIsSoAnnoyingBroooCount = 0;
                items249 = LoopParseFunc(fixGoManGoIsSoAnnoyingBroooFurure[A_Index248 + 1])
                ...
                for (let A_Index250 = 0; A_Index250 < fixGoManGoIsSoAnnoyingBroooCount + 0; A_Index250++) {
                    fixGoManGoIsSoAnnoyingBroooCountSpaceses += " ";
                }
                fixGoManGoIsSoAnnoyingBrooo += fixGoManGoIsSoAnnoyingBroooCountSpaceses + "} " + Trim(fixGoManGoIsSoAnnoyingBroooFurure[A_Index248 + 1]) + Chr(10);
            } else {
                if (fixGoManGoIsSoAnnoyingBroooSkip == 0) {
                    fixGoManGoIsSoAnnoyingBrooo += A_LoopField248 + Chr(10);
                }
    ...
    if (COUNT_programmingBlock_InTheTranspiledLang != 0) {
        for (let A_Index251 = 0; A_Index251 < COUNT_programmingBlock_InTheTranspiledLang + 0; A_Index251++) {
            htCode = StrReplace(htCode, "programmingBlock_InTheTranspiledLang-programmingBlock_InTheTranspiledLang-AA" + STR(A_Index251 + 1) + "AA", programmingBlock_InTheTranspiledLang[A_Index251]);
        }
    }
    if (langToConvertTo == "cpp") {
        if (COUNT_programmingBlock_CPP != 0) {
            for (let A_Index252 = 0; A_Index252 < COUNT_programmingBlock_CPP + 0; A_Index252++) {
                htCode = StrReplace(htCode, "programmingBlock_CPP-programmingBlock_CPP-AA" + STR(A_Index252 + 1) + "AA", programmingBlock_CPP[A_Index252]);
            }
        }
    } else {
        if (COUNT_programmingBlock_CPP != 0) {
            for (let A_Index253 = 0; A_Index253 < COUNT_programmingBlock_CPP + 0; A_Index253++) {
                if (langToConvertTo == langFileExtension_2) {
                    htCode = StrReplace(htCode, "programmingBlock_CPP-programmingBlock_CPP-AA" + STR(A_Index253 + 1) + "AA", keyWordCodeInTheTranspiledLangStartCPP_2 + Chr(10) + programmingBlock_CPP[A_Index253] + Chr(10) + keyWordCodeInTheTranspiledLangEndCPP_2);
                } else {
                    htCode = StrReplace(htCode, "programmingBlock_CPP-programmingBlock_CPP-AA" + STR(A_Index253 + 1) + "AA", Chr(10));
                }
            }
        }
    }
    if (langToConvertTo == "py") {
        if (COUNT_programmingBlock_PY != 0) {
            for (let A_Index254 = 0; A_Index254 < COUNT_programmingBlock_PY + 0; A_Index254++) {
                htCode = StrReplace(htCode, "programmingBlock_PY-programmingBlock_PY-AA" + STR(A_Index254 + 1) + "AA", programmingBlock_PY[A_Index254]);
            }
        }
    } else {
        if (COUNT_programmingBlock_PY != 0) {
            for (let A_Index255 = 0; A_Index255 < COUNT_programmingBlock_PY + 0; A_Index255++) {
                if (langToConvertTo == langFileExtension_2) {
                    htCode = StrReplace(htCode, "programmingBlock_PY-programmingBlock_PY-AA" + STR(A_Index255 + 1) + "AA", keyWordCodeInTheTranspiledLangStartPY_2 + Chr(10) + programmingBlock_PY[A_Index255] + Chr(10) + keyWordCodeInTheTranspiledLangEndPY_2);
                } else {
                    htCode = StrReplace(htCode, "programmingBlock_PY-programmingBlock_PY-AA" + STR(A_Index255 + 1) + "AA", Chr(10));
                }
            }
        }
    }
    if (langToConvertTo == "js") {
        if (COUNT_programmingBlock_JS != 0) {
            for (let A_Index256 = 0; A_Index256 < COUNT_programmingBlock_JS + 0; A_Index256++) {
                htCode = StrReplace(htCode, "programmingBlock_JS-programmingBlock_JS-AA" + STR(A_Index256 + 1) + "AA", programmingBlock_JS[A_Index256]);
            }
        }
    } else {
        if (COUNT_programmingBlock_JS != 0) {
            for (let A_Index257 = 0; A_Index257 < COUNT_programmingBlock_JS + 0; A_Index257++) {
                if (langToConvertTo == langFileExtension_2) {
                    htCode = StrReplace(htCode, "programmingBlock_JS-programmingBlock_JS-AA" + STR(A_Index257 + 1) + "AA", keyWordCodeInTheTranspiledLangStartJS_2 + Chr(10) + programmingBlock_JS[A_Index257] + Chr(10) + keyWordCodeInTheTranspiledLangEndJS_2);
                } else {
                    htCode = StrReplace(htCode, "programmingBlock_JS-programmingBlock_JS-AA" + STR(A_Index257 + 1) + "AA", Chr(10));
                }
            }
        }
    }
    if (langToConvertTo == "go") {
        if (COUNT_programmingBlock_GO != 0) {
            for (let A_Index258 = 0; A_Index258 < COUNT_programmingBlock_GO + 0; A_Index258++) {
                htCode = StrReplace(htCode, "programmingBlock_GO-programmingBlock_GO-AA" + STR(A_Index258 + 1) + "AA", programmingBlock_GO[A_Index258]);
            }
        }
    } else {
        if (COUNT_programmingBlock_GO != 0) {
            for (let A_Index259 = 0; A_Index259 < COUNT_programmingBlock_GO + 0; A_Index259++) {
                if (langToConvertTo == langFileExtension_2) {
                    htCode = StrReplace(htCode, "programmingBlock_GO-programmingBlock_GO-AA" + STR(A_Index259 + 1) + "AA", keyWordCodeInTheTranspiledLangStartGO_2 + Chr(10) + programmingBlock_GO[A_Index259] + Chr(10) + keyWordCodeInTheTranspiledLangEndGO_2);
                } else {
                    htCode = StrReplace(htCode, "programmingBlock_GO-programmingBlock_GO-AA" + STR(A_Index259 + 1) + "AA", Chr(10));
                }
            }
        }
    }
    if (langToConvertTo == "lua") {
        if (COUNT_programmingBlock_LUA != 0) {
            for (let A_Index260 = 0; A_Index260 < COUNT_programmingBlock_LUA + 0; A_Index260++) {
                htCode = StrReplace(htCode, "programmingBlock_LUA-programmingBlock_LUA-AA" + STR(A_Index260 + 1) + "AA", programmingBlock_LUA[A_Index260]);
            }
        }
    } else {
        if (COUNT_programmingBlock_LUA != 0) {
            for (let A_Index261 = 0; A_Index261 < COUNT_programmingBlock_LUA + 0; A_Index261++) {
                if (langToConvertTo == langFileExtension_2) {
                    htCode = StrReplace(htCode, "programmingBlock_LUA-programmingBlock_LUA-AA" + STR(A_Index261 + 1) + "AA", keyWordCodeInTheTranspiledLangStartLUA_2 + Chr(10) + programmingBlock_LUA[A_Index261] + Chr(10) + keyWordCodeInTheTranspiledLangEndLUA_2);
                } else {
                    htCode = StrReplace(htCode, "programmingBlock_LUA-programmingBlock_LUA-AA" + STR(A_Index261 + 1) + "AA", Chr(10));
                }
            }
        }
    }
    if (langToConvertTo == "cs") {
        if (COUNT_programmingBlock_CS != 0) {
            for (let A_Index262 = 0; A_Index262 < COUNT_programmingBlock_CS + 0; A_Index262++) {
                htCode = StrReplace(htCode, "programmingBlock_CS-programmingBlock_CS-AA" + STR(A_Index262 + 1) + "AA", programmingBlock_CS[A_Index262]);
            }
        }
    } else {
        if (COUNT_programmingBlock_CS != 0) {
            for (let A_Index263 = 0; A_Index263 < COUNT_programmingBlock_CS + 0; A_Index263++) {
                if (langToConvertTo == langFileExtension_2) {
                    htCode = StrReplace(htCode, "programmingBlock_CS-programmingBlock_CS-AA" + STR(A_Index263 + 1) + "AA", keyWordCodeInTheTranspiledLangStartCS_2 + Chr(10) + programmingBlock_CS[A_Index263] + Chr(10) + keyWordCodeInTheTranspiledLangEndCS_2);
                } else {
                    htCode = StrReplace(htCode, "programmingBlock_CS-programmingBlock_CS-AA" + STR(A_Index263 + 1) + "AA", Chr(10));
                }
            }
        }
    }
    if (langToConvertTo == "java") {
        if (COUNT_programmingBlock_JAVA != 0) {
            for (let A_Index264 = 0; A_Index264 < COUNT_programmingBlock_JAVA + 0; A_Index264++) {
                htCode = StrReplace(htCode, "programmingBlock_JAVA-programmingBlock_JAVA-AA" + STR(A_Index264 + 1) + "AA", programmingBlock_JAVA[A_Index264]);
            }
        }
    } else {
        if (COUNT_programmingBlock_JAVA != 0) {
            for (let A_Index265 = 0; A_Index265 < COUNT_programmingBlock_JAVA + 0; A_Index265++) {
                if (langToConvertTo == langFileExtension_2) {
                    htCode = StrReplace(htCode, "programmingBlock_JAVA-programmingBlock_JAVA-AA" + STR(A_Index265 + 1) + "AA", keyWordCodeInTheTranspiledLangStartJAVA_2 + Chr(10) + programmingBlock_JAVA[A_Index265] + Chr(10) + keyWordCodeInTheTranspiledLangEndJAVA_2);
                } else {
                    htCode = StrReplace(htCode, "programmingBlock_JAVA-programmingBlock_JAVA-AA" + STR(A_Index265 + 1) + "AA", Chr(10));
                }
            }
        }
    }
    if (langToConvertTo == "kt") {
        if (COUNT_programmingBlock_KT != 0) {
            for (let A_Index266 = 0; A_Index266 < COUNT_programmingBlock_KT + 0; A_Index266++) {
                htCode = StrReplace(htCode, "programmingBlock_KT-programmingBlock_KT-AA" + STR(A_Index266 + 1) + "AA", programmingBlock_KT[A_Index266]);
            }
        }
    } else {
        if (COUNT_programmingBlock_KT != 0) {
            for (let A_Index267 = 0; A_Index267 < COUNT_programmingBlock_KT + 0; A_Index267++) {
                if (langToConvertTo == langFileExtension_2) {
                    htCode = StrReplace(htCode, "programmingBlock_KT-programmingBlock_KT-AA" + STR(A_Index267 + 1) + "AA", keyWordCodeInTheTranspiledLangStartKT_2 + Chr(10) + programmingBlock_KT[A_Index267] + Chr(10) + keyWordCodeInTheTranspiledLangEndKT_2);
                } else {
                    htCode = StrReplace(htCode, "programmingBlock_KT-programmingBlock_KT-AA" + STR(A_Index267 + 1) + "AA", Chr(10));
                }
            }
        }
    }
    if (langToConvertTo == "rb") {
        if (COUNT_programmingBlock_RB != 0) {
            for (let A_Index268 = 0; A_Index268 < COUNT_programmingBlock_RB + 0; A_Index268++) {
                htCode = StrReplace(htCode, "programmingBlock_RB-programmingBlock_RB-AA" + STR(A_Index268 + 1) + "AA", programmingBlock_RB[A_Index268]);
            }
        }
    } else {
        if (COUNT_programmingBlock_RB != 0) {
            for (let A_Index269 = 0; A_Index269 < COUNT_programmingBlock_RB + 0; A_Index269++) {
                if (langToConvertTo == langFileExtension_2) {
                    htCode = StrReplace(htCode, "programmingBlock_RB-programmingBlock_RB-AA" + STR(A_Index269 + 1) + "AA", keyWordCodeInTheTranspiledLangStartRB_2 + Chr(10) + programmingBlock_RB[A_Index269] + Chr(10) + keyWordCodeInTheTranspiledLangEndRB_2);
                } else {
                    htCode = StrReplace(htCode, "programmingBlock_RB-programmingBlock_RB-AA" + STR(A_Index269 + 1) + "AA", Chr(10));
                }
            }
        }
    }
    if (langToConvertTo == "nim") {
        if (COUNT_programmingBlock_NIM != 0) {
            for (let A_Index270 = 0; A_Index270 < COUNT_programmingBlock_NIM + 0; A_Index270++) {
                htCode = StrReplace(htCode, "programmingBlock_NIM-programmingBlock_NIM-AA" + STR(A_Index270 + 1) + "AA", programmingBlock_NIM[A_Index270]);
            }
        }
    } else {
        if (COUNT_programmingBlock_NIM != 0) {
            for (let A_Index271 = 0; A_Index271 < COUNT_programmingBlock_NIM + 0; A_Index271++) {
                if (langToConvertTo == langFileExtension_2) {
                    htCode = StrReplace(htCode, "programmingBlock_NIM-programmingBlock_NIM-AA" + STR(A_Index271 + 1) + "AA", keyWordCodeInTheTranspiledLangStartNIM_2 + Chr(10) + programmingBlock_NIM[A_Index271] + Chr(10) + keyWordCodeInTheTranspiledLangEndNIM_2);
                } else {
                    htCode = StrReplace(htCode, "programmingBlock_NIM-programmingBlock_NIM-AA" + STR(A_Index271 + 1) + "AA", Chr(10));
                }
            }
        }
    }
    if (langToConvertTo == "ahk") {
        if (COUNT_programmingBlock_AHK != 0) {
            for (let A_Index272 = 0; A_Index272 < COUNT_programmingBlock_AHK + 0; A_Index272++) {
                htCode = StrReplace(htCode, "programmingBlock_AHK-programmingBlock_AHK-AA" + STR(A_Index272 + 1) + "AA", programmingBlock_AHK[A_Index272]);
            }
        }
    } else {
        if (COUNT_programmingBlock_AHK != 0) {
            for (let A_Index273 = 0; A_Index273 < COUNT_programmingBlock_AHK + 0; A_Index273++) {
                if (langToConvertTo == langFileExtension_2) {
                    htCode = StrReplace(htCode, "programmingBlock_AHK-programmingBlock_AHK-AA" + STR(A_Index273 + 1) + "AA", keyWordCodeInTheTranspiledLangStartAHK_2 + Chr(10) + programmingBlock_AHK[A_Index273] + Chr(10) + keyWordCodeInTheTranspiledLangEndAHK_2);
                } else {
                    htCode = StrReplace(htCode, "programmingBlock_AHK-programmingBlock_AHK-AA" + STR(A_Index273 + 1) + "AA", Chr(10));
                }
            }
        }
    }
    if (langToConvertTo == "swift") {
        if (COUNT_programmingBlock_SWIFT != 0) {
            for (let A_Index274 = 0; A_Index274 < COUNT_programmingBlock_SWIFT + 0; A_Index274++) {
                htCode = StrReplace(htCode, "programmingBlock_SWIFT-programmingBlock_SWIFT-AA" + STR(A_Index274 + 1) + "AA", programmingBlock_SWIFT[A_Index274]);
            }
        }
    } else {
        if (COUNT_programmingBlock_SWIFT != 0) {
            for (let A_Index275 = 0; A_Index275 < COUNT_programmingBlock_SWIFT + 0; A_Index275++) {
                if (langToConvertTo == langFileExtension_2) {
                    htCode = StrReplace(htCode, "programmingBlock_SWIFT-programmingBlock_SWIFT-AA" + STR(A_Index275 + 1) + "AA", keyWordCodeInTheTranspiledLangStartSWIFT_2 + Chr(10) + programmingBlock_SWIFT[A_Index275] + Chr(10) + keyWordCodeInTheTranspiledLangEndSWIFT_2);
                } else {
                    htCode = StrReplace(htCode, "programmingBlock_SWIFT-programmingBlock_SWIFT-AA" + STR(A_Index275 + 1) + "AA", Chr(10));
                }
            }
        }
    }
    if (langToConvertTo == "dart") {
        if (COUNT_programmingBlock_DART != 0) {
            for (let A_Index276 = 0; A_Index276 < COUNT_programmingBlock_DART + 0; A_Index276++) {
                htCode = StrReplace(htCode, "programmingBlock_DART-programmingBlock_DART-AA" + STR(A_Index276 + 1) + "AA", programmingBlock_DART[A_Index276]);
            }
        }
    } else {
        if (COUNT_programmingBlock_DART != 0) {
            for (let A_Index277 = 0; A_Index277 < COUNT_programmingBlock_DART + 0; A_Index277++) {
                if (langToConvertTo == langFileExtension_2) {
                    htCode = StrReplace(htCode, "programmingBlock_DART-programmingBlock_DART-AA" + STR(A_Index277 + 1) + "AA", keyWordCodeInTheTranspiledLangStartDART_2 + Chr(10) + programmingBlock_DART[A_Index277] + Chr(10) + keyWordCodeInTheTranspiledLangEndDART_2);
                } else {
                    htCode = StrReplace(htCode, "programmingBlock_DART-programmingBlock_DART-AA" + STR(A_Index277 + 1) + "AA", Chr(10));
                }
            }
        }
    }
    if (langToConvertTo == "ts") {
        if (COUNT_programmingBlock_TS != 0) {
            for (let A_Index278 = 0; A_Index278 < COUNT_programmingBlock_TS + 0; A_Index278++) {
                htCode = StrReplace(htCode, "programmingBlock_TS-programmingBlock_TS-AA" + STR(A_Index278 + 1) + "AA", programmingBlock_TS[A_Index278]);
            }
        }
    } else {
        if (COUNT_programmingBlock_TS != 0) {
            for (let A_Index279 = 0; A_Index279 < COUNT_programmingBlock_TS + 0; A_Index279++) {
                if (langToConvertTo == langFileExtension_2) {
                    htCode = StrReplace(htCode, "programmingBlock_TS-programmingBlock_TS-AA" + STR(A_Index279 + 1) + "AA", keyWordCodeInTheTranspiledLangStartTS_2 + Chr(10) + programmingBlock_TS[A_Index279] + Chr(10) + keyWordCodeInTheTranspiledLangEndTS_2);
                } else {
                    htCode = StrReplace(htCode, "programmingBlock_TS-programmingBlock_TS-AA" + STR(A_Index279 + 1) + "AA", Chr(10));
                }
            }
        }
    }
    if (langToConvertTo == "groovy") {
        if (COUNT_programmingBlock_GROOVY != 0) {
            for (let A_Index280 = 0; A_Index280 < COUNT_programmingBlock_GROOVY + 0; A_Index280++) {
                htCode = StrReplace(htCode, "programmingBlock_GROOVY-programmingBlock_GROOVY-AA" + STR(A_Index280 + 1) + "AA", programmingBlock_GROOVY[A_Index280]);
            }
        }
    } else {
        if (COUNT_programmingBlock_GROOVY != 0) {
            for (let A_Index281 = 0; A_Index281 < COUNT_programmingBlock_GROOVY + 0; A_Index281++) {
                if (langToConvertTo == langFileExtension_2) {
                    htCode = StrReplace(htCode, "programmingBlock_GROOVY-programmingBlock_GROOVY-AA" + STR(A_Index281 + 1) + "AA", keyWordCodeInTheTranspiledLangStartGROOVY_2 + Chr(10) + programmingBlock_GROOVY[A_Index281] + Chr(10) + keyWordCodeInTheTranspiledLangEndGROOVY_2);
                } else {
                    htCode = StrReplace(htCode, "programmingBlock_GROOVY-programmingBlock_GROOVY-AA" + STR(A_Index281 + 1) + "AA", Chr(10));
                }
            }
        }
    }
    ...
    if (langToConvertTo == langFileExtension_2) {
        if (COUNT_programmingBlock_HTVM != 0) {
            for (let A_Index282 = 0; A_Index282 < COUNT_programmingBlock_HTVM + 0; A_Index282++) {
                htCode = StrReplace(htCode, "programmingBlock_HTVM-programmingBlock_HTVM-AA" + STR(A_Index282 + 1) + "AA", programmingBlock_HTVM[A_Index282]);
            }
        }
    } else {
        if (COUNT_programmingBlock_HTVM != 0) {
            for (let A_Index283 = 0; A_Index283 < COUNT_programmingBlock_HTVM + 0; A_Index283++) {
                htCode = StrReplace(htCode, "programmingBlock_HTVM-programmingBlock_HTVM-AA" + STR(A_Index283 + 1) + "AA", Chr(10));
            }
        }
    }
    if (langToConvertTo != langFileExtension_2) {
        for (let A_Index284 = 0; A_Index284 < theIdNumOfThe34 + 0; A_Index284++) {
            if (theIdNumOfThe34 == A_Index284 + 1) {
                if (langToConvertTo == langFileExtension_2) {
                    if (keyWordEscpaeChar_2 == "\\" && keyWordEscpaeChar != "\\") {
                        htCode = StrReplace(htCode, "fihuiuuhuuhtheidFor--asds" + str21 + "as--" + str21 + "theuhturtyphoutr--" + Chr(65) + Chr(65) + STR(A_Index284 + 1) + Chr(65) + Chr(65), StrReplace(StrReplace(fString(theIdNumOfThe34theVar[A_Index284 + 1]), "\\", "\\\\"), keyWordEscpaeChar, keyWordEscpaeChar_2) + Chr(34));
                    } else {
                        htCode = StrReplace(htCode, "fihuiuuhuuhtheidFor--asds" + str21 + "as--" + str21 + "theuhturtyphoutr--" + Chr(65) + Chr(65) + STR(A_Index284 + 1) + Chr(65) + Chr(65), StrReplace(fString(theIdNumOfThe34theVar[A_Index284 + 1]), keyWordEscpaeChar, keyWordEscpaeChar_2) + Chr(34));
                    }
                } else {
                    htCode = StrReplace(htCode, "fihuiuuhuuhtheidFor--asds" + str21 + "as--" + str21 + "theuhturtyphoutr--" + Chr(65) + Chr(65) + STR(A_Index284 + 1) + Chr(65) + Chr(65), StrReplace(fString(theIdNumOfThe34theVar[A_Index284 + 1]), keyWordEscpaeChar, "\\") + Chr(34));
                }
            } else {
                if (langToConvertTo == langFileExtension_2) {
                    if (keyWordEscpaeChar_2 == "\\" && keyWordEscpaeChar != "\\") {
                        htCode = StrReplace(htCode, "fihuiuuhuuhtheidFor--asds" + str21 + "as--" + str21 + "theuhturtyphoutr--" + Chr(65) + Chr(65) + STR(A_Index284 + 1) + Chr(65) + Chr(65), StrReplace(StrReplace(fString(theIdNumOfThe34theVar[A_Index284 + 1]), "\\", "\\\\"), keyWordEscpaeChar, keyWordEscpaeChar_2));
                    } else {
                        htCode = StrReplace(htCode, "fihuiuuhuuhtheidFor--asds" + str21 + "as--" + str21 + "theuhturtyphoutr--" + Chr(65) + Chr(65) + STR(A_Index284 + 1) + Chr(65) + Chr(65), StrReplace(fString(theIdNumOfThe34theVar[A_Index284 + 1]), keyWordEscpaeChar, keyWordEscpaeChar_2));
                    }
                } else {
                    htCode = StrReplace(htCode, "fihuiuuhuuhtheidFor--asds" + str21 + "as--" + str21 + "theuhturtyphoutr--" + Chr(65) + Chr(65) + STR(A_Index284 + 1) + Chr(65) + Chr(65), StrReplace(fString(theIdNumOfThe34theVar[A_Index284 + 1]), keyWordEscpaeChar, "\\"));
                }
            }
        }
    }
    ...
    for (let A_Index285 = 0; A_Index285 < items285.length + 0; A_Index285++) {
        ...
        if (areWeInAFuncFromInstructions == 1) {
            if (areWeInAFuncFromInstructionsLineNum == 1) {
                // name of the func
                funcLangHolder = StringTrimLeft(A_LoopField285, 5);
            ...
            if (areWeInAFuncFromInstructionsLineNum == 2) {
                // name of the func
                funcNameHolder = StringTrimLeft(A_LoopField285, 5);
                if (correctLang == 1 && InStr(htCode, Trim(funcNameHolder))) {
            ...
            if (areWeInAFuncFromInstructionsLineNum == 3) {
                // all libs
                funcLibsHolder = StringTrimLeft(A_LoopField285, 5);
                if (correctLang == 1 && InStr(htCode, Trim(funcNameHolder))) {
            ...
            if (areWeInAFuncFromInstructionsLineNum == 4) {
                // func description
                funcDescriptionHolder = StringTrimLeft(A_LoopField285, 12);
                if (correctLang == 1 && InStr(htCode, Trim(funcNameHolder))) {
            ...
            if (areWeInAFuncFromInstructionsLineNum >= 5) {
                // the full func
                if (correctLang == 1 && InStr(htCode, Trim(funcNameHolder))) {
                    funcFuncHolder += A_LoopField285 + Chr(10);
                }
            }
            //MsgBox, % A_LoopField285
            areWeInAFuncFromInstructionsLineNum++;
        }
        if (Trim(A_LoopField285) == "func======================func==============") {
    ...
    if (!(HTVM_Size(allFuncNames) <= 0)) {
        for (let A_Index286 = 0; A_Index286 < HTVM_Size(allFuncNames) + 0; A_Index286++) {
            if (InStr(htCode, allFuncNames[A_Index286]) + "(") {
                //MsgBox, % allFuncNames[A_Index286]
                allFuncsToPutAtTop += allFuncs[A_Index286] + Chr(10);
                if (Trim(allFuncLibs[A_Index286]) != "null") {
                    allLibsToPutAtTop += allFuncLibs[A_Index286] + "|";
                }
            }
        }
        allLibsToPutAtTop = StringTrimRight(allLibsToPutAtTop, 1);
        var allLibsToPutAtTopTEMP = "";
        items287 = LoopParseFunc(allLibsToPutAtTop, "|")
        for (let A_Index287 = 0; A_Index287 < items287.length + 0; A_Index287++) {
            const A_LoopField287 = items287[A_Index287 - 0];
            allLibsToPutAtTopTEMP += A_LoopField287 + Chr(10);
        }
    ...
    if (langToConvertTo != langFileExtension_2) {
        for (let A_Index288 = 0; A_Index288 < HTVM_Size(ospDic1) + 0; A_Index288++) {
            str00 = ospDic1[A_Index288];
            if (langToConvertTo == "rb") {
    ...
    if (size_ospDic != 0) {
        for (let A_Index289 = 0; A_Index289 < size_ospDic + 0; A_Index289++) {
            HTVM_Pop(ospDic);
        }
    }
    //;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;
    ...
    if (size_ospDic1 != 0) {
        for (let A_Index290 = 0; A_Index290 < size_ospDic1 + 0; A_Index290++) {
            HTVM_Pop(ospDic1);
        }
    }
    //;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;
    ...
    if (size_ospDic2 != 0) {
        for (let A_Index291 = 0; A_Index291 < size_ospDic2 + 0; A_Index291++) {
            HTVM_Pop(ospDic2);
        }
    }
    availableFuncsInHTVMInHTVM = "";
    ...
    for (let A_Index292 = 0; A_Index292 < theIdNumOfThe34 + 0; A_Index292++) {
        if (theIdNumOfThe34 == A_Index292 + 1) {
            if (langToConvertTo == langFileExtension_2) {
                if (keyWordEscpaeChar_2 == "\\" && keyWordEscpaeChar != "\\") {
                    htCode = StrReplace(htCode, "ihuiuuhuuhtheidFor--asds" + str21 + "as--" + str21 + "theuhturtyphoutr--" + Chr(65) + Chr(65) + STR(A_Index292 + 1) + Chr(65) + Chr(65), StrReplace(StrReplace(theIdNumOfThe34theVar[A_Index292 + 1], "\\", "\\\\"), keyWordEscpaeChar, keyWordEscpaeChar_2) + Chr(34));
                } else {
                    htCode = StrReplace(htCode, "ihuiuuhuuhtheidFor--asds" + str21 + "as--" + str21 + "theuhturtyphoutr--" + Chr(65) + Chr(65) + STR(A_Index292 + 1) + Chr(65) + Chr(65), StrReplace(theIdNumOfThe34theVar[A_Index292 + 1], keyWordEscpaeChar, keyWordEscpaeChar_2) + Chr(34));
                }
            } else {
                htCode = StrReplace(htCode, "ihuiuuhuuhtheidFor--asds" + str21 + "as--" + str21 + "theuhturtyphoutr--" + Chr(65) + Chr(65) + STR(A_Index292 + 1) + Chr(65) + Chr(65), StrReplace(theIdNumOfThe34theVar[A_Index292 + 1], keyWordEscpaeChar, "\\") + Chr(34));
            }
        } else {
            if (langToConvertTo == langFileExtension_2) {
                if (keyWordEscpaeChar_2 == "\\" && keyWordEscpaeChar != "\\") {
                    htCode = StrReplace(htCode, "ihuiuuhuuhtheidFor--asds" + str21 + "as--" + str21 + "theuhturtyphoutr--" + Chr(65) + Chr(65) + STR(A_Index292 + 1) + Chr(65) + Chr(65), StrReplace(StrReplace(theIdNumOfThe34theVar[A_Index292 + 1], "\\", "\\\\"), keyWordEscpaeChar, keyWordEscpaeChar_2));
                } else {
                    htCode = StrReplace(htCode, "ihuiuuhuuhtheidFor--asds" + str21 + "as--" + str21 + "theuhturtyphoutr--" + Chr(65) + Chr(65) + STR(A_Index292 + 1) + Chr(65) + Chr(65), StrReplace(theIdNumOfThe34theVar[A_Index292 + 1], keyWordEscpaeChar, keyWordEscpaeChar_2));
                }
            } else {
                htCode = StrReplace(htCode, "ihuiuuhuuhtheidFor--asds" + str21 + "as--" + str21 + "theuhturtyphoutr--" + Chr(65) + Chr(65) + STR(A_Index292 + 1) + Chr(65) + Chr(65), StrReplace(theIdNumOfThe34theVar[A_Index292 + 1], keyWordEscpaeChar, "\\"));
            }
        }
    }
    if (langToConvertTo == langFileExtension_2) {
    ...
    if (isNotHTVMfile2 == 0) {
        ...
        if (langToConvertTo == langFileExtension_2) {
            for (let A_Index293 = 0; A_Index293 < HTVM_Size(weAreInMLSarr) + 0; A_Index293++) {
                htCode = StrReplace(htCode, "aejsoydoxubviycbdgsut7eoy-ihsxbvhowadesdio6376e7wuwyuau--szd--AA" + STR(A_Index293) + "AA", weAreInMLSarr[A_Index293]);
            }
    ...
    if (size_weAreInMLSarr != 0) {
        for (let A_Index294 = 0; A_Index294 < size_weAreInMLSarr + 0; A_Index294++) {
...
function HTVMv2() {
    ...
    if (HTVM_getLang_HTVM() == "cpp" || HTVM_getLang_HTVM() == "py") {
        ...
        items296 = LoopParseFunc(allArgs, "\n", "\r")
        for (let A_Index296 = 0; A_Index296 < items296.length + 0; A_Index296++) {
            const A_LoopField296 = items296[A_Index296 - 0];
            if (A_Index296 == 0) {
                numOfParams++;
                argCODE = FileRead(Trim(A_LoopField296));
                argCODEfile = Trim(A_LoopField296);
            }
            else if (A_Index296 == 1) {
                numOfParams++;
                argHTVMinstr = Trim(A_LoopField296);
            }
            else if (A_Index296 == 2) {
                numOfParams++;
                argLangTo = Trim(A_LoopField296);
            } else {
                numOfParams++;
                HTVM_Append(argHTVMinstrMORE, Trim(A_LoopField296));
            }
        }
        //print("===============123431234===========start=====")
        for (let A_Index297 = 0; A_Index297 < HTVM_Size(argHTVMinstrMORE) + 0; A_Index297++) {
            //print(argHTVMinstrMORE[A_Index297])
        }
        //print("===============123431234==========end======")
