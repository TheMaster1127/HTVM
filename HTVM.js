...
function guiParserStep3(guiCode) {
    ...
    for (let A_Index100 = 0; A_Index100 < items100.length + 0; A_Index100++) {
        ...
        if (SubStr(Trim(StrLower(A_LoopField100)), 1, 4) == "gui " || RegExMatch(StrLower(Trim(A_LoopField100)), "^gui[-_][A-Za-z0-9_]+(?:[-_][A-Za-z0-9_]+)*(?=[, ]|$)")) {
            isElement = 0;
            whatElement = "";
            items101 = LoopParseFunc(A_LoopField100, " ")
            ...
            isElement = 0;
            whatElement = "";
            items102 = LoopParseFunc(A_LoopField100, " ")
...
function guiParserStep2(guiCode) {
    ...
    var out = "";
    var temp = "";
    items103 = LoopParseFunc(guiCode, "\n", "\r")
...
function guiParser0(guiCode) {
    ...
    var begin = 0;
    let lookAhead = [];
    items104 = LoopParseFunc(guiCode, "\n", "\r")
...
function guiParser(guiCode) {
    ...
    var begin = 0;
    let lookAhead = [];
    items106 = LoopParseFunc(guiCode, "\n", "\r")
...
function slidingWinFixCommentsRmFUNC(line) {
    ...
    for (let A_Index112 = 0; A_Index112 < StrLen(keyWordComment + keyWordComment) + 2 + 0; A_Index112++) {
...
function fixIfElseIfMultiLineStatementHELP(aloopfield) {
    var out = "";
    var count = 0;
    items113 = LoopParseFunc(aloopfield)
...
function fixIfElseIfMultiLineStatement(someHTVMcode) {
    var doWeExitInsta = 1;
    items114 = LoopParseFunc(someHTVMcode, "\n", "\r")
    ...
    for (let A_Index115 = 0; A_Index115 < 10000 + 0; A_Index115++) {
        out2 = out;
        out = "";
        doWeExit = false;
        items116 = LoopParseFunc(out2, "\n", "\r")
...
function compiler(htCode, allInstructionFile, mode, langToConvertToParam = "") {
    ...
    items117 = LoopParseFunc(allInstructionFile, "\n", "\r")
    ...
    if (HTVM_Size(argHTVMinstrMORE) > 0) {
        ...
        items118 = LoopParseFunc(allInstructionFile_2, "\n", "\r")
    ...
    */
    ...
    items119 = LoopParseFunc(allInstructionFile, "\n", "\r")
    ...
    if (isNotHTVMfile == 0) {
        if (langToConvertTo != langFileExtension_2) {
            if (InStr(StrLower(code), StrLower(keyWordInclude + " "))) {
                for (let A_Index121 = 0; A_Index121 < 10000 + 0; A_Index121++) {
                    doseCodeHaveLibs = 0;
                    allLibCODE = "";
                    if (A_Index121 == 0) {
                        ...
                        for (let A_Index123 = 0; A_Index123 < items123.length + 0; A_Index123++) {
                            ...
                            if (SubStr(StrLower(Trim(A_LoopField123)), 1, StrLen(StrLower(keyWordInclude + " "))) == StrLower(keyWordInclude + " ")) {
                                ...
                                for (let A_Index124 = 0; A_Index124 < HTVM_Size(allLibsWeSee) + 0; A_Index124++) {
                    ...
                    for (let A_Index125 = 0; A_Index125 < HTVM_Size(allLibsWeSee) + 0; A_Index125++) {
                        allLibsWeSeeHOLD = allLibsWeSee[A_Index125];
                        doseCodeHaveLibs_HAS_SEEN2 = 0;
                        for (let A_Index126 = 0; A_Index126 < HTVM_Size(allLibsWeSeeSEE) - HTVM_Size(allLibsWeSee) + 0; A_Index126++) {
    ...
    if (isNotHTVMfile == 0) {
        ...
        for (let A_Index134 = 0; A_Index134 < items134.length + 0; A_Index134++) {
            ...
            if (InStr(A_LoopField134, " " + keyWordComment) != false && SubStr(Trim(A_LoopField134), 1, StrLen(keyWordComment)) != keyWordComment) {
                ...
                for (let A_Index135 = 0; A_Index135 < HTVM_Size(slidingWinFixCommentsRm) + 0; A_Index135++) {
                    slidingWinFixCommentsRmI = A_Index135;
                    slidingWinFixCommentsRmHold = "";
                    for (let A_Index136 = 0; A_Index136 < StrLen(" " + keyWordComment) + 0; A_Index136++) {
        ...
        if (useCurlyBraces == "on") {
            ...
            if (keyWordCurlyBraceClose != "}") {
                ...
                for (let A_Index143 = 0; A_Index143 < items143.length + 0; A_Index143++) {
                    ...
                    items144 = LoopParseFunc(A_LoopField143, " ")
        ...
        for (let A_Index150 = 0; A_Index150 < 2 + 0; A_Index150++) {
            outCodeFixBraces = "";
            items151 = LoopParseFunc(code, "\n", "\r")
        ...
        if (langToConvertTo != "ahk" && langToConvertTo != langFileExtension_2) {
            ...
            if (haveWeEverUsedAloop == 1) {
                ...
                items184 = LoopParseFunc(htCodeLoopfixa, "\n", "\r")
    ...
    if (COUNT_programmingBlock_HTVMsyntax != 0) {
        ...
        for (let A_Index189 = 0; A_Index189 < COUNT_programmingBlock_HTVMsyntax + 0; A_Index189++) {
        ...
        isNotHTVMfile2 = 1;
        for (let A_Index190 = 0; A_Index190 < COUNT_programmingBlock_HTVMsyntax + 0; A_Index190++) {
    ...
    if (langToConvertTo == "go") {
        ...
        for (let A_Index206 = 0; A_Index206 < items206.length + 0; A_Index206++) {
            ...
            if (Trim(A_LoopField206) == "}" && SubStr(Trim(fixGoManGoIsSoAnnoyingBroooFurure[A_Index206 + 1]), 1, StrLen("else if ")) == "else if ") {
                ...
                items207 = LoopParseFunc(fixGoManGoIsSoAnnoyingBroooFurure[A_Index206 + 1])
                ...
                for (let A_Index208 = 0; A_Index208 < fixGoManGoIsSoAnnoyingBroooCount + 0; A_Index208++) {
                    fixGoManGoIsSoAnnoyingBroooCountSpaceses += " ";
                }
    ...
    if (COUNT_programmingBlock_InTheTranspiledLang != 0) {
        for (let A_Index209 = 0; A_Index209 < COUNT_programmingBlock_InTheTranspiledLang + 0; A_Index209++) {
    ...
    if (langToConvertTo == "cpp") {
        if (COUNT_programmingBlock_CPP != 0) {
            for (let A_Index210 = 0; A_Index210 < COUNT_programmingBlock_CPP + 0; A_Index210++) {
        ...
        if (COUNT_programmingBlock_CPP != 0) {
            for (let A_Index211 = 0; A_Index211 < COUNT_programmingBlock_CPP + 0; A_Index211++) {
                if (langToConvertTo == langFileExtension_2) {
    ...
    if (langToConvertTo == "py") {
        if (COUNT_programmingBlock_PY != 0) {
            for (let A_Index212 = 0; A_Index212 < COUNT_programmingBlock_PY + 0; A_Index212++) {
                htCode = StrReplace(htCode, "programmingBlock_PY-programmingBlock_PY-AA" + STR(A_Index212 + 1) + "AA", programmingBlock_PY[A_Index212]);
            }
        }
    } else {
        if (COUNT_programmingBlock_PY != 0) {
            for (let A_Index213 = 0; A_Index213 < COUNT_programmingBlock_PY + 0; A_Index213++) {
                if (langToConvertTo == langFileExtension_2) {
    ...
    if (langToConvertTo == "js") {
        if (COUNT_programmingBlock_JS != 0) {
            for (let A_Index214 = 0; A_Index214 < COUNT_programmingBlock_JS + 0; A_Index214++) {
                htCode = StrReplace(htCode, "programmingBlock_JS-programmingBlock_JS-AA" + STR(A_Index214 + 1) + "AA", programmingBlock_JS[A_Index214]);
            }
        }
    } else {
        if (COUNT_programmingBlock_JS != 0) {
            for (let A_Index215 = 0; A_Index215 < COUNT_programmingBlock_JS + 0; A_Index215++) {
                if (langToConvertTo == langFileExtension_2) {
    ...
    if (langToConvertTo == "go") {
        if (COUNT_programmingBlock_GO != 0) {
            for (let A_Index216 = 0; A_Index216 < COUNT_programmingBlock_GO + 0; A_Index216++) {
                htCode = StrReplace(htCode, "programmingBlock_GO-programmingBlock_GO-AA" + STR(A_Index216 + 1) + "AA", programmingBlock_GO[A_Index216]);
            }
        }
    } else {
        if (COUNT_programmingBlock_GO != 0) {
            for (let A_Index217 = 0; A_Index217 < COUNT_programmingBlock_GO + 0; A_Index217++) {
                if (langToConvertTo == langFileExtension_2) {
    ...
    if (langToConvertTo == "lua") {
        if (COUNT_programmingBlock_LUA != 0) {
            for (let A_Index218 = 0; A_Index218 < COUNT_programmingBlock_LUA + 0; A_Index218++) {
        ...
        if (COUNT_programmingBlock_LUA != 0) {
            for (let A_Index219 = 0; A_Index219 < COUNT_programmingBlock_LUA + 0; A_Index219++) {
                if (langToConvertTo == langFileExtension_2) {
    ...
    if (langToConvertTo == "cs") {
        if (COUNT_programmingBlock_CS != 0) {
            for (let A_Index220 = 0; A_Index220 < COUNT_programmingBlock_CS + 0; A_Index220++) {
                htCode = StrReplace(htCode, "programmingBlock_CS-programmingBlock_CS-AA" + STR(A_Index220 + 1) + "AA", programmingBlock_CS[A_Index220]);
            }
        }
    } else {
        if (COUNT_programmingBlock_CS != 0) {
            for (let A_Index221 = 0; A_Index221 < COUNT_programmingBlock_CS + 0; A_Index221++) {
                if (langToConvertTo == langFileExtension_2) {
    ...
    if (langToConvertTo == "java") {
        if (COUNT_programmingBlock_JAVA != 0) {
            for (let A_Index222 = 0; A_Index222 < COUNT_programmingBlock_JAVA + 0; A_Index222++) {
        ...
        if (COUNT_programmingBlock_JAVA != 0) {
            for (let A_Index223 = 0; A_Index223 < COUNT_programmingBlock_JAVA + 0; A_Index223++) {
                if (langToConvertTo == langFileExtension_2) {
    ...
    if (langToConvertTo == "kt") {
        if (COUNT_programmingBlock_KT != 0) {
            for (let A_Index224 = 0; A_Index224 < COUNT_programmingBlock_KT + 0; A_Index224++) {
                htCode = StrReplace(htCode, "programmingBlock_KT-programmingBlock_KT-AA" + STR(A_Index224 + 1) + "AA", programmingBlock_KT[A_Index224]);
            }
        }
    } else {
        if (COUNT_programmingBlock_KT != 0) {
            for (let A_Index225 = 0; A_Index225 < COUNT_programmingBlock_KT + 0; A_Index225++) {
                if (langToConvertTo == langFileExtension_2) {
    ...
    if (langToConvertTo == "rb") {
        if (COUNT_programmingBlock_RB != 0) {
            for (let A_Index226 = 0; A_Index226 < COUNT_programmingBlock_RB + 0; A_Index226++) {
                htCode = StrReplace(htCode, "programmingBlock_RB-programmingBlock_RB-AA" + STR(A_Index226 + 1) + "AA", programmingBlock_RB[A_Index226]);
            }
        }
    } else {
        if (COUNT_programmingBlock_RB != 0) {
            for (let A_Index227 = 0; A_Index227 < COUNT_programmingBlock_RB + 0; A_Index227++) {
                if (langToConvertTo == langFileExtension_2) {
    ...
    if (langToConvertTo == "nim") {
        if (COUNT_programmingBlock_NIM != 0) {
            for (let A_Index228 = 0; A_Index228 < COUNT_programmingBlock_NIM + 0; A_Index228++) {
        ...
        if (COUNT_programmingBlock_NIM != 0) {
            for (let A_Index229 = 0; A_Index229 < COUNT_programmingBlock_NIM + 0; A_Index229++) {
                if (langToConvertTo == langFileExtension_2) {
    ...
    if (langToConvertTo == "ahk") {
        if (COUNT_programmingBlock_AHK != 0) {
            for (let A_Index230 = 0; A_Index230 < COUNT_programmingBlock_AHK + 0; A_Index230++) {
        ...
        if (COUNT_programmingBlock_AHK != 0) {
            for (let A_Index231 = 0; A_Index231 < COUNT_programmingBlock_AHK + 0; A_Index231++) {
                if (langToConvertTo == langFileExtension_2) {
    ...
    if (langToConvertTo == "swift") {
        if (COUNT_programmingBlock_SWIFT != 0) {
            for (let A_Index232 = 0; A_Index232 < COUNT_programmingBlock_SWIFT + 0; A_Index232++) {
        ...
        if (COUNT_programmingBlock_SWIFT != 0) {
            for (let A_Index233 = 0; A_Index233 < COUNT_programmingBlock_SWIFT + 0; A_Index233++) {
                if (langToConvertTo == langFileExtension_2) {
    ...
    if (langToConvertTo == "dart") {
        if (COUNT_programmingBlock_DART != 0) {
            for (let A_Index234 = 0; A_Index234 < COUNT_programmingBlock_DART + 0; A_Index234++) {
        ...
        if (COUNT_programmingBlock_DART != 0) {
            for (let A_Index235 = 0; A_Index235 < COUNT_programmingBlock_DART + 0; A_Index235++) {
                if (langToConvertTo == langFileExtension_2) {
    ...
    if (langToConvertTo == "ts") {
        if (COUNT_programmingBlock_TS != 0) {
            for (let A_Index236 = 0; A_Index236 < COUNT_programmingBlock_TS + 0; A_Index236++) {
                htCode = StrReplace(htCode, "programmingBlock_TS-programmingBlock_TS-AA" + STR(A_Index236 + 1) + "AA", programmingBlock_TS[A_Index236]);
            }
        }
    } else {
        if (COUNT_programmingBlock_TS != 0) {
            for (let A_Index237 = 0; A_Index237 < COUNT_programmingBlock_TS + 0; A_Index237++) {
                if (langToConvertTo == langFileExtension_2) {
    ...
    if (langToConvertTo == "groovy") {
        if (COUNT_programmingBlock_GROOVY != 0) {
            for (let A_Index238 = 0; A_Index238 < COUNT_programmingBlock_GROOVY + 0; A_Index238++) {
        ...
        if (COUNT_programmingBlock_GROOVY != 0) {
            for (let A_Index239 = 0; A_Index239 < COUNT_programmingBlock_GROOVY + 0; A_Index239++) {
                if (langToConvertTo == langFileExtension_2) {
    ...
    if (langToConvertTo == langFileExtension_2) {
        if (COUNT_programmingBlock_HTVM != 0) {
            for (let A_Index240 = 0; A_Index240 < COUNT_programmingBlock_HTVM + 0; A_Index240++) {
        ...
        if (COUNT_programmingBlock_HTVM != 0) {
            for (let A_Index241 = 0; A_Index241 < COUNT_programmingBlock_HTVM + 0; A_Index241++) {
                htCode = StrReplace(htCode, "programmingBlock_HTVM-programmingBlock_HTVM-AA" + STR(A_Index241 + 1) + "AA", Chr(10));
            }
        }
    }
    if (langToConvertTo != langFileExtension_2) {
        for (let A_Index242 = 0; A_Index242 < theIdNumOfThe34 + 0; A_Index242++) {
    ...
    var correctLang = 0;
    items243 = LoopParseFunc(instructions, "\n", "\r")
    ...
    if (!(HTVM_Size(allFuncNames) <= 0)) {
        for (let A_Index244 = 0; A_Index244 < HTVM_Size(allFuncNames) + 0; A_Index244++) {
        ...
        items245 = LoopParseFunc(allLibsToPutAtTop, "|")
    ...
    if (langToConvertTo != langFileExtension_2) {
        for (let A_Index246 = 0; A_Index246 < HTVM_Size(ospDic1) + 0; A_Index246++) {
    ...
    if (size_ospDic != 0) {
        for (let A_Index247 = 0; A_Index247 < size_ospDic + 0; A_Index247++) {
            HTVM_Pop(ospDic);
        }
    }
    //;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;
    ...
    if (size_ospDic1 != 0) {
        for (let A_Index248 = 0; A_Index248 < size_ospDic1 + 0; A_Index248++) {
            HTVM_Pop(ospDic1);
        }
    }
    //;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;
    ...
    if (size_ospDic2 != 0) {
        for (let A_Index249 = 0; A_Index249 < size_ospDic2 + 0; A_Index249++) {
            HTVM_Pop(ospDic2);
        }
    }
    availableFuncsInHTVMInHTVM = "";
    ...
    for (let A_Index250 = 0; A_Index250 < theIdNumOfThe34 + 0; A_Index250++) {
    ...
    if (isNotHTVMfile2 == 0) {
        ...
        if (langToConvertTo == langFileExtension_2) {
            for (let A_Index251 = 0; A_Index251 < HTVM_Size(weAreInMLSarr) + 0; A_Index251++) {
    ...
    if (size_weAreInMLSarr != 0) {
        for (let A_Index252 = 0; A_Index252 < size_weAreInMLSarr + 0; A_Index252++) {
...
function HTVMv2() {
    ...
    if (HTVM_getLang_HTVM() == "cpp" || HTVM_getLang_HTVM() == "py") {
        ...
        items254 = LoopParseFunc(allArgs, "\n", "\r")
        ...
        for (let A_Index255 = 0; A_Index255 < HTVM_Size(argHTVMinstrMORE) + 0; A_Index255++) {
