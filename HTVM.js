...
function fixJAVAstrCmp_FjavaUGH(javaCode) {
    ...
    var next = 0;
    let items154 = LoopParseFunc(javaCode, " ");
    ...
    var replace4 = "";
    var varName = "";
    for (let A_Index155 = 0; A_Index155 < HTVM_Size(allJavaStringsIG) + 0; A_Index155++) {
...
function fString(line) {
    ...
    if (langToConvertTo != langFileExtension_2) {
        ...
        let items157 = LoopParseFunc(line);
        ...
        let items158 = LoopParseFunc(line);
    ...
    if (langToConvertTo == "rb") {
        for (let A_Index159 = 0; A_Index159 < HTVM_Size(htCodeFixRubyGlobalVars_EXPOSED) + 0; A_Index159++) {
...
function handleMultiLineStrings(code) {
    ...
    let items160 = LoopParseFunc(code, "\n", "\r");
    ...
    let lookAhead = [];
    let items161 = LoopParseFunc(code, "\n", "\r");
    ...
    let items162 = LoopParseFunc(code, "\n", "\r");
...
function slidingWinFixCommentsRmFUNC(line) {
    ...
    let items163 = LoopParseFunc(line);
    ...
    for (let A_Index164 = 0; A_Index164 < StrLen(keyWordComment + keyWordComment) + 2 + 0; A_Index164++) {
...
function fixIfElseIfMultiLineStatementHELP(aloopfield) {
    var out = "";
    var count = 0;
    let items165 = LoopParseFunc(aloopfield);
...
function fixIfElseIfMultiLineStatement(someHTVMcode) {
    var doWeExitInsta = 1;
    let items166 = LoopParseFunc(someHTVMcode, "\n", "\r");
    ...
    for (let A_Index167 = 0; A_Index167 < 10000 + 0; A_Index167++) {
        ...
        out = "";
        doWeExit = false;
        let items168 = LoopParseFunc(out2, "\n", "\r");
...
function doseHaveInclude(TheCodeThatMightHaveInclude) {
    let items169 = LoopParseFunc(TheCodeThatMightHaveInclude, "\n", "\r");
...
function compiler(htCode, allInstructionFile, mode, langToConvertToParam = "") {
    ...
    let items170 = LoopParseFunc(allInstructionFile, "\n", "\r");
    ...
    if (HTVM_Size(argHTVMinstrMORE) > 0) {
        ...
        let items171 = LoopParseFunc(allInstructionFile_2, "\n", "\r");
    ...
    */
    ...
    let items172 = LoopParseFunc(allInstructionFile, "\n", "\r");
    ...
    if (isNotHTVMfile == 0) {
        if (langToConvertTo != langFileExtension_2) {
            ...
            if (doseHaveInclude(code)) {
                ...
                for (let A_Index174 = 0; A_Index174 < 10000 + 0; A_Index174++) {
                    ...
                    for (let A_Index175 = 0; A_Index175 < items175.length + 0; A_Index175++) {
                        ...
                        if (SubStr(StrLower(Trim(currentLine)), 1, StrLen(keyWordInclude + " ")) == StrLower(keyWordInclude + " ")) {
                            ...
                            for (let A_Index176 = 0; A_Index176 < HTVM_Size(includedFilePaths) + 0; A_Index176++) {
    ...
    if (isNotHTVMfile == 0) {
        //;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;
        //;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;
        let items178 = LoopParseFunc(code, "\n", "\r");
        for (let A_Index178 = 0; A_Index178 < items178.length + 0; A_Index178++) {
            ...
            if (SubStr(Trim(A_LoopField178), 1, StrLen(keyWordComment)) == keyWordComment) {
                ...
                let items179 = LoopParseFunc(A_LoopField178);
        ...
        //print("============programmingBlock_CPP==================")
        //print("============programmingBlock_CPP==================")
        let items180 = LoopParseFunc(code);
        ...
        let items181 = LoopParseFunc(code);
        ...
        for (let A_Index185 = 0; A_Index185 < theIdNumOfThe34 + 0; A_Index185++) {
        ...
        if (langToConvertTo == "dart" || langToConvertTo == "groovy") {
            for (let A_Index186 = 0; A_Index186 < HTVM_Size(theIdNumOfThe34theVar) + 0; A_Index186++) {
        ...
        for (let A_Index187 = 0; A_Index187 < items187.length + 0; A_Index187++) {
            ...
            if (InStr(A_LoopField187, " " + keyWordComment) != false && SubStr(Trim(A_LoopField187), 1, StrLen(keyWordComment)) != keyWordComment) {
                ...
                for (let A_Index188 = 0; A_Index188 < HTVM_Size(slidingWinFixCommentsRm) + 0; A_Index188++) {
                    slidingWinFixCommentsRmI = A_Index188;
                    slidingWinFixCommentsRmHold = "";
                    for (let A_Index189 = 0; A_Index189 < StrLen(" " + keyWordComment) + 0; A_Index189++) {
        ...
        if (useCurlyBraces == "on") {
            ...
            if (keyWordCurlyBraceOpen != "{") {
                ...
                for (let A_Index193 = 0; A_Index193 < items193.length + 0; A_Index193++) {
                    ...
                    htCodeOutFixINT--;
                    let items195 = LoopParseFunc(A_LoopField193, " ");
            ...
            if (keyWordCurlyBraceClose != "}") {
                ...
                for (let A_Index196 = 0; A_Index196 < items196.length + 0; A_Index196++) {
                    ...
                    let items197 = LoopParseFunc(A_LoopField196, " ");
        ...
        for (let A_Index203 = 0; A_Index203 < 2 + 0; A_Index203++) {
            outCodeFixBraces = "";
        ...
        for (let A_Index208 = 0; A_Index208 < items208.length + 0; A_Index208++) {
            ...
            if (SubStr(A_LoopField208, 1, StrLen(keyWordComment)) == keyWordComment) {
                ...
                int1 = 0;
                str17 = "";
                let items209 = LoopParseFunc(str16, " ");
                ...
                if (langToConvertTo != langFileExtension_2 && langToConvertTo != "cpp" && langToConvertTo != "go" && langToConvertTo != "cs" && langToConvertTo != "java" && langToConvertTo != "kt" && langToConvertTo != "nim" && langToConvertTo != "swift" && langToConvertTo != "dart" && langToConvertTo != "ts") {
                    if (str1 != "") {
                        str4 = "";
                        let items210 = LoopParseFunc(str1, ",");
                ...
                if (Trim(str1) == "") {
                    ...
                    str19 = "";
                    str20 = "";
                    let items211 = LoopParseFunc(str1, ",");
                    ...
                    if (Trim(str5) != "" && Trim(str10) != "") {
                        let items212 = LoopParseFunc(str5, "\n", "\r");
                        ...
                        let items213 = LoopParseFunc(str10, "\n", "\r");
                        ...
                        if (langToConvertTo == "lua") {
                            let items214 = LoopParseFunc(str12, ",");
                            ...
                            str13 += "{" + Chr(10);
                            let items215 = LoopParseFunc(str12, ",");
                        ...
                        if (langToConvertTo == "go") {
                            ...
                            str13 += "{" + Chr(10) + " ";
                            let items216 = LoopParseFunc(str12, ",");
                            ...
                            let items217 = LoopParseFunc(str12, ",");
                        ...
                        if (langToConvertTo == "java") {
                            ...
                            str13 += "{" + Chr(10);
                            let items218 = LoopParseFunc(str12, ",");
                            for (let A_Index218 = 0; A_Index218 < items218.length + 0; A_Index218++) {
                                ...
                                str20 = "";
                                let items219 = LoopParseFunc(str19, " ");
                            ...
                            let items220 = LoopParseFunc(str12, ",");
                    ...
                    if (Trim(str5) != "" && Trim(str10) == "") {
                        let items221 = LoopParseFunc(str5, "\n", "\r");
                    ...
                    if (Trim(str5) == "" && Trim(str10) != "") {
                        let items222 = LoopParseFunc(str10, "\n", "\r");
                        ...
                        if (langToConvertTo == "lua") {
                            let items223 = LoopParseFunc(str11, ",");
                            ...
                            str14 = StringTrimRight(str14, 2);
                            str13 += "{" + Chr(10);
                            let items224 = LoopParseFunc(str11, ",");
                        ...
                        if (langToConvertTo == "go") {
                            str14 = "__HTVM_V2_TO_GO_optionalParams__ ...interface{}";
                            str13 += "{" + Chr(10) + " ";
                            let items225 = LoopParseFunc(str11, ",");
                            ...
                            let items226 = LoopParseFunc(str11, ",");
                        ...
                        if (langToConvertTo == "java") {
                            str14 = "Object... __HTVM_V2_TO_JAVA_optionalParams__";
                            str13 += "{" + Chr(10);
                            let items227 = LoopParseFunc(str11, ",");
                            for (let A_Index227 = 0; A_Index227 < items227.length + 0; A_Index227++) {
                                ...
                                str20 = "";
                                let items228 = LoopParseFunc(str19, " ");
                            ...
                            let items229 = LoopParseFunc(str11, ",");
                ...
                if (InStr(ALoopField, " ") == false || SubStrLastChars(ALoopField, 1) == "]") {
                    ...
                    if (usePostfixTypeForTypeDefinition == "off") {
                        lineDone = 1;
                        str1 = Trim(StringTrimLeft(Trim(str1), StrLen(keyWordConst)));
                        let items230 = LoopParseFunc(str1, " ");
                    ...
                    if (usePostfixTypeForTypeDefinition == "off") {
                        str1 = Trim(StringTrimLeft(Trim(str1), StrLen(keyWordVar)));
                        let items231 = LoopParseFunc(str1, " ");
                ...
                if (usePostfixTypeForTypeDefinition == "off") {
                    ...
                    let items234 = LoopParseFunc(str1, " ");
                ...
                if (usePostfixTypeForTypeDefinition == "off") {
                    ...
                    let items235 = LoopParseFunc(str1, " ");
        ...
        if (langToConvertTo != "ahk" && langToConvertTo != langFileExtension_2) {
            ...
            if (haveWeEverUsedAloop == 1) {
                ...
                let items237 = LoopParseFunc(htCodeLoopfixa, "\n", "\r");
    ...
    if (COUNT_programmingBlock_HTVMsyntax != 0) {
        ...
        for (let A_Index242 = 0; A_Index242 < COUNT_programmingBlock_HTVMsyntax + 0; A_Index242++) {
        ...
        isNotHTVMfile2 = 1;
        for (let A_Index243 = 0; A_Index243 < COUNT_programmingBlock_HTVMsyntax + 0; A_Index243++) {
    ...
    if (langToConvertTo == "rb") {
        ...
        HTVM_Append(nextWordEndFix, " ");
        let items252 = LoopParseFunc(htCode, "\n", "\r");
    ...
    nextWordEndFixOut = "";
    if (langToConvertTo == "ahk") {
        let items257 = LoopParseFunc(htCode, "\n", "\r");
        for (let A_Index257 = 0; A_Index257 < items257.length + 0; A_Index257++) {
    ...
    if (langToConvertTo == "go") {
        ...
        let items258 = LoopParseFunc(htCode, "\n", "\r");
        for (let A_Index258 = 0; A_Index258 < items258.length + 0; A_Index258++) {
        ...
        for (let A_Index259 = 0; A_Index259 < items259.length + 0; A_Index259++) {
            ...
            if (Trim(A_LoopField259) == "}" && SubStr(Trim(fixGoManGoIsSoAnnoyingBroooFurure[A_Index259 + 1]), 1, StrLen("else if ")) == "else if ") {
                ...
                let items260 = LoopParseFunc(fixGoManGoIsSoAnnoyingBroooFurure[A_Index259 + 1]);
                ...
                for (let A_Index261 = 0; A_Index261 < fixGoManGoIsSoAnnoyingBroooCount + 0; A_Index261++) {
                    fixGoManGoIsSoAnnoyingBroooCountSpaceses += " ";
                }
    ...
    for (let A_Index262 = 0; A_Index262 < HTVM_Size(commentsBugFix) + 0; A_Index262++) {
    ...
    if (COUNT_programmingBlock_InTheTranspiledLang != 0) {
        for (let A_Index263 = 0; A_Index263 < COUNT_programmingBlock_InTheTranspiledLang + 0; A_Index263++) {
    ...
    if (langToConvertTo == "cpp") {
        if (COUNT_programmingBlock_CPP != 0) {
            for (let A_Index264 = 0; A_Index264 < COUNT_programmingBlock_CPP + 0; A_Index264++) {
        ...
        if (COUNT_programmingBlock_CPP != 0) {
            for (let A_Index265 = 0; A_Index265 < COUNT_programmingBlock_CPP + 0; A_Index265++) {
                if (langToConvertTo == langFileExtension_2) {
    ...
    if (langToConvertTo == "py") {
        if (COUNT_programmingBlock_PY != 0) {
            for (let A_Index266 = 0; A_Index266 < COUNT_programmingBlock_PY + 0; A_Index266++) {
                htCode = StrReplace(htCode, "programmingBlock_PY-programmingBlock_PY-AA" + STR(A_Index266 + 1) + "AA", programmingBlock_PY[A_Index266]);
            }
        }
    } else {
        if (COUNT_programmingBlock_PY != 0) {
            for (let A_Index267 = 0; A_Index267 < COUNT_programmingBlock_PY + 0; A_Index267++) {
                if (langToConvertTo == langFileExtension_2) {
    ...
    if (langToConvertTo == "js") {
        if (COUNT_programmingBlock_JS != 0) {
            for (let A_Index268 = 0; A_Index268 < COUNT_programmingBlock_JS + 0; A_Index268++) {
                htCode = StrReplace(htCode, "programmingBlock_JS-programmingBlock_JS-AA" + STR(A_Index268 + 1) + "AA", programmingBlock_JS[A_Index268]);
            }
        }
    } else {
        if (COUNT_programmingBlock_JS != 0) {
            for (let A_Index269 = 0; A_Index269 < COUNT_programmingBlock_JS + 0; A_Index269++) {
                if (langToConvertTo == langFileExtension_2) {
    ...
    if (langToConvertTo == "go") {
        if (COUNT_programmingBlock_GO != 0) {
            for (let A_Index270 = 0; A_Index270 < COUNT_programmingBlock_GO + 0; A_Index270++) {
                htCode = StrReplace(htCode, "programmingBlock_GO-programmingBlock_GO-AA" + STR(A_Index270 + 1) + "AA", programmingBlock_GO[A_Index270]);
            }
        }
    } else {
        if (COUNT_programmingBlock_GO != 0) {
            for (let A_Index271 = 0; A_Index271 < COUNT_programmingBlock_GO + 0; A_Index271++) {
                if (langToConvertTo == langFileExtension_2) {
    ...
    if (langToConvertTo == "lua") {
        if (COUNT_programmingBlock_LUA != 0) {
            for (let A_Index272 = 0; A_Index272 < COUNT_programmingBlock_LUA + 0; A_Index272++) {
        ...
        if (COUNT_programmingBlock_LUA != 0) {
            for (let A_Index273 = 0; A_Index273 < COUNT_programmingBlock_LUA + 0; A_Index273++) {
                if (langToConvertTo == langFileExtension_2) {
    ...
    if (langToConvertTo == "cs") {
        if (COUNT_programmingBlock_CS != 0) {
            for (let A_Index274 = 0; A_Index274 < COUNT_programmingBlock_CS + 0; A_Index274++) {
                htCode = StrReplace(htCode, "programmingBlock_CS-programmingBlock_CS-AA" + STR(A_Index274 + 1) + "AA", programmingBlock_CS[A_Index274]);
            }
        }
    } else {
        if (COUNT_programmingBlock_CS != 0) {
            for (let A_Index275 = 0; A_Index275 < COUNT_programmingBlock_CS + 0; A_Index275++) {
                if (langToConvertTo == langFileExtension_2) {
    ...
    if (langToConvertTo == "java") {
        if (COUNT_programmingBlock_JAVA != 0) {
            for (let A_Index276 = 0; A_Index276 < COUNT_programmingBlock_JAVA + 0; A_Index276++) {
        ...
        if (COUNT_programmingBlock_JAVA != 0) {
            for (let A_Index277 = 0; A_Index277 < COUNT_programmingBlock_JAVA + 0; A_Index277++) {
                if (langToConvertTo == langFileExtension_2) {
    ...
    if (langToConvertTo == "kt") {
        if (COUNT_programmingBlock_KT != 0) {
            for (let A_Index278 = 0; A_Index278 < COUNT_programmingBlock_KT + 0; A_Index278++) {
                htCode = StrReplace(htCode, "programmingBlock_KT-programmingBlock_KT-AA" + STR(A_Index278 + 1) + "AA", programmingBlock_KT[A_Index278]);
            }
        }
    } else {
        if (COUNT_programmingBlock_KT != 0) {
            for (let A_Index279 = 0; A_Index279 < COUNT_programmingBlock_KT + 0; A_Index279++) {
                if (langToConvertTo == langFileExtension_2) {
    ...
    if (langToConvertTo == "rb") {
        if (COUNT_programmingBlock_RB != 0) {
            for (let A_Index280 = 0; A_Index280 < COUNT_programmingBlock_RB + 0; A_Index280++) {
                htCode = StrReplace(htCode, "programmingBlock_RB-programmingBlock_RB-AA" + STR(A_Index280 + 1) + "AA", programmingBlock_RB[A_Index280]);
            }
        }
    } else {
        if (COUNT_programmingBlock_RB != 0) {
            for (let A_Index281 = 0; A_Index281 < COUNT_programmingBlock_RB + 0; A_Index281++) {
                if (langToConvertTo == langFileExtension_2) {
    ...
    if (langToConvertTo == "nim") {
        if (COUNT_programmingBlock_NIM != 0) {
            for (let A_Index282 = 0; A_Index282 < COUNT_programmingBlock_NIM + 0; A_Index282++) {
        ...
        if (COUNT_programmingBlock_NIM != 0) {
            for (let A_Index283 = 0; A_Index283 < COUNT_programmingBlock_NIM + 0; A_Index283++) {
                if (langToConvertTo == langFileExtension_2) {
    ...
    if (langToConvertTo == "ahk") {
        if (COUNT_programmingBlock_AHK != 0) {
            for (let A_Index284 = 0; A_Index284 < COUNT_programmingBlock_AHK + 0; A_Index284++) {
        ...
        if (COUNT_programmingBlock_AHK != 0) {
            for (let A_Index285 = 0; A_Index285 < COUNT_programmingBlock_AHK + 0; A_Index285++) {
                if (langToConvertTo == langFileExtension_2) {
    ...
    if (langToConvertTo == "swift") {
        if (COUNT_programmingBlock_SWIFT != 0) {
            for (let A_Index286 = 0; A_Index286 < COUNT_programmingBlock_SWIFT + 0; A_Index286++) {
        ...
        if (COUNT_programmingBlock_SWIFT != 0) {
            for (let A_Index287 = 0; A_Index287 < COUNT_programmingBlock_SWIFT + 0; A_Index287++) {
                if (langToConvertTo == langFileExtension_2) {
    ...
    if (langToConvertTo == "dart") {
        if (COUNT_programmingBlock_DART != 0) {
            for (let A_Index288 = 0; A_Index288 < COUNT_programmingBlock_DART + 0; A_Index288++) {
        ...
        if (COUNT_programmingBlock_DART != 0) {
            for (let A_Index289 = 0; A_Index289 < COUNT_programmingBlock_DART + 0; A_Index289++) {
                if (langToConvertTo == langFileExtension_2) {
    ...
    if (langToConvertTo == "ts") {
        if (COUNT_programmingBlock_TS != 0) {
            for (let A_Index290 = 0; A_Index290 < COUNT_programmingBlock_TS + 0; A_Index290++) {
                htCode = StrReplace(htCode, "programmingBlock_TS-programmingBlock_TS-AA" + STR(A_Index290 + 1) + "AA", programmingBlock_TS[A_Index290]);
            }
        }
    } else {
        if (COUNT_programmingBlock_TS != 0) {
            for (let A_Index291 = 0; A_Index291 < COUNT_programmingBlock_TS + 0; A_Index291++) {
                if (langToConvertTo == langFileExtension_2) {
    ...
    if (langToConvertTo == "groovy") {
        if (COUNT_programmingBlock_GROOVY != 0) {
            for (let A_Index292 = 0; A_Index292 < COUNT_programmingBlock_GROOVY + 0; A_Index292++) {
        ...
        if (COUNT_programmingBlock_GROOVY != 0) {
            for (let A_Index293 = 0; A_Index293 < COUNT_programmingBlock_GROOVY + 0; A_Index293++) {
                if (langToConvertTo == langFileExtension_2) {
    ...
    if (langToConvertTo == langFileExtension_2) {
        if (COUNT_programmingBlock_HTVM != 0) {
            for (let A_Index294 = 0; A_Index294 < COUNT_programmingBlock_HTVM + 0; A_Index294++) {
        ...
        if (COUNT_programmingBlock_HTVM != 0) {
            for (let A_Index295 = 0; A_Index295 < COUNT_programmingBlock_HTVM + 0; A_Index295++) {
    ...
    if (langToConvertTo != langFileExtension_2) {
        ...
        for (let A_Index296 = 0; A_Index296 < items296.length + 0; A_Index296++) {
            ...
            if (InStr(A_LoopField296, "fihuiuuhuuhtheidFor--asds")) {
                ...
                for (let A_Index297 = 0; A_Index297 < theIdNumOfThe34 + 0; A_Index297++) {
    ...
    var correctLang = 0;
    let items298 = LoopParseFunc(instructions, "\n", "\r");
    for (let A_Index298 = 0; A_Index298 < items298.length + 0; A_Index298++) {
    ...
    if (!(HTVM_Size(allFuncNames) <= 0)) {
        for (let A_Index299 = 0; A_Index299 < HTVM_Size(allFuncNames) + 0; A_Index299++) {
        ...
        let items300 = LoopParseFunc(allLibsToPutAtTop, "|");
    ...
    if (langToConvertTo != langFileExtension_2) {
        for (let A_Index301 = 0; A_Index301 < HTVM_Size(ospDic1) + 0; A_Index301++) {
    ...
    if (size_ospDic != 0) {
        for (let A_Index302 = 0; A_Index302 < size_ospDic + 0; A_Index302++) {
            HTVM_Pop(ospDic);
        }
    }
    //;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;
    ...
    if (size_ospDic1 != 0) {
        for (let A_Index303 = 0; A_Index303 < size_ospDic1 + 0; A_Index303++) {
            HTVM_Pop(ospDic1);
        }
    }
    //;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;
    ...
    if (size_ospDic2 != 0) {
        for (let A_Index304 = 0; A_Index304 < size_ospDic2 + 0; A_Index304++) {
            HTVM_Pop(ospDic2);
        }
    }
    //;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;
    ...
    if (size_htCodeFixRubyGlobalVars_EXPOSED != 0) {
        for (let A_Index305 = 0; A_Index305 < size_htCodeFixRubyGlobalVars_EXPOSED + 0; A_Index305++) {
    ...
    for (let A_Index306 = 0; A_Index306 < theIdNumOfThe34 + 0; A_Index306++) {
    ...
    if (isNotHTVMfile2 == 0) {
        ...
        if (langToConvertTo == langFileExtension_2) {
            for (let A_Index307 = 0; A_Index307 < HTVM_Size(weAreInMLSarr) + 0; A_Index307++) {
    ...
    if (size_weAreInMLSarr != 0) {
        for (let A_Index308 = 0; A_Index308 < size_weAreInMLSarr + 0; A_Index308++) {
...
function HTVMv2() {
    ...
    let items309 = LoopParseFunc(str0);
    for (let A_Index309 = 0; A_Index309 < items309.length + 0; A_Index309++) {
        const A_LoopField309 = items309[A_Index309 - 0];
    ...
    if (HTVM_getLang_HTVM() == "cpp" || HTVM_getLang_HTVM() == "py") {
        ...
        let items310 = LoopParseFunc(allArgs, "\n", "\r");
        ...
        for (let A_Index311 = 0; A_Index311 < HTVM_Size(argHTVMinstrMORE) + 0; A_Index311++) {
