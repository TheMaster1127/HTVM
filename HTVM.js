...
//;;;;;;;;;;;;;;;;;;;;;;;
//;;;;;;;;;;;;;;;;;;;;;;;
function extraFlexableFuncCallsHelp2(line) {
    let allFuncsDot = [];
    var temp = "";
...
function splitFunctionCalls2(line) {
...
function extraFlexableFuncCallsHelp(line) {
    ...
    var out = "";
    var endParentesesToPut = 0;
    for (let A_Index85 = 0; A_Index85 < HTVM_Size(allFuncsDot) + 0; A_Index85++) {
        if (HTVM_Size(allFuncsDot) == A_Index85 + 1) {
            ...
            for (let A_Index86 = 0; A_Index86 < endParentesesToPut + 1 + 0; A_Index86++) {
                out += ")";
            }
        } else {
            endParentesesToPut++;
...
function splitFunctionCalls(line) {
    ...
    var arrToStrSTR = "";
    for (let A_Index92 = 0; A_Index92 < HTVM_Size(funcCalls) + 0; A_Index92++) {
    ...
    arrToStrSTR = StringTrimRight(arrToStrSTR, 1);
    if (doWeAddParentisiesLater != 0) {
        for (let A_Index93 = 0; A_Index93 < doWeAddParentisiesLater + 0; A_Index93++) {
            arrToStrSTR = "(" + arrToStrSTR + ")";
        }
    }
    if (doseBracketsExist == 1) {
        for (let A_Index94 = 0; A_Index94 < HTVM_Size(saveDoseBracketsExist) + 0; A_Index94++) {
    ...
    if (doseFuncAtTheBegggging == 1) {
        for (let A_Index95 = 0; A_Index95 < HTVM_Size(FuncAtTheBegggging) + 0; A_Index95++) {
            arrToStrSTR = FuncAtTheBegggging[A_Index95] + arrToStrSTR;
        }
        for (let A_Index96 = 0; A_Index96 < doWeAddParentisiesLater2 + 0; A_Index96++) {
            arrToStrSTR = arrToStrSTR + ")";
        }
    }
    return arrToStrSTR;
...
function expressionParserTranspiler(expression) {
    ...
    // osp
    if (langToConvertTo != langFileExtension_2) {
        for (let A_Index97 = 0; A_Index97 < HTVM_Size(ospDic1) + 0; A_Index97++) {
            str00 = ospDic1[A_Index97];
            expression = StrReplace(expression, StrReplace(str00, "_", ".") + ".", str00 + "_");
        }
        for (let A_Index98 = 0; A_Index98 < HTVM_Size(ospDic2) + 0; A_Index98++) {
    ...
    for (let A_Index99 = 0; A_Index99 < 29 + 0; A_Index99++) {
...
function FixRubyGlobalVars(line) {
    ...
    for (let A_Index101 = 0; A_Index101 < HTVM_Size(htCodeFixRubyGlobalVars) + 0; A_Index101++) {
...
function slidingWinFixCommentsRmFUNC(line) {
    ...
    for (let A_Index111 = 0; A_Index111 < StrLen(keyWordComment + keyWordComment) + 2 + 0; A_Index111++) {
...
function compiler(htCode, allInstructionFile, mode, langToConvertToParam = "") {
    ...
    items112 = LoopParseFunc(allInstructionFile, "\n", "\r")
    ...
    if (HTVM_Size(argHTVMinstrMORE) > 0) {
        ...
        items113 = LoopParseFunc(allInstructionFile_2, "\n", "\r")
    ...
    */
        ...
        if (langToConvertTo != langFileExtension_2) {
            if (InStr(StrLower(code), StrLower(keyWordInclude + " "))) {
                for (let A_Index115 = 0; A_Index115 < 10000 + 0; A_Index115++) {
                    ...
                    if (A_Index115 == 0) {
                        ...
                        for (let A_Index117 = 0; A_Index117 < items117.length + 0; A_Index117++) {
                            ...
                            if (SubStr(StrLower(Trim(A_LoopField117)), 1, StrLen(StrLower(keyWordInclude + " "))) == StrLower(keyWordInclude + " ")) {
                                ...
                                for (let A_Index118 = 0; A_Index118 < HTVM_Size(allLibsWeSee) + 0; A_Index118++) {
                    ...
                    for (let A_Index119 = 0; A_Index119 < HTVM_Size(allLibsWeSee) + 0; A_Index119++) {
                        allLibsWeSeeHOLD = allLibsWeSee[A_Index119];
                        doseCodeHaveLibs_HAS_SEEN2 = 0;
                        for (let A_Index120 = 0; A_Index120 < HTVM_Size(allLibsWeSeeSEE) - HTVM_Size(allLibsWeSee) + 0; A_Index120++) {
    ...
    if (isNotHTVMfile == 0) {
        ...
        for (let A_Index127 = 0; A_Index127 < theIdNumOfThe34 + 0; A_Index127++) {
        ...
        for (let A_Index128 = 0; A_Index128 < items128.length + 0; A_Index128++) {
            ...
            if (InStr(A_LoopField128, " " + keyWordComment) != false && SubStr(Trim(A_LoopField128), 1, StrLen(keyWordComment)) != keyWordComment) {
                ...
                for (let A_Index129 = 0; A_Index129 < HTVM_Size(slidingWinFixCommentsRm) + 0; A_Index129++) {
                    slidingWinFixCommentsRmI = A_Index129;
                    slidingWinFixCommentsRmHold = "";
                    for (let A_Index130 = 0; A_Index130 < StrLen(" " + keyWordComment) + 0; A_Index130++) {
        ...
        for (let A_Index139 = 0; A_Index139 < 2 + 0; A_Index139++) {
            outCodeFixBraces = "";
        ...
        if (langToConvertTo != "ahk" && langToConvertTo != langFileExtension_2) {
            ...
            if (haveWeEverUsedAloop == 1) {
                ...
                items173 = LoopParseFunc(htCodeLoopfixa, "\n", "\r")
    ...
    if (COUNT_programmingBlock_HTVMsyntax != 0) {
        ...
        for (let A_Index178 = 0; A_Index178 < COUNT_programmingBlock_HTVMsyntax + 0; A_Index178++) {
    ...
    if (langToConvertTo == "go") {
        ...
        for (let A_Index195 = 0; A_Index195 < items195.length + 0; A_Index195++) {
            ...
            if (Trim(A_LoopField195) == "}" && SubStr(Trim(fixGoManGoIsSoAnnoyingBroooFurure[A_Index195 + 1]), 1, StrLen("else if ")) == "else if ") {
                ...
                items196 = LoopParseFunc(fixGoManGoIsSoAnnoyingBroooFurure[A_Index195 + 1])
                ...
                for (let A_Index197 = 0; A_Index197 < fixGoManGoIsSoAnnoyingBroooCount + 0; A_Index197++) {
                    fixGoManGoIsSoAnnoyingBroooCountSpaceses += " ";
                }
    ...
    if (COUNT_programmingBlock_InTheTranspiledLang != 0) {
        for (let A_Index198 = 0; A_Index198 < COUNT_programmingBlock_InTheTranspiledLang + 0; A_Index198++) {
    ...
    if (langToConvertTo == "cpp") {
        if (COUNT_programmingBlock_CPP != 0) {
            for (let A_Index199 = 0; A_Index199 < COUNT_programmingBlock_CPP + 0; A_Index199++) {
    ...
    if (langToConvertTo == "py") {
        if (COUNT_programmingBlock_PY != 0) {
            for (let A_Index201 = 0; A_Index201 < COUNT_programmingBlock_PY + 0; A_Index201++) {
    ...
    if (langToConvertTo == "js") {
        if (COUNT_programmingBlock_JS != 0) {
            for (let A_Index203 = 0; A_Index203 < COUNT_programmingBlock_JS + 0; A_Index203++) {
    ...
    if (langToConvertTo == "go") {
        if (COUNT_programmingBlock_GO != 0) {
            for (let A_Index205 = 0; A_Index205 < COUNT_programmingBlock_GO + 0; A_Index205++) {
    ...
    if (langToConvertTo == "lua") {
        if (COUNT_programmingBlock_LUA != 0) {
            for (let A_Index207 = 0; A_Index207 < COUNT_programmingBlock_LUA + 0; A_Index207++) {
    ...
    if (langToConvertTo == "cs") {
        if (COUNT_programmingBlock_CS != 0) {
            for (let A_Index209 = 0; A_Index209 < COUNT_programmingBlock_CS + 0; A_Index209++) {
    ...
    if (langToConvertTo == "java") {
        if (COUNT_programmingBlock_JAVA != 0) {
            for (let A_Index211 = 0; A_Index211 < COUNT_programmingBlock_JAVA + 0; A_Index211++) {
    ...
    if (langToConvertTo == "kt") {
        if (COUNT_programmingBlock_KT != 0) {
            for (let A_Index213 = 0; A_Index213 < COUNT_programmingBlock_KT + 0; A_Index213++) {
    ...
    if (langToConvertTo == "rb") {
        if (COUNT_programmingBlock_RB != 0) {
            for (let A_Index215 = 0; A_Index215 < COUNT_programmingBlock_RB + 0; A_Index215++) {
    ...
    if (langToConvertTo == "nim") {
        if (COUNT_programmingBlock_NIM != 0) {
            for (let A_Index217 = 0; A_Index217 < COUNT_programmingBlock_NIM + 0; A_Index217++) {
    ...
    if (langToConvertTo == "ahk") {
        if (COUNT_programmingBlock_AHK != 0) {
            for (let A_Index219 = 0; A_Index219 < COUNT_programmingBlock_AHK + 0; A_Index219++) {
    ...
    if (langToConvertTo == "swift") {
        if (COUNT_programmingBlock_SWIFT != 0) {
            for (let A_Index221 = 0; A_Index221 < COUNT_programmingBlock_SWIFT + 0; A_Index221++) {
    ...
    if (langToConvertTo == "dart") {
        if (COUNT_programmingBlock_DART != 0) {
            for (let A_Index223 = 0; A_Index223 < COUNT_programmingBlock_DART + 0; A_Index223++) {
    ...
    if (langToConvertTo == "ts") {
        if (COUNT_programmingBlock_TS != 0) {
            for (let A_Index225 = 0; A_Index225 < COUNT_programmingBlock_TS + 0; A_Index225++) {
    ...
    if (langToConvertTo == "groovy") {
        if (COUNT_programmingBlock_GROOVY != 0) {
            for (let A_Index227 = 0; A_Index227 < COUNT_programmingBlock_GROOVY + 0; A_Index227++) {
    ...
    if (langToConvertTo == langFileExtension_2) {
        if (COUNT_programmingBlock_HTVM != 0) {
            for (let A_Index229 = 0; A_Index229 < COUNT_programmingBlock_HTVM + 0; A_Index229++) {
