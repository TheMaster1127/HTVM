...
std::string handleMultiLineStrings(std::string code) {
    ...
    for (size_t A_Index157 = 0; A_Index157 < items157.size() + 0; A_Index157++) {
        ...
        if (SubStrLastChars(Trim(A_LoopField157), 4) == " mls" || SubStrLastChars(Trim(A_LoopField157), 5) == " fmls") {
            didWeFindOrWeOut = 1;
        }
    }
    ...
    for (size_t A_Index158 = 0; A_Index158 < items158.size() + 0; A_Index158++) {
        ...
        HTVM_Append(lookAhead, Trim(A_LoopField158));
    }
    ...
    for (size_t A_Index159 = 0; A_Index159 < items159.size() + 0; A_Index159++) {
        ...
        if (langToConvertTo == langFileExtension_2) {
            //;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;
            //;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;
            if (weAreIn == 1 && Trim(A_LoopField159) == "mle") {
            ...
            if (weAreIn == 1) {
                ...
                weAreInMLSarrTemp += A_LoopField159;
            }
            if (SubStrLastChars(Trim(A_LoopField159), 4) == " mls" || SubStrLastChars(Trim(A_LoopField159), 5) == " fmls") {
                weAreIn = 1;
                out += A_LoopField159 + Chr(10);
            }
            if (weAreIn == 0) {
                out += A_LoopField159 + Chr(10);
            }
            ...
            if (weAreIn == 1 && Trim(A_LoopField159) == "HTVM-newLineAHw4y9w730q8237w4e8srhgesw0s8rgw04e5g08sgr0e8srdgt--YOU-BETERNOTUSETHISAH--aesdfssz") {
                if (lookAhead[A_Index159 + 1] == "HTVM-newLineAHw4y9w730q8237w4e8srhgesw0s8rgw04e5g08sgr0e8srdgt--YOU-BETERNOTUSETHISAH--aesdfssz") {
                    out += keyWordEscpaeChar + "n";
                }
            }
            if (weAreIn == 1 && Trim(A_LoopField159) != "HTVM-newLineAHw4y9w730q8237w4e8srhgesw0s8rgw04e5g08sgr0e8srdgt--YOU-BETERNOTUSETHISAH--aesdfssz") {
                out += A_LoopField159 + keyWordEscpaeChar + "n";
            }
            if (SubStrLastChars(Trim(A_LoopField159), 4) == " mls" || SubStrLastChars(Trim(A_LoopField159), 5) == " fmls") {
                weAreIn = 1;
                if (SubStrLastChars(Trim(A_LoopField159), 5) == " fmls") {
                    out += TrimR(StringTrimRight(TrimR(A_LoopField159), 3)) + Chr(34);
                } else {
                    out += TrimR(StringTrimRight(TrimR(A_LoopField159), 3)) + " " + Chr(34);
                }
            }
            if (weAreIn == 0) {
                if (Trim(A_LoopField159) != "mle") {
                    out += A_LoopField159 + Chr(10);
                }
            }
            //;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;
...
std::vector<std::string> slidingWinFixCommentsRmFUNC(std::string line) {
    ...
    for (size_t A_Index160 = 0; A_Index160 < items160.size() + 0; A_Index160++) {
        ...
        HTVM_Append(slidingWinFixCommentsRmFFFF, A_LoopField160);
    }
    for (int A_Index161 = 0; A_Index161 < StrLen(keyWordComment + keyWordComment) + 2 + 0; A_Index161++) {
...
std::string fixIfElseIfMultiLineStatementHELP(std::string aloopfield) {
    std::string out = "";
    int count = 0;
    std::vector<std::string> items162 = LoopParseFunc(aloopfield);
...
std::string fixIfElseIfMultiLineStatement(std::string someHTVMcode) {
    int doWeExitInsta = 1;
    std::vector<std::string> items163 = LoopParseFunc(someHTVMcode, "\n", "\r");
    for (size_t A_Index163 = 0; A_Index163 < items163.size() + 0; A_Index163++) {
        std::string A_LoopField163 = items163[A_Index163 - 0];
        if (SubStr(StrLower(Trim(A_LoopField163)), 1, StrLen(keyWordIF)) == keyWordIF) {
            doWeExitInsta = 0;
        }
        else if (SubStr(StrLower(Trim(A_LoopField163)), 1, StrLen(keyWordWhileLoop)) == keyWordWhileLoop) {
            doWeExitInsta = 0;
        }
        else if (SubStr(StrLower(Trim(A_LoopField163)), 1, StrLen(keyWordElseIf)) == keyWordElseIf) {
            doWeExitInsta = 0;
        }
    }
    if (doWeExitInsta == 1) {
    ...
    for (int A_Index164 = 0; A_Index164 < 10000 + 0; A_Index164++) {
        out2 = out;
        out = "";
        doWeExit = false;
        std::vector<std::string> items165 = LoopParseFunc(out2, "\n", "\r");
        for (size_t A_Index165 = 0; A_Index165 < items165.size() + 0; A_Index165++) {
            std::string A_LoopField165 = items165[A_Index165 - 0];
            if (SubStr(StrLower(Trim(A_LoopField165)), 1, StrLen(keyWordIF)) == keyWordIF) {
                doWeJustExit = 1;
                if (fixIfElseIfMultiLineStatementHELP(A_LoopField165) == "") {
                    out += Trim(A_LoopField165) + " ";
                    doWeExit = false;
                    doWeExitHELP--;
                } else {
                    doWeExit = true;
                    out += Trim(A_LoopField165) + Chr(10);
                }
            }
            else if (SubStr(StrLower(Trim(A_LoopField165)), 1, StrLen(keyWordWhileLoop)) == keyWordWhileLoop) {
                doWeJustExit = 1;
                if (fixIfElseIfMultiLineStatementHELP(A_LoopField165) == "") {
                    out += Trim(A_LoopField165) + " ";
                    doWeExit = false;
            ...
            else if (SubStr(StrLower(Trim(A_LoopField165)), 1, StrLen(keyWordElseIf)) == keyWordElseIf) {
                doWeJustExit = 1;
                if (fixIfElseIfMultiLineStatementHELP(A_LoopField165) == "") {
                    out += Trim(A_LoopField165) + " ";
                    doWeExit = false;
...
bool doseHaveInclude(std::string TheCodeThatMightHaveInclude) {
    std::vector<std::string> items166 = LoopParseFunc(TheCodeThatMightHaveInclude, "\n", "\r");
    for (size_t A_Index166 = 0; A_Index166 < items166.size() + 0; A_Index166++) {
        std::string A_LoopField166 = items166[A_Index166 - 0];
        if (SubStr(StrLower(Trim(A_LoopField166)), 1, StrLen(StrLower(keyWordInclude + " "))) == StrLower(keyWordInclude + " ")) {
            return true;
        }
    }
    return false;
...
std::string compiler(std::string htCode, std::string allInstructionFile, std::string mode, std::string langToConvertToParam = "") {
    ...
    std::vector<std::string> items167 = LoopParseFunc(allInstructionFile, "\n", "\r");
    for (size_t A_Index167 = 0; A_Index167 < items167.size() + 0; A_Index167++) {
        std::string A_LoopField167 = items167[A_Index167 - 0];
        if (A_Index167 == 0) {
            langToConvertTo = Trim(A_LoopField167);
        }
        if (A_Index167 == 1) {
            langFileExtension = Trim(A_LoopField167);
        }
        if (A_Index167 == 2) {
            commands = Trim(A_LoopField167);
        }
        if (A_Index167 == 3) {
            keyWordAlliance = Trim(A_LoopField167) + " ";
        }
        if (A_Index167 == 4) {
            keyWordCrew = Trim(A_LoopField167) + " ";
        }
        if (A_Index167 == 5) {
            keyWordProc = Trim(A_LoopField167) + " ";
        }
        if (A_Index167 == 6) {
            keyWordStruct = Trim(A_LoopField167) + " ";
        }
        if (A_Index167 == 7) {
            keyWordProp = Trim(A_LoopField167) + " ";
        }
        if (A_Index167 == 8) {
            keyWordThis = Trim(A_LoopField167);
        }
        if (A_Index167 == 9) {
            keyWordInclude = Trim(A_LoopField167);
        }
        if (A_Index167 == 10) {
            keyWordCodeInTheTranspiledLangStart = Trim(A_LoopField167);
        }
        if (A_Index167 == 11) {
            keyWordCodeInTheTranspiledLangEnd = Trim(A_LoopField167);
        }
        if (A_Index167 == 12) {
            keyWordCodeInTheTranspiledLangStartCPP = Trim(A_LoopField167);
        }
        if (A_Index167 == 13) {
            keyWordCodeInTheTranspiledLangEndCPP = Trim(A_LoopField167);
        }
        if (A_Index167 == 14) {
            keyWordCodeInTheTranspiledLangStartPY = Trim(A_LoopField167);
        }
        if (A_Index167 == 15) {
            keyWordCodeInTheTranspiledLangEndPY = Trim(A_LoopField167);
        }
        if (A_Index167 == 16) {
            keyWordCodeInTheTranspiledLangStartJS = Trim(A_LoopField167);
        }
        if (A_Index167 == 17) {
            keyWordCodeInTheTranspiledLangEndJS = Trim(A_LoopField167);
        }
        if (A_Index167 == 18) {
            keyWordCodeInTheTranspiledLangStartGO = Trim(A_LoopField167);
        }
        if (A_Index167 == 19) {
            keyWordCodeInTheTranspiledLangEndGO = Trim(A_LoopField167);
        }
        if (A_Index167 == 20) {
            keyWordCodeInTheTranspiledLangStartLUA = Trim(A_LoopField167);
        }
        if (A_Index167 == 21) {
            keyWordCodeInTheTranspiledLangEndLUA = Trim(A_LoopField167);
        }
        if (A_Index167 == 22) {
            keyWordCodeInTheTranspiledLangStartCS = Trim(A_LoopField167);
        }
        if (A_Index167 == 23) {
            keyWordCodeInTheTranspiledLangEndCS = Trim(A_LoopField167);
        }
        if (A_Index167 == 24) {
            keyWordCodeInTheTranspiledLangStartJAVA = Trim(A_LoopField167);
        }
        if (A_Index167 == 25) {
            keyWordCodeInTheTranspiledLangEndJAVA = Trim(A_LoopField167);
        }
        if (A_Index167 == 26) {
            keyWordCodeInTheTranspiledLangStartKT = Trim(A_LoopField167);
        }
        if (A_Index167 == 27) {
            keyWordCodeInTheTranspiledLangEndKT = Trim(A_LoopField167);
        }
        if (A_Index167 == 28) {
            keyWordCodeInTheTranspiledLangStartRB = Trim(A_LoopField167);
        }
        if (A_Index167 == 29) {
            keyWordCodeInTheTranspiledLangEndRB = Trim(A_LoopField167);
        }
        if (A_Index167 == 30) {
            keyWordCodeInTheTranspiledLangStartNIM = Trim(A_LoopField167);
        }
        if (A_Index167 == 31) {
            keyWordCodeInTheTranspiledLangEndNIM = Trim(A_LoopField167);
        }
        if (A_Index167 == 32) {
            keyWordCodeInTheTranspiledLangStartAHK = Trim(A_LoopField167);
        }
        if (A_Index167 == 33) {
            keyWordCodeInTheTranspiledLangEndAHK = Trim(A_LoopField167);
        }
        if (A_Index167 == 34) {
            keyWordCodeInTheTranspiledLangStartSWIFT = Trim(A_LoopField167);
        }
        if (A_Index167 == 35) {
            keyWordCodeInTheTranspiledLangEndSWIFT = Trim(A_LoopField167);
        }
        if (A_Index167 == 36) {
            keyWordCodeInTheTranspiledLangStartDART = Trim(A_LoopField167);
        }
        if (A_Index167 == 37) {
            keyWordCodeInTheTranspiledLangEndDART = Trim(A_LoopField167);
        }
        if (A_Index167 == 38) {
            keyWordCodeInTheTranspiledLangStartTS = Trim(A_LoopField167);
        }
        if (A_Index167 == 39) {
            keyWordCodeInTheTranspiledLangEndTS = Trim(A_LoopField167);
        }
        if (A_Index167 == 40) {
            keyWordCodeInTheTranspiledLangStartGROOVY = Trim(A_LoopField167);
        }
        if (A_Index167 == 41) {
            keyWordCodeInTheTranspiledLangEndGROOVY = Trim(A_LoopField167);
        }
        if (A_Index167 == 42) {
            keyWordCodeInTheTranspiledLangStartHTVM = Trim(A_LoopField167);
        }
        if (A_Index167 == 43) {
            keyWordCodeInTheTranspiledLangEndHTVM = Trim(A_LoopField167);
        }
        if (A_Index167 == 44) {
            keyWordCodeInHTVMstart = Trim(A_LoopField167);
        }
        if (A_Index167 == 45) {
            keyWordCodeInHTVMend = Trim(A_LoopField167);
        }
        if (A_Index167 == 46) {
            keyWordCurlyBraceOpen = Trim(A_LoopField167);
        }
        if (A_Index167 == 47) {
            keyWordCurlyBraceClose = Trim(A_LoopField167);
        }
        if (A_Index167 == 48) {
            keyWordNull = Trim(A_LoopField167);
        }
        if (A_Index167 == 49) {
            keyWordTrue = Trim(A_LoopField167);
        }
        if (A_Index167 == 50) {
            keyWordFalse = Trim(A_LoopField167);
        }
        if (A_Index167 == 51) {
            keyWordVoid = Trim(A_LoopField167);
        }
        if (A_Index167 == 52) {
            keyWordDouble = Trim(A_LoopField167);
        }
        if (A_Index167 == 53) {
            keyWordChar = Trim(A_LoopField167);
        }
        if (A_Index167 == 54) {
            keyWordUint8 = Trim(A_LoopField167);
        }
        if (A_Index167 == 55) {
            keyWordUint16 = Trim(A_LoopField167);
        }
        if (A_Index167 == 56) {
            keyWordUint32 = Trim(A_LoopField167);
        }
        if (A_Index167 == 57) {
            keyWordUint64 = Trim(A_LoopField167);
        }
        if (A_Index167 == 58) {
            keyWordINT = Trim(A_LoopField167);
        }
        if (A_Index167 == 59) {
            keyWordSTR = Trim(A_LoopField167);
        }
        if (A_Index167 == 60) {
            keyWordBOOL = Trim(A_LoopField167);
        }
        if (A_Index167 == 61) {
            keyWordFLOAT = Trim(A_LoopField167);
        }
        if (A_Index167 == 62) {
            keyWordINT8 = Trim(A_LoopField167);
        }
        if (A_Index167 == 63) {
            keyWordINT16 = Trim(A_LoopField167);
        }
        if (A_Index167 == 64) {
            keyWordINT32 = Trim(A_LoopField167);
        }
        if (A_Index167 == 65) {
            keyWordINT64 = Trim(A_LoopField167);
        }
        if (A_Index167 == 66) {
            keyWordIF = Trim(A_LoopField167) + " ";
        }
        if (A_Index167 == 67) {
            keyWordElseIf = Trim(A_LoopField167) + " ";
        }
        if (A_Index167 == 68) {
            keyWordElse = Trim(A_LoopField167);
        }
        if (A_Index167 == 69) {
            keyWordWhileLoop = Trim(A_LoopField167) + " ";
        }
        if (A_Index167 == 70) {
            keyWordLoopInfinite = Trim(A_LoopField167);
        }
        if (A_Index167 == 71) {
            keyWordLoop = Trim(A_LoopField167) + " ";
        }
        if (A_Index167 == 72) {
            keyWordLoopParse = Trim(A_LoopField167) + " ";
        }
        if (A_Index167 == 73) {
            keyWordContinue = Trim(A_LoopField167);
        }
        if (A_Index167 == 74) {
            keyWordBreak = Trim(A_LoopField167);
        }
        if (A_Index167 == 75) {
            keyWordFunc = Trim(A_LoopField167) + " ";
        }
        if (A_Index167 == 76) {
            keyWordAwait = Trim(A_LoopField167) + " ";
        }
        if (A_Index167 == 77) {
            keyWordAsync = Trim(A_LoopField167) + " ";
        }
        if (A_Index167 == 78) {
            keyWordThrow = Trim(A_LoopField167) + " ";
        }
        if (A_Index167 == 79) {
            keyWordErrorMsg = Trim(A_LoopField167);
        }
        if (A_Index167 == 80) {
            keyWordTry = Trim(A_LoopField167);
        }
        if (A_Index167 == 81) {
            keyWordCatch = Trim(A_LoopField167) + " ";
        }
        if (A_Index167 == 82) {
            keyWordFinally = Trim(A_LoopField167);
        }
        if (A_Index167 == 83) {
            keyWordReturnStatement = Trim(A_LoopField167) + " ";
        }
        if (A_Index167 == 84) {
            keyWordArrayAppend = Trim(A_LoopField167);
        }
        if (A_Index167 == 85) {
            keyWordArrayPop = Trim(A_LoopField167);
        }
        if (A_Index167 == 86) {
            keyWordArraySize = Trim(A_LoopField167);
        }
        if (A_Index167 == 87) {
            keyWordArrayInsert = Trim(A_LoopField167);
        }
        if (A_Index167 == 88) {
            keyWordArrayRemove = Trim(A_LoopField167);
        }
        if (A_Index167 == 89) {
            keyWordArrayIndexOf = Trim(A_LoopField167);
        }
        if (A_Index167 == 90) {
            keyWordArrayDefinition = Trim(A_LoopField167);
        }
        if (A_Index167 == 91) {
            keyWordArrayOfIntegersDefinition = Trim(A_LoopField167);
        }
        if (A_Index167 == 92) {
            keyWordArrayOfStringsDefinition = Trim(A_LoopField167);
        }
        if (A_Index167 == 93) {
            keyWordArrayOfFloatingPointNumbersDefinition = Trim(A_LoopField167);
        }
        if (A_Index167 == 94) {
            keyWordArrayOfBooleansDefinition = Trim(A_LoopField167);
        }
        if (A_Index167 == 95) {
            keyWordVar = Trim(A_LoopField167) + " ";
        }
        if (A_Index167 == 96) {
            keyWordLet = Trim(A_LoopField167) + " ";
        }
        if (A_Index167 == 97) {
            keyWordConst = Trim(A_LoopField167) + " ";
        }
        if (A_Index167 == 98) {
            keyWordEnd = Trim(A_LoopField167);
        }
        if (A_Index167 == 99) {
            keyWordGlobal = Trim(A_LoopField167) + " ";
        }
        if (A_Index167 == 100) {
            keyWordComment = Trim(A_LoopField167);
        }
        if (A_Index167 == 101) {
            keyWordCommentOpenMultiLine = Trim(A_LoopField167);
        }
        if (A_Index167 == 102) {
            keyWordCommentCloseMultiLine = Trim(A_LoopField167);
        }
        if (A_Index167 == 103) {
            keyWordEscpaeChar = Trim(A_LoopField167);
        }
        if (A_Index167 == 104) {
            keyWordMainLabel = Trim(A_LoopField167);
        }
        if (A_Index167 == 105) {
            keyWordConcat = Trim(A_LoopField167);
        }
        if (A_Index167 == 106) {
            keyWordAdd = Trim(A_LoopField167);
        }
        if (A_Index167 == 107) {
            keyWordSub = Trim(A_LoopField167);
        }
        if (A_Index167 == 108) {
            keyWordMul = Trim(A_LoopField167);
        }
        if (A_Index167 == 109) {
            keyWordDiv = Trim(A_LoopField167);
        }
        if (A_Index167 == 110) {
            keyWordMod = Trim(A_LoopField167);
        }
        if (A_Index167 == 111) {
            keyWordExp = Trim(A_LoopField167);
        }
        if (A_Index167 == 112) {
            keyWordEqual = Trim(A_LoopField167);
        }
        if (A_Index167 == 113) {
            keyWordStrictEqual = Trim(A_LoopField167);
        }
        if (A_Index167 == 114) {
            keyWordNotEqual = Trim(A_LoopField167);
        }
        if (A_Index167 == 115) {
            keyWordGreater = Trim(A_LoopField167);
        }
        if (A_Index167 == 116) {
            keyWordLess = Trim(A_LoopField167);
        }
        if (A_Index167 == 117) {
            keyWordGreaterEqual = Trim(A_LoopField167);
        }
        if (A_Index167 == 118) {
            keyWordLessEqual = Trim(A_LoopField167);
        }
        if (A_Index167 == 119) {
            keyWordAnd = Trim(A_LoopField167);
        }
        if (A_Index167 == 120) {
            keyWordOr = Trim(A_LoopField167);
        }
        if (A_Index167 == 121) {
            keyWordNot = Trim(A_LoopField167);
        }
        if (A_Index167 == 122) {
            keyWordBitAnd = Trim(A_LoopField167);
        }
        if (A_Index167 == 123) {
            keyWordBitOr = Trim(A_LoopField167);
        }
        if (A_Index167 == 124) {
            keyWordBitXor = Trim(A_LoopField167);
        }
        if (A_Index167 == 125) {
            keyWordBitNot = Trim(A_LoopField167);
        }
        if (A_Index167 == 126) {
            keyWordShiftLeft = Trim(A_LoopField167);
        }
        if (A_Index167 == 127) {
            keyWordShiftRight = Trim(A_LoopField167);
        }
        if (A_Index167 == 128) {
            keyWordShiftUnsignedRight = Trim(A_LoopField167);
        }
        if (A_Index167 == 129) {
            keyWordAssign = Trim(A_LoopField167);
        }
        if (A_Index167 == 130) {
            keyWordAssignAdd = Trim(A_LoopField167);
        }
        if (A_Index167 == 131) {
            keyWordAssignConcat = Trim(A_LoopField167);
        }
        if (A_Index167 == 132) {
            keyWordAssignSub = Trim(A_LoopField167);
        }
        if (A_Index167 == 133) {
            keyWordAssignMul = Trim(A_LoopField167);
        }
        if (A_Index167 == 134) {
            keyWordAssignDiv = Trim(A_LoopField167);
        }
        if (A_Index167 == 135) {
            keyWordAssignMod = Trim(A_LoopField167);
        }
        if (A_Index167 == 136) {
            keyWordAssignShiftLeft = Trim(A_LoopField167);
        }
        if (A_Index167 == 137) {
            keyWordAssignShiftRight = Trim(A_LoopField167);
        }
        if (A_Index167 == 138) {
            keyWordLogicalAssignShiftRight = Trim(A_LoopField167);
        }
        if (A_Index167 == 139) {
            keyWordAssignBitAnd = Trim(A_LoopField167);
        }
        if (A_Index167 == 140) {
            keyWordAssignBitOr = Trim(A_LoopField167);
        }
        if (A_Index167 == 141) {
            keyWordAssignBitXor = Trim(A_LoopField167);
        }
        if (A_Index167 == 142) {
            keyWordTernary1 = Trim(A_LoopField167);
        }
        if (A_Index167 == 143) {
            keyWordTernary2 = Trim(A_LoopField167);
        }
        if (A_Index167 == 144) {
            keyWordInc = Trim(A_LoopField167);
        }
        if (A_Index167 == 145) {
            keyWordDec = Trim(A_LoopField167);
        }
        if (A_Index167 == 146) {
            AHKlikeLoopsIndexedAt = Trim(A_LoopField167);
        }
        if (A_Index167 == 147) {
            keyWordAIndex = Trim(A_LoopField167);
        }
        if (A_Index167 == 148) {
            keyWordALoopField = Trim(A_LoopField167);
        }
        if (A_Index167 == 149) {
            useCurlyBraces = Trim(A_LoopField167);
        }
        if (A_Index167 == 150) {
            useEnd = Trim(A_LoopField167);
        }
        if (A_Index167 == 151) {
            useSemicolon = Trim(A_LoopField167);
        }
        if (A_Index167 == 152) {
            useParentheses = Trim(A_LoopField167);
        }
        if (A_Index167 == 153) {
            usePrefixTypeForTypeDefinition = Trim(A_LoopField167);
        }
        if (A_Index167 == 154) {
            usePostfixTypeForTypeDefinition = Trim(A_LoopField167);
        }
        if (A_Index167 == 155) {
            usePythonicColonSyntax = Trim(A_LoopField167);
        }
        if (A_Index167 == 156) {
            useCurlyBracesSyntaxForArrayDef = Trim(A_LoopField167);
        }
        if (A_Index167 == 157) {
            useInJavaScriptAlwaysUseVar = Trim(A_LoopField167);
        }
        if (A_Index167 == 158) {
            useJavaScriptInAfullHTMLfile = Trim(A_LoopField167);
        }
        if (A_Index167 == 159) {
            useJavaScriptAmainFuncDef = Trim(A_LoopField167);
        }
        if (A_Index167 == 160) {
            useJavaScriptAllFuncsAreAsync = Trim(A_LoopField167);
        }
        if (A_Index167 == 161) {
            useJavaScriptAlwaysTripleEqual = Trim(A_LoopField167);
        }
    }
    if (HTVM_Size(argHTVMinstrMORE) > 0) {
        ...
        std::vector<std::string> items168 = LoopParseFunc(allInstructionFile_2, "\n", "\r");
        for (size_t A_Index168 = 0; A_Index168 < items168.size() + 0; A_Index168++) {
            std::string A_LoopField168 = items168[A_Index168 - 0];
            if (A_Index168 == 0) {
                langToConvertTo_2 = Trim(A_LoopField168);
            }
            if (A_Index168 == 1) {
                langFileExtension_2 = Trim(A_LoopField168);
            }
            if (A_Index168 == 2) {
                commands_2 = Trim(A_LoopField168);
            }
            if (A_Index168 == 3) {
                keyWordAlliance_2 = Trim(A_LoopField168) + " ";
            }
            if (A_Index168 == 4) {
                keyWordCrew_2 = Trim(A_LoopField168) + " ";
            }
            if (A_Index168 == 5) {
                keyWordProc_2 = Trim(A_LoopField168) + " ";
            }
            if (A_Index168 == 6) {
                keyWordStruct_2 = Trim(A_LoopField168) + " ";
            }
            if (A_Index168 == 7) {
                keyWordProp_2 = Trim(A_LoopField168) + " ";
            }
            if (A_Index168 == 8) {
                keyWordThis_2 = Trim(A_LoopField168);
            }
            if (A_Index168 == 9) {
                keyWordInclude_2 = Trim(A_LoopField168);
            }
            if (A_Index168 == 10) {
                keyWordCodeInTheTranspiledLangStart_2 = Trim(A_LoopField168);
            }
            if (A_Index168 == 11) {
                keyWordCodeInTheTranspiledLangEnd_2 = Trim(A_LoopField168);
            }
            if (A_Index168 == 12) {
                keyWordCodeInTheTranspiledLangStartCPP_2 = Trim(A_LoopField168);
            }
            if (A_Index168 == 13) {
                keyWordCodeInTheTranspiledLangEndCPP_2 = Trim(A_LoopField168);
            }
            if (A_Index168 == 14) {
                keyWordCodeInTheTranspiledLangStartPY_2 = Trim(A_LoopField168);
            }
            if (A_Index168 == 15) {
                keyWordCodeInTheTranspiledLangEndPY_2 = Trim(A_LoopField168);
            }
            if (A_Index168 == 16) {
                keyWordCodeInTheTranspiledLangStartJS_2 = Trim(A_LoopField168);
            }
            if (A_Index168 == 17) {
                keyWordCodeInTheTranspiledLangEndJS_2 = Trim(A_LoopField168);
            }
            if (A_Index168 == 18) {
                keyWordCodeInTheTranspiledLangStartGO_2 = Trim(A_LoopField168);
            }
            if (A_Index168 == 19) {
                keyWordCodeInTheTranspiledLangEndGO_2 = Trim(A_LoopField168);
            }
            if (A_Index168 == 20) {
                keyWordCodeInTheTranspiledLangStartLUA_2 = Trim(A_LoopField168);
            }
            if (A_Index168 == 21) {
                keyWordCodeInTheTranspiledLangEndLUA_2 = Trim(A_LoopField168);
            }
            if (A_Index168 == 22) {
                keyWordCodeInTheTranspiledLangStartCS_2 = Trim(A_LoopField168);
            }
            if (A_Index168 == 23) {
                keyWordCodeInTheTranspiledLangEndCS_2 = Trim(A_LoopField168);
            }
            if (A_Index168 == 24) {
                keyWordCodeInTheTranspiledLangStartJAVA_2 = Trim(A_LoopField168);
            }
            if (A_Index168 == 25) {
                keyWordCodeInTheTranspiledLangEndJAVA_2 = Trim(A_LoopField168);
            }
            if (A_Index168 == 26) {
                keyWordCodeInTheTranspiledLangStartKT_2 = Trim(A_LoopField168);
            }
            if (A_Index168 == 27) {
                keyWordCodeInTheTranspiledLangEndKT_2 = Trim(A_LoopField168);
            }
            if (A_Index168 == 28) {
                keyWordCodeInTheTranspiledLangStartRB_2 = Trim(A_LoopField168);
            }
            if (A_Index168 == 29) {
                keyWordCodeInTheTranspiledLangEndRB_2 = Trim(A_LoopField168);
            }
            if (A_Index168 == 30) {
                keyWordCodeInTheTranspiledLangStartNIM_2 = Trim(A_LoopField168);
            }
            if (A_Index168 == 31) {
                keyWordCodeInTheTranspiledLangEndNIM_2 = Trim(A_LoopField168);
            }
            if (A_Index168 == 32) {
                keyWordCodeInTheTranspiledLangStartAHK_2 = Trim(A_LoopField168);
            }
            if (A_Index168 == 33) {
                keyWordCodeInTheTranspiledLangEndAHK_2 = Trim(A_LoopField168);
            }
            if (A_Index168 == 34) {
                keyWordCodeInTheTranspiledLangStartSWIFT_2 = Trim(A_LoopField168);
            }
            if (A_Index168 == 35) {
                keyWordCodeInTheTranspiledLangEndSWIFT_2 = Trim(A_LoopField168);
            }
            if (A_Index168 == 36) {
                keyWordCodeInTheTranspiledLangStartDART_2 = Trim(A_LoopField168);
            }
            if (A_Index168 == 37) {
                keyWordCodeInTheTranspiledLangEndDART_2 = Trim(A_LoopField168);
            }
            if (A_Index168 == 38) {
                keyWordCodeInTheTranspiledLangStartTS_2 = Trim(A_LoopField168);
            }
            if (A_Index168 == 39) {
                keyWordCodeInTheTranspiledLangEndTS_2 = Trim(A_LoopField168);
            }
            if (A_Index168 == 40) {
                keyWordCodeInTheTranspiledLangStartGROOVY_2 = Trim(A_LoopField168);
            }
            if (A_Index168 == 41) {
                keyWordCodeInTheTranspiledLangEndGROOVY_2 = Trim(A_LoopField168);
            }
            if (A_Index168 == 42) {
                keyWordCodeInTheTranspiledLangStartHTVM_2 = Trim(A_LoopField168);
            }
            if (A_Index168 == 43) {
                keyWordCodeInTheTranspiledLangEndHTVM_2 = Trim(A_LoopField168);
            }
            if (A_Index168 == 44) {
                keyWordCodeInHTVMstart_2 = Trim(A_LoopField168);
            }
            if (A_Index168 == 45) {
                keyWordCodeInHTVMend_2 = Trim(A_LoopField168);
            }
            if (A_Index168 == 46) {
                keyWordCurlyBraceOpen_2 = Trim(A_LoopField168);
            }
            if (A_Index168 == 47) {
                keyWordCurlyBraceClose_2 = Trim(A_LoopField168);
            }
            if (A_Index168 == 48) {
                keyWordNull_2 = Trim(A_LoopField168);
            }
            if (A_Index168 == 49) {
                keyWordTrue_2 = Trim(A_LoopField168);
            }
            if (A_Index168 == 50) {
                keyWordFalse_2 = Trim(A_LoopField168);
            }
            if (A_Index168 == 51) {
                keyWordVoid_2 = Trim(A_LoopField168);
            }
            if (A_Index168 == 52) {
                keyWordDouble_2 = Trim(A_LoopField168);
            }
            if (A_Index168 == 53) {
                keyWordChar_2 = Trim(A_LoopField168);
            }
            if (A_Index168 == 54) {
                keyWordUint8_2 = Trim(A_LoopField168);
            }
            if (A_Index168 == 55) {
                keyWordUint16_2 = Trim(A_LoopField168);
            }
            if (A_Index168 == 56) {
                keyWordUint32_2 = Trim(A_LoopField168);
            }
            if (A_Index168 == 57) {
                keyWordUint64_2 = Trim(A_LoopField168);
            }
            if (A_Index168 == 58) {
                keyWordINT_2 = Trim(A_LoopField168);
            }
            if (A_Index168 == 59) {
                keyWordSTR_2 = Trim(A_LoopField168);
            }
            if (A_Index168 == 60) {
                keyWordBOOL_2 = Trim(A_LoopField168);
            }
            if (A_Index168 == 61) {
                keyWordFLOAT_2 = Trim(A_LoopField168);
            }
            if (A_Index168 == 62) {
                keyWordINT8_2 = Trim(A_LoopField168);
            }
            if (A_Index168 == 63) {
                keyWordINT16_2 = Trim(A_LoopField168);
            }
            if (A_Index168 == 64) {
                keyWordINT32_2 = Trim(A_LoopField168);
            }
            if (A_Index168 == 65) {
                keyWordINT64_2 = Trim(A_LoopField168);
            }
            if (A_Index168 == 66) {
                keyWordIF_2 = Trim(A_LoopField168) + " ";
            }
            if (A_Index168 == 67) {
                keyWordElseIf_2 = Trim(A_LoopField168) + " ";
            }
            if (A_Index168 == 68) {
                keyWordElse_2 = Trim(A_LoopField168);
            }
            if (A_Index168 == 69) {
                keyWordWhileLoop_2 = Trim(A_LoopField168) + " ";
            }
            if (A_Index168 == 70) {
                keyWordLoopInfinite_2 = Trim(A_LoopField168);
            }
            if (A_Index168 == 71) {
                keyWordLoop_2 = Trim(A_LoopField168) + " ";
            }
            if (A_Index168 == 72) {
                keyWordLoopParse_2 = Trim(A_LoopField168) + " ";
            }
            if (A_Index168 == 73) {
                keyWordContinue_2 = Trim(A_LoopField168);
            }
            if (A_Index168 == 74) {
                keyWordBreak_2 = Trim(A_LoopField168);
            }
            if (A_Index168 == 75) {
                keyWordFunc_2 = Trim(A_LoopField168) + " ";
            }
            if (A_Index168 == 76) {
                keyWordAwait_2 = Trim(A_LoopField168) + " ";
            }
            if (A_Index168 == 77) {
                keyWordAsync_2 = Trim(A_LoopField168) + " ";
            }
            if (A_Index168 == 78) {
                keyWordThrow_2 = Trim(A_LoopField168) + " ";
            }
            if (A_Index168 == 79) {
                keyWordErrorMsg_2 = Trim(A_LoopField168);
            }
            if (A_Index168 == 80) {
                keyWordTry_2 = Trim(A_LoopField168);
            }
            if (A_Index168 == 81) {
                keyWordCatch_2 = Trim(A_LoopField168) + " ";
            }
            if (A_Index168 == 82) {
                keyWordFinally_2 = Trim(A_LoopField168);
            }
            if (A_Index168 == 83) {
                keyWordReturnStatement_2 = Trim(A_LoopField168) + " ";
            }
            if (A_Index168 == 84) {
                keyWordArrayAppend_2 = Trim(A_LoopField168);
            }
            if (A_Index168 == 85) {
                keyWordArrayPop_2 = Trim(A_LoopField168);
            }
            if (A_Index168 == 86) {
                keyWordArraySize_2 = Trim(A_LoopField168);
            }
            if (A_Index168 == 87) {
                keyWordArrayInsert_2 = Trim(A_LoopField168);
            }
            if (A_Index168 == 88) {
                keyWordArrayRemove_2 = Trim(A_LoopField168);
            }
            if (A_Index168 == 89) {
                keyWordArrayIndexOf_2 = Trim(A_LoopField168);
            }
            if (A_Index168 == 90) {
                keyWordArrayDefinition_2 = Trim(A_LoopField168);
            }
            if (A_Index168 == 91) {
                keyWordArrayOfIntegersDefinition_2 = Trim(A_LoopField168);
            }
            if (A_Index168 == 92) {
                keyWordArrayOfStringsDefinition_2 = Trim(A_LoopField168);
            }
            if (A_Index168 == 93) {
                keyWordArrayOfFloatingPointNumbersDefinition_2 = Trim(A_LoopField168);
            }
            if (A_Index168 == 94) {
                keyWordArrayOfBooleansDefinition_2 = Trim(A_LoopField168);
            }
            if (A_Index168 == 95) {
                keyWordVar_2 = Trim(A_LoopField168);
            }
            if (A_Index168 == 96) {
                keyWordLet_2 = Trim(A_LoopField168);
            }
            if (A_Index168 == 97) {
                keyWordConst_2 = Trim(A_LoopField168);
            }
            if (A_Index168 == 98) {
                keyWordEnd_2 = Trim(A_LoopField168);
            }
            if (A_Index168 == 99) {
                keyWordGlobal_2 = Trim(A_LoopField168) + " ";
            }
            if (A_Index168 == 100) {
                keyWordComment_2 = Trim(A_LoopField168);
            }
            if (A_Index168 == 101) {
                keyWordCommentOpenMultiLine_2 = Trim(A_LoopField168);
            }
            if (A_Index168 == 102) {
                keyWordCommentCloseMultiLine_2 = Trim(A_LoopField168);
            }
            if (A_Index168 == 103) {
                keyWordEscpaeChar_2 = Trim(A_LoopField168);
            }
            if (A_Index168 == 104) {
                keyWordMainLabel_2 = Trim(A_LoopField168);
            }
            if (A_Index168 == 105) {
                keyWordConcat_2 = Trim(A_LoopField168);
            }
            if (A_Index168 == 106) {
                keyWordAdd_2 = Trim(A_LoopField168);
            }
            if (A_Index168 == 107) {
                keyWordSub_2 = Trim(A_LoopField168);
            }
            if (A_Index168 == 108) {
                keyWordMul_2 = Trim(A_LoopField168);
            }
            if (A_Index168 == 109) {
                keyWordDiv_2 = Trim(A_LoopField168);
            }
            if (A_Index168 == 110) {
                keyWordMod_2 = Trim(A_LoopField168);
            }
            if (A_Index168 == 111) {
                keyWordExp_2 = Trim(A_LoopField168);
            }
            if (A_Index168 == 112) {
                keyWordEqual_2 = Trim(A_LoopField168);
            }
            if (A_Index168 == 113) {
                keyWordStrictEqual_2 = Trim(A_LoopField168);
            }
            if (A_Index168 == 114) {
                keyWordNotEqual_2 = Trim(A_LoopField168);
            }
            if (A_Index168 == 115) {
                keyWordGreater_2 = Trim(A_LoopField168);
            }
            if (A_Index168 == 116) {
                keyWordLess_2 = Trim(A_LoopField168);
            }
            if (A_Index168 == 117) {
                keyWordGreaterEqual_2 = Trim(A_LoopField168);
            }
            if (A_Index168 == 118) {
                keyWordLessEqual_2 = Trim(A_LoopField168);
            }
            if (A_Index168 == 119) {
                keyWordAnd_2 = Trim(A_LoopField168);
            }
            if (A_Index168 == 120) {
                keyWordOr_2 = Trim(A_LoopField168);
            }
            if (A_Index168 == 121) {
                keyWordNot_2 = Trim(A_LoopField168);
            }
            if (A_Index168 == 122) {
                keyWordBitAnd_2 = Trim(A_LoopField168);
            }
            if (A_Index168 == 123) {
                keyWordBitOr_2 = Trim(A_LoopField168);
            }
            if (A_Index168 == 124) {
                keyWordBitXor_2 = Trim(A_LoopField168);
            }
            if (A_Index168 == 125) {
                keyWordBitNot_2 = Trim(A_LoopField168);
            }
            if (A_Index168 == 126) {
                keyWordShiftLeft_2 = Trim(A_LoopField168);
            }
            if (A_Index168 == 127) {
                keyWordShiftRight_2 = Trim(A_LoopField168);
            }
            if (A_Index168 == 128) {
                keyWordShiftUnsignedRight_2 = Trim(A_LoopField168);
            }
            if (A_Index168 == 129) {
                keyWordAssign_2 = Trim(A_LoopField168);
            }
            if (A_Index168 == 130) {
                keyWordAssignAdd_2 = Trim(A_LoopField168);
            }
            if (A_Index168 == 131) {
                keyWordAssignConcat_2 = Trim(A_LoopField168);
            }
            if (A_Index168 == 132) {
                keyWordAssignSub_2 = Trim(A_LoopField168);
            }
            if (A_Index168 == 133) {
                keyWordAssignMul_2 = Trim(A_LoopField168);
            }
            if (A_Index168 == 134) {
                keyWordAssignDiv_2 = Trim(A_LoopField168);
            }
            if (A_Index168 == 135) {
                keyWordAssignMod_2 = Trim(A_LoopField168);
            }
            if (A_Index168 == 136) {
                keyWordAssignShiftLeft_2 = Trim(A_LoopField168);
            }
            if (A_Index168 == 137) {
                keyWordAssignShiftRight_2 = Trim(A_LoopField168);
            }
            if (A_Index168 == 138) {
                keyWordLogicalAssignShiftRight_2 = Trim(A_LoopField168);
            }
            if (A_Index168 == 139) {
                keyWordAssignBitAnd_2 = Trim(A_LoopField168);
            }
            if (A_Index168 == 140) {
                keyWordAssignBitOr_2 = Trim(A_LoopField168);
            }
            if (A_Index168 == 141) {
                keyWordAssignBitXor_2 = Trim(A_LoopField168);
            }
            if (A_Index168 == 142) {
                keyWordTernary1_2 = Trim(A_LoopField168);
            }
            if (A_Index168 == 143) {
                keyWordTernary2_2 = Trim(A_LoopField168);
            }
            if (A_Index168 == 144) {
                keyWordInc_2 = Trim(A_LoopField168);
            }
            if (A_Index168 == 145) {
                keyWordDec_2 = Trim(A_LoopField168);
            }
            if (A_Index168 == 146) {
                AHKlikeLoopsIndexedAt_2 = Trim(A_LoopField168);
            }
            if (A_Index168 == 147) {
                keyWordAIndex_2 = Trim(A_LoopField168);
            }
            if (A_Index168 == 148) {
                keyWordALoopField_2 = Trim(A_LoopField168);
            }
            if (A_Index168 == 149) {
                useCurlyBraces_2 = Trim(A_LoopField168);
            }
            if (A_Index168 == 150) {
                useEnd_2 = Trim(A_LoopField168);
            }
            if (A_Index168 == 151) {
                useSemicolon_2 = Trim(A_LoopField168);
            }
            if (A_Index168 == 152) {
                useParentheses_2 = Trim(A_LoopField168);
            }
            if (A_Index168 == 153) {
                usePrefixTypeForTypeDefinition_2 = Trim(A_LoopField168);
            }
            if (A_Index168 == 154) {
                usePostfixTypeForTypeDefinition_2 = Trim(A_LoopField168);
            }
            if (A_Index168 == 155) {
                usePythonicColonSyntax_2 = Trim(A_LoopField168);
            }
            if (A_Index168 == 156) {
                useCurlyBracesSyntaxForArrayDef_2 = Trim(A_LoopField168);
            }
            if (A_Index168 == 157) {
                useInJavaScriptAlwaysUseVar_2 = Trim(A_LoopField168);
            }
            if (A_Index168 == 158) {
                useJavaScriptInAfullHTMLfile_2 = Trim(A_LoopField168);
            }
            if (A_Index168 == 159) {
                useJavaScriptAmainFuncDef_2 = Trim(A_LoopField168);
            }
            if (A_Index168 == 160) {
                useJavaScriptAllFuncsAreAsync_2 = Trim(A_LoopField168);
            }
            if (A_Index168 == 161) {
                useJavaScriptAlwaysTripleEqual_2 = Trim(A_LoopField168);
            }
        }
    }
    ...
    */
    ...
    std::vector<std::string> items169 = LoopParseFunc(allInstructionFile, "\n", "\r");
    for (size_t A_Index169 = 0; A_Index169 < items169.size() + 0; A_Index169++) {
        std::string A_LoopField169 = items169[A_Index169 - 0];
        if (RegExMatch(Trim(A_LoopField169), "^name: [A-Za-z0-9_]+")) {
            availableFuncsInHTVMInHTVM += Trim(StrSplit(Trim(A_LoopField169), "name: ", 2)) + Chr(10);
        }
    }
    ...
    std::vector<std::string> items170 = LoopParseFunc(code, "\n", "\r");
    ...
    if (isNotHTVMfile == 0) {
                ...
                for (int A_Index171 = 0; A_Index171 < 10000 + 0; A_Index171++) {
                    ...
                    for (size_t A_Index172 = 0; A_Index172 < items172.size() + 0; A_Index172++) {
                        ...
                        if (SubStr(StrLower(Trim(currentLine)), 1, StrLen(keyWordInclude + " ")) == StrLower(keyWordInclude + " ")) {
                            ...
                            for (int A_Index173 = 0; A_Index173 < HTVM_Size(includedFilePaths) + 0; A_Index173++) {
                                if (filePathToInclude == includedFilePaths[A_Index173]) {
        ...
        for (size_t A_Index174 = 0; A_Index174 < items174.size() + 0; A_Index174++) {
            ...
            if (Trim(StrLower(A_LoopField174)) == StrLower(keyWordCodeInTheTranspiledLangEnd)) {
                COUNT_programmingBlock_InTheTranspiledLang++;
                holdTempDataProgrammingBlockThenPutInArr = StringTrimRight(holdTempDataProgrammingBlockThenPutInArr, 1);
                programmingBlocksTemp += "programmingBlock_InTheTranspiledLang-programmingBlock_InTheTranspiledLang-AA" + STR(COUNT_programmingBlock_InTheTranspiledLang) + "AA\n";
                HTVM_Append(programmingBlock_InTheTranspiledLang, Chr(10) + holdTempDataProgrammingBlockThenPutInArr);
            ...
            else if (Trim(StrLower(A_LoopField174)) == StrLower(keyWordCodeInTheTranspiledLangEndCPP)) {
                COUNT_programmingBlock_CPP++;
                holdTempDataProgrammingBlockThenPutInArr = StringTrimRight(holdTempDataProgrammingBlockThenPutInArr, 1);
                programmingBlocksTemp += "programmingBlock_CPP-programmingBlock_CPP-AA" + STR(COUNT_programmingBlock_CPP) + "AA\n";
                HTVM_Append(programmingBlock_CPP, Chr(10) + holdTempDataProgrammingBlockThenPutInArr);
            ...
            else if (Trim(StrLower(A_LoopField174)) == StrLower(keyWordCodeInTheTranspiledLangEndPY)) {
                COUNT_programmingBlock_PY++;
                holdTempDataProgrammingBlockThenPutInArr = StringTrimRight(holdTempDataProgrammingBlockThenPutInArr, 1);
                programmingBlocksTemp += "programmingBlock_PY-programmingBlock_PY-AA" + STR(COUNT_programmingBlock_PY) + "AA\n";
                HTVM_Append(programmingBlock_PY, Chr(10) + holdTempDataProgrammingBlockThenPutInArr);
            ...
            else if (Trim(StrLower(A_LoopField174)) == StrLower(keyWordCodeInTheTranspiledLangEndJS)) {
                COUNT_programmingBlock_JS++;
                holdTempDataProgrammingBlockThenPutInArr = StringTrimRight(holdTempDataProgrammingBlockThenPutInArr, 1);
                programmingBlocksTemp += "programmingBlock_JS-programmingBlock_JS-AA" + STR(COUNT_programmingBlock_JS) + "AA\n";
                HTVM_Append(programmingBlock_JS, Chr(10) + holdTempDataProgrammingBlockThenPutInArr);
            ...
            else if (Trim(StrLower(A_LoopField174)) == StrLower(keyWordCodeInTheTranspiledLangEndGO)) {
                COUNT_programmingBlock_GO++;
                holdTempDataProgrammingBlockThenPutInArr = StringTrimRight(holdTempDataProgrammingBlockThenPutInArr, 1);
                programmingBlocksTemp += "programmingBlock_GO-programmingBlock_GO-AA" + STR(COUNT_programmingBlock_GO) + "AA\n";
                HTVM_Append(programmingBlock_GO, Chr(10) + holdTempDataProgrammingBlockThenPutInArr);
            ...
            else if (Trim(StrLower(A_LoopField174)) == StrLower(keyWordCodeInTheTranspiledLangEndLUA)) {
                COUNT_programmingBlock_LUA++;
                holdTempDataProgrammingBlockThenPutInArr = StringTrimRight(holdTempDataProgrammingBlockThenPutInArr, 1);
                programmingBlocksTemp += "programmingBlock_LUA-programmingBlock_LUA-AA" + STR(COUNT_programmingBlock_LUA) + "AA\n";
                HTVM_Append(programmingBlock_LUA, Chr(10) + holdTempDataProgrammingBlockThenPutInArr);
            ...
            else if (Trim(StrLower(A_LoopField174)) == StrLower(keyWordCodeInTheTranspiledLangEndCS)) {
                COUNT_programmingBlock_CS++;
                holdTempDataProgrammingBlockThenPutInArr = StringTrimRight(holdTempDataProgrammingBlockThenPutInArr, 1);
                programmingBlocksTemp += "programmingBlock_CS-programmingBlock_CS-AA" + STR(COUNT_programmingBlock_CS) + "AA\n";
                HTVM_Append(programmingBlock_CS, Chr(10) + holdTempDataProgrammingBlockThenPutInArr);
            ...
            else if (Trim(StrLower(A_LoopField174)) == StrLower(keyWordCodeInTheTranspiledLangEndJAVA)) {
                COUNT_programmingBlock_JAVA++;
                holdTempDataProgrammingBlockThenPutInArr = StringTrimRight(holdTempDataProgrammingBlockThenPutInArr, 1);
                programmingBlocksTemp += "programmingBlock_JAVA-programmingBlock_JAVA-AA" + STR(COUNT_programmingBlock_JAVA) + "AA\n";
                HTVM_Append(programmingBlock_JAVA, Chr(10) + holdTempDataProgrammingBlockThenPutInArr);
            ...
            else if (Trim(StrLower(A_LoopField174)) == StrLower(keyWordCodeInTheTranspiledLangEndKT)) {
                COUNT_programmingBlock_KT++;
                holdTempDataProgrammingBlockThenPutInArr = StringTrimRight(holdTempDataProgrammingBlockThenPutInArr, 1);
                programmingBlocksTemp += "programmingBlock_KT-programmingBlock_KT-AA" + STR(COUNT_programmingBlock_KT) + "AA\n";
                HTVM_Append(programmingBlock_KT, Chr(10) + holdTempDataProgrammingBlockThenPutInArr);
            ...
            else if (Trim(StrLower(A_LoopField174)) == StrLower(keyWordCodeInTheTranspiledLangEndRB)) {
                COUNT_programmingBlock_RB++;
                holdTempDataProgrammingBlockThenPutInArr = StringTrimRight(holdTempDataProgrammingBlockThenPutInArr, 1);
                programmingBlocksTemp += "programmingBlock_RB-programmingBlock_RB-AA" + STR(COUNT_programmingBlock_RB) + "AA\n";
                HTVM_Append(programmingBlock_RB, Chr(10) + holdTempDataProgrammingBlockThenPutInArr);
            ...
            else if (Trim(StrLower(A_LoopField174)) == StrLower(keyWordCodeInTheTranspiledLangEndNIM)) {
                COUNT_programmingBlock_NIM++;
                holdTempDataProgrammingBlockThenPutInArr = StringTrimRight(holdTempDataProgrammingBlockThenPutInArr, 1);
                programmingBlocksTemp += "programmingBlock_NIM-programmingBlock_NIM-AA" + STR(COUNT_programmingBlock_NIM) + "AA\n";
                HTVM_Append(programmingBlock_NIM, Chr(10) + holdTempDataProgrammingBlockThenPutInArr);
            ...
            else if (Trim(StrLower(A_LoopField174)) == StrLower(keyWordCodeInTheTranspiledLangEndAHK)) {
                COUNT_programmingBlock_AHK++;
                holdTempDataProgrammingBlockThenPutInArr = StringTrimRight(holdTempDataProgrammingBlockThenPutInArr, 1);
                programmingBlocksTemp += "programmingBlock_AHK-programmingBlock_AHK-AA" + STR(COUNT_programmingBlock_AHK) + "AA\n";
                HTVM_Append(programmingBlock_AHK, Chr(10) + holdTempDataProgrammingBlockThenPutInArr);
            ...
            else if (Trim(StrLower(A_LoopField174)) == StrLower(keyWordCodeInTheTranspiledLangEndSWIFT)) {
                COUNT_programmingBlock_SWIFT++;
                holdTempDataProgrammingBlockThenPutInArr = StringTrimRight(holdTempDataProgrammingBlockThenPutInArr, 1);
                programmingBlocksTemp += "programmingBlock_SWIFT-programmingBlock_SWIFT-AA" + STR(COUNT_programmingBlock_SWIFT) + "AA\n";
                HTVM_Append(programmingBlock_SWIFT, Chr(10) + holdTempDataProgrammingBlockThenPutInArr);
            ...
            else if (Trim(StrLower(A_LoopField174)) == StrLower(keyWordCodeInTheTranspiledLangEndDART)) {
                COUNT_programmingBlock_DART++;
                holdTempDataProgrammingBlockThenPutInArr = StringTrimRight(holdTempDataProgrammingBlockThenPutInArr, 1);
                programmingBlocksTemp += "programmingBlock_DART-programmingBlock_DART-AA" + STR(COUNT_programmingBlock_DART) + "AA\n";
                HTVM_Append(programmingBlock_DART, Chr(10) + holdTempDataProgrammingBlockThenPutInArr);
            ...
            else if (Trim(StrLower(A_LoopField174)) == StrLower(keyWordCodeInTheTranspiledLangEndTS)) {
                COUNT_programmingBlock_TS++;
                holdTempDataProgrammingBlockThenPutInArr = StringTrimRight(holdTempDataProgrammingBlockThenPutInArr, 1);
                programmingBlocksTemp += "programmingBlock_TS-programmingBlock_TS-AA" + STR(COUNT_programmingBlock_TS) + "AA\n";
                HTVM_Append(programmingBlock_TS, Chr(10) + holdTempDataProgrammingBlockThenPutInArr);
            ...
            else if (Trim(StrLower(A_LoopField174)) == StrLower(keyWordCodeInTheTranspiledLangEndGROOVY)) {
                COUNT_programmingBlock_GROOVY++;
                holdTempDataProgrammingBlockThenPutInArr = StringTrimRight(holdTempDataProgrammingBlockThenPutInArr, 1);
                programmingBlocksTemp += "programmingBlock_GROOVY-programmingBlock_GROOVY-AA" + STR(COUNT_programmingBlock_GROOVY) + "AA\n";
                HTVM_Append(programmingBlock_GROOVY, Chr(10) + holdTempDataProgrammingBlockThenPutInArr);
            ...
            else if (Trim(StrLower(A_LoopField174)) == StrLower(keyWordCodeInTheTranspiledLangEndHTVM)) {
                COUNT_programmingBlock_HTVM++;
                holdTempDataProgrammingBlockThenPutInArr = StringTrimRight(holdTempDataProgrammingBlockThenPutInArr, 1);
                programmingBlocksTemp += "programmingBlock_HTVM-programmingBlock_HTVM-AA" + STR(COUNT_programmingBlock_HTVM) + "AA\n";
                HTVM_Append(programmingBlock_HTVM, Chr(10) + holdTempDataProgrammingBlockThenPutInArr);
            ...
            else if (Trim(StrLower(A_LoopField174)) == StrLower(keyWordCodeInHTVMend)) {
                //;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;
                //;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;
                COUNT_programmingBlock_HTVMsyntax++;
                holdTempDataProgrammingBlockThenPutInArr = StringTrimRight(holdTempDataProgrammingBlockThenPutInArr, 1);
                ...
                holdTempDataProgrammingBlockThenPutInArr += A_LoopField174 + Chr(10);
            }
            else if (Trim(StrLower(A_LoopField174)) == StrLower(keyWordCodeInTheTranspiledLangStart)) {
                holdTempDataProgrammingBlockThenPutInArr = "";
                inProgarmmingBlock = 1;
            }
            else if (Trim(StrLower(A_LoopField174)) == StrLower(keyWordCodeInTheTranspiledLangStartCPP)) {
                holdTempDataProgrammingBlockThenPutInArr = "";
                inProgarmmingBlock = 1;
            }
            else if (Trim(StrLower(A_LoopField174)) == StrLower(keyWordCodeInTheTranspiledLangStartPY)) {
                holdTempDataProgrammingBlockThenPutInArr = "";
                inProgarmmingBlock = 1;
            }
            else if (Trim(StrLower(A_LoopField174)) == StrLower(keyWordCodeInTheTranspiledLangStartJS)) {
                holdTempDataProgrammingBlockThenPutInArr = "";
                inProgarmmingBlock = 1;
            }
            else if (Trim(StrLower(A_LoopField174)) == StrLower(keyWordCodeInTheTranspiledLangStartGO)) {
                holdTempDataProgrammingBlockThenPutInArr = "";
                inProgarmmingBlock = 1;
            }
            else if (Trim(StrLower(A_LoopField174)) == StrLower(keyWordCodeInTheTranspiledLangStartLUA)) {
                holdTempDataProgrammingBlockThenPutInArr = "";
                inProgarmmingBlock = 1;
            }
            else if (Trim(StrLower(A_LoopField174)) == StrLower(keyWordCodeInTheTranspiledLangStartCS)) {
                holdTempDataProgrammingBlockThenPutInArr = "";
                inProgarmmingBlock = 1;
            }
            else if (Trim(StrLower(A_LoopField174)) == StrLower(keyWordCodeInTheTranspiledLangStartJAVA)) {
                holdTempDataProgrammingBlockThenPutInArr = "";
                inProgarmmingBlock = 1;
            }
            else if (Trim(StrLower(A_LoopField174)) == StrLower(keyWordCodeInTheTranspiledLangStartKT)) {
                holdTempDataProgrammingBlockThenPutInArr = "";
                inProgarmmingBlock = 1;
            }
            else if (Trim(StrLower(A_LoopField174)) == StrLower(keyWordCodeInTheTranspiledLangStartRB)) {
                holdTempDataProgrammingBlockThenPutInArr = "";
                inProgarmmingBlock = 1;
            }
            else if (Trim(StrLower(A_LoopField174)) == StrLower(keyWordCodeInTheTranspiledLangStartNIM)) {
                holdTempDataProgrammingBlockThenPutInArr = "";
                inProgarmmingBlock = 1;
            }
            else if (Trim(StrLower(A_LoopField174)) == StrLower(keyWordCodeInTheTranspiledLangStartAHK)) {
                holdTempDataProgrammingBlockThenPutInArr = "";
                inProgarmmingBlock = 1;
            }
            else if (Trim(StrLower(A_LoopField174)) == StrLower(keyWordCodeInTheTranspiledLangStartSWIFT)) {
                holdTempDataProgrammingBlockThenPutInArr = "";
                inProgarmmingBlock = 1;
            }
            else if (Trim(StrLower(A_LoopField174)) == StrLower(keyWordCodeInTheTranspiledLangStartDART)) {
                holdTempDataProgrammingBlockThenPutInArr = "";
                inProgarmmingBlock = 1;
            }
            else if (Trim(StrLower(A_LoopField174)) == StrLower(keyWordCodeInTheTranspiledLangStartTS)) {
                holdTempDataProgrammingBlockThenPutInArr = "";
                inProgarmmingBlock = 1;
            }
            else if (Trim(StrLower(A_LoopField174)) == StrLower(keyWordCodeInTheTranspiledLangStartGROOVY)) {
                holdTempDataProgrammingBlockThenPutInArr = "";
                inProgarmmingBlock = 1;
            }
            else if (Trim(StrLower(A_LoopField174)) == StrLower(keyWordCodeInTheTranspiledLangStartHTVM)) {
                holdTempDataProgrammingBlockThenPutInArr = "";
                inProgarmmingBlock = 1;
            }
            else if (Trim(StrLower(A_LoopField174)) == StrLower(keyWordCodeInHTVMstart)) {
                //;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;
                //;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;
                holdTempDataProgrammingBlockThenPutInArr = "";
                inProgarmmingBlock = 1;
                ...
                programmingBlocksTemp += A_LoopField174 + Chr(10);
            }
        }
        code = StringTrimRight(programmingBlocksTemp, 1);
        //;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;; ----------------
    ...
    if (isNotHTVMfile == 0) {
        ...
        for (size_t A_Index175 = 0; A_Index175 < items175.size() + 0; A_Index175++) {
            ...
            if (SubStr(Trim(A_LoopField175), 1, StrLen(keyWordComment)) == keyWordComment) {
                commentsBugFixHold = StringTrimLeft(Trim(A_LoopField175), StrLen(keyWordComment));
                std::vector<std::string> items176 = LoopParseFunc(A_LoopField175);
                for (size_t A_Index176 = 0; A_Index176 < items176.size() + 0; A_Index176++) {
                    std::string A_LoopField176 = items176[A_Index176 - 0];
                    if (InStr(A_LoopField176, " ") || InStr(A_LoopField176, Chr(9))) {
                        commentsBugFixHold1 = A_LoopField176;
                    } else {
                        break;
                    }
                }
                ...
                commentsBugFixOut += A_LoopField175 + Chr(10);
            }
        }
        ...
        for (size_t A_Index178 = 0; A_Index178 < items178.size() + 0; A_Index178++) {
            ...
            HTVM_Append(getAllCharForTheFurtureSoIcanAddEscapeChar, A_LoopField178);
        }
        ...
        for (size_t A_Index179 = 0; A_Index179 < items179.size() + 0; A_Index179++) {
            ...
            if (A_LoopField179 == keyWordEscpaeChar && getAllCharForTheFurtureSoIcanAddEscapeChar[A_Index179 + 1] == Chr(34)) {
                ...
                if (fixOutFixDoubleQuotesInsideDoubleQuotesFIXok != 1) {
                    OutFixDoubleQuotesInsideDoubleQuotes += A_LoopField179;
                } else {
        ...
        if (keyWordEscpaeChar == Chr(92) && langToConvertTo != langFileExtension_2) {
            ...
            for (size_t A_Index180 = 0; A_Index180 < items180.size() + 0; A_Index180++) {
                ...
                if (areWEinSome34sNum == 1) {
                    if (A_LoopField180 != Chr(34)) {
                        if (A_LoopField180 == keyWordEscpaeChar && langToConvertTo != langFileExtension_2) {
            ...
            for (size_t A_Index181 = 0; A_Index181 < items181.size() + 0; A_Index181++) {
                ...
                if (areWEinSome34sNum == 1) {
                    if (A_LoopField181 != Chr(34)) {
                        if (A_LoopField181 == keyWordEscpaeChar && keyWordEscpaeChar == getAllCharForTheFurtureSoIcanAddEscapeChar[A_Index181 + 1] && langToConvertTo != langFileExtension_2) {
                        ...
                        else if (A_LoopField181 == keyWordEscpaeChar && langToConvertTo != langFileExtension_2) {
        ...
        for (int A_Index182 = 0; A_Index182 < theIdNumOfThe34 + 0; A_Index182++) {
            theIdNumOfThe34theVar[A_Index182] = theIdNumOfThe34theVar[A_Index182] + Chr(34);
        }
        HTVM_Append(theIdNumOfThe34theVar, Chr(34));
        if (langToConvertTo == "dart" || langToConvertTo == "groovy") {
            for (int A_Index183 = 0; A_Index183 < HTVM_Size(theIdNumOfThe34theVar) + 0; A_Index183++) {
                theIdNumOfThe34theVar[A_Index183] = StrReplace(theIdNumOfThe34theVar[A_Index183], "$", Chr(92) + "$");
            }
        }
        ...
        for (size_t A_Index184 = 0; A_Index184 < items184.size() + 0; A_Index184++) {
            ...
            if (InStr(A_LoopField184, " " + keyWordComment) != false && SubStr(Trim(A_LoopField184), 1, StrLen(keyWordComment)) != keyWordComment) {
                posForRemoveCommentsOnTheEndOfTheLine = 0;
                slidingWinFixCommentsRmI = 0;
                slidingWinFixCommentsRm = slidingWinFixCommentsRmFUNC(A_LoopField184);
                for (int A_Index185 = 0; A_Index185 < HTVM_Size(slidingWinFixCommentsRm) + 0; A_Index185++) {
                    slidingWinFixCommentsRmI = A_Index185;
                    slidingWinFixCommentsRmHold = "";
                    for (int A_Index186 = 0; A_Index186 < StrLen(" " + keyWordComment) + 0; A_Index186++) {
                        slidingWinFixCommentsRmHold += slidingWinFixCommentsRm[slidingWinFixCommentsRmI + A_Index186];
                    }
                    if (Trim(slidingWinFixCommentsRmHold) == keyWordComment) {
                        posForRemoveCommentsOnTheEndOfTheLine = A_Index185;
                        break;
                    }
                }
                if (posForRemoveCommentsOnTheEndOfTheLine > 0) {
                    // Keep only the part before the comment
                    newStringOutCode += SubStr(A_LoopField184, 1, posForRemoveCommentsOnTheEndOfTheLine) + Chr(10);
                } else {
                    // Keep the whole line as it is
                    newStringOutCode += A_LoopField184 + Chr(10);
                }
            } else {
                newStringOutCode += A_LoopField184 + Chr(10);
            }
        ...
        if (langToConvertTo != langFileExtension_2) {
            ...
            for (size_t A_Index187 = 0; A_Index187 < items187.size() + 0; A_Index187++) {
                ...
                if (SubStr(Trim(A_LoopField187), 1, 1) != ";") {
                    semiColonForOneLinerOut += StrReplace(A_LoopField187, ";", Chr(10)) + Chr(10);
                } else {
                    semiColonForOneLinerOut += A_LoopField187 + Chr(10);
                }
            }
        ...
        if (useCurlyBraces == "on") {
            ...
            for (size_t A_Index188 = 0; A_Index188 < items188.size() + 0; A_Index188++) {
                ...
                str1 = "";
                if (InStr(A_LoopField188, keyWordCurlyBraceOpen) || InStr(A_LoopField188, keyWordCurlyBraceClose)) {
                    if (SubStr(Trim(A_LoopField188), 1, StrLen(Trim(keyWordComment))) == Trim(keyWordComment) && isLineAconstruct(Trim(A_LoopField188)) == false) {
                        fixSomeCurlyBraceForExratFlexability += A_LoopField188 + Chr(10);
                    } else {
                        if (InStr(A_LoopField188, Trim(keyWordArrayDefinition) + " " + keyWordAssign + " ") || InStr(A_LoopField188, Trim(keyWordArrayOfIntegersDefinition) + " " + keyWordAssign + " ") || InStr(A_LoopField188, Trim(keyWordArrayOfStringsDefinition) + " " + keyWordAssign + " ") || InStr(A_LoopField188, Trim(keyWordArrayOfFloatingPointNumbersDefinition) + " " + keyWordAssign + " ") || InStr(A_LoopField188, Trim(keyWordArrayOfBooleansDefinition) + " " + keyWordAssign + " ") && useCurlyBracesSyntaxForArrayDef == "on") {
                            fixSomeCurlyBraceForExratFlexability += A_LoopField188 + Chr(10);
                        } else {
                            str1 = StrReplace(A_LoopField188, keyWordCurlyBraceOpen, "{");
            ...
            for (size_t A_Index189 = 0; A_Index189 < items189.size() + 0; A_Index189++) {
                ...
                fixSomeCurlyBraceForExratFlexabilityTriHELP += Trim(A_LoopField189) + Chr(10);
            }
            ...
            if (keyWordCurlyBraceOpen != "{") {
                ...
                for (size_t A_Index190 = 0; A_Index190 < items190.size() + 0; A_Index190++) {
                    ...
                    htCodeOutFixINT2 = 0;
                    if (isLineAconstruct(Trim(A_LoopField190)) == true || Trim(A_LoopField190) == keyWordCurlyBraceOpen) {
                        htCodeOutFixINT2 = 1;
                    }
                    htCodeOutFixINT = 0;
                    ...
                    htCodeOutFixINT--;
                    std::vector<std::string> items192 = LoopParseFunc(A_LoopField190, " ");
                    for (size_t A_Index192 = 0; A_Index192 < items192.size() + 0; A_Index192++) {
                        std::string A_LoopField192 = items192[A_Index192 - 0];
                        if (htCodeOutFixINT == A_Index192 && htCodeOutFixINT2 == 1) {
                            htCodeOutFixINT2 = 0;
                            htCodeOutFixEnd += HTVMmatchStrRrplace(A_LoopField192, keyWordCurlyBraceOpen, "{") + " ";
                        } else {
            ...
            if (keyWordCurlyBraceClose != "}") {
                ...
                for (size_t A_Index193 = 0; A_Index193 < items193.size() + 0; A_Index193++) {
                    ...
                    htCodeOutFixINT2 = 0;
                    if (isLineAconstruct(Trim(A_LoopField193)) == true || Trim(A_LoopField193) == keyWordCurlyBraceClose) {
                        htCodeOutFixINT2 = 1;
                    }
                    ...
                    for (size_t A_Index194 = 0; A_Index194 < items194.size() + 0; A_Index194++) {
                        ...
                        if (htCodeOutFixINT2 == 1 || Trim(A_LoopField194) == keyWordCurlyBraceClose) {
                            htCodeOutFixINT2 = 0;
                            htCodeOutFixEnd += HTVMmatchStrRrplace(A_LoopField194, keyWordCurlyBraceClose, "}") + " ";
                        } else {
        ...
        if (useCurlyBraces == "on") {
            ...
            for (size_t A_Index195 = 0; A_Index195 < items195.size() + 0; A_Index195++) {
                ...
                HTVM_Append(addNoCurlyFixWhenNextLineAfterConstructLookAhead, Trim(A_LoopField195));
            }
            ...
            for (size_t A_Index196 = 0; A_Index196 < items196.size() + 0; A_Index196++) {
                ...
                if (isLineAconstruct(Trim(A_LoopField196)) && addNoCurlyFixWhenNextLineAfterConstructLookAhead[A_Index196 + 1] != "{") {
                    int5 = 1;
                    addNoCurlyFixWhenNextLineAfterConstructOut += A_LoopField196 + Chr(10) + "{" + Chr(10);
                } else {
                    if (int5 == 1) {
                        int5 = 0;
                        addNoCurlyFixWhenNextLineAfterConstructOut += A_LoopField196 + Chr(10) + "}" + Chr(10);
                    } else {
                        addNoCurlyFixWhenNextLineAfterConstructOut += A_LoopField196 + Chr(10);
                    }
                }
            }
        ...
        if (useEnd == "on") {
            ...
            for (size_t A_Index197 = 0; A_Index197 < items197.size() + 0; A_Index197++) {
                ...
                if (StrLower(Trim(A_LoopField197)) == StrLower(keyWordEnd)) {
                    htCodeOutFixEnd += "}\n";
                }
                else if (SubStr(StrLower(Trim(A_LoopField197)), 1, StrLen(StrLower(keyWordIF))) == StrLower(keyWordIF) && isLineAconstruct(Trim(A_LoopField197)) == true) {
                    htCodeOutFixEnd += A_LoopField197 + "\n{\n";
                }
                else if (SubStr(StrLower(Trim(A_LoopField197)), 1, StrLen(StrLower(keyWordWhileLoop))) == StrLower(keyWordWhileLoop) && isLineAconstruct(Trim(A_LoopField197)) == true) {
                    htCodeOutFixEnd += A_LoopField197 + "\n{\n";
                }
                else if (SubStr(StrLower(Trim(A_LoopField197)), 1, StrLen(StrLower(keyWordElseIf))) == StrLower(keyWordElseIf) && isLineAconstruct(Trim(A_LoopField197)) == true) {
                    htCodeOutFixEnd += "\n}\n" + A_LoopField197 + "\n{\n";
                }
                else if (SubStr(StrLower(Trim(A_LoopField197)), 1, StrLen(StrLower(keyWordElse))) == StrLower(keyWordElse) && isLineAconstruct(Trim(A_LoopField197)) == true) {
                    htCodeOutFixEnd += "\n}\n" + A_LoopField197 + "\n{\n";
                }
                else if (SubStr(StrLower(Trim(A_LoopField197)), 1, StrLen(StrLower(keyWordCatch))) == StrLower(keyWordCatch) && isLineAconstruct(Trim(A_LoopField197)) == true) {
                    htCodeOutFixEnd += "\n}\n" + A_LoopField197 + "\n{\n";
                }
                else if (SubStr(StrLower(Trim(A_LoopField197)), 1, StrLen(StrLower(keyWordFinally))) == StrLower(keyWordFinally) && isLineAconstruct(Trim(A_LoopField197)) == true) {
                    htCodeOutFixEnd += "\n}\n" + A_LoopField197 + "\n{\n";
                }
                else if (isLineAconstruct(Trim(A_LoopField197)) == true) {
                    htCodeOutFixEnd += A_LoopField197 + "\n{\n";
                } else {
        ...
        for (int A_Index200 = 0; A_Index200 < 2 + 0; A_Index200++) {
            ...
            for (size_t A_Index201 = 0; A_Index201 < items201.size() + 0; A_Index201++) {
                ...
                if (InStr(A_LoopField201, Trim(keyWordArrayDefinition) + " " + keyWordAssign + " ") || InStr(A_LoopField201, Trim(keyWordArrayOfIntegersDefinition) + " " + keyWordAssign + " ") || InStr(A_LoopField201, Trim(keyWordArrayOfStringsDefinition) + " " + keyWordAssign + " ") || InStr(A_LoopField201, Trim(keyWordArrayOfFloatingPointNumbersDefinition) + " " + keyWordAssign + " ") || InStr(A_LoopField201, Trim(keyWordArrayOfBooleansDefinition) + " " + keyWordAssign + " ") && useCurlyBracesSyntaxForArrayDef == "on") {
                    if (SubStr(StrLower(A_LoopField201), 1, StrLen(StrLower(keyWordFunc))) == StrLower(keyWordFunc) || SubStr(StrLower(A_LoopField201), 1, StrLen(StrLower(keyWordAsync) + StrLower(keyWordFunc))) == StrLower(keyWordAsync) + StrLower(keyWordFunc)) {
                        if (InStr(Trim(A_LoopField201), "{") && Trim(A_LoopField201) != "{" && SubStr(Trim(A_LoopField201), 1, StrLen(Trim(keyWordComment))) != Trim(keyWordComment)) {
                            outCodeFixBraces += Trim(StrReplace(Trim(A_LoopField201), "{", "")) + "\n{\n";
                        }
                        else if (InStr(Trim(A_LoopField201), "}") && Trim(A_LoopField201) != "}" && SubStr(Trim(A_LoopField201), 1, StrLen(Trim(keyWordComment))) != Trim(keyWordComment)) {
                            outCodeFixBraces += "}\n" + Trim(StrReplace(Trim(A_LoopField201), "}", "")) + Chr(10);
                        } else {
                            outCodeFixBraces += Trim(A_LoopField201) + Chr(10);
                        }
                    } else {
                        outCodeFixBraces += Trim(A_LoopField201) + Chr(10);
                    }
        ...
        if (usePythonicColonSyntax == "on" || useCurlyBraces == "off" && useEnd == "off") {
            ...
            for (size_t A_Index202 = 0; A_Index202 < items202.size() + 0; A_Index202++) {
                ...
                if (isLineAconstruct(Trim(A_LoopField202)) == true) {
                    htCodeOutFixEnd += StringTrimRight(A_LoopField202, 1) + Chr(10);
                } else {
        ...
        if (isNotHTVMfile2 == 0) {
            ...
            for (size_t A_Index203 = 0; A_Index203 < items203.size() + 0; A_Index203++) {
                ...
                if (StrLower(Trim(A_LoopField203)) == StrLower(keyWordMainLabel)) {
                    didWeUseMainLabel = 1;
                    if (langToConvertTo == langFileExtension_2) {
                    ...
                    codeOutFixAndAddMainFunc += A_LoopField203 + Chr(10);
                }
            }
        ...
        for (size_t A_Index204 = 0; A_Index204 < items204.size() + 0; A_Index204++) {
            ...
            HTVM_Append(lookIntoTheNextLineForFuncWhitNoKeyWord, A_LoopField204);
        }
        ...
        for (size_t A_Index205 = 0; A_Index205 < items205.size() + 0; A_Index205++) {
            ...
            lineDone = 0;
            if (SubStr(A_LoopField205, 1, StrLen(keyWordComment)) == keyWordComment) {
                lineDone = 1;
                str1 = StringTrimLeft(A_LoopField205, StrLen(keyWordComment));
                if (langToConvertTo == langFileExtension_2) {
            ...
            else if (InStr(A_LoopField205, "guiInit(") || InStr(A_LoopField205, "guiAdd(") || InStr(A_LoopField205, "guiAddElement(") || InStr(A_LoopField205, "guiControl(") || InStr(A_LoopField205, "HTVM_init(") || InStr(A_LoopField205, "HTVM_register(") || InStr(A_LoopField205, "HTVM_custom_port(") && lineDone == 0) {
                lineDone = 1;
                if (InStr(A_LoopField205, "HTVM_init(")) {
                    htCode += Trim(A_LoopField205) + Chr(10);
                } else {
                    htCode += Trim(expressionParserTranspiler(Trim(A_LoopField205))) + Chr(10);
                }
            }
            else if (Trim(A_LoopField205) == keyWordCommentOpenMultiLine) {
                lineDone = 1;
                if (langToConvertTo == langFileExtension_2) {
            ...
            else if (Trim(A_LoopField205) == keyWordCommentCloseMultiLine) {
                lineDone = 1;
                if (langToConvertTo == langFileExtension_2) {
                ...
                lineDone = 1;
                str1 = StrSplit(A_LoopField205, " ", 2);
            ...
            else if (SubStr(StrLower(Trim(A_LoopField205)), 1, StrLen(StrLower(keyWordStruct))) == StrLower(keyWordStruct)) {
                lineDone = 1;
                str0 = Trim(StringTrimLeft(A_LoopField205, StrLen(keyWordStruct)));
                if (langToConvertTo == langFileExtension_2) {
                    htCode += Trim(keyWordStruct_2) + " " + str0 + Chr(10);
                }
            }
            else if (SubStr(StrLower(Trim(A_LoopField205)), 1, StrLen(StrLower(keyWordAlliance))) == StrLower(keyWordAlliance)) {
                lineDone = 1;
                str0 = Trim(StringTrimLeft(A_LoopField205, StrLen(keyWordAlliance)));
                if (langToConvertTo == langFileExtension_2) {
                    htCode += Trim(keyWordAlliance_2) + " " + str0 + Chr(10);
                }
            }
            else if (SubStr(StrLower(Trim(A_LoopField205)), 1, StrLen(StrLower(keyWordCrew))) == StrLower(keyWordCrew)) {
                lineDone = 1;
                str0 = Trim(StringTrimLeft(A_LoopField205, StrLen(keyWordCrew)));
                if (langToConvertTo == langFileExtension_2) {
                    htCode += Trim(keyWordCrew_2) + " " + str0 + Chr(10);
                }
            }
            else if (InStr(A_LoopField205, "[end][end][end][end][end] ")) {
                lineDone = 1;
                if (langToConvertTo == langFileExtension_2) {
                    htCode += "}" + Chr(10);
                }
            }
            else if (SubStr(StrLower(Trim(A_LoopField205)), 1, StrLen(StrLower(keyWordProp))) == StrLower(keyWordProp)) {
                lineDone = 1;
                str0 = Trim(StringTrimLeft(A_LoopField205, StrLen(keyWordProp)));
            ...
            else if (SubStr(StrLower(Trim(A_LoopField205)), 1, StrLen(StrLower(keyWordProc))) == StrLower(keyWordProc)) {
                lineDone = 1;
                str0 = Trim(StringTrimLeft(A_LoopField205, StrLen(keyWordProc)));
                str1 = Trim(StrSplit(str0, "-----------------------HTVM-OSP-HTVM-OBJ--------------", 1));
                str20 = Trim(StrSplit(str0, "-----------------------HTVM-OSP-HTVM-OBJ--------------", 2));
            ...
            else if (SubStr(StrLower(Trim(A_LoopField205)), 1, StrLen(StrLower(keyWordIF))) == StrLower(keyWordIF) && SubStr(StrLower(Trim(A_LoopField205)), 1, StrLen(StrLower(keyWordElseIf))) != StrLower(keyWordElseIf)) {
                lineDone = 1;
                str1 = Trim(StringTrimLeft(A_LoopField205, StrLen(keyWordIF)));
            ...
            else if (SubStr(StrLower(Trim(A_LoopField205)), 1, StrLen(StrLower(keyWordElseIf))) == StrLower(keyWordElseIf) && SubStr(StrLower(Trim(A_LoopField205)), 1, StrLen(StrLower(keyWordIF))) != StrLower(keyWordIF)) {
                lineDone = 1;
                str1 = Trim(StringTrimLeft(A_LoopField205, StrLen(keyWordElseIf)));
                str1 = Trim(expressionParserTranspiler("(" + str1 + ")"));
                str1 = StringTrimRight(str1, 1);
                str1 = StringTrimLeft(str1, 1);
                if (langToConvertTo == langFileExtension_2) {
            ...
            else if (StrLower(Trim(A_LoopField205)) == StrLower(keyWordElse) || StrLower(Trim(A_LoopField205)) == StrLower(keyWordElse + ":")) {
                lineDone = 1;
                if (langToConvertTo == langFileExtension_2) {
                    if (usePythonicColonSyntax_2 == "on") {
            ...
            else if (SubStr(StrLower(Trim(A_LoopField205)), 1, StrLen(StrLower(keyWordWhileLoop))) == StrLower(keyWordWhileLoop)) {
                lineDone = 1;
                str1 = Trim(StringTrimLeft(A_LoopField205, StrLen(keyWordWhileLoop)));
                str1 = Trim(expressionParserTranspiler("(" + str1 + ")"));
                str1 = StringTrimRight(str1, 1);
                str1 = StringTrimLeft(str1, 1);
                if (langToConvertTo == langFileExtension_2) {
            ...
            else if (StrLower(Trim(A_LoopField205)) == StrLower(keyWordTry) || StrLower(Trim(A_LoopField205)) == StrLower(keyWordTry + ":")) {
                lineDone = 1;
                if (langToConvertTo == langFileExtension_2) {
                    if (usePythonicColonSyntax_2 == "on") {
                        htCode += keyWordTry_2 + ":\n";
            ...
            else if (SubStr(StrLower(Trim(A_LoopField205)), 1, StrLen(StrLower(keyWordCatch))) == StrLower(keyWordCatch)) {
                lineDone = 1;
                str1 = Trim(StrReplace(StrReplace(StringTrimLeft(A_LoopField205, StrLen(keyWordCatch)), "(", ""), ")", ""));
            ...
            else if (StrLower(Trim(A_LoopField205)) == StrLower(keyWordFinally) || StrLower(Trim(A_LoopField205)) == StrLower(keyWordFinally + ":")) {
                lineDone = 1;
                if (langToConvertTo == langFileExtension_2) {
                    if (usePythonicColonSyntax_2 == "on") {
            ...
            else if (SubStr(StrLower(Trim(A_LoopField205)), 1, StrLen(StrLower(keyWordThrow))) == StrLower(keyWordThrow)) {
                lineDone = 1;
                str1 = Trim(A_LoopField205);
                if (SubStrLastChars(str1, 1) == ";") {
                    str1 = StringTrimRight(str1, 1);
                }
            ...
            else if (StrLower(A_LoopField205) == StrLower(keyWordBreak) || StrLower(A_LoopField205) == StrLower(keyWordBreak + ";")) {
                lineDone = 1;
                if (langToConvertTo == langFileExtension_2) {
                    if (useSemicolon_2 == "on") {
            ...
            else if (StrLower(A_LoopField205) == StrLower(Trim(keyWordReturnStatement)) || StrLower(A_LoopField205) == StrLower(Trim(keyWordReturnStatement) + ";")) {
                lineDone = 1;
                if (langToConvertTo == langFileExtension_2) {
                    if (useSemicolon_2 == "on") {
            ...
            else if (SubStr(Trim(StrLower(A_LoopField205)), 1, StrLen(StrLower(keyWordReturnStatement))) == StrLower(keyWordReturnStatement)) {
                lineDone = 1;
                str1 = Trim(expressionParserTranspiler(Trim(StringTrimLeft(A_LoopField205, StrLen(keyWordReturnStatement)))));
                if (SubStrLastChars(str1, 1) == ";") {
                    str1 = StringTrimRight(str1, 1);
                }
                if (langToConvertTo == langFileExtension_2) {
            ...
            else if (StrLower(A_LoopField205) == StrLower(keyWordContinue) || StrLower(A_LoopField205) == StrLower(keyWordContinue + ";")) {
                lineDone = 1;
                if (langToConvertTo == langFileExtension_2) {
                    if (useSemicolon_2 == "on") {
            ...
            else if (StrLower(A_LoopField205) == "::" + keyWordContinue || StrLower(A_LoopField205) == "::" + keyWordContinue + ";") {
                lineDone = 1;
                if (langToConvertTo == langFileExtension_2) {
                    htCode += "::" + keyWordContinue_2 + Chr(10);
                }
            ...
            else if (StrLower(A_LoopField205) == StrLower(keyWordLoopInfinite) || StrLower(A_LoopField205) == StrLower(keyWordLoopInfinite + ":")) {
                // infinity loops
            ...
            else if (SubStr(Trim(StrLower(A_LoopField205)), 1, StrLen(StrLower(keyWordLoop))) == StrLower(keyWordLoop) && SubStr(Trim(StrLower(A_LoopField205)), 1, StrLen(StrLower(keyWordLoop)) + 2) != StrLower(keyWordLoop) + "% " && SubStr(Trim(StrLower(A_LoopField205)), 1, StrLen(StrLower(keyWordLoopParse))) != StrLower(keyWordLoopParse)) {
                out2 = StringTrimLeft(A_LoopField205, StrLen(keyWordLoop));
            ...
            else if (SubStr(Trim(StrLower(A_LoopField205)), 1, StrLen(StrLower(keyWordLoop)) + 2) == StrLower(keyWordLoop) + "% ") {
                howMany_fixAindexInGoUnused++;
                out2 = StringTrimLeft(A_LoopField205, StrLen(keyWordLoop) + 2);
                out2 = StrReplace(out2, ":", "");
                //MsgBox % out2
                //MsgBox, % out2
                out2 = Trim(out2);
            ...
            else if (SubStr(StrLower(A_LoopField205), 1, StrLen(StrLower(keyWordLoopParse))) == StrLower(keyWordLoopParse)) {
                howMany_fixAindexInGoUnused++;
                var1 = StringTrimLeft(A_LoopField205, StrLen(keyWordLoopParse));
                out2 = StrReplace(out2, ":", "");
                lineDone = 1;
            ...
            else if (SubStrLastChars(Trim(A_LoopField205), StrLen(keyWordInc)) == keyWordInc || SubStrLastChars(Trim(A_LoopField205), StrLen(keyWordInc + ";")) == keyWordInc + ";" && lineDone == 0) {
                if (useSemicolon == "on" || SubStrLastChars(Trim(A_LoopField205), 1) == ";") {
                    str1 = Trim(StringTrimRight(Trim(A_LoopField205), StrLen(keyWordInc + ";")));
                } else {
                    str1 = Trim(StringTrimRight(Trim(A_LoopField205), StrLen(keyWordInc)));
                }
                str1 = Trim(expressionParserTranspiler(str1));
            ...
            else if (SubStrLastChars(Trim(A_LoopField205), StrLen(keyWordDec)) == keyWordDec || SubStrLastChars(Trim(A_LoopField205), StrLen(keyWordDec + ";")) == keyWordDec + ";" && lineDone == 0) {
                if (useSemicolon == "on" || SubStrLastChars(Trim(A_LoopField205), 1) == ";") {
                    str1 = Trim(StringTrimRight(Trim(A_LoopField205), StrLen(keyWordDec + ";")));
                } else {
                    str1 = Trim(StringTrimRight(Trim(A_LoopField205), StrLen(keyWordDec)));
                }
                str1 = Trim(expressionParserTranspiler(str1));
            ...
            else if (SubStr(StrLower(Trim(A_LoopField205)), 1, StrLen(StrLower(keyWordInclude + " "))) == StrLower(keyWordInclude + " ") && langToConvertTo == langFileExtension_2) {
                htCode += keyWordInclude_2 + " " + Trim(StringTrimLeft(Trim(A_LoopField205), StrLen(StrLower(keyWordInclude) + " "))) + Chr(10);
            }
            else if (StrLower(A_LoopField205) == StrLower(keyWordGlobal) || StrLower(A_LoopField205) == StrLower(keyWordGlobal + ";")) {
                if (langToConvertTo == "ahk") {
                    htCode += "global\n";
                }
            }
            else if (SubStr(StrLower(A_LoopField205), 1, StrLen(StrLower(keyWordAwait))) == StrLower(keyWordAwait)) {
                str1 = StringTrimLeft(A_LoopField205, StrLen(keyWordAwait));
                //MsgBox, % A_LoopField
                lineDone = 1;
            ...
            else if (SubStr(StrLower(A_LoopField205), 1, StrLen(StrLower(keyWordGlobal))) == StrLower(keyWordGlobal)) {
                str1 = StringTrimLeft(A_LoopField205, StrLen(keyWordGlobal));
                //MsgBox, % A_LoopField
            ...
            else if (SubStr(StrLower(A_LoopField205), 1, StrLen(keyWordFunc)) == StrLower(keyWordFunc) || SubStr(StrLower(A_LoopField205), 1, StrLen(StrLower(keyWordAsync) + StrLower(keyWordFunc))) == StrLower(keyWordAsync) + StrLower(keyWordFunc) && InStr(StrLower(A_LoopField205), " main(") == false && lineDone == 0) {
                lineDone = 1;
                if (SubStr(StrLower(A_LoopField205), 1, StrLen(StrLower(keyWordAsync) + StrLower(keyWordFunc))) == StrLower(keyWordAsync) + StrLower(keyWordFunc)) {
                    // is async
                    int1 = 1;
                    str1 = Trim(StringTrimLeft(A_LoopField205, StrLen(StrLower(keyWordAsync + keyWordFunc))));
                } else {
                    // is async
                    int1 = 0;
                    str1 = Trim(StringTrimLeft(A_LoopField205, StrLen(StrLower(keyWordFunc))));
                }
                if (langToConvertTo != langFileExtension_2 && langToConvertTo != "cpp" && langToConvertTo != "go" && langToConvertTo != "cs" && langToConvertTo != "java" && langToConvertTo != "kt" && langToConvertTo != "nim" && langToConvertTo != "swift" && langToConvertTo != "dart" && langToConvertTo != "ts") {
                    if (str1 != "") {
                        str4 = "";
                        std::vector<std::string> items207 = LoopParseFunc(str1, ",");
                        for (size_t A_Index207 = 0; A_Index207 < items207.size() + 0; A_Index207++) {
                            std::string A_LoopField207 = items207[A_Index207 - 0];
                            if (InStr(A_LoopField207, " := ")) {
                                if (InStr(Trim(StrSplit(A_LoopField207, " := ", 1)), " ") != true) {
                                    str4 += "optanal-HTVM-opt-parmsNOtWorking-theirewIs-a-bug-Iusfbudfbuoeshfuisbav=-fwaegs-awedsf-dd-sfgc " + A_LoopField207 + ", ";
                                } else {
                                    str4 += A_LoopField207 + ", ";
                                }
                            }
                            else if (InStr(A_LoopField207, " = ")) {
                                if (InStr(Trim(StrSplit(A_LoopField207, " = ", 1)), " ") != true) {
                                    str4 += "optanal-HTVM-opt-parmsNOtWorking-theirewIs-a-bug-Iusfbudfbuoeshfuisbav=-fwaegs-awedsf-dd-sfgc " + A_LoopField207 + ", ";
                                } else {
                                    str4 += A_LoopField207 + ", ";
                            ...
                            else if (InStr(A_LoopField207, " " + Trim(keyWordAssign) + " ")) {
                                if (InStr(Trim(StrSplit(A_LoopField207, " " + Trim(keyWordAssign) + " ", 1)), " ") != true) {
                                    str4 += "optanal-HTVM-opt-parmsNOtWorking-theirewIs-a-bug-Iusfbudfbuoeshfuisbav=-fwaegs-awedsf-dd-sfgc " + A_LoopField207 + ", ";
                                } else {
                                    str4 += A_LoopField207 + ", ";
                                }
                ...
                if (Trim(str1) == "") {
                    ...
                    for (size_t A_Index208 = 0; A_Index208 < items208.size() + 0; A_Index208++) {
                        ...
                        if (InStr(Trim(A_LoopField208), " ")) {
                            // there is space aka there is a type
                            if (InStr(A_LoopField208, " " + Trim(keyWordAssign) + " ") == false) {
                                if (usePrefixTypeForTypeDefinition == "on") {
                                    // normal
                                    str6 = Trim(A_LoopField208);
                                ...
                                // optanal param
                                if (InStr(Trim(StrSplit(Trim(A_LoopField208), " " + Trim(keyWordAssign) + " ", 1)), " ")) {
                                    // there is a type
                                    if (usePrefixTypeForTypeDefinition == "on") {
                                        // normal
                                        str6 = Trim(StrSplit(Trim(A_LoopField208), " " + Trim(keyWordAssign) + " ", 1));
                                        str9 = Trim(expressionParserTranspiler(Trim(StrSplit(Trim(A_LoopField208), " " + Trim(keyWordAssign) + " ", 2))));
                                        str7 = getTheLastWord(str6);
                                        str8 = getFuncTypeConvert(Trim(StringTrimRight(str6, StrLen(Trim(str7)) + 1)));
                                        //;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;
                                        //;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;
                                    ...
                                    // there is no type
                                    str8 = Trim(StrSplit(Trim(A_LoopField208), " " + Trim(keyWordAssign) + " ", 1));
                                    str9 = Trim(expressionParserTranspiler(Trim(StrSplit(Trim(A_LoopField208), " " + Trim(keyWordAssign) + " ", 2))));
                                    // str10
                                    // skipLeftCuleyForFuncPLS := 1
                                    //;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;
                            ...
                            str5 += Trim(A_LoopField208) + Chr(10);
                        }
                    }
                    //print(str5)
                    //print(str10)
                    ...
                    if (Trim(str5) != "" && Trim(str10) != "") {
                        ...
                        if (langToConvertTo == "lua") {
                            ...
                            for (size_t A_Index211 = 0; A_Index211 < items211.size() + 0; A_Index211++) {
                                ...
                                str14 += Trim(StrSplit(Trim(A_LoopField211), " = ", 1)) + ", ";
                            }
                            ...
                            for (size_t A_Index212 = 0; A_Index212 < items212.size() + 0; A_Index212++) {
                                ...
                                str13 += Trim(StrSplit(Trim(A_LoopField212), " = ", 1)) + " = " + Trim(StrSplit(Trim(A_LoopField212), " = ", 1)) + " or " + Trim(StrSplit(Trim(A_LoopField212), " = ", 2)) + Chr(10);
                            }
                            str12 = str14;
                        }
                        if (langToConvertTo == "go") {
                            ...
                            for (size_t A_Index213 = 0; A_Index213 < items213.size() + 0; A_Index213++) {
                                ...
                                str16 = Trim(StrSplit(Trim(StrSplit(Trim(A_LoopField213), " = ", 1)), " ", 1));
                                str17 = Trim(StrSplit(Trim(A_LoopField213), " = ", 2));
                            ...
                            for (size_t A_Index214 = 0; A_Index214 < items214.size() + 0; A_Index214++) {
                                ...
                                str18 = Trim(StrSplit(Trim(StrSplit(Trim(A_LoopField214), " = ", 1)), " ", 2));
                                // str16 = var name
                                // str18 = var type
                                str13 += "if len(__HTVM_V2_TO_GO_optionalParams__) " + Chr(62) + " " + STR(A_Index214) + Chr(10) + "{" + Chr(10) + str16 + " = __HTVM_V2_TO_GO_optionalParams__[" + STR(A_Index214) + "].(" + str18 + ")" + Chr(10) + "}" + Chr(10);
                            }
                            str12 = str14;
                        }
                        if (langToConvertTo == "java") {
                            ...
                            for (size_t A_Index215 = 0; A_Index215 < items215.size() + 0; A_Index215++) {
                                ...
                                str19 = Trim(A_LoopField215);
                                str20 = "";
                                std::vector<std::string> items216 = LoopParseFunc(str19, " ");
                                for (size_t A_Index216 = 0; A_Index216 < items216.size() + 0; A_Index216++) {
                                    std::string A_LoopField216 = items216[A_Index216 - 0];
                                    if (A_Index216 == 3) {
                                        if (RegExMatch(A_LoopField216, "^-?\\d+(\\.\\d+)?$")) {
                                            str20 += A_LoopField216 + "f ";
                                        } else {
                                            str20 += A_LoopField216 + " ";
                                        }
                                    } else {
                                        str20 += A_LoopField216 + " ";
                                    }
                            ...
                            for (size_t A_Index217 = 0; A_Index217 < items217.size() + 0; A_Index217++) {
                                ...
                                str16 = Trim(StrSplit(Trim(StrSplit(Trim(A_LoopField217), " = ", 1)), " ", 2));
                                str17 = Trim(StrSplit(Trim(StrSplit(Trim(A_LoopField217), " = ", 1)), " ", 1));
                                // str16 = var name
                                // str17 = type name
                                str13 += "if (__HTVM_V2_TO_JAVA_optionalParams__.length " + Chr(62) + " " + STR(A_Index217) + ") " + str16 + " = (" + str17 + ") __HTVM_V2_TO_JAVA_optionalParams__[" + STR(A_Index217) + "];" + Chr(10);
                            }
                            str12 = str14;
                        }
                        str11 += str12;
                    ...
                    if (Trim(str5) == "" && Trim(str10) != "") {
                        ...
                        if (langToConvertTo == "lua") {
                            ...
                            for (size_t A_Index220 = 0; A_Index220 < items220.size() + 0; A_Index220++) {
                                ...
                                str14 += Trim(StrSplit(Trim(A_LoopField220), " = ", 1)) + ", ";
                            }
                            str14 = StringTrimRight(str14, 2);
                            str13 += "{" + Chr(10);
                            ...
                            for (size_t A_Index221 = 0; A_Index221 < items221.size() + 0; A_Index221++) {
                                ...
                                str13 += Trim(StrSplit(Trim(A_LoopField221), " = ", 1)) + " = " + Trim(StrSplit(Trim(A_LoopField221), " = ", 1)) + " or " + Trim(StrSplit(Trim(A_LoopField221), " = ", 2)) + Chr(10);
                            }
                            str11 = str14;
                        }
                        if (langToConvertTo == "go") {
                            ...
                            for (size_t A_Index222 = 0; A_Index222 < items222.size() + 0; A_Index222++) {
                                ...
                                str16 = Trim(StrSplit(Trim(StrSplit(Trim(A_LoopField222), " = ", 1)), " ", 1));
                                str17 = Trim(StrSplit(Trim(A_LoopField222), " = ", 2));
                                str13 += str16 + " := " + str17 + Chr(10);
                            }
                            ...
                            for (size_t A_Index223 = 0; A_Index223 < items223.size() + 0; A_Index223++) {
                                ...
                                str18 = Trim(StrSplit(Trim(StrSplit(Trim(A_LoopField223), " = ", 1)), " ", 2));
                                // str16 = var name
                                // str18 = var type
                                str13 += "if len(__HTVM_V2_TO_GO_optionalParams__) " + Chr(62) + " " + STR(A_Index223) + Chr(10) + "{" + Chr(10) + str16 + " = __HTVM_V2_TO_GO_optionalParams__[" + STR(A_Index223) + "].(" + str18 + ")" + Chr(10) + "}" + Chr(10);
                            }
                            str11 = str14;
                        }
                        if (langToConvertTo == "java") {
                            ...
                            for (size_t A_Index224 = 0; A_Index224 < items224.size() + 0; A_Index224++) {
                                ...
                                str19 = Trim(A_LoopField224);
                                str20 = "";
                                std::vector<std::string> items225 = LoopParseFunc(str19, " ");
                                for (size_t A_Index225 = 0; A_Index225 < items225.size() + 0; A_Index225++) {
                                    std::string A_LoopField225 = items225[A_Index225 - 0];
                                    if (A_Index225 == 3) {
                                        if (RegExMatch(A_LoopField225, "^-?\\d+(\\.\\d+)?$")) {
                                            str20 += A_LoopField225 + "f ";
                                        } else {
                                            str20 += A_LoopField225 + " ";
                                        }
                                    } else {
                                        str20 += A_LoopField225 + " ";
                                    }
                            ...
                            for (size_t A_Index226 = 0; A_Index226 < items226.size() + 0; A_Index226++) {
                                ...
                                str16 = Trim(StrSplit(Trim(StrSplit(Trim(A_LoopField226), " = ", 1)), " ", 2));
                                str17 = Trim(StrSplit(Trim(StrSplit(Trim(A_LoopField226), " = ", 1)), " ", 1));
                                // str16 = var name
                                // str17 = type name
                                str13 += "if (__HTVM_V2_TO_JAVA_optionalParams__.length " + Chr(62) + " " + STR(A_Index226) + ") " + str16 + " = (" + str17 + ") __HTVM_V2_TO_JAVA_optionalParams__[" + STR(A_Index226) + "];" + Chr(10);
                            }
                            str11 = str14;
                        }
                    }
            ...
            else if (InStr(StrLower(A_LoopField205), " main(") && lineDone == 0) {
                lineDone = 1;
                javaMainFuncSeen = 1;
                csMainFuncSeen = 1;
                htCode += A_LoopField205 + Chr(10);
            }
            else if (InStr(A_LoopField205, " " + keyWordAssign + " ") || InStr(A_LoopField205, " " + keyWordAssignAdd + " ") || InStr(A_LoopField205, " " + keyWordAssignConcat + " ") || InStr(A_LoopField205, " " + keyWordAssignSub + " ") || InStr(A_LoopField205, " " + keyWordAssignMul + " ") || InStr(A_LoopField205, " " + keyWordAssignDiv + " ") || InStr(A_LoopField205, " " + keyWordAssignMod + " ") || InStr(A_LoopField205, " " + keyWordAssignShiftLeft + " ") || InStr(A_LoopField205, " " + keyWordAssignShiftRight + " ") || InStr(A_LoopField205, " " + keyWordLogicalAssignShiftRight + " ") || InStr(A_LoopField205, " " + keyWordAssignBitAnd + " ") || InStr(A_LoopField205, " " + keyWordAssignBitOr + " ") || InStr(A_LoopField205, " " + keyWordAssignBitXor + " ") && lineDone == 0) {
                allVarsSoWeDontReDecVars_FIX_TOGGLE = 1;
                //keyWordAssign
                //keyWordAssignAdd
                //keyWordAssignConcat
                ...
                //allVarsSoWeDontReDecVars_FIX_int64
                //allVarsSoWeDontReDecVars_FIX_float
                if (InStr(A_LoopField205, " " + keyWordAssign + " ")) {
                    str1 = StrSplit(A_LoopField205, " " + keyWordAssign + " ", 1);
                    str6 = StrSplit(A_LoopField205, " " + keyWordAssign + " ", 2);
                    str10 = keyWordAssign;
                    str11 = keyWordAssign_2;
                    if (langToConvertTo == "ahk") {
                        str12 = ":=";
                ...
                else if (InStr(A_LoopField205, " " + keyWordAssignAdd + " ")) {
                    str1 = StrSplit(A_LoopField205, " " + keyWordAssignAdd + " ", 1);
                    str6 = StrSplit(A_LoopField205, " " + keyWordAssignAdd + " ", 2);
                    str10 = keyWordAssignAdd;
                    str11 = keyWordAssignAdd_2;
                    str12 = "+=";
                }
                else if (InStr(A_LoopField205, " " + keyWordAssignConcat + " ")) {
                    str1 = StrSplit(A_LoopField205, " " + keyWordAssignConcat + " ", 1);
                    str6 = StrSplit(A_LoopField205, " " + keyWordAssignConcat + " ", 2);
                    str10 = keyWordAssignConcat;
                    str11 = keyWordAssignConcat_2;
                    if (langToConvertTo == "ahk") {
                        str12 = ".=";
                ...
                else if (InStr(A_LoopField205, " " + keyWordAssignSub + " ")) {
                    str1 = StrSplit(A_LoopField205, " " + keyWordAssignSub + " ", 1);
                    str6 = StrSplit(A_LoopField205, " " + keyWordAssignSub + " ", 2);
                    str10 = keyWordAssignSub;
                    str11 = keyWordAssignSub_2;
                    str12 = "-=";
                }
                else if (InStr(A_LoopField205, " " + keyWordAssignMul + " ")) {
                    str1 = StrSplit(A_LoopField205, " " + keyWordAssignMul + " ", 1);
                    str6 = StrSplit(A_LoopField205, " " + keyWordAssignMul + " ", 2);
                    str10 = keyWordAssignMul;
                    str11 = keyWordAssignMul_2;
                    str12 = "*=";
                }
                else if (InStr(A_LoopField205, " " + keyWordAssignDiv + " ")) {
                    str1 = StrSplit(A_LoopField205, " " + keyWordAssignDiv + " ", 1);
                    str6 = StrSplit(A_LoopField205, " " + keyWordAssignDiv + " ", 2);
                    str10 = keyWordAssignDiv;
                    str11 = keyWordAssignDiv_2;
                    str12 = "/=";
                }
                else if (InStr(A_LoopField205, " " + keyWordAssignMod + " ")) {
                    str1 = StrSplit(A_LoopField205, " " + keyWordAssignMod + " ", 1);
                    str6 = StrSplit(A_LoopField205, " " + keyWordAssignMod + " ", 2);
                    str10 = keyWordAssignMod;
                    str11 = keyWordAssignMod_2;
                    str12 = "%=";
                }
                else if (InStr(A_LoopField205, " " + keyWordAssignShiftLeft + " ")) {
                    str1 = StrSplit(A_LoopField205, " " + keyWordAssignShiftLeft + " ", 1);
                    str6 = StrSplit(A_LoopField205, " " + keyWordAssignShiftLeft + " ", 2);
                    str10 = keyWordAssignShiftLeft;
                    str11 = keyWordAssignShiftLeft_2;
                    str12 = "<<=";
                }
                else if (InStr(A_LoopField205, " " + keyWordAssignShiftRight + " ")) {
                    str1 = StrSplit(A_LoopField205, " " + keyWordAssignShiftRight + " ", 1);
                    str6 = StrSplit(A_LoopField205, " " + keyWordAssignShiftRight + " ", 2);
                    str10 = keyWordAssignShiftRight;
                    str11 = keyWordAssignShiftRight_2;
                    str12 = ">>=";
                }
                else if (InStr(A_LoopField205, " " + keyWordLogicalAssignShiftRight + " ")) {
                    str1 = StrSplit(A_LoopField205, " " + keyWordLogicalAssignShiftRight + " ", 1);
                    str6 = StrSplit(A_LoopField205, " " + keyWordLogicalAssignShiftRight + " ", 2);
                    str10 = keyWordLogicalAssignShiftRight;
                    str11 = keyWordLogicalAssignShiftRight_2;
                    str12 = ">>>=";
                }
                else if (InStr(A_LoopField205, " " + keyWordAssignBitAnd + " ")) {
                    str1 = StrSplit(A_LoopField205, " " + keyWordAssignBitAnd + " ", 1);
                    str6 = StrSplit(A_LoopField205, " " + keyWordAssignBitAnd + " ", 2);
                    str10 = keyWordAssignBitAnd;
                    str11 = keyWordAssignBitAnd_2;
                    str12 = "&=";
                }
                else if (InStr(A_LoopField205, " " + keyWordAssignBitOr + " ")) {
                    str1 = StrSplit(A_LoopField205, " " + keyWordAssignBitOr + " ", 1);
                    str6 = StrSplit(A_LoopField205, " " + keyWordAssignBitOr + " ", 2);
                    str10 = keyWordAssignBitOr;
                    str11 = keyWordAssignBitOr_2;
                    str12 = "|=";
                }
                else if (InStr(A_LoopField205, " " + keyWordAssignBitXor + " ")) {
                    str1 = StrSplit(A_LoopField205, " " + keyWordAssignBitXor + " ", 1);
                    str6 = StrSplit(A_LoopField205, " " + keyWordAssignBitXor + " ", 2);
                    str10 = keyWordAssignBitXor;
                    str11 = keyWordAssignBitXor_2;
                    str12 = "^=";
                }
            ...
            else if (SubStr(Trim(A_LoopField205), 1, StrLen(keyWordVar)) == keyWordVar && lineDone == 0) {
                allVarsSoWeDontReDecVars_FIX_TOGGLE = 1;
                if (usePostfixTypeForTypeDefinition == "off") {
                    lineDone = 1;
                    str1 = Trim(StringTrimLeft(Trim(A_LoopField205), StrLen(keyWordVar)));
                    if (SubStrLastChars(str1, 1) == ";") {
                        str1 = StringTrimRight(str1, 1);
                    }
            ...
            else if (SubStr(Trim(A_LoopField205), 1, StrLen(keyWordLet)) == keyWordLet && lineDone == 0) {
                allVarsSoWeDontReDecVars_FIX_TOGGLE = 1;
                if (usePostfixTypeForTypeDefinition == "off") {
                    lineDone = 1;
                    str1 = Trim(StringTrimLeft(Trim(A_LoopField205), StrLen(keyWordLet)));
                    if (SubStrLastChars(str1, 1) == ";") {
                        str1 = StringTrimRight(str1, 1);
                    }
            ...
            else if (SubStr(Trim(A_LoopField205), 1, StrLen(keyWordDouble + " ")) == keyWordDouble + " " || SubStr(Trim(A_LoopField205), 1, StrLen(keyWordChar + " ")) == keyWordChar + " " || SubStr(Trim(A_LoopField205), 1, StrLen(keyWordUint8 + " ")) == keyWordUint8 + " " || SubStr(Trim(A_LoopField205), 1, StrLen(keyWordUint16 + " ")) == keyWordUint16 + " " || SubStr(Trim(A_LoopField205), 1, StrLen(keyWordUint32 + " ")) == keyWordUint32 + " " || SubStr(Trim(A_LoopField205), 1, StrLen(keyWordUint64 + " ")) == keyWordUint64 + " " || SubStr(Trim(A_LoopField205), 1, StrLen(keyWordINT + " ")) == keyWordINT + " " || SubStr(Trim(A_LoopField205), 1, StrLen(keyWordSTR + " ")) == keyWordSTR + " " || SubStr(Trim(A_LoopField205), 1, StrLen(keyWordBOOL + " ")) == keyWordBOOL + " " || SubStr(Trim(A_LoopField205), 1, StrLen(keyWordFLOAT + " ")) == keyWordFLOAT + " " || SubStr(Trim(A_LoopField205), 1, StrLen(keyWordINT8 + " ")) == keyWordINT8 + " " || SubStr(Trim(A_LoopField205), 1, StrLen(keyWordINT16 + " ")) == keyWordINT16 + " " || SubStr(Trim(A_LoopField205), 1, StrLen(keyWordINT32 + " ")) == keyWordINT32 + " " || SubStr(Trim(A_LoopField205), 1, StrLen(keyWordINT64 + " ")) == keyWordINT64 + " " || SubStr(Trim(A_LoopField205), 1, StrLen(keyWordArrayDefinition + " ")) == keyWordArrayDefinition + " " || SubStr(Trim(A_LoopField205), 1, StrLen(keyWordArrayOfIntegersDefinition + " ")) == keyWordArrayOfIntegersDefinition + " " || SubStr(Trim(A_LoopField205), 1, StrLen(keyWordArrayOfStringsDefinition + " ")) == keyWordArrayOfStringsDefinition + " " || SubStr(Trim(A_LoopField205), 1, StrLen(keyWordArrayOfFloatingPointNumbersDefinition + " ")) == keyWordArrayOfFloatingPointNumbersDefinition + " " || SubStr(Trim(A_LoopField205), 1, StrLen(keyWordArrayOfBooleansDefinition + " ")) == keyWordArrayOfBooleansDefinition + " " && usePrefixTypeForTypeDefinition == "on" && lineDone == 0) {
                lineDone = 1;
                allVarsSoWeDontReDecVars_FIX_TOGGLE = 1;
                str1 = Trim(A_LoopField205);
                if (SubStrLastChars(str1, 1) == ";") {
            ...
            else if (RegExMatch(Trim(A_LoopField205), "^[a-zA-Z_][a-zA-Z0-9_]*: [^:]*;?$") && usePostfixTypeForTypeDefinition == "on" && lineDone == 0) {
                lineDone = 1;
                allVarsSoWeDontReDecVars_FIX_TOGGLE = 1;
                str1 = Trim(A_LoopField205);
                if (SubStrLastChars(str1, 1) == ";") {
            ...
            else if (RegExMatch(Trim(A_LoopField205), "^[a-zA-Z0-9_\\.]+\\(") && !(InStr(StrLower(A_LoopField205), " main(")) && lineDone == 0) {
                lineDone = 1;
                str1 = Trim(A_LoopField205);
                if (SubStrLastChars(str1, 1) == ";") {
                    str1 = StringTrimRight(str1, 1);
            ...
            else if (RegExMatch(A_LoopField205, "\\.[a-zA-Z_][a-zA-Z0-9_]*") && lineDone == 0) {
                lineDone = 1;
                str1 = Trim(A_LoopField205);
                if (SubStrLastChars(str1, 1) == ";") {
                    str1 = StringTrimRight(str1, 1);
            ...
            else if (KeyWordsCommands(A_LoopField205, "check", commands, langToConvertTo) == "true" && lineDone == 0) {
                lineDone = 1;
                if (langToConvertTo == langFileExtension_2) {
                    htCode += A_LoopField205 + Chr(10);
                } else {
                    out_KeyWordsCommands = KeyWordsCommands(A_LoopField205, "transpile", commands, langToConvertTo);
                ...
                if (lineDone != 1) {
                    if (skipLeftCuleyForFuncPLS != 1) {
                        if (SubStr(Trim(StrLower(A_LoopField205)), 1, 1) == Chr(125)) {
                            htCode += Chr(125) + Chr(10);
                        } else {
                            if (htCodeAcurlyBraceAddSomeVrasFixLP == 1 && SubStr(Trim(StrLower(A_LoopField205)), 1, 1) == Chr(123)) {
                                htCodeAcurlyBraceAddSomeVrasFixLP = 0;
                                htCode += A_LoopField205 + Chr(10) + theFixTextLoopLP + Chr(10);
                            } else {
                                if (htCodeAcurlyBraceAddSomeVrasFixNL == 1 && SubStr(Trim(StrLower(A_LoopField205)), 1, 1) == Chr(123)) {
                                    htCodeAcurlyBraceAddSomeVrasFixNL = 0;
                                    htCode += A_LoopField205 + Chr(10) + theFixTextLoopNL + Chr(10);
                                } else {
                                    htCode += A_LoopField205 + Chr(10);
                                }
                            }
        ...
        if (langToConvertTo != "ahk" && langToConvertTo != langFileExtension_2) {
            ...
            if (haveWeEverUsedAloop == 1) {
                ...
                std::vector<std::string> items234 = LoopParseFunc(htCodeLoopfixa, "\n", "\r");
                for (size_t A_Index234 = 0; A_Index234 < items234.size() + 0; A_Index234++) {
                    std::string A_LoopField234 = items234[A_Index234 - 0];
                    sstr123 = A_LoopField234;
                    fixLoopLokingFor = A_LoopField234;
                    ...
                    if (out1 == "nl") {
                        ...
                        for (size_t A_Index235 = 0; A_Index235 < items235.size() + 0; A_Index235++) {
                            ...
                            if (InStr(A_LoopField235, fixLoopLokingFor) && insdeAnestedLoopBAD != 1) {
                                fixLoopLokingForNum = 1;
                                //MsgBox, do we came here 1
                            }
                            if (SubStr(Trim(A_LoopField235), 1, 4) == "for " && weAreDoneHereCurly != 1 && insdeAnestedLoopBAD != 1 && fixLoopLokingForNum == 1) {
                                s = StrSplit(A_LoopField235 ,"" + keyWordAIndex + "", 2);
                                out1z = s;
                                s = StrSplit(out1z, " ", 1);
                                out1z = Trim(s);
                                //MsgBox, % out1z
                                ...
                                //MsgBox, % A_LoopField
                                dontSaveStr = 1;
                                ALoopField = A_LoopField235;
                                DeleayOneCuzOfLoopParse = 1;
                                out4758686d86d86d86578991a += ALoopField + Chr(10);
                            }
                            if (inTarget == 1 && InStr(A_LoopField235, Chr(123)) && insdeAnestedLoopBAD != 1) {
                                insideBracket = 1;
                            }
                            if (insideBracket == 1 && InStr(A_LoopField235, Chr(123)) && insdeAnestedLoopBAD != 1) {
                                netsedCurly++;
                            }
                            if (insideBracket == 1 && InStr(A_LoopField235, Chr(125)) && insdeAnestedLoopBAD != 1) {
                                netsedCurly--;
                                readyToEnd = 1;
                            }
                            if (SubStr(Trim(A_LoopField235), 1, 4) == "for " && insdeAnestedLoopBAD != 1 && foundTheTopLoop >= 2) {
                            ...
                            if (insdeAnestedLoopBAD == 1) {
                                if (InStr(A_LoopField235, Chr(123))) {
                                    insideBracket1 = 1;
                                }
                                if (insideBracket1 == 1 && InStr(A_LoopField235, Chr(123))) {
                                    netsedCurly1++;
                                }
                                if (insideBracket1 == 1 && InStr(A_LoopField235, Chr(125))) {
                                    netsedCurly1--;
                                    readyToEnd1 = 1;
                                }
                                if (InStr(A_LoopField235, Chr(125)) && readyToEnd1 == 1 && netsedCurly1 == 0 && insideBracket == 1) {
                                    //MsgBox, % A_LoopField
                                    eldLoopNestedBADlol = 1;
                                }
                                out4758686d86d86d86578991a += A_LoopField235 + Chr(10);
                            }
                            ...
                            if (inTarget == 1 && InStr(A_LoopField235, Chr(125)) && readyToEnd == 1 && netsedCurly == 0 && weAreDoneHereCurly == 0 && dontSaveStr != 1 && insdeAnestedLoopBAD != 1) {
                                //MsgBox, % A_LoopField
                ...
                for (size_t A_Index237 = 0; A_Index237 < items237.size() + 0; A_Index237++) {
                    ...
                    ignore = 0;
                    if (SubStr(Trim(A_LoopField237), 1, 4) == "for ") {
                        if (hold == 1 && holdText == A_LoopField237) {
                            ignore = 1;
                        } else {
                            holdText = A_LoopField237;
                            hold = 1;
                        }
                    }
                    if (!ignore) {
                        out4758686d86dgt8r754444444 += A_LoopField237 + Chr(10);
                    }
                }
    ...
    if (COUNT_programmingBlock_HTVMsyntax != 0) {
        ...
        for (int A_Index239 = 0; A_Index239 < COUNT_programmingBlock_HTVMsyntax + 0; A_Index239++) {
            ADD_ALL_programmingBlock_HTVMsyntax += programmingBlock_HTVMsyntax[A_Index239] + Chr(10);
        }
        ...
        for (int A_Index240 = 0; A_Index240 < COUNT_programmingBlock_HTVMsyntax + 0; A_Index240++) {
            htCode = StrReplace(htCode, "programmingBlock_HTVMsyntax-programmingBlock_HTVMsyntax-AA" + STR(A_Index240 + 1) + "AA", compiler(programmingBlock_HTVMsyntax[A_Index240], allInstructionFile, "full", langToConvertToParam));
        }
        isNotHTVMfile2 = 0;
    }
    //;;;;;;;;;;;;;;;;;;;;;;;;;;;
    ...
    if (langToConvertTo == langFileExtension_2) {
        ...
        if (useEnd_2 == "on") {
            ...
            for (size_t A_Index241 = 0; A_Index241 < items241.size() + 0; A_Index241++) {
                ...
                if (Trim(A_LoopField241) == keyWordCurlyBraceClose_2) {
                    fixLuaAndRuby += StrReplace(A_LoopField241, keyWordCurlyBraceClose_2, Trim(keyWordEnd_2)) + Chr(10);
                }
                else if (SubStrLastChars(A_LoopField241, 2) == " " + keyWordCurlyBraceOpen_2) {
        ...
        if (langToConvertTo == "lua" || langToConvertTo == "rb" || langToConvertTo == langFileExtension_2) {
            if (langToConvertTo == langFileExtension_2 && useEnd_2 == "on") {
                ...
                for (size_t A_Index242 = 0; A_Index242 < items242.size() + 0; A_Index242++) {
                    ...
                    if (Trim(A_LoopField242) == "}") {
                        fixLuaAndRuby += StrReplace(A_LoopField242, "}", fixLuaAndRubyHELP) + Chr(10);
                    }
                    else if (SubStrLastChars(A_LoopField242, 2) == " {") {
    ...
    if (langToConvertTo == langFileExtension_2 && useEnd_2 == "on") {
        ...
        for (size_t A_Index245 = 0; A_Index245 < items245.size() + 0; A_Index245++) {
            ...
            if (Trim(A_LoopField245) == Trim(keyWordEnd_2) && SubStr(Trim(nextWordEndFix[A_Index245 + 1]), 1, StrLen(Trim(keyWordElseIf_2) + " ")) == Trim(keyWordElseIf_2) + " " || Trim(A_LoopField245) == Trim(keyWordEnd_2) && (Trim(nextWordEndFix[A_Index245 + 1]) == Trim(keyWordElse_2) || Trim(nextWordEndFix[A_Index245 + 1]) == Trim(keyWordElse_2 + ":"))) {
                nextWordEndFixOut += "";
            } else {
    ...
    if (langToConvertTo == "lua") {
        ...
        for (size_t A_Index247 = 0; A_Index247 < items247.size() + 0; A_Index247++) {
            ...
            if (Trim(A_LoopField247) == "end" && SubStr(Trim(nextWordEndFix[A_Index247 + 1]), 1, StrLen("elseif ")) == "elseif " || Trim(A_LoopField247) == "end" && Trim(nextWordEndFix[A_Index247 + 1]) == "else") {
                nextWordEndFixOut += "";
            } else {
    ...
    if (langToConvertTo == "rb") {
        ...
        for (size_t A_Index248 = 0; A_Index248 < items248.size() + 0; A_Index248++) {
            ...
            HTVM_Append(nextWordEndFix, A_LoopField248);
        }
        HTVM_Append(nextWordEndFix, " ");
        ...
        for (size_t A_Index249 = 0; A_Index249 < items249.size() + 0; A_Index249++) {
            ...
            if (Trim(A_LoopField249) == "end" && SubStr(Trim(nextWordEndFix[A_Index249 + 1]), 1, StrLen("elsif ")) == "elsif " || Trim(A_LoopField249) == "end" && Trim(nextWordEndFix[A_Index249 + 1]) == "else") {
                nextWordEndFixOut += "";
            } else {
    ...
    if (langToConvertTo == "rb") {
        ...
        for (size_t A_Index251 = 0; A_Index251 < items251.size() + 0; A_Index251++) {
            ...
            if (Trim(A_LoopField251) == "end" && SubStr(Trim(nextWordEndFixRB[A_Index251 + 1]), 1, StrLen("rescue ")) == "rescue " || Trim(A_LoopField251) == "end" && Trim(nextWordEndFixRB[A_Index251 + 1]) == "ensure") {
                nextWordEndFixOut += "";
            } else {
    ...
    if (langToConvertTo == langFileExtension_2 && useEnd_2 == "on") {
        ...
        for (size_t A_Index252 = 0; A_Index252 < items252.size() + 0; A_Index252++) {
            ...
            HTVM_Append(nextWordEndFixRB, A_LoopField252);
        }
        HTVM_Append(nextWordEndFixRB, " ");
        ...
        for (size_t A_Index253 = 0; A_Index253 < items253.size() + 0; A_Index253++) {
            ...
            if (Trim(A_LoopField253) == Trim(keyWordEnd_2) && SubStr(Trim(nextWordEndFixRB[A_Index253 + 1]), 1, StrLen(Trim(keyWordCatch_2) + " ")) == Trim(keyWordCatch_2) + " " || Trim(A_LoopField253) == Trim(keyWordEnd_2) && Trim(nextWordEndFixRB[A_Index253 + 1]) == Trim(keyWordFinally_2) || Trim(nextWordEndFixRB[A_Index253 + 1]) == Trim(keyWordFinally_2 + ":")) {
                nextWordEndFixOut += "";
            } else {
                nextWordEndFixOut += A_LoopField253 + Chr(10);
            }
        }
        htCode = StringTrimRight(nextWordEndFixOut, 1);
    }
    ...
    if (langToConvertTo == "ahk") {
        ...
        for (size_t A_Index254 = 0; A_Index254 < items254.size() + 0; A_Index254++) {
            ...
            if (SubStr(Trim(A_LoopField254), 1, StrLen("Loop, Parse, ")) == "Loop, Parse, ") {
                nextWordEndFixOut += StrReplace(A_LoopField254, "{", Chr(10) + "{" + Chr(10)) + Chr(10);
            } else {
    ...
    if (langToConvertTo == "go") {
        ...
        for (size_t A_Index255 = 0; A_Index255 < items255.size() + 0; A_Index255++) {
            ...
            HTVM_Append(fixGoManGoIsSoAnnoyingBroooFurure, A_LoopField255);
        }
        ...
        for (size_t A_Index256 = 0; A_Index256 < items256.size() + 0; A_Index256++) {
            ...
            if (Trim(A_LoopField256) == "}" && SubStr(Trim(fixGoManGoIsSoAnnoyingBroooFurure[A_Index256 + 1]), 1, StrLen("else if ")) == "else if ") {
                fixGoManGoIsSoAnnoyingBroooSkip = 1;
                fixGoManGoIsSoAnnoyingBroooCount = 0;
                std::vector<std::string> items257 = LoopParseFunc(fixGoManGoIsSoAnnoyingBroooFurure[A_Index256 + 1]);
                ...
                for (int A_Index258 = 0; A_Index258 < fixGoManGoIsSoAnnoyingBroooCount + 0; A_Index258++) {
                    fixGoManGoIsSoAnnoyingBroooCountSpaceses += " ";
                }
                fixGoManGoIsSoAnnoyingBrooo += fixGoManGoIsSoAnnoyingBroooCountSpaceses + "} " + Trim(fixGoManGoIsSoAnnoyingBroooFurure[A_Index256 + 1]) + Chr(10);
            } else {
                if (fixGoManGoIsSoAnnoyingBroooSkip == 0) {
                    fixGoManGoIsSoAnnoyingBrooo += A_LoopField256 + Chr(10);
                }
    ...
    for (int A_Index259 = 0; A_Index259 < HTVM_Size(commentsBugFix) + 0; A_Index259++) {
        htCode = StrReplace(htCode, "HTVM--cnavisdofbuvsesdivdidufg79wregwewaeosd8ges7dfdftuawdiHTVMv2yerheyziberlasduci6syiq--AA" + STR(A_Index259) + "AA", commentsBugFix[A_Index259]);
    }
    //;;;;;;;;;;;;;;;;;;;;;;;;; fix here
    //;;;;;;;;;;;;;;;;;;;;;;;;; fix here
    //;;;;;;;;;;;;;;;;;;;;;;;;; fix here
    if (COUNT_programmingBlock_InTheTranspiledLang != 0) {
        for (int A_Index260 = 0; A_Index260 < COUNT_programmingBlock_InTheTranspiledLang + 0; A_Index260++) {
            htCode = StrReplace(htCode, "programmingBlock_InTheTranspiledLang-programmingBlock_InTheTranspiledLang-AA" + STR(A_Index260 + 1) + "AA", programmingBlock_InTheTranspiledLang[A_Index260]);
        }
    }
    if (langToConvertTo == "cpp") {
        if (COUNT_programmingBlock_CPP != 0) {
            for (int A_Index261 = 0; A_Index261 < COUNT_programmingBlock_CPP + 0; A_Index261++) {
                htCode = StrReplace(htCode, "programmingBlock_CPP-programmingBlock_CPP-AA" + STR(A_Index261 + 1) + "AA", programmingBlock_CPP[A_Index261]);
            }
        }
    } else {
        if (COUNT_programmingBlock_CPP != 0) {
            for (int A_Index262 = 0; A_Index262 < COUNT_programmingBlock_CPP + 0; A_Index262++) {
                if (langToConvertTo == langFileExtension_2) {
                    htCode = StrReplace(htCode, "programmingBlock_CPP-programmingBlock_CPP-AA" + STR(A_Index262 + 1) + "AA", keyWordCodeInTheTranspiledLangStartCPP_2 + Chr(10) + programmingBlock_CPP[A_Index262] + Chr(10) + keyWordCodeInTheTranspiledLangEndCPP_2);
                } else {
                    htCode = StrReplace(htCode, "programmingBlock_CPP-programmingBlock_CPP-AA" + STR(A_Index262 + 1) + "AA", Chr(10));
                }
            }
        }
    }
    if (langToConvertTo == "py") {
        if (COUNT_programmingBlock_PY != 0) {
            for (int A_Index263 = 0; A_Index263 < COUNT_programmingBlock_PY + 0; A_Index263++) {
                htCode = StrReplace(htCode, "programmingBlock_PY-programmingBlock_PY-AA" + STR(A_Index263 + 1) + "AA", programmingBlock_PY[A_Index263]);
            }
        }
    } else {
        if (COUNT_programmingBlock_PY != 0) {
            for (int A_Index264 = 0; A_Index264 < COUNT_programmingBlock_PY + 0; A_Index264++) {
                if (langToConvertTo == langFileExtension_2) {
                    htCode = StrReplace(htCode, "programmingBlock_PY-programmingBlock_PY-AA" + STR(A_Index264 + 1) + "AA", keyWordCodeInTheTranspiledLangStartPY_2 + Chr(10) + programmingBlock_PY[A_Index264] + Chr(10) + keyWordCodeInTheTranspiledLangEndPY_2);
                } else {
                    htCode = StrReplace(htCode, "programmingBlock_PY-programmingBlock_PY-AA" + STR(A_Index264 + 1) + "AA", Chr(10));
                }
            }
        }
    }
    if (langToConvertTo == "js") {
        if (COUNT_programmingBlock_JS != 0) {
            for (int A_Index265 = 0; A_Index265 < COUNT_programmingBlock_JS + 0; A_Index265++) {
                htCode = StrReplace(htCode, "programmingBlock_JS-programmingBlock_JS-AA" + STR(A_Index265 + 1) + "AA", programmingBlock_JS[A_Index265]);
            }
        }
    } else {
        if (COUNT_programmingBlock_JS != 0) {
            for (int A_Index266 = 0; A_Index266 < COUNT_programmingBlock_JS + 0; A_Index266++) {
                if (langToConvertTo == langFileExtension_2) {
                    htCode = StrReplace(htCode, "programmingBlock_JS-programmingBlock_JS-AA" + STR(A_Index266 + 1) + "AA", keyWordCodeInTheTranspiledLangStartJS_2 + Chr(10) + programmingBlock_JS[A_Index266] + Chr(10) + keyWordCodeInTheTranspiledLangEndJS_2);
                } else {
                    htCode = StrReplace(htCode, "programmingBlock_JS-programmingBlock_JS-AA" + STR(A_Index266 + 1) + "AA", Chr(10));
                }
            }
        }
    }
    if (langToConvertTo == "go") {
        if (COUNT_programmingBlock_GO != 0) {
            for (int A_Index267 = 0; A_Index267 < COUNT_programmingBlock_GO + 0; A_Index267++) {
                htCode = StrReplace(htCode, "programmingBlock_GO-programmingBlock_GO-AA" + STR(A_Index267 + 1) + "AA", programmingBlock_GO[A_Index267]);
            }
        }
    } else {
        if (COUNT_programmingBlock_GO != 0) {
            for (int A_Index268 = 0; A_Index268 < COUNT_programmingBlock_GO + 0; A_Index268++) {
                if (langToConvertTo == langFileExtension_2) {
                    htCode = StrReplace(htCode, "programmingBlock_GO-programmingBlock_GO-AA" + STR(A_Index268 + 1) + "AA", keyWordCodeInTheTranspiledLangStartGO_2 + Chr(10) + programmingBlock_GO[A_Index268] + Chr(10) + keyWordCodeInTheTranspiledLangEndGO_2);
                } else {
                    htCode = StrReplace(htCode, "programmingBlock_GO-programmingBlock_GO-AA" + STR(A_Index268 + 1) + "AA", Chr(10));
                }
            }
        }
    }
    if (langToConvertTo == "lua") {
        if (COUNT_programmingBlock_LUA != 0) {
            for (int A_Index269 = 0; A_Index269 < COUNT_programmingBlock_LUA + 0; A_Index269++) {
                htCode = StrReplace(htCode, "programmingBlock_LUA-programmingBlock_LUA-AA" + STR(A_Index269 + 1) + "AA", programmingBlock_LUA[A_Index269]);
            }
        }
    } else {
        if (COUNT_programmingBlock_LUA != 0) {
            for (int A_Index270 = 0; A_Index270 < COUNT_programmingBlock_LUA + 0; A_Index270++) {
                if (langToConvertTo == langFileExtension_2) {
                    htCode = StrReplace(htCode, "programmingBlock_LUA-programmingBlock_LUA-AA" + STR(A_Index270 + 1) + "AA", keyWordCodeInTheTranspiledLangStartLUA_2 + Chr(10) + programmingBlock_LUA[A_Index270] + Chr(10) + keyWordCodeInTheTranspiledLangEndLUA_2);
                } else {
                    htCode = StrReplace(htCode, "programmingBlock_LUA-programmingBlock_LUA-AA" + STR(A_Index270 + 1) + "AA", Chr(10));
                }
            }
        }
    }
    if (langToConvertTo == "cs") {
        if (COUNT_programmingBlock_CS != 0) {
            for (int A_Index271 = 0; A_Index271 < COUNT_programmingBlock_CS + 0; A_Index271++) {
                htCode = StrReplace(htCode, "programmingBlock_CS-programmingBlock_CS-AA" + STR(A_Index271 + 1) + "AA", programmingBlock_CS[A_Index271]);
            }
        }
    } else {
        if (COUNT_programmingBlock_CS != 0) {
            for (int A_Index272 = 0; A_Index272 < COUNT_programmingBlock_CS + 0; A_Index272++) {
                if (langToConvertTo == langFileExtension_2) {
                    htCode = StrReplace(htCode, "programmingBlock_CS-programmingBlock_CS-AA" + STR(A_Index272 + 1) + "AA", keyWordCodeInTheTranspiledLangStartCS_2 + Chr(10) + programmingBlock_CS[A_Index272] + Chr(10) + keyWordCodeInTheTranspiledLangEndCS_2);
                } else {
                    htCode = StrReplace(htCode, "programmingBlock_CS-programmingBlock_CS-AA" + STR(A_Index272 + 1) + "AA", Chr(10));
                }
            }
        }
    }
    if (langToConvertTo == "java") {
        if (COUNT_programmingBlock_JAVA != 0) {
            for (int A_Index273 = 0; A_Index273 < COUNT_programmingBlock_JAVA + 0; A_Index273++) {
                htCode = StrReplace(htCode, "programmingBlock_JAVA-programmingBlock_JAVA-AA" + STR(A_Index273 + 1) + "AA", programmingBlock_JAVA[A_Index273]);
            }
        }
    } else {
        if (COUNT_programmingBlock_JAVA != 0) {
            for (int A_Index274 = 0; A_Index274 < COUNT_programmingBlock_JAVA + 0; A_Index274++) {
                if (langToConvertTo == langFileExtension_2) {
                    htCode = StrReplace(htCode, "programmingBlock_JAVA-programmingBlock_JAVA-AA" + STR(A_Index274 + 1) + "AA", keyWordCodeInTheTranspiledLangStartJAVA_2 + Chr(10) + programmingBlock_JAVA[A_Index274] + Chr(10) + keyWordCodeInTheTranspiledLangEndJAVA_2);
                } else {
                    htCode = StrReplace(htCode, "programmingBlock_JAVA-programmingBlock_JAVA-AA" + STR(A_Index274 + 1) + "AA", Chr(10));
                }
            }
        }
    }
    if (langToConvertTo == "kt") {
        if (COUNT_programmingBlock_KT != 0) {
            for (int A_Index275 = 0; A_Index275 < COUNT_programmingBlock_KT + 0; A_Index275++) {
                htCode = StrReplace(htCode, "programmingBlock_KT-programmingBlock_KT-AA" + STR(A_Index275 + 1) + "AA", programmingBlock_KT[A_Index275]);
            }
        }
    } else {
        if (COUNT_programmingBlock_KT != 0) {
            for (int A_Index276 = 0; A_Index276 < COUNT_programmingBlock_KT + 0; A_Index276++) {
                if (langToConvertTo == langFileExtension_2) {
                    htCode = StrReplace(htCode, "programmingBlock_KT-programmingBlock_KT-AA" + STR(A_Index276 + 1) + "AA", keyWordCodeInTheTranspiledLangStartKT_2 + Chr(10) + programmingBlock_KT[A_Index276] + Chr(10) + keyWordCodeInTheTranspiledLangEndKT_2);
                } else {
                    htCode = StrReplace(htCode, "programmingBlock_KT-programmingBlock_KT-AA" + STR(A_Index276 + 1) + "AA", Chr(10));
                }
            }
        }
    }
    if (langToConvertTo == "rb") {
        if (COUNT_programmingBlock_RB != 0) {
            for (int A_Index277 = 0; A_Index277 < COUNT_programmingBlock_RB + 0; A_Index277++) {
                htCode = StrReplace(htCode, "programmingBlock_RB-programmingBlock_RB-AA" + STR(A_Index277 + 1) + "AA", programmingBlock_RB[A_Index277]);
            }
        }
    } else {
        if (COUNT_programmingBlock_RB != 0) {
            for (int A_Index278 = 0; A_Index278 < COUNT_programmingBlock_RB + 0; A_Index278++) {
                if (langToConvertTo == langFileExtension_2) {
                    htCode = StrReplace(htCode, "programmingBlock_RB-programmingBlock_RB-AA" + STR(A_Index278 + 1) + "AA", keyWordCodeInTheTranspiledLangStartRB_2 + Chr(10) + programmingBlock_RB[A_Index278] + Chr(10) + keyWordCodeInTheTranspiledLangEndRB_2);
                } else {
                    htCode = StrReplace(htCode, "programmingBlock_RB-programmingBlock_RB-AA" + STR(A_Index278 + 1) + "AA", Chr(10));
                }
            }
        }
    }
    if (langToConvertTo == "nim") {
        if (COUNT_programmingBlock_NIM != 0) {
            for (int A_Index279 = 0; A_Index279 < COUNT_programmingBlock_NIM + 0; A_Index279++) {
                htCode = StrReplace(htCode, "programmingBlock_NIM-programmingBlock_NIM-AA" + STR(A_Index279 + 1) + "AA", programmingBlock_NIM[A_Index279]);
            }
        }
    } else {
        if (COUNT_programmingBlock_NIM != 0) {
            for (int A_Index280 = 0; A_Index280 < COUNT_programmingBlock_NIM + 0; A_Index280++) {
                if (langToConvertTo == langFileExtension_2) {
                    htCode = StrReplace(htCode, "programmingBlock_NIM-programmingBlock_NIM-AA" + STR(A_Index280 + 1) + "AA", keyWordCodeInTheTranspiledLangStartNIM_2 + Chr(10) + programmingBlock_NIM[A_Index280] + Chr(10) + keyWordCodeInTheTranspiledLangEndNIM_2);
                } else {
                    htCode = StrReplace(htCode, "programmingBlock_NIM-programmingBlock_NIM-AA" + STR(A_Index280 + 1) + "AA", Chr(10));
                }
            }
        }
    }
    if (langToConvertTo == "ahk") {
        if (COUNT_programmingBlock_AHK != 0) {
            for (int A_Index281 = 0; A_Index281 < COUNT_programmingBlock_AHK + 0; A_Index281++) {
                htCode = StrReplace(htCode, "programmingBlock_AHK-programmingBlock_AHK-AA" + STR(A_Index281 + 1) + "AA", programmingBlock_AHK[A_Index281]);
            }
        }
    } else {
        if (COUNT_programmingBlock_AHK != 0) {
            for (int A_Index282 = 0; A_Index282 < COUNT_programmingBlock_AHK + 0; A_Index282++) {
                if (langToConvertTo == langFileExtension_2) {
                    htCode = StrReplace(htCode, "programmingBlock_AHK-programmingBlock_AHK-AA" + STR(A_Index282 + 1) + "AA", keyWordCodeInTheTranspiledLangStartAHK_2 + Chr(10) + programmingBlock_AHK[A_Index282] + Chr(10) + keyWordCodeInTheTranspiledLangEndAHK_2);
                } else {
                    htCode = StrReplace(htCode, "programmingBlock_AHK-programmingBlock_AHK-AA" + STR(A_Index282 + 1) + "AA", Chr(10));
                }
            }
        }
    }
    if (langToConvertTo == "swift") {
        if (COUNT_programmingBlock_SWIFT != 0) {
            for (int A_Index283 = 0; A_Index283 < COUNT_programmingBlock_SWIFT + 0; A_Index283++) {
                htCode = StrReplace(htCode, "programmingBlock_SWIFT-programmingBlock_SWIFT-AA" + STR(A_Index283 + 1) + "AA", programmingBlock_SWIFT[A_Index283]);
            }
        }
    } else {
        if (COUNT_programmingBlock_SWIFT != 0) {
            for (int A_Index284 = 0; A_Index284 < COUNT_programmingBlock_SWIFT + 0; A_Index284++) {
                if (langToConvertTo == langFileExtension_2) {
                    htCode = StrReplace(htCode, "programmingBlock_SWIFT-programmingBlock_SWIFT-AA" + STR(A_Index284 + 1) + "AA", keyWordCodeInTheTranspiledLangStartSWIFT_2 + Chr(10) + programmingBlock_SWIFT[A_Index284] + Chr(10) + keyWordCodeInTheTranspiledLangEndSWIFT_2);
                } else {
                    htCode = StrReplace(htCode, "programmingBlock_SWIFT-programmingBlock_SWIFT-AA" + STR(A_Index284 + 1) + "AA", Chr(10));
                }
            }
        }
    }
    if (langToConvertTo == "dart") {
        if (COUNT_programmingBlock_DART != 0) {
            for (int A_Index285 = 0; A_Index285 < COUNT_programmingBlock_DART + 0; A_Index285++) {
                htCode = StrReplace(htCode, "programmingBlock_DART-programmingBlock_DART-AA" + STR(A_Index285 + 1) + "AA", programmingBlock_DART[A_Index285]);
            }
        }
    } else {
        if (COUNT_programmingBlock_DART != 0) {
            for (int A_Index286 = 0; A_Index286 < COUNT_programmingBlock_DART + 0; A_Index286++) {
                if (langToConvertTo == langFileExtension_2) {
                    htCode = StrReplace(htCode, "programmingBlock_DART-programmingBlock_DART-AA" + STR(A_Index286 + 1) + "AA", keyWordCodeInTheTranspiledLangStartDART_2 + Chr(10) + programmingBlock_DART[A_Index286] + Chr(10) + keyWordCodeInTheTranspiledLangEndDART_2);
                } else {
                    htCode = StrReplace(htCode, "programmingBlock_DART-programmingBlock_DART-AA" + STR(A_Index286 + 1) + "AA", Chr(10));
                }
            }
        }
    }
    if (langToConvertTo == "ts") {
        if (COUNT_programmingBlock_TS != 0) {
            for (int A_Index287 = 0; A_Index287 < COUNT_programmingBlock_TS + 0; A_Index287++) {
                htCode = StrReplace(htCode, "programmingBlock_TS-programmingBlock_TS-AA" + STR(A_Index287 + 1) + "AA", programmingBlock_TS[A_Index287]);
            }
        }
    } else {
        if (COUNT_programmingBlock_TS != 0) {
            for (int A_Index288 = 0; A_Index288 < COUNT_programmingBlock_TS + 0; A_Index288++) {
                if (langToConvertTo == langFileExtension_2) {
                    htCode = StrReplace(htCode, "programmingBlock_TS-programmingBlock_TS-AA" + STR(A_Index288 + 1) + "AA", keyWordCodeInTheTranspiledLangStartTS_2 + Chr(10) + programmingBlock_TS[A_Index288] + Chr(10) + keyWordCodeInTheTranspiledLangEndTS_2);
                } else {
                    htCode = StrReplace(htCode, "programmingBlock_TS-programmingBlock_TS-AA" + STR(A_Index288 + 1) + "AA", Chr(10));
                }
            }
        }
    }
    if (langToConvertTo == "groovy") {
        if (COUNT_programmingBlock_GROOVY != 0) {
            for (int A_Index289 = 0; A_Index289 < COUNT_programmingBlock_GROOVY + 0; A_Index289++) {
                htCode = StrReplace(htCode, "programmingBlock_GROOVY-programmingBlock_GROOVY-AA" + STR(A_Index289 + 1) + "AA", programmingBlock_GROOVY[A_Index289]);
            }
        }
    } else {
        if (COUNT_programmingBlock_GROOVY != 0) {
            for (int A_Index290 = 0; A_Index290 < COUNT_programmingBlock_GROOVY + 0; A_Index290++) {
                if (langToConvertTo == langFileExtension_2) {
                    htCode = StrReplace(htCode, "programmingBlock_GROOVY-programmingBlock_GROOVY-AA" + STR(A_Index290 + 1) + "AA", keyWordCodeInTheTranspiledLangStartGROOVY_2 + Chr(10) + programmingBlock_GROOVY[A_Index290] + Chr(10) + keyWordCodeInTheTranspiledLangEndGROOVY_2);
                } else {
                    htCode = StrReplace(htCode, "programmingBlock_GROOVY-programmingBlock_GROOVY-AA" + STR(A_Index290 + 1) + "AA", Chr(10));
                }
            }
        }
    }
    ...
    if (langToConvertTo == langFileExtension_2) {
        if (COUNT_programmingBlock_HTVM != 0) {
            for (int A_Index291 = 0; A_Index291 < COUNT_programmingBlock_HTVM + 0; A_Index291++) {
                htCode = StrReplace(htCode, "programmingBlock_HTVM-programmingBlock_HTVM-AA" + STR(A_Index291 + 1) + "AA", programmingBlock_HTVM[A_Index291]);
            }
        }
    } else {
        if (COUNT_programmingBlock_HTVM != 0) {
            for (int A_Index292 = 0; A_Index292 < COUNT_programmingBlock_HTVM + 0; A_Index292++) {
                htCode = StrReplace(htCode, "programmingBlock_HTVM-programmingBlock_HTVM-AA" + STR(A_Index292 + 1) + "AA", Chr(10));
            }
        }
    }
    if (langToConvertTo != langFileExtension_2) {
        ...
        for (int A_Index293 = 0; A_Index293 < theIdNumOfThe34 + 0; A_Index293++) {
            if (theIdNumOfThe34 == A_Index293 + 1) {
                if (langToConvertTo == langFileExtension_2) {
                    if (keyWordEscpaeChar_2 == "\\" && keyWordEscpaeChar != "\\") {
                        htCode = StrReplace(htCode, "fihuiuuhuuhtheidFor--asds" + str21 + "as--" + str21 + "theuhturtyphoutr--" + Chr(65) + Chr(65) + STR(A_Index293 + 1) + Chr(65) + Chr(65), StrReplace(StrReplace(fString(theIdNumOfThe34theVar[A_Index293 + 1]), "\\", "\\\\"), keyWordEscpaeChar, keyWordEscpaeChar_2) + Chr(34));
                    } else {
                        htCode = StrReplace(htCode, "fihuiuuhuuhtheidFor--asds" + str21 + "as--" + str21 + "theuhturtyphoutr--" + Chr(65) + Chr(65) + STR(A_Index293 + 1) + Chr(65) + Chr(65), StrReplace(fString(theIdNumOfThe34theVar[A_Index293 + 1]), keyWordEscpaeChar, keyWordEscpaeChar_2) + Chr(34));
                    }
                } else {
                    htCode = StrReplace(htCode, "fihuiuuhuuhtheidFor--asds" + str21 + "as--" + str21 + "theuhturtyphoutr--" + Chr(65) + Chr(65) + STR(A_Index293 + 1) + Chr(65) + Chr(65), StrReplace(fString(theIdNumOfThe34theVar[A_Index293 + 1]), keyWordEscpaeChar, "\\") + Chr(34));
                }
            } else {
                if (langToConvertTo == langFileExtension_2) {
                    if (keyWordEscpaeChar_2 == "\\" && keyWordEscpaeChar != "\\") {
                        htCode = StrReplace(htCode, "fihuiuuhuuhtheidFor--asds" + str21 + "as--" + str21 + "theuhturtyphoutr--" + Chr(65) + Chr(65) + STR(A_Index293 + 1) + Chr(65) + Chr(65), StrReplace(StrReplace(fString(theIdNumOfThe34theVar[A_Index293 + 1]), "\\", "\\\\"), keyWordEscpaeChar, keyWordEscpaeChar_2));
                    } else {
                        htCode = StrReplace(htCode, "fihuiuuhuuhtheidFor--asds" + str21 + "as--" + str21 + "theuhturtyphoutr--" + Chr(65) + Chr(65) + STR(A_Index293 + 1) + Chr(65) + Chr(65), StrReplace(fString(theIdNumOfThe34theVar[A_Index293 + 1]), keyWordEscpaeChar, keyWordEscpaeChar_2));
                    }
                } else {
                    htCode = StrReplace(htCode, "fihuiuuhuuhtheidFor--asds" + str21 + "as--" + str21 + "theuhturtyphoutr--" + Chr(65) + Chr(65) + STR(A_Index293 + 1) + Chr(65) + Chr(65), StrReplace(fString(theIdNumOfThe34theVar[A_Index293 + 1]), keyWordEscpaeChar, "\\"));
                }
            }
        }
    }
    ...
    for (size_t A_Index294 = 0; A_Index294 < items294.size() + 0; A_Index294++) {
        ...
        if (areWeInAFuncFromInstructions == 1) {
            if (areWeInAFuncFromInstructionsLineNum == 1) {
                // name of the func
                funcLangHolder = StringTrimLeft(A_LoopField294, 5);
            ...
            if (areWeInAFuncFromInstructionsLineNum == 2) {
                // name of the func
                funcNameHolder = StringTrimLeft(A_LoopField294, 5);
                if (correctLang == 1 && InStr(htCode, Trim(funcNameHolder))) {
            ...
            if (areWeInAFuncFromInstructionsLineNum == 3) {
                // all libs
                funcLibsHolder = StringTrimLeft(A_LoopField294, 5);
                if (correctLang == 1 && InStr(htCode, Trim(funcNameHolder))) {
            ...
            if (areWeInAFuncFromInstructionsLineNum == 4) {
                // func description
                funcDescriptionHolder = StringTrimLeft(A_LoopField294, 12);
                if (correctLang == 1 && InStr(htCode, Trim(funcNameHolder))) {
            ...
            if (areWeInAFuncFromInstructionsLineNum >= 5) {
                // the full func
                if (correctLang == 1 && InStr(htCode, Trim(funcNameHolder))) {
                    funcFuncHolder += A_LoopField294 + Chr(10);
                }
            }
            //MsgBox, % A_LoopField
            areWeInAFuncFromInstructionsLineNum++;
        }
        if (Trim(A_LoopField294) == "func======================func==============") {
    ...
    if (!(HTVM_Size(allFuncNames) <= 0)) {
        for (int A_Index295 = 0; A_Index295 < HTVM_Size(allFuncNames) + 0; A_Index295++) {
            if (InStr(htCode, allFuncNames[A_Index295]) + "(") {
                //MsgBox, % allFuncNames[A_Index]
                allFuncsToPutAtTop += allFuncs[A_Index295] + Chr(10);
                if (Trim(allFuncLibs[A_Index295]) != "null") {
                    allLibsToPutAtTop += allFuncLibs[A_Index295] + "|";
                }
            }
        }
        allLibsToPutAtTop = StringTrimRight(allLibsToPutAtTop, 1);
        std::string allLibsToPutAtTopTEMP = "";
        std::vector<std::string> items296 = LoopParseFunc(allLibsToPutAtTop, "|");
        for (size_t A_Index296 = 0; A_Index296 < items296.size() + 0; A_Index296++) {
            std::string A_LoopField296 = items296[A_Index296 - 0];
            allLibsToPutAtTopTEMP += A_LoopField296 + Chr(10);
        }
    ...
    if (langToConvertTo != langFileExtension_2) {
        for (int A_Index297 = 0; A_Index297 < HTVM_Size(ospDic1) + 0; A_Index297++) {
            str00 = ospDic1[A_Index297];
            if (langToConvertTo == "rb") {
    ...
    if (size_ospDic != 0) {
        for (int A_Index298 = 0; A_Index298 < size_ospDic + 0; A_Index298++) {
            HTVM_Pop(ospDic);
        }
    }
    //;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;
    ...
    if (size_ospDic1 != 0) {
        for (int A_Index299 = 0; A_Index299 < size_ospDic1 + 0; A_Index299++) {
            HTVM_Pop(ospDic1);
        }
    }
    //;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;
    ...
    if (size_ospDic2 != 0) {
        for (int A_Index300 = 0; A_Index300 < size_ospDic2 + 0; A_Index300++) {
            HTVM_Pop(ospDic2);
        }
    }
    availableFuncsInHTVMInHTVM = "";
    saveAllArrayVarNamesSoWeCanDoAfix = "";
    for (int A_Index301 = 0; A_Index301 < theIdNumOfThe34 + 0; A_Index301++) {
        if (theIdNumOfThe34 == A_Index301 + 1) {
            if (langToConvertTo == langFileExtension_2) {
                if (keyWordEscpaeChar_2 == "\\" && keyWordEscpaeChar != "\\") {
                    htCode = StrReplace(htCode, "ihuiuuhuuhtheidFor--asds" + str21 + "as--" + str21 + "theuhturtyphoutr--" + Chr(65) + Chr(65) + STR(A_Index301 + 1) + Chr(65) + Chr(65), StrReplace(StrReplace(theIdNumOfThe34theVar[A_Index301 + 1], "\\", "\\\\"), keyWordEscpaeChar, keyWordEscpaeChar_2) + Chr(34));
                } else {
                    htCode = StrReplace(htCode, "ihuiuuhuuhtheidFor--asds" + str21 + "as--" + str21 + "theuhturtyphoutr--" + Chr(65) + Chr(65) + STR(A_Index301 + 1) + Chr(65) + Chr(65), StrReplace(theIdNumOfThe34theVar[A_Index301 + 1], keyWordEscpaeChar, keyWordEscpaeChar_2) + Chr(34));
                }
            } else {
                htCode = StrReplace(htCode, "ihuiuuhuuhtheidFor--asds" + str21 + "as--" + str21 + "theuhturtyphoutr--" + Chr(65) + Chr(65) + STR(A_Index301 + 1) + Chr(65) + Chr(65), StrReplace(theIdNumOfThe34theVar[A_Index301 + 1], keyWordEscpaeChar, "\\") + Chr(34));
            }
        } else {
            if (langToConvertTo == langFileExtension_2) {
                if (keyWordEscpaeChar_2 == "\\" && keyWordEscpaeChar != "\\") {
                    htCode = StrReplace(htCode, "ihuiuuhuuhtheidFor--asds" + str21 + "as--" + str21 + "theuhturtyphoutr--" + Chr(65) + Chr(65) + STR(A_Index301 + 1) + Chr(65) + Chr(65), StrReplace(StrReplace(theIdNumOfThe34theVar[A_Index301 + 1], "\\", "\\\\"), keyWordEscpaeChar, keyWordEscpaeChar_2));
                } else {
                    htCode = StrReplace(htCode, "ihuiuuhuuhtheidFor--asds" + str21 + "as--" + str21 + "theuhturtyphoutr--" + Chr(65) + Chr(65) + STR(A_Index301 + 1) + Chr(65) + Chr(65), StrReplace(theIdNumOfThe34theVar[A_Index301 + 1], keyWordEscpaeChar, keyWordEscpaeChar_2));
                }
            } else {
                htCode = StrReplace(htCode, "ihuiuuhuuhtheidFor--asds" + str21 + "as--" + str21 + "theuhturtyphoutr--" + Chr(65) + Chr(65) + STR(A_Index301 + 1) + Chr(65) + Chr(65), StrReplace(theIdNumOfThe34theVar[A_Index301 + 1], keyWordEscpaeChar, "\\"));
            }
        }
    }
    if (langToConvertTo == langFileExtension_2) {
    ...
    if (isNotHTVMfile2 == 0) {
        ...
        if (langToConvertTo == langFileExtension_2) {
            for (int A_Index302 = 0; A_Index302 < HTVM_Size(weAreInMLSarr) + 0; A_Index302++) {
                htCode = StrReplace(htCode, "aejsoydoxubviycbdgsut7eoy-ihsxbvhowadesdio6376e7wuwyuau--szd--AA" + STR(A_Index302) + "AA", weAreInMLSarr[A_Index302]);
            }
    ...
    if (size_weAreInMLSarr != 0) {
        for (int A_Index303 = 0; A_Index303 < size_weAreInMLSarr + 0; A_Index303++) {
...
void HTVMv2() {
    ...
    if (HTVM_getLang_HTVM() == "cpp" || HTVM_getLang_HTVM() == "py") {
        ...
        std::vector<std::string> items305 = LoopParseFunc(allArgs, "\n", "\r");
        for (size_t A_Index305 = 0; A_Index305 < items305.size() + 0; A_Index305++) {
            std::string A_LoopField305 = items305[A_Index305 - 0];
            if (A_Index305 == 0) {
                numOfParams++;
                argCODE = FileRead(Trim(A_LoopField305));
                argCODEfile = Trim(A_LoopField305);
            }
            else if (A_Index305 == 1) {
                numOfParams++;
                argHTVMinstr = Trim(A_LoopField305);
            }
            else if (A_Index305 == 2) {
                numOfParams++;
                argLangTo = Trim(A_LoopField305);
            } else {
                numOfParams++;
                HTVM_Append(argHTVMinstrMORE, Trim(A_LoopField305));
            }
        }
        //print("===============123431234===========start=====")
        for (int A_Index306 = 0; A_Index306 < HTVM_Size(argHTVMinstrMORE) + 0; A_Index306++) {
