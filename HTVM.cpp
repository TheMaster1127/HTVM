...
std::string compiler(std::string htCode, std::string allInstructionFile, std::string mode, std::string langToConvertToParam = "") {
    ...
    */
    ...
    if (isNotHTVMfile == 0) {
        if (langToConvertTo != langFileExtension_2) {
            if (InStr(StrLower(code), StrLower(keyWordInclude + " "))) {
                for (int A_Index77 = 0; A_Index77 < 10000 + 0; A_Index77++) {
                    doseCodeHaveLibs = 0;
                    allLibCODE = "";
                    if (A_Index77 == 0) {
                    ...
                    for (int A_Index81 = 0; A_Index81 < HTVM_Size(allLibsWeSee) + 0; A_Index81++) {
                        ...
                        for (int A_Index82 = 0; A_Index82 < HTVM_Size(allLibsWeSeeSEE) - HTVM_Size(allLibsWeSee) + 0; A_Index82++) {
    ...
    if (langToConvertTo == "go") {
        ...
        for (size_t A_Index148 = 0; A_Index148 < items148.size() + 0; A_Index148++) {
            ...
            if (Trim(A_LoopField148) == "}" && SubStr(Trim(fixGoManGoIsSoAnnoyingBroooFurure[A_Index148 + 1]), 1, StrLen("else if ")) == "else if ") {
                ...
                std::vector<std::string> items149 = LoopParseFunc(fixGoManGoIsSoAnnoyingBroooFurure[A_Index148 + 1]);
                ...
                for (int A_Index150 = 0; A_Index150 < fixGoManGoIsSoAnnoyingBroooCount + 0; A_Index150++) {
                    fixGoManGoIsSoAnnoyingBroooCountSpaceses += " ";
                }
    ...
    if (COUNT_programmingBlock_InTheTranspiledLang != 0) {
        for (int A_Index151 = 0; A_Index151 < COUNT_programmingBlock_InTheTranspiledLang + 0; A_Index151++) {
    ...
    if (langToConvertTo == langFileExtension_2) {
        ...
        if (COUNT_programmingBlock_HTVM != 0) {
            for (int A_Index183 = 0; A_Index183 < COUNT_programmingBlock_HTVM + 0; A_Index183++) {
    ...
    int correctLang = 0;
    std::vector<std::string> items184 = LoopParseFunc(instructions, "\n", "\r");
    ...
    if (!(HTVM_Size(allFuncNames) <= 0)) {
        for (int A_Index185 = 0; A_Index185 < HTVM_Size(allFuncNames) + 0; A_Index185++) {
        ...
        std::vector<std::string> items186 = LoopParseFunc(allLibsToPutAtTop, "|");
...
void HTVMv2() {
    ...
    std::vector<std::string> items188 = LoopParseFunc(str0);
    ...
    if (HTVM_getLang_HTVM() == "cpp" || HTVM_getLang_HTVM() == "py") {
        ...
        std::vector<std::string> items189 = LoopParseFunc(allArgs, "\n", "\r");
        ...
        for (int A_Index190 = 0; A_Index190 < HTVM_Size(argHTVMinstrMORE) + 0; A_Index190++) {
