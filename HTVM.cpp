...
std::string splitFunctionCalls(std::string line) {
    ...
    for (size_t A_Index76 = 0; A_Index76 < items76.size() + 0; A_Index76++) {
        ...
        else if (A_LoopField76 == "(" && seenDot == 1) {
            parenthesesDepth++;
            temp += A_LoopField76;
        }
        else if (A_LoopField76 == ")" && seenDot == 1) {
    ...
    for (int A_Index77 = 0; A_Index77 < HTVM_Size(funcCalls) + 0; A_Index77++) {
        if (RegExMatch(funcCalls[A_Index77], "\\.[a-zA-Z_][a-zA-Z0-9_]*")) {
            arrToStrSTR += extraFlexableFuncCallsHelp(funcCalls[A_Index77]) + " ";
        } else {
            arrToStrSTR += funcCalls[A_Index77] + " ";
        }
    }
    arrToStrSTR = StringTrimRight(arrToStrSTR, 1);
    if (doWeAddParentisiesLater != 0) {
        for (int A_Index78 = 0; A_Index78 < doWeAddParentisiesLater + 0; A_Index78++) {
            arrToStrSTR = "(" + arrToStrSTR + ")";
        }
    }
    if (doseBracketsExist == 1) {
        for (int A_Index79 = 0; A_Index79 < HTVM_Size(saveDoseBracketsExist) + 0; A_Index79++) {
            arrToStrSTR = StrReplace(arrToStrSTR, "brackests--rpelacse-brackets--HTVM-new-BUG--AA" + STR(A_Index79) + "AA", StringTrimLeft(StringTrimRight(saveDoseBracketsExist[A_Index79], 1), 1));
        }
    }
    return arrToStrSTR;
}
std::string extraFlexableFuncCalls(std::string line) {
    std::string out = line;
...
std::string expressionParserTranspiler(std::string expression) {
    ...
    // osp
    if (langToConvertTo != langFileExtension_2) {
        for (int A_Index80 = 0; A_Index80 < HTVM_Size(ospDic1) + 0; A_Index80++) {
            str00 = ospDic1[A_Index80];
            expression = StrReplace(expression, StrReplace(str00, "_", ".") + ".", str00 + "_");
        }
        for (int A_Index81 = 0; A_Index81 < HTVM_Size(ospDic2) + 0; A_Index81++) {
            str00 = ospDic2[A_Index81];
    ...
    for (int A_Index82 = 0; A_Index82 < 29 + 0; A_Index82++) {
        if (InStr(expression, fullLangAllOperators_HELP[A_Index82])) {
            if (A_Index82 == 0 || A_Index82 == 1 || A_Index82 == 2) {
                expression = RegExReplace(expression, "\\b" + fullLangAllOperators_HELP[A_Index82] + "\\b", fullLangAllOperators[A_Index82]);
            } else {
                if (A_Index82 == 19) {
                    if (langToConvertTo == "py" || langToConvertTo == "lua" || langToConvertTo == "nim") {
                        expression = RegExReplace(expression, "\\b" + fullLangAllOperators_HELP[A_Index82] + "\\b", "not");
                    } else {
                        expression = RegExReplace(expression, "\\b" + fullLangAllOperators_HELP[A_Index82] + "\\b", "!");
                    }
                    if (langToConvertTo == "py" || langToConvertTo == "lua" || langToConvertTo == "nim") {
                    ...
                    expression = StrReplace(expression, " " + fullLangAllOperators_HELP[A_Index82] + " ", " " + fullLangAllOperators[A_Index82] + " ");
                    if (A_Index82 == 28) {
                        if (langToConvertTo == "py") {
    ...
    if (RegExMatch(expression, "\\.[a-zA-Z_][a-zA-Z0-9_]*") && langToConvertTo != langFileExtension_2) {
        expression = extraFlexableFuncCalls(expression);
    }
...
std::string FixRubyGlobalVars(std::string line) {
    ...
    for (size_t A_Index83 = 0; A_Index83 < items83.size() + 0; A_Index83++) {
        ...
        if (RegExMatch(A_LoopField83, "^[a-zA-Z_][a-zA-Z0-9_]* = .*")) {
            HTVM_Append(htCodeFixRubyGlobalVars, Trim(StrSplit(A_LoopField83, " = ", 1)));
        }
    }
    for (int A_Index84 = 0; A_Index84 < HTVM_Size(htCodeFixRubyGlobalVars) + 0; A_Index84++) {
        line = RegExReplace(line, "\\b" + htCodeFixRubyGlobalVars[A_Index84] + "\\b", "$" + htCodeFixRubyGlobalVars[A_Index84]);
    }
    return line;
}
//;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;
...
std::string propHELP(std::string line, std::string lineOspHelpLine) {
    ...
    for (size_t A_Index85 = 0; A_Index85 < items85.size() + 0; A_Index85++) {
        ...
        if (InStr(A_LoopField85, " " + keyWordAssign + " ") || InStr(A_LoopField85, " " + keyWordAssignAdd + " ") || InStr(A_LoopField85, " " + keyWordAssignConcat + " ") || InStr(A_LoopField85, " " + keyWordAssignSub + " ") || InStr(A_LoopField85, " " + keyWordAssignMul + " ") || InStr(A_LoopField85, " " + keyWordAssignDiv + " ") || InStr(A_LoopField85, " " + keyWordAssignMod + " ") || InStr(A_LoopField85, " " + keyWordAssignShiftLeft + " ") || InStr(A_LoopField85, " " + keyWordAssignShiftRight + " ") || InStr(A_LoopField85, " " + keyWordLogicalAssignShiftRight + " ") || InStr(A_LoopField85, " " + keyWordAssignBitAnd + " ") || InStr(A_LoopField85, " " + keyWordAssignBitOr + " ") || InStr(A_LoopField85, " " + keyWordAssignBitXor + " ") && lineDone == 0) {
            ...
            if (InStr(A_LoopField85, " " + keyWordAssign + " ")) {
                str1 = StrSplit(A_LoopField85, " " + keyWordAssign + " ", 1);
                str6 = StrSplit(A_LoopField85, " " + keyWordAssign + " ", 2);
                str10 = keyWordAssign;
            ...
            else if (InStr(A_LoopField85, " " + keyWordAssignAdd + " ")) {
                str1 = StrSplit(A_LoopField85, " " + keyWordAssignAdd + " ", 1);
                str6 = StrSplit(A_LoopField85, " " + keyWordAssignAdd + " ", 2);
                str10 = keyWordAssignAdd;
                str11 = keyWordAssignAdd_2;
                str12 = "+=";
            }
            else if (InStr(A_LoopField85, " " + keyWordAssignConcat + " ")) {
                str1 = StrSplit(A_LoopField85, " " + keyWordAssignConcat + " ", 1);
                str6 = StrSplit(A_LoopField85, " " + keyWordAssignConcat + " ", 2);
            ...
            else if (InStr(A_LoopField85, " " + keyWordAssignSub + " ")) {
                str1 = StrSplit(A_LoopField85, " " + keyWordAssignSub + " ", 1);
                str6 = StrSplit(A_LoopField85, " " + keyWordAssignSub + " ", 2);
                str10 = keyWordAssignSub;
                str11 = keyWordAssignSub_2;
                str12 = "-=";
            }
            else if (InStr(A_LoopField85, " " + keyWordAssignMul + " ")) {
                str1 = StrSplit(A_LoopField85, " " + keyWordAssignMul + " ", 1);
                str6 = StrSplit(A_LoopField85, " " + keyWordAssignMul + " ", 2);
                str10 = keyWordAssignMul;
                str11 = keyWordAssignMul_2;
                str12 = "*=";
            }
            else if (InStr(A_LoopField85, " " + keyWordAssignDiv + " ")) {
                str1 = StrSplit(A_LoopField85, " " + keyWordAssignDiv + " ", 1);
                str6 = StrSplit(A_LoopField85, " " + keyWordAssignDiv + " ", 2);
                str10 = keyWordAssignDiv;
                str11 = keyWordAssignDiv_2;
                str12 = "/=";
            }
            else if (InStr(A_LoopField85, " " + keyWordAssignMod + " ")) {
                str1 = StrSplit(A_LoopField85, " " + keyWordAssignMod + " ", 1);
                str6 = StrSplit(A_LoopField85, " " + keyWordAssignMod + " ", 2);
                str10 = keyWordAssignMod;
                str11 = keyWordAssignMod_2;
                str12 = "%=";
            }
            else if (InStr(A_LoopField85, " " + keyWordAssignShiftLeft + " ")) {
                str1 = StrSplit(A_LoopField85, " " + keyWordAssignShiftLeft + " ", 1);
                str6 = StrSplit(A_LoopField85, " " + keyWordAssignShiftLeft + " ", 2);
                str10 = keyWordAssignShiftLeft;
                str11 = keyWordAssignShiftLeft_2;
                str12 = "<<=";
            }
            else if (InStr(A_LoopField85, " " + keyWordAssignShiftRight + " ")) {
                str1 = StrSplit(A_LoopField85, " " + keyWordAssignShiftRight + " ", 1);
                str6 = StrSplit(A_LoopField85, " " + keyWordAssignShiftRight + " ", 2);
                str10 = keyWordAssignShiftRight;
                str11 = keyWordAssignShiftRight_2;
                str12 = ">>=";
            }
            else if (InStr(A_LoopField85, " " + keyWordLogicalAssignShiftRight + " ")) {
                str1 = StrSplit(A_LoopField85, " " + keyWordLogicalAssignShiftRight + " ", 1);
                str6 = StrSplit(A_LoopField85, " " + keyWordLogicalAssignShiftRight + " ", 2);
                str10 = keyWordLogicalAssignShiftRight;
                str11 = keyWordLogicalAssignShiftRight_2;
                str12 = ">>>=";
            }
            else if (InStr(A_LoopField85, " " + keyWordAssignBitAnd + " ")) {
                str1 = StrSplit(A_LoopField85, " " + keyWordAssignBitAnd + " ", 1);
                str6 = StrSplit(A_LoopField85, " " + keyWordAssignBitAnd + " ", 2);
                str10 = keyWordAssignBitAnd;
                str11 = keyWordAssignBitAnd_2;
                str12 = "&=";
            }
            else if (InStr(A_LoopField85, " " + keyWordAssignBitOr + " ")) {
                str1 = StrSplit(A_LoopField85, " " + keyWordAssignBitOr + " ", 1);
                str6 = StrSplit(A_LoopField85, " " + keyWordAssignBitOr + " ", 2);
                str10 = keyWordAssignBitOr;
                str11 = keyWordAssignBitOr_2;
                str12 = "|=";
            }
            else if (InStr(A_LoopField85, " " + keyWordAssignBitXor + " ")) {
                str1 = StrSplit(A_LoopField85, " " + keyWordAssignBitXor + " ", 1);
                str6 = StrSplit(A_LoopField85, " " + keyWordAssignBitXor + " ", 2);
            ...
            if (InStr(ALoopField, " ") == false) {
                ...
                if (usePostfixTypeForTypeDefinition == "off") {
                    ...
                    std::vector<std::string> items86 = LoopParseFunc(str1, " ");
                ...
                if (usePostfixTypeForTypeDefinition == "off") {
                    ...
                    std::vector<std::string> items87 = LoopParseFunc(str1, " ");
                ...
                if (usePostfixTypeForTypeDefinition == "off") {
                    ...
                    std::vector<std::string> items88 = LoopParseFunc(str1, " ");
                ...
                lineDone = 1;
                std::vector<std::string> items89 = LoopParseFunc(str1, " ");
        ...
        else if (SubStr(Trim(A_LoopField85), 1, StrLen(keyWordVar)) == keyWordVar && lineDone == 0) {
            allVarsSoWeDontReDecVars_FIX_TOGGLE = 1;
            if (usePostfixTypeForTypeDefinition == "off") {
                lineDone = 1;
                str1 = Trim(StringTrimLeft(Trim(A_LoopField85), StrLen(keyWordVar)));
        ...
        else if (SubStr(Trim(A_LoopField85), 1, StrLen(keyWordLet)) == keyWordLet && lineDone == 0) {
            allVarsSoWeDontReDecVars_FIX_TOGGLE = 1;
            if (usePostfixTypeForTypeDefinition == "off") {
                lineDone = 1;
                str1 = Trim(StringTrimLeft(Trim(A_LoopField85), StrLen(keyWordLet)));
                if (SubStrLastChars(str1, 1) == ";") {
                    str1 = StringTrimRight(str1, 1);
                }
        ...
        else if (SubStr(Trim(A_LoopField85), 1, StrLen(keyWordDouble + " ")) == keyWordDouble + " " || SubStr(Trim(A_LoopField85), 1, StrLen(keyWordChar + " ")) == keyWordChar + " " || SubStr(Trim(A_LoopField85), 1, StrLen(keyWordUint8 + " ")) == keyWordUint8 + " " || SubStr(Trim(A_LoopField85), 1, StrLen(keyWordUint16 + " ")) == keyWordUint16 + " " || SubStr(Trim(A_LoopField85), 1, StrLen(keyWordUint32 + " ")) == keyWordUint32 + " " || SubStr(Trim(A_LoopField85), 1, StrLen(keyWordUint64 + " ")) == keyWordUint64 + " " || SubStr(Trim(A_LoopField85), 1, StrLen(keyWordINT + " ")) == keyWordINT + " " || SubStr(Trim(A_LoopField85), 1, StrLen(keyWordSTR + " ")) == keyWordSTR + " " || SubStr(Trim(A_LoopField85), 1, StrLen(keyWordBOOL + " ")) == keyWordBOOL + " " || SubStr(Trim(A_LoopField85), 1, StrLen(keyWordFLOAT + " ")) == keyWordFLOAT + " " || SubStr(Trim(A_LoopField85), 1, StrLen(keyWordINT8 + " ")) == keyWordINT8 + " " || SubStr(Trim(A_LoopField85), 1, StrLen(keyWordINT16 + " ")) == keyWordINT16 + " " || SubStr(Trim(A_LoopField85), 1, StrLen(keyWordINT32 + " ")) == keyWordINT32 + " " || SubStr(Trim(A_LoopField85), 1, StrLen(keyWordINT64 + " ")) == keyWordINT64 + " " || SubStr(Trim(A_LoopField85), 1, StrLen(keyWordArrayDefinition + " ")) == keyWordArrayDefinition + " " || SubStr(Trim(A_LoopField85), 1, StrLen(keyWordArrayOfIntegersDefinition + " ")) == keyWordArrayOfIntegersDefinition + " " || SubStr(Trim(A_LoopField85), 1, StrLen(keyWordArrayOfStringsDefinition + " ")) == keyWordArrayOfStringsDefinition + " " || SubStr(Trim(A_LoopField85), 1, StrLen(keyWordArrayOfFloatingPointNumbersDefinition + " ")) == keyWordArrayOfFloatingPointNumbersDefinition + " " || SubStr(Trim(A_LoopField85), 1, StrLen(keyWordArrayOfBooleansDefinition + " ")) == keyWordArrayOfBooleansDefinition + " " && usePrefixTypeForTypeDefinition == "on" && lineDone == 0) {
            lineDone = 1;
            allVarsSoWeDontReDecVars_FIX_TOGGLE = 1;
            str1 = Trim(A_LoopField85);
            if (SubStrLastChars(str1, 1) == ";") {
                str1 = StringTrimRight(str1, 1);
            }
        ...
        else if (RegExMatch(Trim(A_LoopField85), "^[a-zA-Z_][a-zA-Z0-9_]*: [^:]*;?$") && usePostfixTypeForTypeDefinition == "on" && lineDone == 0) {
            lineDone = 1;
            allVarsSoWeDontReDecVars_FIX_TOGGLE = 1;
            str1 = Trim(A_LoopField85);
            if (SubStrLastChars(str1, 1) == ";") {
            ...
            htCodeOUTOUT += A_LoopField85 + Chr(10);
        }
    }
...
std::vector<std::string> slidingWinFixCommentsRmFUNC(std::string line) {
    ...
    std::vector<std::string> items93 = LoopParseFunc(line);
    for (size_t A_Index93 = 0; A_Index93 < items93.size() + 0; A_Index93++) {
        std::string A_LoopField93 = items93[A_Index93 - 0];
        HTVM_Append(slidingWinFixCommentsRmFFFF, A_LoopField93);
    }
    for (int A_Index94 = 0; A_Index94 < StrLen(keyWordComment + keyWordComment) + 2 + 0; A_Index94++) {
...
std::string compiler(std::string htCode, std::string allInstructionFile, std::string mode, std::string langToConvertToParam = "") {
    ...
    std::vector<std::string> items95 = LoopParseFunc(allInstructionFile, "\n", "\r");
    for (size_t A_Index95 = 0; A_Index95 < items95.size() + 0; A_Index95++) {
        std::string A_LoopField95 = items95[A_Index95 - 0];
        if (A_Index95 == 0) {
            langToConvertTo = Trim(A_LoopField95);
        }
        if (A_Index95 == 1) {
            langFileExtension = Trim(A_LoopField95);
        }
        if (A_Index95 == 2) {
            commands = Trim(A_LoopField95);
        }
        if (A_Index95 == 3) {
            keyWordAlliance = Trim(A_LoopField95) + " ";
        }
        if (A_Index95 == 4) {
            keyWordCrew = Trim(A_LoopField95) + " ";
        }
        if (A_Index95 == 5) {
            keyWordMethod = Trim(A_LoopField95) + " ";
        }
        if (A_Index95 == 6) {
            keyWordDefObj = Trim(A_LoopField95) + " ";
        }
        if (A_Index95 == 7) {
            keyWordProp = Trim(A_LoopField95) + " ";
        }
        if (A_Index95 == 8) {
            keyWordThis = Trim(A_LoopField95);
        }
        if (A_Index95 == 9) {
            keyWordInclude = Trim(A_LoopField95);
        }
        if (A_Index95 == 10) {
            keyWordCodeInTheTranspiledLangStart = Trim(A_LoopField95);
        }
        if (A_Index95 == 11) {
            keyWordCodeInTheTranspiledLangEnd = Trim(A_LoopField95);
        }
        if (A_Index95 == 12) {
            keyWordCodeInTheTranspiledLangStartCPP = Trim(A_LoopField95);
        }
        if (A_Index95 == 13) {
            keyWordCodeInTheTranspiledLangEndCPP = Trim(A_LoopField95);
        }
        if (A_Index95 == 14) {
            keyWordCodeInTheTranspiledLangStartPY = Trim(A_LoopField95);
        }
        if (A_Index95 == 15) {
            keyWordCodeInTheTranspiledLangEndPY = Trim(A_LoopField95);
        }
        if (A_Index95 == 16) {
            keyWordCodeInTheTranspiledLangStartJS = Trim(A_LoopField95);
        }
        if (A_Index95 == 17) {
            keyWordCodeInTheTranspiledLangEndJS = Trim(A_LoopField95);
        }
        if (A_Index95 == 18) {
            keyWordCodeInTheTranspiledLangStartGO = Trim(A_LoopField95);
        }
        if (A_Index95 == 19) {
            keyWordCodeInTheTranspiledLangEndGO = Trim(A_LoopField95);
        }
        if (A_Index95 == 20) {
            keyWordCodeInTheTranspiledLangStartLUA = Trim(A_LoopField95);
        }
        if (A_Index95 == 21) {
            keyWordCodeInTheTranspiledLangEndLUA = Trim(A_LoopField95);
        }
        if (A_Index95 == 22) {
            keyWordCodeInTheTranspiledLangStartCS = Trim(A_LoopField95);
        }
        if (A_Index95 == 23) {
            keyWordCodeInTheTranspiledLangEndCS = Trim(A_LoopField95);
        }
        if (A_Index95 == 24) {
            keyWordCodeInTheTranspiledLangStartJAVA = Trim(A_LoopField95);
        }
        if (A_Index95 == 25) {
            keyWordCodeInTheTranspiledLangEndJAVA = Trim(A_LoopField95);
        }
        if (A_Index95 == 26) {
            keyWordCodeInTheTranspiledLangStartKT = Trim(A_LoopField95);
        }
        if (A_Index95 == 27) {
            keyWordCodeInTheTranspiledLangEndKT = Trim(A_LoopField95);
        }
        if (A_Index95 == 28) {
            keyWordCodeInTheTranspiledLangStartRB = Trim(A_LoopField95);
        }
        if (A_Index95 == 29) {
            keyWordCodeInTheTranspiledLangEndRB = Trim(A_LoopField95);
        }
        if (A_Index95 == 30) {
            keyWordCodeInTheTranspiledLangStartNIM = Trim(A_LoopField95);
        }
        if (A_Index95 == 31) {
            keyWordCodeInTheTranspiledLangEndNIM = Trim(A_LoopField95);
        }
        if (A_Index95 == 32) {
            keyWordCodeInTheTranspiledLangStartAHK = Trim(A_LoopField95);
        }
        if (A_Index95 == 33) {
            keyWordCodeInTheTranspiledLangEndAHK = Trim(A_LoopField95);
        }
        if (A_Index95 == 34) {
            keyWordCodeInTheTranspiledLangStartSWIFT = Trim(A_LoopField95);
        }
        if (A_Index95 == 35) {
            keyWordCodeInTheTranspiledLangEndSWIFT = Trim(A_LoopField95);
        }
        if (A_Index95 == 36) {
            keyWordCodeInTheTranspiledLangStartDART = Trim(A_LoopField95);
        }
        if (A_Index95 == 37) {
            keyWordCodeInTheTranspiledLangEndDART = Trim(A_LoopField95);
        }
        if (A_Index95 == 38) {
            keyWordCodeInTheTranspiledLangStartTS = Trim(A_LoopField95);
        }
        if (A_Index95 == 39) {
            keyWordCodeInTheTranspiledLangEndTS = Trim(A_LoopField95);
        }
        if (A_Index95 == 40) {
            keyWordCodeInTheTranspiledLangStartGROOVY = Trim(A_LoopField95);
        }
        if (A_Index95 == 41) {
            keyWordCodeInTheTranspiledLangEndGROOVY = Trim(A_LoopField95);
        }
        if (A_Index95 == 42) {
            keyWordCodeInTheTranspiledLangStartHTVM = Trim(A_LoopField95);
        }
        if (A_Index95 == 43) {
            keyWordCodeInTheTranspiledLangEndHTVM = Trim(A_LoopField95);
        }
        if (A_Index95 == 44) {
            keyWordCodeInHTVMstart = Trim(A_LoopField95);
        }
        if (A_Index95 == 45) {
            keyWordCodeInHTVMend = Trim(A_LoopField95);
        }
        if (A_Index95 == 46) {
            keyWordCurlyBraceOpen = Trim(A_LoopField95);
        }
        if (A_Index95 == 47) {
            keyWordCurlyBraceClose = Trim(A_LoopField95);
        }
        if (A_Index95 == 48) {
            keyWordNull = Trim(A_LoopField95);
        }
        if (A_Index95 == 49) {
            keyWordTrue = Trim(A_LoopField95);
        }
        if (A_Index95 == 50) {
            keyWordFalse = Trim(A_LoopField95);
        }
        if (A_Index95 == 51) {
            keyWordVoid = Trim(A_LoopField95);
        }
        if (A_Index95 == 52) {
            keyWordDouble = Trim(A_LoopField95);
        }
        if (A_Index95 == 53) {
            keyWordChar = Trim(A_LoopField95);
        }
        if (A_Index95 == 54) {
            keyWordUint8 = Trim(A_LoopField95);
        }
        if (A_Index95 == 55) {
            keyWordUint16 = Trim(A_LoopField95);
        }
        if (A_Index95 == 56) {
            keyWordUint32 = Trim(A_LoopField95);
        }
        if (A_Index95 == 57) {
            keyWordUint64 = Trim(A_LoopField95);
        }
        if (A_Index95 == 58) {
            keyWordINT = Trim(A_LoopField95);
        }
        if (A_Index95 == 59) {
            keyWordSTR = Trim(A_LoopField95);
        }
        if (A_Index95 == 60) {
            keyWordBOOL = Trim(A_LoopField95);
        }
        if (A_Index95 == 61) {
            keyWordFLOAT = Trim(A_LoopField95);
        }
        if (A_Index95 == 62) {
            keyWordINT8 = Trim(A_LoopField95);
        }
        if (A_Index95 == 63) {
            keyWordINT16 = Trim(A_LoopField95);
        }
        if (A_Index95 == 64) {
            keyWordINT32 = Trim(A_LoopField95);
        }
        if (A_Index95 == 65) {
            keyWordINT64 = Trim(A_LoopField95);
        }
        if (A_Index95 == 66) {
            keyWordIF = Trim(A_LoopField95) + " ";
        }
        if (A_Index95 == 67) {
            keyWordElseIf = Trim(A_LoopField95) + " ";
        }
        if (A_Index95 == 68) {
            keyWordElse = Trim(A_LoopField95);
        }
        if (A_Index95 == 69) {
            keyWordWhileLoop = Trim(A_LoopField95) + " ";
        }
        if (A_Index95 == 70) {
            keyWordLoopInfinite = Trim(A_LoopField95);
        }
        if (A_Index95 == 71) {
            keyWordLoop = Trim(A_LoopField95) + " ";
        }
        if (A_Index95 == 72) {
            keyWordLoopParse = Trim(A_LoopField95) + " ";
        }
        if (A_Index95 == 73) {
            keyWordContinue = Trim(A_LoopField95);
        }
        if (A_Index95 == 74) {
            keyWordBreak = Trim(A_LoopField95);
        }
        if (A_Index95 == 75) {
            keyWordFunc = Trim(A_LoopField95) + " ";
        }
        if (A_Index95 == 76) {
            keyWordAwait = Trim(A_LoopField95) + " ";
        }
        if (A_Index95 == 77) {
            keyWordAsync = Trim(A_LoopField95) + " ";
        }
        if (A_Index95 == 78) {
            keyWordThrow = Trim(A_LoopField95) + " ";
        }
        if (A_Index95 == 79) {
            keyWordErrorMsg = Trim(A_LoopField95);
        }
        if (A_Index95 == 80) {
            keyWordTry = Trim(A_LoopField95);
        }
        if (A_Index95 == 81) {
            keyWordCatch = Trim(A_LoopField95) + " ";
        }
        if (A_Index95 == 82) {
            keyWordFinally = Trim(A_LoopField95);
        }
        if (A_Index95 == 83) {
            keyWordReturnStatement = Trim(A_LoopField95) + " ";
        }
        if (A_Index95 == 84) {
            keyWordArrayAppend = Trim(A_LoopField95);
        }
        if (A_Index95 == 85) {
            keyWordArrayPop = Trim(A_LoopField95);
        }
        if (A_Index95 == 86) {
            keyWordArraySize = Trim(A_LoopField95);
        }
        if (A_Index95 == 87) {
            keyWordArrayInsert = Trim(A_LoopField95);
        }
        if (A_Index95 == 88) {
            keyWordArrayRemove = Trim(A_LoopField95);
        }
        if (A_Index95 == 89) {
            keyWordArrayIndexOf = Trim(A_LoopField95);
        }
        if (A_Index95 == 90) {
            keyWordArrayDefinition = Trim(A_LoopField95);
        }
        if (A_Index95 == 91) {
            keyWordArrayOfIntegersDefinition = Trim(A_LoopField95);
        }
        if (A_Index95 == 92) {
            keyWordArrayOfStringsDefinition = Trim(A_LoopField95);
        }
        if (A_Index95 == 93) {
            keyWordArrayOfFloatingPointNumbersDefinition = Trim(A_LoopField95);
        }
        if (A_Index95 == 94) {
            keyWordArrayOfBooleansDefinition = Trim(A_LoopField95);
        }
        if (A_Index95 == 95) {
            keyWordVar = Trim(A_LoopField95) + " ";
        }
        if (A_Index95 == 96) {
            keyWordLet = Trim(A_LoopField95) + " ";
        }
        if (A_Index95 == 97) {
            keyWordConst = Trim(A_LoopField95) + " ";
        }
        if (A_Index95 == 98) {
            keyWordEnd = Trim(A_LoopField95);
        }
        if (A_Index95 == 99) {
            keyWordGlobal = Trim(A_LoopField95) + " ";
        }
        if (A_Index95 == 100) {
            keyWordComment = Trim(A_LoopField95);
        }
        if (A_Index95 == 101) {
            keyWordCommentOpenMultiLine = Trim(A_LoopField95);
        }
        if (A_Index95 == 102) {
            keyWordCommentCloseMultiLine = Trim(A_LoopField95);
        }
        if (A_Index95 == 103) {
            keyWordEscpaeChar = Trim(A_LoopField95);
        }
        if (A_Index95 == 104) {
            keyWordMainLabel = Trim(A_LoopField95);
        }
        if (A_Index95 == 105) {
            keyWordConcat = Trim(A_LoopField95);
        }
        if (A_Index95 == 106) {
            keyWordAdd = Trim(A_LoopField95);
        }
        if (A_Index95 == 107) {
            keyWordSub = Trim(A_LoopField95);
        }
        if (A_Index95 == 108) {
            keyWordMul = Trim(A_LoopField95);
        }
        if (A_Index95 == 109) {
            keyWordDiv = Trim(A_LoopField95);
        }
        if (A_Index95 == 110) {
            keyWordMod = Trim(A_LoopField95);
        }
        if (A_Index95 == 111) {
            keyWordExp = Trim(A_LoopField95);
        }
        if (A_Index95 == 112) {
            keyWordEqual = Trim(A_LoopField95);
        }
        if (A_Index95 == 113) {
            keyWordStrictEqual = Trim(A_LoopField95);
        }
        if (A_Index95 == 114) {
            keyWordNotEqual = Trim(A_LoopField95);
        }
        if (A_Index95 == 115) {
            keyWordGreater = Trim(A_LoopField95);
        }
        if (A_Index95 == 116) {
            keyWordLess = Trim(A_LoopField95);
        }
        if (A_Index95 == 117) {
            keyWordGreaterEqual = Trim(A_LoopField95);
        }
        if (A_Index95 == 118) {
            keyWordLessEqual = Trim(A_LoopField95);
        }
        if (A_Index95 == 119) {
            keyWordAnd = Trim(A_LoopField95);
        }
        if (A_Index95 == 120) {
            keyWordOr = Trim(A_LoopField95);
        }
        if (A_Index95 == 121) {
            keyWordNot = Trim(A_LoopField95);
        }
        if (A_Index95 == 122) {
            keyWordBitAnd = Trim(A_LoopField95);
        }
        if (A_Index95 == 123) {
            keyWordBitOr = Trim(A_LoopField95);
        }
        if (A_Index95 == 124) {
            keyWordBitXor = Trim(A_LoopField95);
        }
        if (A_Index95 == 125) {
            keyWordBitNot = Trim(A_LoopField95);
        }
        if (A_Index95 == 126) {
            keyWordShiftLeft = Trim(A_LoopField95);
        }
        if (A_Index95 == 127) {
            keyWordShiftRight = Trim(A_LoopField95);
        }
        if (A_Index95 == 128) {
            keyWordShiftUnsignedRight = Trim(A_LoopField95);
        }
        if (A_Index95 == 129) {
            keyWordAssign = Trim(A_LoopField95);
        }
        if (A_Index95 == 130) {
            keyWordAssignAdd = Trim(A_LoopField95);
        }
        if (A_Index95 == 131) {
            keyWordAssignConcat = Trim(A_LoopField95);
        }
        if (A_Index95 == 132) {
            keyWordAssignSub = Trim(A_LoopField95);
        }
        if (A_Index95 == 133) {
            keyWordAssignMul = Trim(A_LoopField95);
        }
        if (A_Index95 == 134) {
            keyWordAssignDiv = Trim(A_LoopField95);
        }
        if (A_Index95 == 135) {
            keyWordAssignMod = Trim(A_LoopField95);
        }
        if (A_Index95 == 136) {
            keyWordAssignShiftLeft = Trim(A_LoopField95);
        }
        if (A_Index95 == 137) {
            keyWordAssignShiftRight = Trim(A_LoopField95);
        }
        if (A_Index95 == 138) {
            keyWordLogicalAssignShiftRight = Trim(A_LoopField95);
        }
        if (A_Index95 == 139) {
            keyWordAssignBitAnd = Trim(A_LoopField95);
        }
        if (A_Index95 == 140) {
            keyWordAssignBitOr = Trim(A_LoopField95);
        }
        if (A_Index95 == 141) {
            keyWordAssignBitXor = Trim(A_LoopField95);
        }
        if (A_Index95 == 142) {
            keyWordTernary1 = Trim(A_LoopField95);
        }
        if (A_Index95 == 143) {
            keyWordTernary2 = Trim(A_LoopField95);
        }
        if (A_Index95 == 144) {
            keyWordInc = Trim(A_LoopField95);
        }
        if (A_Index95 == 145) {
            keyWordDec = Trim(A_LoopField95);
        }
        if (A_Index95 == 146) {
            AHKlikeLoopsIndexedAt = Trim(A_LoopField95);
        }
        if (A_Index95 == 147) {
            keyWordAIndex = Trim(A_LoopField95);
        }
        if (A_Index95 == 148) {
            keyWordALoopField = Trim(A_LoopField95);
        }
        if (A_Index95 == 149) {
            useCurlyBraces = Trim(A_LoopField95);
        }
        if (A_Index95 == 150) {
            useEnd = Trim(A_LoopField95);
        }
        if (A_Index95 == 151) {
            useSemicolon = Trim(A_LoopField95);
        }
        if (A_Index95 == 152) {
            useParentheses = Trim(A_LoopField95);
        }
        if (A_Index95 == 153) {
            usePrefixTypeForTypeDefinition = Trim(A_LoopField95);
        }
        if (A_Index95 == 154) {
            usePostfixTypeForTypeDefinition = Trim(A_LoopField95);
        }
        if (A_Index95 == 155) {
            usePythonicColonSyntax = Trim(A_LoopField95);
        }
        if (A_Index95 == 156) {
            useCurlyBracesSyntaxForArrayDef = Trim(A_LoopField95);
        }
        if (A_Index95 == 157) {
            useInJavaScriptAlwaysUseVar = Trim(A_LoopField95);
        }
        if (A_Index95 == 158) {
            useJavaScriptInAfullHTMLfile = Trim(A_LoopField95);
        }
        if (A_Index95 == 159) {
            useJavaScriptAmainFuncDef = Trim(A_LoopField95);
        }
        if (A_Index95 == 160) {
            useJavaScriptAllFuncsAreAsync = Trim(A_LoopField95);
        }
        if (A_Index95 == 161) {
            useJavaScriptAlwaysTripleEqual = Trim(A_LoopField95);
        }
    }
    if (HTVM_Size(argHTVMinstrMORE) > 0) {
        ...
        std::vector<std::string> items96 = LoopParseFunc(allInstructionFile_2, "\n", "\r");
        for (size_t A_Index96 = 0; A_Index96 < items96.size() + 0; A_Index96++) {
            std::string A_LoopField96 = items96[A_Index96 - 0];
            if (A_Index96 == 0) {
                langToConvertTo_2 = Trim(A_LoopField96);
            }
            if (A_Index96 == 1) {
                langFileExtension_2 = Trim(A_LoopField96);
            }
            if (A_Index96 == 2) {
                commands_2 = Trim(A_LoopField96);
            }
            if (A_Index96 == 3) {
                keyWordAlliance_2 = Trim(A_LoopField96) + " ";
            }
            if (A_Index96 == 4) {
                keyWordCrew_2 = Trim(A_LoopField96) + " ";
            }
            if (A_Index96 == 5) {
                keyWordMethod_2 = Trim(A_LoopField96) + " ";
            }
            if (A_Index96 == 6) {
                keyWordDefObj_2 = Trim(A_LoopField96) + " ";
            }
            if (A_Index96 == 7) {
                keyWordProp_2 = Trim(A_LoopField96) + " ";
            }
            if (A_Index96 == 8) {
                keyWordThis_2 = Trim(A_LoopField96);
            }
            if (A_Index96 == 9) {
                keyWordInclude_2 = Trim(A_LoopField96);
            }
            if (A_Index96 == 10) {
                keyWordCodeInTheTranspiledLangStart_2 = Trim(A_LoopField96);
            }
            if (A_Index96 == 11) {
                keyWordCodeInTheTranspiledLangEnd_2 = Trim(A_LoopField96);
            }
            if (A_Index96 == 12) {
                keyWordCodeInTheTranspiledLangStartCPP_2 = Trim(A_LoopField96);
            }
            if (A_Index96 == 13) {
                keyWordCodeInTheTranspiledLangEndCPP_2 = Trim(A_LoopField96);
            }
            if (A_Index96 == 14) {
                keyWordCodeInTheTranspiledLangStartPY_2 = Trim(A_LoopField96);
            }
            if (A_Index96 == 15) {
                keyWordCodeInTheTranspiledLangEndPY_2 = Trim(A_LoopField96);
            }
            if (A_Index96 == 16) {
                keyWordCodeInTheTranspiledLangStartJS_2 = Trim(A_LoopField96);
            }
            if (A_Index96 == 17) {
                keyWordCodeInTheTranspiledLangEndJS_2 = Trim(A_LoopField96);
            }
            if (A_Index96 == 18) {
                keyWordCodeInTheTranspiledLangStartGO_2 = Trim(A_LoopField96);
            }
            if (A_Index96 == 19) {
                keyWordCodeInTheTranspiledLangEndGO_2 = Trim(A_LoopField96);
            }
            if (A_Index96 == 20) {
                keyWordCodeInTheTranspiledLangStartLUA_2 = Trim(A_LoopField96);
            }
            if (A_Index96 == 21) {
                keyWordCodeInTheTranspiledLangEndLUA_2 = Trim(A_LoopField96);
            }
            if (A_Index96 == 22) {
                keyWordCodeInTheTranspiledLangStartCS_2 = Trim(A_LoopField96);
            }
            if (A_Index96 == 23) {
                keyWordCodeInTheTranspiledLangEndCS_2 = Trim(A_LoopField96);
            }
            if (A_Index96 == 24) {
                keyWordCodeInTheTranspiledLangStartJAVA_2 = Trim(A_LoopField96);
            }
            if (A_Index96 == 25) {
                keyWordCodeInTheTranspiledLangEndJAVA_2 = Trim(A_LoopField96);
            }
            if (A_Index96 == 26) {
                keyWordCodeInTheTranspiledLangStartKT_2 = Trim(A_LoopField96);
            }
            if (A_Index96 == 27) {
                keyWordCodeInTheTranspiledLangEndKT_2 = Trim(A_LoopField96);
            }
            if (A_Index96 == 28) {
                keyWordCodeInTheTranspiledLangStartRB_2 = Trim(A_LoopField96);
            }
            if (A_Index96 == 29) {
                keyWordCodeInTheTranspiledLangEndRB_2 = Trim(A_LoopField96);
            }
            if (A_Index96 == 30) {
                keyWordCodeInTheTranspiledLangStartNIM_2 = Trim(A_LoopField96);
            }
            if (A_Index96 == 31) {
                keyWordCodeInTheTranspiledLangEndNIM_2 = Trim(A_LoopField96);
            }
            if (A_Index96 == 32) {
                keyWordCodeInTheTranspiledLangStartAHK_2 = Trim(A_LoopField96);
            }
            if (A_Index96 == 33) {
                keyWordCodeInTheTranspiledLangEndAHK_2 = Trim(A_LoopField96);
            }
            if (A_Index96 == 34) {
                keyWordCodeInTheTranspiledLangStartSWIFT_2 = Trim(A_LoopField96);
            }
            if (A_Index96 == 35) {
                keyWordCodeInTheTranspiledLangEndSWIFT_2 = Trim(A_LoopField96);
            }
            if (A_Index96 == 36) {
                keyWordCodeInTheTranspiledLangStartDART_2 = Trim(A_LoopField96);
            }
            if (A_Index96 == 37) {
                keyWordCodeInTheTranspiledLangEndDART_2 = Trim(A_LoopField96);
            }
            if (A_Index96 == 38) {
                keyWordCodeInTheTranspiledLangStartTS_2 = Trim(A_LoopField96);
            }
            if (A_Index96 == 39) {
                keyWordCodeInTheTranspiledLangEndTS_2 = Trim(A_LoopField96);
            }
            if (A_Index96 == 40) {
                keyWordCodeInTheTranspiledLangStartGROOVY_2 = Trim(A_LoopField96);
            }
            if (A_Index96 == 41) {
                keyWordCodeInTheTranspiledLangEndGROOVY_2 = Trim(A_LoopField96);
            }
            if (A_Index96 == 42) {
                keyWordCodeInTheTranspiledLangStartHTVM_2 = Trim(A_LoopField96);
            }
            if (A_Index96 == 43) {
                keyWordCodeInTheTranspiledLangEndHTVM_2 = Trim(A_LoopField96);
            }
            if (A_Index96 == 44) {
                keyWordCodeInHTVMstart_2 = Trim(A_LoopField96);
            }
            if (A_Index96 == 45) {
                keyWordCodeInHTVMend_2 = Trim(A_LoopField96);
            }
            if (A_Index96 == 46) {
                keyWordCurlyBraceOpen_2 = Trim(A_LoopField96);
            }
            if (A_Index96 == 47) {
                keyWordCurlyBraceClose_2 = Trim(A_LoopField96);
            }
            if (A_Index96 == 48) {
                keyWordNull_2 = Trim(A_LoopField96);
            }
            if (A_Index96 == 49) {
                keyWordTrue_2 = Trim(A_LoopField96);
            }
            if (A_Index96 == 50) {
                keyWordFalse_2 = Trim(A_LoopField96);
            }
            if (A_Index96 == 51) {
                keyWordVoid_2 = Trim(A_LoopField96);
            }
            if (A_Index96 == 52) {
                keyWordDouble_2 = Trim(A_LoopField96);
            }
            if (A_Index96 == 53) {
                keyWordChar_2 = Trim(A_LoopField96);
            }
            if (A_Index96 == 54) {
                keyWordUint8_2 = Trim(A_LoopField96);
            }
            if (A_Index96 == 55) {
                keyWordUint16_2 = Trim(A_LoopField96);
            }
            if (A_Index96 == 56) {
                keyWordUint32_2 = Trim(A_LoopField96);
            }
            if (A_Index96 == 57) {
                keyWordUint64_2 = Trim(A_LoopField96);
            }
            if (A_Index96 == 58) {
                keyWordINT_2 = Trim(A_LoopField96);
            }
            if (A_Index96 == 59) {
                keyWordSTR_2 = Trim(A_LoopField96);
            }
            if (A_Index96 == 60) {
                keyWordBOOL_2 = Trim(A_LoopField96);
            }
            if (A_Index96 == 61) {
                keyWordFLOAT_2 = Trim(A_LoopField96);
            }
            if (A_Index96 == 62) {
                keyWordINT8_2 = Trim(A_LoopField96);
            }
            if (A_Index96 == 63) {
                keyWordINT16_2 = Trim(A_LoopField96);
            }
            if (A_Index96 == 64) {
                keyWordINT32_2 = Trim(A_LoopField96);
            }
            if (A_Index96 == 65) {
                keyWordINT64_2 = Trim(A_LoopField96);
            }
            if (A_Index96 == 66) {
                keyWordIF_2 = Trim(A_LoopField96) + " ";
            }
            if (A_Index96 == 67) {
                keyWordElseIf_2 = Trim(A_LoopField96) + " ";
            }
            if (A_Index96 == 68) {
                keyWordElse_2 = Trim(A_LoopField96);
            }
            if (A_Index96 == 69) {
                keyWordWhileLoop_2 = Trim(A_LoopField96) + " ";
            }
            if (A_Index96 == 70) {
                keyWordLoopInfinite_2 = Trim(A_LoopField96);
            }
            if (A_Index96 == 71) {
                keyWordLoop_2 = Trim(A_LoopField96) + " ";
            }
            if (A_Index96 == 72) {
                keyWordLoopParse_2 = Trim(A_LoopField96) + " ";
            }
            if (A_Index96 == 73) {
                keyWordContinue_2 = Trim(A_LoopField96);
            }
            if (A_Index96 == 74) {
                keyWordBreak_2 = Trim(A_LoopField96);
            }
            if (A_Index96 == 75) {
                keyWordFunc_2 = Trim(A_LoopField96) + " ";
            }
            if (A_Index96 == 76) {
                keyWordAwait_2 = Trim(A_LoopField96) + " ";
            }
            if (A_Index96 == 77) {
                keyWordAsync_2 = Trim(A_LoopField96) + " ";
            }
            if (A_Index96 == 78) {
                keyWordThrow_2 = Trim(A_LoopField96) + " ";
            }
            if (A_Index96 == 79) {
                keyWordErrorMsg_2 = Trim(A_LoopField96);
            }
            if (A_Index96 == 80) {
                keyWordTry_2 = Trim(A_LoopField96);
            }
            if (A_Index96 == 81) {
                keyWordCatch_2 = Trim(A_LoopField96) + " ";
            }
            if (A_Index96 == 82) {
                keyWordFinally_2 = Trim(A_LoopField96);
            }
            if (A_Index96 == 83) {
                keyWordReturnStatement_2 = Trim(A_LoopField96) + " ";
            }
            if (A_Index96 == 84) {
                keyWordArrayAppend_2 = Trim(A_LoopField96);
            }
            if (A_Index96 == 85) {
                keyWordArrayPop_2 = Trim(A_LoopField96);
            }
            if (A_Index96 == 86) {
                keyWordArraySize_2 = Trim(A_LoopField96);
            }
            if (A_Index96 == 87) {
                keyWordArrayInsert_2 = Trim(A_LoopField96);
            }
            if (A_Index96 == 88) {
                keyWordArrayRemove_2 = Trim(A_LoopField96);
            }
            if (A_Index96 == 89) {
                keyWordArrayIndexOf_2 = Trim(A_LoopField96);
            }
            if (A_Index96 == 90) {
                keyWordArrayDefinition_2 = Trim(A_LoopField96);
            }
            if (A_Index96 == 91) {
                keyWordArrayOfIntegersDefinition_2 = Trim(A_LoopField96);
            }
            if (A_Index96 == 92) {
                keyWordArrayOfStringsDefinition_2 = Trim(A_LoopField96);
            }
            if (A_Index96 == 93) {
                keyWordArrayOfFloatingPointNumbersDefinition_2 = Trim(A_LoopField96);
            }
            if (A_Index96 == 94) {
                keyWordArrayOfBooleansDefinition_2 = Trim(A_LoopField96);
            }
            if (A_Index96 == 95) {
                keyWordVar_2 = Trim(A_LoopField96);
            }
            if (A_Index96 == 96) {
                keyWordLet_2 = Trim(A_LoopField96);
            }
            if (A_Index96 == 97) {
                keyWordConst_2 = Trim(A_LoopField96);
            }
            if (A_Index96 == 98) {
                keyWordEnd_2 = Trim(A_LoopField96);
            }
            if (A_Index96 == 99) {
                keyWordGlobal_2 = Trim(A_LoopField96) + " ";
            }
            if (A_Index96 == 100) {
                keyWordComment_2 = Trim(A_LoopField96);
            }
            if (A_Index96 == 101) {
                keyWordCommentOpenMultiLine_2 = Trim(A_LoopField96);
            }
            if (A_Index96 == 102) {
                keyWordCommentCloseMultiLine_2 = Trim(A_LoopField96);
            }
            if (A_Index96 == 103) {
                keyWordEscpaeChar_2 = Trim(A_LoopField96);
            }
            if (A_Index96 == 104) {
                keyWordMainLabel_2 = Trim(A_LoopField96);
            }
            if (A_Index96 == 105) {
                keyWordConcat_2 = Trim(A_LoopField96);
            }
            if (A_Index96 == 106) {
                keyWordAdd_2 = Trim(A_LoopField96);
            }
            if (A_Index96 == 107) {
                keyWordSub_2 = Trim(A_LoopField96);
            }
            if (A_Index96 == 108) {
                keyWordMul_2 = Trim(A_LoopField96);
            }
            if (A_Index96 == 109) {
                keyWordDiv_2 = Trim(A_LoopField96);
            }
            if (A_Index96 == 110) {
                keyWordMod_2 = Trim(A_LoopField96);
            }
            if (A_Index96 == 111) {
                keyWordExp_2 = Trim(A_LoopField96);
            }
            if (A_Index96 == 112) {
                keyWordEqual_2 = Trim(A_LoopField96);
            }
            if (A_Index96 == 113) {
                keyWordStrictEqual_2 = Trim(A_LoopField96);
            }
            if (A_Index96 == 114) {
                keyWordNotEqual_2 = Trim(A_LoopField96);
            }
            if (A_Index96 == 115) {
                keyWordGreater_2 = Trim(A_LoopField96);
            }
            if (A_Index96 == 116) {
                keyWordLess_2 = Trim(A_LoopField96);
            }
            if (A_Index96 == 117) {
                keyWordGreaterEqual_2 = Trim(A_LoopField96);
            }
            if (A_Index96 == 118) {
                keyWordLessEqual_2 = Trim(A_LoopField96);
            }
            if (A_Index96 == 119) {
                keyWordAnd_2 = Trim(A_LoopField96);
            }
            if (A_Index96 == 120) {
                keyWordOr_2 = Trim(A_LoopField96);
            }
            if (A_Index96 == 121) {
                keyWordNot_2 = Trim(A_LoopField96);
            }
            if (A_Index96 == 122) {
                keyWordBitAnd_2 = Trim(A_LoopField96);
            }
            if (A_Index96 == 123) {
                keyWordBitOr_2 = Trim(A_LoopField96);
            }
            if (A_Index96 == 124) {
                keyWordBitXor_2 = Trim(A_LoopField96);
            }
            if (A_Index96 == 125) {
                keyWordBitNot_2 = Trim(A_LoopField96);
            }
            if (A_Index96 == 126) {
                keyWordShiftLeft_2 = Trim(A_LoopField96);
            }
            if (A_Index96 == 127) {
                keyWordShiftRight_2 = Trim(A_LoopField96);
            }
            if (A_Index96 == 128) {
                keyWordShiftUnsignedRight_2 = Trim(A_LoopField96);
            }
            if (A_Index96 == 129) {
                keyWordAssign_2 = Trim(A_LoopField96);
            }
            if (A_Index96 == 130) {
                keyWordAssignAdd_2 = Trim(A_LoopField96);
            }
            if (A_Index96 == 131) {
                keyWordAssignConcat_2 = Trim(A_LoopField96);
            }
            if (A_Index96 == 132) {
                keyWordAssignSub_2 = Trim(A_LoopField96);
            }
            if (A_Index96 == 133) {
                keyWordAssignMul_2 = Trim(A_LoopField96);
            }
            if (A_Index96 == 134) {
                keyWordAssignDiv_2 = Trim(A_LoopField96);
            }
            if (A_Index96 == 135) {
                keyWordAssignMod_2 = Trim(A_LoopField96);
            }
            if (A_Index96 == 136) {
                keyWordAssignShiftLeft_2 = Trim(A_LoopField96);
            }
            if (A_Index96 == 137) {
                keyWordAssignShiftRight_2 = Trim(A_LoopField96);
            }
            if (A_Index96 == 138) {
                keyWordLogicalAssignShiftRight_2 = Trim(A_LoopField96);
            }
            if (A_Index96 == 139) {
                keyWordAssignBitAnd_2 = Trim(A_LoopField96);
            }
            if (A_Index96 == 140) {
                keyWordAssignBitOr_2 = Trim(A_LoopField96);
            }
            if (A_Index96 == 141) {
                keyWordAssignBitXor_2 = Trim(A_LoopField96);
            }
            if (A_Index96 == 142) {
                keyWordTernary1_2 = Trim(A_LoopField96);
            }
            if (A_Index96 == 143) {
                keyWordTernary2_2 = Trim(A_LoopField96);
            }
            if (A_Index96 == 144) {
                keyWordInc_2 = Trim(A_LoopField96);
            }
            if (A_Index96 == 145) {
                keyWordDec_2 = Trim(A_LoopField96);
            }
            if (A_Index96 == 146) {
                AHKlikeLoopsIndexedAt_2 = Trim(A_LoopField96);
            }
            if (A_Index96 == 147) {
                keyWordAIndex_2 = Trim(A_LoopField96);
            }
            if (A_Index96 == 148) {
                keyWordALoopField_2 = Trim(A_LoopField96);
            }
            if (A_Index96 == 149) {
                useCurlyBraces_2 = Trim(A_LoopField96);
            }
            if (A_Index96 == 150) {
                useEnd_2 = Trim(A_LoopField96);
            }
            if (A_Index96 == 151) {
                useSemicolon_2 = Trim(A_LoopField96);
            }
            if (A_Index96 == 152) {
                useParentheses_2 = Trim(A_LoopField96);
            }
            if (A_Index96 == 153) {
                usePrefixTypeForTypeDefinition_2 = Trim(A_LoopField96);
            }
            if (A_Index96 == 154) {
                usePostfixTypeForTypeDefinition_2 = Trim(A_LoopField96);
            }
            if (A_Index96 == 155) {
                usePythonicColonSyntax_2 = Trim(A_LoopField96);
            }
            if (A_Index96 == 156) {
                useCurlyBracesSyntaxForArrayDef_2 = Trim(A_LoopField96);
            }
            if (A_Index96 == 157) {
                useInJavaScriptAlwaysUseVar_2 = Trim(A_LoopField96);
            }
            if (A_Index96 == 158) {
                useJavaScriptInAfullHTMLfile_2 = Trim(A_LoopField96);
            }
            if (A_Index96 == 159) {
                useJavaScriptAmainFuncDef_2 = Trim(A_LoopField96);
            }
            if (A_Index96 == 160) {
                useJavaScriptAllFuncsAreAsync_2 = Trim(A_LoopField96);
            }
            if (A_Index96 == 161) {
                useJavaScriptAlwaysTripleEqual_2 = Trim(A_LoopField96);
            }
        }
    }
    ...
    */
    ...
    for (size_t A_Index97 = 0; A_Index97 < items97.size() + 0; A_Index97++) {
        ...
        if (Trim(StrLower(A_LoopField97)) == StrLower(keyWordCodeInTheTranspiledLangEnd)) {
        ...
        else if (Trim(StrLower(A_LoopField97)) == StrLower(keyWordCodeInTheTranspiledLangEndCPP)) {
        ...
        else if (Trim(StrLower(A_LoopField97)) == StrLower(keyWordCodeInTheTranspiledLangEndPY)) {
        ...
        else if (Trim(StrLower(A_LoopField97)) == StrLower(keyWordCodeInTheTranspiledLangEndJS)) {
        ...
        else if (Trim(StrLower(A_LoopField97)) == StrLower(keyWordCodeInTheTranspiledLangEndGO)) {
        ...
        else if (Trim(StrLower(A_LoopField97)) == StrLower(keyWordCodeInTheTranspiledLangEndLUA)) {
        ...
        else if (Trim(StrLower(A_LoopField97)) == StrLower(keyWordCodeInTheTranspiledLangEndCS)) {
        ...
        else if (Trim(StrLower(A_LoopField97)) == StrLower(keyWordCodeInTheTranspiledLangEndJAVA)) {
        ...
        else if (Trim(StrLower(A_LoopField97)) == StrLower(keyWordCodeInTheTranspiledLangEndKT)) {
        ...
        else if (Trim(StrLower(A_LoopField97)) == StrLower(keyWordCodeInTheTranspiledLangEndRB)) {
        ...
        else if (Trim(StrLower(A_LoopField97)) == StrLower(keyWordCodeInTheTranspiledLangEndNIM)) {
        ...
        else if (Trim(StrLower(A_LoopField97)) == StrLower(keyWordCodeInTheTranspiledLangEndAHK)) {
        ...
        else if (Trim(StrLower(A_LoopField97)) == StrLower(keyWordCodeInTheTranspiledLangEndSWIFT)) {
        ...
        else if (Trim(StrLower(A_LoopField97)) == StrLower(keyWordCodeInTheTranspiledLangEndDART)) {
        ...
        else if (Trim(StrLower(A_LoopField97)) == StrLower(keyWordCodeInTheTranspiledLangEndTS)) {
        ...
        else if (Trim(StrLower(A_LoopField97)) == StrLower(keyWordCodeInTheTranspiledLangEndGROOVY)) {
        ...
        else if (Trim(StrLower(A_LoopField97)) == StrLower(keyWordCodeInTheTranspiledLangEndHTVM)) {
        ...
        else if (Trim(StrLower(A_LoopField97)) == StrLower(keyWordCodeInHTVMend)) {
            //;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;
            //;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;
            ...
            holdTempDataProgrammingBlockThenPutInArr += A_LoopField97 + Chr(10);
        }
        else if (Trim(StrLower(A_LoopField97)) == StrLower(keyWordCodeInTheTranspiledLangStart)) {
            holdTempDataProgrammingBlockThenPutInArr = "";
            inProgarmmingBlock = 1;
        }
        else if (Trim(StrLower(A_LoopField97)) == StrLower(keyWordCodeInTheTranspiledLangStartCPP)) {
            holdTempDataProgrammingBlockThenPutInArr = "";
            inProgarmmingBlock = 1;
        }
        else if (Trim(StrLower(A_LoopField97)) == StrLower(keyWordCodeInTheTranspiledLangStartPY)) {
            holdTempDataProgrammingBlockThenPutInArr = "";
            inProgarmmingBlock = 1;
        }
        else if (Trim(StrLower(A_LoopField97)) == StrLower(keyWordCodeInTheTranspiledLangStartJS)) {
            holdTempDataProgrammingBlockThenPutInArr = "";
            inProgarmmingBlock = 1;
        }
        else if (Trim(StrLower(A_LoopField97)) == StrLower(keyWordCodeInTheTranspiledLangStartGO)) {
            holdTempDataProgrammingBlockThenPutInArr = "";
            inProgarmmingBlock = 1;
        }
        else if (Trim(StrLower(A_LoopField97)) == StrLower(keyWordCodeInTheTranspiledLangStartLUA)) {
            holdTempDataProgrammingBlockThenPutInArr = "";
            inProgarmmingBlock = 1;
        }
        else if (Trim(StrLower(A_LoopField97)) == StrLower(keyWordCodeInTheTranspiledLangStartCS)) {
            holdTempDataProgrammingBlockThenPutInArr = "";
            inProgarmmingBlock = 1;
        }
        else if (Trim(StrLower(A_LoopField97)) == StrLower(keyWordCodeInTheTranspiledLangStartJAVA)) {
            holdTempDataProgrammingBlockThenPutInArr = "";
            inProgarmmingBlock = 1;
        }
        else if (Trim(StrLower(A_LoopField97)) == StrLower(keyWordCodeInTheTranspiledLangStartKT)) {
            holdTempDataProgrammingBlockThenPutInArr = "";
            inProgarmmingBlock = 1;
        }
        else if (Trim(StrLower(A_LoopField97)) == StrLower(keyWordCodeInTheTranspiledLangStartRB)) {
            holdTempDataProgrammingBlockThenPutInArr = "";
            inProgarmmingBlock = 1;
        }
        else if (Trim(StrLower(A_LoopField97)) == StrLower(keyWordCodeInTheTranspiledLangStartNIM)) {
            holdTempDataProgrammingBlockThenPutInArr = "";
            inProgarmmingBlock = 1;
        }
        else if (Trim(StrLower(A_LoopField97)) == StrLower(keyWordCodeInTheTranspiledLangStartAHK)) {
            holdTempDataProgrammingBlockThenPutInArr = "";
            inProgarmmingBlock = 1;
        }
        else if (Trim(StrLower(A_LoopField97)) == StrLower(keyWordCodeInTheTranspiledLangStartSWIFT)) {
            holdTempDataProgrammingBlockThenPutInArr = "";
            inProgarmmingBlock = 1;
        }
        else if (Trim(StrLower(A_LoopField97)) == StrLower(keyWordCodeInTheTranspiledLangStartDART)) {
            holdTempDataProgrammingBlockThenPutInArr = "";
            inProgarmmingBlock = 1;
        }
        else if (Trim(StrLower(A_LoopField97)) == StrLower(keyWordCodeInTheTranspiledLangStartTS)) {
            holdTempDataProgrammingBlockThenPutInArr = "";
            inProgarmmingBlock = 1;
        }
        else if (Trim(StrLower(A_LoopField97)) == StrLower(keyWordCodeInTheTranspiledLangStartGROOVY)) {
            holdTempDataProgrammingBlockThenPutInArr = "";
            inProgarmmingBlock = 1;
        }
        else if (Trim(StrLower(A_LoopField97)) == StrLower(keyWordCodeInTheTranspiledLangStartHTVM)) {
            holdTempDataProgrammingBlockThenPutInArr = "";
            inProgarmmingBlock = 1;
        }
        else if (Trim(StrLower(A_LoopField97)) == StrLower(keyWordCodeInHTVMstart)) {
            //;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;
            //;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;
            holdTempDataProgrammingBlockThenPutInArr = "";
            inProgarmmingBlock = 1;
            ...
            programmingBlocksTemp += A_LoopField97 + Chr(10);
        }
    }
    ...
    if (isNotHTVMfile == 0) {
        if (langToConvertTo != langFileExtension_2) {
            if (InStr(StrLower(code), StrLower(keyWordInclude + " "))) {
                for (int A_Index98 = 0; A_Index98 < 10000 + 0; A_Index98++) {
                    doseCodeHaveLibs = 0;
                    allLibCODE = "";
                    if (A_Index98 == 0) {
                        ...
                        for (size_t A_Index99 = 0; A_Index99 < items99.size() + 0; A_Index99++) {
                            ...
                            if (SubStr(StrLower(Trim(A_LoopField99)), 1, StrLen(StrLower(keyWordInclude + " "))) == StrLower(keyWordInclude + " ")) {
                                HTVM_Append(allLibsWeSee, StrReplace(StringTrimLeft(Trim(A_LoopField99), StrLen(StrLower(keyWordInclude) + " ")), Chr(34), ""));
                                HTVM_Append(allLibsWeSeeSEE, StrReplace(StringTrimLeft(Trim(A_LoopField99), StrLen(StrLower(keyWordInclude) + " ")), Chr(34), ""));
                                doseCodeHaveLibs = 1;
                            }
                        }
                    } else {
                        ...
                        for (size_t A_Index100 = 0; A_Index100 < items100.size() + 0; A_Index100++) {
                            ...
                            ALoopField = A_LoopField100;
                            if (SubStr(StrLower(Trim(A_LoopField100)), 1, StrLen(StrLower(keyWordInclude + " "))) == StrLower(keyWordInclude + " ")) {
                                doseCodeHaveLibs_HAS_SEEN = 0;
                                for (int A_Index101 = 0; A_Index101 < HTVM_Size(allLibsWeSee) + 0; A_Index101++) {
                                    if (StrReplace(StringTrimLeft(Trim(ALoopField), StrLen(StrLower(keyWordInclude) + " ")), Chr(34), "") == allLibsWeSee[A_Index101]) {
                                        doseCodeHaveLibs_HAS_SEEN = 1;
                                    }
                                }
                                if (doseCodeHaveLibs_HAS_SEEN == 0) {
                                    HTVM_Append(allLibsWeSee, StrReplace(StringTrimLeft(Trim(A_LoopField100), StrLen(StrLower(keyWordInclude) + " ")), Chr(34), ""));
                                    HTVM_Append(allLibsWeSeeSEE, StrReplace(StringTrimLeft(Trim(A_LoopField100), StrLen(StrLower(keyWordInclude) + " ")), Chr(34), ""));
                                    doseCodeHaveLibs = 1;
                                }
                            }
                        }
                    }
                    for (int A_Index102 = 0; A_Index102 < HTVM_Size(allLibsWeSee) + 0; A_Index102++) {
                        allLibsWeSeeHOLD = allLibsWeSee[A_Index102];
                        doseCodeHaveLibs_HAS_SEEN2 = 0;
                        for (int A_Index103 = 0; A_Index103 < HTVM_Size(allLibsWeSeeSEE) - HTVM_Size(allLibsWeSee) + 0; A_Index103++) {
                            if (allLibsWeSeeHOLD == allLibsWeSeeSEE[A_Index103]) {
                                doseCodeHaveLibs_HAS_SEEN2 = 1;
                            }
                        }
                        if (doseCodeHaveLibs_HAS_SEEN2 == 0) {
                            allLibCODE += FileRead(Trim(allLibsWeSee[HTVM_Size(allLibsWeSee) - A_Index102 - 1])) + Chr(10);
                        }
                    ...
                    for (size_t A_Index104 = 0; A_Index104 < items104.size() + 0; A_Index104++) {
                        ...
                        if (SubStr(StrLower(Trim(A_LoopField104)), 1, StrLen(StrLower(keyWordInclude + " "))) != StrLower(keyWordInclude + " ")) {
                            allLibCODEfix += A_LoopField104 + Chr(10);
                        }
                    }
    ...
    if (isNotHTVMfile == 0) {
        ...
        for (size_t A_Index106 = 0; A_Index106 < items106.size() + 0; A_Index106++) {
            ...
            theIdNumOfThe34theVar[A_Index106] = theIdNumOfThe34theVar[A_Index106] + Chr(34);
            HTVM_Append(getAllCharForTheFurtureSoIcanAddEscapeChar, A_LoopField106);
        }
        ...
        std::vector<std::string> items107 = LoopParseFunc(code);
        for (size_t A_Index107 = 0; A_Index107 < items107.size() + 0; A_Index107++) {
            std::string A_LoopField107 = items107[A_Index107 - 0];
            if (A_LoopField107 == keyWordEscpaeChar && getAllCharForTheFurtureSoIcanAddEscapeChar[A_Index107 + 1] == Chr(34)) {
                ...
                if (fixOutFixDoubleQuotesInsideDoubleQuotesFIXok != 1) {
                    OutFixDoubleQuotesInsideDoubleQuotes += A_LoopField107;
                } else {
        ...
        if (keyWordEscpaeChar == Chr(92) && langToConvertTo != langFileExtension_2) {
            ...
            for (size_t A_Index108 = 0; A_Index108 < items108.size() + 0; A_Index108++) {
                ...
                if (A_LoopField108 == Chr(34)) {
                    areWEinSome34sNum++;
                }
                if (areWEinSome34sNum == 1) {
                    if (A_LoopField108 != Chr(34)) {
                        if (A_LoopField108 == keyWordEscpaeChar && langToConvertTo != langFileExtension_2) {
                            theIdNumOfThe34theVar[theIdNumOfThe34] = theIdNumOfThe34theVar[theIdNumOfThe34] + Chr(92);
                        } else {
                            theIdNumOfThe34theVar[theIdNumOfThe34] = theIdNumOfThe34theVar[theIdNumOfThe34] + A_LoopField108;
                        }
                    } else {
                        theIdNumOfThe34++;
                ...
                if (areWEinSome34sNum == 2 || areWEinSome34sNum == 0) {
                    if (A_LoopField108 != Chr(34)) {
                        htCodeOUT754754 += A_LoopField108;
                    }
                    areWEinSome34sNum = 0;
                }
            }
            ...
            for (size_t A_Index109 = 0; A_Index109 < items109.size() + 0; A_Index109++) {
                ...
                if (A_LoopField109 == Chr(34)) {
                    areWEinSome34sNum++;
                }
                if (areWEinSome34sNum == 1) {
                    if (A_LoopField109 != Chr(34)) {
                        if (A_LoopField109 == keyWordEscpaeChar && keyWordEscpaeChar == getAllCharForTheFurtureSoIcanAddEscapeChar[A_Index109 + 1] && langToConvertTo != langFileExtension_2) {
                        ...
                        else if (A_LoopField109 == keyWordEscpaeChar && langToConvertTo != langFileExtension_2) {
                            ...
                            theIdNumOfThe34theVar[theIdNumOfThe34] = theIdNumOfThe34theVar[theIdNumOfThe34] + A_LoopField109;
                        }
                    } else {
                        theIdNumOfThe34++;
                        htCodeOUT754754 += "ihuiuuhuuhtheidFor--asds" + str21 + "as--" + str21 + "theuhturtyphoutr--" + Chr(65) + Chr(65) + STR(theIdNumOfThe34) + Chr(65) + Chr(65);
                ...
                if (areWEinSome34sNum == 2 || areWEinSome34sNum == 0) {
                    if (A_LoopField109 != Chr(34)) {
                        htCodeOUT754754 += A_LoopField109;
                    }
                    areWEinSome34sNum = 0;
                }
            }
        ...
        for (int A_Index110 = 0; A_Index110 < theIdNumOfThe34 + 0; A_Index110++) {
        ...
        for (size_t A_Index111 = 0; A_Index111 < items111.size() + 0; A_Index111++) {
            ...
            if (InStr(A_LoopField111, " " + keyWordComment) != false && SubStr(Trim(A_LoopField111), 1, StrLen(keyWordComment)) != keyWordComment) {
                posForRemoveCommentsOnTheEndOfTheLine = 0;
                slidingWinFixCommentsRmI = 0;
                slidingWinFixCommentsRm = slidingWinFixCommentsRmFUNC(A_LoopField111);
                for (int A_Index112 = 0; A_Index112 < HTVM_Size(slidingWinFixCommentsRm) + 0; A_Index112++) {
                    slidingWinFixCommentsRmI = A_Index112;
                    slidingWinFixCommentsRmHold = "";
                    for (int A_Index113 = 0; A_Index113 < StrLen(" " + keyWordComment) + 0; A_Index113++) {
                        slidingWinFixCommentsRmHold += slidingWinFixCommentsRm[slidingWinFixCommentsRmI + A_Index113];
                    }
                    if (Trim(slidingWinFixCommentsRmHold) == keyWordComment) {
                        posForRemoveCommentsOnTheEndOfTheLine = A_Index112;
                        break;
                    }
                }
                if (posForRemoveCommentsOnTheEndOfTheLine > 0) {
                    newStringOutCode += SubStr(A_LoopField111, 1, posForRemoveCommentsOnTheEndOfTheLine) + Chr(10);
                } else {
                    newStringOutCode += A_LoopField111 + Chr(10);
                }
            } else {
                newStringOutCode += A_LoopField111 + Chr(10);
            }
        ...
        if (useCurlyBraces == "on") {
            if (keyWordCurlyBraceOpen != "{") {
                ...
                for (size_t A_Index114 = 0; A_Index114 < items114.size() + 0; A_Index114++) {
                    ...
                    htCodeOutFixINT2 = 0;
                    if (isLineAconstruct(Trim(A_LoopField114)) == true || Trim(A_LoopField114) == keyWordCurlyBraceOpen) {
                        htCodeOutFixINT2 = 1;
                    }
                    htCodeOutFixINT = 0;
                    std::vector<std::string> items115 = LoopParseFunc(A_LoopField114, " ");
                    for (size_t A_Index115 = 0; A_Index115 < items115.size() + 0; A_Index115++) {
                        std::string A_LoopField115 = items115[A_Index115 - 0];
                        htCodeOutFixINT++;
                    }
                    htCodeOutFixINT--;
                    std::vector<std::string> items116 = LoopParseFunc(A_LoopField114, " ");
                    for (size_t A_Index116 = 0; A_Index116 < items116.size() + 0; A_Index116++) {
                        std::string A_LoopField116 = items116[A_Index116 - 0];
                        if (htCodeOutFixINT == A_Index116 && htCodeOutFixINT2 == 1) {
                            htCodeOutFixINT2 = 0;
                            htCodeOutFixEnd += HTVMmatchStrRrplace(A_LoopField116, keyWordCurlyBraceOpen, "{") + " ";
                        } else {
                            htCodeOutFixEnd += A_LoopField116 + " ";
                        }
                    }
            ...
            if (keyWordCurlyBraceClose != "}") {
                std::vector<std::string> items117 = LoopParseFunc(code, "\n", "\r");
                for (size_t A_Index117 = 0; A_Index117 < items117.size() + 0; A_Index117++) {
                    std::string A_LoopField117 = items117[A_Index117 - 0];
                    htCodeOutFixINT2 = 0;
                    if (isLineAconstruct(Trim(A_LoopField117)) == true || Trim(A_LoopField117) == keyWordCurlyBraceClose) {
                        htCodeOutFixINT2 = 1;
                    }
                    std::vector<std::string> items118 = LoopParseFunc(A_LoopField117, " ");
                    for (size_t A_Index118 = 0; A_Index118 < items118.size() + 0; A_Index118++) {
                        std::string A_LoopField118 = items118[A_Index118 - 0];
                        if (htCodeOutFixINT2 == 1 || Trim(A_LoopField118) == keyWordCurlyBraceClose) {
                            htCodeOutFixINT2 = 0;
                            htCodeOutFixEnd += HTVMmatchStrRrplace(A_LoopField118, keyWordCurlyBraceClose, "}") + " ";
                        } else {
                            htCodeOutFixEnd += A_LoopField118 + " ";
                        }
                    }
                    htCodeOutFixEnd = StringTrimRight(htCodeOutFixEnd, 1);
                    htCodeOutFixEnd += Chr(10);
        ...
        htCodeOutFixEnd = "";
        if (useEnd == "on") {
            std::vector<std::string> items119 = LoopParseFunc(code, "\n", "\r");
            for (size_t A_Index119 = 0; A_Index119 < items119.size() + 0; A_Index119++) {
                std::string A_LoopField119 = items119[A_Index119 - 0];
                if (StrLower(Trim(A_LoopField119)) == StrLower(keyWordEnd)) {
                    htCodeOutFixEnd += "}\n";
                }
                else if (SubStr(StrLower(Trim(A_LoopField119)), 1, StrLen(StrLower(keyWordIF))) == StrLower(keyWordIF) && isLineAconstruct(Trim(A_LoopField119)) == true) {
                    htCodeOutFixEnd += A_LoopField119 + "\n{\n";
                }
                else if (SubStr(StrLower(Trim(A_LoopField119)), 1, StrLen(StrLower(keyWordWhileLoop))) == StrLower(keyWordWhileLoop) && isLineAconstruct(Trim(A_LoopField119)) == true) {
                    htCodeOutFixEnd += A_LoopField119 + "\n{\n";
                }
                else if (SubStr(StrLower(Trim(A_LoopField119)), 1, StrLen(StrLower(keyWordElseIf))) == StrLower(keyWordElseIf) && isLineAconstruct(Trim(A_LoopField119)) == true) {
                    htCodeOutFixEnd += "\n}\n" + A_LoopField119 + "\n{\n";
                }
                else if (SubStr(StrLower(Trim(A_LoopField119)), 1, StrLen(StrLower(keyWordElse))) == StrLower(keyWordElse) && isLineAconstruct(Trim(A_LoopField119)) == true) {
                    htCodeOutFixEnd += "\n}\n" + A_LoopField119 + "\n{\n";
                }
                else if (SubStr(StrLower(Trim(A_LoopField119)), 1, StrLen(StrLower(keyWordCatch))) == StrLower(keyWordCatch) && isLineAconstruct(Trim(A_LoopField119)) == true) {
                    htCodeOutFixEnd += "\n}\n" + A_LoopField119 + "\n{\n";
                }
                else if (SubStr(StrLower(Trim(A_LoopField119)), 1, StrLen(StrLower(keyWordFinally))) == StrLower(keyWordFinally) && isLineAconstruct(Trim(A_LoopField119)) == true) {
                    htCodeOutFixEnd += "\n}\n" + A_LoopField119 + "\n{\n";
                }
                else if (isLineAconstruct(Trim(A_LoopField119)) == true) {
                    htCodeOutFixEnd += A_LoopField119 + "\n{\n";
                } else {
                    htCodeOutFixEnd += A_LoopField119 + Chr(10);
                }
            }
            code = StringTrimRight(htCodeOutFixEnd, 1);
        }
        if (usePythonicColonSyntax == "off") {
            if (useEnd == "off" && useCurlyBraces == "off") {
                ...
                for (size_t A_Index120 = 0; A_Index120 < items120.size() + 0; A_Index120++) {
                    ...
                    if (isLineAconstruct(Trim(A_LoopField120)) == true) {
                        htCodeOutFixEnd += A_LoopField120 + ":\n";
                    } else {
                        htCodeOutFixEnd += A_LoopField120 + Chr(10);
                    }
                }
                code = StringTrimRight(htCodeOutFixEnd, 1);
            }
        ...
        for (int A_Index122 = 0; A_Index122 < 2 + 0; A_Index122++) {
            ...
            for (size_t A_Index123 = 0; A_Index123 < items123.size() + 0; A_Index123++) {
                ...
                if (InStr(A_LoopField123, " " + keyWordAssign + " ") || InStr(A_LoopField123, " " + keyWordAssignAdd + " ") || InStr(A_LoopField123, " " + keyWordAssignConcat + " ") || InStr(A_LoopField123, " " + keyWordAssignSub + " ") || InStr(A_LoopField123, " " + keyWordAssignMul + " ") || InStr(A_LoopField123, " " + keyWordAssignDiv + " ") || InStr(A_LoopField123, " " + keyWordAssignMod + " ") || InStr(A_LoopField123, " " + keyWordAssignShiftLeft + " ") || InStr(A_LoopField123, " " + keyWordAssignShiftRight + " ") || InStr(A_LoopField123, " " + keyWordLogicalAssignShiftRight + " ") || InStr(A_LoopField123, " " + keyWordAssignBitAnd + " ") || InStr(A_LoopField123, " " + keyWordAssignBitOr + " ") || InStr(A_LoopField123, " " + keyWordAssignBitXor + " ") && useCurlyBracesSyntaxForArrayDef == "on") {
                    if (SubStr(StrLower(A_LoopField123), 1, StrLen(StrLower(keyWordFunc))) == StrLower(keyWordFunc) || SubStr(StrLower(A_LoopField123), 1, StrLen(StrLower(keyWordAsync) + StrLower(keyWordFunc))) == StrLower(keyWordAsync) + StrLower(keyWordFunc)) {
                        if (InStr(Trim(A_LoopField123), "{") && Trim(A_LoopField123) != "{") {
                            outCodeFixBraces += Trim(StrReplace(Trim(A_LoopField123), "{", "")) + "\n{\n";
                        }
                        else if (InStr(Trim(A_LoopField123), "}") && Trim(A_LoopField123) != "}") {
                            outCodeFixBraces += "}\n" + Trim(StrReplace(Trim(A_LoopField123), "}", "")) + Chr(10);
                        } else {
                            outCodeFixBraces += Trim(A_LoopField123) + Chr(10);
                        }
                    } else {
                        outCodeFixBraces += Trim(A_LoopField123) + Chr(10);
                    }
        ...
        if (usePythonicColonSyntax == "on" || useCurlyBraces == "off" && useEnd == "off") {
            ...
            for (size_t A_Index124 = 0; A_Index124 < items124.size() + 0; A_Index124++) {
                ...
                if (isLineAconstruct(Trim(A_LoopField124)) == true) {
                    htCodeOutFixEnd += StringTrimRight(A_LoopField124, 1) + Chr(10);
                } else {
        ...
        if (isNotHTVMfile2 == 0) {
            ...
            for (size_t A_Index125 = 0; A_Index125 < items125.size() + 0; A_Index125++) {
                ...
                if (StrLower(Trim(A_LoopField125)) == StrLower(keyWordMainLabel)) {
                    ...
                    codeOutFixAndAddMainFunc += A_LoopField125 + Chr(10);
                }
            }
        ...
        std::vector<std::string> items126 = LoopParseFunc(code, "\n", "\r");
        for (size_t A_Index126 = 0; A_Index126 < items126.size() + 0; A_Index126++) {
            std::string A_LoopField126 = items126[A_Index126 - 0];
            HTVM_Append(lookIntoTheNextLineForFuncWhitNoKeyWord, A_LoopField126);
        }
        ...
        for (size_t A_Index127 = 0; A_Index127 < items127.size() + 0; A_Index127++) {
            ...
            lineDone = 0;
            if (SubStr(A_LoopField127, 1, StrLen(keyWordComment)) == keyWordComment) {
                lineDone = 1;
                str1 = StringTrimLeft(A_LoopField127, StrLen(keyWordComment));
                if (langToConvertTo == langFileExtension_2) {
            ...
            else if (Trim(A_LoopField127) == keyWordCommentOpenMultiLine) {
                lineDone = 1;
                if (langToConvertTo == langFileExtension_2) {
            ...
            else if (Trim(A_LoopField127) == keyWordCommentCloseMultiLine) {
                lineDone = 1;
                if (langToConvertTo == langFileExtension_2) {
            ...
            else if (InStr(A_LoopField127, "[][][][][] ")) {
                lineDone = 1;
                str1 = StrSplit(A_LoopField127, " ", 2);
            ...
            else if (SubStr(StrLower(Trim(A_LoopField127)), 1, StrLen(StrLower(keyWordDefObj))) == StrLower(keyWordDefObj)) {
                lineDone = 1;
                str0 = Trim(StringTrimLeft(A_LoopField127, StrLen(keyWordDefObj)));
                if (langToConvertTo == langFileExtension_2) {
                    htCode += Trim(keyWordDefObj_2) + " " + str0 + Chr(10);
                }
            }
            else if (SubStr(StrLower(Trim(A_LoopField127)), 1, StrLen(StrLower(keyWordAlliance))) == StrLower(keyWordAlliance)) {
                lineDone = 1;
                str0 = Trim(StringTrimLeft(A_LoopField127, StrLen(keyWordAlliance)));
                if (langToConvertTo == langFileExtension_2) {
                    htCode += Trim(keyWordAlliance_2) + " " + str0 + Chr(10);
                }
            }
            else if (SubStr(StrLower(Trim(A_LoopField127)), 1, StrLen(StrLower(keyWordCrew))) == StrLower(keyWordCrew)) {
                lineDone = 1;
                str0 = Trim(StringTrimLeft(A_LoopField127, StrLen(keyWordCrew)));
                if (langToConvertTo == langFileExtension_2) {
                    htCode += Trim(keyWordCrew_2) + " " + str0 + Chr(10);
                }
            }
            else if (InStr(A_LoopField127, "[end][end][end][end][end] ")) {
                lineDone = 1;
                if (langToConvertTo == langFileExtension_2) {
                    htCode += "}" + Chr(10);
                }
            }
            else if (SubStr(StrLower(Trim(A_LoopField127)), 1, StrLen(StrLower(keyWordProp))) == StrLower(keyWordProp)) {
                lineDone = 1;
                str0 = Trim(StringTrimLeft(A_LoopField127, StrLen(keyWordProp)));
            ...
            else if (SubStr(StrLower(Trim(A_LoopField127)), 1, StrLen(StrLower(keyWordMethod))) == StrLower(keyWordMethod)) {
                lineDone = 1;
                str0 = Trim(StringTrimLeft(A_LoopField127, StrLen(keyWordMethod)));
                str1 = Trim(StrSplit(str0, "-----------------------HTVM-OSP-HTVM-OBJ--------------", 1));
                str20 = Trim(StrSplit(str0, "-----------------------HTVM-OSP-HTVM-OBJ--------------", 2));
            ...
            else if (StrLower(Trim(A_LoopField127)) == StrLower(keyWordElse) || StrLower(Trim(A_LoopField127)) == StrLower(keyWordElse + ":")) {
                lineDone = 1;
                if (langToConvertTo == langFileExtension_2) {
                    if (usePythonicColonSyntax_2 == "on") {
            ...
            else if (SubStr(StrLower(Trim(A_LoopField127)), 1, StrLen(StrLower(keyWordWhileLoop))) == StrLower(keyWordWhileLoop)) {
                lineDone = 1;
                str1 = Trim(StringTrimLeft(A_LoopField127, StrLen(keyWordWhileLoop)));
                str1 = Trim(expressionParserTranspiler("(" + str1 + ")"));
                str1 = StringTrimRight(str1, 1);
                str1 = StringTrimLeft(str1, 1);
                if (langToConvertTo == langFileExtension_2) {
            ...
            else if (StrLower(Trim(A_LoopField127)) == StrLower(keyWordTry) || StrLower(Trim(A_LoopField127)) == StrLower(keyWordTry + ":")) {
                lineDone = 1;
                if (langToConvertTo == langFileExtension_2) {
                    if (usePythonicColonSyntax_2 == "on") {
                        htCode += keyWordTry_2 + ":\n";
            ...
            else if (SubStr(StrLower(Trim(A_LoopField127)), 1, StrLen(StrLower(keyWordCatch))) == StrLower(keyWordCatch)) {
                lineDone = 1;
                str1 = Trim(StrReplace(StrReplace(StringTrimLeft(A_LoopField127, StrLen(keyWordCatch)), "(", ""), ")", ""));
            ...
            else if (StrLower(Trim(A_LoopField127)) == StrLower(keyWordFinally) || StrLower(Trim(A_LoopField127)) == StrLower(keyWordFinally + ":")) {
                lineDone = 1;
                if (langToConvertTo == langFileExtension_2) {
                    if (usePythonicColonSyntax_2 == "on") {
            ...
            else if (SubStr(StrLower(Trim(A_LoopField127)), 1, StrLen(StrLower(keyWordThrow))) == StrLower(keyWordThrow)) {
                lineDone = 1;
                str1 = Trim(A_LoopField127);
                if (SubStrLastChars(str1, 1) == ";") {
                    str1 = StringTrimRight(str1, 1);
                }
            ...
            else if (StrLower(A_LoopField127) == StrLower(keyWordBreak) || StrLower(A_LoopField127) == StrLower(keyWordBreak + ";")) {
                lineDone = 1;
                if (langToConvertTo == langFileExtension_2) {
                    if (useSemicolon_2 == "on") {
            ...
            else if (StrLower(A_LoopField127) == StrLower(Trim(keyWordReturnStatement)) || StrLower(A_LoopField127) == StrLower(Trim(keyWordReturnStatement) + ";")) {
                lineDone = 1;
                if (langToConvertTo == langFileExtension_2) {
                    if (useSemicolon_2 == "on") {
            ...
            else if (SubStr(Trim(StrLower(A_LoopField127)), 1, StrLen(StrLower(keyWordReturnStatement))) == StrLower(keyWordReturnStatement)) {
                lineDone = 1;
                str1 = Trim(expressionParserTranspiler(Trim(StringTrimLeft(A_LoopField127, StrLen(keyWordReturnStatement)))));
                if (SubStrLastChars(str1, 1) == ";") {
                    str1 = StringTrimRight(str1, 1);
                }
                if (langToConvertTo == langFileExtension_2) {
            ...
            else if (StrLower(A_LoopField127) == StrLower(keyWordContinue) || StrLower(A_LoopField127) == StrLower(keyWordContinue + ";")) {
                lineDone = 1;
                if (langToConvertTo == langFileExtension_2) {
                    if (useSemicolon_2 == "on") {
            ...
            else if (StrLower(A_LoopField127) == "::" + keyWordContinue) {
                lineDone = 1;
                if (langToConvertTo == langFileExtension_2) {
                    htCode += "::" + keyWordContinue_2 + Chr(10);
                }
            ...
            else if (StrLower(A_LoopField127) == StrLower(keyWordLoopInfinite) || StrLower(A_LoopField127) == StrLower(keyWordLoopInfinite + ":")) {
                // infinity loops
            ...
            else if (SubStr(Trim(StrLower(A_LoopField127)), 1, StrLen(StrLower(keyWordLoop))) == StrLower(keyWordLoop) && SubStr(Trim(StrLower(A_LoopField127)), 1, StrLen(StrLower(keyWordLoop)) + 2) != StrLower(keyWordLoop) + "% " && SubStr(Trim(StrLower(A_LoopField127)), 1, StrLen(StrLower(keyWordLoopParse))) != StrLower(keyWordLoopParse)) {
                out2 = StringTrimLeft(A_LoopField127, StrLen(keyWordLoop));
            ...
            else if (SubStr(Trim(StrLower(A_LoopField127)), 1, StrLen(StrLower(keyWordLoop)) + 2) == StrLower(keyWordLoop) + "% ") {
                howMany_fixAindexInGoUnused++;
                out2 = StringTrimLeft(A_LoopField127, StrLen(keyWordLoop) + 2);
                out2 = StrReplace(out2, ":", "");
                //MsgBox % out2
                //MsgBox, % out2
                out2 = Trim(out2);
            ...
            else if (SubStr(StrLower(A_LoopField127), 1, StrLen(StrLower(keyWordLoopParse))) == StrLower(keyWordLoopParse)) {
                howMany_fixAindexInGoUnused++;
                var1 = StringTrimLeft(A_LoopField127, StrLen(keyWordLoopParse));
                out2 = StrReplace(out2, ":", "");
                lineDone = 1;
            ...
            else if (SubStrLastChars(Trim(A_LoopField127), StrLen(keyWordInc)) == keyWordInc || SubStrLastChars(Trim(A_LoopField127), StrLen(keyWordInc + ";")) == keyWordInc + ";" && lineDone == 0) {
                if (useSemicolon == "on") {
                    str1 = Trim(StringTrimRight(Trim(A_LoopField127), StrLen(keyWordInc + ";")));
                } else {
                    str1 = Trim(StringTrimRight(Trim(A_LoopField127), StrLen(keyWordInc)));
                }
                if (langToConvertTo == langFileExtension_2) {
            ...
            else if (SubStrLastChars(Trim(A_LoopField127), StrLen(keyWordDec)) == keyWordDec || SubStrLastChars(Trim(A_LoopField127), StrLen(keyWordDec + ";")) == keyWordDec + ";" && lineDone == 0) {
                if (useSemicolon == "on") {
                    str1 = Trim(StringTrimRight(Trim(A_LoopField127), StrLen(keyWordDec + ";")));
                } else {
                    str1 = Trim(StringTrimRight(Trim(A_LoopField127), StrLen(keyWordDec)));
                }
                if (langToConvertTo == langFileExtension_2) {
            ...
            else if (SubStr(StrLower(Trim(A_LoopField127)), 1, StrLen(StrLower(keyWordInclude + " "))) == StrLower(keyWordInclude + " ") && langToConvertTo == langFileExtension_2) {
                htCode += keyWordInclude_2 + " " + Trim(StringTrimLeft(Trim(A_LoopField127), StrLen(StrLower(keyWordInclude) + " "))) + Chr(10);
            }
            else if (StrLower(A_LoopField127) == StrLower(keyWordGlobal) || StrLower(A_LoopField127) == StrLower(keyWordGlobal + ";")) {
                if (langToConvertTo == "ahk") {
                    htCode += "global\n";
                }
            }
            else if (SubStr(StrLower(A_LoopField127), 1, StrLen(StrLower(keyWordAwait))) == StrLower(keyWordAwait)) {
                str1 = StringTrimLeft(A_LoopField127, StrLen(keyWordAwait));
            ...
            else if (SubStr(StrLower(A_LoopField127), 1, StrLen(StrLower(keyWordGlobal))) == StrLower(keyWordGlobal)) {
                str1 = StringTrimLeft(A_LoopField127, StrLen(keyWordGlobal));
                //MsgBox, % A_LoopField127
            ...
            else if (SubStr(StrLower(A_LoopField127), 1, StrLen(keyWordFunc)) == StrLower(keyWordFunc) || SubStr(StrLower(A_LoopField127), 1, StrLen(StrLower(keyWordAsync) + StrLower(keyWordFunc))) == StrLower(keyWordAsync) + StrLower(keyWordFunc) && InStr(StrLower(A_LoopField127), " main(") == false && lineDone == 0) {
                lineDone = 1;
                if (SubStr(StrLower(A_LoopField127), 1, StrLen(StrLower(keyWordAsync) + StrLower(keyWordFunc))) == StrLower(keyWordAsync) + StrLower(keyWordFunc)) {
                    // is async
                    int1 = 1;
                    str1 = Trim(StringTrimLeft(A_LoopField127, StrLen(StrLower(keyWordAsync + keyWordFunc))));
                } else {
                    // is async
                    int1 = 0;
                    str1 = Trim(StringTrimLeft(A_LoopField127, StrLen(StrLower(keyWordFunc))));
                }
                if (langToConvertTo != langFileExtension_2 && langToConvertTo != "cpp" && langToConvertTo != "go" && langToConvertTo != "cs" && langToConvertTo != "java" && langToConvertTo != "kt" && langToConvertTo != "nim" && langToConvertTo != "swift" && langToConvertTo != "dart" && langToConvertTo != "ts") {
                    if (str1 != "") {
                        ...
                        for (size_t A_Index129 = 0; A_Index129 < items129.size() + 0; A_Index129++) {
                            ...
                            if (InStr(A_LoopField129, " := ")) {
                                if (InStr(Trim(StrSplit(A_LoopField129, " := ", 1)), " ") != true) {
                                    str4 += "optanal-HTVM-opt-parmsNOtWorking-theirewIs-a-bug-Iusfbudfbuoeshfuisbav=-fwaegs-awedsf-dd-sfgc " + A_LoopField129 + ", ";
                                } else {
                                    str4 += A_LoopField129 + ", ";
                                }
                            }
                            else if (InStr(A_LoopField129, " = ")) {
                                if (InStr(Trim(StrSplit(A_LoopField129, " = ", 1)), " ") != true) {
                                    str4 += "optanal-HTVM-opt-parmsNOtWorking-theirewIs-a-bug-Iusfbudfbuoeshfuisbav=-fwaegs-awedsf-dd-sfgc " + A_LoopField129 + ", ";
                                } else {
                                    str4 += A_LoopField129 + ", ";
                            ...
                            else if (InStr(A_LoopField129, " " + Trim(keyWordAssign) + " ")) {
                                if (InStr(Trim(StrSplit(A_LoopField129, " " + Trim(keyWordAssign) + " ", 1)), " ") != true) {
                                    str4 += "optanal-HTVM-opt-parmsNOtWorking-theirewIs-a-bug-Iusfbudfbuoeshfuisbav=-fwaegs-awedsf-dd-sfgc " + A_LoopField129 + ", ";
                                } else {
                                    str4 += A_LoopField129 + ", ";
                                }
                ...
                if (Trim(str1) == "") {
                    ...
                    for (size_t A_Index130 = 0; A_Index130 < items130.size() + 0; A_Index130++) {
                        ...
                        //print("|" . Trim(A_LoopField130) . "|")
                        if (InStr(Trim(A_LoopField130), " ")) {
                            // there is space aka there is a type
                            if (InStr(A_LoopField130, " " + Trim(keyWordAssign) + " ") == false) {
                                if (usePrefixTypeForTypeDefinition == "on") {
                                    // normal
                                    str6 = Trim(A_LoopField130);
                                ...
                                // optanal param
                                if (InStr(Trim(StrSplit(Trim(A_LoopField130), " " + Trim(keyWordAssign) + " ", 1)), " ")) {
                                    // there is a type
                                    if (usePrefixTypeForTypeDefinition == "on") {
                                        // normal
                                        str6 = Trim(StrSplit(Trim(A_LoopField130), " " + Trim(keyWordAssign) + " ", 1));
                                        str9 = Trim(expressionParserTranspiler(Trim(StrSplit(Trim(A_LoopField130), " " + Trim(keyWordAssign) + " ", 2))));
                                        str7 = getTheLastWord(str6);
                                        str8 = getFuncTypeConvert(Trim(StringTrimRight(str6, StrLen(Trim(str7)) + 1)));
                                        //;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;
                                        //;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;
                                    ...
                                    // there is no type
                                    str8 = Trim(StrSplit(Trim(A_LoopField130), " " + Trim(keyWordAssign) + " ", 1));
                                    str9 = Trim(expressionParserTranspiler(Trim(StrSplit(Trim(A_LoopField130), " " + Trim(keyWordAssign) + " ", 2))));
                                    // str10
                                    // skipLeftCuleyForFuncPLS := 1
                                    //;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;
                            ...
                            str5 += Trim(A_LoopField130) + Chr(10);
                        }
                    }
                    //print(str5)
                    //print(str10)
                    ...
                    if (Trim(str5) != "" && Trim(str10) != "") {
                        ...
                        if (langToConvertTo == "lua") {
                            std::vector<std::string> items133 = LoopParseFunc(str12, ",");
                            for (size_t A_Index133 = 0; A_Index133 < items133.size() + 0; A_Index133++) {
                                std::string A_LoopField133 = items133[A_Index133 - 0];
                                str14 += Trim(StrSplit(Trim(A_LoopField133), " = ", 1)) + ", ";
                            }
                            str14 = StringTrimRight(str14, 2);
                            str13 += "{" + Chr(10);
                            std::vector<std::string> items134 = LoopParseFunc(str12, ",");
                            for (size_t A_Index134 = 0; A_Index134 < items134.size() + 0; A_Index134++) {
                                std::string A_LoopField134 = items134[A_Index134 - 0];
                                str13 += Trim(StrSplit(Trim(A_LoopField134), " = ", 1)) + " = " + Trim(StrSplit(Trim(A_LoopField134), " = ", 1)) + " or " + Trim(StrSplit(Trim(A_LoopField134), " = ", 2)) + Chr(10);
                            }
                            str12 = str14;
                        }
                        if (langToConvertTo == "go") {
                            ...
                            str13 += "{" + Chr(10) + " ";
                            std::vector<std::string> items135 = LoopParseFunc(str12, ",");
                            for (size_t A_Index135 = 0; A_Index135 < items135.size() + 0; A_Index135++) {
                                std::string A_LoopField135 = items135[A_Index135 - 0];
                                str16 = Trim(StrSplit(Trim(StrSplit(Trim(A_LoopField135), " = ", 1)), " ", 1));
                                str17 = Trim(StrSplit(Trim(A_LoopField135), " = ", 2));
                            ...
                            for (size_t A_Index136 = 0; A_Index136 < items136.size() + 0; A_Index136++) {
                                ...
                                str16 = Trim(StrSplit(Trim(StrSplit(Trim(A_LoopField136), " = ", 1)), " ", 1));
                                str18 = Trim(StrSplit(Trim(StrSplit(Trim(A_LoopField136), " = ", 1)), " ", 2));
                                // str16 = var name
                                // str18 = var type
                                str13 += "if len(__HTVM_V2_TO_GO_optionalParams__) " + Chr(62) + " " + STR(A_Index136) + Chr(10) + "{" + Chr(10) + str16 + " = __HTVM_V2_TO_GO_optionalParams__[" + STR(A_Index136) + "].(" + str18 + ")" + Chr(10) + "}" + Chr(10);
                            }
                            str12 = str14;
                        }
                        if (langToConvertTo == "java") {
                            ...
                            for (size_t A_Index137 = 0; A_Index137 < items137.size() + 0; A_Index137++) {
                                ...
                                str19 = Trim(A_LoopField137);
                                str20 = "";
                                std::vector<std::string> items138 = LoopParseFunc(str19, " ");
                                for (size_t A_Index138 = 0; A_Index138 < items138.size() + 0; A_Index138++) {
                                    std::string A_LoopField138 = items138[A_Index138 - 0];
                                    if (A_Index138 == 3) {
                                        if (RegExMatch(A_LoopField138, "^\\d+(\\.\\d+)?$")) {
                                            str20 += A_LoopField138 + "f ";
                                        } else {
                                            str20 += A_LoopField138 + " ";
                                        }
                                    } else {
                                        str20 += A_LoopField138 + " ";
                                    }
                            ...
                            for (size_t A_Index139 = 0; A_Index139 < items139.size() + 0; A_Index139++) {
                                ...
                                str16 = Trim(StrSplit(Trim(StrSplit(Trim(A_LoopField139), " = ", 1)), " ", 2));
                                str17 = Trim(StrSplit(Trim(StrSplit(Trim(A_LoopField139), " = ", 1)), " ", 1));
                                // str16 = var name
                                // str17 = type name
                                str13 += "if (__HTVM_V2_TO_JAVA_optionalParams__.length " + Chr(62) + " " + STR(A_Index139) + ") " + str16 + " = (" + str17 + ") __HTVM_V2_TO_JAVA_optionalParams__[" + STR(A_Index139) + "];" + Chr(10);
                            }
                            str12 = str14;
                        }
                        str11 += str12;
                    ...
                    if (Trim(str5) == "" && Trim(str10) != "") {
                        ...
                        if (langToConvertTo == "lua") {
                            std::vector<std::string> items142 = LoopParseFunc(str11, ",");
                            for (size_t A_Index142 = 0; A_Index142 < items142.size() + 0; A_Index142++) {
                                std::string A_LoopField142 = items142[A_Index142 - 0];
                                str14 += Trim(StrSplit(Trim(A_LoopField142), " = ", 1)) + ", ";
                            }
                            str14 = StringTrimRight(str14, 2);
                            str13 += "{" + Chr(10);
                            std::vector<std::string> items143 = LoopParseFunc(str11, ",");
                            for (size_t A_Index143 = 0; A_Index143 < items143.size() + 0; A_Index143++) {
                                std::string A_LoopField143 = items143[A_Index143 - 0];
                                str13 += Trim(StrSplit(Trim(A_LoopField143), " = ", 1)) + " = " + Trim(StrSplit(Trim(A_LoopField143), " = ", 1)) + " or " + Trim(StrSplit(Trim(A_LoopField143), " = ", 2)) + Chr(10);
                            }
                            str11 = str14;
                        }
                        if (langToConvertTo == "go") {
                            str14 = "__HTVM_V2_TO_GO_optionalParams__ ...interface{}";
                            str13 += "{" + Chr(10) + " ";
                            std::vector<std::string> items144 = LoopParseFunc(str11, ",");
                            for (size_t A_Index144 = 0; A_Index144 < items144.size() + 0; A_Index144++) {
                                std::string A_LoopField144 = items144[A_Index144 - 0];
                                str16 = Trim(StrSplit(Trim(StrSplit(Trim(A_LoopField144), " = ", 1)), " ", 1));
                                str17 = Trim(StrSplit(Trim(A_LoopField144), " = ", 2));
                                str13 += str16 + " := " + str17 + Chr(10);
                            }
                            ...
                            for (size_t A_Index145 = 0; A_Index145 < items145.size() + 0; A_Index145++) {
                                ...
                                str16 = Trim(StrSplit(Trim(StrSplit(Trim(A_LoopField145), " = ", 1)), " ", 1));
                                str18 = Trim(StrSplit(Trim(StrSplit(Trim(A_LoopField145), " = ", 1)), " ", 2));
                                // str16 = var name
                                // str18 = var type
                                str13 += "if len(__HTVM_V2_TO_GO_optionalParams__) " + Chr(62) + " " + STR(A_Index145) + Chr(10) + "{" + Chr(10) + str16 + " = __HTVM_V2_TO_GO_optionalParams__[" + STR(A_Index145) + "].(" + str18 + ")" + Chr(10) + "}" + Chr(10);
                            }
                            str11 = str14;
                        }
                        if (langToConvertTo == "java") {
                            ...
                            for (size_t A_Index146 = 0; A_Index146 < items146.size() + 0; A_Index146++) {
                                ...
                                str19 = Trim(A_LoopField146);
                                str20 = "";
                                std::vector<std::string> items147 = LoopParseFunc(str19, " ");
                                for (size_t A_Index147 = 0; A_Index147 < items147.size() + 0; A_Index147++) {
                                    std::string A_LoopField147 = items147[A_Index147 - 0];
                                    if (A_Index147 == 3) {
                                        if (RegExMatch(A_LoopField147, "^\\d+(\\.\\d+)?$")) {
                                            str20 += A_LoopField147 + "f ";
                                        } else {
                                            str20 += A_LoopField147 + " ";
                                        }
                                    } else {
                                        str20 += A_LoopField147 + " ";
                                    }
                            ...
                            for (size_t A_Index148 = 0; A_Index148 < items148.size() + 0; A_Index148++) {
                                ...
                                str16 = Trim(StrSplit(Trim(StrSplit(Trim(A_LoopField148), " = ", 1)), " ", 2));
                                str17 = Trim(StrSplit(Trim(StrSplit(Trim(A_LoopField148), " = ", 1)), " ", 1));
                                // str16 = var name
                                // str17 = type name
                                str13 += "if (__HTVM_V2_TO_JAVA_optionalParams__.length " + Chr(62) + " " + STR(A_Index148) + ") " + str16 + " = (" + str17 + ") __HTVM_V2_TO_JAVA_optionalParams__[" + STR(A_Index148) + "];" + Chr(10);
                            }
                            str11 = str14;
                        }
                    }
            ...
            else if (InStr(StrLower(A_LoopField127), " main(") && lineDone == 0) {
                lineDone = 1;
                javaMainFuncSeen = 1;
                csMainFuncSeen = 1;
                htCode += A_LoopField127 + Chr(10);
            }
            else if (InStr(A_LoopField127, " " + keyWordAssign + " ") || InStr(A_LoopField127, " " + keyWordAssignAdd + " ") || InStr(A_LoopField127, " " + keyWordAssignConcat + " ") || InStr(A_LoopField127, " " + keyWordAssignSub + " ") || InStr(A_LoopField127, " " + keyWordAssignMul + " ") || InStr(A_LoopField127, " " + keyWordAssignDiv + " ") || InStr(A_LoopField127, " " + keyWordAssignMod + " ") || InStr(A_LoopField127, " " + keyWordAssignShiftLeft + " ") || InStr(A_LoopField127, " " + keyWordAssignShiftRight + " ") || InStr(A_LoopField127, " " + keyWordLogicalAssignShiftRight + " ") || InStr(A_LoopField127, " " + keyWordAssignBitAnd + " ") || InStr(A_LoopField127, " " + keyWordAssignBitOr + " ") || InStr(A_LoopField127, " " + keyWordAssignBitXor + " ") && lineDone == 0) {
                allVarsSoWeDontReDecVars_FIX_TOGGLE = 1;
                //keyWordAssign
                //keyWordAssignAdd
                //keyWordAssignConcat
                ...
                //allVarsSoWeDontReDecVars_FIX_int64
                //allVarsSoWeDontReDecVars_FIX_float
                if (InStr(A_LoopField127, " " + keyWordAssign + " ")) {
                    str1 = StrSplit(A_LoopField127, " " + keyWordAssign + " ", 1);
                    str6 = StrSplit(A_LoopField127, " " + keyWordAssign + " ", 2);
                    str10 = keyWordAssign;
                    str11 = keyWordAssign_2;
                    if (langToConvertTo == "ahk") {
                        str12 = ":=";
                ...
                else if (InStr(A_LoopField127, " " + keyWordAssignAdd + " ")) {
                    str1 = StrSplit(A_LoopField127, " " + keyWordAssignAdd + " ", 1);
                    str6 = StrSplit(A_LoopField127, " " + keyWordAssignAdd + " ", 2);
                    str10 = keyWordAssignAdd;
                    str11 = keyWordAssignAdd_2;
                    str12 = "+=";
                }
                else if (InStr(A_LoopField127, " " + keyWordAssignConcat + " ")) {
                    str1 = StrSplit(A_LoopField127, " " + keyWordAssignConcat + " ", 1);
                    str6 = StrSplit(A_LoopField127, " " + keyWordAssignConcat + " ", 2);
                    str10 = keyWordAssignConcat;
                    str11 = keyWordAssignConcat_2;
                    if (langToConvertTo == "ahk") {
                        str12 = ".=";
                ...
                else if (InStr(A_LoopField127, " " + keyWordAssignSub + " ")) {
                    str1 = StrSplit(A_LoopField127, " " + keyWordAssignSub + " ", 1);
                    str6 = StrSplit(A_LoopField127, " " + keyWordAssignSub + " ", 2);
                    str10 = keyWordAssignSub;
                    str11 = keyWordAssignSub_2;
                    str12 = "-=";
                }
                else if (InStr(A_LoopField127, " " + keyWordAssignMul + " ")) {
                    str1 = StrSplit(A_LoopField127, " " + keyWordAssignMul + " ", 1);
                    str6 = StrSplit(A_LoopField127, " " + keyWordAssignMul + " ", 2);
                    str10 = keyWordAssignMul;
                    str11 = keyWordAssignMul_2;
                    str12 = "*=";
                }
                else if (InStr(A_LoopField127, " " + keyWordAssignDiv + " ")) {
                    str1 = StrSplit(A_LoopField127, " " + keyWordAssignDiv + " ", 1);
                    str6 = StrSplit(A_LoopField127, " " + keyWordAssignDiv + " ", 2);
                    str10 = keyWordAssignDiv;
                    str11 = keyWordAssignDiv_2;
                    str12 = "/=";
                }
                else if (InStr(A_LoopField127, " " + keyWordAssignMod + " ")) {
                    str1 = StrSplit(A_LoopField127, " " + keyWordAssignMod + " ", 1);
                    str6 = StrSplit(A_LoopField127, " " + keyWordAssignMod + " ", 2);
                    str10 = keyWordAssignMod;
                    str11 = keyWordAssignMod_2;
                    str12 = "%=";
                }
                else if (InStr(A_LoopField127, " " + keyWordAssignShiftLeft + " ")) {
                    str1 = StrSplit(A_LoopField127, " " + keyWordAssignShiftLeft + " ", 1);
                    str6 = StrSplit(A_LoopField127, " " + keyWordAssignShiftLeft + " ", 2);
                    str10 = keyWordAssignShiftLeft;
                    str11 = keyWordAssignShiftLeft_2;
                    str12 = "<<=";
                }
                else if (InStr(A_LoopField127, " " + keyWordAssignShiftRight + " ")) {
                    str1 = StrSplit(A_LoopField127, " " + keyWordAssignShiftRight + " ", 1);
                    str6 = StrSplit(A_LoopField127, " " + keyWordAssignShiftRight + " ", 2);
                    str10 = keyWordAssignShiftRight;
                    str11 = keyWordAssignShiftRight_2;
                    str12 = ">>=";
                }
                else if (InStr(A_LoopField127, " " + keyWordLogicalAssignShiftRight + " ")) {
                    str1 = StrSplit(A_LoopField127, " " + keyWordLogicalAssignShiftRight + " ", 1);
                    str6 = StrSplit(A_LoopField127, " " + keyWordLogicalAssignShiftRight + " ", 2);
                    str10 = keyWordLogicalAssignShiftRight;
                    str11 = keyWordLogicalAssignShiftRight_2;
                    str12 = ">>>=";
                }
                else if (InStr(A_LoopField127, " " + keyWordAssignBitAnd + " ")) {
                    str1 = StrSplit(A_LoopField127, " " + keyWordAssignBitAnd + " ", 1);
                    str6 = StrSplit(A_LoopField127, " " + keyWordAssignBitAnd + " ", 2);
                    str10 = keyWordAssignBitAnd;
                    str11 = keyWordAssignBitAnd_2;
                    str12 = "&=";
                }
                else if (InStr(A_LoopField127, " " + keyWordAssignBitOr + " ")) {
                    str1 = StrSplit(A_LoopField127, " " + keyWordAssignBitOr + " ", 1);
                    str6 = StrSplit(A_LoopField127, " " + keyWordAssignBitOr + " ", 2);
                    str10 = keyWordAssignBitOr;
                    str11 = keyWordAssignBitOr_2;
                    str12 = "|=";
                }
                else if (InStr(A_LoopField127, " " + keyWordAssignBitXor + " ")) {
                    str1 = StrSplit(A_LoopField127, " " + keyWordAssignBitXor + " ", 1);
                    str6 = StrSplit(A_LoopField127, " " + keyWordAssignBitXor + " ", 2);
                    str10 = keyWordAssignBitXor;
                    str11 = keyWordAssignBitXor_2;
                    str12 = "^=";
                }
                ...
                if (InStr(ALoopField, " ") == false) {
                    ...
                    if (usePostfixTypeForTypeDefinition == "off") {
                        lineDone = 1;
                        str1 = Trim(StringTrimLeft(Trim(str1), StrLen(keyWordConst)));
                        std::vector<std::string> items149 = LoopParseFunc(str1, " ");
                    ...
                    if (usePostfixTypeForTypeDefinition == "off") {
                        str1 = Trim(StringTrimLeft(Trim(str1), StrLen(keyWordVar)));
                        std::vector<std::string> items150 = LoopParseFunc(str1, " ");
                    ...
                    if (usePostfixTypeForTypeDefinition == "off") {
                        str1 = Trim(StringTrimLeft(Trim(str1), StrLen(keyWordLet)));
                        std::vector<std::string> items151 = LoopParseFunc(str1, " ");
                    ...
                    lineDone = 1;
                    std::vector<std::string> items152 = LoopParseFunc(str1, " ");
            ...
            else if (SubStr(Trim(A_LoopField127), 1, StrLen(keyWordVar)) == keyWordVar && lineDone == 0) {
                allVarsSoWeDontReDecVars_FIX_TOGGLE = 1;
                if (usePostfixTypeForTypeDefinition == "off") {
                    lineDone = 1;
                    str1 = Trim(StringTrimLeft(Trim(A_LoopField127), StrLen(keyWordVar)));
                    if (SubStrLastChars(str1, 1) == ";") {
                        str1 = StringTrimRight(str1, 1);
                    }
            ...
            else if (SubStr(Trim(A_LoopField127), 1, StrLen(keyWordLet)) == keyWordLet && lineDone == 0) {
                allVarsSoWeDontReDecVars_FIX_TOGGLE = 1;
                if (usePostfixTypeForTypeDefinition == "off") {
                    lineDone = 1;
                    str1 = Trim(StringTrimLeft(Trim(A_LoopField127), StrLen(keyWordLet)));
                    if (SubStrLastChars(str1, 1) == ";") {
                        str1 = StringTrimRight(str1, 1);
                    }
            ...
            else if (SubStr(Trim(A_LoopField127), 1, StrLen(keyWordDouble + " ")) == keyWordDouble + " " || SubStr(Trim(A_LoopField127), 1, StrLen(keyWordChar + " ")) == keyWordChar + " " || SubStr(Trim(A_LoopField127), 1, StrLen(keyWordUint8 + " ")) == keyWordUint8 + " " || SubStr(Trim(A_LoopField127), 1, StrLen(keyWordUint16 + " ")) == keyWordUint16 + " " || SubStr(Trim(A_LoopField127), 1, StrLen(keyWordUint32 + " ")) == keyWordUint32 + " " || SubStr(Trim(A_LoopField127), 1, StrLen(keyWordUint64 + " ")) == keyWordUint64 + " " || SubStr(Trim(A_LoopField127), 1, StrLen(keyWordINT + " ")) == keyWordINT + " " || SubStr(Trim(A_LoopField127), 1, StrLen(keyWordSTR + " ")) == keyWordSTR + " " || SubStr(Trim(A_LoopField127), 1, StrLen(keyWordBOOL + " ")) == keyWordBOOL + " " || SubStr(Trim(A_LoopField127), 1, StrLen(keyWordFLOAT + " ")) == keyWordFLOAT + " " || SubStr(Trim(A_LoopField127), 1, StrLen(keyWordINT8 + " ")) == keyWordINT8 + " " || SubStr(Trim(A_LoopField127), 1, StrLen(keyWordINT16 + " ")) == keyWordINT16 + " " || SubStr(Trim(A_LoopField127), 1, StrLen(keyWordINT32 + " ")) == keyWordINT32 + " " || SubStr(Trim(A_LoopField127), 1, StrLen(keyWordINT64 + " ")) == keyWordINT64 + " " || SubStr(Trim(A_LoopField127), 1, StrLen(keyWordArrayDefinition + " ")) == keyWordArrayDefinition + " " || SubStr(Trim(A_LoopField127), 1, StrLen(keyWordArrayOfIntegersDefinition + " ")) == keyWordArrayOfIntegersDefinition + " " || SubStr(Trim(A_LoopField127), 1, StrLen(keyWordArrayOfStringsDefinition + " ")) == keyWordArrayOfStringsDefinition + " " || SubStr(Trim(A_LoopField127), 1, StrLen(keyWordArrayOfFloatingPointNumbersDefinition + " ")) == keyWordArrayOfFloatingPointNumbersDefinition + " " || SubStr(Trim(A_LoopField127), 1, StrLen(keyWordArrayOfBooleansDefinition + " ")) == keyWordArrayOfBooleansDefinition + " " && usePrefixTypeForTypeDefinition == "on" && lineDone == 0) {
                lineDone = 1;
                allVarsSoWeDontReDecVars_FIX_TOGGLE = 1;
                str1 = Trim(A_LoopField127);
                if (SubStrLastChars(str1, 1) == ";") {
            ...
            else if (RegExMatch(Trim(A_LoopField127), "^[a-zA-Z_][a-zA-Z0-9_]*: [^:]*;?$") && usePostfixTypeForTypeDefinition == "on" && lineDone == 0) {
                lineDone = 1;
                allVarsSoWeDontReDecVars_FIX_TOGGLE = 1;
                str1 = Trim(A_LoopField127);
                if (SubStrLastChars(str1, 1) == ";") {
            ...
            else if (RegExMatch(Trim(A_LoopField127), "^[a-zA-Z0-9_\\.]+\\(") && !(InStr(StrLower(A_LoopField127), " main(")) && lineDone == 0) {
                lineDone = 1;
                str1 = Trim(A_LoopField127);
                if (SubStrLastChars(str1, 1) == ";") {
                    str1 = StringTrimRight(str1, 1);
            ...
            else if (RegExMatch(A_LoopField127, "\\.[a-zA-Z_][a-zA-Z0-9_]*") && lineDone == 0) {
                lineDone = 1;
                str1 = Trim(A_LoopField127);
                if (SubStrLastChars(str1, 1) == ";") {
                    str1 = StringTrimRight(str1, 1);
                ...
                if (langToConvertTo != langFileExtension_2) {
                    if (langToConvertTo == "py" || langToConvertTo == "nim" || langToConvertTo == "ahk" || langToConvertTo == "go" || langToConvertTo == "lua" || langToConvertTo == "kt" || langToConvertTo == "rb" || langToConvertTo == "swift" || langToConvertTo == "groovy") {
                        htCode += extraFlexableFuncCalls(str1) + Chr(10);
                    } else {
                        htCode += extraFlexableFuncCalls(str1) + ";" + Chr(10);
                    }
                } else {
            ...
            else if (KeyWordsCommands(A_LoopField127, "check", commands, langToConvertTo) == "true" && lineDone == 0) {
                lineDone = 1;
                if (langToConvertTo == langFileExtension_2) {
                    htCode += A_LoopField127 + Chr(10);
                } else {
                    out_KeyWordsCommands = KeyWordsCommands(A_LoopField127, "transpile", commands, langToConvertTo);
                    htCode += out_KeyWordsCommands + Chr(10);
                }
            } else {
                //print("else else else " . A_LoopField127)
                // this is THE else
                //;;;;;;;;;;;;;;;;;;
                if (lineDone != 1) {
                    if (skipLeftCuleyForFuncPLS != 1) {
                        if (SubStr(Trim(StrLower(A_LoopField127)), 1, 1) == Chr(125)) {
                            htCode += Chr(125) + Chr(10);
                        } else {
                            if (htCodeAcurlyBraceAddSomeVrasFixLP == 1 && SubStr(Trim(StrLower(A_LoopField127)), 1, 1) == Chr(123)) {
                                htCodeAcurlyBraceAddSomeVrasFixLP = 0;
                                htCode += A_LoopField127 + Chr(10) + theFixTextLoopLP + Chr(10);
                            } else {
                                if (htCodeAcurlyBraceAddSomeVrasFixNL == 1 && SubStr(Trim(StrLower(A_LoopField127)), 1, 1) == Chr(123)) {
                                    htCodeAcurlyBraceAddSomeVrasFixNL = 0;
                                    htCode += A_LoopField127 + Chr(10) + theFixTextLoopNL + Chr(10);
                                } else {
                                    htCode += A_LoopField127 + Chr(10);
                                }
                            }
        ...
        if (langToConvertTo != "ahk" && langToConvertTo != langFileExtension_2) {
            ...
            if (haveWeEverUsedAloop == 1) {
                ...
                std::vector<std::string> items156 = LoopParseFunc(htCodeLoopfixa, "\n", "\r");
                for (size_t A_Index156 = 0; A_Index156 < items156.size() + 0; A_Index156++) {
                    std::string A_LoopField156 = items156[A_Index156 - 0];
                    sstr123 = A_LoopField156;
                    fixLoopLokingFor = A_LoopField156;
                    ...
                    if (out1 == "nl") {
                        ...
                        for (size_t A_Index157 = 0; A_Index157 < items157.size() + 0; A_Index157++) {
                            ...
                            //MsgBox, |%A_LoopField157%|`n|%fixLoopLokingFor%|
                            if (InStr(A_LoopField157, fixLoopLokingFor) && insdeAnestedLoopBAD != 1) {
                                fixLoopLokingForNum = 1;
                                //MsgBox, do we came here 1
                            }
                            if (SubStr(Trim(A_LoopField157), 1, 4) == "for " && weAreDoneHereCurly != 1 && insdeAnestedLoopBAD != 1 && fixLoopLokingForNum == 1) {
                                s = StrSplit(A_LoopField157 ,"" + keyWordAIndex + "", 2);
                                out1z = s;
                                s = StrSplit(out1z, " ", 1);
                                out1z = Trim(s);
                                //MsgBox, % out1z
                                ...
                                foundTheTopLoop++;
                                inTarget = 1;
                                //MsgBox, % A_LoopField157
                                dontSaveStr = 1;
                                ALoopField = A_LoopField157;
                                DeleayOneCuzOfLoopParse = 1;
                                out4758686d86d86d86578991a += ALoopField + Chr(10);
                            }
                            if (inTarget == 1 && InStr(A_LoopField157, Chr(123)) && insdeAnestedLoopBAD != 1) {
                                insideBracket = 1;
                            }
                            if (insideBracket == 1 && InStr(A_LoopField157, Chr(123)) && insdeAnestedLoopBAD != 1) {
                                netsedCurly++;
                            }
                            if (insideBracket == 1 && InStr(A_LoopField157, Chr(125)) && insdeAnestedLoopBAD != 1) {
                                netsedCurly--;
                                readyToEnd = 1;
                            }
                            if (SubStr(Trim(A_LoopField157), 1, 4) == "for " && insdeAnestedLoopBAD != 1 && foundTheTopLoop >= 2) {
                            ...
                            if (insdeAnestedLoopBAD == 1) {
                                if (InStr(A_LoopField157, Chr(123))) {
                                    insideBracket1 = 1;
                                }
                                if (insideBracket1 == 1 && InStr(A_LoopField157, Chr(123))) {
                                    netsedCurly1++;
                                }
                                if (insideBracket1 == 1 && InStr(A_LoopField157, Chr(125))) {
                                    netsedCurly1--;
                                    readyToEnd1 = 1;
                                }
                                if (InStr(A_LoopField157, Chr(125)) && readyToEnd1 == 1 && netsedCurly1 == 0 && insideBracket == 1) {
                                    //MsgBox, % A_LoopField157
                                    eldLoopNestedBADlol = 1;
                                }
                                out4758686d86d86d86578991a += A_LoopField157 + Chr(10);
                            }
                            ...
                            if (inTarget == 1 && InStr(A_LoopField157, Chr(125)) && readyToEnd == 1 && netsedCurly == 0 && weAreDoneHereCurly == 0 && dontSaveStr != 1 && insdeAnestedLoopBAD != 1) {
                                //MsgBox, % A_LoopField157
                        ...
                        for (size_t A_Index158 = 0; A_Index158 < items158.size() + 0; A_Index158++) {
                            ...
                            if (InStr(A_LoopField158 , fixLoopLokingFor) && insdeAnestedLoopBAD != 1) {
                                fixLoopLokingForNum = 1;
                                //MsgBox, do we came here 3
                            }
                            if (SubStr(Trim(A_LoopField158), 1, 4) == "for " && weAreDoneHereCurly != 1 && insdeAnestedLoopBAD != 1 && fixLoopLokingForNum == 1) {
                                s = StrSplit(A_LoopField158 ,"" + keyWordAIndex + "", 2);
                                out1z = s;
                                s = StrSplit(out1z, " ", 1);
                                ...
                                foundTheTopLoop++;
                                inTarget = 1;
                                //MsgBox, % A_LoopField158
                                dontSaveStr = 1;
                                ALoopField = A_LoopField158;
                                DeleayOneCuzOfLoopParse = 1;
                                out4758686d86d86d86578991a += ALoopField + Chr(10);
                            }
                            if (inTarget == 1 && InStr(A_LoopField158, Chr(123)) && insdeAnestedLoopBAD != 1) {
                                insideBracket = 1;
                            }
                            if (insideBracket == 1 && InStr(A_LoopField158, Chr(123)) && insdeAnestedLoopBAD != 1) {
                                netsedCurly++;
                            }
                            if (insideBracket == 1 && InStr(A_LoopField158, Chr(125)) && insdeAnestedLoopBAD != 1) {
                                netsedCurly--;
                                readyToEnd = 1;
                            }
                            if (SubStr(Trim(A_LoopField158), 1, 4) == "for " && insdeAnestedLoopBAD != 1 && foundTheTopLoop >= 2) {
                                insdeAnestedLoopBAD = 1;
                                insideBracket1 = 0;
                                netsedCurly1 = 0;
                            }
                            ...
                            if (insdeAnestedLoopBAD == 1) {
                                if (InStr(A_LoopField158, Chr(123))) {
                                    insideBracket1 = 1;
                                }
                                if (insideBracket1 == 1 && InStr(A_LoopField158, Chr(123))) {
                                    netsedCurly1++;
                                }
                                if (insideBracket1 == 1 && InStr(A_LoopField158, Chr(125))) {
                                    netsedCurly1--;
                                    readyToEnd1 = 1;
                                }
                                if (InStr(A_LoopField158, Chr(125)) && readyToEnd1 == 1 && netsedCurly1 == 0 && insideBracket == 1) {
                                    //MsgBox, % A_LoopField158
                                    eldLoopNestedBADlol = 1;
                                }
                                out4758686d86d86d86578991a += A_LoopField158 + Chr(10);
                            }
                            if (inTarget == 1 && dontSaveStr != 1 && fixLoopLokingForNum != 1 && insdeAnestedLoopBAD != 1) {
                                ALoopField = A_LoopField158;
                                // Replace "A_Index" with or without a following digit with "A_Index" + out1z
                            ...
                            if ((inTarget == 1 && InStr(A_LoopField158, Chr(125)) && readyToEnd == 1 && netsedCurly == 0 && weAreDoneHereCurly == 0 && dontSaveStr != 1 && insdeAnestedLoopBAD != 1)) {
                                //MsgBox, % A_LoopField158
                                weAreDoneHereCurly = 1;
                                inTarget = 0;
                                endBracketDOntPutThere = 1;
                ...
                for (size_t A_Index159 = 0; A_Index159 < items159.size() + 0; A_Index159++) {
                    ...
                    ignore = 0;
                    if (SubStr(Trim(A_LoopField159), 1, 4) == "for ") {
                        if (hold == 1 && holdText == A_LoopField159) {
                            ignore = 1;
                        } else {
                            holdText = A_LoopField159;
                            hold = 1;
                        }
                    }
                    if (!ignore) {
                        out4758686d86dgt8r754444444 += A_LoopField159 + Chr(10);
                    }
                }
    ...
    if (COUNT_programmingBlock_HTVMsyntax != 0) {
        ...
        for (int A_Index161 = 0; A_Index161 < COUNT_programmingBlock_HTVMsyntax + 0; A_Index161++) {
            ADD_ALL_programmingBlock_HTVMsyntax += programmingBlock_HTVMsyntax[A_Index161] + Chr(10);
        }
        ...
        isNotHTVMfile2 = 1;
        for (int A_Index162 = 0; A_Index162 < COUNT_programmingBlock_HTVMsyntax + 0; A_Index162++) {
            htCode = StrReplace(htCode, "programmingBlock_HTVMsyntax-programmingBlock_HTVMsyntax-AA" + STR(A_Index162 + 1) + "AA", compiler(programmingBlock_HTVMsyntax[A_Index162], allInstructionFile, "full", langToConvertToParam));
        }
        isNotHTVMfile2 = 0;
    }
    //;;;;;;;;;;;;;;;;;;;;;;;;;;;
    ...
    if (langToConvertTo == langFileExtension_2) {
        ...
        if (useEnd_2 == "on") {
            ...
            for (size_t A_Index163 = 0; A_Index163 < items163.size() + 0; A_Index163++) {
                ...
                if (Trim(A_LoopField163) == keyWordCurlyBraceClose_2) {
                    fixLuaAndRuby += StrReplace(A_LoopField163, keyWordCurlyBraceClose_2, Trim(keyWordEnd_2)) + Chr(10);
                }
                else if (SubStrLastChars(A_LoopField163, 2) == " " + keyWordCurlyBraceOpen_2) {
                    fixLuaAndRuby += StringTrimRight(A_LoopField163, 2) + Chr(10);
                } else {
                    fixLuaAndRuby += A_LoopField163 + Chr(10);
                }
            }
        ...
        if (langToConvertTo == "lua" || langToConvertTo == "rb" || langToConvertTo == langFileExtension_2) {
            if (langToConvertTo == langFileExtension_2 && useEnd_2 == "on") {
                ...
                for (size_t A_Index164 = 0; A_Index164 < items164.size() + 0; A_Index164++) {
                    ...
                    if (Trim(A_LoopField164) == "}") {
                        fixLuaAndRuby += StrReplace(A_LoopField164, "}", fixLuaAndRubyHELP) + Chr(10);
                    }
                    else if (SubStrLastChars(A_LoopField164, 2) == " {") {
                        fixLuaAndRuby += StringTrimRight(A_LoopField164, 2) + Chr(10);
                    } else {
                        fixLuaAndRuby += A_LoopField164 + Chr(10);
                    }
                }
                htCode = StringTrimRight(fixLuaAndRuby, 1);
            } else {
                ...
                for (size_t A_Index165 = 0; A_Index165 < items165.size() + 0; A_Index165++) {
                    ...
                    if (Trim(A_LoopField165) == "}") {
                        fixLuaAndRuby += StrReplace(A_LoopField165, "}", fixLuaAndRubyHELP) + Chr(10);
                    }
                    else if (SubStrLastChars(A_LoopField165, 2) == " {") {
                        fixLuaAndRuby += StringTrimRight(A_LoopField165, 2) + Chr(10);
                    } else {
                        fixLuaAndRuby += A_LoopField165 + Chr(10);
                    }
                }
                htCode = StringTrimRight(fixLuaAndRuby, 1);
            }
    ...
    if (langToConvertTo == langFileExtension_2 && useEnd_2 == "on") {
        ...
        for (size_t A_Index166 = 0; A_Index166 < items166.size() + 0; A_Index166++) {
            ...
            HTVM_Append(nextWordEndFix, A_LoopField166);
        }
        ...
        for (size_t A_Index167 = 0; A_Index167 < items167.size() + 0; A_Index167++) {
            ...
            if (Trim(A_LoopField167) == Trim(keyWordEnd_2) && SubStr(Trim(nextWordEndFix[A_Index167 + 1]), 1, StrLen(Trim(keyWordElseIf_2) + " ")) == Trim(keyWordElseIf_2) + " " || Trim(A_LoopField167) == Trim(keyWordEnd_2) && (Trim(nextWordEndFix[A_Index167 + 1]) == Trim(keyWordElse_2) || Trim(nextWordEndFix[A_Index167 + 1]) == Trim(keyWordElse_2 + ":"))) {
                nextWordEndFixOut += "";
            } else {
                nextWordEndFixOut += A_LoopField167 + Chr(10);
            }
        }
    ...
    if (langToConvertTo == "lua") {
        ...
        for (size_t A_Index168 = 0; A_Index168 < items168.size() + 0; A_Index168++) {
            ...
            HTVM_Append(nextWordEndFix, A_LoopField168);
        }
        HTVM_Append(nextWordEndFix, " ");
        ...
        for (size_t A_Index169 = 0; A_Index169 < items169.size() + 0; A_Index169++) {
            ...
            if (Trim(A_LoopField169) == "end" && SubStr(Trim(nextWordEndFix[A_Index169 + 1]), 1, StrLen("elseif ")) == "elseif " || Trim(A_LoopField169) == "end" && Trim(nextWordEndFix[A_Index169 + 1]) == "else") {
                nextWordEndFixOut += "";
            } else {
                nextWordEndFixOut += A_LoopField169 + Chr(10);
            }
        }
        htCode = StringTrimRight(nextWordEndFixOut, 1);
    }
    if (langToConvertTo == "rb") {
        ...
        for (size_t A_Index171 = 0; A_Index171 < items171.size() + 0; A_Index171++) {
            ...
            if (Trim(A_LoopField171) == "end" && SubStr(Trim(nextWordEndFix[A_Index171 + 1]), 1, StrLen("elsif ")) == "elsif " || Trim(A_LoopField171) == "end" && Trim(nextWordEndFix[A_Index171 + 1]) == "else") {
                nextWordEndFixOut += "";
            } else {
    ...
    if (langToConvertTo == "rb") {
        ...
        for (size_t A_Index172 = 0; A_Index172 < items172.size() + 0; A_Index172++) {
            ...
            HTVM_Append(nextWordEndFixRB, A_LoopField172);
        }
        ...
        for (size_t A_Index173 = 0; A_Index173 < items173.size() + 0; A_Index173++) {
            ...
            if (Trim(A_LoopField173) == "end" && SubStr(Trim(nextWordEndFixRB[A_Index173 + 1]), 1, StrLen("rescue ")) == "rescue " || Trim(A_LoopField173) == "end" && Trim(nextWordEndFixRB[A_Index173 + 1]) == "ensure") {
                nextWordEndFixOut += "";
            } else {
    ...
    if (langToConvertTo == langFileExtension_2 && useEnd_2 == "on") {
        ...
        for (size_t A_Index174 = 0; A_Index174 < items174.size() + 0; A_Index174++) {
            ...
            HTVM_Append(nextWordEndFixRB, A_LoopField174);
        }
        HTVM_Append(nextWordEndFixRB, " ");
        ...
        for (size_t A_Index175 = 0; A_Index175 < items175.size() + 0; A_Index175++) {
            ...
            if (Trim(A_LoopField175) == Trim(keyWordEnd_2) && SubStr(Trim(nextWordEndFixRB[A_Index175 + 1]), 1, StrLen(Trim(keyWordCatch_2) + " ")) == Trim(keyWordCatch_2) + " " || Trim(A_LoopField175) == Trim(keyWordEnd_2) && Trim(nextWordEndFixRB[A_Index175 + 1]) == Trim(keyWordFinally_2) || Trim(nextWordEndFixRB[A_Index175 + 1]) == Trim(keyWordFinally_2 + ":")) {
                nextWordEndFixOut += "";
            } else {
    ...
    if (langToConvertTo == "ahk") {
        ...
        for (size_t A_Index176 = 0; A_Index176 < items176.size() + 0; A_Index176++) {
            ...
            if (SubStr(Trim(A_LoopField176), 1, StrLen("Loop, Parse, ")) == "Loop, Parse, ") {
                nextWordEndFixOut += StrReplace(A_LoopField176, "{", Chr(10) + "{" + Chr(10)) + Chr(10);
            } else {
                nextWordEndFixOut += A_LoopField176 + Chr(10);
            }
        }
        htCode = StringTrimRight(nextWordEndFixOut, 1);
    }
    if (langToConvertTo == "go") {
        ...
        for (size_t A_Index177 = 0; A_Index177 < items177.size() + 0; A_Index177++) {
            ...
            HTVM_Append(fixGoManGoIsSoAnnoyingBroooFurure, A_LoopField177);
        }
        ...
        for (size_t A_Index178 = 0; A_Index178 < items178.size() + 0; A_Index178++) {
            ...
            if (Trim(A_LoopField178) == "}" && SubStr(Trim(fixGoManGoIsSoAnnoyingBroooFurure[A_Index178 + 1]), 1, StrLen("else if ")) == "else if ") {
                fixGoManGoIsSoAnnoyingBroooSkip = 1;
                fixGoManGoIsSoAnnoyingBroooCount = 0;
                std::vector<std::string> items179 = LoopParseFunc(fixGoManGoIsSoAnnoyingBroooFurure[A_Index178 + 1]);
                ...
                for (int A_Index180 = 0; A_Index180 < fixGoManGoIsSoAnnoyingBroooCount + 0; A_Index180++) {
                    fixGoManGoIsSoAnnoyingBroooCountSpaceses += " ";
                }
                fixGoManGoIsSoAnnoyingBrooo += fixGoManGoIsSoAnnoyingBroooCountSpaceses + "} " + Trim(fixGoManGoIsSoAnnoyingBroooFurure[A_Index178 + 1]) + Chr(10);
            } else {
                if (fixGoManGoIsSoAnnoyingBroooSkip == 0) {
                    fixGoManGoIsSoAnnoyingBrooo += A_LoopField178 + Chr(10);
                }
    ...
    if (COUNT_programmingBlock_InTheTranspiledLang != 0) {
        for (int A_Index181 = 0; A_Index181 < COUNT_programmingBlock_InTheTranspiledLang + 0; A_Index181++) {
            htCode = StrReplace(htCode, "programmingBlock_InTheTranspiledLang-programmingBlock_InTheTranspiledLang-AA" + STR(A_Index181 + 1) + "AA", programmingBlock_InTheTranspiledLang[A_Index181]);
        }
    }
    if (langToConvertTo == "cpp") {
        if (COUNT_programmingBlock_CPP != 0) {
            for (int A_Index182 = 0; A_Index182 < COUNT_programmingBlock_CPP + 0; A_Index182++) {
                htCode = StrReplace(htCode, "programmingBlock_CPP-programmingBlock_CPP-AA" + STR(A_Index182 + 1) + "AA", programmingBlock_CPP[A_Index182]);
            }
        }
    } else {
        if (COUNT_programmingBlock_CPP != 0) {
            for (int A_Index183 = 0; A_Index183 < COUNT_programmingBlock_CPP + 0; A_Index183++) {
                if (langToConvertTo == langFileExtension_2) {
                    htCode = StrReplace(htCode, "programmingBlock_CPP-programmingBlock_CPP-AA" + STR(A_Index183 + 1) + "AA", keyWordCodeInTheTranspiledLangStartCPP_2 + Chr(10) + programmingBlock_CPP[A_Index183] + Chr(10) + keyWordCodeInTheTranspiledLangEndCPP_2);
                } else {
                    htCode = StrReplace(htCode, "programmingBlock_CPP-programmingBlock_CPP-AA" + STR(A_Index183 + 1) + "AA", Chr(10));
                }
            }
        }
    }
    if (langToConvertTo == "py") {
        if (COUNT_programmingBlock_PY != 0) {
            for (int A_Index184 = 0; A_Index184 < COUNT_programmingBlock_PY + 0; A_Index184++) {
                htCode = StrReplace(htCode, "programmingBlock_PY-programmingBlock_PY-AA" + STR(A_Index184 + 1) + "AA", programmingBlock_PY[A_Index184]);
            }
        }
    } else {
        if (COUNT_programmingBlock_PY != 0) {
            for (int A_Index185 = 0; A_Index185 < COUNT_programmingBlock_PY + 0; A_Index185++) {
                if (langToConvertTo == langFileExtension_2) {
                    htCode = StrReplace(htCode, "programmingBlock_PY-programmingBlock_PY-AA" + STR(A_Index185 + 1) + "AA", keyWordCodeInTheTranspiledLangStartPY_2 + Chr(10) + programmingBlock_PY[A_Index185] + Chr(10) + keyWordCodeInTheTranspiledLangEndPY_2);
                } else {
                    htCode = StrReplace(htCode, "programmingBlock_PY-programmingBlock_PY-AA" + STR(A_Index185 + 1) + "AA", Chr(10));
                }
            }
        }
    }
    if (langToConvertTo == "js") {
        if (COUNT_programmingBlock_JS != 0) {
            for (int A_Index186 = 0; A_Index186 < COUNT_programmingBlock_JS + 0; A_Index186++) {
                htCode = StrReplace(htCode, "programmingBlock_JS-programmingBlock_JS-AA" + STR(A_Index186 + 1) + "AA", programmingBlock_JS[A_Index186]);
            }
        }
    } else {
        if (COUNT_programmingBlock_JS != 0) {
            for (int A_Index187 = 0; A_Index187 < COUNT_programmingBlock_JS + 0; A_Index187++) {
                if (langToConvertTo == langFileExtension_2) {
                    htCode = StrReplace(htCode, "programmingBlock_JS-programmingBlock_JS-AA" + STR(A_Index187 + 1) + "AA", keyWordCodeInTheTranspiledLangStartJS_2 + Chr(10) + programmingBlock_JS[A_Index187] + Chr(10) + keyWordCodeInTheTranspiledLangEndJS_2);
                } else {
                    htCode = StrReplace(htCode, "programmingBlock_JS-programmingBlock_JS-AA" + STR(A_Index187 + 1) + "AA", Chr(10));
                }
            }
        }
    }
    if (langToConvertTo == "go") {
        if (COUNT_programmingBlock_GO != 0) {
            for (int A_Index188 = 0; A_Index188 < COUNT_programmingBlock_GO + 0; A_Index188++) {
                htCode = StrReplace(htCode, "programmingBlock_GO-programmingBlock_GO-AA" + STR(A_Index188 + 1) + "AA", programmingBlock_GO[A_Index188]);
            }
        }
    } else {
        if (COUNT_programmingBlock_GO != 0) {
            for (int A_Index189 = 0; A_Index189 < COUNT_programmingBlock_GO + 0; A_Index189++) {
                if (langToConvertTo == langFileExtension_2) {
                    htCode = StrReplace(htCode, "programmingBlock_GO-programmingBlock_GO-AA" + STR(A_Index189 + 1) + "AA", keyWordCodeInTheTranspiledLangStartGO_2 + Chr(10) + programmingBlock_GO[A_Index189] + Chr(10) + keyWordCodeInTheTranspiledLangEndGO_2);
                } else {
                    htCode = StrReplace(htCode, "programmingBlock_GO-programmingBlock_GO-AA" + STR(A_Index189 + 1) + "AA", Chr(10));
                }
            }
        }
    }
    if (langToConvertTo == "lua") {
        if (COUNT_programmingBlock_LUA != 0) {
            for (int A_Index190 = 0; A_Index190 < COUNT_programmingBlock_LUA + 0; A_Index190++) {
                htCode = StrReplace(htCode, "programmingBlock_LUA-programmingBlock_LUA-AA" + STR(A_Index190 + 1) + "AA", programmingBlock_LUA[A_Index190]);
            }
        }
    } else {
        if (COUNT_programmingBlock_LUA != 0) {
            for (int A_Index191 = 0; A_Index191 < COUNT_programmingBlock_LUA + 0; A_Index191++) {
                if (langToConvertTo == langFileExtension_2) {
                    htCode = StrReplace(htCode, "programmingBlock_LUA-programmingBlock_LUA-AA" + STR(A_Index191 + 1) + "AA", keyWordCodeInTheTranspiledLangStartLUA_2 + Chr(10) + programmingBlock_LUA[A_Index191] + Chr(10) + keyWordCodeInTheTranspiledLangEndLUA_2);
                } else {
                    htCode = StrReplace(htCode, "programmingBlock_LUA-programmingBlock_LUA-AA" + STR(A_Index191 + 1) + "AA", Chr(10));
                }
            }
        }
    }
    if (langToConvertTo == "cs") {
        if (COUNT_programmingBlock_CS != 0) {
            for (int A_Index192 = 0; A_Index192 < COUNT_programmingBlock_CS + 0; A_Index192++) {
                htCode = StrReplace(htCode, "programmingBlock_CS-programmingBlock_CS-AA" + STR(A_Index192 + 1) + "AA", programmingBlock_CS[A_Index192]);
            }
        }
    } else {
        if (COUNT_programmingBlock_CS != 0) {
            for (int A_Index193 = 0; A_Index193 < COUNT_programmingBlock_CS + 0; A_Index193++) {
                if (langToConvertTo == langFileExtension_2) {
                    htCode = StrReplace(htCode, "programmingBlock_CS-programmingBlock_CS-AA" + STR(A_Index193 + 1) + "AA", keyWordCodeInTheTranspiledLangStartCS_2 + Chr(10) + programmingBlock_CS[A_Index193] + Chr(10) + keyWordCodeInTheTranspiledLangEndCS_2);
                } else {
                    htCode = StrReplace(htCode, "programmingBlock_CS-programmingBlock_CS-AA" + STR(A_Index193 + 1) + "AA", Chr(10));
                }
            }
        }
    }
    if (langToConvertTo == "java") {
        if (COUNT_programmingBlock_JAVA != 0) {
            for (int A_Index194 = 0; A_Index194 < COUNT_programmingBlock_JAVA + 0; A_Index194++) {
                htCode = StrReplace(htCode, "programmingBlock_JAVA-programmingBlock_JAVA-AA" + STR(A_Index194 + 1) + "AA", programmingBlock_JAVA[A_Index194]);
            }
        }
    } else {
        if (COUNT_programmingBlock_JAVA != 0) {
            for (int A_Index195 = 0; A_Index195 < COUNT_programmingBlock_JAVA + 0; A_Index195++) {
                if (langToConvertTo == langFileExtension_2) {
                    htCode = StrReplace(htCode, "programmingBlock_JAVA-programmingBlock_JAVA-AA" + STR(A_Index195 + 1) + "AA", keyWordCodeInTheTranspiledLangStartJAVA_2 + Chr(10) + programmingBlock_JAVA[A_Index195] + Chr(10) + keyWordCodeInTheTranspiledLangEndJAVA_2);
                } else {
                    htCode = StrReplace(htCode, "programmingBlock_JAVA-programmingBlock_JAVA-AA" + STR(A_Index195 + 1) + "AA", Chr(10));
                }
            }
        }
    }
    if (langToConvertTo == "kt") {
        if (COUNT_programmingBlock_KT != 0) {
            for (int A_Index196 = 0; A_Index196 < COUNT_programmingBlock_KT + 0; A_Index196++) {
                htCode = StrReplace(htCode, "programmingBlock_KT-programmingBlock_KT-AA" + STR(A_Index196 + 1) + "AA", programmingBlock_KT[A_Index196]);
            }
        }
    } else {
        if (COUNT_programmingBlock_KT != 0) {
            for (int A_Index197 = 0; A_Index197 < COUNT_programmingBlock_KT + 0; A_Index197++) {
                if (langToConvertTo == langFileExtension_2) {
                    htCode = StrReplace(htCode, "programmingBlock_KT-programmingBlock_KT-AA" + STR(A_Index197 + 1) + "AA", keyWordCodeInTheTranspiledLangStartKT_2 + Chr(10) + programmingBlock_KT[A_Index197] + Chr(10) + keyWordCodeInTheTranspiledLangEndKT_2);
                } else {
                    htCode = StrReplace(htCode, "programmingBlock_KT-programmingBlock_KT-AA" + STR(A_Index197 + 1) + "AA", Chr(10));
                }
            }
        }
    }
    if (langToConvertTo == "rb") {
        if (COUNT_programmingBlock_RB != 0) {
            for (int A_Index198 = 0; A_Index198 < COUNT_programmingBlock_RB + 0; A_Index198++) {
                htCode = StrReplace(htCode, "programmingBlock_RB-programmingBlock_RB-AA" + STR(A_Index198 + 1) + "AA", programmingBlock_RB[A_Index198]);
            }
        }
    } else {
        if (COUNT_programmingBlock_RB != 0) {
            for (int A_Index199 = 0; A_Index199 < COUNT_programmingBlock_RB + 0; A_Index199++) {
                if (langToConvertTo == langFileExtension_2) {
                    htCode = StrReplace(htCode, "programmingBlock_RB-programmingBlock_RB-AA" + STR(A_Index199 + 1) + "AA", keyWordCodeInTheTranspiledLangStartRB_2 + Chr(10) + programmingBlock_RB[A_Index199] + Chr(10) + keyWordCodeInTheTranspiledLangEndRB_2);
                } else {
                    htCode = StrReplace(htCode, "programmingBlock_RB-programmingBlock_RB-AA" + STR(A_Index199 + 1) + "AA", Chr(10));
                }
            }
        }
    }
    if (langToConvertTo == "nim") {
        if (COUNT_programmingBlock_NIM != 0) {
            for (int A_Index200 = 0; A_Index200 < COUNT_programmingBlock_NIM + 0; A_Index200++) {
                htCode = StrReplace(htCode, "programmingBlock_NIM-programmingBlock_NIM-AA" + STR(A_Index200 + 1) + "AA", programmingBlock_NIM[A_Index200]);
            }
        }
    } else {
        if (COUNT_programmingBlock_NIM != 0) {
            for (int A_Index201 = 0; A_Index201 < COUNT_programmingBlock_NIM + 0; A_Index201++) {
                if (langToConvertTo == langFileExtension_2) {
                    htCode = StrReplace(htCode, "programmingBlock_NIM-programmingBlock_NIM-AA" + STR(A_Index201 + 1) + "AA", keyWordCodeInTheTranspiledLangStartNIM_2 + Chr(10) + programmingBlock_NIM[A_Index201] + Chr(10) + keyWordCodeInTheTranspiledLangEndNIM_2);
                } else {
                    htCode = StrReplace(htCode, "programmingBlock_NIM-programmingBlock_NIM-AA" + STR(A_Index201 + 1) + "AA", Chr(10));
                }
            }
        }
    }
    if (langToConvertTo == "ahk") {
        if (COUNT_programmingBlock_AHK != 0) {
            for (int A_Index202 = 0; A_Index202 < COUNT_programmingBlock_AHK + 0; A_Index202++) {
                htCode = StrReplace(htCode, "programmingBlock_AHK-programmingBlock_AHK-AA" + STR(A_Index202 + 1) + "AA", programmingBlock_AHK[A_Index202]);
            }
        }
    } else {
        if (COUNT_programmingBlock_AHK != 0) {
            for (int A_Index203 = 0; A_Index203 < COUNT_programmingBlock_AHK + 0; A_Index203++) {
                if (langToConvertTo == langFileExtension_2) {
                    htCode = StrReplace(htCode, "programmingBlock_AHK-programmingBlock_AHK-AA" + STR(A_Index203 + 1) + "AA", keyWordCodeInTheTranspiledLangStartAHK_2 + Chr(10) + programmingBlock_AHK[A_Index203] + Chr(10) + keyWordCodeInTheTranspiledLangEndAHK_2);
                } else {
                    htCode = StrReplace(htCode, "programmingBlock_AHK-programmingBlock_AHK-AA" + STR(A_Index203 + 1) + "AA", Chr(10));
                }
            }
        }
    }
    if (langToConvertTo == "swift") {
        if (COUNT_programmingBlock_SWIFT != 0) {
            for (int A_Index204 = 0; A_Index204 < COUNT_programmingBlock_SWIFT + 0; A_Index204++) {
                htCode = StrReplace(htCode, "programmingBlock_SWIFT-programmingBlock_SWIFT-AA" + STR(A_Index204 + 1) + "AA", programmingBlock_SWIFT[A_Index204]);
            }
        }
    } else {
        if (COUNT_programmingBlock_SWIFT != 0) {
            for (int A_Index205 = 0; A_Index205 < COUNT_programmingBlock_SWIFT + 0; A_Index205++) {
                if (langToConvertTo == langFileExtension_2) {
                    htCode = StrReplace(htCode, "programmingBlock_SWIFT-programmingBlock_SWIFT-AA" + STR(A_Index205 + 1) + "AA", keyWordCodeInTheTranspiledLangStartSWIFT_2 + Chr(10) + programmingBlock_SWIFT[A_Index205] + Chr(10) + keyWordCodeInTheTranspiledLangEndSWIFT_2);
                } else {
                    htCode = StrReplace(htCode, "programmingBlock_SWIFT-programmingBlock_SWIFT-AA" + STR(A_Index205 + 1) + "AA", Chr(10));
                }
            }
        }
    }
    if (langToConvertTo == "dart") {
        if (COUNT_programmingBlock_DART != 0) {
            for (int A_Index206 = 0; A_Index206 < COUNT_programmingBlock_DART + 0; A_Index206++) {
                htCode = StrReplace(htCode, "programmingBlock_DART-programmingBlock_DART-AA" + STR(A_Index206 + 1) + "AA", programmingBlock_DART[A_Index206]);
            }
        }
    } else {
        if (COUNT_programmingBlock_DART != 0) {
            for (int A_Index207 = 0; A_Index207 < COUNT_programmingBlock_DART + 0; A_Index207++) {
                if (langToConvertTo == langFileExtension_2) {
                    htCode = StrReplace(htCode, "programmingBlock_DART-programmingBlock_DART-AA" + STR(A_Index207 + 1) + "AA", keyWordCodeInTheTranspiledLangStartDART_2 + Chr(10) + programmingBlock_DART[A_Index207] + Chr(10) + keyWordCodeInTheTranspiledLangEndDART_2);
                } else {
                    htCode = StrReplace(htCode, "programmingBlock_DART-programmingBlock_DART-AA" + STR(A_Index207 + 1) + "AA", Chr(10));
                }
            }
        }
    }
    if (langToConvertTo == "ts") {
        if (COUNT_programmingBlock_TS != 0) {
            for (int A_Index208 = 0; A_Index208 < COUNT_programmingBlock_TS + 0; A_Index208++) {
                htCode = StrReplace(htCode, "programmingBlock_TS-programmingBlock_TS-AA" + STR(A_Index208 + 1) + "AA", programmingBlock_TS[A_Index208]);
            }
        }
    } else {
        if (COUNT_programmingBlock_TS != 0) {
            for (int A_Index209 = 0; A_Index209 < COUNT_programmingBlock_TS + 0; A_Index209++) {
                if (langToConvertTo == langFileExtension_2) {
                    htCode = StrReplace(htCode, "programmingBlock_TS-programmingBlock_TS-AA" + STR(A_Index209 + 1) + "AA", keyWordCodeInTheTranspiledLangStartTS_2 + Chr(10) + programmingBlock_TS[A_Index209] + Chr(10) + keyWordCodeInTheTranspiledLangEndTS_2);
                } else {
                    htCode = StrReplace(htCode, "programmingBlock_TS-programmingBlock_TS-AA" + STR(A_Index209 + 1) + "AA", Chr(10));
                }
            }
        }
    }
    if (langToConvertTo == "groovy") {
        if (COUNT_programmingBlock_GROOVY != 0) {
            for (int A_Index210 = 0; A_Index210 < COUNT_programmingBlock_GROOVY + 0; A_Index210++) {
                htCode = StrReplace(htCode, "programmingBlock_GROOVY-programmingBlock_GROOVY-AA" + STR(A_Index210 + 1) + "AA", programmingBlock_GROOVY[A_Index210]);
            }
        }
    } else {
        if (COUNT_programmingBlock_GROOVY != 0) {
            for (int A_Index211 = 0; A_Index211 < COUNT_programmingBlock_GROOVY + 0; A_Index211++) {
                if (langToConvertTo == langFileExtension_2) {
                    htCode = StrReplace(htCode, "programmingBlock_GROOVY-programmingBlock_GROOVY-AA" + STR(A_Index211 + 1) + "AA", keyWordCodeInTheTranspiledLangStartGROOVY_2 + Chr(10) + programmingBlock_GROOVY[A_Index211] + Chr(10) + keyWordCodeInTheTranspiledLangEndGROOVY_2);
                } else {
                    htCode = StrReplace(htCode, "programmingBlock_GROOVY-programmingBlock_GROOVY-AA" + STR(A_Index211 + 1) + "AA", Chr(10));
                }
            }
        }
    }
    ...
    if (langToConvertTo == langFileExtension_2) {
        if (COUNT_programmingBlock_HTVM != 0) {
            for (int A_Index212 = 0; A_Index212 < COUNT_programmingBlock_HTVM + 0; A_Index212++) {
                htCode = StrReplace(htCode, "programmingBlock_HTVM-programmingBlock_HTVM-AA" + STR(A_Index212 + 1) + "AA", programmingBlock_HTVM[A_Index212]);
            }
        }
    } else {
        if (COUNT_programmingBlock_HTVM != 0) {
            for (int A_Index213 = 0; A_Index213 < COUNT_programmingBlock_HTVM + 0; A_Index213++) {
                htCode = StrReplace(htCode, "programmingBlock_HTVM-programmingBlock_HTVM-AA" + STR(A_Index213 + 1) + "AA", Chr(10));
            }
        }
    }
    ...
    for (size_t A_Index214 = 0; A_Index214 < items214.size() + 0; A_Index214++) {
        ...
        if (areWeInAFuncFromInstructions == 1) {
            if (areWeInAFuncFromInstructionsLineNum == 1) {
                // name of the func
                funcLangHolder = StringTrimLeft(A_LoopField214, 5);
            ...
            if (areWeInAFuncFromInstructionsLineNum == 2) {
                // name of the func
                funcNameHolder = StringTrimLeft(A_LoopField214, 5);
                if (correctLang == 1 && InStr(htCode, Trim(funcNameHolder))) {
            ...
            if (areWeInAFuncFromInstructionsLineNum == 3) {
                // all libs
                funcLibsHolder = StringTrimLeft(A_LoopField214, 5);
                if (correctLang == 1 && InStr(htCode, Trim(funcNameHolder))) {
            ...
            if (areWeInAFuncFromInstructionsLineNum == 4) {
                // func description
                funcDescriptionHolder = StringTrimLeft(A_LoopField214, 12);
                if (correctLang == 1 && InStr(htCode, Trim(funcNameHolder))) {
            ...
            if (areWeInAFuncFromInstructionsLineNum >= 5) {
                // the full func
                if (correctLang == 1 && InStr(htCode, Trim(funcNameHolder))) {
                    funcFuncHolder += A_LoopField214 + Chr(10);
                }
            }
            //MsgBox, % A_LoopField214
            areWeInAFuncFromInstructionsLineNum++;
        }
        if (Trim(A_LoopField214) == "func======================func==============") {
    ...
    if (!(HTVM_Size(allFuncNames) <= 0)) {
        for (int A_Index215 = 0; A_Index215 < HTVM_Size(allFuncNames) + 0; A_Index215++) {
            if (InStr(htCode, allFuncNames[A_Index215]) + "(") {
                //MsgBox, % allFuncNames[A_Index215]
                allFuncsToPutAtTop += allFuncs[A_Index215] + Chr(10);
                if (Trim(allFuncLibs[A_Index215]) != "null") {
                    allLibsToPutAtTop += allFuncLibs[A_Index215] + "|";
                }
            }
        }
        allLibsToPutAtTop = StringTrimRight(allLibsToPutAtTop, 1);
        std::string allLibsToPutAtTopTEMP = "";
        std::vector<std::string> items216 = LoopParseFunc(allLibsToPutAtTop, "|");
        for (size_t A_Index216 = 0; A_Index216 < items216.size() + 0; A_Index216++) {
            std::string A_LoopField216 = items216[A_Index216 - 0];
            allLibsToPutAtTopTEMP += A_LoopField216 + Chr(10);
        }
    ...
    if (langToConvertTo != langFileExtension_2) {
        for (int A_Index217 = 0; A_Index217 < HTVM_Size(ospDic1) + 0; A_Index217++) {
            str00 = ospDic1[A_Index217];
            if (langToConvertTo == "rb") {
    ...
    if (size_ospDic != 0) {
        for (int A_Index218 = 0; A_Index218 < size_ospDic + 0; A_Index218++) {
            HTVM_Pop(ospDic);
        }
    }
    //;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;
    ...
    if (size_ospDic1 != 0) {
        for (int A_Index219 = 0; A_Index219 < size_ospDic1 + 0; A_Index219++) {
            HTVM_Pop(ospDic1);
        }
    }
    //;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;
    ...
    if (size_ospDic2 != 0) {
        for (int A_Index220 = 0; A_Index220 < size_ospDic2 + 0; A_Index220++) {
            HTVM_Pop(ospDic2);
        }
    }
    if (langToConvertTo == "rb") {
    ...
    for (int A_Index221 = 0; A_Index221 < theIdNumOfThe34 + 0; A_Index221++) {
        if (theIdNumOfThe34 == A_Index221 + 1) {
            if (langToConvertTo == langFileExtension_2) {
                if (keyWordEscpaeChar_2 == "\\" && keyWordEscpaeChar != "\\") {
                    htCode = StrReplace(htCode, "ihuiuuhuuhtheidFor--asds" + str21 + "as--" + str21 + "theuhturtyphoutr--" + Chr(65) + Chr(65) + STR(A_Index221 + 1) + Chr(65) + Chr(65), StrReplace(StrReplace(theIdNumOfThe34theVar[A_Index221 + 1], "\\", "\\\\"), keyWordEscpaeChar, keyWordEscpaeChar_2) + Chr(34));
                } else {
                    htCode = StrReplace(htCode, "ihuiuuhuuhtheidFor--asds" + str21 + "as--" + str21 + "theuhturtyphoutr--" + Chr(65) + Chr(65) + STR(A_Index221 + 1) + Chr(65) + Chr(65), StrReplace(theIdNumOfThe34theVar[A_Index221 + 1], keyWordEscpaeChar, keyWordEscpaeChar_2) + Chr(34));
                }
            } else {
                htCode = StrReplace(htCode, "ihuiuuhuuhtheidFor--asds" + str21 + "as--" + str21 + "theuhturtyphoutr--" + Chr(65) + Chr(65) + STR(A_Index221 + 1) + Chr(65) + Chr(65), StrReplace(theIdNumOfThe34theVar[A_Index221 + 1], keyWordEscpaeChar, "\\") + Chr(34));
            }
        } else {
            if (langToConvertTo == langFileExtension_2) {
                if (keyWordEscpaeChar_2 == "\\" && keyWordEscpaeChar != "\\") {
                    htCode = StrReplace(htCode, "ihuiuuhuuhtheidFor--asds" + str21 + "as--" + str21 + "theuhturtyphoutr--" + Chr(65) + Chr(65) + STR(A_Index221 + 1) + Chr(65) + Chr(65), StrReplace(StrReplace(theIdNumOfThe34theVar[A_Index221 + 1], "\\", "\\\\"), keyWordEscpaeChar, keyWordEscpaeChar_2));
                } else {
                    htCode = StrReplace(htCode, "ihuiuuhuuhtheidFor--asds" + str21 + "as--" + str21 + "theuhturtyphoutr--" + Chr(65) + Chr(65) + STR(A_Index221 + 1) + Chr(65) + Chr(65), StrReplace(theIdNumOfThe34theVar[A_Index221 + 1], keyWordEscpaeChar, keyWordEscpaeChar_2));
                }
            } else {
                htCode = StrReplace(htCode, "ihuiuuhuuhtheidFor--asds" + str21 + "as--" + str21 + "theuhturtyphoutr--" + Chr(65) + Chr(65) + STR(A_Index221 + 1) + Chr(65) + Chr(65), StrReplace(theIdNumOfThe34theVar[A_Index221 + 1], keyWordEscpaeChar, "\\"));
            }
        }
    }
    if (langToConvertTo == langFileExtension_2) {
...
void HTVMv2() {
    ...
    if (HTVM_getLang_HTVM() == "cpp" || HTVM_getLang_HTVM() == "py") {
        ...
        std::vector<std::string> items223 = LoopParseFunc(allArgs, "\n", "\r");
        for (size_t A_Index223 = 0; A_Index223 < items223.size() + 0; A_Index223++) {
            std::string A_LoopField223 = items223[A_Index223 - 0];
            if (A_Index223 == 0) {
                numOfParams++;
                argCODE = FileRead(Trim(A_LoopField223));
                argCODEfile = Trim(A_LoopField223);
            }
            else if (A_Index223 == 1) {
                numOfParams++;
                argHTVMinstr = Trim(A_LoopField223);
            }
            else if (A_Index223 == 2) {
                numOfParams++;
                argLangTo = Trim(A_LoopField223);
            } else {
                numOfParams++;
                HTVM_Append(argHTVMinstrMORE, Trim(A_LoopField223));
            }
        }
        //print("===============123431234===========start=====")
        for (int A_Index224 = 0; A_Index224 < HTVM_Size(argHTVMinstrMORE) + 0; A_Index224++) {
            //print(argHTVMinstrMORE[A_Index224])
        }
        //print("===============123431234==========end======")
