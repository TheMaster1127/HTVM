...
std::string fString(std::string line) {
    ...
    if (langToConvertTo == "rb") {
        for (int A_Index157 = 0; A_Index157 < HTVM_Size(htCodeFixRubyGlobalVars_EXPOSED) + 0; A_Index157++) {
            line = RegExReplace(line, "\\b" + htCodeFixRubyGlobalVars_EXPOSED[A_Index157] + "\\b", "$" + htCodeFixRubyGlobalVars_EXPOSED[A_Index157]);
        }
    }
    return line;
}
...
std::string handleMultiLineStrings(std::string code) {
    ...
    for (size_t A_Index160 = 0; A_Index160 < items160.size() + 0; A_Index160++) {
        ...
        if (langToConvertTo == langFileExtension_2) {
            //;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;
            //;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;
            if (weAreIn == 1 && Trim(A_LoopField160) == "mle") {
            ...
            if (weAreIn == 1 && Trim(A_LoopField160) == "HTVM-newLineAHw4y9w730q8237w4e8srhgesw0s8rgw04e5g08sgr0e8srdgt--YOU-BETERNOTUSETHISAH--aesdfssz") {
                if (lookAhead[A_Index160 + 1] == "HTVM-newLineAHw4y9w730q8237w4e8srhgesw0s8rgw04e5g08sgr0e8srdgt--YOU-BETERNOTUSETHISAH--aesdfssz") {
                    out += keyWordEscpaeChar + "n";
                }
            }
            if (weAreIn == 1 && Trim(A_LoopField160) != "HTVM-newLineAHw4y9w730q8237w4e8srhgesw0s8rgw04e5g08sgr0e8srdgt--YOU-BETERNOTUSETHISAH--aesdfssz") {
                out += A_LoopField160 + keyWordEscpaeChar + "n";
            }
            if (SubStrLastChars(Trim(A_LoopField160), 4) == " mls" || SubStrLastChars(Trim(A_LoopField160), 5) == " fmls") {
                weAreIn = 1;
                if (SubStrLastChars(Trim(A_LoopField160), 5) == " fmls") {
                    out += TrimR(StringTrimRight(TrimR(A_LoopField160), 3)) + Chr(34);
                } else {
                    out += TrimR(StringTrimRight(TrimR(A_LoopField160), 3)) + " " + Chr(34);
                }
            }
            if (weAreIn == 0) {
                if (Trim(A_LoopField160) != "mle") {
                    out += A_LoopField160 + Chr(10);
                }
            }
            //;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;
...
std::vector<std::string> slidingWinFixCommentsRmFUNC(std::string line) {
    ...
    for (size_t A_Index161 = 0; A_Index161 < items161.size() + 0; A_Index161++) {
        ...
        HTVM_Append(slidingWinFixCommentsRmFFFF, A_LoopField161);
    }
    for (int A_Index162 = 0; A_Index162 < StrLen(keyWordComment + keyWordComment) + 2 + 0; A_Index162++) {
...
std::string fixIfElseIfMultiLineStatementHELP(std::string aloopfield) {
    std::string out = "";
    int count = 0;
    std::vector<std::string> items163 = LoopParseFunc(aloopfield);
...
std::string fixIfElseIfMultiLineStatement(std::string someHTVMcode) {
    int doWeExitInsta = 1;
    std::vector<std::string> items164 = LoopParseFunc(someHTVMcode, "\n", "\r");
    ...
    for (int A_Index165 = 0; A_Index165 < 10000 + 0; A_Index165++) {
        out2 = out;
        out = "";
        doWeExit = false;
        std::vector<std::string> items166 = LoopParseFunc(out2, "\n", "\r");
        for (size_t A_Index166 = 0; A_Index166 < items166.size() + 0; A_Index166++) {
            ...
            if (SubStr(StrLower(Trim(A_LoopField166)), 1, StrLen(keyWordIF)) == keyWordIF) {
                doWeJustExit = 1;
                if (fixIfElseIfMultiLineStatementHELP(A_LoopField166) == "") {
                    out += Trim(A_LoopField166) + " ";
                    doWeExit = false;
                    doWeExitHELP--;
                } else {
                    doWeExit = true;
                    out += Trim(A_LoopField166) + Chr(10);
                }
            }
...
bool doseHaveInclude(std::string TheCodeThatMightHaveInclude) {
    std::vector<std::string> items167 = LoopParseFunc(TheCodeThatMightHaveInclude, "\n", "\r");
    for (size_t A_Index167 = 0; A_Index167 < items167.size() + 0; A_Index167++) {
        std::string A_LoopField167 = items167[A_Index167 - 0];
        if (SubStr(StrLower(Trim(A_LoopField167)), 1, StrLen(StrLower(keyWordInclude + " "))) == StrLower(keyWordInclude + " ")) {
            return true;
        }
    }
    return false;
...
std::string compiler(std::string htCode, std::string allInstructionFile, std::string mode, std::string langToConvertToParam = "") {
    ...
    for (size_t A_Index168 = 0; A_Index168 < items168.size() + 0; A_Index168++) {
        ...
        if (A_Index168 == 0) {
            langToConvertTo = Trim(A_LoopField168);
        }
        if (A_Index168 == 1) {
            langFileExtension = Trim(A_LoopField168);
        }
        if (A_Index168 == 2) {
            commands = Trim(A_LoopField168);
        }
        if (A_Index168 == 3) {
            keyWordAlliance = Trim(A_LoopField168) + " ";
        }
        if (A_Index168 == 4) {
            keyWordCrew = Trim(A_LoopField168) + " ";
        }
        if (A_Index168 == 5) {
            keyWordProc = Trim(A_LoopField168) + " ";
        }
        if (A_Index168 == 6) {
            keyWordStruct = Trim(A_LoopField168) + " ";
        }
        if (A_Index168 == 7) {
            keyWordProp = Trim(A_LoopField168) + " ";
        }
        if (A_Index168 == 8) {
            keyWordThis = Trim(A_LoopField168);
        }
        if (A_Index168 == 9) {
            keyWordInclude = Trim(A_LoopField168);
        }
        if (A_Index168 == 10) {
            keyWordCodeInTheTranspiledLangStart = Trim(A_LoopField168);
        }
        if (A_Index168 == 11) {
            keyWordCodeInTheTranspiledLangEnd = Trim(A_LoopField168);
        }
        if (A_Index168 == 12) {
            keyWordCodeInTheTranspiledLangStartCPP = Trim(A_LoopField168);
        }
        if (A_Index168 == 13) {
            keyWordCodeInTheTranspiledLangEndCPP = Trim(A_LoopField168);
        }
        if (A_Index168 == 14) {
            keyWordCodeInTheTranspiledLangStartPY = Trim(A_LoopField168);
        }
        if (A_Index168 == 15) {
            keyWordCodeInTheTranspiledLangEndPY = Trim(A_LoopField168);
        }
        if (A_Index168 == 16) {
            keyWordCodeInTheTranspiledLangStartJS = Trim(A_LoopField168);
        }
        if (A_Index168 == 17) {
            keyWordCodeInTheTranspiledLangEndJS = Trim(A_LoopField168);
        }
        if (A_Index168 == 18) {
            keyWordCodeInTheTranspiledLangStartGO = Trim(A_LoopField168);
        }
        if (A_Index168 == 19) {
            keyWordCodeInTheTranspiledLangEndGO = Trim(A_LoopField168);
        }
        if (A_Index168 == 20) {
            keyWordCodeInTheTranspiledLangStartLUA = Trim(A_LoopField168);
        }
        if (A_Index168 == 21) {
            keyWordCodeInTheTranspiledLangEndLUA = Trim(A_LoopField168);
        }
        if (A_Index168 == 22) {
            keyWordCodeInTheTranspiledLangStartCS = Trim(A_LoopField168);
        }
        if (A_Index168 == 23) {
            keyWordCodeInTheTranspiledLangEndCS = Trim(A_LoopField168);
        }
        if (A_Index168 == 24) {
            keyWordCodeInTheTranspiledLangStartJAVA = Trim(A_LoopField168);
        }
        if (A_Index168 == 25) {
            keyWordCodeInTheTranspiledLangEndJAVA = Trim(A_LoopField168);
        }
        if (A_Index168 == 26) {
            keyWordCodeInTheTranspiledLangStartKT = Trim(A_LoopField168);
        }
        if (A_Index168 == 27) {
            keyWordCodeInTheTranspiledLangEndKT = Trim(A_LoopField168);
        }
        if (A_Index168 == 28) {
            keyWordCodeInTheTranspiledLangStartRB = Trim(A_LoopField168);
        }
        if (A_Index168 == 29) {
            keyWordCodeInTheTranspiledLangEndRB = Trim(A_LoopField168);
        }
        if (A_Index168 == 30) {
            keyWordCodeInTheTranspiledLangStartNIM = Trim(A_LoopField168);
        }
        if (A_Index168 == 31) {
            keyWordCodeInTheTranspiledLangEndNIM = Trim(A_LoopField168);
        }
        if (A_Index168 == 32) {
            keyWordCodeInTheTranspiledLangStartAHK = Trim(A_LoopField168);
        }
        if (A_Index168 == 33) {
            keyWordCodeInTheTranspiledLangEndAHK = Trim(A_LoopField168);
        }
        if (A_Index168 == 34) {
            keyWordCodeInTheTranspiledLangStartSWIFT = Trim(A_LoopField168);
        }
        if (A_Index168 == 35) {
            keyWordCodeInTheTranspiledLangEndSWIFT = Trim(A_LoopField168);
        }
        if (A_Index168 == 36) {
            keyWordCodeInTheTranspiledLangStartDART = Trim(A_LoopField168);
        }
        if (A_Index168 == 37) {
            keyWordCodeInTheTranspiledLangEndDART = Trim(A_LoopField168);
        }
        if (A_Index168 == 38) {
            keyWordCodeInTheTranspiledLangStartTS = Trim(A_LoopField168);
        }
        if (A_Index168 == 39) {
            keyWordCodeInTheTranspiledLangEndTS = Trim(A_LoopField168);
        }
        if (A_Index168 == 40) {
            keyWordCodeInTheTranspiledLangStartGROOVY = Trim(A_LoopField168);
        }
        if (A_Index168 == 41) {
            keyWordCodeInTheTranspiledLangEndGROOVY = Trim(A_LoopField168);
        }
        if (A_Index168 == 42) {
            keyWordCodeInTheTranspiledLangStartHTVM = Trim(A_LoopField168);
        }
        if (A_Index168 == 43) {
            keyWordCodeInTheTranspiledLangEndHTVM = Trim(A_LoopField168);
        }
        if (A_Index168 == 44) {
            keyWordCodeInHTVMstart = Trim(A_LoopField168);
        }
        if (A_Index168 == 45) {
            keyWordCodeInHTVMend = Trim(A_LoopField168);
        }
        if (A_Index168 == 46) {
            keyWordCurlyBraceOpen = Trim(A_LoopField168);
        }
        if (A_Index168 == 47) {
            keyWordCurlyBraceClose = Trim(A_LoopField168);
        }
        if (A_Index168 == 48) {
            keyWordNull = Trim(A_LoopField168);
        }
        if (A_Index168 == 49) {
            keyWordTrue = Trim(A_LoopField168);
        }
        if (A_Index168 == 50) {
            keyWordFalse = Trim(A_LoopField168);
        }
        if (A_Index168 == 51) {
            keyWordVoid = Trim(A_LoopField168);
        }
        if (A_Index168 == 52) {
            keyWordDouble = Trim(A_LoopField168);
        }
        if (A_Index168 == 53) {
            keyWordChar = Trim(A_LoopField168);
        }
        if (A_Index168 == 54) {
            keyWordUint8 = Trim(A_LoopField168);
        }
        if (A_Index168 == 55) {
            keyWordUint16 = Trim(A_LoopField168);
        }
        if (A_Index168 == 56) {
            keyWordUint32 = Trim(A_LoopField168);
        }
        if (A_Index168 == 57) {
            keyWordUint64 = Trim(A_LoopField168);
        }
        if (A_Index168 == 58) {
            keyWordINT = Trim(A_LoopField168);
        }
        if (A_Index168 == 59) {
            keyWordSTR = Trim(A_LoopField168);
        }
        if (A_Index168 == 60) {
            keyWordBOOL = Trim(A_LoopField168);
        }
        if (A_Index168 == 61) {
            keyWordFLOAT = Trim(A_LoopField168);
        }
        if (A_Index168 == 62) {
            keyWordINT8 = Trim(A_LoopField168);
        }
        if (A_Index168 == 63) {
            keyWordINT16 = Trim(A_LoopField168);
        }
        if (A_Index168 == 64) {
            keyWordINT32 = Trim(A_LoopField168);
        }
        if (A_Index168 == 65) {
            keyWordINT64 = Trim(A_LoopField168);
        }
        if (A_Index168 == 66) {
            keyWordIF = Trim(A_LoopField168) + " ";
        }
        if (A_Index168 == 67) {
            keyWordElseIf = Trim(A_LoopField168) + " ";
        }
        if (A_Index168 == 68) {
            keyWordElse = Trim(A_LoopField168);
        }
        if (A_Index168 == 69) {
            keyWordWhileLoop = Trim(A_LoopField168) + " ";
        }
        if (A_Index168 == 70) {
            keyWordLoopInfinite = Trim(A_LoopField168);
        }
        if (A_Index168 == 71) {
            keyWordLoop = Trim(A_LoopField168) + " ";
        }
        if (A_Index168 == 72) {
            keyWordLoopParse = Trim(A_LoopField168) + " ";
        }
        if (A_Index168 == 73) {
            keyWordContinue = Trim(A_LoopField168);
        }
        if (A_Index168 == 74) {
            keyWordBreak = Trim(A_LoopField168);
        }
        if (A_Index168 == 75) {
            keyWordFunc = Trim(A_LoopField168) + " ";
        }
        if (A_Index168 == 76) {
            keyWordAwait = Trim(A_LoopField168) + " ";
        }
        if (A_Index168 == 77) {
            keyWordAsync = Trim(A_LoopField168) + " ";
        }
        if (A_Index168 == 78) {
            keyWordThrow = Trim(A_LoopField168) + " ";
        }
        if (A_Index168 == 79) {
            keyWordErrorMsg = Trim(A_LoopField168);
        }
        if (A_Index168 == 80) {
            keyWordTry = Trim(A_LoopField168);
        }
        if (A_Index168 == 81) {
            keyWordCatch = Trim(A_LoopField168) + " ";
        }
        if (A_Index168 == 82) {
            keyWordFinally = Trim(A_LoopField168);
        }
        if (A_Index168 == 83) {
            keyWordReturnStatement = Trim(A_LoopField168) + " ";
        }
        if (A_Index168 == 84) {
            keyWordArrayAppend = Trim(A_LoopField168);
        }
        if (A_Index168 == 85) {
            keyWordArrayPop = Trim(A_LoopField168);
        }
        if (A_Index168 == 86) {
            keyWordArraySize = Trim(A_LoopField168);
        }
        if (A_Index168 == 87) {
            keyWordArrayInsert = Trim(A_LoopField168);
        }
        if (A_Index168 == 88) {
            keyWordArrayRemove = Trim(A_LoopField168);
        }
        if (A_Index168 == 89) {
            keyWordArrayIndexOf = Trim(A_LoopField168);
        }
        if (A_Index168 == 90) {
            keyWordArrayDefinition = Trim(A_LoopField168);
        }
        if (A_Index168 == 91) {
            keyWordArrayOfIntegersDefinition = Trim(A_LoopField168);
        }
        if (A_Index168 == 92) {
            keyWordArrayOfStringsDefinition = Trim(A_LoopField168);
        }
        if (A_Index168 == 93) {
            keyWordArrayOfFloatingPointNumbersDefinition = Trim(A_LoopField168);
        }
        if (A_Index168 == 94) {
            keyWordArrayOfBooleansDefinition = Trim(A_LoopField168);
        }
        if (A_Index168 == 95) {
            keyWordVar = Trim(A_LoopField168) + " ";
        }
        if (A_Index168 == 96) {
            keyWordLet = Trim(A_LoopField168) + " ";
        }
        if (A_Index168 == 97) {
            keyWordConst = Trim(A_LoopField168) + " ";
        }
        if (A_Index168 == 98) {
            keyWordEnd = Trim(A_LoopField168);
        }
        if (A_Index168 == 99) {
            keyWordGlobal = Trim(A_LoopField168) + " ";
        }
        if (A_Index168 == 100) {
            keyWordComment = Trim(A_LoopField168);
        }
        if (A_Index168 == 101) {
            keyWordCommentOpenMultiLine = Trim(A_LoopField168);
        }
        if (A_Index168 == 102) {
            keyWordCommentCloseMultiLine = Trim(A_LoopField168);
        }
        if (A_Index168 == 103) {
            keyWordEscpaeChar = Trim(A_LoopField168);
        }
        if (A_Index168 == 104) {
            keyWordMainLabel = Trim(A_LoopField168);
        }
        if (A_Index168 == 105) {
            keyWordConcat = Trim(A_LoopField168);
        }
        if (A_Index168 == 106) {
            keyWordAdd = Trim(A_LoopField168);
        }
        if (A_Index168 == 107) {
            keyWordSub = Trim(A_LoopField168);
        }
        if (A_Index168 == 108) {
            keyWordMul = Trim(A_LoopField168);
        }
        if (A_Index168 == 109) {
            keyWordDiv = Trim(A_LoopField168);
        }
        if (A_Index168 == 110) {
            keyWordMod = Trim(A_LoopField168);
        }
        if (A_Index168 == 111) {
            keyWordExp = Trim(A_LoopField168);
        }
        if (A_Index168 == 112) {
            keyWordEqual = Trim(A_LoopField168);
        }
        if (A_Index168 == 113) {
            keyWordStrictEqual = Trim(A_LoopField168);
        }
        if (A_Index168 == 114) {
            keyWordNotEqual = Trim(A_LoopField168);
        }
        if (A_Index168 == 115) {
            keyWordGreater = Trim(A_LoopField168);
        }
        if (A_Index168 == 116) {
            keyWordLess = Trim(A_LoopField168);
        }
        if (A_Index168 == 117) {
            keyWordGreaterEqual = Trim(A_LoopField168);
        }
        if (A_Index168 == 118) {
            keyWordLessEqual = Trim(A_LoopField168);
        }
        if (A_Index168 == 119) {
            keyWordAnd = Trim(A_LoopField168);
        }
        if (A_Index168 == 120) {
            keyWordOr = Trim(A_LoopField168);
        }
        if (A_Index168 == 121) {
            keyWordNot = Trim(A_LoopField168);
        }
        if (A_Index168 == 122) {
            keyWordBitAnd = Trim(A_LoopField168);
        }
        if (A_Index168 == 123) {
            keyWordBitOr = Trim(A_LoopField168);
        }
        if (A_Index168 == 124) {
            keyWordBitXor = Trim(A_LoopField168);
        }
        if (A_Index168 == 125) {
            keyWordBitNot = Trim(A_LoopField168);
        }
        if (A_Index168 == 126) {
            keyWordShiftLeft = Trim(A_LoopField168);
        }
        if (A_Index168 == 127) {
            keyWordShiftRight = Trim(A_LoopField168);
        }
        if (A_Index168 == 128) {
            keyWordShiftUnsignedRight = Trim(A_LoopField168);
        }
        if (A_Index168 == 129) {
            keyWordAssign = Trim(A_LoopField168);
        }
        if (A_Index168 == 130) {
            keyWordAssignAdd = Trim(A_LoopField168);
        }
        if (A_Index168 == 131) {
            keyWordAssignConcat = Trim(A_LoopField168);
        }
        if (A_Index168 == 132) {
            keyWordAssignSub = Trim(A_LoopField168);
        }
        if (A_Index168 == 133) {
            keyWordAssignMul = Trim(A_LoopField168);
        }
        if (A_Index168 == 134) {
            keyWordAssignDiv = Trim(A_LoopField168);
        }
        if (A_Index168 == 135) {
            keyWordAssignMod = Trim(A_LoopField168);
        }
        if (A_Index168 == 136) {
            keyWordAssignShiftLeft = Trim(A_LoopField168);
        }
        if (A_Index168 == 137) {
            keyWordAssignShiftRight = Trim(A_LoopField168);
        }
        if (A_Index168 == 138) {
            keyWordLogicalAssignShiftRight = Trim(A_LoopField168);
        }
        if (A_Index168 == 139) {
            keyWordAssignBitAnd = Trim(A_LoopField168);
        }
        if (A_Index168 == 140) {
            keyWordAssignBitOr = Trim(A_LoopField168);
        }
        if (A_Index168 == 141) {
            keyWordAssignBitXor = Trim(A_LoopField168);
        }
        if (A_Index168 == 142) {
            keyWordTernary1 = Trim(A_LoopField168);
        }
        if (A_Index168 == 143) {
            keyWordTernary2 = Trim(A_LoopField168);
        }
        if (A_Index168 == 144) {
            keyWordInc = Trim(A_LoopField168);
        }
        if (A_Index168 == 145) {
            keyWordDec = Trim(A_LoopField168);
        }
        if (A_Index168 == 146) {
            AHKlikeLoopsIndexedAt = Trim(A_LoopField168);
        }
        if (A_Index168 == 147) {
            keyWordAIndex = Trim(A_LoopField168);
        }
        if (A_Index168 == 148) {
            keyWordALoopField = Trim(A_LoopField168);
        }
        if (A_Index168 == 149) {
            useCurlyBraces = Trim(A_LoopField168);
        }
        if (A_Index168 == 150) {
            useEnd = Trim(A_LoopField168);
        }
        if (A_Index168 == 151) {
            useSemicolon = Trim(A_LoopField168);
        }
        if (A_Index168 == 152) {
            useParentheses = Trim(A_LoopField168);
        }
        if (A_Index168 == 153) {
            usePrefixTypeForTypeDefinition = Trim(A_LoopField168);
        }
        if (A_Index168 == 154) {
            usePostfixTypeForTypeDefinition = Trim(A_LoopField168);
        }
        if (A_Index168 == 155) {
            usePythonicColonSyntax = Trim(A_LoopField168);
        }
        if (A_Index168 == 156) {
            useCurlyBracesSyntaxForArrayDef = Trim(A_LoopField168);
        }
        if (A_Index168 == 157) {
            useInJavaScriptAlwaysUseVar = Trim(A_LoopField168);
        }
        if (A_Index168 == 158) {
            useJavaScriptInAfullHTMLfile = Trim(A_LoopField168);
        }
        if (A_Index168 == 159) {
            useJavaScriptAmainFuncDef = Trim(A_LoopField168);
        }
        if (A_Index168 == 160) {
            useJavaScriptAllFuncsAreAsync = Trim(A_LoopField168);
        }
        if (A_Index168 == 161) {
            useJavaScriptAlwaysTripleEqual = Trim(A_LoopField168);
        }
    }
    if (HTVM_Size(argHTVMinstrMORE) > 0) {
        ...
        std::vector<std::string> items169 = LoopParseFunc(allInstructionFile_2, "\n", "\r");
        for (size_t A_Index169 = 0; A_Index169 < items169.size() + 0; A_Index169++) {
            std::string A_LoopField169 = items169[A_Index169 - 0];
            if (A_Index169 == 0) {
                langToConvertTo_2 = Trim(A_LoopField169);
            }
            if (A_Index169 == 1) {
                langFileExtension_2 = Trim(A_LoopField169);
            }
            if (A_Index169 == 2) {
                commands_2 = Trim(A_LoopField169);
            }
            if (A_Index169 == 3) {
                keyWordAlliance_2 = Trim(A_LoopField169) + " ";
            }
            if (A_Index169 == 4) {
                keyWordCrew_2 = Trim(A_LoopField169) + " ";
            }
            if (A_Index169 == 5) {
                keyWordProc_2 = Trim(A_LoopField169) + " ";
            }
            if (A_Index169 == 6) {
                keyWordStruct_2 = Trim(A_LoopField169) + " ";
            }
            if (A_Index169 == 7) {
                keyWordProp_2 = Trim(A_LoopField169) + " ";
            }
            if (A_Index169 == 8) {
                keyWordThis_2 = Trim(A_LoopField169);
            }
            if (A_Index169 == 9) {
                keyWordInclude_2 = Trim(A_LoopField169);
            }
            if (A_Index169 == 10) {
                keyWordCodeInTheTranspiledLangStart_2 = Trim(A_LoopField169);
            }
            if (A_Index169 == 11) {
                keyWordCodeInTheTranspiledLangEnd_2 = Trim(A_LoopField169);
            }
            if (A_Index169 == 12) {
                keyWordCodeInTheTranspiledLangStartCPP_2 = Trim(A_LoopField169);
            }
            if (A_Index169 == 13) {
                keyWordCodeInTheTranspiledLangEndCPP_2 = Trim(A_LoopField169);
            }
            if (A_Index169 == 14) {
                keyWordCodeInTheTranspiledLangStartPY_2 = Trim(A_LoopField169);
            }
            if (A_Index169 == 15) {
                keyWordCodeInTheTranspiledLangEndPY_2 = Trim(A_LoopField169);
            }
            if (A_Index169 == 16) {
                keyWordCodeInTheTranspiledLangStartJS_2 = Trim(A_LoopField169);
            }
            if (A_Index169 == 17) {
                keyWordCodeInTheTranspiledLangEndJS_2 = Trim(A_LoopField169);
            }
            if (A_Index169 == 18) {
                keyWordCodeInTheTranspiledLangStartGO_2 = Trim(A_LoopField169);
            }
            if (A_Index169 == 19) {
                keyWordCodeInTheTranspiledLangEndGO_2 = Trim(A_LoopField169);
            }
            if (A_Index169 == 20) {
                keyWordCodeInTheTranspiledLangStartLUA_2 = Trim(A_LoopField169);
            }
            if (A_Index169 == 21) {
                keyWordCodeInTheTranspiledLangEndLUA_2 = Trim(A_LoopField169);
            }
            if (A_Index169 == 22) {
                keyWordCodeInTheTranspiledLangStartCS_2 = Trim(A_LoopField169);
            }
            if (A_Index169 == 23) {
                keyWordCodeInTheTranspiledLangEndCS_2 = Trim(A_LoopField169);
            }
            if (A_Index169 == 24) {
                keyWordCodeInTheTranspiledLangStartJAVA_2 = Trim(A_LoopField169);
            }
            if (A_Index169 == 25) {
                keyWordCodeInTheTranspiledLangEndJAVA_2 = Trim(A_LoopField169);
            }
            if (A_Index169 == 26) {
                keyWordCodeInTheTranspiledLangStartKT_2 = Trim(A_LoopField169);
            }
            if (A_Index169 == 27) {
                keyWordCodeInTheTranspiledLangEndKT_2 = Trim(A_LoopField169);
            }
            if (A_Index169 == 28) {
                keyWordCodeInTheTranspiledLangStartRB_2 = Trim(A_LoopField169);
            }
            if (A_Index169 == 29) {
                keyWordCodeInTheTranspiledLangEndRB_2 = Trim(A_LoopField169);
            }
            if (A_Index169 == 30) {
                keyWordCodeInTheTranspiledLangStartNIM_2 = Trim(A_LoopField169);
            }
            if (A_Index169 == 31) {
                keyWordCodeInTheTranspiledLangEndNIM_2 = Trim(A_LoopField169);
            }
            if (A_Index169 == 32) {
                keyWordCodeInTheTranspiledLangStartAHK_2 = Trim(A_LoopField169);
            }
            if (A_Index169 == 33) {
                keyWordCodeInTheTranspiledLangEndAHK_2 = Trim(A_LoopField169);
            }
            if (A_Index169 == 34) {
                keyWordCodeInTheTranspiledLangStartSWIFT_2 = Trim(A_LoopField169);
            }
            if (A_Index169 == 35) {
                keyWordCodeInTheTranspiledLangEndSWIFT_2 = Trim(A_LoopField169);
            }
            if (A_Index169 == 36) {
                keyWordCodeInTheTranspiledLangStartDART_2 = Trim(A_LoopField169);
            }
            if (A_Index169 == 37) {
                keyWordCodeInTheTranspiledLangEndDART_2 = Trim(A_LoopField169);
            }
            if (A_Index169 == 38) {
                keyWordCodeInTheTranspiledLangStartTS_2 = Trim(A_LoopField169);
            }
            if (A_Index169 == 39) {
                keyWordCodeInTheTranspiledLangEndTS_2 = Trim(A_LoopField169);
            }
            if (A_Index169 == 40) {
                keyWordCodeInTheTranspiledLangStartGROOVY_2 = Trim(A_LoopField169);
            }
            if (A_Index169 == 41) {
                keyWordCodeInTheTranspiledLangEndGROOVY_2 = Trim(A_LoopField169);
            }
            if (A_Index169 == 42) {
                keyWordCodeInTheTranspiledLangStartHTVM_2 = Trim(A_LoopField169);
            }
            if (A_Index169 == 43) {
                keyWordCodeInTheTranspiledLangEndHTVM_2 = Trim(A_LoopField169);
            }
            if (A_Index169 == 44) {
                keyWordCodeInHTVMstart_2 = Trim(A_LoopField169);
            }
            if (A_Index169 == 45) {
                keyWordCodeInHTVMend_2 = Trim(A_LoopField169);
            }
            if (A_Index169 == 46) {
                keyWordCurlyBraceOpen_2 = Trim(A_LoopField169);
            }
            if (A_Index169 == 47) {
                keyWordCurlyBraceClose_2 = Trim(A_LoopField169);
            }
            if (A_Index169 == 48) {
                keyWordNull_2 = Trim(A_LoopField169);
            }
            if (A_Index169 == 49) {
                keyWordTrue_2 = Trim(A_LoopField169);
            }
            if (A_Index169 == 50) {
                keyWordFalse_2 = Trim(A_LoopField169);
            }
            if (A_Index169 == 51) {
                keyWordVoid_2 = Trim(A_LoopField169);
            }
            if (A_Index169 == 52) {
                keyWordDouble_2 = Trim(A_LoopField169);
            }
            if (A_Index169 == 53) {
                keyWordChar_2 = Trim(A_LoopField169);
            }
            if (A_Index169 == 54) {
                keyWordUint8_2 = Trim(A_LoopField169);
            }
            if (A_Index169 == 55) {
                keyWordUint16_2 = Trim(A_LoopField169);
            }
            if (A_Index169 == 56) {
                keyWordUint32_2 = Trim(A_LoopField169);
            }
            if (A_Index169 == 57) {
                keyWordUint64_2 = Trim(A_LoopField169);
            }
            if (A_Index169 == 58) {
                keyWordINT_2 = Trim(A_LoopField169);
            }
            if (A_Index169 == 59) {
                keyWordSTR_2 = Trim(A_LoopField169);
            }
            if (A_Index169 == 60) {
                keyWordBOOL_2 = Trim(A_LoopField169);
            }
            if (A_Index169 == 61) {
                keyWordFLOAT_2 = Trim(A_LoopField169);
            }
            if (A_Index169 == 62) {
                keyWordINT8_2 = Trim(A_LoopField169);
            }
            if (A_Index169 == 63) {
                keyWordINT16_2 = Trim(A_LoopField169);
            }
            if (A_Index169 == 64) {
                keyWordINT32_2 = Trim(A_LoopField169);
            }
            if (A_Index169 == 65) {
                keyWordINT64_2 = Trim(A_LoopField169);
            }
            if (A_Index169 == 66) {
                keyWordIF_2 = Trim(A_LoopField169) + " ";
            }
            if (A_Index169 == 67) {
                keyWordElseIf_2 = Trim(A_LoopField169) + " ";
            }
            if (A_Index169 == 68) {
                keyWordElse_2 = Trim(A_LoopField169);
            }
            if (A_Index169 == 69) {
                keyWordWhileLoop_2 = Trim(A_LoopField169) + " ";
            }
            if (A_Index169 == 70) {
                keyWordLoopInfinite_2 = Trim(A_LoopField169);
            }
            if (A_Index169 == 71) {
                keyWordLoop_2 = Trim(A_LoopField169) + " ";
            }
            if (A_Index169 == 72) {
                keyWordLoopParse_2 = Trim(A_LoopField169) + " ";
            }
            if (A_Index169 == 73) {
                keyWordContinue_2 = Trim(A_LoopField169);
            }
            if (A_Index169 == 74) {
                keyWordBreak_2 = Trim(A_LoopField169);
            }
            if (A_Index169 == 75) {
                keyWordFunc_2 = Trim(A_LoopField169) + " ";
            }
            if (A_Index169 == 76) {
                keyWordAwait_2 = Trim(A_LoopField169) + " ";
            }
            if (A_Index169 == 77) {
                keyWordAsync_2 = Trim(A_LoopField169) + " ";
            }
            if (A_Index169 == 78) {
                keyWordThrow_2 = Trim(A_LoopField169) + " ";
            }
            if (A_Index169 == 79) {
                keyWordErrorMsg_2 = Trim(A_LoopField169);
            }
            if (A_Index169 == 80) {
                keyWordTry_2 = Trim(A_LoopField169);
            }
            if (A_Index169 == 81) {
                keyWordCatch_2 = Trim(A_LoopField169) + " ";
            }
            if (A_Index169 == 82) {
                keyWordFinally_2 = Trim(A_LoopField169);
            }
            if (A_Index169 == 83) {
                keyWordReturnStatement_2 = Trim(A_LoopField169) + " ";
            }
            if (A_Index169 == 84) {
                keyWordArrayAppend_2 = Trim(A_LoopField169);
            }
            if (A_Index169 == 85) {
                keyWordArrayPop_2 = Trim(A_LoopField169);
            }
            if (A_Index169 == 86) {
                keyWordArraySize_2 = Trim(A_LoopField169);
            }
            if (A_Index169 == 87) {
                keyWordArrayInsert_2 = Trim(A_LoopField169);
            }
            if (A_Index169 == 88) {
                keyWordArrayRemove_2 = Trim(A_LoopField169);
            }
            if (A_Index169 == 89) {
                keyWordArrayIndexOf_2 = Trim(A_LoopField169);
            }
            if (A_Index169 == 90) {
                keyWordArrayDefinition_2 = Trim(A_LoopField169);
            }
            if (A_Index169 == 91) {
                keyWordArrayOfIntegersDefinition_2 = Trim(A_LoopField169);
            }
            if (A_Index169 == 92) {
                keyWordArrayOfStringsDefinition_2 = Trim(A_LoopField169);
            }
            if (A_Index169 == 93) {
                keyWordArrayOfFloatingPointNumbersDefinition_2 = Trim(A_LoopField169);
            }
            if (A_Index169 == 94) {
                keyWordArrayOfBooleansDefinition_2 = Trim(A_LoopField169);
            }
            if (A_Index169 == 95) {
                keyWordVar_2 = Trim(A_LoopField169);
            }
            if (A_Index169 == 96) {
                keyWordLet_2 = Trim(A_LoopField169);
            }
            if (A_Index169 == 97) {
                keyWordConst_2 = Trim(A_LoopField169);
            }
            if (A_Index169 == 98) {
                keyWordEnd_2 = Trim(A_LoopField169);
            }
            if (A_Index169 == 99) {
                keyWordGlobal_2 = Trim(A_LoopField169) + " ";
            }
            if (A_Index169 == 100) {
                keyWordComment_2 = Trim(A_LoopField169);
            }
            if (A_Index169 == 101) {
                keyWordCommentOpenMultiLine_2 = Trim(A_LoopField169);
            }
            if (A_Index169 == 102) {
                keyWordCommentCloseMultiLine_2 = Trim(A_LoopField169);
            }
            if (A_Index169 == 103) {
                keyWordEscpaeChar_2 = Trim(A_LoopField169);
            }
            if (A_Index169 == 104) {
                keyWordMainLabel_2 = Trim(A_LoopField169);
            }
            if (A_Index169 == 105) {
                keyWordConcat_2 = Trim(A_LoopField169);
            }
            if (A_Index169 == 106) {
                keyWordAdd_2 = Trim(A_LoopField169);
            }
            if (A_Index169 == 107) {
                keyWordSub_2 = Trim(A_LoopField169);
            }
            if (A_Index169 == 108) {
                keyWordMul_2 = Trim(A_LoopField169);
            }
            if (A_Index169 == 109) {
                keyWordDiv_2 = Trim(A_LoopField169);
            }
            if (A_Index169 == 110) {
                keyWordMod_2 = Trim(A_LoopField169);
            }
            if (A_Index169 == 111) {
                keyWordExp_2 = Trim(A_LoopField169);
            }
            if (A_Index169 == 112) {
                keyWordEqual_2 = Trim(A_LoopField169);
            }
            if (A_Index169 == 113) {
                keyWordStrictEqual_2 = Trim(A_LoopField169);
            }
            if (A_Index169 == 114) {
                keyWordNotEqual_2 = Trim(A_LoopField169);
            }
            if (A_Index169 == 115) {
                keyWordGreater_2 = Trim(A_LoopField169);
            }
            if (A_Index169 == 116) {
                keyWordLess_2 = Trim(A_LoopField169);
            }
            if (A_Index169 == 117) {
                keyWordGreaterEqual_2 = Trim(A_LoopField169);
            }
            if (A_Index169 == 118) {
                keyWordLessEqual_2 = Trim(A_LoopField169);
            }
            if (A_Index169 == 119) {
                keyWordAnd_2 = Trim(A_LoopField169);
            }
            if (A_Index169 == 120) {
                keyWordOr_2 = Trim(A_LoopField169);
            }
            if (A_Index169 == 121) {
                keyWordNot_2 = Trim(A_LoopField169);
            }
            if (A_Index169 == 122) {
                keyWordBitAnd_2 = Trim(A_LoopField169);
            }
            if (A_Index169 == 123) {
                keyWordBitOr_2 = Trim(A_LoopField169);
            }
            if (A_Index169 == 124) {
                keyWordBitXor_2 = Trim(A_LoopField169);
            }
            if (A_Index169 == 125) {
                keyWordBitNot_2 = Trim(A_LoopField169);
            }
            if (A_Index169 == 126) {
                keyWordShiftLeft_2 = Trim(A_LoopField169);
            }
            if (A_Index169 == 127) {
                keyWordShiftRight_2 = Trim(A_LoopField169);
            }
            if (A_Index169 == 128) {
                keyWordShiftUnsignedRight_2 = Trim(A_LoopField169);
            }
            if (A_Index169 == 129) {
                keyWordAssign_2 = Trim(A_LoopField169);
            }
            if (A_Index169 == 130) {
                keyWordAssignAdd_2 = Trim(A_LoopField169);
            }
            if (A_Index169 == 131) {
                keyWordAssignConcat_2 = Trim(A_LoopField169);
            }
            if (A_Index169 == 132) {
                keyWordAssignSub_2 = Trim(A_LoopField169);
            }
            if (A_Index169 == 133) {
                keyWordAssignMul_2 = Trim(A_LoopField169);
            }
            if (A_Index169 == 134) {
                keyWordAssignDiv_2 = Trim(A_LoopField169);
            }
            if (A_Index169 == 135) {
                keyWordAssignMod_2 = Trim(A_LoopField169);
            }
            if (A_Index169 == 136) {
                keyWordAssignShiftLeft_2 = Trim(A_LoopField169);
            }
            if (A_Index169 == 137) {
                keyWordAssignShiftRight_2 = Trim(A_LoopField169);
            }
            if (A_Index169 == 138) {
                keyWordLogicalAssignShiftRight_2 = Trim(A_LoopField169);
            }
            if (A_Index169 == 139) {
                keyWordAssignBitAnd_2 = Trim(A_LoopField169);
            }
            if (A_Index169 == 140) {
                keyWordAssignBitOr_2 = Trim(A_LoopField169);
            }
            if (A_Index169 == 141) {
                keyWordAssignBitXor_2 = Trim(A_LoopField169);
            }
            if (A_Index169 == 142) {
                keyWordTernary1_2 = Trim(A_LoopField169);
            }
            if (A_Index169 == 143) {
                keyWordTernary2_2 = Trim(A_LoopField169);
            }
            if (A_Index169 == 144) {
                keyWordInc_2 = Trim(A_LoopField169);
            }
            if (A_Index169 == 145) {
                keyWordDec_2 = Trim(A_LoopField169);
            }
            if (A_Index169 == 146) {
                AHKlikeLoopsIndexedAt_2 = Trim(A_LoopField169);
            }
            if (A_Index169 == 147) {
                keyWordAIndex_2 = Trim(A_LoopField169);
            }
            if (A_Index169 == 148) {
                keyWordALoopField_2 = Trim(A_LoopField169);
            }
            if (A_Index169 == 149) {
                useCurlyBraces_2 = Trim(A_LoopField169);
            }
            if (A_Index169 == 150) {
                useEnd_2 = Trim(A_LoopField169);
            }
            if (A_Index169 == 151) {
                useSemicolon_2 = Trim(A_LoopField169);
            }
            if (A_Index169 == 152) {
                useParentheses_2 = Trim(A_LoopField169);
            }
            if (A_Index169 == 153) {
                usePrefixTypeForTypeDefinition_2 = Trim(A_LoopField169);
            }
            if (A_Index169 == 154) {
                usePostfixTypeForTypeDefinition_2 = Trim(A_LoopField169);
            }
            if (A_Index169 == 155) {
                usePythonicColonSyntax_2 = Trim(A_LoopField169);
            }
            if (A_Index169 == 156) {
                useCurlyBracesSyntaxForArrayDef_2 = Trim(A_LoopField169);
            }
            if (A_Index169 == 157) {
                useInJavaScriptAlwaysUseVar_2 = Trim(A_LoopField169);
            }
            if (A_Index169 == 158) {
                useJavaScriptInAfullHTMLfile_2 = Trim(A_LoopField169);
            }
            if (A_Index169 == 159) {
                useJavaScriptAmainFuncDef_2 = Trim(A_LoopField169);
            }
            if (A_Index169 == 160) {
                useJavaScriptAllFuncsAreAsync_2 = Trim(A_LoopField169);
            }
            if (A_Index169 == 161) {
                useJavaScriptAlwaysTripleEqual_2 = Trim(A_LoopField169);
            }
        }
    }
    ...
    */
    ...
    for (size_t A_Index170 = 0; A_Index170 < items170.size() + 0; A_Index170++) {
        ...
        if (RegExMatch(Trim(A_LoopField170), "^name: [A-Za-z0-9_]+")) {
            availableFuncsInHTVMInHTVM += Trim(StrSplit(Trim(A_LoopField170), "name: ", 2)) + Chr(10);
        }
    }
    ...
    if (isNotHTVMfile == 0) {
        if (langToConvertTo != langFileExtension_2) {
            ...
            if (doseHaveInclude(code)) {
                ...
                for (int A_Index172 = 0; A_Index172 < 10000 + 0; A_Index172++) {
                    ...
                    for (size_t A_Index173 = 0; A_Index173 < items173.size() + 0; A_Index173++) {
                        ...
                        if (SubStr(StrLower(Trim(currentLine)), 1, StrLen(keyWordInclude + " ")) == StrLower(keyWordInclude + " ")) {
                            ...
                            for (int A_Index174 = 0; A_Index174 < HTVM_Size(includedFilePaths) + 0; A_Index174++) {
                                if (filePathToInclude == includedFilePaths[A_Index174]) {
        ...
        for (size_t A_Index175 = 0; A_Index175 < items175.size() + 0; A_Index175++) {
            ...
            if (Trim(StrLower(A_LoopField175)) == StrLower(keyWordCodeInTheTranspiledLangEnd)) {
                COUNT_programmingBlock_InTheTranspiledLang++;
                holdTempDataProgrammingBlockThenPutInArr = StringTrimRight(holdTempDataProgrammingBlockThenPutInArr, 1);
                programmingBlocksTemp += "programmingBlock_InTheTranspiledLang-programmingBlock_InTheTranspiledLang-AA" + STR(COUNT_programmingBlock_InTheTranspiledLang) + "AA\n";
                HTVM_Append(programmingBlock_InTheTranspiledLang, Chr(10) + holdTempDataProgrammingBlockThenPutInArr);
            ...
            else if (Trim(StrLower(A_LoopField175)) == StrLower(keyWordCodeInTheTranspiledLangEndCPP)) {
                COUNT_programmingBlock_CPP++;
                holdTempDataProgrammingBlockThenPutInArr = StringTrimRight(holdTempDataProgrammingBlockThenPutInArr, 1);
                programmingBlocksTemp += "programmingBlock_CPP-programmingBlock_CPP-AA" + STR(COUNT_programmingBlock_CPP) + "AA\n";
                HTVM_Append(programmingBlock_CPP, Chr(10) + holdTempDataProgrammingBlockThenPutInArr);
            ...
            else if (Trim(StrLower(A_LoopField175)) == StrLower(keyWordCodeInTheTranspiledLangEndPY)) {
                COUNT_programmingBlock_PY++;
                holdTempDataProgrammingBlockThenPutInArr = StringTrimRight(holdTempDataProgrammingBlockThenPutInArr, 1);
                programmingBlocksTemp += "programmingBlock_PY-programmingBlock_PY-AA" + STR(COUNT_programmingBlock_PY) + "AA\n";
                HTVM_Append(programmingBlock_PY, Chr(10) + holdTempDataProgrammingBlockThenPutInArr);
            ...
            else if (Trim(StrLower(A_LoopField175)) == StrLower(keyWordCodeInTheTranspiledLangEndJS)) {
                COUNT_programmingBlock_JS++;
                holdTempDataProgrammingBlockThenPutInArr = StringTrimRight(holdTempDataProgrammingBlockThenPutInArr, 1);
                programmingBlocksTemp += "programmingBlock_JS-programmingBlock_JS-AA" + STR(COUNT_programmingBlock_JS) + "AA\n";
                HTVM_Append(programmingBlock_JS, Chr(10) + holdTempDataProgrammingBlockThenPutInArr);
            ...
            else if (Trim(StrLower(A_LoopField175)) == StrLower(keyWordCodeInTheTranspiledLangEndGO)) {
                COUNT_programmingBlock_GO++;
                holdTempDataProgrammingBlockThenPutInArr = StringTrimRight(holdTempDataProgrammingBlockThenPutInArr, 1);
                programmingBlocksTemp += "programmingBlock_GO-programmingBlock_GO-AA" + STR(COUNT_programmingBlock_GO) + "AA\n";
                HTVM_Append(programmingBlock_GO, Chr(10) + holdTempDataProgrammingBlockThenPutInArr);
            ...
            else if (Trim(StrLower(A_LoopField175)) == StrLower(keyWordCodeInTheTranspiledLangEndLUA)) {
                COUNT_programmingBlock_LUA++;
                holdTempDataProgrammingBlockThenPutInArr = StringTrimRight(holdTempDataProgrammingBlockThenPutInArr, 1);
                programmingBlocksTemp += "programmingBlock_LUA-programmingBlock_LUA-AA" + STR(COUNT_programmingBlock_LUA) + "AA\n";
                HTVM_Append(programmingBlock_LUA, Chr(10) + holdTempDataProgrammingBlockThenPutInArr);
            ...
            else if (Trim(StrLower(A_LoopField175)) == StrLower(keyWordCodeInTheTranspiledLangEndCS)) {
                COUNT_programmingBlock_CS++;
                holdTempDataProgrammingBlockThenPutInArr = StringTrimRight(holdTempDataProgrammingBlockThenPutInArr, 1);
                programmingBlocksTemp += "programmingBlock_CS-programmingBlock_CS-AA" + STR(COUNT_programmingBlock_CS) + "AA\n";
                HTVM_Append(programmingBlock_CS, Chr(10) + holdTempDataProgrammingBlockThenPutInArr);
            ...
            else if (Trim(StrLower(A_LoopField175)) == StrLower(keyWordCodeInTheTranspiledLangEndJAVA)) {
                COUNT_programmingBlock_JAVA++;
                holdTempDataProgrammingBlockThenPutInArr = StringTrimRight(holdTempDataProgrammingBlockThenPutInArr, 1);
                programmingBlocksTemp += "programmingBlock_JAVA-programmingBlock_JAVA-AA" + STR(COUNT_programmingBlock_JAVA) + "AA\n";
                HTVM_Append(programmingBlock_JAVA, Chr(10) + holdTempDataProgrammingBlockThenPutInArr);
            ...
            else if (Trim(StrLower(A_LoopField175)) == StrLower(keyWordCodeInTheTranspiledLangEndKT)) {
                COUNT_programmingBlock_KT++;
                holdTempDataProgrammingBlockThenPutInArr = StringTrimRight(holdTempDataProgrammingBlockThenPutInArr, 1);
                programmingBlocksTemp += "programmingBlock_KT-programmingBlock_KT-AA" + STR(COUNT_programmingBlock_KT) + "AA\n";
                HTVM_Append(programmingBlock_KT, Chr(10) + holdTempDataProgrammingBlockThenPutInArr);
            ...
            else if (Trim(StrLower(A_LoopField175)) == StrLower(keyWordCodeInTheTranspiledLangEndRB)) {
                COUNT_programmingBlock_RB++;
                holdTempDataProgrammingBlockThenPutInArr = StringTrimRight(holdTempDataProgrammingBlockThenPutInArr, 1);
                programmingBlocksTemp += "programmingBlock_RB-programmingBlock_RB-AA" + STR(COUNT_programmingBlock_RB) + "AA\n";
                HTVM_Append(programmingBlock_RB, Chr(10) + holdTempDataProgrammingBlockThenPutInArr);
            ...
            else if (Trim(StrLower(A_LoopField175)) == StrLower(keyWordCodeInTheTranspiledLangEndNIM)) {
                COUNT_programmingBlock_NIM++;
                holdTempDataProgrammingBlockThenPutInArr = StringTrimRight(holdTempDataProgrammingBlockThenPutInArr, 1);
                programmingBlocksTemp += "programmingBlock_NIM-programmingBlock_NIM-AA" + STR(COUNT_programmingBlock_NIM) + "AA\n";
                HTVM_Append(programmingBlock_NIM, Chr(10) + holdTempDataProgrammingBlockThenPutInArr);
            ...
            else if (Trim(StrLower(A_LoopField175)) == StrLower(keyWordCodeInTheTranspiledLangEndAHK)) {
                COUNT_programmingBlock_AHK++;
                holdTempDataProgrammingBlockThenPutInArr = StringTrimRight(holdTempDataProgrammingBlockThenPutInArr, 1);
                programmingBlocksTemp += "programmingBlock_AHK-programmingBlock_AHK-AA" + STR(COUNT_programmingBlock_AHK) + "AA\n";
                HTVM_Append(programmingBlock_AHK, Chr(10) + holdTempDataProgrammingBlockThenPutInArr);
            ...
            else if (Trim(StrLower(A_LoopField175)) == StrLower(keyWordCodeInTheTranspiledLangEndSWIFT)) {
                COUNT_programmingBlock_SWIFT++;
                holdTempDataProgrammingBlockThenPutInArr = StringTrimRight(holdTempDataProgrammingBlockThenPutInArr, 1);
                programmingBlocksTemp += "programmingBlock_SWIFT-programmingBlock_SWIFT-AA" + STR(COUNT_programmingBlock_SWIFT) + "AA\n";
                HTVM_Append(programmingBlock_SWIFT, Chr(10) + holdTempDataProgrammingBlockThenPutInArr);
            ...
            else if (Trim(StrLower(A_LoopField175)) == StrLower(keyWordCodeInTheTranspiledLangEndDART)) {
                COUNT_programmingBlock_DART++;
                holdTempDataProgrammingBlockThenPutInArr = StringTrimRight(holdTempDataProgrammingBlockThenPutInArr, 1);
                programmingBlocksTemp += "programmingBlock_DART-programmingBlock_DART-AA" + STR(COUNT_programmingBlock_DART) + "AA\n";
                HTVM_Append(programmingBlock_DART, Chr(10) + holdTempDataProgrammingBlockThenPutInArr);
            ...
            else if (Trim(StrLower(A_LoopField175)) == StrLower(keyWordCodeInTheTranspiledLangEndTS)) {
                COUNT_programmingBlock_TS++;
                holdTempDataProgrammingBlockThenPutInArr = StringTrimRight(holdTempDataProgrammingBlockThenPutInArr, 1);
                programmingBlocksTemp += "programmingBlock_TS-programmingBlock_TS-AA" + STR(COUNT_programmingBlock_TS) + "AA\n";
                HTVM_Append(programmingBlock_TS, Chr(10) + holdTempDataProgrammingBlockThenPutInArr);
            ...
            else if (Trim(StrLower(A_LoopField175)) == StrLower(keyWordCodeInTheTranspiledLangEndGROOVY)) {
                COUNT_programmingBlock_GROOVY++;
                holdTempDataProgrammingBlockThenPutInArr = StringTrimRight(holdTempDataProgrammingBlockThenPutInArr, 1);
                programmingBlocksTemp += "programmingBlock_GROOVY-programmingBlock_GROOVY-AA" + STR(COUNT_programmingBlock_GROOVY) + "AA\n";
                HTVM_Append(programmingBlock_GROOVY, Chr(10) + holdTempDataProgrammingBlockThenPutInArr);
            ...
            else if (Trim(StrLower(A_LoopField175)) == StrLower(keyWordCodeInTheTranspiledLangEndHTVM)) {
                COUNT_programmingBlock_HTVM++;
                holdTempDataProgrammingBlockThenPutInArr = StringTrimRight(holdTempDataProgrammingBlockThenPutInArr, 1);
                programmingBlocksTemp += "programmingBlock_HTVM-programmingBlock_HTVM-AA" + STR(COUNT_programmingBlock_HTVM) + "AA\n";
                HTVM_Append(programmingBlock_HTVM, Chr(10) + holdTempDataProgrammingBlockThenPutInArr);
            ...
            else if (Trim(StrLower(A_LoopField175)) == StrLower(keyWordCodeInHTVMend)) {
                //;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;
                //;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;
                COUNT_programmingBlock_HTVMsyntax++;
                holdTempDataProgrammingBlockThenPutInArr = StringTrimRight(holdTempDataProgrammingBlockThenPutInArr, 1);
                ...
                holdTempDataProgrammingBlockThenPutInArr += A_LoopField175 + Chr(10);
            }
            else if (Trim(StrLower(A_LoopField175)) == StrLower(keyWordCodeInTheTranspiledLangStart)) {
                holdTempDataProgrammingBlockThenPutInArr = "";
                inProgarmmingBlock = 1;
            }
            else if (Trim(StrLower(A_LoopField175)) == StrLower(keyWordCodeInTheTranspiledLangStartCPP)) {
                holdTempDataProgrammingBlockThenPutInArr = "";
                inProgarmmingBlock = 1;
            }
            else if (Trim(StrLower(A_LoopField175)) == StrLower(keyWordCodeInTheTranspiledLangStartPY)) {
                holdTempDataProgrammingBlockThenPutInArr = "";
                inProgarmmingBlock = 1;
            }
            else if (Trim(StrLower(A_LoopField175)) == StrLower(keyWordCodeInTheTranspiledLangStartJS)) {
                holdTempDataProgrammingBlockThenPutInArr = "";
                inProgarmmingBlock = 1;
            }
            else if (Trim(StrLower(A_LoopField175)) == StrLower(keyWordCodeInTheTranspiledLangStartGO)) {
                holdTempDataProgrammingBlockThenPutInArr = "";
                inProgarmmingBlock = 1;
            }
            else if (Trim(StrLower(A_LoopField175)) == StrLower(keyWordCodeInTheTranspiledLangStartLUA)) {
                holdTempDataProgrammingBlockThenPutInArr = "";
                inProgarmmingBlock = 1;
            }
            else if (Trim(StrLower(A_LoopField175)) == StrLower(keyWordCodeInTheTranspiledLangStartCS)) {
                holdTempDataProgrammingBlockThenPutInArr = "";
                inProgarmmingBlock = 1;
            }
            else if (Trim(StrLower(A_LoopField175)) == StrLower(keyWordCodeInTheTranspiledLangStartJAVA)) {
                holdTempDataProgrammingBlockThenPutInArr = "";
                inProgarmmingBlock = 1;
            }
            else if (Trim(StrLower(A_LoopField175)) == StrLower(keyWordCodeInTheTranspiledLangStartKT)) {
                holdTempDataProgrammingBlockThenPutInArr = "";
                inProgarmmingBlock = 1;
            }
            else if (Trim(StrLower(A_LoopField175)) == StrLower(keyWordCodeInTheTranspiledLangStartRB)) {
                holdTempDataProgrammingBlockThenPutInArr = "";
                inProgarmmingBlock = 1;
            }
            else if (Trim(StrLower(A_LoopField175)) == StrLower(keyWordCodeInTheTranspiledLangStartNIM)) {
                holdTempDataProgrammingBlockThenPutInArr = "";
                inProgarmmingBlock = 1;
            }
            else if (Trim(StrLower(A_LoopField175)) == StrLower(keyWordCodeInTheTranspiledLangStartAHK)) {
                holdTempDataProgrammingBlockThenPutInArr = "";
                inProgarmmingBlock = 1;
            }
            else if (Trim(StrLower(A_LoopField175)) == StrLower(keyWordCodeInTheTranspiledLangStartSWIFT)) {
                holdTempDataProgrammingBlockThenPutInArr = "";
                inProgarmmingBlock = 1;
            }
            else if (Trim(StrLower(A_LoopField175)) == StrLower(keyWordCodeInTheTranspiledLangStartDART)) {
                holdTempDataProgrammingBlockThenPutInArr = "";
                inProgarmmingBlock = 1;
            }
            else if (Trim(StrLower(A_LoopField175)) == StrLower(keyWordCodeInTheTranspiledLangStartTS)) {
                holdTempDataProgrammingBlockThenPutInArr = "";
                inProgarmmingBlock = 1;
            }
            else if (Trim(StrLower(A_LoopField175)) == StrLower(keyWordCodeInTheTranspiledLangStartGROOVY)) {
                holdTempDataProgrammingBlockThenPutInArr = "";
                inProgarmmingBlock = 1;
            }
            else if (Trim(StrLower(A_LoopField175)) == StrLower(keyWordCodeInTheTranspiledLangStartHTVM)) {
                holdTempDataProgrammingBlockThenPutInArr = "";
                inProgarmmingBlock = 1;
            }
            else if (Trim(StrLower(A_LoopField175)) == StrLower(keyWordCodeInHTVMstart)) {
                //;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;
                //;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;
                holdTempDataProgrammingBlockThenPutInArr = "";
                inProgarmmingBlock = 1;
                ...
                programmingBlocksTemp += A_LoopField175 + Chr(10);
            }
        }
        code = StringTrimRight(programmingBlocksTemp, 1);
        //;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;; ----------------
    ...
    if (isNotHTVMfile == 0) {
        ...
        for (size_t A_Index176 = 0; A_Index176 < items176.size() + 0; A_Index176++) {
            ...
            if (SubStr(Trim(A_LoopField176), 1, StrLen(keyWordComment)) == keyWordComment) {
                commentsBugFixHold = StringTrimLeft(Trim(A_LoopField176), StrLen(keyWordComment));
                std::vector<std::string> items177 = LoopParseFunc(A_LoopField176);
                for (size_t A_Index177 = 0; A_Index177 < items177.size() + 0; A_Index177++) {
                    std::string A_LoopField177 = items177[A_Index177 - 0];
                    if (InStr(A_LoopField177, " ") || InStr(A_LoopField177, Chr(9))) {
                        commentsBugFixHold1 = A_LoopField177;
                    } else {
                        break;
                    }
                }
                ...
                commentsBugFixOut += A_LoopField176 + Chr(10);
            }
        }
        ...
        for (size_t A_Index179 = 0; A_Index179 < items179.size() + 0; A_Index179++) {
            ...
            HTVM_Append(getAllCharForTheFurtureSoIcanAddEscapeChar, A_LoopField179);
        }
        ...
        for (size_t A_Index180 = 0; A_Index180 < items180.size() + 0; A_Index180++) {
            ...
            if (A_LoopField180 == keyWordEscpaeChar && getAllCharForTheFurtureSoIcanAddEscapeChar[A_Index180 + 1] == Chr(34)) {
                ...
                if (fixOutFixDoubleQuotesInsideDoubleQuotesFIXok != 1) {
                    OutFixDoubleQuotesInsideDoubleQuotes += A_LoopField180;
                } else {
        ...
        if (keyWordEscpaeChar == Chr(92) && langToConvertTo != langFileExtension_2) {
            ...
            for (size_t A_Index181 = 0; A_Index181 < items181.size() + 0; A_Index181++) {
                ...
                if (A_LoopField181 == Chr(34)) {
                    areWEinSome34sNum++;
                }
                if (areWEinSome34sNum == 1) {
                    if (A_LoopField181 != Chr(34)) {
                        if (A_LoopField181 == keyWordEscpaeChar && langToConvertTo != langFileExtension_2) {
                            theIdNumOfThe34theVar[theIdNumOfThe34] = theIdNumOfThe34theVar[theIdNumOfThe34] + Chr(92);
                        } else {
                            theIdNumOfThe34theVar[theIdNumOfThe34] = theIdNumOfThe34theVar[theIdNumOfThe34] + A_LoopField181;
                        }
                    } else {
                        theIdNumOfThe34++;
                ...
                if (areWEinSome34sNum == 2 || areWEinSome34sNum == 0) {
                    if (A_LoopField181 != Chr(34)) {
                        htCodeOUT754754 += A_LoopField181;
                    }
                    areWEinSome34sNum = 0;
                }
            }
            ...
            for (size_t A_Index182 = 0; A_Index182 < items182.size() + 0; A_Index182++) {
                ...
                if (A_LoopField182 == Chr(34)) {
                    areWEinSome34sNum++;
                }
                if (areWEinSome34sNum == 1) {
                    if (A_LoopField182 != Chr(34)) {
                        if (A_LoopField182 == keyWordEscpaeChar && keyWordEscpaeChar == getAllCharForTheFurtureSoIcanAddEscapeChar[A_Index182 + 1] && langToConvertTo != langFileExtension_2) {
                        ...
                        else if (A_LoopField182 == keyWordEscpaeChar && langToConvertTo != langFileExtension_2) {
                            ...
                            theIdNumOfThe34theVar[theIdNumOfThe34] = theIdNumOfThe34theVar[theIdNumOfThe34] + A_LoopField182;
                        }
                    } else {
                        theIdNumOfThe34++;
                        htCodeOUT754754 += "ihuiuuhuuhtheidFor--asds" + str21 + "as--" + str21 + "theuhturtyphoutr--" + Chr(65) + Chr(65) + STR(theIdNumOfThe34) + Chr(65) + Chr(65);
                ...
                if (areWEinSome34sNum == 2 || areWEinSome34sNum == 0) {
                    if (A_LoopField182 != Chr(34)) {
                        htCodeOUT754754 += A_LoopField182;
                    }
                    areWEinSome34sNum = 0;
                }
            }
        ...
        for (int A_Index183 = 0; A_Index183 < theIdNumOfThe34 + 0; A_Index183++) {
            theIdNumOfThe34theVar[A_Index183] = theIdNumOfThe34theVar[A_Index183] + Chr(34);
        }
        HTVM_Append(theIdNumOfThe34theVar, Chr(34));
        if (langToConvertTo == "dart" || langToConvertTo == "groovy") {
            for (int A_Index184 = 0; A_Index184 < HTVM_Size(theIdNumOfThe34theVar) + 0; A_Index184++) {
                theIdNumOfThe34theVar[A_Index184] = StrReplace(theIdNumOfThe34theVar[A_Index184], "$", Chr(92) + "$");
            }
        }
        ...
        for (size_t A_Index185 = 0; A_Index185 < items185.size() + 0; A_Index185++) {
            ...
            if (InStr(A_LoopField185, " " + keyWordComment) != false && SubStr(Trim(A_LoopField185), 1, StrLen(keyWordComment)) != keyWordComment) {
                posForRemoveCommentsOnTheEndOfTheLine = 0;
                slidingWinFixCommentsRmI = 0;
                slidingWinFixCommentsRm = slidingWinFixCommentsRmFUNC(A_LoopField185);
                for (int A_Index186 = 0; A_Index186 < HTVM_Size(slidingWinFixCommentsRm) + 0; A_Index186++) {
                    slidingWinFixCommentsRmI = A_Index186;
                    slidingWinFixCommentsRmHold = "";
                    for (int A_Index187 = 0; A_Index187 < StrLen(" " + keyWordComment) + 0; A_Index187++) {
                        slidingWinFixCommentsRmHold += slidingWinFixCommentsRm[slidingWinFixCommentsRmI + A_Index187];
                    }
                    if (Trim(slidingWinFixCommentsRmHold) == keyWordComment) {
                        posForRemoveCommentsOnTheEndOfTheLine = A_Index186;
                        break;
                    }
                }
                if (posForRemoveCommentsOnTheEndOfTheLine > 0) {
                    // Keep only the part before the comment
                    newStringOutCode += SubStr(A_LoopField185, 1, posForRemoveCommentsOnTheEndOfTheLine) + Chr(10);
                } else {
                    // Keep the whole line as it is
                    newStringOutCode += A_LoopField185 + Chr(10);
                }
            } else {
                newStringOutCode += A_LoopField185 + Chr(10);
            }
        ...
        if (langToConvertTo != langFileExtension_2) {
            ...
            for (size_t A_Index188 = 0; A_Index188 < items188.size() + 0; A_Index188++) {
                ...
                if (SubStr(Trim(A_LoopField188), 1, 1) != ";") {
                    semiColonForOneLinerOut += StrReplace(A_LoopField188, ";", Chr(10)) + Chr(10);
                } else {
                    semiColonForOneLinerOut += A_LoopField188 + Chr(10);
                }
            }
        ...
        if (useCurlyBraces == "on") {
            ...
            for (size_t A_Index189 = 0; A_Index189 < items189.size() + 0; A_Index189++) {
                ...
                str1 = "";
                if (InStr(A_LoopField189, keyWordCurlyBraceOpen) || InStr(A_LoopField189, keyWordCurlyBraceClose)) {
                    if (SubStr(Trim(A_LoopField189), 1, StrLen(Trim(keyWordComment))) == Trim(keyWordComment) && isLineAconstruct(Trim(A_LoopField189)) == false) {
                        fixSomeCurlyBraceForExratFlexability += A_LoopField189 + Chr(10);
                    } else {
                        if (InStr(A_LoopField189, Trim(keyWordArrayDefinition) + " " + keyWordAssign + " ") || InStr(A_LoopField189, Trim(keyWordArrayOfIntegersDefinition) + " " + keyWordAssign + " ") || InStr(A_LoopField189, Trim(keyWordArrayOfStringsDefinition) + " " + keyWordAssign + " ") || InStr(A_LoopField189, Trim(keyWordArrayOfFloatingPointNumbersDefinition) + " " + keyWordAssign + " ") || InStr(A_LoopField189, Trim(keyWordArrayOfBooleansDefinition) + " " + keyWordAssign + " ") && useCurlyBracesSyntaxForArrayDef == "on") {
                            fixSomeCurlyBraceForExratFlexability += A_LoopField189 + Chr(10);
                        } else {
                            str1 = StrReplace(A_LoopField189, keyWordCurlyBraceOpen, "{");
            ...
            for (size_t A_Index190 = 0; A_Index190 < items190.size() + 0; A_Index190++) {
                ...
                fixSomeCurlyBraceForExratFlexabilityTriHELP += Trim(A_LoopField190) + Chr(10);
            }
            ...
            if (keyWordCurlyBraceOpen != "{") {
                ...
                for (size_t A_Index191 = 0; A_Index191 < items191.size() + 0; A_Index191++) {
                    ...
                    htCodeOutFixINT2 = 0;
                    if (isLineAconstruct(Trim(A_LoopField191)) == true || Trim(A_LoopField191) == keyWordCurlyBraceOpen) {
                        htCodeOutFixINT2 = 1;
                    }
                    htCodeOutFixINT = 0;
                    ...
                    htCodeOutFixINT--;
                    std::vector<std::string> items193 = LoopParseFunc(A_LoopField191, " ");
                    for (size_t A_Index193 = 0; A_Index193 < items193.size() + 0; A_Index193++) {
                        std::string A_LoopField193 = items193[A_Index193 - 0];
                        if (htCodeOutFixINT == A_Index193 && htCodeOutFixINT2 == 1) {
                            htCodeOutFixINT2 = 0;
                            htCodeOutFixEnd += HTVMmatchStrRrplace(A_LoopField193, keyWordCurlyBraceOpen, "{") + " ";
                        } else {
            ...
            if (keyWordCurlyBraceClose != "}") {
                ...
                for (size_t A_Index194 = 0; A_Index194 < items194.size() + 0; A_Index194++) {
                    ...
                    htCodeOutFixINT2 = 0;
                    if (isLineAconstruct(Trim(A_LoopField194)) == true || Trim(A_LoopField194) == keyWordCurlyBraceClose) {
                        htCodeOutFixINT2 = 1;
                    }
                    std::vector<std::string> items195 = LoopParseFunc(A_LoopField194, " ");
                    for (size_t A_Index195 = 0; A_Index195 < items195.size() + 0; A_Index195++) {
                        std::string A_LoopField195 = items195[A_Index195 - 0];
                        if (htCodeOutFixINT2 == 1 || Trim(A_LoopField195) == keyWordCurlyBraceClose) {
                            htCodeOutFixINT2 = 0;
                            htCodeOutFixEnd += HTVMmatchStrRrplace(A_LoopField195, keyWordCurlyBraceClose, "}") + " ";
                        } else {
        ...
        if (useCurlyBraces == "on") {
            ...
            for (size_t A_Index196 = 0; A_Index196 < items196.size() + 0; A_Index196++) {
                ...
                HTVM_Append(addNoCurlyFixWhenNextLineAfterConstructLookAhead, Trim(A_LoopField196));
            }
            ...
            for (size_t A_Index197 = 0; A_Index197 < items197.size() + 0; A_Index197++) {
                ...
                if (isLineAconstruct(Trim(A_LoopField197)) && addNoCurlyFixWhenNextLineAfterConstructLookAhead[A_Index197 + 1] != "{") {
                    int5 = 1;
                    addNoCurlyFixWhenNextLineAfterConstructOut += A_LoopField197 + Chr(10) + "{" + Chr(10);
                } else {
                    if (int5 == 1) {
                        int5 = 0;
                        addNoCurlyFixWhenNextLineAfterConstructOut += A_LoopField197 + Chr(10) + "}" + Chr(10);
                    } else {
                        addNoCurlyFixWhenNextLineAfterConstructOut += A_LoopField197 + Chr(10);
                    }
                }
            }
        ...
        if (useEnd == "on") {
            ...
            for (size_t A_Index198 = 0; A_Index198 < items198.size() + 0; A_Index198++) {
                ...
                if (StrLower(Trim(A_LoopField198)) == StrLower(keyWordEnd)) {
                    htCodeOutFixEnd += "}\n";
                }
                else if (SubStr(StrLower(Trim(A_LoopField198)), 1, StrLen(StrLower(keyWordIF))) == StrLower(keyWordIF) && isLineAconstruct(Trim(A_LoopField198)) == true) {
                    htCodeOutFixEnd += A_LoopField198 + "\n{\n";
                }
                else if (SubStr(StrLower(Trim(A_LoopField198)), 1, StrLen(StrLower(keyWordWhileLoop))) == StrLower(keyWordWhileLoop) && isLineAconstruct(Trim(A_LoopField198)) == true) {
                    htCodeOutFixEnd += A_LoopField198 + "\n{\n";
                }
                else if (SubStr(StrLower(Trim(A_LoopField198)), 1, StrLen(StrLower(keyWordElseIf))) == StrLower(keyWordElseIf) && isLineAconstruct(Trim(A_LoopField198)) == true) {
                    htCodeOutFixEnd += "\n}\n" + A_LoopField198 + "\n{\n";
                }
                else if (SubStr(StrLower(Trim(A_LoopField198)), 1, StrLen(StrLower(keyWordElse))) == StrLower(keyWordElse) && isLineAconstruct(Trim(A_LoopField198)) == true) {
                    htCodeOutFixEnd += "\n}\n" + A_LoopField198 + "\n{\n";
                }
                else if (SubStr(StrLower(Trim(A_LoopField198)), 1, StrLen(StrLower(keyWordCatch))) == StrLower(keyWordCatch) && isLineAconstruct(Trim(A_LoopField198)) == true) {
                    htCodeOutFixEnd += "\n}\n" + A_LoopField198 + "\n{\n";
                }
                else if (SubStr(StrLower(Trim(A_LoopField198)), 1, StrLen(StrLower(keyWordFinally))) == StrLower(keyWordFinally) && isLineAconstruct(Trim(A_LoopField198)) == true) {
                    htCodeOutFixEnd += "\n}\n" + A_LoopField198 + "\n{\n";
                }
                else if (isLineAconstruct(Trim(A_LoopField198)) == true) {
                    htCodeOutFixEnd += A_LoopField198 + "\n{\n";
                } else {
        ...
        if (usePythonicColonSyntax == "off") {
            if (useEnd == "off" && useCurlyBraces == "off") {
                ...
                for (size_t A_Index199 = 0; A_Index199 < items199.size() + 0; A_Index199++) {
                    ...
                    if (isLineAconstruct(Trim(A_LoopField199)) == true) {
                        htCodeOutFixEnd += A_LoopField199 + ":\n";
                    } else {
                        htCodeOutFixEnd += A_LoopField199 + Chr(10);
                    }
                }
                code = StringTrimRight(htCodeOutFixEnd, 1);
            }
        ...
        for (int A_Index201 = 0; A_Index201 < 2 + 0; A_Index201++) {
            ...
            for (size_t A_Index202 = 0; A_Index202 < items202.size() + 0; A_Index202++) {
                ...
                if (InStr(A_LoopField202, Trim(keyWordArrayDefinition) + " " + keyWordAssign + " ") || InStr(A_LoopField202, Trim(keyWordArrayOfIntegersDefinition) + " " + keyWordAssign + " ") || InStr(A_LoopField202, Trim(keyWordArrayOfStringsDefinition) + " " + keyWordAssign + " ") || InStr(A_LoopField202, Trim(keyWordArrayOfFloatingPointNumbersDefinition) + " " + keyWordAssign + " ") || InStr(A_LoopField202, Trim(keyWordArrayOfBooleansDefinition) + " " + keyWordAssign + " ") && useCurlyBracesSyntaxForArrayDef == "on") {
                    if (SubStr(StrLower(A_LoopField202), 1, StrLen(StrLower(keyWordFunc))) == StrLower(keyWordFunc) || SubStr(StrLower(A_LoopField202), 1, StrLen(StrLower(keyWordAsync) + StrLower(keyWordFunc))) == StrLower(keyWordAsync) + StrLower(keyWordFunc)) {
                        if (InStr(Trim(A_LoopField202), "{") && Trim(A_LoopField202) != "{" && SubStr(Trim(A_LoopField202), 1, StrLen(Trim(keyWordComment))) != Trim(keyWordComment)) {
                            outCodeFixBraces += Trim(StrReplace(Trim(A_LoopField202), "{", "")) + "\n{\n";
                        }
                        else if (InStr(Trim(A_LoopField202), "}") && Trim(A_LoopField202) != "}" && SubStr(Trim(A_LoopField202), 1, StrLen(Trim(keyWordComment))) != Trim(keyWordComment)) {
                            outCodeFixBraces += "}\n" + Trim(StrReplace(Trim(A_LoopField202), "}", "")) + Chr(10);
                        } else {
                            outCodeFixBraces += Trim(A_LoopField202) + Chr(10);
                        }
                    } else {
                        outCodeFixBraces += Trim(A_LoopField202) + Chr(10);
                    }
        ...
        if (usePythonicColonSyntax == "on" || useCurlyBraces == "off" && useEnd == "off") {
            ...
            for (size_t A_Index203 = 0; A_Index203 < items203.size() + 0; A_Index203++) {
                ...
                if (isLineAconstruct(Trim(A_LoopField203)) == true) {
                    htCodeOutFixEnd += StringTrimRight(A_LoopField203, 1) + Chr(10);
                } else {
                    htCodeOutFixEnd += A_LoopField203 + Chr(10);
                }
            }
            code = StringTrimRight(htCodeOutFixEnd, 1);
        }
        ...
        if (isNotHTVMfile2 == 0) {
            ...
            for (size_t A_Index204 = 0; A_Index204 < items204.size() + 0; A_Index204++) {
                ...
                if (StrLower(Trim(A_LoopField204)) == StrLower(keyWordMainLabel)) {
                    didWeUseMainLabel = 1;
                    if (langToConvertTo == langFileExtension_2) {
                    ...
                    codeOutFixAndAddMainFunc += A_LoopField204 + Chr(10);
                }
            }
        ...
        for (size_t A_Index205 = 0; A_Index205 < items205.size() + 0; A_Index205++) {
            ...
            HTVM_Append(lookIntoTheNextLineForFuncWhitNoKeyWord, A_LoopField205);
        }
        ...
        for (size_t A_Index206 = 0; A_Index206 < items206.size() + 0; A_Index206++) {
            ...
            lineDone = 0;
            if (SubStr(A_LoopField206, 1, StrLen(keyWordComment)) == keyWordComment) {
                lineDone = 1;
                str1 = StringTrimLeft(A_LoopField206, StrLen(keyWordComment));
                if (langToConvertTo == langFileExtension_2) {
            ...
            else if (InStr(A_LoopField206, "guiInit(") || InStr(A_LoopField206, "guiAdd(") || InStr(A_LoopField206, "guiAddElement(") || InStr(A_LoopField206, "guiControl(") || InStr(A_LoopField206, "HTVM_init(") || InStr(A_LoopField206, "HTVM_register(") || InStr(A_LoopField206, "HTVM_custom_port(") && lineDone == 0) {
                lineDone = 1;
                if (InStr(A_LoopField206, "HTVM_init(")) {
                    htCode += Trim(A_LoopField206) + Chr(10);
                } else {
                    htCode += Trim(expressionParserTranspiler(Trim(A_LoopField206))) + Chr(10);
                }
            }
            else if (Trim(A_LoopField206) == keyWordCommentOpenMultiLine) {
                lineDone = 1;
                if (langToConvertTo == langFileExtension_2) {
            ...
            else if (Trim(A_LoopField206) == keyWordCommentCloseMultiLine) {
                lineDone = 1;
                if (langToConvertTo == langFileExtension_2) {
            ...
            else if (InStr(A_LoopField206, "[][][][][] ")) {
                lineDone = 1;
                str1 = StrSplit(A_LoopField206, " ", 2);
            ...
            else if (SubStr(StrLower(Trim(A_LoopField206)), 1, StrLen(StrLower(keyWordStruct))) == StrLower(keyWordStruct)) {
                lineDone = 1;
                str0 = Trim(StringTrimLeft(A_LoopField206, StrLen(keyWordStruct)));
                if (langToConvertTo == langFileExtension_2) {
                    htCode += Trim(keyWordStruct_2) + " " + str0 + Chr(10);
                }
            }
            else if (SubStr(StrLower(Trim(A_LoopField206)), 1, StrLen(StrLower(keyWordAlliance))) == StrLower(keyWordAlliance)) {
                lineDone = 1;
                str0 = Trim(StringTrimLeft(A_LoopField206, StrLen(keyWordAlliance)));
                if (langToConvertTo == langFileExtension_2) {
                    htCode += Trim(keyWordAlliance_2) + " " + str0 + Chr(10);
                }
            }
            else if (SubStr(StrLower(Trim(A_LoopField206)), 1, StrLen(StrLower(keyWordCrew))) == StrLower(keyWordCrew)) {
                lineDone = 1;
                str0 = Trim(StringTrimLeft(A_LoopField206, StrLen(keyWordCrew)));
                if (langToConvertTo == langFileExtension_2) {
                    htCode += Trim(keyWordCrew_2) + " " + str0 + Chr(10);
                }
            }
            else if (InStr(A_LoopField206, "[end][end][end][end][end] ")) {
                lineDone = 1;
                if (langToConvertTo == langFileExtension_2) {
                    htCode += "}" + Chr(10);
                }
            }
            else if (SubStr(StrLower(Trim(A_LoopField206)), 1, StrLen(StrLower(keyWordProp))) == StrLower(keyWordProp)) {
                lineDone = 1;
                str0 = Trim(StringTrimLeft(A_LoopField206, StrLen(keyWordProp)));
            ...
            else if (SubStr(StrLower(Trim(A_LoopField206)), 1, StrLen(StrLower(keyWordProc))) == StrLower(keyWordProc)) {
                lineDone = 1;
                str0 = Trim(StringTrimLeft(A_LoopField206, StrLen(keyWordProc)));
                str1 = Trim(StrSplit(str0, "-----------------------HTVM-OSP-HTVM-OBJ--------------", 1));
                str20 = Trim(StrSplit(str0, "-----------------------HTVM-OSP-HTVM-OBJ--------------", 2));
            ...
            else if (SubStr(StrLower(Trim(A_LoopField206)), 1, StrLen(StrLower(keyWordIF))) == StrLower(keyWordIF) && SubStr(StrLower(Trim(A_LoopField206)), 1, StrLen(StrLower(keyWordElseIf))) != StrLower(keyWordElseIf)) {
                lineDone = 1;
                str1 = Trim(StringTrimLeft(A_LoopField206, StrLen(keyWordIF)));
            ...
            else if (SubStr(StrLower(Trim(A_LoopField206)), 1, StrLen(StrLower(keyWordElseIf))) == StrLower(keyWordElseIf) && SubStr(StrLower(Trim(A_LoopField206)), 1, StrLen(StrLower(keyWordIF))) != StrLower(keyWordIF)) {
                lineDone = 1;
                str1 = Trim(StringTrimLeft(A_LoopField206, StrLen(keyWordElseIf)));
                str1 = Trim(expressionParserTranspiler("(" + str1 + ")"));
                str1 = StringTrimRight(str1, 1);
                str1 = StringTrimLeft(str1, 1);
                if (langToConvertTo == langFileExtension_2) {
            ...
            else if (StrLower(Trim(A_LoopField206)) == StrLower(keyWordElse) || StrLower(Trim(A_LoopField206)) == StrLower(keyWordElse + ":")) {
                lineDone = 1;
                if (langToConvertTo == langFileExtension_2) {
                    if (usePythonicColonSyntax_2 == "on") {
            ...
            else if (SubStr(StrLower(Trim(A_LoopField206)), 1, StrLen(StrLower(keyWordWhileLoop))) == StrLower(keyWordWhileLoop)) {
                lineDone = 1;
                str1 = Trim(StringTrimLeft(A_LoopField206, StrLen(keyWordWhileLoop)));
                str1 = Trim(expressionParserTranspiler("(" + str1 + ")"));
                str1 = StringTrimRight(str1, 1);
                str1 = StringTrimLeft(str1, 1);
                if (langToConvertTo == langFileExtension_2) {
            ...
            else if (StrLower(Trim(A_LoopField206)) == StrLower(keyWordTry) || StrLower(Trim(A_LoopField206)) == StrLower(keyWordTry + ":")) {
                lineDone = 1;
                if (langToConvertTo == langFileExtension_2) {
                    if (usePythonicColonSyntax_2 == "on") {
                        htCode += keyWordTry_2 + ":\n";
            ...
            else if (SubStr(StrLower(Trim(A_LoopField206)), 1, StrLen(StrLower(keyWordCatch))) == StrLower(keyWordCatch)) {
                lineDone = 1;
                str1 = Trim(StrReplace(StrReplace(StringTrimLeft(A_LoopField206, StrLen(keyWordCatch)), "(", ""), ")", ""));
            ...
            else if (StrLower(Trim(A_LoopField206)) == StrLower(keyWordFinally) || StrLower(Trim(A_LoopField206)) == StrLower(keyWordFinally + ":")) {
                lineDone = 1;
                if (langToConvertTo == langFileExtension_2) {
                    if (usePythonicColonSyntax_2 == "on") {
            ...
            else if (SubStr(StrLower(Trim(A_LoopField206)), 1, StrLen(StrLower(keyWordThrow))) == StrLower(keyWordThrow)) {
                lineDone = 1;
                str1 = Trim(A_LoopField206);
                if (SubStrLastChars(str1, 1) == ";") {
                    str1 = StringTrimRight(str1, 1);
                }
            ...
            else if (StrLower(A_LoopField206) == StrLower(keyWordBreak) || StrLower(A_LoopField206) == StrLower(keyWordBreak + ";")) {
                lineDone = 1;
                if (langToConvertTo == langFileExtension_2) {
                    if (useSemicolon_2 == "on") {
            ...
            else if (StrLower(A_LoopField206) == StrLower(Trim(keyWordReturnStatement)) || StrLower(A_LoopField206) == StrLower(Trim(keyWordReturnStatement) + ";")) {
                lineDone = 1;
                if (langToConvertTo == langFileExtension_2) {
                    if (useSemicolon_2 == "on") {
            ...
            else if (SubStr(Trim(StrLower(A_LoopField206)), 1, StrLen(StrLower(keyWordReturnStatement))) == StrLower(keyWordReturnStatement)) {
                lineDone = 1;
                str1 = Trim(expressionParserTranspiler(Trim(StringTrimLeft(A_LoopField206, StrLen(keyWordReturnStatement)))));
                if (SubStrLastChars(str1, 1) == ";") {
                    str1 = StringTrimRight(str1, 1);
                }
                if (langToConvertTo == langFileExtension_2) {
            ...
            else if (StrLower(A_LoopField206) == StrLower(keyWordContinue) || StrLower(A_LoopField206) == StrLower(keyWordContinue + ";")) {
                lineDone = 1;
                if (langToConvertTo == langFileExtension_2) {
                    if (useSemicolon_2 == "on") {
            ...
            else if (StrLower(A_LoopField206) == "::" + keyWordContinue || StrLower(A_LoopField206) == "::" + keyWordContinue + ";") {
                lineDone = 1;
                if (langToConvertTo == langFileExtension_2) {
                    htCode += "::" + keyWordContinue_2 + Chr(10);
                }
            ...
            else if (StrLower(A_LoopField206) == StrLower(keyWordLoopInfinite) || StrLower(A_LoopField206) == StrLower(keyWordLoopInfinite + ":")) {
                // infinity loops
            ...
            else if (SubStr(Trim(StrLower(A_LoopField206)), 1, StrLen(StrLower(keyWordLoop))) == StrLower(keyWordLoop) && SubStr(Trim(StrLower(A_LoopField206)), 1, StrLen(StrLower(keyWordLoop)) + 2) != StrLower(keyWordLoop) + "% " && SubStr(Trim(StrLower(A_LoopField206)), 1, StrLen(StrLower(keyWordLoopParse))) != StrLower(keyWordLoopParse)) {
                out2 = StringTrimLeft(A_LoopField206, StrLen(keyWordLoop));
            ...
            else if (SubStr(Trim(StrLower(A_LoopField206)), 1, StrLen(StrLower(keyWordLoop)) + 2) == StrLower(keyWordLoop) + "% ") {
                howMany_fixAindexInGoUnused++;
                out2 = StringTrimLeft(A_LoopField206, StrLen(keyWordLoop) + 2);
                out2 = StrReplace(out2, ":", "");
                //MsgBox % out2
                //MsgBox, % out2
                out2 = Trim(out2);
            ...
            else if (SubStr(StrLower(A_LoopField206), 1, StrLen(StrLower(keyWordLoopParse))) == StrLower(keyWordLoopParse)) {
                howMany_fixAindexInGoUnused++;
                var1 = StringTrimLeft(A_LoopField206, StrLen(keyWordLoopParse));
                out2 = StrReplace(out2, ":", "");
                lineDone = 1;
            ...
            else if (SubStrLastChars(Trim(A_LoopField206), StrLen(keyWordInc)) == keyWordInc || SubStrLastChars(Trim(A_LoopField206), StrLen(keyWordInc + ";")) == keyWordInc + ";" && lineDone == 0) {
                if (useSemicolon == "on" || SubStrLastChars(Trim(A_LoopField206), 1) == ";") {
                    str1 = Trim(StringTrimRight(Trim(A_LoopField206), StrLen(keyWordInc + ";")));
                } else {
                    str1 = Trim(StringTrimRight(Trim(A_LoopField206), StrLen(keyWordInc)));
                }
                str1 = Trim(expressionParserTranspiler(str1));
            ...
            else if (SubStrLastChars(Trim(A_LoopField206), StrLen(keyWordDec)) == keyWordDec || SubStrLastChars(Trim(A_LoopField206), StrLen(keyWordDec + ";")) == keyWordDec + ";" && lineDone == 0) {
                if (useSemicolon == "on" || SubStrLastChars(Trim(A_LoopField206), 1) == ";") {
                    str1 = Trim(StringTrimRight(Trim(A_LoopField206), StrLen(keyWordDec + ";")));
                } else {
                    str1 = Trim(StringTrimRight(Trim(A_LoopField206), StrLen(keyWordDec)));
                }
                str1 = Trim(expressionParserTranspiler(str1));
            ...
            else if (SubStr(StrLower(Trim(A_LoopField206)), 1, StrLen(StrLower(keyWordInclude + " "))) == StrLower(keyWordInclude + " ") && langToConvertTo == langFileExtension_2) {
                htCode += keyWordInclude_2 + " " + Trim(StringTrimLeft(Trim(A_LoopField206), StrLen(StrLower(keyWordInclude) + " "))) + Chr(10);
            }
            else if (StrLower(A_LoopField206) == StrLower(keyWordGlobal) || StrLower(A_LoopField206) == StrLower(keyWordGlobal + ";")) {
                if (langToConvertTo == "ahk") {
                    htCode += "global\n";
                }
            }
            else if (SubStr(StrLower(A_LoopField206), 1, StrLen(StrLower(keyWordAwait))) == StrLower(keyWordAwait)) {
                str1 = StringTrimLeft(A_LoopField206, StrLen(keyWordAwait));
                //MsgBox, % A_LoopField
                lineDone = 1;
            ...
            else if (SubStr(StrLower(A_LoopField206), 1, StrLen(StrLower(keyWordGlobal))) == StrLower(keyWordGlobal)) {
                str1 = StringTrimLeft(A_LoopField206, StrLen(keyWordGlobal));
                //MsgBox, % A_LoopField
            ...
            else if (SubStr(StrLower(A_LoopField206), 1, StrLen(keyWordFunc)) == StrLower(keyWordFunc) || SubStr(StrLower(A_LoopField206), 1, StrLen(StrLower(keyWordAsync) + StrLower(keyWordFunc))) == StrLower(keyWordAsync) + StrLower(keyWordFunc) && InStr(StrLower(A_LoopField206), " main(") == false && lineDone == 0) {
                lineDone = 1;
                if (SubStr(StrLower(A_LoopField206), 1, StrLen(StrLower(keyWordAsync) + StrLower(keyWordFunc))) == StrLower(keyWordAsync) + StrLower(keyWordFunc)) {
                    // is async
                    int1 = 1;
                    str1 = Trim(StringTrimLeft(A_LoopField206, StrLen(StrLower(keyWordAsync + keyWordFunc))));
                } else {
                    // is async
                    int1 = 0;
                    str1 = Trim(StringTrimLeft(A_LoopField206, StrLen(StrLower(keyWordFunc))));
                }
                if (langToConvertTo != langFileExtension_2 && langToConvertTo != "cpp" && langToConvertTo != "go" && langToConvertTo != "cs" && langToConvertTo != "java" && langToConvertTo != "kt" && langToConvertTo != "nim" && langToConvertTo != "swift" && langToConvertTo != "dart" && langToConvertTo != "ts") {
                    if (str1 != "") {
                        str4 = "";
                        std::vector<std::string> items208 = LoopParseFunc(str1, ",");
                        for (size_t A_Index208 = 0; A_Index208 < items208.size() + 0; A_Index208++) {
                            std::string A_LoopField208 = items208[A_Index208 - 0];
                            if (InStr(A_LoopField208, " := ")) {
                                if (InStr(Trim(StrSplit(A_LoopField208, " := ", 1)), " ") != true) {
                                    str4 += "optanal-HTVM-opt-parmsNOtWorking-theirewIs-a-bug-Iusfbudfbuoeshfuisbav=-fwaegs-awedsf-dd-sfgc " + A_LoopField208 + ", ";
                                } else {
                                    str4 += A_LoopField208 + ", ";
                                }
                            }
                            ...
                            else if (InStr(A_LoopField208, " " + Trim(keyWordAssign) + " ")) {
                                if (InStr(Trim(StrSplit(A_LoopField208, " " + Trim(keyWordAssign) + " ", 1)), " ") != true) {
                                    str4 += "optanal-HTVM-opt-parmsNOtWorking-theirewIs-a-bug-Iusfbudfbuoeshfuisbav=-fwaegs-awedsf-dd-sfgc " + A_LoopField208 + ", ";
                                } else {
                                    str4 += A_LoopField208 + ", ";
                                }
                ...
                if (Trim(str1) == "") {
                    ...
                    str19 = "";
                    str20 = "";
                    std::vector<std::string> items209 = LoopParseFunc(str1, ",");
                    for (size_t A_Index209 = 0; A_Index209 < items209.size() + 0; A_Index209++) {
                        ...
                        if (InStr(Trim(A_LoopField209), " ")) {
                            // there is space aka there is a type
                            if (InStr(A_LoopField209, " " + Trim(keyWordAssign) + " ") == false) {
                                if (usePrefixTypeForTypeDefinition == "on") {
                                    // normal
                                    str6 = Trim(A_LoopField209);
                                ...
                                // optanal param
                                if (InStr(Trim(StrSplit(Trim(A_LoopField209), " " + Trim(keyWordAssign) + " ", 1)), " ")) {
                                    // there is a type
                                    if (usePrefixTypeForTypeDefinition == "on") {
                                        // normal
                                        str6 = Trim(StrSplit(Trim(A_LoopField209), " " + Trim(keyWordAssign) + " ", 1));
                                        str9 = Trim(expressionParserTranspiler(Trim(StrSplit(Trim(A_LoopField209), " " + Trim(keyWordAssign) + " ", 2))));
                                        str7 = getTheLastWord(str6);
                                        str8 = getFuncTypeConvert(Trim(StringTrimRight(str6, StrLen(Trim(str7)) + 1)));
                                        //;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;
                                        //;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;
                                    ...
                                    // there is no type
                                    str8 = Trim(StrSplit(Trim(A_LoopField209), " " + Trim(keyWordAssign) + " ", 1));
                                    str9 = Trim(expressionParserTranspiler(Trim(StrSplit(Trim(A_LoopField209), " " + Trim(keyWordAssign) + " ", 2))));
                                    // str10
                                    // skipLeftCuleyForFuncPLS := 1
                                    //;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;
                            ...
                            str5 += Trim(A_LoopField209) + Chr(10);
                        }
                    }
                    //print(str5)
                    //print(str10)
                    ...
                    if (Trim(str5) != "" && Trim(str10) != "") {
                        ...
                        if (langToConvertTo == "lua") {
                            ...
                            for (size_t A_Index212 = 0; A_Index212 < items212.size() + 0; A_Index212++) {
                                ...
                                str14 += Trim(StrSplit(Trim(A_LoopField212), " = ", 1)) + ", ";
                            }
                            ...
                            for (size_t A_Index213 = 0; A_Index213 < items213.size() + 0; A_Index213++) {
                                ...
                                str13 += Trim(StrSplit(Trim(A_LoopField213), " = ", 1)) + " = " + Trim(StrSplit(Trim(A_LoopField213), " = ", 1)) + " or " + Trim(StrSplit(Trim(A_LoopField213), " = ", 2)) + Chr(10);
                            }
                            str12 = str14;
                        }
                        if (langToConvertTo == "go") {
                            ...
                            for (size_t A_Index214 = 0; A_Index214 < items214.size() + 0; A_Index214++) {
                                ...
                                str16 = Trim(StrSplit(Trim(StrSplit(Trim(A_LoopField214), " = ", 1)), " ", 1));
                                str17 = Trim(StrSplit(Trim(A_LoopField214), " = ", 2));
                            ...
                            for (size_t A_Index215 = 0; A_Index215 < items215.size() + 0; A_Index215++) {
                                ...
                                str16 = Trim(StrSplit(Trim(StrSplit(Trim(A_LoopField215), " = ", 1)), " ", 1));
                                str18 = Trim(StrSplit(Trim(StrSplit(Trim(A_LoopField215), " = ", 1)), " ", 2));
                                // str16 = var name
                                // str18 = var type
                                str13 += "if len(__HTVM_V2_TO_GO_optionalParams__) " + Chr(62) + " " + STR(A_Index215) + Chr(10) + "{" + Chr(10) + str16 + " = __HTVM_V2_TO_GO_optionalParams__[" + STR(A_Index215) + "].(" + str18 + ")" + Chr(10) + "}" + Chr(10);
                            }
                            str12 = str14;
                        }
                        if (langToConvertTo == "java") {
                            ...
                            for (size_t A_Index216 = 0; A_Index216 < items216.size() + 0; A_Index216++) {
                                ...
                                str19 = Trim(A_LoopField216);
                                str20 = "";
                                std::vector<std::string> items217 = LoopParseFunc(str19, " ");
                                for (size_t A_Index217 = 0; A_Index217 < items217.size() + 0; A_Index217++) {
                                    std::string A_LoopField217 = items217[A_Index217 - 0];
                                    if (A_Index217 == 3) {
                                        if (RegExMatch(A_LoopField217, "^-?\\d+(\\.\\d+)?$")) {
                                            str20 += A_LoopField217 + "f ";
                                        } else {
                                            str20 += A_LoopField217 + " ";
                                        }
                                    } else {
                                        str20 += A_LoopField217 + " ";
                                    }
                            ...
                            for (size_t A_Index218 = 0; A_Index218 < items218.size() + 0; A_Index218++) {
                                ...
                                str16 = Trim(StrSplit(Trim(StrSplit(Trim(A_LoopField218), " = ", 1)), " ", 2));
                                str17 = Trim(StrSplit(Trim(StrSplit(Trim(A_LoopField218), " = ", 1)), " ", 1));
                                // str16 = var name
                                // str17 = type name
                                str13 += "if (__HTVM_V2_TO_JAVA_optionalParams__.length " + Chr(62) + " " + STR(A_Index218) + ") " + str16 + " = (" + str17 + ") __HTVM_V2_TO_JAVA_optionalParams__[" + STR(A_Index218) + "];" + Chr(10);
                            }
                            str12 = str14;
                        }
                        str11 += str12;
                    ...
                    if (Trim(str5) == "" && Trim(str10) != "") {
                        ...
                        if (langToConvertTo == "lua") {
                            ...
                            for (size_t A_Index221 = 0; A_Index221 < items221.size() + 0; A_Index221++) {
                                ...
                                str14 += Trim(StrSplit(Trim(A_LoopField221), " = ", 1)) + ", ";
                            }
                            str14 = StringTrimRight(str14, 2);
                            str13 += "{" + Chr(10);
                            ...
                            for (size_t A_Index222 = 0; A_Index222 < items222.size() + 0; A_Index222++) {
                                ...
                                str13 += Trim(StrSplit(Trim(A_LoopField222), " = ", 1)) + " = " + Trim(StrSplit(Trim(A_LoopField222), " = ", 1)) + " or " + Trim(StrSplit(Trim(A_LoopField222), " = ", 2)) + Chr(10);
                            }
                            str11 = str14;
                        }
                        if (langToConvertTo == "go") {
                            ...
                            for (size_t A_Index223 = 0; A_Index223 < items223.size() + 0; A_Index223++) {
                                ...
                                str16 = Trim(StrSplit(Trim(StrSplit(Trim(A_LoopField223), " = ", 1)), " ", 1));
                                str17 = Trim(StrSplit(Trim(A_LoopField223), " = ", 2));
                                str13 += str16 + " := " + str17 + Chr(10);
                            }
                            ...
                            for (size_t A_Index224 = 0; A_Index224 < items224.size() + 0; A_Index224++) {
                                ...
                                str16 = Trim(StrSplit(Trim(StrSplit(Trim(A_LoopField224), " = ", 1)), " ", 1));
                                str18 = Trim(StrSplit(Trim(StrSplit(Trim(A_LoopField224), " = ", 1)), " ", 2));
                                // str16 = var name
                                // str18 = var type
                                str13 += "if len(__HTVM_V2_TO_GO_optionalParams__) " + Chr(62) + " " + STR(A_Index224) + Chr(10) + "{" + Chr(10) + str16 + " = __HTVM_V2_TO_GO_optionalParams__[" + STR(A_Index224) + "].(" + str18 + ")" + Chr(10) + "}" + Chr(10);
                            }
                            str11 = str14;
                        }
                        if (langToConvertTo == "java") {
                            ...
                            for (size_t A_Index225 = 0; A_Index225 < items225.size() + 0; A_Index225++) {
                                ...
                                str19 = Trim(A_LoopField225);
                                str20 = "";
                                std::vector<std::string> items226 = LoopParseFunc(str19, " ");
                                for (size_t A_Index226 = 0; A_Index226 < items226.size() + 0; A_Index226++) {
                                    std::string A_LoopField226 = items226[A_Index226 - 0];
                                    if (A_Index226 == 3) {
                                        if (RegExMatch(A_LoopField226, "^-?\\d+(\\.\\d+)?$")) {
                                            str20 += A_LoopField226 + "f ";
                                        } else {
                                            str20 += A_LoopField226 + " ";
                                        }
                                    } else {
                                        str20 += A_LoopField226 + " ";
                                    }
                            ...
                            for (size_t A_Index227 = 0; A_Index227 < items227.size() + 0; A_Index227++) {
                                ...
                                str16 = Trim(StrSplit(Trim(StrSplit(Trim(A_LoopField227), " = ", 1)), " ", 2));
                                str17 = Trim(StrSplit(Trim(StrSplit(Trim(A_LoopField227), " = ", 1)), " ", 1));
                                // str16 = var name
                                // str17 = type name
                                str13 += "if (__HTVM_V2_TO_JAVA_optionalParams__.length " + Chr(62) + " " + STR(A_Index227) + ") " + str16 + " = (" + str17 + ") __HTVM_V2_TO_JAVA_optionalParams__[" + STR(A_Index227) + "];" + Chr(10);
                            }
                            str11 = str14;
                        }
                    }
            ...
            else if (InStr(StrLower(A_LoopField206), " main(") && lineDone == 0) {
                lineDone = 1;
                javaMainFuncSeen = 1;
                csMainFuncSeen = 1;
                htCode += A_LoopField206 + Chr(10);
            }
            else if (InStr(A_LoopField206, " " + keyWordAssign + " ") || InStr(A_LoopField206, " " + keyWordAssignAdd + " ") || InStr(A_LoopField206, " " + keyWordAssignConcat + " ") || InStr(A_LoopField206, " " + keyWordAssignSub + " ") || InStr(A_LoopField206, " " + keyWordAssignMul + " ") || InStr(A_LoopField206, " " + keyWordAssignDiv + " ") || InStr(A_LoopField206, " " + keyWordAssignMod + " ") || InStr(A_LoopField206, " " + keyWordAssignShiftLeft + " ") || InStr(A_LoopField206, " " + keyWordAssignShiftRight + " ") || InStr(A_LoopField206, " " + keyWordLogicalAssignShiftRight + " ") || InStr(A_LoopField206, " " + keyWordAssignBitAnd + " ") || InStr(A_LoopField206, " " + keyWordAssignBitOr + " ") || InStr(A_LoopField206, " " + keyWordAssignBitXor + " ") && lineDone == 0) {
                allVarsSoWeDontReDecVars_FIX_TOGGLE = 1;
                //keyWordAssign
                //keyWordAssignAdd
                //keyWordAssignConcat
                ...
                //allVarsSoWeDontReDecVars_FIX_int64
                //allVarsSoWeDontReDecVars_FIX_float
                if (InStr(A_LoopField206, " " + keyWordAssign + " ")) {
                    str1 = StrSplit(A_LoopField206, " " + keyWordAssign + " ", 1);
                    str6 = StrSplit(A_LoopField206, " " + keyWordAssign + " ", 2);
                    str10 = keyWordAssign;
                    str11 = keyWordAssign_2;
                    if (langToConvertTo == "ahk") {
                        str12 = ":=";
                ...
                else if (InStr(A_LoopField206, " " + keyWordAssignAdd + " ")) {
                    str1 = StrSplit(A_LoopField206, " " + keyWordAssignAdd + " ", 1);
                    str6 = StrSplit(A_LoopField206, " " + keyWordAssignAdd + " ", 2);
                    str10 = keyWordAssignAdd;
                    str11 = keyWordAssignAdd_2;
                    str12 = "+=";
                }
                else if (InStr(A_LoopField206, " " + keyWordAssignConcat + " ")) {
                    str1 = StrSplit(A_LoopField206, " " + keyWordAssignConcat + " ", 1);
                    str6 = StrSplit(A_LoopField206, " " + keyWordAssignConcat + " ", 2);
                    str10 = keyWordAssignConcat;
                    str11 = keyWordAssignConcat_2;
                    if (langToConvertTo == "ahk") {
                        str12 = ".=";
                ...
                else if (InStr(A_LoopField206, " " + keyWordAssignSub + " ")) {
                    str1 = StrSplit(A_LoopField206, " " + keyWordAssignSub + " ", 1);
                    str6 = StrSplit(A_LoopField206, " " + keyWordAssignSub + " ", 2);
                    str10 = keyWordAssignSub;
                    str11 = keyWordAssignSub_2;
                    str12 = "-=";
                }
                else if (InStr(A_LoopField206, " " + keyWordAssignMul + " ")) {
                    str1 = StrSplit(A_LoopField206, " " + keyWordAssignMul + " ", 1);
                    str6 = StrSplit(A_LoopField206, " " + keyWordAssignMul + " ", 2);
                    str10 = keyWordAssignMul;
                    str11 = keyWordAssignMul_2;
                    str12 = "*=";
                }
                else if (InStr(A_LoopField206, " " + keyWordAssignDiv + " ")) {
                    str1 = StrSplit(A_LoopField206, " " + keyWordAssignDiv + " ", 1);
                    str6 = StrSplit(A_LoopField206, " " + keyWordAssignDiv + " ", 2);
                    str10 = keyWordAssignDiv;
                    str11 = keyWordAssignDiv_2;
                    str12 = "/=";
                }
                else if (InStr(A_LoopField206, " " + keyWordAssignMod + " ")) {
                    str1 = StrSplit(A_LoopField206, " " + keyWordAssignMod + " ", 1);
                    str6 = StrSplit(A_LoopField206, " " + keyWordAssignMod + " ", 2);
                    str10 = keyWordAssignMod;
                    str11 = keyWordAssignMod_2;
                    str12 = "%=";
                }
                else if (InStr(A_LoopField206, " " + keyWordAssignShiftLeft + " ")) {
                    str1 = StrSplit(A_LoopField206, " " + keyWordAssignShiftLeft + " ", 1);
                    str6 = StrSplit(A_LoopField206, " " + keyWordAssignShiftLeft + " ", 2);
                    str10 = keyWordAssignShiftLeft;
                    str11 = keyWordAssignShiftLeft_2;
                    str12 = "<<=";
                }
                else if (InStr(A_LoopField206, " " + keyWordAssignShiftRight + " ")) {
                    str1 = StrSplit(A_LoopField206, " " + keyWordAssignShiftRight + " ", 1);
                    str6 = StrSplit(A_LoopField206, " " + keyWordAssignShiftRight + " ", 2);
                    str10 = keyWordAssignShiftRight;
                    str11 = keyWordAssignShiftRight_2;
                    str12 = ">>=";
                }
                else if (InStr(A_LoopField206, " " + keyWordLogicalAssignShiftRight + " ")) {
                    str1 = StrSplit(A_LoopField206, " " + keyWordLogicalAssignShiftRight + " ", 1);
                    str6 = StrSplit(A_LoopField206, " " + keyWordLogicalAssignShiftRight + " ", 2);
                    str10 = keyWordLogicalAssignShiftRight;
                    str11 = keyWordLogicalAssignShiftRight_2;
                    str12 = ">>>=";
                }
                else if (InStr(A_LoopField206, " " + keyWordAssignBitAnd + " ")) {
                    str1 = StrSplit(A_LoopField206, " " + keyWordAssignBitAnd + " ", 1);
                    str6 = StrSplit(A_LoopField206, " " + keyWordAssignBitAnd + " ", 2);
                    str10 = keyWordAssignBitAnd;
                    str11 = keyWordAssignBitAnd_2;
                    str12 = "&=";
                }
                else if (InStr(A_LoopField206, " " + keyWordAssignBitOr + " ")) {
                    str1 = StrSplit(A_LoopField206, " " + keyWordAssignBitOr + " ", 1);
                    str6 = StrSplit(A_LoopField206, " " + keyWordAssignBitOr + " ", 2);
                    str10 = keyWordAssignBitOr;
                    str11 = keyWordAssignBitOr_2;
                    str12 = "|=";
                }
                else if (InStr(A_LoopField206, " " + keyWordAssignBitXor + " ")) {
                    str1 = StrSplit(A_LoopField206, " " + keyWordAssignBitXor + " ", 1);
                    str6 = StrSplit(A_LoopField206, " " + keyWordAssignBitXor + " ", 2);
                    str10 = keyWordAssignBitXor;
                    str11 = keyWordAssignBitXor_2;
                    str12 = "^=";
                }
            ...
            else if (SubStr(Trim(A_LoopField206), 1, StrLen(keyWordVar)) == keyWordVar && lineDone == 0) {
                allVarsSoWeDontReDecVars_FIX_TOGGLE = 1;
                if (usePostfixTypeForTypeDefinition == "off") {
                    lineDone = 1;
                    str1 = Trim(StringTrimLeft(Trim(A_LoopField206), StrLen(keyWordVar)));
                    if (SubStrLastChars(str1, 1) == ";") {
                        str1 = StringTrimRight(str1, 1);
                    }
            ...
            else if (SubStr(Trim(A_LoopField206), 1, StrLen(keyWordLet)) == keyWordLet && lineDone == 0) {
                allVarsSoWeDontReDecVars_FIX_TOGGLE = 1;
                if (usePostfixTypeForTypeDefinition == "off") {
                    lineDone = 1;
                    str1 = Trim(StringTrimLeft(Trim(A_LoopField206), StrLen(keyWordLet)));
                    if (SubStrLastChars(str1, 1) == ";") {
                        str1 = StringTrimRight(str1, 1);
                    }
            ...
            else if (SubStr(Trim(A_LoopField206), 1, StrLen(keyWordDouble + " ")) == keyWordDouble + " " || SubStr(Trim(A_LoopField206), 1, StrLen(keyWordChar + " ")) == keyWordChar + " " || SubStr(Trim(A_LoopField206), 1, StrLen(keyWordUint8 + " ")) == keyWordUint8 + " " || SubStr(Trim(A_LoopField206), 1, StrLen(keyWordUint16 + " ")) == keyWordUint16 + " " || SubStr(Trim(A_LoopField206), 1, StrLen(keyWordUint32 + " ")) == keyWordUint32 + " " || SubStr(Trim(A_LoopField206), 1, StrLen(keyWordUint64 + " ")) == keyWordUint64 + " " || SubStr(Trim(A_LoopField206), 1, StrLen(keyWordINT + " ")) == keyWordINT + " " || SubStr(Trim(A_LoopField206), 1, StrLen(keyWordSTR + " ")) == keyWordSTR + " " || SubStr(Trim(A_LoopField206), 1, StrLen(keyWordBOOL + " ")) == keyWordBOOL + " " || SubStr(Trim(A_LoopField206), 1, StrLen(keyWordFLOAT + " ")) == keyWordFLOAT + " " || SubStr(Trim(A_LoopField206), 1, StrLen(keyWordINT8 + " ")) == keyWordINT8 + " " || SubStr(Trim(A_LoopField206), 1, StrLen(keyWordINT16 + " ")) == keyWordINT16 + " " || SubStr(Trim(A_LoopField206), 1, StrLen(keyWordINT32 + " ")) == keyWordINT32 + " " || SubStr(Trim(A_LoopField206), 1, StrLen(keyWordINT64 + " ")) == keyWordINT64 + " " || SubStr(Trim(A_LoopField206), 1, StrLen(keyWordArrayDefinition + " ")) == keyWordArrayDefinition + " " || SubStr(Trim(A_LoopField206), 1, StrLen(keyWordArrayOfIntegersDefinition + " ")) == keyWordArrayOfIntegersDefinition + " " || SubStr(Trim(A_LoopField206), 1, StrLen(keyWordArrayOfStringsDefinition + " ")) == keyWordArrayOfStringsDefinition + " " || SubStr(Trim(A_LoopField206), 1, StrLen(keyWordArrayOfFloatingPointNumbersDefinition + " ")) == keyWordArrayOfFloatingPointNumbersDefinition + " " || SubStr(Trim(A_LoopField206), 1, StrLen(keyWordArrayOfBooleansDefinition + " ")) == keyWordArrayOfBooleansDefinition + " " && usePrefixTypeForTypeDefinition == "on" && lineDone == 0) {
                lineDone = 1;
                allVarsSoWeDontReDecVars_FIX_TOGGLE = 1;
                str1 = Trim(A_LoopField206);
                if (SubStrLastChars(str1, 1) == ";") {
            ...
            else if (RegExMatch(Trim(A_LoopField206), "^[a-zA-Z_][a-zA-Z0-9_]*: [^:]*;?$") && usePostfixTypeForTypeDefinition == "on" && lineDone == 0) {
                lineDone = 1;
                allVarsSoWeDontReDecVars_FIX_TOGGLE = 1;
                str1 = Trim(A_LoopField206);
                if (SubStrLastChars(str1, 1) == ";") {
            ...
            else if (RegExMatch(Trim(A_LoopField206), "^[a-zA-Z0-9_\\.]+\\(") && !(InStr(StrLower(A_LoopField206), " main(")) && lineDone == 0) {
                lineDone = 1;
                str1 = Trim(A_LoopField206);
                if (SubStrLastChars(str1, 1) == ";") {
                    str1 = StringTrimRight(str1, 1);
            ...
            else if (RegExMatch(A_LoopField206, "\\.[a-zA-Z_][a-zA-Z0-9_]*") && lineDone == 0) {
                lineDone = 1;
                str1 = Trim(A_LoopField206);
                if (SubStrLastChars(str1, 1) == ";") {
                    str1 = StringTrimRight(str1, 1);
            ...
            else if (KeyWordsCommands(A_LoopField206, "check", commands, langToConvertTo) == "true" && lineDone == 0) {
                lineDone = 1;
                if (langToConvertTo == langFileExtension_2) {
                    htCode += A_LoopField206 + Chr(10);
                } else {
                    out_KeyWordsCommands = KeyWordsCommands(A_LoopField206, "transpile", commands, langToConvertTo);
                ...
                if (lineDone != 1) {
                    if (skipLeftCuleyForFuncPLS != 1) {
                        if (SubStr(Trim(StrLower(A_LoopField206)), 1, 1) == Chr(125)) {
                            htCode += Chr(125) + Chr(10);
                        } else {
                            if (htCodeAcurlyBraceAddSomeVrasFixLP == 1 && SubStr(Trim(StrLower(A_LoopField206)), 1, 1) == Chr(123)) {
                                htCodeAcurlyBraceAddSomeVrasFixLP = 0;
                                htCode += A_LoopField206 + Chr(10) + theFixTextLoopLP + Chr(10);
                            } else {
                                if (htCodeAcurlyBraceAddSomeVrasFixNL == 1 && SubStr(Trim(StrLower(A_LoopField206)), 1, 1) == Chr(123)) {
                                    htCodeAcurlyBraceAddSomeVrasFixNL = 0;
                                    htCode += A_LoopField206 + Chr(10) + theFixTextLoopNL + Chr(10);
                                } else {
                                    htCode += A_LoopField206 + Chr(10);
                                }
                            }
        ...
        if (langToConvertTo != "ahk" && langToConvertTo != langFileExtension_2) {
            ...
            if (haveWeEverUsedAloop == 1) {
                ...
                std::vector<std::string> items235 = LoopParseFunc(htCodeLoopfixa, "\n", "\r");
                for (size_t A_Index235 = 0; A_Index235 < items235.size() + 0; A_Index235++) {
                    std::string A_LoopField235 = items235[A_Index235 - 0];
                    sstr123 = A_LoopField235;
                    fixLoopLokingFor = A_LoopField235;
                    ...
                    if (out1 == "nl") {
                        ...
                        for (size_t A_Index236 = 0; A_Index236 < items236.size() + 0; A_Index236++) {
                            ...
                            if (InStr(A_LoopField236, fixLoopLokingFor) && insdeAnestedLoopBAD != 1) {
                                fixLoopLokingForNum = 1;
                                //MsgBox, do we came here 1
                            }
                            if (SubStr(Trim(A_LoopField236), 1, 4) == "for " && weAreDoneHereCurly != 1 && insdeAnestedLoopBAD != 1 && fixLoopLokingForNum == 1) {
                                s = StrSplit(A_LoopField236 ,"" + keyWordAIndex + "", 2);
                                out1z = s;
                                s = StrSplit(out1z, " ", 1);
                                out1z = Trim(s);
                                //MsgBox, % out1z
                                ...
                                //MsgBox, % A_LoopField
                                dontSaveStr = 1;
                                ALoopField = A_LoopField236;
                                DeleayOneCuzOfLoopParse = 1;
                                out4758686d86d86d86578991a += ALoopField + Chr(10);
                            }
                            if (inTarget == 1 && InStr(A_LoopField236, Chr(123)) && insdeAnestedLoopBAD != 1) {
                                insideBracket = 1;
                            }
                            if (insideBracket == 1 && InStr(A_LoopField236, Chr(123)) && insdeAnestedLoopBAD != 1) {
                                netsedCurly++;
                            }
                            if (insideBracket == 1 && InStr(A_LoopField236, Chr(125)) && insdeAnestedLoopBAD != 1) {
                                netsedCurly--;
                                readyToEnd = 1;
                            }
                            if (SubStr(Trim(A_LoopField236), 1, 4) == "for " && insdeAnestedLoopBAD != 1 && foundTheTopLoop >= 2) {
                            ...
                            if (insdeAnestedLoopBAD == 1) {
                                if (InStr(A_LoopField236, Chr(123))) {
                                    insideBracket1 = 1;
                                }
                                if (insideBracket1 == 1 && InStr(A_LoopField236, Chr(123))) {
                                    netsedCurly1++;
                                }
                                if (insideBracket1 == 1 && InStr(A_LoopField236, Chr(125))) {
                                    netsedCurly1--;
                                    readyToEnd1 = 1;
                                }
                                if (InStr(A_LoopField236, Chr(125)) && readyToEnd1 == 1 && netsedCurly1 == 0 && insideBracket == 1) {
                                    //MsgBox, % A_LoopField
                                    eldLoopNestedBADlol = 1;
                                }
                                out4758686d86d86d86578991a += A_LoopField236 + Chr(10);
                            }
                            ...
                            if (inTarget == 1 && InStr(A_LoopField236, Chr(125)) && readyToEnd == 1 && netsedCurly == 0 && weAreDoneHereCurly == 0 && dontSaveStr != 1 && insdeAnestedLoopBAD != 1) {
                                //MsgBox, % A_LoopField
                        ...
                        for (size_t A_Index237 = 0; A_Index237 < items237.size() + 0; A_Index237++) {
                            ...
                            if (InStr(A_LoopField237 , fixLoopLokingFor) && insdeAnestedLoopBAD != 1) {
                                fixLoopLokingForNum = 1;
                                //MsgBox, do we came here 3
                            }
                            if (SubStr(Trim(A_LoopField237), 1, 4) == "for " && weAreDoneHereCurly != 1 && insdeAnestedLoopBAD != 1 && fixLoopLokingForNum == 1) {
                                s = StrSplit(A_LoopField237 ,"" + keyWordAIndex + "", 2);
                                out1z = s;
                                s = StrSplit(out1z, " ", 1);
                                ...
                                //MsgBox, % A_LoopField
                                dontSaveStr = 1;
                                ALoopField = A_LoopField237;
                                DeleayOneCuzOfLoopParse = 1;
                                out4758686d86d86d86578991a += ALoopField + Chr(10);
                            }
                            if (inTarget == 1 && InStr(A_LoopField237, Chr(123)) && insdeAnestedLoopBAD != 1) {
                                insideBracket = 1;
                            }
                            if (insideBracket == 1 && InStr(A_LoopField237, Chr(123)) && insdeAnestedLoopBAD != 1) {
                                netsedCurly++;
                            }
                            if (insideBracket == 1 && InStr(A_LoopField237, Chr(125)) && insdeAnestedLoopBAD != 1) {
                                netsedCurly--;
                                readyToEnd = 1;
                            }
                            if (SubStr(Trim(A_LoopField237), 1, 4) == "for " && insdeAnestedLoopBAD != 1 && foundTheTopLoop >= 2) {
                                insdeAnestedLoopBAD = 1;
                                insideBracket1 = 0;
                                netsedCurly1 = 0;
                            }
                            ...
                            if (insdeAnestedLoopBAD == 1) {
                                if (InStr(A_LoopField237, Chr(123))) {
                                    insideBracket1 = 1;
                                }
                                if (insideBracket1 == 1 && InStr(A_LoopField237, Chr(123))) {
                                    netsedCurly1++;
                                }
                                if (insideBracket1 == 1 && InStr(A_LoopField237, Chr(125))) {
                                    netsedCurly1--;
                                    readyToEnd1 = 1;
                                }
                                if (InStr(A_LoopField237, Chr(125)) && readyToEnd1 == 1 && netsedCurly1 == 0 && insideBracket == 1) {
                                    //MsgBox, % A_LoopField
                                    eldLoopNestedBADlol = 1;
                                }
                                out4758686d86d86d86578991a += A_LoopField237 + Chr(10);
                            }
                            if (inTarget == 1 && dontSaveStr != 1 && fixLoopLokingForNum != 1 && insdeAnestedLoopBAD != 1) {
                                ALoopField = A_LoopField237;
                                // Replace "A_Index" with or without a following digit with "A_Index" + out1z
                            ...
                            if ((inTarget == 1 && InStr(A_LoopField237, Chr(125)) && readyToEnd == 1 && netsedCurly == 0 && weAreDoneHereCurly == 0 && dontSaveStr != 1 && insdeAnestedLoopBAD != 1)) {
                                //MsgBox, % A_LoopField
                                weAreDoneHereCurly = 1;
                                inTarget = 0;
                                endBracketDOntPutThere = 1;
                ...
                for (size_t A_Index238 = 0; A_Index238 < items238.size() + 0; A_Index238++) {
                    ...
                    ignore = 0;
                    if (SubStr(Trim(A_LoopField238), 1, 4) == "for ") {
                        if (hold == 1 && holdText == A_LoopField238) {
                            ignore = 1;
                        } else {
                            holdText = A_LoopField238;
                            hold = 1;
                        }
                    }
                    if (!ignore) {
                        out4758686d86dgt8r754444444 += A_LoopField238 + Chr(10);
                    }
                }
    ...
    if (COUNT_programmingBlock_HTVMsyntax != 0) {
        ...
        for (int A_Index240 = 0; A_Index240 < COUNT_programmingBlock_HTVMsyntax + 0; A_Index240++) {
            ADD_ALL_programmingBlock_HTVMsyntax += programmingBlock_HTVMsyntax[A_Index240] + Chr(10);
        }
        ...
        isNotHTVMfile2 = 1;
        for (int A_Index241 = 0; A_Index241 < COUNT_programmingBlock_HTVMsyntax + 0; A_Index241++) {
            htCode = StrReplace(htCode, "programmingBlock_HTVMsyntax-programmingBlock_HTVMsyntax-AA" + STR(A_Index241 + 1) + "AA", compiler(programmingBlock_HTVMsyntax[A_Index241], allInstructionFile, "full", langToConvertToParam));
        }
        isNotHTVMfile2 = 0;
    }
    //;;;;;;;;;;;;;;;;;;;;;;;;;;;
    ...
    if (langToConvertTo == langFileExtension_2) {
        ...
        if (useEnd_2 == "on") {
            ...
            for (size_t A_Index242 = 0; A_Index242 < items242.size() + 0; A_Index242++) {
                ...
                if (Trim(A_LoopField242) == keyWordCurlyBraceClose_2) {
                    fixLuaAndRuby += StrReplace(A_LoopField242, keyWordCurlyBraceClose_2, Trim(keyWordEnd_2)) + Chr(10);
                }
                else if (SubStrLastChars(A_LoopField242, 2) == " " + keyWordCurlyBraceOpen_2) {
                    fixLuaAndRuby += StringTrimRight(A_LoopField242, 2) + Chr(10);
                } else {
                    fixLuaAndRuby += A_LoopField242 + Chr(10);
                }
            }
        ...
        if (langToConvertTo == "lua" || langToConvertTo == "rb" || langToConvertTo == langFileExtension_2) {
            if (langToConvertTo == langFileExtension_2 && useEnd_2 == "on") {
                ...
                for (size_t A_Index243 = 0; A_Index243 < items243.size() + 0; A_Index243++) {
                    ...
                    if (Trim(A_LoopField243) == "}") {
                        fixLuaAndRuby += StrReplace(A_LoopField243, "}", fixLuaAndRubyHELP) + Chr(10);
                    }
                    else if (SubStrLastChars(A_LoopField243, 2) == " {") {
                        fixLuaAndRuby += StringTrimRight(A_LoopField243, 2) + Chr(10);
                    } else {
                        fixLuaAndRuby += A_LoopField243 + Chr(10);
                    }
                }
                htCode = StringTrimRight(fixLuaAndRuby, 1);
            } else {
                ...
                for (size_t A_Index244 = 0; A_Index244 < items244.size() + 0; A_Index244++) {
                    ...
                    if (Trim(A_LoopField244) == "}") {
                        fixLuaAndRuby += StrReplace(A_LoopField244, "}", fixLuaAndRubyHELP) + Chr(10);
                    }
                    else if (SubStrLastChars(A_LoopField244, 2) == " {") {
    ...
    if (langToConvertTo == langFileExtension_2 && useEnd_2 == "on") {
        ...
        for (size_t A_Index245 = 0; A_Index245 < items245.size() + 0; A_Index245++) {
            ...
            HTVM_Append(nextWordEndFix, A_LoopField245);
        }
        ...
        for (size_t A_Index246 = 0; A_Index246 < items246.size() + 0; A_Index246++) {
            ...
            if (Trim(A_LoopField246) == Trim(keyWordEnd_2) && SubStr(Trim(nextWordEndFix[A_Index246 + 1]), 1, StrLen(Trim(keyWordElseIf_2) + " ")) == Trim(keyWordElseIf_2) + " " || Trim(A_LoopField246) == Trim(keyWordEnd_2) && (Trim(nextWordEndFix[A_Index246 + 1]) == Trim(keyWordElse_2) || Trim(nextWordEndFix[A_Index246 + 1]) == Trim(keyWordElse_2 + ":"))) {
                nextWordEndFixOut += "";
            } else {
    ...
    if (langToConvertTo == "lua") {
        ...
        for (size_t A_Index248 = 0; A_Index248 < items248.size() + 0; A_Index248++) {
            ...
            if (Trim(A_LoopField248) == "end" && SubStr(Trim(nextWordEndFix[A_Index248 + 1]), 1, StrLen("elseif ")) == "elseif " || Trim(A_LoopField248) == "end" && Trim(nextWordEndFix[A_Index248 + 1]) == "else") {
                nextWordEndFixOut += "";
            } else {
    ...
    if (langToConvertTo == "rb") {
        ...
        for (size_t A_Index250 = 0; A_Index250 < items250.size() + 0; A_Index250++) {
            ...
            if (Trim(A_LoopField250) == "end" && SubStr(Trim(nextWordEndFix[A_Index250 + 1]), 1, StrLen("elsif ")) == "elsif " || Trim(A_LoopField250) == "end" && Trim(nextWordEndFix[A_Index250 + 1]) == "else") {
                nextWordEndFixOut += "";
            } else {
    ...
    if (langToConvertTo == "rb") {
        ...
        for (size_t A_Index251 = 0; A_Index251 < items251.size() + 0; A_Index251++) {
            ...
            HTVM_Append(nextWordEndFixRB, A_LoopField251);
        }
        ...
        for (size_t A_Index252 = 0; A_Index252 < items252.size() + 0; A_Index252++) {
            ...
            if (Trim(A_LoopField252) == "end" && SubStr(Trim(nextWordEndFixRB[A_Index252 + 1]), 1, StrLen("rescue ")) == "rescue " || Trim(A_LoopField252) == "end" && Trim(nextWordEndFixRB[A_Index252 + 1]) == "ensure") {
                nextWordEndFixOut += "";
            } else {
    ...
    if (langToConvertTo == langFileExtension_2 && useEnd_2 == "on") {
        ...
        for (size_t A_Index254 = 0; A_Index254 < items254.size() + 0; A_Index254++) {
            ...
            if (Trim(A_LoopField254) == Trim(keyWordEnd_2) && SubStr(Trim(nextWordEndFixRB[A_Index254 + 1]), 1, StrLen(Trim(keyWordCatch_2) + " ")) == Trim(keyWordCatch_2) + " " || Trim(A_LoopField254) == Trim(keyWordEnd_2) && Trim(nextWordEndFixRB[A_Index254 + 1]) == Trim(keyWordFinally_2) || Trim(nextWordEndFixRB[A_Index254 + 1]) == Trim(keyWordFinally_2 + ":")) {
                nextWordEndFixOut += "";
            } else {
    ...
    if (langToConvertTo == "ahk") {
        ...
        for (size_t A_Index255 = 0; A_Index255 < items255.size() + 0; A_Index255++) {
            ...
            if (SubStr(Trim(A_LoopField255), 1, StrLen("Loop, Parse, ")) == "Loop, Parse, ") {
                nextWordEndFixOut += StrReplace(A_LoopField255, "{", Chr(10) + "{" + Chr(10)) + Chr(10);
            } else {
                nextWordEndFixOut += A_LoopField255 + Chr(10);
            }
        }
        htCode = StringTrimRight(nextWordEndFixOut, 1);
    }
    if (langToConvertTo == "go") {
        ...
        for (size_t A_Index256 = 0; A_Index256 < items256.size() + 0; A_Index256++) {
            ...
            HTVM_Append(fixGoManGoIsSoAnnoyingBroooFurure, A_LoopField256);
        }
        ...
        for (size_t A_Index257 = 0; A_Index257 < items257.size() + 0; A_Index257++) {
            ...
            if (Trim(A_LoopField257) == "}" && SubStr(Trim(fixGoManGoIsSoAnnoyingBroooFurure[A_Index257 + 1]), 1, StrLen("else if ")) == "else if ") {
                fixGoManGoIsSoAnnoyingBroooSkip = 1;
                fixGoManGoIsSoAnnoyingBroooCount = 0;
                std::vector<std::string> items258 = LoopParseFunc(fixGoManGoIsSoAnnoyingBroooFurure[A_Index257 + 1]);
                ...
                for (int A_Index259 = 0; A_Index259 < fixGoManGoIsSoAnnoyingBroooCount + 0; A_Index259++) {
                    fixGoManGoIsSoAnnoyingBroooCountSpaceses += " ";
                }
                fixGoManGoIsSoAnnoyingBrooo += fixGoManGoIsSoAnnoyingBroooCountSpaceses + "} " + Trim(fixGoManGoIsSoAnnoyingBroooFurure[A_Index257 + 1]) + Chr(10);
            } else {
                if (fixGoManGoIsSoAnnoyingBroooSkip == 0) {
                    fixGoManGoIsSoAnnoyingBrooo += A_LoopField257 + Chr(10);
                }
    ...
    for (int A_Index260 = 0; A_Index260 < HTVM_Size(commentsBugFix) + 0; A_Index260++) {
        htCode = StrReplace(htCode, "HTVM--cnavisdofbuvsesdivdidufg79wregwewaeosd8ges7dfdftuawdiHTVMv2yerheyziberlasduci6syiq--AA" + STR(A_Index260) + "AA", commentsBugFix[A_Index260]);
    }
    //;;;;;;;;;;;;;;;;;;;;;;;;; fix here
    //;;;;;;;;;;;;;;;;;;;;;;;;; fix here
    //;;;;;;;;;;;;;;;;;;;;;;;;; fix here
    if (COUNT_programmingBlock_InTheTranspiledLang != 0) {
        for (int A_Index261 = 0; A_Index261 < COUNT_programmingBlock_InTheTranspiledLang + 0; A_Index261++) {
            htCode = StrReplace(htCode, "programmingBlock_InTheTranspiledLang-programmingBlock_InTheTranspiledLang-AA" + STR(A_Index261 + 1) + "AA", programmingBlock_InTheTranspiledLang[A_Index261]);
        }
    }
    if (langToConvertTo == "cpp") {
        if (COUNT_programmingBlock_CPP != 0) {
            for (int A_Index262 = 0; A_Index262 < COUNT_programmingBlock_CPP + 0; A_Index262++) {
                htCode = StrReplace(htCode, "programmingBlock_CPP-programmingBlock_CPP-AA" + STR(A_Index262 + 1) + "AA", programmingBlock_CPP[A_Index262]);
            }
        }
    } else {
        if (COUNT_programmingBlock_CPP != 0) {
            for (int A_Index263 = 0; A_Index263 < COUNT_programmingBlock_CPP + 0; A_Index263++) {
                if (langToConvertTo == langFileExtension_2) {
                    htCode = StrReplace(htCode, "programmingBlock_CPP-programmingBlock_CPP-AA" + STR(A_Index263 + 1) + "AA", keyWordCodeInTheTranspiledLangStartCPP_2 + Chr(10) + programmingBlock_CPP[A_Index263] + Chr(10) + keyWordCodeInTheTranspiledLangEndCPP_2);
                } else {
                    htCode = StrReplace(htCode, "programmingBlock_CPP-programmingBlock_CPP-AA" + STR(A_Index263 + 1) + "AA", Chr(10));
                }
            }
        }
    }
    if (langToConvertTo == "py") {
        if (COUNT_programmingBlock_PY != 0) {
            for (int A_Index264 = 0; A_Index264 < COUNT_programmingBlock_PY + 0; A_Index264++) {
                htCode = StrReplace(htCode, "programmingBlock_PY-programmingBlock_PY-AA" + STR(A_Index264 + 1) + "AA", programmingBlock_PY[A_Index264]);
            }
        }
    } else {
        if (COUNT_programmingBlock_PY != 0) {
            for (int A_Index265 = 0; A_Index265 < COUNT_programmingBlock_PY + 0; A_Index265++) {
                if (langToConvertTo == langFileExtension_2) {
                    htCode = StrReplace(htCode, "programmingBlock_PY-programmingBlock_PY-AA" + STR(A_Index265 + 1) + "AA", keyWordCodeInTheTranspiledLangStartPY_2 + Chr(10) + programmingBlock_PY[A_Index265] + Chr(10) + keyWordCodeInTheTranspiledLangEndPY_2);
                } else {
                    htCode = StrReplace(htCode, "programmingBlock_PY-programmingBlock_PY-AA" + STR(A_Index265 + 1) + "AA", Chr(10));
                }
            }
        }
    }
    if (langToConvertTo == "js") {
        if (COUNT_programmingBlock_JS != 0) {
            for (int A_Index266 = 0; A_Index266 < COUNT_programmingBlock_JS + 0; A_Index266++) {
                htCode = StrReplace(htCode, "programmingBlock_JS-programmingBlock_JS-AA" + STR(A_Index266 + 1) + "AA", programmingBlock_JS[A_Index266]);
            }
        }
    } else {
        if (COUNT_programmingBlock_JS != 0) {
            for (int A_Index267 = 0; A_Index267 < COUNT_programmingBlock_JS + 0; A_Index267++) {
                if (langToConvertTo == langFileExtension_2) {
                    htCode = StrReplace(htCode, "programmingBlock_JS-programmingBlock_JS-AA" + STR(A_Index267 + 1) + "AA", keyWordCodeInTheTranspiledLangStartJS_2 + Chr(10) + programmingBlock_JS[A_Index267] + Chr(10) + keyWordCodeInTheTranspiledLangEndJS_2);
                } else {
                    htCode = StrReplace(htCode, "programmingBlock_JS-programmingBlock_JS-AA" + STR(A_Index267 + 1) + "AA", Chr(10));
                }
            }
        }
    }
    if (langToConvertTo == "go") {
        if (COUNT_programmingBlock_GO != 0) {
            for (int A_Index268 = 0; A_Index268 < COUNT_programmingBlock_GO + 0; A_Index268++) {
                htCode = StrReplace(htCode, "programmingBlock_GO-programmingBlock_GO-AA" + STR(A_Index268 + 1) + "AA", programmingBlock_GO[A_Index268]);
            }
        }
    } else {
        if (COUNT_programmingBlock_GO != 0) {
            for (int A_Index269 = 0; A_Index269 < COUNT_programmingBlock_GO + 0; A_Index269++) {
                if (langToConvertTo == langFileExtension_2) {
                    htCode = StrReplace(htCode, "programmingBlock_GO-programmingBlock_GO-AA" + STR(A_Index269 + 1) + "AA", keyWordCodeInTheTranspiledLangStartGO_2 + Chr(10) + programmingBlock_GO[A_Index269] + Chr(10) + keyWordCodeInTheTranspiledLangEndGO_2);
                } else {
                    htCode = StrReplace(htCode, "programmingBlock_GO-programmingBlock_GO-AA" + STR(A_Index269 + 1) + "AA", Chr(10));
                }
            }
        }
    }
    if (langToConvertTo == "lua") {
        if (COUNT_programmingBlock_LUA != 0) {
            for (int A_Index270 = 0; A_Index270 < COUNT_programmingBlock_LUA + 0; A_Index270++) {
                htCode = StrReplace(htCode, "programmingBlock_LUA-programmingBlock_LUA-AA" + STR(A_Index270 + 1) + "AA", programmingBlock_LUA[A_Index270]);
            }
        }
    } else {
        if (COUNT_programmingBlock_LUA != 0) {
            for (int A_Index271 = 0; A_Index271 < COUNT_programmingBlock_LUA + 0; A_Index271++) {
                if (langToConvertTo == langFileExtension_2) {
                    htCode = StrReplace(htCode, "programmingBlock_LUA-programmingBlock_LUA-AA" + STR(A_Index271 + 1) + "AA", keyWordCodeInTheTranspiledLangStartLUA_2 + Chr(10) + programmingBlock_LUA[A_Index271] + Chr(10) + keyWordCodeInTheTranspiledLangEndLUA_2);
                } else {
                    htCode = StrReplace(htCode, "programmingBlock_LUA-programmingBlock_LUA-AA" + STR(A_Index271 + 1) + "AA", Chr(10));
                }
            }
        }
    }
    if (langToConvertTo == "cs") {
        if (COUNT_programmingBlock_CS != 0) {
            for (int A_Index272 = 0; A_Index272 < COUNT_programmingBlock_CS + 0; A_Index272++) {
                htCode = StrReplace(htCode, "programmingBlock_CS-programmingBlock_CS-AA" + STR(A_Index272 + 1) + "AA", programmingBlock_CS[A_Index272]);
            }
        }
    } else {
        if (COUNT_programmingBlock_CS != 0) {
            for (int A_Index273 = 0; A_Index273 < COUNT_programmingBlock_CS + 0; A_Index273++) {
                if (langToConvertTo == langFileExtension_2) {
                    htCode = StrReplace(htCode, "programmingBlock_CS-programmingBlock_CS-AA" + STR(A_Index273 + 1) + "AA", keyWordCodeInTheTranspiledLangStartCS_2 + Chr(10) + programmingBlock_CS[A_Index273] + Chr(10) + keyWordCodeInTheTranspiledLangEndCS_2);
                } else {
                    htCode = StrReplace(htCode, "programmingBlock_CS-programmingBlock_CS-AA" + STR(A_Index273 + 1) + "AA", Chr(10));
                }
            }
        }
    }
    if (langToConvertTo == "java") {
        if (COUNT_programmingBlock_JAVA != 0) {
            for (int A_Index274 = 0; A_Index274 < COUNT_programmingBlock_JAVA + 0; A_Index274++) {
                htCode = StrReplace(htCode, "programmingBlock_JAVA-programmingBlock_JAVA-AA" + STR(A_Index274 + 1) + "AA", programmingBlock_JAVA[A_Index274]);
            }
        }
    } else {
        if (COUNT_programmingBlock_JAVA != 0) {
            for (int A_Index275 = 0; A_Index275 < COUNT_programmingBlock_JAVA + 0; A_Index275++) {
                if (langToConvertTo == langFileExtension_2) {
                    htCode = StrReplace(htCode, "programmingBlock_JAVA-programmingBlock_JAVA-AA" + STR(A_Index275 + 1) + "AA", keyWordCodeInTheTranspiledLangStartJAVA_2 + Chr(10) + programmingBlock_JAVA[A_Index275] + Chr(10) + keyWordCodeInTheTranspiledLangEndJAVA_2);
                } else {
                    htCode = StrReplace(htCode, "programmingBlock_JAVA-programmingBlock_JAVA-AA" + STR(A_Index275 + 1) + "AA", Chr(10));
                }
            }
        }
    }
    if (langToConvertTo == "kt") {
        if (COUNT_programmingBlock_KT != 0) {
            for (int A_Index276 = 0; A_Index276 < COUNT_programmingBlock_KT + 0; A_Index276++) {
                htCode = StrReplace(htCode, "programmingBlock_KT-programmingBlock_KT-AA" + STR(A_Index276 + 1) + "AA", programmingBlock_KT[A_Index276]);
            }
        }
    } else {
        if (COUNT_programmingBlock_KT != 0) {
            for (int A_Index277 = 0; A_Index277 < COUNT_programmingBlock_KT + 0; A_Index277++) {
                if (langToConvertTo == langFileExtension_2) {
                    htCode = StrReplace(htCode, "programmingBlock_KT-programmingBlock_KT-AA" + STR(A_Index277 + 1) + "AA", keyWordCodeInTheTranspiledLangStartKT_2 + Chr(10) + programmingBlock_KT[A_Index277] + Chr(10) + keyWordCodeInTheTranspiledLangEndKT_2);
                } else {
                    htCode = StrReplace(htCode, "programmingBlock_KT-programmingBlock_KT-AA" + STR(A_Index277 + 1) + "AA", Chr(10));
                }
            }
        }
    }
    if (langToConvertTo == "rb") {
        if (COUNT_programmingBlock_RB != 0) {
            for (int A_Index278 = 0; A_Index278 < COUNT_programmingBlock_RB + 0; A_Index278++) {
                htCode = StrReplace(htCode, "programmingBlock_RB-programmingBlock_RB-AA" + STR(A_Index278 + 1) + "AA", programmingBlock_RB[A_Index278]);
            }
        }
    } else {
        if (COUNT_programmingBlock_RB != 0) {
            for (int A_Index279 = 0; A_Index279 < COUNT_programmingBlock_RB + 0; A_Index279++) {
                if (langToConvertTo == langFileExtension_2) {
                    htCode = StrReplace(htCode, "programmingBlock_RB-programmingBlock_RB-AA" + STR(A_Index279 + 1) + "AA", keyWordCodeInTheTranspiledLangStartRB_2 + Chr(10) + programmingBlock_RB[A_Index279] + Chr(10) + keyWordCodeInTheTranspiledLangEndRB_2);
                } else {
                    htCode = StrReplace(htCode, "programmingBlock_RB-programmingBlock_RB-AA" + STR(A_Index279 + 1) + "AA", Chr(10));
                }
            }
        }
    }
    if (langToConvertTo == "nim") {
        if (COUNT_programmingBlock_NIM != 0) {
            for (int A_Index280 = 0; A_Index280 < COUNT_programmingBlock_NIM + 0; A_Index280++) {
                htCode = StrReplace(htCode, "programmingBlock_NIM-programmingBlock_NIM-AA" + STR(A_Index280 + 1) + "AA", programmingBlock_NIM[A_Index280]);
            }
        }
    } else {
        if (COUNT_programmingBlock_NIM != 0) {
            for (int A_Index281 = 0; A_Index281 < COUNT_programmingBlock_NIM + 0; A_Index281++) {
                if (langToConvertTo == langFileExtension_2) {
                    htCode = StrReplace(htCode, "programmingBlock_NIM-programmingBlock_NIM-AA" + STR(A_Index281 + 1) + "AA", keyWordCodeInTheTranspiledLangStartNIM_2 + Chr(10) + programmingBlock_NIM[A_Index281] + Chr(10) + keyWordCodeInTheTranspiledLangEndNIM_2);
                } else {
                    htCode = StrReplace(htCode, "programmingBlock_NIM-programmingBlock_NIM-AA" + STR(A_Index281 + 1) + "AA", Chr(10));
                }
            }
        }
    }
    if (langToConvertTo == "ahk") {
        if (COUNT_programmingBlock_AHK != 0) {
            for (int A_Index282 = 0; A_Index282 < COUNT_programmingBlock_AHK + 0; A_Index282++) {
                htCode = StrReplace(htCode, "programmingBlock_AHK-programmingBlock_AHK-AA" + STR(A_Index282 + 1) + "AA", programmingBlock_AHK[A_Index282]);
            }
        }
    } else {
        if (COUNT_programmingBlock_AHK != 0) {
            for (int A_Index283 = 0; A_Index283 < COUNT_programmingBlock_AHK + 0; A_Index283++) {
                if (langToConvertTo == langFileExtension_2) {
                    htCode = StrReplace(htCode, "programmingBlock_AHK-programmingBlock_AHK-AA" + STR(A_Index283 + 1) + "AA", keyWordCodeInTheTranspiledLangStartAHK_2 + Chr(10) + programmingBlock_AHK[A_Index283] + Chr(10) + keyWordCodeInTheTranspiledLangEndAHK_2);
                } else {
                    htCode = StrReplace(htCode, "programmingBlock_AHK-programmingBlock_AHK-AA" + STR(A_Index283 + 1) + "AA", Chr(10));
                }
            }
        }
    }
    if (langToConvertTo == "swift") {
        if (COUNT_programmingBlock_SWIFT != 0) {
            for (int A_Index284 = 0; A_Index284 < COUNT_programmingBlock_SWIFT + 0; A_Index284++) {
                htCode = StrReplace(htCode, "programmingBlock_SWIFT-programmingBlock_SWIFT-AA" + STR(A_Index284 + 1) + "AA", programmingBlock_SWIFT[A_Index284]);
            }
        }
    } else {
        if (COUNT_programmingBlock_SWIFT != 0) {
            for (int A_Index285 = 0; A_Index285 < COUNT_programmingBlock_SWIFT + 0; A_Index285++) {
                if (langToConvertTo == langFileExtension_2) {
                    htCode = StrReplace(htCode, "programmingBlock_SWIFT-programmingBlock_SWIFT-AA" + STR(A_Index285 + 1) + "AA", keyWordCodeInTheTranspiledLangStartSWIFT_2 + Chr(10) + programmingBlock_SWIFT[A_Index285] + Chr(10) + keyWordCodeInTheTranspiledLangEndSWIFT_2);
                } else {
                    htCode = StrReplace(htCode, "programmingBlock_SWIFT-programmingBlock_SWIFT-AA" + STR(A_Index285 + 1) + "AA", Chr(10));
                }
            }
        }
    }
    if (langToConvertTo == "dart") {
        if (COUNT_programmingBlock_DART != 0) {
            for (int A_Index286 = 0; A_Index286 < COUNT_programmingBlock_DART + 0; A_Index286++) {
                htCode = StrReplace(htCode, "programmingBlock_DART-programmingBlock_DART-AA" + STR(A_Index286 + 1) + "AA", programmingBlock_DART[A_Index286]);
            }
        }
    } else {
        if (COUNT_programmingBlock_DART != 0) {
            for (int A_Index287 = 0; A_Index287 < COUNT_programmingBlock_DART + 0; A_Index287++) {
                if (langToConvertTo == langFileExtension_2) {
                    htCode = StrReplace(htCode, "programmingBlock_DART-programmingBlock_DART-AA" + STR(A_Index287 + 1) + "AA", keyWordCodeInTheTranspiledLangStartDART_2 + Chr(10) + programmingBlock_DART[A_Index287] + Chr(10) + keyWordCodeInTheTranspiledLangEndDART_2);
                } else {
                    htCode = StrReplace(htCode, "programmingBlock_DART-programmingBlock_DART-AA" + STR(A_Index287 + 1) + "AA", Chr(10));
                }
            }
        }
    }
    if (langToConvertTo == "ts") {
        if (COUNT_programmingBlock_TS != 0) {
            for (int A_Index288 = 0; A_Index288 < COUNT_programmingBlock_TS + 0; A_Index288++) {
                htCode = StrReplace(htCode, "programmingBlock_TS-programmingBlock_TS-AA" + STR(A_Index288 + 1) + "AA", programmingBlock_TS[A_Index288]);
            }
        }
    } else {
        if (COUNT_programmingBlock_TS != 0) {
            for (int A_Index289 = 0; A_Index289 < COUNT_programmingBlock_TS + 0; A_Index289++) {
                if (langToConvertTo == langFileExtension_2) {
                    htCode = StrReplace(htCode, "programmingBlock_TS-programmingBlock_TS-AA" + STR(A_Index289 + 1) + "AA", keyWordCodeInTheTranspiledLangStartTS_2 + Chr(10) + programmingBlock_TS[A_Index289] + Chr(10) + keyWordCodeInTheTranspiledLangEndTS_2);
                } else {
                    htCode = StrReplace(htCode, "programmingBlock_TS-programmingBlock_TS-AA" + STR(A_Index289 + 1) + "AA", Chr(10));
                }
            }
        }
    }
    if (langToConvertTo == "groovy") {
        if (COUNT_programmingBlock_GROOVY != 0) {
            for (int A_Index290 = 0; A_Index290 < COUNT_programmingBlock_GROOVY + 0; A_Index290++) {
                htCode = StrReplace(htCode, "programmingBlock_GROOVY-programmingBlock_GROOVY-AA" + STR(A_Index290 + 1) + "AA", programmingBlock_GROOVY[A_Index290]);
            }
        }
    } else {
        if (COUNT_programmingBlock_GROOVY != 0) {
            for (int A_Index291 = 0; A_Index291 < COUNT_programmingBlock_GROOVY + 0; A_Index291++) {
                if (langToConvertTo == langFileExtension_2) {
                    htCode = StrReplace(htCode, "programmingBlock_GROOVY-programmingBlock_GROOVY-AA" + STR(A_Index291 + 1) + "AA", keyWordCodeInTheTranspiledLangStartGROOVY_2 + Chr(10) + programmingBlock_GROOVY[A_Index291] + Chr(10) + keyWordCodeInTheTranspiledLangEndGROOVY_2);
                } else {
                    htCode = StrReplace(htCode, "programmingBlock_GROOVY-programmingBlock_GROOVY-AA" + STR(A_Index291 + 1) + "AA", Chr(10));
                }
            }
        }
    }
    ...
    if (langToConvertTo == langFileExtension_2) {
        if (COUNT_programmingBlock_HTVM != 0) {
            for (int A_Index292 = 0; A_Index292 < COUNT_programmingBlock_HTVM + 0; A_Index292++) {
                htCode = StrReplace(htCode, "programmingBlock_HTVM-programmingBlock_HTVM-AA" + STR(A_Index292 + 1) + "AA", programmingBlock_HTVM[A_Index292]);
            }
        }
    } else {
        if (COUNT_programmingBlock_HTVM != 0) {
            for (int A_Index293 = 0; A_Index293 < COUNT_programmingBlock_HTVM + 0; A_Index293++) {
                htCode = StrReplace(htCode, "programmingBlock_HTVM-programmingBlock_HTVM-AA" + STR(A_Index293 + 1) + "AA", Chr(10));
            }
        }
    }
    if (langToConvertTo != langFileExtension_2) {
        ...
        for (int A_Index294 = 0; A_Index294 < theIdNumOfThe34 + 0; A_Index294++) {
            if (theIdNumOfThe34 == A_Index294 + 1) {
                if (langToConvertTo == langFileExtension_2) {
                    if (keyWordEscpaeChar_2 == "\\" && keyWordEscpaeChar != "\\") {
                        htCode = StrReplace(htCode, "fihuiuuhuuhtheidFor--asds" + str21 + "as--" + str21 + "theuhturtyphoutr--" + Chr(65) + Chr(65) + STR(A_Index294 + 1) + Chr(65) + Chr(65), StrReplace(StrReplace(fString(theIdNumOfThe34theVar[A_Index294 + 1]), "\\", "\\\\"), keyWordEscpaeChar, keyWordEscpaeChar_2) + Chr(34));
                    } else {
                        htCode = StrReplace(htCode, "fihuiuuhuuhtheidFor--asds" + str21 + "as--" + str21 + "theuhturtyphoutr--" + Chr(65) + Chr(65) + STR(A_Index294 + 1) + Chr(65) + Chr(65), StrReplace(fString(theIdNumOfThe34theVar[A_Index294 + 1]), keyWordEscpaeChar, keyWordEscpaeChar_2) + Chr(34));
                    }
                } else {
                    htCode = StrReplace(htCode, "fihuiuuhuuhtheidFor--asds" + str21 + "as--" + str21 + "theuhturtyphoutr--" + Chr(65) + Chr(65) + STR(A_Index294 + 1) + Chr(65) + Chr(65), StrReplace(fString(theIdNumOfThe34theVar[A_Index294 + 1]), keyWordEscpaeChar, "\\") + Chr(34));
                }
            } else {
                if (langToConvertTo == langFileExtension_2) {
                    if (keyWordEscpaeChar_2 == "\\" && keyWordEscpaeChar != "\\") {
                        htCode = StrReplace(htCode, "fihuiuuhuuhtheidFor--asds" + str21 + "as--" + str21 + "theuhturtyphoutr--" + Chr(65) + Chr(65) + STR(A_Index294 + 1) + Chr(65) + Chr(65), StrReplace(StrReplace(fString(theIdNumOfThe34theVar[A_Index294 + 1]), "\\", "\\\\"), keyWordEscpaeChar, keyWordEscpaeChar_2));
                    } else {
                        htCode = StrReplace(htCode, "fihuiuuhuuhtheidFor--asds" + str21 + "as--" + str21 + "theuhturtyphoutr--" + Chr(65) + Chr(65) + STR(A_Index294 + 1) + Chr(65) + Chr(65), StrReplace(fString(theIdNumOfThe34theVar[A_Index294 + 1]), keyWordEscpaeChar, keyWordEscpaeChar_2));
                    }
                } else {
                    htCode = StrReplace(htCode, "fihuiuuhuuhtheidFor--asds" + str21 + "as--" + str21 + "theuhturtyphoutr--" + Chr(65) + Chr(65) + STR(A_Index294 + 1) + Chr(65) + Chr(65), StrReplace(fString(theIdNumOfThe34theVar[A_Index294 + 1]), keyWordEscpaeChar, "\\"));
                }
            }
        }
    }
    ...
    int correctLang = 0;
    std::vector<std::string> items295 = LoopParseFunc(instructions, "\n", "\r");
    for (size_t A_Index295 = 0; A_Index295 < items295.size() + 0; A_Index295++) {
        std::string A_LoopField295 = items295[A_Index295 - 0];
        if (Trim(A_LoopField295) == "funcEND======================funcEND==============") {
            areWeInAFuncFromInstructions = 0;
            areWeInAFuncFromInstructionsLineNum = 0;
        ...
        if (areWeInAFuncFromInstructions == 1) {
            if (areWeInAFuncFromInstructionsLineNum == 1) {
                // name of the func
                funcLangHolder = StringTrimLeft(A_LoopField295, 5);
            ...
            if (areWeInAFuncFromInstructionsLineNum == 2) {
                // name of the func
                funcNameHolder = StringTrimLeft(A_LoopField295, 5);
                if (correctLang == 1 && InStr(htCode, Trim(funcNameHolder))) {
            ...
            if (areWeInAFuncFromInstructionsLineNum == 3) {
                // all libs
                funcLibsHolder = StringTrimLeft(A_LoopField295, 5);
                if (correctLang == 1 && InStr(htCode, Trim(funcNameHolder))) {
            ...
            if (areWeInAFuncFromInstructionsLineNum == 4) {
                // func description
                funcDescriptionHolder = StringTrimLeft(A_LoopField295, 12);
                if (correctLang == 1 && InStr(htCode, Trim(funcNameHolder))) {
            ...
            if (areWeInAFuncFromInstructionsLineNum >= 5) {
                // the full func
                if (correctLang == 1 && InStr(htCode, Trim(funcNameHolder))) {
                    funcFuncHolder += A_LoopField295 + Chr(10);
                }
            }
            //MsgBox, % A_LoopField
            areWeInAFuncFromInstructionsLineNum++;
        }
        if (Trim(A_LoopField295) == "func======================func==============") {
    ...
    if (!(HTVM_Size(allFuncNames) <= 0)) {
        for (int A_Index296 = 0; A_Index296 < HTVM_Size(allFuncNames) + 0; A_Index296++) {
            if (InStr(htCode, allFuncNames[A_Index296]) + "(") {
                //MsgBox, % allFuncNames[A_Index]
                allFuncsToPutAtTop += allFuncs[A_Index296] + Chr(10);
                if (Trim(allFuncLibs[A_Index296]) != "null") {
                    allLibsToPutAtTop += allFuncLibs[A_Index296] + "|";
                }
            }
        }
        allLibsToPutAtTop = StringTrimRight(allLibsToPutAtTop, 1);
        std::string allLibsToPutAtTopTEMP = "";
        std::vector<std::string> items297 = LoopParseFunc(allLibsToPutAtTop, "|");
        for (size_t A_Index297 = 0; A_Index297 < items297.size() + 0; A_Index297++) {
            std::string A_LoopField297 = items297[A_Index297 - 0];
            allLibsToPutAtTopTEMP += A_LoopField297 + Chr(10);
        }
    ...
    if (langToConvertTo != langFileExtension_2) {
        for (int A_Index298 = 0; A_Index298 < HTVM_Size(ospDic1) + 0; A_Index298++) {
            str00 = ospDic1[A_Index298];
            if (langToConvertTo == "rb") {
    ...
    if (size_ospDic != 0) {
        for (int A_Index299 = 0; A_Index299 < size_ospDic + 0; A_Index299++) {
            HTVM_Pop(ospDic);
        }
    }
    //;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;
    ...
    if (size_ospDic1 != 0) {
        for (int A_Index300 = 0; A_Index300 < size_ospDic1 + 0; A_Index300++) {
            HTVM_Pop(ospDic1);
        }
    }
    //;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;
    ...
    if (size_ospDic2 != 0) {
        for (int A_Index301 = 0; A_Index301 < size_ospDic2 + 0; A_Index301++) {
            HTVM_Pop(ospDic2);
        }
    }
    //;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;
    ...
    if (size_htCodeFixRubyGlobalVars_EXPOSED != 0) {
        for (int A_Index302 = 0; A_Index302 < size_htCodeFixRubyGlobalVars_EXPOSED + 0; A_Index302++) {
    ...
    for (int A_Index303 = 0; A_Index303 < theIdNumOfThe34 + 0; A_Index303++) {
        if (theIdNumOfThe34 == A_Index303 + 1) {
            if (langToConvertTo == langFileExtension_2) {
                if (keyWordEscpaeChar_2 == "\\" && keyWordEscpaeChar != "\\") {
                    htCode = StrReplace(htCode, "ihuiuuhuuhtheidFor--asds" + str21 + "as--" + str21 + "theuhturtyphoutr--" + Chr(65) + Chr(65) + STR(A_Index303 + 1) + Chr(65) + Chr(65), StrReplace(StrReplace(theIdNumOfThe34theVar[A_Index303 + 1], "\\", "\\\\"), keyWordEscpaeChar, keyWordEscpaeChar_2) + Chr(34));
                } else {
                    htCode = StrReplace(htCode, "ihuiuuhuuhtheidFor--asds" + str21 + "as--" + str21 + "theuhturtyphoutr--" + Chr(65) + Chr(65) + STR(A_Index303 + 1) + Chr(65) + Chr(65), StrReplace(theIdNumOfThe34theVar[A_Index303 + 1], keyWordEscpaeChar, keyWordEscpaeChar_2) + Chr(34));
                }
            } else {
                htCode = StrReplace(htCode, "ihuiuuhuuhtheidFor--asds" + str21 + "as--" + str21 + "theuhturtyphoutr--" + Chr(65) + Chr(65) + STR(A_Index303 + 1) + Chr(65) + Chr(65), StrReplace(theIdNumOfThe34theVar[A_Index303 + 1], keyWordEscpaeChar, "\\") + Chr(34));
            }
        } else {
            if (langToConvertTo == langFileExtension_2) {
                if (keyWordEscpaeChar_2 == "\\" && keyWordEscpaeChar != "\\") {
                    htCode = StrReplace(htCode, "ihuiuuhuuhtheidFor--asds" + str21 + "as--" + str21 + "theuhturtyphoutr--" + Chr(65) + Chr(65) + STR(A_Index303 + 1) + Chr(65) + Chr(65), StrReplace(StrReplace(theIdNumOfThe34theVar[A_Index303 + 1], "\\", "\\\\"), keyWordEscpaeChar, keyWordEscpaeChar_2));
                } else {
                    htCode = StrReplace(htCode, "ihuiuuhuuhtheidFor--asds" + str21 + "as--" + str21 + "theuhturtyphoutr--" + Chr(65) + Chr(65) + STR(A_Index303 + 1) + Chr(65) + Chr(65), StrReplace(theIdNumOfThe34theVar[A_Index303 + 1], keyWordEscpaeChar, keyWordEscpaeChar_2));
                }
            } else {
                htCode = StrReplace(htCode, "ihuiuuhuuhtheidFor--asds" + str21 + "as--" + str21 + "theuhturtyphoutr--" + Chr(65) + Chr(65) + STR(A_Index303 + 1) + Chr(65) + Chr(65), StrReplace(theIdNumOfThe34theVar[A_Index303 + 1], keyWordEscpaeChar, "\\"));
            }
        }
    }
    if (langToConvertTo == langFileExtension_2) {
    ...
    if (isNotHTVMfile2 == 0) {
        ...
        if (langToConvertTo == langFileExtension_2) {
            for (int A_Index304 = 0; A_Index304 < HTVM_Size(weAreInMLSarr) + 0; A_Index304++) {
                htCode = StrReplace(htCode, "aejsoydoxubviycbdgsut7eoy-ihsxbvhowadesdio6376e7wuwyuau--szd--AA" + STR(A_Index304) + "AA", weAreInMLSarr[A_Index304]);
            }
    ...
    if (size_weAreInMLSarr != 0) {
        for (int A_Index305 = 0; A_Index305 < size_weAreInMLSarr + 0; A_Index305++) {
...
void HTVMv2() {
    ...
    std::vector<std::string> items306 = LoopParseFunc(str0);
    for (size_t A_Index306 = 0; A_Index306 < items306.size() + 0; A_Index306++) {
        std::string A_LoopField306 = items306[A_Index306 - 0];
        str00 = Trim(A_LoopField306);
    ...
    if (HTVM_getLang_HTVM() == "cpp" || HTVM_getLang_HTVM() == "py") {
        ...
        std::vector<std::string> items307 = LoopParseFunc(allArgs, "\n", "\r");
        for (size_t A_Index307 = 0; A_Index307 < items307.size() + 0; A_Index307++) {
            std::string A_LoopField307 = items307[A_Index307 - 0];
            if (A_Index307 == 0) {
                numOfParams++;
                argCODE = FileRead(Trim(A_LoopField307));
                argCODEfile = Trim(A_LoopField307);
            }
            else if (A_Index307 == 1) {
                numOfParams++;
                argHTVMinstr = Trim(A_LoopField307);
            }
            else if (A_Index307 == 2) {
                numOfParams++;
                argLangTo = Trim(A_LoopField307);
            } else {
                numOfParams++;
                HTVM_Append(argHTVMinstrMORE, Trim(A_LoopField307));
            }
        }
        //print("===============123431234===========start=====")
        for (int A_Index308 = 0; A_Index308 < HTVM_Size(argHTVMinstrMORE) + 0; A_Index308++) {
