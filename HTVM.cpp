...
std::string fixJAVAstrCmp_FjavaUGH(std::string javaCode) {
    ...
    int next = 0;
    std::vector<std::string> items154 = LoopParseFunc(javaCode, " ");
    for (size_t A_Index154 = 0; A_Index154 < items154.size() + 0; A_Index154++) {
        std::string A_LoopField154 = items154[A_Index154 - 0];
        if (next == 1) {
            if (RegExMatch(Trim(A_LoopField154), "^[A-Za-z_][A-Za-z0-9_]*$")) {
                HTVM_Append(allJavaStringsIG, Trim(A_LoopField154));
            }
            next = 0;
        }
        if (A_LoopField154 == "String") {
            next = 1;
        }
    }
    std::string pattern1 = "";
    ...
    std::string replace4 = "";
    std::string varName = "";
    for (int A_Index155 = 0; A_Index155 < HTVM_Size(allJavaStringsIG) + 0; A_Index155++) {
        varName = allJavaStringsIG[A_Index155];
        pattern1 = "\\b" + varName + "\\s*==\\s*([A-Za-z0-9_\\-]+)\\b";
        replace1 = varName + ".equals($1)";
        javaCode = RegExReplace(javaCode, pattern1, replace1);
        pattern2 = "\\b([A-Za-z0-9_\\-]+)\\s*==\\s*" + varName + "\\b";
        replace2 = varName + ".equals($1)";
        javaCode = RegExReplace(javaCode, pattern2, replace2);
        pattern3 = "\\b" + varName + "\\s*!=\\s*([A-Za-z0-9_\\-]+)\\b";
        replace3 = "!" + varName + ".equals($1)";
        javaCode = RegExReplace(javaCode, pattern3, replace3);
        pattern4 = "\\b([A-Za-z0-9_\\-]+)\\s*!=\\s*" + varName + "\\b";
...
std::string FixDoubleInThe4langs(std::string line) {
    ...
    std::vector<std::string> items156 = LoopParseFunc(line, "\n", "\r");
    for (size_t A_Index156 = 0; A_Index156 < items156.size() + 0; A_Index156++) {
        std::string A_LoopField156 = items156[A_Index156 - 0];
        if (RegExMatch(A_LoopField156, "\\bdouble\\b") || RegExMatch(A_LoopField156, "\\bDouble\\b")) {
            FixDoubleInThe4langsOUT += RegExReplace(A_LoopField156, "(?<![A-Za-z0-9_])(-?\\d+\\.\\d+)f(?![A-Za-z0-9_])", "$1") + Chr(10);
        } else {
            FixDoubleInThe4langsOUT += A_LoopField156 + Chr(10);
        }
    }
...
std::string fString(std::string line) {
    ...
    if (langToConvertTo != langFileExtension_2) {
        ...
        for (size_t A_Index157 = 0; A_Index157 < items157.size() + 0; A_Index157++) {
            ...
            if (areWeIn == 1 && A_LoopField157 != "{" && A_LoopField157 != "}") {
                outHelp += A_LoopField157;
            } else {
                if (A_LoopField157 == "{") {
                    out += Chr(34) + " " + concatStringInOtherLangs + " fstr(";
                }
                else if (A_LoopField157 == "}" && areWeIn != 0) {
        ...
        std::vector<std::string> items158 = LoopParseFunc(line);
        for (size_t A_Index158 = 0; A_Index158 < items158.size() + 0; A_Index158++) {
            std::string A_LoopField158 = items158[A_Index158 - 0];
            if (areWeIn == 1 && A_LoopField158 != "{" && A_LoopField158 != "}") {
                outHelp += A_LoopField158;
            } else {
                if (A_LoopField158 == "{") {
                    out += "{";
                }
                else if (A_LoopField158 == "}" && areWeIn != 0) {
    ...
    if (langToConvertTo == "rb") {
        for (int A_Index159 = 0; A_Index159 < HTVM_Size(htCodeFixRubyGlobalVars_EXPOSED) + 0; A_Index159++) {
            out = RegExReplace(out, "\\b" + htCodeFixRubyGlobalVars_EXPOSED[A_Index159] + "\\b", "$" + htCodeFixRubyGlobalVars_EXPOSED[A_Index159]);
        }
    }
    return out;
}
...
std::string handleMultiLineStrings(std::string code) {
    ...
    std::vector<std::string> items160 = LoopParseFunc(code, "\n", "\r");
    for (size_t A_Index160 = 0; A_Index160 < items160.size() + 0; A_Index160++) {
        std::string A_LoopField160 = items160[A_Index160 - 0];
        if (SubStrLastChars(Trim(A_LoopField160), 4) == " mls" || SubStrLastChars(Trim(A_LoopField160), 5) == " fmls") {
            didWeFindOrWeOut = 1;
        }
    }
    ...
    for (size_t A_Index162 = 0; A_Index162 < items162.size() + 0; A_Index162++) {
        ...
        if (langToConvertTo == langFileExtension_2) {
            ...
            if (weAreIn == 1) {
                ...
                weAreInMLSarrTemp += A_LoopField162;
            }
            if (SubStrLastChars(Trim(A_LoopField162), 4) == " mls" || SubStrLastChars(Trim(A_LoopField162), 5) == " fmls") {
                weAreIn = 1;
                out += A_LoopField162 + Chr(10);
            }
            if (weAreIn == 0) {
                out += A_LoopField162 + Chr(10);
            }
            ...
            if (weAreIn == 1 && Trim(A_LoopField162) == "HTVM-newLineAHw4y9w730q8237w4e8srhgesw0s8rgw04e5g08sgr0e8srdgt--YOU-BETERNOTUSETHISAH--aesdfssz") {
                if (lookAhead[A_Index162 + 1] == "HTVM-newLineAHw4y9w730q8237w4e8srhgesw0s8rgw04e5g08sgr0e8srdgt--YOU-BETERNOTUSETHISAH--aesdfssz") {
                    out += keyWordEscpaeChar + "n";
                }
            }
            if (weAreIn == 1 && Trim(A_LoopField162) != "HTVM-newLineAHw4y9w730q8237w4e8srhgesw0s8rgw04e5g08sgr0e8srdgt--YOU-BETERNOTUSETHISAH--aesdfssz") {
                out += A_LoopField162 + keyWordEscpaeChar + "n";
            }
            if (SubStrLastChars(Trim(A_LoopField162), 4) == " mls" || SubStrLastChars(Trim(A_LoopField162), 5) == " fmls") {
                weAreIn = 1;
                if (SubStrLastChars(Trim(A_LoopField162), 5) == " fmls") {
                    out += TrimR(StringTrimRight(TrimR(A_LoopField162), 3)) + Chr(34);
                } else {
                    out += TrimR(StringTrimRight(TrimR(A_LoopField162), 3)) + " " + Chr(34);
                }
            }
            if (weAreIn == 0) {
                if (Trim(A_LoopField162) != "mle") {
                    out += A_LoopField162 + Chr(10);
                }
            }
            //;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;
...
std::vector<std::string> slidingWinFixCommentsRmFUNC(std::string line) {
    ...
    std::vector<std::string> items163 = LoopParseFunc(line);
    for (size_t A_Index163 = 0; A_Index163 < items163.size() + 0; A_Index163++) {
        std::string A_LoopField163 = items163[A_Index163 - 0];
        HTVM_Append(slidingWinFixCommentsRmFFFF, A_LoopField163);
    }
    for (int A_Index164 = 0; A_Index164 < StrLen(keyWordComment + keyWordComment) + 2 + 0; A_Index164++) {
...
std::string fixIfElseIfMultiLineStatementHELP(std::string aloopfield) {
    std::string out = "";
    int count = 0;
    std::vector<std::string> items165 = LoopParseFunc(aloopfield);
...
std::string fixIfElseIfMultiLineStatement(std::string someHTVMcode) {
    int doWeExitInsta = 1;
    std::vector<std::string> items166 = LoopParseFunc(someHTVMcode, "\n", "\r");
    for (size_t A_Index166 = 0; A_Index166 < items166.size() + 0; A_Index166++) {
        std::string A_LoopField166 = items166[A_Index166 - 0];
        if (SubStr(StrLower(Trim(A_LoopField166)), 1, StrLen(keyWordIF)) == keyWordIF) {
            doWeExitInsta = 0;
        }
        else if (SubStr(StrLower(Trim(A_LoopField166)), 1, StrLen(keyWordWhileLoop)) == keyWordWhileLoop) {
            doWeExitInsta = 0;
        }
        else if (SubStr(StrLower(Trim(A_LoopField166)), 1, StrLen(keyWordElseIf)) == keyWordElseIf) {
            doWeExitInsta = 0;
        }
    }
    if (doWeExitInsta == 1) {
    ...
    for (int A_Index167 = 0; A_Index167 < 10000 + 0; A_Index167++) {
        ...
        out = "";
        doWeExit = false;
        std::vector<std::string> items168 = LoopParseFunc(out2, "\n", "\r");
        for (size_t A_Index168 = 0; A_Index168 < items168.size() + 0; A_Index168++) {
            std::string A_LoopField168 = items168[A_Index168 - 0];
            if (SubStr(StrLower(Trim(A_LoopField168)), 1, StrLen(keyWordIF)) == keyWordIF) {
                doWeJustExit = 1;
                if (fixIfElseIfMultiLineStatementHELP(A_LoopField168) == "") {
                    out += Trim(A_LoopField168) + " ";
                    doWeExit = false;
                    doWeExitHELP--;
                } else {
                    doWeExit = true;
                    out += Trim(A_LoopField168) + Chr(10);
                }
            }
            else if (SubStr(StrLower(Trim(A_LoopField168)), 1, StrLen(keyWordWhileLoop)) == keyWordWhileLoop) {
                doWeJustExit = 1;
                if (fixIfElseIfMultiLineStatementHELP(A_LoopField168) == "") {
                    out += Trim(A_LoopField168) + " ";
                    doWeExit = false;
            ...
            else if (SubStr(StrLower(Trim(A_LoopField168)), 1, StrLen(keyWordElseIf)) == keyWordElseIf) {
                doWeJustExit = 1;
                if (fixIfElseIfMultiLineStatementHELP(A_LoopField168) == "") {
                    out += Trim(A_LoopField168) + " ";
                    doWeExit = false;
...
bool doseHaveInclude(std::string TheCodeThatMightHaveInclude) {
    std::vector<std::string> items169 = LoopParseFunc(TheCodeThatMightHaveInclude, "\n", "\r");
    for (size_t A_Index169 = 0; A_Index169 < items169.size() + 0; A_Index169++) {
        std::string A_LoopField169 = items169[A_Index169 - 0];
        if (SubStr(StrLower(Trim(A_LoopField169)), 1, StrLen(StrLower(keyWordInclude + " "))) == StrLower(keyWordInclude + " ")) {
            return true;
        }
    }
    return false;
...
std::string compiler(std::string htCode, std::string allInstructionFile, std::string mode, std::string langToConvertToParam = "") {
    ...
    std::vector<std::string> items170 = LoopParseFunc(allInstructionFile, "\n", "\r");
    for (size_t A_Index170 = 0; A_Index170 < items170.size() + 0; A_Index170++) {
        std::string A_LoopField170 = items170[A_Index170 - 0];
        if (A_Index170 == 0) {
            langToConvertTo = Trim(A_LoopField170);
        }
        if (A_Index170 == 1) {
            langFileExtension = Trim(A_LoopField170);
        }
        if (A_Index170 == 2) {
            commands = Trim(A_LoopField170);
        }
        if (A_Index170 == 3) {
            keyWordAlliance = Trim(A_LoopField170) + " ";
        }
        if (A_Index170 == 4) {
            keyWordCrew = Trim(A_LoopField170) + " ";
        }
        if (A_Index170 == 5) {
            keyWordProc = Trim(A_LoopField170) + " ";
        }
        if (A_Index170 == 6) {
            keyWordStruct = Trim(A_LoopField170) + " ";
        }
        if (A_Index170 == 7) {
            keyWordProp = Trim(A_LoopField170) + " ";
        }
        if (A_Index170 == 8) {
            keyWordThis = Trim(A_LoopField170);
        }
        if (A_Index170 == 9) {
            keyWordInclude = Trim(A_LoopField170);
        }
        if (A_Index170 == 10) {
            keyWordCodeInTheTranspiledLangStart = Trim(A_LoopField170);
        }
        if (A_Index170 == 11) {
            keyWordCodeInTheTranspiledLangEnd = Trim(A_LoopField170);
        }
        if (A_Index170 == 12) {
            keyWordCodeInTheTranspiledLangStartCPP = Trim(A_LoopField170);
        }
        if (A_Index170 == 13) {
            keyWordCodeInTheTranspiledLangEndCPP = Trim(A_LoopField170);
        }
        if (A_Index170 == 14) {
            keyWordCodeInTheTranspiledLangStartPY = Trim(A_LoopField170);
        }
        if (A_Index170 == 15) {
            keyWordCodeInTheTranspiledLangEndPY = Trim(A_LoopField170);
        }
        if (A_Index170 == 16) {
            keyWordCodeInTheTranspiledLangStartJS = Trim(A_LoopField170);
        }
        if (A_Index170 == 17) {
            keyWordCodeInTheTranspiledLangEndJS = Trim(A_LoopField170);
        }
        if (A_Index170 == 18) {
            keyWordCodeInTheTranspiledLangStartGO = Trim(A_LoopField170);
        }
        if (A_Index170 == 19) {
            keyWordCodeInTheTranspiledLangEndGO = Trim(A_LoopField170);
        }
        if (A_Index170 == 20) {
            keyWordCodeInTheTranspiledLangStartLUA = Trim(A_LoopField170);
        }
        if (A_Index170 == 21) {
            keyWordCodeInTheTranspiledLangEndLUA = Trim(A_LoopField170);
        }
        if (A_Index170 == 22) {
            keyWordCodeInTheTranspiledLangStartCS = Trim(A_LoopField170);
        }
        if (A_Index170 == 23) {
            keyWordCodeInTheTranspiledLangEndCS = Trim(A_LoopField170);
        }
        if (A_Index170 == 24) {
            keyWordCodeInTheTranspiledLangStartJAVA = Trim(A_LoopField170);
        }
        if (A_Index170 == 25) {
            keyWordCodeInTheTranspiledLangEndJAVA = Trim(A_LoopField170);
        }
        if (A_Index170 == 26) {
            keyWordCodeInTheTranspiledLangStartKT = Trim(A_LoopField170);
        }
        if (A_Index170 == 27) {
            keyWordCodeInTheTranspiledLangEndKT = Trim(A_LoopField170);
        }
        if (A_Index170 == 28) {
            keyWordCodeInTheTranspiledLangStartRB = Trim(A_LoopField170);
        }
        if (A_Index170 == 29) {
            keyWordCodeInTheTranspiledLangEndRB = Trim(A_LoopField170);
        }
        if (A_Index170 == 30) {
            keyWordCodeInTheTranspiledLangStartNIM = Trim(A_LoopField170);
        }
        if (A_Index170 == 31) {
            keyWordCodeInTheTranspiledLangEndNIM = Trim(A_LoopField170);
        }
        if (A_Index170 == 32) {
            keyWordCodeInTheTranspiledLangStartAHK = Trim(A_LoopField170);
        }
        if (A_Index170 == 33) {
            keyWordCodeInTheTranspiledLangEndAHK = Trim(A_LoopField170);
        }
        if (A_Index170 == 34) {
            keyWordCodeInTheTranspiledLangStartSWIFT = Trim(A_LoopField170);
        }
        if (A_Index170 == 35) {
            keyWordCodeInTheTranspiledLangEndSWIFT = Trim(A_LoopField170);
        }
        if (A_Index170 == 36) {
            keyWordCodeInTheTranspiledLangStartDART = Trim(A_LoopField170);
        }
        if (A_Index170 == 37) {
            keyWordCodeInTheTranspiledLangEndDART = Trim(A_LoopField170);
        }
        if (A_Index170 == 38) {
            keyWordCodeInTheTranspiledLangStartTS = Trim(A_LoopField170);
        }
        if (A_Index170 == 39) {
            keyWordCodeInTheTranspiledLangEndTS = Trim(A_LoopField170);
        }
        if (A_Index170 == 40) {
            keyWordCodeInTheTranspiledLangStartGROOVY = Trim(A_LoopField170);
        }
        if (A_Index170 == 41) {
            keyWordCodeInTheTranspiledLangEndGROOVY = Trim(A_LoopField170);
        }
        if (A_Index170 == 42) {
            keyWordCodeInTheTranspiledLangStartHTVM = Trim(A_LoopField170);
        }
        if (A_Index170 == 43) {
            keyWordCodeInTheTranspiledLangEndHTVM = Trim(A_LoopField170);
        }
        if (A_Index170 == 44) {
            keyWordCodeInHTVMstart = Trim(A_LoopField170);
        }
        if (A_Index170 == 45) {
            keyWordCodeInHTVMend = Trim(A_LoopField170);
        }
        if (A_Index170 == 46) {
            keyWordCurlyBraceOpen = Trim(A_LoopField170);
        }
        if (A_Index170 == 47) {
            keyWordCurlyBraceClose = Trim(A_LoopField170);
        }
        if (A_Index170 == 48) {
            keyWordNull = Trim(A_LoopField170);
        }
        if (A_Index170 == 49) {
            keyWordTrue = Trim(A_LoopField170);
        }
        if (A_Index170 == 50) {
            keyWordFalse = Trim(A_LoopField170);
        }
        if (A_Index170 == 51) {
            keyWordVoid = Trim(A_LoopField170);
        }
        if (A_Index170 == 52) {
            keyWordDouble = Trim(A_LoopField170);
        }
        if (A_Index170 == 53) {
            keyWordChar = Trim(A_LoopField170);
        }
        if (A_Index170 == 54) {
            keyWordUint8 = Trim(A_LoopField170);
        }
        if (A_Index170 == 55) {
            keyWordUint16 = Trim(A_LoopField170);
        }
        if (A_Index170 == 56) {
            keyWordUint32 = Trim(A_LoopField170);
        }
        if (A_Index170 == 57) {
            keyWordUint64 = Trim(A_LoopField170);
        }
        if (A_Index170 == 58) {
            keyWordINT = Trim(A_LoopField170);
        }
        if (A_Index170 == 59) {
            keyWordSTR = Trim(A_LoopField170);
        }
        if (A_Index170 == 60) {
            keyWordBOOL = Trim(A_LoopField170);
        }
        if (A_Index170 == 61) {
            keyWordFLOAT = Trim(A_LoopField170);
        }
        if (A_Index170 == 62) {
            keyWordINT8 = Trim(A_LoopField170);
        }
        if (A_Index170 == 63) {
            keyWordINT16 = Trim(A_LoopField170);
        }
        if (A_Index170 == 64) {
            keyWordINT32 = Trim(A_LoopField170);
        }
        if (A_Index170 == 65) {
            keyWordINT64 = Trim(A_LoopField170);
        }
        if (A_Index170 == 66) {
            keyWordIF = Trim(A_LoopField170) + " ";
        }
        if (A_Index170 == 67) {
            keyWordElseIf = Trim(A_LoopField170) + " ";
        }
        if (A_Index170 == 68) {
            keyWordElse = Trim(A_LoopField170);
        }
        if (A_Index170 == 69) {
            keyWordWhileLoop = Trim(A_LoopField170) + " ";
        }
        if (A_Index170 == 70) {
            keyWordLoopInfinite = Trim(A_LoopField170);
        }
        if (A_Index170 == 71) {
            keyWordLoop = Trim(A_LoopField170) + " ";
        }
        if (A_Index170 == 72) {
            keyWordLoopParse = Trim(A_LoopField170) + " ";
        }
        if (A_Index170 == 73) {
            keyWordContinue = Trim(A_LoopField170);
        }
        if (A_Index170 == 74) {
            keyWordBreak = Trim(A_LoopField170);
        }
        if (A_Index170 == 75) {
            keyWordFunc = Trim(A_LoopField170) + " ";
        }
        if (A_Index170 == 76) {
            keyWordAwait = Trim(A_LoopField170) + " ";
        }
        if (A_Index170 == 77) {
            keyWordAsync = Trim(A_LoopField170) + " ";
        }
        if (A_Index170 == 78) {
            keyWordThrow = Trim(A_LoopField170) + " ";
        }
        if (A_Index170 == 79) {
            keyWordErrorMsg = Trim(A_LoopField170);
        }
        if (A_Index170 == 80) {
            keyWordTry = Trim(A_LoopField170);
        }
        if (A_Index170 == 81) {
            keyWordCatch = Trim(A_LoopField170) + " ";
        }
        if (A_Index170 == 82) {
            keyWordFinally = Trim(A_LoopField170);
        }
        if (A_Index170 == 83) {
            keyWordReturnStatement = Trim(A_LoopField170) + " ";
        }
        if (A_Index170 == 84) {
            keyWordArrayAppend = Trim(A_LoopField170);
        }
        if (A_Index170 == 85) {
            keyWordArrayPop = Trim(A_LoopField170);
        }
        if (A_Index170 == 86) {
            keyWordArraySize = Trim(A_LoopField170);
        }
        if (A_Index170 == 87) {
            keyWordArrayInsert = Trim(A_LoopField170);
        }
        if (A_Index170 == 88) {
            keyWordArrayRemove = Trim(A_LoopField170);
        }
        if (A_Index170 == 89) {
            keyWordArrayIndexOf = Trim(A_LoopField170);
        }
        if (A_Index170 == 90) {
            keyWordArrayDefinition = Trim(A_LoopField170);
        }
        if (A_Index170 == 91) {
            keyWordArrayOfIntegersDefinition = Trim(A_LoopField170);
        }
        if (A_Index170 == 92) {
            keyWordArrayOfStringsDefinition = Trim(A_LoopField170);
        }
        if (A_Index170 == 93) {
            keyWordArrayOfFloatingPointNumbersDefinition = Trim(A_LoopField170);
        }
        if (A_Index170 == 94) {
            keyWordArrayOfBooleansDefinition = Trim(A_LoopField170);
        }
        if (A_Index170 == 95) {
            keyWordVar = Trim(A_LoopField170) + " ";
        }
        if (A_Index170 == 96) {
            keyWordLet = Trim(A_LoopField170) + " ";
        }
        if (A_Index170 == 97) {
            keyWordConst = Trim(A_LoopField170) + " ";
        }
        if (A_Index170 == 98) {
            keyWordEnd = Trim(A_LoopField170);
        }
        if (A_Index170 == 99) {
            keyWordGlobal = Trim(A_LoopField170) + " ";
        }
        if (A_Index170 == 100) {
            keyWordComment = Trim(A_LoopField170);
        }
        if (A_Index170 == 101) {
            keyWordCommentOpenMultiLine = Trim(A_LoopField170);
        }
        if (A_Index170 == 102) {
            keyWordCommentCloseMultiLine = Trim(A_LoopField170);
        }
        if (A_Index170 == 103) {
            keyWordEscpaeChar = Trim(A_LoopField170);
        }
        if (A_Index170 == 104) {
            keyWordMainLabel = Trim(A_LoopField170);
        }
        if (A_Index170 == 105) {
            keyWordConcat = Trim(A_LoopField170);
        }
        if (A_Index170 == 106) {
            keyWordAdd = Trim(A_LoopField170);
        }
        if (A_Index170 == 107) {
            keyWordSub = Trim(A_LoopField170);
        }
        if (A_Index170 == 108) {
            keyWordMul = Trim(A_LoopField170);
        }
        if (A_Index170 == 109) {
            keyWordDiv = Trim(A_LoopField170);
        }
        if (A_Index170 == 110) {
            keyWordMod = Trim(A_LoopField170);
        }
        if (A_Index170 == 111) {
            keyWordExp = Trim(A_LoopField170);
        }
        if (A_Index170 == 112) {
            keyWordEqual = Trim(A_LoopField170);
        }
        if (A_Index170 == 113) {
            keyWordStrictEqual = Trim(A_LoopField170);
        }
        if (A_Index170 == 114) {
            keyWordNotEqual = Trim(A_LoopField170);
        }
        if (A_Index170 == 115) {
            keyWordGreater = Trim(A_LoopField170);
        }
        if (A_Index170 == 116) {
            keyWordLess = Trim(A_LoopField170);
        }
        if (A_Index170 == 117) {
            keyWordGreaterEqual = Trim(A_LoopField170);
        }
        if (A_Index170 == 118) {
            keyWordLessEqual = Trim(A_LoopField170);
        }
        if (A_Index170 == 119) {
            keyWordAnd = Trim(A_LoopField170);
        }
        if (A_Index170 == 120) {
            keyWordOr = Trim(A_LoopField170);
        }
        if (A_Index170 == 121) {
            keyWordNot = Trim(A_LoopField170);
        }
        if (A_Index170 == 122) {
            keyWordBitAnd = Trim(A_LoopField170);
        }
        if (A_Index170 == 123) {
            keyWordBitOr = Trim(A_LoopField170);
        }
        if (A_Index170 == 124) {
            keyWordBitXor = Trim(A_LoopField170);
        }
        if (A_Index170 == 125) {
            keyWordBitNot = Trim(A_LoopField170);
        }
        if (A_Index170 == 126) {
            keyWordShiftLeft = Trim(A_LoopField170);
        }
        if (A_Index170 == 127) {
            keyWordShiftRight = Trim(A_LoopField170);
        }
        if (A_Index170 == 128) {
            keyWordShiftUnsignedRight = Trim(A_LoopField170);
        }
        if (A_Index170 == 129) {
            keyWordAssign = Trim(A_LoopField170);
        }
        if (A_Index170 == 130) {
            keyWordAssignAdd = Trim(A_LoopField170);
        }
        if (A_Index170 == 131) {
            keyWordAssignConcat = Trim(A_LoopField170);
        }
        if (A_Index170 == 132) {
            keyWordAssignSub = Trim(A_LoopField170);
        }
        if (A_Index170 == 133) {
            keyWordAssignMul = Trim(A_LoopField170);
        }
        if (A_Index170 == 134) {
            keyWordAssignDiv = Trim(A_LoopField170);
        }
        if (A_Index170 == 135) {
            keyWordAssignMod = Trim(A_LoopField170);
        }
        if (A_Index170 == 136) {
            keyWordAssignShiftLeft = Trim(A_LoopField170);
        }
        if (A_Index170 == 137) {
            keyWordAssignShiftRight = Trim(A_LoopField170);
        }
        if (A_Index170 == 138) {
            keyWordLogicalAssignShiftRight = Trim(A_LoopField170);
        }
        if (A_Index170 == 139) {
            keyWordAssignBitAnd = Trim(A_LoopField170);
        }
        if (A_Index170 == 140) {
            keyWordAssignBitOr = Trim(A_LoopField170);
        }
        if (A_Index170 == 141) {
            keyWordAssignBitXor = Trim(A_LoopField170);
        }
        if (A_Index170 == 142) {
            keyWordTernary1 = Trim(A_LoopField170);
        }
        if (A_Index170 == 143) {
            keyWordTernary2 = Trim(A_LoopField170);
        }
        if (A_Index170 == 144) {
            keyWordInc = Trim(A_LoopField170);
        }
        if (A_Index170 == 145) {
            keyWordDec = Trim(A_LoopField170);
        }
        if (A_Index170 == 146) {
            AHKlikeLoopsIndexedAt = Trim(A_LoopField170);
        }
        if (A_Index170 == 147) {
            keyWordAIndex = Trim(A_LoopField170);
        }
        if (A_Index170 == 148) {
            keyWordALoopField = Trim(A_LoopField170);
        }
        if (A_Index170 == 149) {
            useCurlyBraces = Trim(A_LoopField170);
        }
        if (A_Index170 == 150) {
            useEnd = Trim(A_LoopField170);
        }
        if (A_Index170 == 151) {
            useSemicolon = Trim(A_LoopField170);
        }
        if (A_Index170 == 152) {
            useParentheses = Trim(A_LoopField170);
        }
        if (A_Index170 == 153) {
            usePrefixTypeForTypeDefinition = Trim(A_LoopField170);
        }
        if (A_Index170 == 154) {
            usePostfixTypeForTypeDefinition = Trim(A_LoopField170);
        }
        if (A_Index170 == 155) {
            usePythonicColonSyntax = Trim(A_LoopField170);
        }
        if (A_Index170 == 156) {
            useCurlyBracesSyntaxForArrayDef = Trim(A_LoopField170);
        }
        if (A_Index170 == 157) {
            useInJavaScriptAlwaysUseVar = Trim(A_LoopField170);
        }
        if (A_Index170 == 158) {
            useJavaScriptInAfullHTMLfile = Trim(A_LoopField170);
        }
        if (A_Index170 == 159) {
            useJavaScriptAmainFuncDef = Trim(A_LoopField170);
        }
        if (A_Index170 == 160) {
            useJavaScriptAllFuncsAreAsync = Trim(A_LoopField170);
        }
        if (A_Index170 == 161) {
            useJavaScriptAlwaysTripleEqual = Trim(A_LoopField170);
        }
    }
    if (HTVM_Size(argHTVMinstrMORE) > 0) {
        ...
        std::vector<std::string> items171 = LoopParseFunc(allInstructionFile_2, "\n", "\r");
        for (size_t A_Index171 = 0; A_Index171 < items171.size() + 0; A_Index171++) {
            std::string A_LoopField171 = items171[A_Index171 - 0];
            if (A_Index171 == 0) {
                langToConvertTo_2 = Trim(A_LoopField171);
            }
            if (A_Index171 == 1) {
                langFileExtension_2 = Trim(A_LoopField171);
            }
            if (A_Index171 == 2) {
                commands_2 = Trim(A_LoopField171);
            }
            if (A_Index171 == 3) {
                keyWordAlliance_2 = Trim(A_LoopField171) + " ";
            }
            if (A_Index171 == 4) {
                keyWordCrew_2 = Trim(A_LoopField171) + " ";
            }
            if (A_Index171 == 5) {
                keyWordProc_2 = Trim(A_LoopField171) + " ";
            }
            if (A_Index171 == 6) {
                keyWordStruct_2 = Trim(A_LoopField171) + " ";
            }
            if (A_Index171 == 7) {
                keyWordProp_2 = Trim(A_LoopField171) + " ";
            }
            if (A_Index171 == 8) {
                keyWordThis_2 = Trim(A_LoopField171);
            }
            if (A_Index171 == 9) {
                keyWordInclude_2 = Trim(A_LoopField171);
            }
            if (A_Index171 == 10) {
                keyWordCodeInTheTranspiledLangStart_2 = Trim(A_LoopField171);
            }
            if (A_Index171 == 11) {
                keyWordCodeInTheTranspiledLangEnd_2 = Trim(A_LoopField171);
            }
            if (A_Index171 == 12) {
                keyWordCodeInTheTranspiledLangStartCPP_2 = Trim(A_LoopField171);
            }
            if (A_Index171 == 13) {
                keyWordCodeInTheTranspiledLangEndCPP_2 = Trim(A_LoopField171);
            }
            if (A_Index171 == 14) {
                keyWordCodeInTheTranspiledLangStartPY_2 = Trim(A_LoopField171);
            }
            if (A_Index171 == 15) {
                keyWordCodeInTheTranspiledLangEndPY_2 = Trim(A_LoopField171);
            }
            if (A_Index171 == 16) {
                keyWordCodeInTheTranspiledLangStartJS_2 = Trim(A_LoopField171);
            }
            if (A_Index171 == 17) {
                keyWordCodeInTheTranspiledLangEndJS_2 = Trim(A_LoopField171);
            }
            if (A_Index171 == 18) {
                keyWordCodeInTheTranspiledLangStartGO_2 = Trim(A_LoopField171);
            }
            if (A_Index171 == 19) {
                keyWordCodeInTheTranspiledLangEndGO_2 = Trim(A_LoopField171);
            }
            if (A_Index171 == 20) {
                keyWordCodeInTheTranspiledLangStartLUA_2 = Trim(A_LoopField171);
            }
            if (A_Index171 == 21) {
                keyWordCodeInTheTranspiledLangEndLUA_2 = Trim(A_LoopField171);
            }
            if (A_Index171 == 22) {
                keyWordCodeInTheTranspiledLangStartCS_2 = Trim(A_LoopField171);
            }
            if (A_Index171 == 23) {
                keyWordCodeInTheTranspiledLangEndCS_2 = Trim(A_LoopField171);
            }
            if (A_Index171 == 24) {
                keyWordCodeInTheTranspiledLangStartJAVA_2 = Trim(A_LoopField171);
            }
            if (A_Index171 == 25) {
                keyWordCodeInTheTranspiledLangEndJAVA_2 = Trim(A_LoopField171);
            }
            if (A_Index171 == 26) {
                keyWordCodeInTheTranspiledLangStartKT_2 = Trim(A_LoopField171);
            }
            if (A_Index171 == 27) {
                keyWordCodeInTheTranspiledLangEndKT_2 = Trim(A_LoopField171);
            }
            if (A_Index171 == 28) {
                keyWordCodeInTheTranspiledLangStartRB_2 = Trim(A_LoopField171);
            }
            if (A_Index171 == 29) {
                keyWordCodeInTheTranspiledLangEndRB_2 = Trim(A_LoopField171);
            }
            if (A_Index171 == 30) {
                keyWordCodeInTheTranspiledLangStartNIM_2 = Trim(A_LoopField171);
            }
            if (A_Index171 == 31) {
                keyWordCodeInTheTranspiledLangEndNIM_2 = Trim(A_LoopField171);
            }
            if (A_Index171 == 32) {
                keyWordCodeInTheTranspiledLangStartAHK_2 = Trim(A_LoopField171);
            }
            if (A_Index171 == 33) {
                keyWordCodeInTheTranspiledLangEndAHK_2 = Trim(A_LoopField171);
            }
            if (A_Index171 == 34) {
                keyWordCodeInTheTranspiledLangStartSWIFT_2 = Trim(A_LoopField171);
            }
            if (A_Index171 == 35) {
                keyWordCodeInTheTranspiledLangEndSWIFT_2 = Trim(A_LoopField171);
            }
            if (A_Index171 == 36) {
                keyWordCodeInTheTranspiledLangStartDART_2 = Trim(A_LoopField171);
            }
            if (A_Index171 == 37) {
                keyWordCodeInTheTranspiledLangEndDART_2 = Trim(A_LoopField171);
            }
            if (A_Index171 == 38) {
                keyWordCodeInTheTranspiledLangStartTS_2 = Trim(A_LoopField171);
            }
            if (A_Index171 == 39) {
                keyWordCodeInTheTranspiledLangEndTS_2 = Trim(A_LoopField171);
            }
            if (A_Index171 == 40) {
                keyWordCodeInTheTranspiledLangStartGROOVY_2 = Trim(A_LoopField171);
            }
            if (A_Index171 == 41) {
                keyWordCodeInTheTranspiledLangEndGROOVY_2 = Trim(A_LoopField171);
            }
            if (A_Index171 == 42) {
                keyWordCodeInTheTranspiledLangStartHTVM_2 = Trim(A_LoopField171);
            }
            if (A_Index171 == 43) {
                keyWordCodeInTheTranspiledLangEndHTVM_2 = Trim(A_LoopField171);
            }
            if (A_Index171 == 44) {
                keyWordCodeInHTVMstart_2 = Trim(A_LoopField171);
            }
            if (A_Index171 == 45) {
                keyWordCodeInHTVMend_2 = Trim(A_LoopField171);
            }
            if (A_Index171 == 46) {
                keyWordCurlyBraceOpen_2 = Trim(A_LoopField171);
            }
            if (A_Index171 == 47) {
                keyWordCurlyBraceClose_2 = Trim(A_LoopField171);
            }
            if (A_Index171 == 48) {
                keyWordNull_2 = Trim(A_LoopField171);
            }
            if (A_Index171 == 49) {
                keyWordTrue_2 = Trim(A_LoopField171);
            }
            if (A_Index171 == 50) {
                keyWordFalse_2 = Trim(A_LoopField171);
            }
            if (A_Index171 == 51) {
                keyWordVoid_2 = Trim(A_LoopField171);
            }
            if (A_Index171 == 52) {
                keyWordDouble_2 = Trim(A_LoopField171);
            }
            if (A_Index171 == 53) {
                keyWordChar_2 = Trim(A_LoopField171);
            }
            if (A_Index171 == 54) {
                keyWordUint8_2 = Trim(A_LoopField171);
            }
            if (A_Index171 == 55) {
                keyWordUint16_2 = Trim(A_LoopField171);
            }
            if (A_Index171 == 56) {
                keyWordUint32_2 = Trim(A_LoopField171);
            }
            if (A_Index171 == 57) {
                keyWordUint64_2 = Trim(A_LoopField171);
            }
            if (A_Index171 == 58) {
                keyWordINT_2 = Trim(A_LoopField171);
            }
            if (A_Index171 == 59) {
                keyWordSTR_2 = Trim(A_LoopField171);
            }
            if (A_Index171 == 60) {
                keyWordBOOL_2 = Trim(A_LoopField171);
            }
            if (A_Index171 == 61) {
                keyWordFLOAT_2 = Trim(A_LoopField171);
            }
            if (A_Index171 == 62) {
                keyWordINT8_2 = Trim(A_LoopField171);
            }
            if (A_Index171 == 63) {
                keyWordINT16_2 = Trim(A_LoopField171);
            }
            if (A_Index171 == 64) {
                keyWordINT32_2 = Trim(A_LoopField171);
            }
            if (A_Index171 == 65) {
                keyWordINT64_2 = Trim(A_LoopField171);
            }
            if (A_Index171 == 66) {
                keyWordIF_2 = Trim(A_LoopField171) + " ";
            }
            if (A_Index171 == 67) {
                keyWordElseIf_2 = Trim(A_LoopField171) + " ";
            }
            if (A_Index171 == 68) {
                keyWordElse_2 = Trim(A_LoopField171);
            }
            if (A_Index171 == 69) {
                keyWordWhileLoop_2 = Trim(A_LoopField171) + " ";
            }
            if (A_Index171 == 70) {
                keyWordLoopInfinite_2 = Trim(A_LoopField171);
            }
            if (A_Index171 == 71) {
                keyWordLoop_2 = Trim(A_LoopField171) + " ";
            }
            if (A_Index171 == 72) {
                keyWordLoopParse_2 = Trim(A_LoopField171) + " ";
            }
            if (A_Index171 == 73) {
                keyWordContinue_2 = Trim(A_LoopField171);
            }
            if (A_Index171 == 74) {
                keyWordBreak_2 = Trim(A_LoopField171);
            }
            if (A_Index171 == 75) {
                keyWordFunc_2 = Trim(A_LoopField171) + " ";
            }
            if (A_Index171 == 76) {
                keyWordAwait_2 = Trim(A_LoopField171) + " ";
            }
            if (A_Index171 == 77) {
                keyWordAsync_2 = Trim(A_LoopField171) + " ";
            }
            if (A_Index171 == 78) {
                keyWordThrow_2 = Trim(A_LoopField171) + " ";
            }
            if (A_Index171 == 79) {
                keyWordErrorMsg_2 = Trim(A_LoopField171);
            }
            if (A_Index171 == 80) {
                keyWordTry_2 = Trim(A_LoopField171);
            }
            if (A_Index171 == 81) {
                keyWordCatch_2 = Trim(A_LoopField171) + " ";
            }
            if (A_Index171 == 82) {
                keyWordFinally_2 = Trim(A_LoopField171);
            }
            if (A_Index171 == 83) {
                keyWordReturnStatement_2 = Trim(A_LoopField171) + " ";
            }
            if (A_Index171 == 84) {
                keyWordArrayAppend_2 = Trim(A_LoopField171);
            }
            if (A_Index171 == 85) {
                keyWordArrayPop_2 = Trim(A_LoopField171);
            }
            if (A_Index171 == 86) {
                keyWordArraySize_2 = Trim(A_LoopField171);
            }
            if (A_Index171 == 87) {
                keyWordArrayInsert_2 = Trim(A_LoopField171);
            }
            if (A_Index171 == 88) {
                keyWordArrayRemove_2 = Trim(A_LoopField171);
            }
            if (A_Index171 == 89) {
                keyWordArrayIndexOf_2 = Trim(A_LoopField171);
            }
            if (A_Index171 == 90) {
                keyWordArrayDefinition_2 = Trim(A_LoopField171);
            }
            if (A_Index171 == 91) {
                keyWordArrayOfIntegersDefinition_2 = Trim(A_LoopField171);
            }
            if (A_Index171 == 92) {
                keyWordArrayOfStringsDefinition_2 = Trim(A_LoopField171);
            }
            if (A_Index171 == 93) {
                keyWordArrayOfFloatingPointNumbersDefinition_2 = Trim(A_LoopField171);
            }
            if (A_Index171 == 94) {
                keyWordArrayOfBooleansDefinition_2 = Trim(A_LoopField171);
            }
            if (A_Index171 == 95) {
                keyWordVar_2 = Trim(A_LoopField171);
            }
            if (A_Index171 == 96) {
                keyWordLet_2 = Trim(A_LoopField171);
            }
            if (A_Index171 == 97) {
                keyWordConst_2 = Trim(A_LoopField171);
            }
            if (A_Index171 == 98) {
                keyWordEnd_2 = Trim(A_LoopField171);
            }
            if (A_Index171 == 99) {
                keyWordGlobal_2 = Trim(A_LoopField171) + " ";
            }
            if (A_Index171 == 100) {
                keyWordComment_2 = Trim(A_LoopField171);
            }
            if (A_Index171 == 101) {
                keyWordCommentOpenMultiLine_2 = Trim(A_LoopField171);
            }
            if (A_Index171 == 102) {
                keyWordCommentCloseMultiLine_2 = Trim(A_LoopField171);
            }
            if (A_Index171 == 103) {
                keyWordEscpaeChar_2 = Trim(A_LoopField171);
            }
            if (A_Index171 == 104) {
                keyWordMainLabel_2 = Trim(A_LoopField171);
            }
            if (A_Index171 == 105) {
                keyWordConcat_2 = Trim(A_LoopField171);
            }
            if (A_Index171 == 106) {
                keyWordAdd_2 = Trim(A_LoopField171);
            }
            if (A_Index171 == 107) {
                keyWordSub_2 = Trim(A_LoopField171);
            }
            if (A_Index171 == 108) {
                keyWordMul_2 = Trim(A_LoopField171);
            }
            if (A_Index171 == 109) {
                keyWordDiv_2 = Trim(A_LoopField171);
            }
            if (A_Index171 == 110) {
                keyWordMod_2 = Trim(A_LoopField171);
            }
            if (A_Index171 == 111) {
                keyWordExp_2 = Trim(A_LoopField171);
            }
            if (A_Index171 == 112) {
                keyWordEqual_2 = Trim(A_LoopField171);
            }
            if (A_Index171 == 113) {
                keyWordStrictEqual_2 = Trim(A_LoopField171);
            }
            if (A_Index171 == 114) {
                keyWordNotEqual_2 = Trim(A_LoopField171);
            }
            if (A_Index171 == 115) {
                keyWordGreater_2 = Trim(A_LoopField171);
            }
            if (A_Index171 == 116) {
                keyWordLess_2 = Trim(A_LoopField171);
            }
            if (A_Index171 == 117) {
                keyWordGreaterEqual_2 = Trim(A_LoopField171);
            }
            if (A_Index171 == 118) {
                keyWordLessEqual_2 = Trim(A_LoopField171);
            }
            if (A_Index171 == 119) {
                keyWordAnd_2 = Trim(A_LoopField171);
            }
            if (A_Index171 == 120) {
                keyWordOr_2 = Trim(A_LoopField171);
            }
            if (A_Index171 == 121) {
                keyWordNot_2 = Trim(A_LoopField171);
            }
            if (A_Index171 == 122) {
                keyWordBitAnd_2 = Trim(A_LoopField171);
            }
            if (A_Index171 == 123) {
                keyWordBitOr_2 = Trim(A_LoopField171);
            }
            if (A_Index171 == 124) {
                keyWordBitXor_2 = Trim(A_LoopField171);
            }
            if (A_Index171 == 125) {
                keyWordBitNot_2 = Trim(A_LoopField171);
            }
            if (A_Index171 == 126) {
                keyWordShiftLeft_2 = Trim(A_LoopField171);
            }
            if (A_Index171 == 127) {
                keyWordShiftRight_2 = Trim(A_LoopField171);
            }
            if (A_Index171 == 128) {
                keyWordShiftUnsignedRight_2 = Trim(A_LoopField171);
            }
            if (A_Index171 == 129) {
                keyWordAssign_2 = Trim(A_LoopField171);
            }
            if (A_Index171 == 130) {
                keyWordAssignAdd_2 = Trim(A_LoopField171);
            }
            if (A_Index171 == 131) {
                keyWordAssignConcat_2 = Trim(A_LoopField171);
            }
            if (A_Index171 == 132) {
                keyWordAssignSub_2 = Trim(A_LoopField171);
            }
            if (A_Index171 == 133) {
                keyWordAssignMul_2 = Trim(A_LoopField171);
            }
            if (A_Index171 == 134) {
                keyWordAssignDiv_2 = Trim(A_LoopField171);
            }
            if (A_Index171 == 135) {
                keyWordAssignMod_2 = Trim(A_LoopField171);
            }
            if (A_Index171 == 136) {
                keyWordAssignShiftLeft_2 = Trim(A_LoopField171);
            }
            if (A_Index171 == 137) {
                keyWordAssignShiftRight_2 = Trim(A_LoopField171);
            }
            if (A_Index171 == 138) {
                keyWordLogicalAssignShiftRight_2 = Trim(A_LoopField171);
            }
            if (A_Index171 == 139) {
                keyWordAssignBitAnd_2 = Trim(A_LoopField171);
            }
            if (A_Index171 == 140) {
                keyWordAssignBitOr_2 = Trim(A_LoopField171);
            }
            if (A_Index171 == 141) {
                keyWordAssignBitXor_2 = Trim(A_LoopField171);
            }
            if (A_Index171 == 142) {
                keyWordTernary1_2 = Trim(A_LoopField171);
            }
            if (A_Index171 == 143) {
                keyWordTernary2_2 = Trim(A_LoopField171);
            }
            if (A_Index171 == 144) {
                keyWordInc_2 = Trim(A_LoopField171);
            }
            if (A_Index171 == 145) {
                keyWordDec_2 = Trim(A_LoopField171);
            }
            if (A_Index171 == 146) {
                AHKlikeLoopsIndexedAt_2 = Trim(A_LoopField171);
            }
            if (A_Index171 == 147) {
                keyWordAIndex_2 = Trim(A_LoopField171);
            }
            if (A_Index171 == 148) {
                keyWordALoopField_2 = Trim(A_LoopField171);
            }
            if (A_Index171 == 149) {
                useCurlyBraces_2 = Trim(A_LoopField171);
            }
            if (A_Index171 == 150) {
                useEnd_2 = Trim(A_LoopField171);
            }
            if (A_Index171 == 151) {
                useSemicolon_2 = Trim(A_LoopField171);
            }
            if (A_Index171 == 152) {
                useParentheses_2 = Trim(A_LoopField171);
            }
            if (A_Index171 == 153) {
                usePrefixTypeForTypeDefinition_2 = Trim(A_LoopField171);
            }
            if (A_Index171 == 154) {
                usePostfixTypeForTypeDefinition_2 = Trim(A_LoopField171);
            }
            if (A_Index171 == 155) {
                usePythonicColonSyntax_2 = Trim(A_LoopField171);
            }
            if (A_Index171 == 156) {
                useCurlyBracesSyntaxForArrayDef_2 = Trim(A_LoopField171);
            }
            if (A_Index171 == 157) {
                useInJavaScriptAlwaysUseVar_2 = Trim(A_LoopField171);
            }
            if (A_Index171 == 158) {
                useJavaScriptInAfullHTMLfile_2 = Trim(A_LoopField171);
            }
            if (A_Index171 == 159) {
                useJavaScriptAmainFuncDef_2 = Trim(A_LoopField171);
            }
            if (A_Index171 == 160) {
                useJavaScriptAllFuncsAreAsync_2 = Trim(A_LoopField171);
            }
            if (A_Index171 == 161) {
                useJavaScriptAlwaysTripleEqual_2 = Trim(A_LoopField171);
            }
        }
    }
    ...
    */
    ...
    std::vector<std::string> items172 = LoopParseFunc(allInstructionFile, "\n", "\r");
    for (size_t A_Index172 = 0; A_Index172 < items172.size() + 0; A_Index172++) {
        std::string A_LoopField172 = items172[A_Index172 - 0];
        if (RegExMatch(Trim(A_LoopField172), "^name: [A-Za-z0-9_]+")) {
            availableFuncsInHTVMInHTVM += Trim(StrSplit(Trim(A_LoopField172), "name: ", 2)) + Chr(10);
        }
    }
    ...
    if (isNotHTVMfile == 0) {
        if (langToConvertTo != langFileExtension_2) {
            ...
            if (doseHaveInclude(code)) {
                ...
                for (int A_Index174 = 0; A_Index174 < 10000 + 0; A_Index174++) {
                    ...
                    for (size_t A_Index175 = 0; A_Index175 < items175.size() + 0; A_Index175++) {
                        ...
                        if (SubStr(StrLower(Trim(currentLine)), 1, StrLen(keyWordInclude + " ")) == StrLower(keyWordInclude + " ")) {
                            ...
                            for (int A_Index176 = 0; A_Index176 < HTVM_Size(includedFilePaths) + 0; A_Index176++) {
                                if (filePathToInclude == includedFilePaths[A_Index176]) {
        ...
        for (size_t A_Index177 = 0; A_Index177 < items177.size() + 0; A_Index177++) {
            ...
            if (Trim(StrLower(A_LoopField177)) == StrLower(keyWordCodeInTheTranspiledLangEnd)) {
                COUNT_programmingBlock_InTheTranspiledLang++;
                holdTempDataProgrammingBlockThenPutInArr = StringTrimRight(holdTempDataProgrammingBlockThenPutInArr, 1);
                programmingBlocksTemp += "programmingBlock_InTheTranspiledLang-programmingBlock_InTheTranspiledLang-AA" + STR(COUNT_programmingBlock_InTheTranspiledLang) + "AA\n";
                HTVM_Append(programmingBlock_InTheTranspiledLang, Chr(10) + holdTempDataProgrammingBlockThenPutInArr);
            ...
            else if (Trim(StrLower(A_LoopField177)) == StrLower(keyWordCodeInTheTranspiledLangEndCPP)) {
                COUNT_programmingBlock_CPP++;
                holdTempDataProgrammingBlockThenPutInArr = StringTrimRight(holdTempDataProgrammingBlockThenPutInArr, 1);
                programmingBlocksTemp += "programmingBlock_CPP-programmingBlock_CPP-AA" + STR(COUNT_programmingBlock_CPP) + "AA\n";
                HTVM_Append(programmingBlock_CPP, Chr(10) + holdTempDataProgrammingBlockThenPutInArr);
            ...
            else if (Trim(StrLower(A_LoopField177)) == StrLower(keyWordCodeInTheTranspiledLangEndPY)) {
                COUNT_programmingBlock_PY++;
                holdTempDataProgrammingBlockThenPutInArr = StringTrimRight(holdTempDataProgrammingBlockThenPutInArr, 1);
                programmingBlocksTemp += "programmingBlock_PY-programmingBlock_PY-AA" + STR(COUNT_programmingBlock_PY) + "AA\n";
                HTVM_Append(programmingBlock_PY, Chr(10) + holdTempDataProgrammingBlockThenPutInArr);
            ...
            else if (Trim(StrLower(A_LoopField177)) == StrLower(keyWordCodeInTheTranspiledLangEndJS)) {
                COUNT_programmingBlock_JS++;
                holdTempDataProgrammingBlockThenPutInArr = StringTrimRight(holdTempDataProgrammingBlockThenPutInArr, 1);
                programmingBlocksTemp += "programmingBlock_JS-programmingBlock_JS-AA" + STR(COUNT_programmingBlock_JS) + "AA\n";
                HTVM_Append(programmingBlock_JS, Chr(10) + holdTempDataProgrammingBlockThenPutInArr);
            ...
            else if (Trim(StrLower(A_LoopField177)) == StrLower(keyWordCodeInTheTranspiledLangEndGO)) {
                COUNT_programmingBlock_GO++;
                holdTempDataProgrammingBlockThenPutInArr = StringTrimRight(holdTempDataProgrammingBlockThenPutInArr, 1);
                programmingBlocksTemp += "programmingBlock_GO-programmingBlock_GO-AA" + STR(COUNT_programmingBlock_GO) + "AA\n";
                HTVM_Append(programmingBlock_GO, Chr(10) + holdTempDataProgrammingBlockThenPutInArr);
            ...
            else if (Trim(StrLower(A_LoopField177)) == StrLower(keyWordCodeInTheTranspiledLangEndLUA)) {
                COUNT_programmingBlock_LUA++;
                holdTempDataProgrammingBlockThenPutInArr = StringTrimRight(holdTempDataProgrammingBlockThenPutInArr, 1);
                programmingBlocksTemp += "programmingBlock_LUA-programmingBlock_LUA-AA" + STR(COUNT_programmingBlock_LUA) + "AA\n";
                HTVM_Append(programmingBlock_LUA, Chr(10) + holdTempDataProgrammingBlockThenPutInArr);
            ...
            else if (Trim(StrLower(A_LoopField177)) == StrLower(keyWordCodeInTheTranspiledLangEndCS)) {
                COUNT_programmingBlock_CS++;
                holdTempDataProgrammingBlockThenPutInArr = StringTrimRight(holdTempDataProgrammingBlockThenPutInArr, 1);
                programmingBlocksTemp += "programmingBlock_CS-programmingBlock_CS-AA" + STR(COUNT_programmingBlock_CS) + "AA\n";
                HTVM_Append(programmingBlock_CS, Chr(10) + holdTempDataProgrammingBlockThenPutInArr);
            ...
            else if (Trim(StrLower(A_LoopField177)) == StrLower(keyWordCodeInTheTranspiledLangEndJAVA)) {
                COUNT_programmingBlock_JAVA++;
                holdTempDataProgrammingBlockThenPutInArr = StringTrimRight(holdTempDataProgrammingBlockThenPutInArr, 1);
                programmingBlocksTemp += "programmingBlock_JAVA-programmingBlock_JAVA-AA" + STR(COUNT_programmingBlock_JAVA) + "AA\n";
                HTVM_Append(programmingBlock_JAVA, Chr(10) + holdTempDataProgrammingBlockThenPutInArr);
            ...
            else if (Trim(StrLower(A_LoopField177)) == StrLower(keyWordCodeInTheTranspiledLangEndKT)) {
                COUNT_programmingBlock_KT++;
                holdTempDataProgrammingBlockThenPutInArr = StringTrimRight(holdTempDataProgrammingBlockThenPutInArr, 1);
                programmingBlocksTemp += "programmingBlock_KT-programmingBlock_KT-AA" + STR(COUNT_programmingBlock_KT) + "AA\n";
                HTVM_Append(programmingBlock_KT, Chr(10) + holdTempDataProgrammingBlockThenPutInArr);
            ...
            else if (Trim(StrLower(A_LoopField177)) == StrLower(keyWordCodeInTheTranspiledLangEndRB)) {
                COUNT_programmingBlock_RB++;
                holdTempDataProgrammingBlockThenPutInArr = StringTrimRight(holdTempDataProgrammingBlockThenPutInArr, 1);
                programmingBlocksTemp += "programmingBlock_RB-programmingBlock_RB-AA" + STR(COUNT_programmingBlock_RB) + "AA\n";
                HTVM_Append(programmingBlock_RB, Chr(10) + holdTempDataProgrammingBlockThenPutInArr);
            ...
            else if (Trim(StrLower(A_LoopField177)) == StrLower(keyWordCodeInTheTranspiledLangEndNIM)) {
                COUNT_programmingBlock_NIM++;
                holdTempDataProgrammingBlockThenPutInArr = StringTrimRight(holdTempDataProgrammingBlockThenPutInArr, 1);
                programmingBlocksTemp += "programmingBlock_NIM-programmingBlock_NIM-AA" + STR(COUNT_programmingBlock_NIM) + "AA\n";
                HTVM_Append(programmingBlock_NIM, Chr(10) + holdTempDataProgrammingBlockThenPutInArr);
            ...
            else if (Trim(StrLower(A_LoopField177)) == StrLower(keyWordCodeInTheTranspiledLangEndAHK)) {
                COUNT_programmingBlock_AHK++;
                holdTempDataProgrammingBlockThenPutInArr = StringTrimRight(holdTempDataProgrammingBlockThenPutInArr, 1);
                programmingBlocksTemp += "programmingBlock_AHK-programmingBlock_AHK-AA" + STR(COUNT_programmingBlock_AHK) + "AA\n";
                HTVM_Append(programmingBlock_AHK, Chr(10) + holdTempDataProgrammingBlockThenPutInArr);
            ...
            else if (Trim(StrLower(A_LoopField177)) == StrLower(keyWordCodeInTheTranspiledLangEndSWIFT)) {
                COUNT_programmingBlock_SWIFT++;
                holdTempDataProgrammingBlockThenPutInArr = StringTrimRight(holdTempDataProgrammingBlockThenPutInArr, 1);
                programmingBlocksTemp += "programmingBlock_SWIFT-programmingBlock_SWIFT-AA" + STR(COUNT_programmingBlock_SWIFT) + "AA\n";
                HTVM_Append(programmingBlock_SWIFT, Chr(10) + holdTempDataProgrammingBlockThenPutInArr);
            ...
            else if (Trim(StrLower(A_LoopField177)) == StrLower(keyWordCodeInTheTranspiledLangEndDART)) {
                COUNT_programmingBlock_DART++;
                holdTempDataProgrammingBlockThenPutInArr = StringTrimRight(holdTempDataProgrammingBlockThenPutInArr, 1);
                programmingBlocksTemp += "programmingBlock_DART-programmingBlock_DART-AA" + STR(COUNT_programmingBlock_DART) + "AA\n";
                HTVM_Append(programmingBlock_DART, Chr(10) + holdTempDataProgrammingBlockThenPutInArr);
            ...
            else if (Trim(StrLower(A_LoopField177)) == StrLower(keyWordCodeInTheTranspiledLangEndTS)) {
                COUNT_programmingBlock_TS++;
                holdTempDataProgrammingBlockThenPutInArr = StringTrimRight(holdTempDataProgrammingBlockThenPutInArr, 1);
                programmingBlocksTemp += "programmingBlock_TS-programmingBlock_TS-AA" + STR(COUNT_programmingBlock_TS) + "AA\n";
                HTVM_Append(programmingBlock_TS, Chr(10) + holdTempDataProgrammingBlockThenPutInArr);
            ...
            else if (Trim(StrLower(A_LoopField177)) == StrLower(keyWordCodeInTheTranspiledLangEndGROOVY)) {
                COUNT_programmingBlock_GROOVY++;
                holdTempDataProgrammingBlockThenPutInArr = StringTrimRight(holdTempDataProgrammingBlockThenPutInArr, 1);
                programmingBlocksTemp += "programmingBlock_GROOVY-programmingBlock_GROOVY-AA" + STR(COUNT_programmingBlock_GROOVY) + "AA\n";
                HTVM_Append(programmingBlock_GROOVY, Chr(10) + holdTempDataProgrammingBlockThenPutInArr);
            ...
            else if (Trim(StrLower(A_LoopField177)) == StrLower(keyWordCodeInTheTranspiledLangEndHTVM)) {
                COUNT_programmingBlock_HTVM++;
                holdTempDataProgrammingBlockThenPutInArr = StringTrimRight(holdTempDataProgrammingBlockThenPutInArr, 1);
                programmingBlocksTemp += "programmingBlock_HTVM-programmingBlock_HTVM-AA" + STR(COUNT_programmingBlock_HTVM) + "AA\n";
                HTVM_Append(programmingBlock_HTVM, Chr(10) + holdTempDataProgrammingBlockThenPutInArr);
            ...
            else if (Trim(StrLower(A_LoopField177)) == StrLower(keyWordCodeInHTVMend)) {
                //;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;
                //;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;
                COUNT_programmingBlock_HTVMsyntax++;
                holdTempDataProgrammingBlockThenPutInArr = StringTrimRight(holdTempDataProgrammingBlockThenPutInArr, 1);
                ...
                holdTempDataProgrammingBlockThenPutInArr += A_LoopField177 + Chr(10);
            }
            else if (Trim(StrLower(A_LoopField177)) == StrLower(keyWordCodeInTheTranspiledLangStart)) {
                holdTempDataProgrammingBlockThenPutInArr = "";
                inProgarmmingBlock = 1;
            }
            else if (Trim(StrLower(A_LoopField177)) == StrLower(keyWordCodeInTheTranspiledLangStartCPP)) {
                holdTempDataProgrammingBlockThenPutInArr = "";
                inProgarmmingBlock = 1;
            }
            else if (Trim(StrLower(A_LoopField177)) == StrLower(keyWordCodeInTheTranspiledLangStartPY)) {
                holdTempDataProgrammingBlockThenPutInArr = "";
                inProgarmmingBlock = 1;
            }
            else if (Trim(StrLower(A_LoopField177)) == StrLower(keyWordCodeInTheTranspiledLangStartJS)) {
                holdTempDataProgrammingBlockThenPutInArr = "";
                inProgarmmingBlock = 1;
            }
            else if (Trim(StrLower(A_LoopField177)) == StrLower(keyWordCodeInTheTranspiledLangStartGO)) {
                holdTempDataProgrammingBlockThenPutInArr = "";
                inProgarmmingBlock = 1;
            }
            else if (Trim(StrLower(A_LoopField177)) == StrLower(keyWordCodeInTheTranspiledLangStartLUA)) {
                holdTempDataProgrammingBlockThenPutInArr = "";
                inProgarmmingBlock = 1;
            }
            else if (Trim(StrLower(A_LoopField177)) == StrLower(keyWordCodeInTheTranspiledLangStartCS)) {
                holdTempDataProgrammingBlockThenPutInArr = "";
                inProgarmmingBlock = 1;
            }
            else if (Trim(StrLower(A_LoopField177)) == StrLower(keyWordCodeInTheTranspiledLangStartJAVA)) {
                holdTempDataProgrammingBlockThenPutInArr = "";
                inProgarmmingBlock = 1;
            }
            else if (Trim(StrLower(A_LoopField177)) == StrLower(keyWordCodeInTheTranspiledLangStartKT)) {
                holdTempDataProgrammingBlockThenPutInArr = "";
                inProgarmmingBlock = 1;
            }
            else if (Trim(StrLower(A_LoopField177)) == StrLower(keyWordCodeInTheTranspiledLangStartRB)) {
                holdTempDataProgrammingBlockThenPutInArr = "";
                inProgarmmingBlock = 1;
            }
            else if (Trim(StrLower(A_LoopField177)) == StrLower(keyWordCodeInTheTranspiledLangStartNIM)) {
                holdTempDataProgrammingBlockThenPutInArr = "";
                inProgarmmingBlock = 1;
            }
            else if (Trim(StrLower(A_LoopField177)) == StrLower(keyWordCodeInTheTranspiledLangStartAHK)) {
                holdTempDataProgrammingBlockThenPutInArr = "";
                inProgarmmingBlock = 1;
            }
            else if (Trim(StrLower(A_LoopField177)) == StrLower(keyWordCodeInTheTranspiledLangStartSWIFT)) {
                holdTempDataProgrammingBlockThenPutInArr = "";
                inProgarmmingBlock = 1;
            }
            else if (Trim(StrLower(A_LoopField177)) == StrLower(keyWordCodeInTheTranspiledLangStartDART)) {
                holdTempDataProgrammingBlockThenPutInArr = "";
                inProgarmmingBlock = 1;
            }
            else if (Trim(StrLower(A_LoopField177)) == StrLower(keyWordCodeInTheTranspiledLangStartTS)) {
                holdTempDataProgrammingBlockThenPutInArr = "";
                inProgarmmingBlock = 1;
            }
            else if (Trim(StrLower(A_LoopField177)) == StrLower(keyWordCodeInTheTranspiledLangStartGROOVY)) {
                holdTempDataProgrammingBlockThenPutInArr = "";
                inProgarmmingBlock = 1;
            }
            else if (Trim(StrLower(A_LoopField177)) == StrLower(keyWordCodeInTheTranspiledLangStartHTVM)) {
                holdTempDataProgrammingBlockThenPutInArr = "";
                inProgarmmingBlock = 1;
            }
            else if (Trim(StrLower(A_LoopField177)) == StrLower(keyWordCodeInHTVMstart)) {
                //;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;
                //;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;
                holdTempDataProgrammingBlockThenPutInArr = "";
                inProgarmmingBlock = 1;
                ...
                programmingBlocksTemp += A_LoopField177 + Chr(10);
            }
        }
        code = StringTrimRight(programmingBlocksTemp, 1);
        //;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;; ----------------
    ...
    if (isNotHTVMfile == 0) {
        ...
        for (size_t A_Index178 = 0; A_Index178 < items178.size() + 0; A_Index178++) {
            ...
            if (SubStr(Trim(A_LoopField178), 1, StrLen(keyWordComment)) == keyWordComment) {
                commentsBugFixHold = StringTrimLeft(Trim(A_LoopField178), StrLen(keyWordComment));
                std::vector<std::string> items179 = LoopParseFunc(A_LoopField178);
                for (size_t A_Index179 = 0; A_Index179 < items179.size() + 0; A_Index179++) {
                    std::string A_LoopField179 = items179[A_Index179 - 0];
                    if (InStr(A_LoopField179, " ") || InStr(A_LoopField179, Chr(9))) {
                        commentsBugFixHold1 = A_LoopField179;
                    } else {
                        break;
                    }
                }
                ...
                commentsBugFixOut += A_LoopField178 + Chr(10);
            }
        }
        ...
        std::vector<std::string> items181 = LoopParseFunc(code);
        for (size_t A_Index181 = 0; A_Index181 < items181.size() + 0; A_Index181++) {
            ...
            HTVM_Append(getAllCharForTheFurtureSoIcanAddEscapeChar, A_LoopField181);
        }
        ...
        for (size_t A_Index182 = 0; A_Index182 < items182.size() + 0; A_Index182++) {
            ...
            if (A_LoopField182 == keyWordEscpaeChar && getAllCharForTheFurtureSoIcanAddEscapeChar[A_Index182 + 1] == Chr(34)) {
                ...
                if (fixOutFixDoubleQuotesInsideDoubleQuotesFIXok != 1) {
                    OutFixDoubleQuotesInsideDoubleQuotes += A_LoopField182;
                } else {
        ...
        if (keyWordEscpaeChar == Chr(92) && langToConvertTo != langFileExtension_2) {
            ...
            for (size_t A_Index183 = 0; A_Index183 < items183.size() + 0; A_Index183++) {
                ...
                if (A_LoopField183 == Chr(34)) {
                    areWEinSome34sNum++;
                }
                if (areWEinSome34sNum == 1) {
                    if (A_LoopField183 != Chr(34)) {
                        if (A_LoopField183 == keyWordEscpaeChar && langToConvertTo != langFileExtension_2) {
                            theIdNumOfThe34theVar[theIdNumOfThe34] = theIdNumOfThe34theVar[theIdNumOfThe34] + Chr(92);
                        } else {
                            theIdNumOfThe34theVar[theIdNumOfThe34] = theIdNumOfThe34theVar[theIdNumOfThe34] + A_LoopField183;
                        }
                    } else {
                        theIdNumOfThe34++;
                ...
                if (areWEinSome34sNum == 2 || areWEinSome34sNum == 0) {
                    if (A_LoopField183 != Chr(34)) {
                        htCodeOUT754754 += A_LoopField183;
                    }
                    areWEinSome34sNum = 0;
                }
            }
            ...
            for (size_t A_Index184 = 0; A_Index184 < items184.size() + 0; A_Index184++) {
                ...
                if (A_LoopField184 == Chr(34)) {
                    areWEinSome34sNum++;
                }
                if (areWEinSome34sNum == 1) {
                    if (A_LoopField184 != Chr(34)) {
                        if (A_LoopField184 == keyWordEscpaeChar && keyWordEscpaeChar == getAllCharForTheFurtureSoIcanAddEscapeChar[A_Index184 + 1] && langToConvertTo != langFileExtension_2) {
                        ...
                        else if (A_LoopField184 == keyWordEscpaeChar && langToConvertTo != langFileExtension_2) {
                            ...
                            theIdNumOfThe34theVar[theIdNumOfThe34] = theIdNumOfThe34theVar[theIdNumOfThe34] + A_LoopField184;
                        }
                    } else {
                        theIdNumOfThe34++;
                        htCodeOUT754754 += "ihuiuuhuuhtheidFor--asds" + str21 + "as--" + str21 + "theuhturtyphoutr--" + Chr(65) + Chr(65) + STR(theIdNumOfThe34) + Chr(65) + Chr(65);
                ...
                if (areWEinSome34sNum == 2 || areWEinSome34sNum == 0) {
                    if (A_LoopField184 != Chr(34)) {
                        htCodeOUT754754 += A_LoopField184;
                    }
                    areWEinSome34sNum = 0;
                }
            }
        ...
        for (int A_Index185 = 0; A_Index185 < theIdNumOfThe34 + 0; A_Index185++) {
            theIdNumOfThe34theVar[A_Index185] = theIdNumOfThe34theVar[A_Index185] + Chr(34);
        }
        HTVM_Append(theIdNumOfThe34theVar, Chr(34));
        if (langToConvertTo == "dart" || langToConvertTo == "groovy") {
            for (int A_Index186 = 0; A_Index186 < HTVM_Size(theIdNumOfThe34theVar) + 0; A_Index186++) {
                theIdNumOfThe34theVar[A_Index186] = StrReplace(theIdNumOfThe34theVar[A_Index186], "$", Chr(92) + "$");
            }
        }
        ...
        for (size_t A_Index187 = 0; A_Index187 < items187.size() + 0; A_Index187++) {
            ...
            if (InStr(A_LoopField187, " " + keyWordComment) != false && SubStr(Trim(A_LoopField187), 1, StrLen(keyWordComment)) != keyWordComment) {
                posForRemoveCommentsOnTheEndOfTheLine = 0;
                slidingWinFixCommentsRmI = 0;
                slidingWinFixCommentsRm = slidingWinFixCommentsRmFUNC(A_LoopField187);
                for (int A_Index188 = 0; A_Index188 < HTVM_Size(slidingWinFixCommentsRm) + 0; A_Index188++) {
                    slidingWinFixCommentsRmI = A_Index188;
                    slidingWinFixCommentsRmHold = "";
                    for (int A_Index189 = 0; A_Index189 < StrLen(" " + keyWordComment) + 0; A_Index189++) {
                        slidingWinFixCommentsRmHold += slidingWinFixCommentsRm[slidingWinFixCommentsRmI + A_Index189];
                    }
                    if (Trim(slidingWinFixCommentsRmHold) == keyWordComment) {
                        posForRemoveCommentsOnTheEndOfTheLine = A_Index188;
                        break;
                    }
                }
                if (posForRemoveCommentsOnTheEndOfTheLine > 0) {
                    // Keep only the part before the comment
                    newStringOutCode += SubStr(A_LoopField187, 1, posForRemoveCommentsOnTheEndOfTheLine) + Chr(10);
                } else {
                    // Keep the whole line as it is
                    newStringOutCode += A_LoopField187 + Chr(10);
                }
            } else {
                newStringOutCode += A_LoopField187 + Chr(10);
            }
        ...
        if (langToConvertTo != langFileExtension_2) {
            ...
            for (size_t A_Index190 = 0; A_Index190 < items190.size() + 0; A_Index190++) {
                ...
                if (SubStr(Trim(A_LoopField190), 1, 1) != ";") {
                    semiColonForOneLinerOut += StrReplace(A_LoopField190, ";", Chr(10)) + Chr(10);
                } else {
                    semiColonForOneLinerOut += A_LoopField190 + Chr(10);
                }
            }
        ...
        if (useCurlyBraces == "on") {
            ...
            for (size_t A_Index191 = 0; A_Index191 < items191.size() + 0; A_Index191++) {
                ...
                str1 = "";
                if (InStr(A_LoopField191, keyWordCurlyBraceOpen) || InStr(A_LoopField191, keyWordCurlyBraceClose)) {
                    if (SubStr(Trim(A_LoopField191), 1, StrLen(Trim(keyWordComment))) == Trim(keyWordComment) && isLineAconstruct(Trim(A_LoopField191)) == false) {
                        fixSomeCurlyBraceForExratFlexability += A_LoopField191 + Chr(10);
                    } else {
                        if (InStr(A_LoopField191, Trim(keyWordArrayDefinition) + " " + keyWordAssign + " ") || InStr(A_LoopField191, Trim(keyWordArrayOfIntegersDefinition) + " " + keyWordAssign + " ") || InStr(A_LoopField191, Trim(keyWordArrayOfStringsDefinition) + " " + keyWordAssign + " ") || InStr(A_LoopField191, Trim(keyWordArrayOfFloatingPointNumbersDefinition) + " " + keyWordAssign + " ") || InStr(A_LoopField191, Trim(keyWordArrayOfBooleansDefinition) + " " + keyWordAssign + " ") && useCurlyBracesSyntaxForArrayDef == "on") {
                            fixSomeCurlyBraceForExratFlexability += A_LoopField191 + Chr(10);
                        } else {
                            str1 = StrReplace(A_LoopField191, keyWordCurlyBraceOpen, "{");
            ...
            for (size_t A_Index192 = 0; A_Index192 < items192.size() + 0; A_Index192++) {
                ...
                fixSomeCurlyBraceForExratFlexabilityTriHELP += Trim(A_LoopField192) + Chr(10);
            }
            ...
            if (keyWordCurlyBraceOpen != "{") {
                ...
                for (size_t A_Index193 = 0; A_Index193 < items193.size() + 0; A_Index193++) {
                    ...
                    htCodeOutFixINT2 = 0;
                    if (isLineAconstruct(Trim(A_LoopField193)) == true || Trim(A_LoopField193) == keyWordCurlyBraceOpen) {
                        htCodeOutFixINT2 = 1;
                    }
                    htCodeOutFixINT = 0;
                    std::vector<std::string> items194 = LoopParseFunc(A_LoopField193, " ");
                    for (size_t A_Index194 = 0; A_Index194 < items194.size() + 0; A_Index194++) {
                        std::string A_LoopField194 = items194[A_Index194 - 0];
                        htCodeOutFixINT++;
                    }
                    htCodeOutFixINT--;
                    std::vector<std::string> items195 = LoopParseFunc(A_LoopField193, " ");
                    for (size_t A_Index195 = 0; A_Index195 < items195.size() + 0; A_Index195++) {
                        std::string A_LoopField195 = items195[A_Index195 - 0];
                        if (htCodeOutFixINT == A_Index195 && htCodeOutFixINT2 == 1) {
                            htCodeOutFixINT2 = 0;
                            htCodeOutFixEnd += HTVMmatchStrRrplace(A_LoopField195, keyWordCurlyBraceOpen, "{") + " ";
                        } else {
            ...
            if (keyWordCurlyBraceClose != "}") {
                ...
                for (size_t A_Index196 = 0; A_Index196 < items196.size() + 0; A_Index196++) {
                    ...
                    htCodeOutFixINT2 = 0;
                    if (isLineAconstruct(Trim(A_LoopField196)) == true || Trim(A_LoopField196) == keyWordCurlyBraceClose) {
                        htCodeOutFixINT2 = 1;
                    }
                    std::vector<std::string> items197 = LoopParseFunc(A_LoopField196, " ");
                    for (size_t A_Index197 = 0; A_Index197 < items197.size() + 0; A_Index197++) {
                        std::string A_LoopField197 = items197[A_Index197 - 0];
                        if (htCodeOutFixINT2 == 1 || Trim(A_LoopField197) == keyWordCurlyBraceClose) {
                            htCodeOutFixINT2 = 0;
                            htCodeOutFixEnd += HTVMmatchStrRrplace(A_LoopField197, keyWordCurlyBraceClose, "}") + " ";
                        } else {
                            htCodeOutFixEnd += A_LoopField197 + " ";
                        }
                    }
                    htCodeOutFixEnd = StringTrimRight(htCodeOutFixEnd, 1);
                    htCodeOutFixEnd += Chr(10);
        ...
        if (useCurlyBraces == "on") {
            ...
            for (size_t A_Index198 = 0; A_Index198 < items198.size() + 0; A_Index198++) {
                ...
                HTVM_Append(addNoCurlyFixWhenNextLineAfterConstructLookAhead, Trim(A_LoopField198));
            }
            ...
            for (size_t A_Index199 = 0; A_Index199 < items199.size() + 0; A_Index199++) {
                ...
                if (isLineAconstruct(Trim(A_LoopField199)) && addNoCurlyFixWhenNextLineAfterConstructLookAhead[A_Index199 + 1] != "{") {
                    int5 = 1;
                    addNoCurlyFixWhenNextLineAfterConstructOut += A_LoopField199 + Chr(10) + "{" + Chr(10);
                } else {
                    if (int5 == 1) {
                        int5 = 0;
                        addNoCurlyFixWhenNextLineAfterConstructOut += A_LoopField199 + Chr(10) + "}" + Chr(10);
                    } else {
                        addNoCurlyFixWhenNextLineAfterConstructOut += A_LoopField199 + Chr(10);
                    }
                }
            }
        ...
        if (useEnd == "on") {
            ...
            for (size_t A_Index200 = 0; A_Index200 < items200.size() + 0; A_Index200++) {
                ...
                if (StrLower(Trim(A_LoopField200)) == StrLower(keyWordEnd)) {
                    htCodeOutFixEnd += "}\n";
                }
                else if (SubStr(StrLower(Trim(A_LoopField200)), 1, StrLen(StrLower(keyWordIF))) == StrLower(keyWordIF) && isLineAconstruct(Trim(A_LoopField200)) == true) {
                    htCodeOutFixEnd += A_LoopField200 + "\n{\n";
                }
                else if (SubStr(StrLower(Trim(A_LoopField200)), 1, StrLen(StrLower(keyWordWhileLoop))) == StrLower(keyWordWhileLoop) && isLineAconstruct(Trim(A_LoopField200)) == true) {
                    htCodeOutFixEnd += A_LoopField200 + "\n{\n";
                }
                else if (SubStr(StrLower(Trim(A_LoopField200)), 1, StrLen(StrLower(keyWordElseIf))) == StrLower(keyWordElseIf) && isLineAconstruct(Trim(A_LoopField200)) == true) {
                    htCodeOutFixEnd += "\n}\n" + A_LoopField200 + "\n{\n";
                }
                else if (SubStr(StrLower(Trim(A_LoopField200)), 1, StrLen(StrLower(keyWordElse))) == StrLower(keyWordElse) && isLineAconstruct(Trim(A_LoopField200)) == true) {
                    htCodeOutFixEnd += "\n}\n" + A_LoopField200 + "\n{\n";
                }
                else if (SubStr(StrLower(Trim(A_LoopField200)), 1, StrLen(StrLower(keyWordCatch))) == StrLower(keyWordCatch) && isLineAconstruct(Trim(A_LoopField200)) == true) {
                    htCodeOutFixEnd += "\n}\n" + A_LoopField200 + "\n{\n";
                }
                else if (SubStr(StrLower(Trim(A_LoopField200)), 1, StrLen(StrLower(keyWordFinally))) == StrLower(keyWordFinally) && isLineAconstruct(Trim(A_LoopField200)) == true) {
                    htCodeOutFixEnd += "\n}\n" + A_LoopField200 + "\n{\n";
                }
                else if (isLineAconstruct(Trim(A_LoopField200)) == true) {
                    htCodeOutFixEnd += A_LoopField200 + "\n{\n";
                } else {
        ...
        if (usePythonicColonSyntax == "off") {
            if (useEnd == "off" && useCurlyBraces == "off") {
                ...
                for (size_t A_Index201 = 0; A_Index201 < items201.size() + 0; A_Index201++) {
                    ...
                    if (isLineAconstruct(Trim(A_LoopField201)) == true) {
                        htCodeOutFixEnd += A_LoopField201 + ":\n";
                    } else {
                        htCodeOutFixEnd += A_LoopField201 + Chr(10);
                    }
                }
                code = StringTrimRight(htCodeOutFixEnd, 1);
            }
        ...
        for (int A_Index203 = 0; A_Index203 < 2 + 0; A_Index203++) {
            ...
            for (size_t A_Index204 = 0; A_Index204 < items204.size() + 0; A_Index204++) {
                ...
                if (InStr(A_LoopField204, Trim(keyWordArrayDefinition) + " " + keyWordAssign + " ") || InStr(A_LoopField204, Trim(keyWordArrayOfIntegersDefinition) + " " + keyWordAssign + " ") || InStr(A_LoopField204, Trim(keyWordArrayOfStringsDefinition) + " " + keyWordAssign + " ") || InStr(A_LoopField204, Trim(keyWordArrayOfFloatingPointNumbersDefinition) + " " + keyWordAssign + " ") || InStr(A_LoopField204, Trim(keyWordArrayOfBooleansDefinition) + " " + keyWordAssign + " ") && useCurlyBracesSyntaxForArrayDef == "on") {
                    if (SubStr(StrLower(A_LoopField204), 1, StrLen(StrLower(keyWordFunc))) == StrLower(keyWordFunc) || SubStr(StrLower(A_LoopField204), 1, StrLen(StrLower(keyWordAsync) + StrLower(keyWordFunc))) == StrLower(keyWordAsync) + StrLower(keyWordFunc)) {
                        if (InStr(Trim(A_LoopField204), "{") && Trim(A_LoopField204) != "{" && SubStr(Trim(A_LoopField204), 1, StrLen(Trim(keyWordComment))) != Trim(keyWordComment)) {
                            outCodeFixBraces += Trim(StrReplace(Trim(A_LoopField204), "{", "")) + "\n{\n";
                        }
                        else if (InStr(Trim(A_LoopField204), "}") && Trim(A_LoopField204) != "}" && SubStr(Trim(A_LoopField204), 1, StrLen(Trim(keyWordComment))) != Trim(keyWordComment)) {
                            outCodeFixBraces += "}\n" + Trim(StrReplace(Trim(A_LoopField204), "}", "")) + Chr(10);
                        } else {
                            outCodeFixBraces += Trim(A_LoopField204) + Chr(10);
                        }
                    } else {
                        outCodeFixBraces += Trim(A_LoopField204) + Chr(10);
                    }
        ...
        if (usePythonicColonSyntax == "on" || useCurlyBraces == "off" && useEnd == "off") {
            ...
            for (size_t A_Index205 = 0; A_Index205 < items205.size() + 0; A_Index205++) {
                ...
                if (isLineAconstruct(Trim(A_LoopField205)) == true) {
                    htCodeOutFixEnd += StringTrimRight(A_LoopField205, 1) + Chr(10);
                } else {
                    htCodeOutFixEnd += A_LoopField205 + Chr(10);
                }
            }
            code = StringTrimRight(htCodeOutFixEnd, 1);
        }
        ...
        if (isNotHTVMfile2 == 0) {
            ...
            for (size_t A_Index206 = 0; A_Index206 < items206.size() + 0; A_Index206++) {
                ...
                if (StrLower(Trim(A_LoopField206)) == StrLower(keyWordMainLabel)) {
                    didWeUseMainLabel = 1;
                    if (langToConvertTo == langFileExtension_2) {
                    ...
                    codeOutFixAndAddMainFunc += A_LoopField206 + Chr(10);
                }
            }
        ...
        for (size_t A_Index207 = 0; A_Index207 < items207.size() + 0; A_Index207++) {
            ...
            HTVM_Append(lookIntoTheNextLineForFuncWhitNoKeyWord, A_LoopField207);
        }
        ...
        for (size_t A_Index208 = 0; A_Index208 < items208.size() + 0; A_Index208++) {
            ...
            lineDone = 0;
            if (SubStr(A_LoopField208, 1, StrLen(keyWordComment)) == keyWordComment) {
                lineDone = 1;
                str1 = StringTrimLeft(A_LoopField208, StrLen(keyWordComment));
                if (langToConvertTo == langFileExtension_2) {
            ...
            else if (InStr(A_LoopField208, "guiInit(") || InStr(A_LoopField208, "guiAdd(") || InStr(A_LoopField208, "guiAddElement(") || InStr(A_LoopField208, "guiControl(") || InStr(A_LoopField208, "HTVM_init(") || InStr(A_LoopField208, "HTVM_register(") || InStr(A_LoopField208, "HTVM_custom_port(") && lineDone == 0) {
                lineDone = 1;
                if (InStr(A_LoopField208, "HTVM_init(")) {
                    htCode += Trim(A_LoopField208) + Chr(10);
                } else {
                    htCode += Trim(expressionParserTranspiler(Trim(A_LoopField208))) + Chr(10);
                }
            }
            else if (Trim(A_LoopField208) == keyWordCommentOpenMultiLine) {
                lineDone = 1;
                if (langToConvertTo == langFileExtension_2) {
            ...
            else if (Trim(A_LoopField208) == keyWordCommentCloseMultiLine) {
                lineDone = 1;
                if (langToConvertTo == langFileExtension_2) {
            ...
            else if (InStr(A_LoopField208, "[][][][][] ")) {
                lineDone = 1;
                str1 = StrSplit(A_LoopField208, " ", 2);
            ...
            else if (SubStr(StrLower(Trim(A_LoopField208)), 1, StrLen(StrLower(keyWordStruct))) == StrLower(keyWordStruct)) {
                lineDone = 1;
                str0 = Trim(StringTrimLeft(A_LoopField208, StrLen(keyWordStruct)));
                if (langToConvertTo == langFileExtension_2) {
                    htCode += Trim(keyWordStruct_2) + " " + str0 + Chr(10);
                }
            }
            else if (SubStr(StrLower(Trim(A_LoopField208)), 1, StrLen(StrLower(keyWordAlliance))) == StrLower(keyWordAlliance)) {
                lineDone = 1;
                str0 = Trim(StringTrimLeft(A_LoopField208, StrLen(keyWordAlliance)));
                if (langToConvertTo == langFileExtension_2) {
                    htCode += Trim(keyWordAlliance_2) + " " + str0 + Chr(10);
                }
            }
            else if (SubStr(StrLower(Trim(A_LoopField208)), 1, StrLen(StrLower(keyWordCrew))) == StrLower(keyWordCrew)) {
                lineDone = 1;
                str0 = Trim(StringTrimLeft(A_LoopField208, StrLen(keyWordCrew)));
                if (langToConvertTo == langFileExtension_2) {
                    htCode += Trim(keyWordCrew_2) + " " + str0 + Chr(10);
                }
            }
            else if (InStr(A_LoopField208, "[end][end][end][end][end] ")) {
                lineDone = 1;
                if (langToConvertTo == langFileExtension_2) {
                    htCode += "}" + Chr(10);
                }
            }
            else if (SubStr(StrLower(Trim(A_LoopField208)), 1, StrLen(StrLower(keyWordProp))) == StrLower(keyWordProp)) {
                lineDone = 1;
                str0 = Trim(StringTrimLeft(A_LoopField208, StrLen(keyWordProp)));
            ...
            else if (SubStr(StrLower(Trim(A_LoopField208)), 1, StrLen(StrLower(keyWordProc))) == StrLower(keyWordProc)) {
                lineDone = 1;
                str0 = Trim(StringTrimLeft(A_LoopField208, StrLen(keyWordProc)));
                str1 = Trim(StrSplit(str0, "-----------------------HTVM-OSP-HTVM-OBJ--------------", 1));
                str20 = Trim(StrSplit(str0, "-----------------------HTVM-OSP-HTVM-OBJ--------------", 2));
            ...
            else if (SubStr(StrLower(Trim(A_LoopField208)), 1, StrLen(StrLower(keyWordIF))) == StrLower(keyWordIF) && SubStr(StrLower(Trim(A_LoopField208)), 1, StrLen(StrLower(keyWordElseIf))) != StrLower(keyWordElseIf)) {
                lineDone = 1;
                str1 = Trim(StringTrimLeft(A_LoopField208, StrLen(keyWordIF)));
            ...
            else if (SubStr(StrLower(Trim(A_LoopField208)), 1, StrLen(StrLower(keyWordElseIf))) == StrLower(keyWordElseIf) && SubStr(StrLower(Trim(A_LoopField208)), 1, StrLen(StrLower(keyWordIF))) != StrLower(keyWordIF)) {
                lineDone = 1;
                str1 = Trim(StringTrimLeft(A_LoopField208, StrLen(keyWordElseIf)));
                str1 = Trim(expressionParserTranspiler("(" + str1 + ")"));
                str1 = StringTrimRight(str1, 1);
                str1 = StringTrimLeft(str1, 1);
                if (langToConvertTo == langFileExtension_2) {
            ...
            else if (StrLower(Trim(A_LoopField208)) == StrLower(keyWordElse) || StrLower(Trim(A_LoopField208)) == StrLower(keyWordElse + ":")) {
                lineDone = 1;
                if (langToConvertTo == langFileExtension_2) {
                    if (usePythonicColonSyntax_2 == "on") {
            ...
            else if (SubStr(StrLower(Trim(A_LoopField208)), 1, StrLen(StrLower(keyWordWhileLoop))) == StrLower(keyWordWhileLoop)) {
                lineDone = 1;
                str1 = Trim(StringTrimLeft(A_LoopField208, StrLen(keyWordWhileLoop)));
                str1 = Trim(expressionParserTranspiler("(" + str1 + ")"));
                str1 = StringTrimRight(str1, 1);
                str1 = StringTrimLeft(str1, 1);
                if (langToConvertTo == langFileExtension_2) {
            ...
            else if (StrLower(Trim(A_LoopField208)) == StrLower(keyWordTry) || StrLower(Trim(A_LoopField208)) == StrLower(keyWordTry + ":")) {
                lineDone = 1;
                if (langToConvertTo == langFileExtension_2) {
                    if (usePythonicColonSyntax_2 == "on") {
                        htCode += keyWordTry_2 + ":\n";
            ...
            else if (SubStr(StrLower(Trim(A_LoopField208)), 1, StrLen(StrLower(keyWordCatch))) == StrLower(keyWordCatch)) {
                lineDone = 1;
                str1 = Trim(StrReplace(StrReplace(StringTrimLeft(A_LoopField208, StrLen(keyWordCatch)), "(", ""), ")", ""));
            ...
            else if (StrLower(Trim(A_LoopField208)) == StrLower(keyWordFinally) || StrLower(Trim(A_LoopField208)) == StrLower(keyWordFinally + ":")) {
                lineDone = 1;
                if (langToConvertTo == langFileExtension_2) {
                    if (usePythonicColonSyntax_2 == "on") {
            ...
            else if (SubStr(StrLower(Trim(A_LoopField208)), 1, StrLen(StrLower(keyWordThrow))) == StrLower(keyWordThrow)) {
                lineDone = 1;
                str1 = Trim(A_LoopField208);
                if (SubStrLastChars(str1, 1) == ";") {
                    str1 = StringTrimRight(str1, 1);
                }
            ...
            else if (StrLower(A_LoopField208) == StrLower(keyWordBreak) || StrLower(A_LoopField208) == StrLower(keyWordBreak + ";")) {
                lineDone = 1;
                if (langToConvertTo == langFileExtension_2) {
                    if (useSemicolon_2 == "on") {
            ...
            else if (StrLower(A_LoopField208) == StrLower(Trim(keyWordReturnStatement)) || StrLower(A_LoopField208) == StrLower(Trim(keyWordReturnStatement) + ";")) {
                lineDone = 1;
                if (langToConvertTo == langFileExtension_2) {
                    if (useSemicolon_2 == "on") {
            ...
            else if (SubStr(Trim(StrLower(A_LoopField208)), 1, StrLen(StrLower(keyWordReturnStatement))) == StrLower(keyWordReturnStatement)) {
                lineDone = 1;
                str1 = Trim(expressionParserTranspiler(Trim(StringTrimLeft(A_LoopField208, StrLen(keyWordReturnStatement)))));
                if (SubStrLastChars(str1, 1) == ";") {
                    str1 = StringTrimRight(str1, 1);
                }
                if (langToConvertTo == langFileExtension_2) {
            ...
            else if (StrLower(A_LoopField208) == StrLower(keyWordContinue) || StrLower(A_LoopField208) == StrLower(keyWordContinue + ";")) {
                lineDone = 1;
                if (langToConvertTo == langFileExtension_2) {
                    if (useSemicolon_2 == "on") {
            ...
            else if (StrLower(A_LoopField208) == "::" + keyWordContinue || StrLower(A_LoopField208) == "::" + keyWordContinue + ";") {
                lineDone = 1;
                if (langToConvertTo == langFileExtension_2) {
                    htCode += "::" + keyWordContinue_2 + Chr(10);
                }
            ...
            else if (StrLower(A_LoopField208) == StrLower(keyWordLoopInfinite) || StrLower(A_LoopField208) == StrLower(keyWordLoopInfinite + ":")) {
                // infinity loops
            ...
            else if (SubStr(Trim(StrLower(A_LoopField208)), 1, StrLen(StrLower(keyWordLoop))) == StrLower(keyWordLoop) && SubStr(Trim(StrLower(A_LoopField208)), 1, StrLen(StrLower(keyWordLoop)) + 2) != StrLower(keyWordLoop) + "% " && SubStr(Trim(StrLower(A_LoopField208)), 1, StrLen(StrLower(keyWordLoopParse))) != StrLower(keyWordLoopParse)) {
                out2 = StringTrimLeft(A_LoopField208, StrLen(keyWordLoop));
            ...
            else if (SubStr(Trim(StrLower(A_LoopField208)), 1, StrLen(StrLower(keyWordLoop)) + 2) == StrLower(keyWordLoop) + "% ") {
                howMany_fixAindexInGoUnused++;
                out2 = StringTrimLeft(A_LoopField208, StrLen(keyWordLoop) + 2);
                out2 = StrReplace(out2, ":", "");
                //MsgBox % out2
                //MsgBox, % out2
                out2 = Trim(out2);
            ...
            else if (SubStr(StrLower(A_LoopField208), 1, StrLen(StrLower(keyWordLoopParse))) == StrLower(keyWordLoopParse)) {
                howMany_fixAindexInGoUnused++;
                var1 = StringTrimLeft(A_LoopField208, StrLen(keyWordLoopParse));
                out2 = StrReplace(out2, ":", "");
                lineDone = 1;
            ...
            else if (SubStrLastChars(Trim(A_LoopField208), StrLen(keyWordInc)) == keyWordInc || SubStrLastChars(Trim(A_LoopField208), StrLen(keyWordInc + ";")) == keyWordInc + ";" && lineDone == 0) {
                if (useSemicolon == "on" || SubStrLastChars(Trim(A_LoopField208), 1) == ";") {
                    str1 = Trim(StringTrimRight(Trim(A_LoopField208), StrLen(keyWordInc + ";")));
                } else {
                    str1 = Trim(StringTrimRight(Trim(A_LoopField208), StrLen(keyWordInc)));
                }
                str1 = Trim(expressionParserTranspiler(str1));
            ...
            else if (SubStrLastChars(Trim(A_LoopField208), StrLen(keyWordDec)) == keyWordDec || SubStrLastChars(Trim(A_LoopField208), StrLen(keyWordDec + ";")) == keyWordDec + ";" && lineDone == 0) {
                if (useSemicolon == "on" || SubStrLastChars(Trim(A_LoopField208), 1) == ";") {
                    str1 = Trim(StringTrimRight(Trim(A_LoopField208), StrLen(keyWordDec + ";")));
                } else {
                    str1 = Trim(StringTrimRight(Trim(A_LoopField208), StrLen(keyWordDec)));
                }
                str1 = Trim(expressionParserTranspiler(str1));
            ...
            else if (SubStr(StrLower(Trim(A_LoopField208)), 1, StrLen(StrLower(keyWordInclude + " "))) == StrLower(keyWordInclude + " ") && langToConvertTo == langFileExtension_2) {
                htCode += keyWordInclude_2 + " " + Trim(StringTrimLeft(Trim(A_LoopField208), StrLen(StrLower(keyWordInclude) + " "))) + Chr(10);
            }
            else if (StrLower(A_LoopField208) == StrLower(keyWordGlobal) || StrLower(A_LoopField208) == StrLower(keyWordGlobal + ";")) {
                if (langToConvertTo == "ahk") {
                    htCode += "global\n";
                }
            }
            else if (SubStr(StrLower(A_LoopField208), 1, StrLen(StrLower(keyWordAwait))) == StrLower(keyWordAwait)) {
                str1 = StringTrimLeft(A_LoopField208, StrLen(keyWordAwait));
                //MsgBox, % A_LoopField
                lineDone = 1;
            ...
            else if (SubStr(StrLower(A_LoopField208), 1, StrLen(StrLower(keyWordGlobal))) == StrLower(keyWordGlobal)) {
                str1 = StringTrimLeft(A_LoopField208, StrLen(keyWordGlobal));
                //MsgBox, % A_LoopField
            ...
            else if (SubStr(StrLower(A_LoopField208), 1, StrLen(keyWordFunc)) == StrLower(keyWordFunc) || SubStr(StrLower(A_LoopField208), 1, StrLen(StrLower(keyWordAsync) + StrLower(keyWordFunc))) == StrLower(keyWordAsync) + StrLower(keyWordFunc) && InStr(StrLower(A_LoopField208), " main(") == false && lineDone == 0) {
                lineDone = 1;
                if (SubStr(StrLower(A_LoopField208), 1, StrLen(StrLower(keyWordAsync) + StrLower(keyWordFunc))) == StrLower(keyWordAsync) + StrLower(keyWordFunc)) {
                    // is async
                    int1 = 1;
                    str1 = Trim(StringTrimLeft(A_LoopField208, StrLen(StrLower(keyWordAsync + keyWordFunc))));
                } else {
                    // is async
                    int1 = 0;
                    str1 = Trim(StringTrimLeft(A_LoopField208, StrLen(StrLower(keyWordFunc))));
                }
                if (langToConvertTo != langFileExtension_2 && langToConvertTo != "cpp" && langToConvertTo != "go" && langToConvertTo != "cs" && langToConvertTo != "java" && langToConvertTo != "kt" && langToConvertTo != "nim" && langToConvertTo != "swift" && langToConvertTo != "dart" && langToConvertTo != "ts") {
                    if (str1 != "") {
                        str4 = "";
                        std::vector<std::string> items210 = LoopParseFunc(str1, ",");
                        for (size_t A_Index210 = 0; A_Index210 < items210.size() + 0; A_Index210++) {
                            std::string A_LoopField210 = items210[A_Index210 - 0];
                            if (InStr(A_LoopField210, " := ")) {
                                if (InStr(Trim(StrSplit(A_LoopField210, " := ", 1)), " ") != true) {
                                    str4 += "optanal-HTVM-opt-parmsNOtWorking-theirewIs-a-bug-Iusfbudfbuoeshfuisbav=-fwaegs-awedsf-dd-sfgc " + A_LoopField210 + ", ";
                                } else {
                                    str4 += A_LoopField210 + ", ";
                                }
                            }
                            else if (InStr(A_LoopField210, " = ")) {
                                if (InStr(Trim(StrSplit(A_LoopField210, " = ", 1)), " ") != true) {
                                    str4 += "optanal-HTVM-opt-parmsNOtWorking-theirewIs-a-bug-Iusfbudfbuoeshfuisbav=-fwaegs-awedsf-dd-sfgc " + A_LoopField210 + ", ";
                                } else {
                                    str4 += A_LoopField210 + ", ";
                            ...
                            else if (InStr(A_LoopField210, " " + Trim(keyWordAssign) + " ")) {
                                if (InStr(Trim(StrSplit(A_LoopField210, " " + Trim(keyWordAssign) + " ", 1)), " ") != true) {
                                    str4 += "optanal-HTVM-opt-parmsNOtWorking-theirewIs-a-bug-Iusfbudfbuoeshfuisbav=-fwaegs-awedsf-dd-sfgc " + A_LoopField210 + ", ";
                                } else {
                                    str4 += A_LoopField210 + ", ";
                                }
                ...
                if (Trim(str1) == "") {
                    ...
                    str19 = "";
                    str20 = "";
                    std::vector<std::string> items211 = LoopParseFunc(str1, ",");
                    for (size_t A_Index211 = 0; A_Index211 < items211.size() + 0; A_Index211++) {
                        ...
                        if (InStr(Trim(A_LoopField211), " ")) {
                            // there is space aka there is a type
                            if (InStr(A_LoopField211, " " + Trim(keyWordAssign) + " ") == false) {
                                if (usePrefixTypeForTypeDefinition == "on") {
                                    // normal
                                    str6 = Trim(A_LoopField211);
                                ...
                                // optanal param
                                if (InStr(Trim(StrSplit(Trim(A_LoopField211), " " + Trim(keyWordAssign) + " ", 1)), " ")) {
                                    // there is a type
                                    if (usePrefixTypeForTypeDefinition == "on") {
                                        // normal
                                        str6 = Trim(StrSplit(Trim(A_LoopField211), " " + Trim(keyWordAssign) + " ", 1));
                                        str9 = Trim(expressionParserTranspiler(Trim(StrSplit(Trim(A_LoopField211), " " + Trim(keyWordAssign) + " ", 2))));
                                        str7 = getTheLastWord(str6);
                                        str8 = getFuncTypeConvert(Trim(StringTrimRight(str6, StrLen(Trim(str7)) + 1)));
                                        //;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;
                                        //;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;
                                    ...
                                    // there is no type
                                    str8 = Trim(StrSplit(Trim(A_LoopField211), " " + Trim(keyWordAssign) + " ", 1));
                                    str9 = Trim(expressionParserTranspiler(Trim(StrSplit(Trim(A_LoopField211), " " + Trim(keyWordAssign) + " ", 2))));
                                    // str10
                                    // skipLeftCuleyForFuncPLS := 1
                                    //;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;
                            ...
                            str5 += Trim(A_LoopField211) + Chr(10);
                        }
                    }
                    //print(str5)
                    //print(str10)
                    ...
                    if (Trim(str5) != "" && Trim(str10) != "") {
                        ...
                        if (langToConvertTo == "lua") {
                            ...
                            for (size_t A_Index215 = 0; A_Index215 < items215.size() + 0; A_Index215++) {
                                ...
                                str13 += Trim(StrSplit(Trim(A_LoopField215), " = ", 1)) + " = " + Trim(StrSplit(Trim(A_LoopField215), " = ", 1)) + " or " + Trim(StrSplit(Trim(A_LoopField215), " = ", 2)) + Chr(10);
                            }
                            str12 = str14;
                        }
                        if (langToConvertTo == "go") {
                            ...
                            str13 += "{" + Chr(10) + " ";
                            std::vector<std::string> items216 = LoopParseFunc(str12, ",");
                            for (size_t A_Index216 = 0; A_Index216 < items216.size() + 0; A_Index216++) {
                                std::string A_LoopField216 = items216[A_Index216 - 0];
                                str16 = Trim(StrSplit(Trim(StrSplit(Trim(A_LoopField216), " = ", 1)), " ", 1));
                                str17 = Trim(StrSplit(Trim(A_LoopField216), " = ", 2));
                                str13 += str16 + " := " + str17 + Chr(10);
                            }
                            std::vector<std::string> items217 = LoopParseFunc(str12, ",");
                            for (size_t A_Index217 = 0; A_Index217 < items217.size() + 0; A_Index217++) {
                                ...
                                str18 = Trim(StrSplit(Trim(StrSplit(Trim(A_LoopField217), " = ", 1)), " ", 2));
                                // str16 = var name
                                // str18 = var type
                                str13 += "if len(__HTVM_V2_TO_GO_optionalParams__) " + Chr(62) + " " + STR(A_Index217) + Chr(10) + "{" + Chr(10) + str16 + " = __HTVM_V2_TO_GO_optionalParams__[" + STR(A_Index217) + "].(" + str18 + ")" + Chr(10) + "}" + Chr(10);
                            }
                            str12 = str14;
                        }
                        if (langToConvertTo == "java") {
                            ...
                            std::vector<std::string> items220 = LoopParseFunc(str12, ",");
                            for (size_t A_Index220 = 0; A_Index220 < items220.size() + 0; A_Index220++) {
                                std::string A_LoopField220 = items220[A_Index220 - 0];
                                str16 = Trim(StrSplit(Trim(StrSplit(Trim(A_LoopField220), " = ", 1)), " ", 2));
                                str17 = Trim(StrSplit(Trim(StrSplit(Trim(A_LoopField220), " = ", 1)), " ", 1));
                                // str16 = var name
                                // str17 = type name
                                str13 += "if (__HTVM_V2_TO_JAVA_optionalParams__.length " + Chr(62) + " " + STR(A_Index220) + ") " + str16 + " = (" + str17 + ") __HTVM_V2_TO_JAVA_optionalParams__[" + STR(A_Index220) + "];" + Chr(10);
                            }
                            str12 = str14;
                        }
                        str11 += str12;
                    ...
                    if (Trim(str5) == "" && Trim(str10) != "") {
                        ...
                        if (langToConvertTo == "lua") {
                            ...
                            for (size_t A_Index224 = 0; A_Index224 < items224.size() + 0; A_Index224++) {
                                ...
                                str13 += Trim(StrSplit(Trim(A_LoopField224), " = ", 1)) + " = " + Trim(StrSplit(Trim(A_LoopField224), " = ", 1)) + " or " + Trim(StrSplit(Trim(A_LoopField224), " = ", 2)) + Chr(10);
                            }
                            str11 = str14;
                        }
                        if (langToConvertTo == "go") {
                            str14 = "__HTVM_V2_TO_GO_optionalParams__ ...interface{}";
                            str13 += "{" + Chr(10) + " ";
                            std::vector<std::string> items225 = LoopParseFunc(str11, ",");
                            for (size_t A_Index225 = 0; A_Index225 < items225.size() + 0; A_Index225++) {
                                std::string A_LoopField225 = items225[A_Index225 - 0];
                                str16 = Trim(StrSplit(Trim(StrSplit(Trim(A_LoopField225), " = ", 1)), " ", 1));
                                str17 = Trim(StrSplit(Trim(A_LoopField225), " = ", 2));
                                str13 += str16 + " := " + str17 + Chr(10);
                            }
                            std::vector<std::string> items226 = LoopParseFunc(str11, ",");
                            for (size_t A_Index226 = 0; A_Index226 < items226.size() + 0; A_Index226++) {
                                std::string A_LoopField226 = items226[A_Index226 - 0];
                                str16 = Trim(StrSplit(Trim(StrSplit(Trim(A_LoopField226), " = ", 1)), " ", 1));
                                str18 = Trim(StrSplit(Trim(StrSplit(Trim(A_LoopField226), " = ", 1)), " ", 2));
                                // str16 = var name
                                // str18 = var type
                                str13 += "if len(__HTVM_V2_TO_GO_optionalParams__) " + Chr(62) + " " + STR(A_Index226) + Chr(10) + "{" + Chr(10) + str16 + " = __HTVM_V2_TO_GO_optionalParams__[" + STR(A_Index226) + "].(" + str18 + ")" + Chr(10) + "}" + Chr(10);
                            }
                            str11 = str14;
                        }
                        if (langToConvertTo == "java") {
                            ...
                            std::vector<std::string> items229 = LoopParseFunc(str11, ",");
                            for (size_t A_Index229 = 0; A_Index229 < items229.size() + 0; A_Index229++) {
                                std::string A_LoopField229 = items229[A_Index229 - 0];
                                str16 = Trim(StrSplit(Trim(StrSplit(Trim(A_LoopField229), " = ", 1)), " ", 2));
                                str17 = Trim(StrSplit(Trim(StrSplit(Trim(A_LoopField229), " = ", 1)), " ", 1));
                                // str16 = var name
                                // str17 = type name
                                str13 += "if (__HTVM_V2_TO_JAVA_optionalParams__.length " + Chr(62) + " " + STR(A_Index229) + ") " + str16 + " = (" + str17 + ") __HTVM_V2_TO_JAVA_optionalParams__[" + STR(A_Index229) + "];" + Chr(10);
                            }
                            str11 = str14;
                        }
                    }
            ...
            else if (InStr(StrLower(A_LoopField208), " main(") && lineDone == 0) {
                lineDone = 1;
                javaMainFuncSeen = 1;
                csMainFuncSeen = 1;
                htCode += A_LoopField208 + Chr(10);
            }
            else if (InStr(A_LoopField208, " " + keyWordAssign + " ") || InStr(A_LoopField208, " " + keyWordAssignAdd + " ") || InStr(A_LoopField208, " " + keyWordAssignConcat + " ") || InStr(A_LoopField208, " " + keyWordAssignSub + " ") || InStr(A_LoopField208, " " + keyWordAssignMul + " ") || InStr(A_LoopField208, " " + keyWordAssignDiv + " ") || InStr(A_LoopField208, " " + keyWordAssignMod + " ") || InStr(A_LoopField208, " " + keyWordAssignShiftLeft + " ") || InStr(A_LoopField208, " " + keyWordAssignShiftRight + " ") || InStr(A_LoopField208, " " + keyWordLogicalAssignShiftRight + " ") || InStr(A_LoopField208, " " + keyWordAssignBitAnd + " ") || InStr(A_LoopField208, " " + keyWordAssignBitOr + " ") || InStr(A_LoopField208, " " + keyWordAssignBitXor + " ") && lineDone == 0) {
                allVarsSoWeDontReDecVars_FIX_TOGGLE = 1;
                //keyWordAssign
                //keyWordAssignAdd
                //keyWordAssignConcat
                ...
                //allVarsSoWeDontReDecVars_FIX_int64
                //allVarsSoWeDontReDecVars_FIX_float
                if (InStr(A_LoopField208, " " + keyWordAssign + " ")) {
                    str1 = StrSplit(A_LoopField208, " " + keyWordAssign + " ", 1);
                    str6 = StrSplit(A_LoopField208, " " + keyWordAssign + " ", 2);
                    str10 = keyWordAssign;
                    str11 = keyWordAssign_2;
                    if (langToConvertTo == "ahk") {
                        str12 = ":=";
                ...
                else if (InStr(A_LoopField208, " " + keyWordAssignAdd + " ")) {
                    str1 = StrSplit(A_LoopField208, " " + keyWordAssignAdd + " ", 1);
                    str6 = StrSplit(A_LoopField208, " " + keyWordAssignAdd + " ", 2);
                    str10 = keyWordAssignAdd;
                    str11 = keyWordAssignAdd_2;
                    str12 = "+=";
                }
                else if (InStr(A_LoopField208, " " + keyWordAssignConcat + " ")) {
                    str1 = StrSplit(A_LoopField208, " " + keyWordAssignConcat + " ", 1);
                    str6 = StrSplit(A_LoopField208, " " + keyWordAssignConcat + " ", 2);
                    str10 = keyWordAssignConcat;
                    str11 = keyWordAssignConcat_2;
                    if (langToConvertTo == "ahk") {
                        str12 = ".=";
                ...
                else if (InStr(A_LoopField208, " " + keyWordAssignSub + " ")) {
                    str1 = StrSplit(A_LoopField208, " " + keyWordAssignSub + " ", 1);
                    str6 = StrSplit(A_LoopField208, " " + keyWordAssignSub + " ", 2);
                    str10 = keyWordAssignSub;
                    str11 = keyWordAssignSub_2;
                    str12 = "-=";
                }
                else if (InStr(A_LoopField208, " " + keyWordAssignMul + " ")) {
                    str1 = StrSplit(A_LoopField208, " " + keyWordAssignMul + " ", 1);
                    str6 = StrSplit(A_LoopField208, " " + keyWordAssignMul + " ", 2);
                    str10 = keyWordAssignMul;
                    str11 = keyWordAssignMul_2;
                    str12 = "*=";
                }
                else if (InStr(A_LoopField208, " " + keyWordAssignDiv + " ")) {
                    str1 = StrSplit(A_LoopField208, " " + keyWordAssignDiv + " ", 1);
                    str6 = StrSplit(A_LoopField208, " " + keyWordAssignDiv + " ", 2);
                    str10 = keyWordAssignDiv;
                    str11 = keyWordAssignDiv_2;
                    str12 = "/=";
                }
                else if (InStr(A_LoopField208, " " + keyWordAssignMod + " ")) {
                    str1 = StrSplit(A_LoopField208, " " + keyWordAssignMod + " ", 1);
                    str6 = StrSplit(A_LoopField208, " " + keyWordAssignMod + " ", 2);
                    str10 = keyWordAssignMod;
                    str11 = keyWordAssignMod_2;
                    str12 = "%=";
                }
                else if (InStr(A_LoopField208, " " + keyWordAssignShiftLeft + " ")) {
                    str1 = StrSplit(A_LoopField208, " " + keyWordAssignShiftLeft + " ", 1);
                    str6 = StrSplit(A_LoopField208, " " + keyWordAssignShiftLeft + " ", 2);
                    str10 = keyWordAssignShiftLeft;
                    str11 = keyWordAssignShiftLeft_2;
                    str12 = "<<=";
                }
                else if (InStr(A_LoopField208, " " + keyWordAssignShiftRight + " ")) {
                    str1 = StrSplit(A_LoopField208, " " + keyWordAssignShiftRight + " ", 1);
                    str6 = StrSplit(A_LoopField208, " " + keyWordAssignShiftRight + " ", 2);
                    str10 = keyWordAssignShiftRight;
                    str11 = keyWordAssignShiftRight_2;
                    str12 = ">>=";
                }
                else if (InStr(A_LoopField208, " " + keyWordLogicalAssignShiftRight + " ")) {
                    str1 = StrSplit(A_LoopField208, " " + keyWordLogicalAssignShiftRight + " ", 1);
                    str6 = StrSplit(A_LoopField208, " " + keyWordLogicalAssignShiftRight + " ", 2);
                    str10 = keyWordLogicalAssignShiftRight;
                    str11 = keyWordLogicalAssignShiftRight_2;
                    str12 = ">>>=";
                }
                else if (InStr(A_LoopField208, " " + keyWordAssignBitAnd + " ")) {
                    str1 = StrSplit(A_LoopField208, " " + keyWordAssignBitAnd + " ", 1);
                    str6 = StrSplit(A_LoopField208, " " + keyWordAssignBitAnd + " ", 2);
                    str10 = keyWordAssignBitAnd;
                    str11 = keyWordAssignBitAnd_2;
                    str12 = "&=";
                }
                else if (InStr(A_LoopField208, " " + keyWordAssignBitOr + " ")) {
                    str1 = StrSplit(A_LoopField208, " " + keyWordAssignBitOr + " ", 1);
                    str6 = StrSplit(A_LoopField208, " " + keyWordAssignBitOr + " ", 2);
                    str10 = keyWordAssignBitOr;
                    str11 = keyWordAssignBitOr_2;
                    str12 = "|=";
                }
                else if (InStr(A_LoopField208, " " + keyWordAssignBitXor + " ")) {
                    str1 = StrSplit(A_LoopField208, " " + keyWordAssignBitXor + " ", 1);
                    str6 = StrSplit(A_LoopField208, " " + keyWordAssignBitXor + " ", 2);
                    str10 = keyWordAssignBitXor;
                    str11 = keyWordAssignBitXor_2;
                    str12 = "^=";
                }
                ...
                if (InStr(ALoopField, " ") == false || SubStrLastChars(ALoopField, 1) == "]") {
                    ...
                    if (usePostfixTypeForTypeDefinition == "off") {
                        str1 = Trim(StringTrimLeft(Trim(str1), StrLen(keyWordLet)));
                        std::vector<std::string> items232 = LoopParseFunc(str1, " ");
                    ...
                    lineDone = 1;
                    std::vector<std::string> items233 = LoopParseFunc(str1, " ");
            ...
            else if (SubStr(Trim(A_LoopField208), 1, StrLen(keyWordVar)) == keyWordVar && lineDone == 0) {
                allVarsSoWeDontReDecVars_FIX_TOGGLE = 1;
                if (usePostfixTypeForTypeDefinition == "off") {
                    lineDone = 1;
                    str1 = Trim(StringTrimLeft(Trim(A_LoopField208), StrLen(keyWordVar)));
                    if (SubStrLastChars(str1, 1) == ";") {
                        str1 = StringTrimRight(str1, 1);
                    }
                    std::vector<std::string> items234 = LoopParseFunc(str1, " ");
            ...
            else if (SubStr(Trim(A_LoopField208), 1, StrLen(keyWordLet)) == keyWordLet && lineDone == 0) {
                allVarsSoWeDontReDecVars_FIX_TOGGLE = 1;
                if (usePostfixTypeForTypeDefinition == "off") {
                    lineDone = 1;
                    str1 = Trim(StringTrimLeft(Trim(A_LoopField208), StrLen(keyWordLet)));
                    if (SubStrLastChars(str1, 1) == ";") {
                        str1 = StringTrimRight(str1, 1);
                    }
                    std::vector<std::string> items235 = LoopParseFunc(str1, " ");
            ...
            else if (SubStr(Trim(A_LoopField208), 1, StrLen(keyWordDouble + " ")) == keyWordDouble + " " || SubStr(Trim(A_LoopField208), 1, StrLen(keyWordChar + " ")) == keyWordChar + " " || SubStr(Trim(A_LoopField208), 1, StrLen(keyWordUint8 + " ")) == keyWordUint8 + " " || SubStr(Trim(A_LoopField208), 1, StrLen(keyWordUint16 + " ")) == keyWordUint16 + " " || SubStr(Trim(A_LoopField208), 1, StrLen(keyWordUint32 + " ")) == keyWordUint32 + " " || SubStr(Trim(A_LoopField208), 1, StrLen(keyWordUint64 + " ")) == keyWordUint64 + " " || SubStr(Trim(A_LoopField208), 1, StrLen(keyWordINT + " ")) == keyWordINT + " " || SubStr(Trim(A_LoopField208), 1, StrLen(keyWordSTR + " ")) == keyWordSTR + " " || SubStr(Trim(A_LoopField208), 1, StrLen(keyWordBOOL + " ")) == keyWordBOOL + " " || SubStr(Trim(A_LoopField208), 1, StrLen(keyWordFLOAT + " ")) == keyWordFLOAT + " " || SubStr(Trim(A_LoopField208), 1, StrLen(keyWordINT8 + " ")) == keyWordINT8 + " " || SubStr(Trim(A_LoopField208), 1, StrLen(keyWordINT16 + " ")) == keyWordINT16 + " " || SubStr(Trim(A_LoopField208), 1, StrLen(keyWordINT32 + " ")) == keyWordINT32 + " " || SubStr(Trim(A_LoopField208), 1, StrLen(keyWordINT64 + " ")) == keyWordINT64 + " " || SubStr(Trim(A_LoopField208), 1, StrLen(keyWordArrayDefinition + " ")) == keyWordArrayDefinition + " " || SubStr(Trim(A_LoopField208), 1, StrLen(keyWordArrayOfIntegersDefinition + " ")) == keyWordArrayOfIntegersDefinition + " " || SubStr(Trim(A_LoopField208), 1, StrLen(keyWordArrayOfStringsDefinition + " ")) == keyWordArrayOfStringsDefinition + " " || SubStr(Trim(A_LoopField208), 1, StrLen(keyWordArrayOfFloatingPointNumbersDefinition + " ")) == keyWordArrayOfFloatingPointNumbersDefinition + " " || SubStr(Trim(A_LoopField208), 1, StrLen(keyWordArrayOfBooleansDefinition + " ")) == keyWordArrayOfBooleansDefinition + " " && usePrefixTypeForTypeDefinition == "on" && lineDone == 0) {
                lineDone = 1;
                allVarsSoWeDontReDecVars_FIX_TOGGLE = 1;
                str1 = Trim(A_LoopField208);
                if (SubStrLastChars(str1, 1) == ";") {
                ...
                std::vector<std::string> items236 = LoopParseFunc(str1, " ");
            ...
            else if (RegExMatch(Trim(A_LoopField208), "^[a-zA-Z_][a-zA-Z0-9_]*: [^:]*;?$") && usePostfixTypeForTypeDefinition == "on" && lineDone == 0) {
                lineDone = 1;
                allVarsSoWeDontReDecVars_FIX_TOGGLE = 1;
                str1 = Trim(A_LoopField208);
                if (SubStrLastChars(str1, 1) == ";") {
            ...
            else if (RegExMatch(Trim(A_LoopField208), "^[a-zA-Z0-9_\\.]+\\(") && !(InStr(StrLower(A_LoopField208), " main(")) && lineDone == 0) {
                lineDone = 1;
                str1 = Trim(A_LoopField208);
                if (SubStrLastChars(str1, 1) == ";") {
                    str1 = StringTrimRight(str1, 1);
            ...
            else if (RegExMatch(A_LoopField208, "\\.[a-zA-Z_][a-zA-Z0-9_]*") && lineDone == 0) {
                lineDone = 1;
                str1 = Trim(A_LoopField208);
                if (SubStrLastChars(str1, 1) == ";") {
                    str1 = StringTrimRight(str1, 1);
            ...
            else if (KeyWordsCommands(A_LoopField208, "check", commands, langToConvertTo) == "true" && lineDone == 0) {
                lineDone = 1;
                if (langToConvertTo == langFileExtension_2) {
                    htCode += A_LoopField208 + Chr(10);
                } else {
                    out_KeyWordsCommands = KeyWordsCommands(A_LoopField208, "transpile", commands, langToConvertTo);
                ...
                if (lineDone != 1) {
                    if (skipLeftCuleyForFuncPLS != 1) {
                        if (SubStr(Trim(StrLower(A_LoopField208)), 1, 1) == Chr(125)) {
                            htCode += Chr(125) + Chr(10);
                        } else {
                            if (htCodeAcurlyBraceAddSomeVrasFixLP == 1 && SubStr(Trim(StrLower(A_LoopField208)), 1, 1) == Chr(123)) {
                                htCodeAcurlyBraceAddSomeVrasFixLP = 0;
                                htCode += A_LoopField208 + Chr(10) + theFixTextLoopLP + Chr(10);
                            } else {
                                if (htCodeAcurlyBraceAddSomeVrasFixNL == 1 && SubStr(Trim(StrLower(A_LoopField208)), 1, 1) == Chr(123)) {
                                    htCodeAcurlyBraceAddSomeVrasFixNL = 0;
                                    htCode += A_LoopField208 + Chr(10) + theFixTextLoopNL + Chr(10);
                                } else {
                                    htCode += A_LoopField208 + Chr(10);
                                }
                            }
        ...
        if (langToConvertTo != "ahk" && langToConvertTo != langFileExtension_2) {
            ...
            if (haveWeEverUsedAloop == 1) {
                ...
                std::vector<std::string> items237 = LoopParseFunc(htCodeLoopfixa, "\n", "\r");
                for (size_t A_Index237 = 0; A_Index237 < items237.size() + 0; A_Index237++) {
                    std::string A_LoopField237 = items237[A_Index237 - 0];
                    sstr123 = A_LoopField237;
                    fixLoopLokingFor = A_LoopField237;
                    ...
                    if (out1 == "nl") {
                        ...
                        for (size_t A_Index238 = 0; A_Index238 < items238.size() + 0; A_Index238++) {
                            ...
                            if (InStr(A_LoopField238, fixLoopLokingFor) && insdeAnestedLoopBAD != 1) {
                                fixLoopLokingForNum = 1;
                                //MsgBox, do we came here 1
                            }
                            if (SubStr(Trim(A_LoopField238), 1, 4) == "for " && weAreDoneHereCurly != 1 && insdeAnestedLoopBAD != 1 && fixLoopLokingForNum == 1) {
                                s = StrSplit(A_LoopField238 ,"" + keyWordAIndex + "", 2);
                                out1z = s;
                                s = StrSplit(out1z, " ", 1);
                                out1z = Trim(s);
                                //MsgBox, % out1z
                                ...
                                //MsgBox, % A_LoopField
                                dontSaveStr = 1;
                                ALoopField = A_LoopField238;
                                DeleayOneCuzOfLoopParse = 1;
                                out4758686d86d86d86578991a += ALoopField + Chr(10);
                            }
                            if (inTarget == 1 && InStr(A_LoopField238, Chr(123)) && insdeAnestedLoopBAD != 1) {
                                insideBracket = 1;
                            }
                            if (insideBracket == 1 && InStr(A_LoopField238, Chr(123)) && insdeAnestedLoopBAD != 1) {
                                netsedCurly++;
                            }
                            if (insideBracket == 1 && InStr(A_LoopField238, Chr(125)) && insdeAnestedLoopBAD != 1) {
                                netsedCurly--;
                                readyToEnd = 1;
                            }
                            if (SubStr(Trim(A_LoopField238), 1, 4) == "for " && insdeAnestedLoopBAD != 1 && foundTheTopLoop >= 2) {
                            ...
                            if (insdeAnestedLoopBAD == 1) {
                                if (InStr(A_LoopField238, Chr(123))) {
                                    insideBracket1 = 1;
                                }
                                if (insideBracket1 == 1 && InStr(A_LoopField238, Chr(123))) {
                                    netsedCurly1++;
                                }
                                if (insideBracket1 == 1 && InStr(A_LoopField238, Chr(125))) {
                                    netsedCurly1--;
                                    readyToEnd1 = 1;
                                }
                                if (InStr(A_LoopField238, Chr(125)) && readyToEnd1 == 1 && netsedCurly1 == 0 && insideBracket == 1) {
                                    //MsgBox, % A_LoopField
                                    eldLoopNestedBADlol = 1;
                                }
                                out4758686d86d86d86578991a += A_LoopField238 + Chr(10);
                            }
                            ...
                            if (inTarget == 1 && InStr(A_LoopField238, Chr(125)) && readyToEnd == 1 && netsedCurly == 0 && weAreDoneHereCurly == 0 && dontSaveStr != 1 && insdeAnestedLoopBAD != 1) {
                                //MsgBox, % A_LoopField
                        ...
                        for (size_t A_Index239 = 0; A_Index239 < items239.size() + 0; A_Index239++) {
                            ...
                            if (InStr(A_LoopField239 , fixLoopLokingFor) && insdeAnestedLoopBAD != 1) {
                                fixLoopLokingForNum = 1;
                                //MsgBox, do we came here 3
                            }
                            if (SubStr(Trim(A_LoopField239), 1, 4) == "for " && weAreDoneHereCurly != 1 && insdeAnestedLoopBAD != 1 && fixLoopLokingForNum == 1) {
                                s = StrSplit(A_LoopField239 ,"" + keyWordAIndex + "", 2);
                                out1z = s;
                                s = StrSplit(out1z, " ", 1);
                                ...
                                //MsgBox, % A_LoopField
                                dontSaveStr = 1;
                                ALoopField = A_LoopField239;
                                DeleayOneCuzOfLoopParse = 1;
                                out4758686d86d86d86578991a += ALoopField + Chr(10);
                            }
                            if (inTarget == 1 && InStr(A_LoopField239, Chr(123)) && insdeAnestedLoopBAD != 1) {
                                insideBracket = 1;
                            }
                            if (insideBracket == 1 && InStr(A_LoopField239, Chr(123)) && insdeAnestedLoopBAD != 1) {
                                netsedCurly++;
                            }
                            if (insideBracket == 1 && InStr(A_LoopField239, Chr(125)) && insdeAnestedLoopBAD != 1) {
                                netsedCurly--;
                                readyToEnd = 1;
                            }
                            if (SubStr(Trim(A_LoopField239), 1, 4) == "for " && insdeAnestedLoopBAD != 1 && foundTheTopLoop >= 2) {
                                insdeAnestedLoopBAD = 1;
                                insideBracket1 = 0;
                                netsedCurly1 = 0;
                            }
                            ...
                            if (insdeAnestedLoopBAD == 1) {
                                if (InStr(A_LoopField239, Chr(123))) {
                                    insideBracket1 = 1;
                                }
                                if (insideBracket1 == 1 && InStr(A_LoopField239, Chr(123))) {
                                    netsedCurly1++;
                                }
                                if (insideBracket1 == 1 && InStr(A_LoopField239, Chr(125))) {
                                    netsedCurly1--;
                                    readyToEnd1 = 1;
                                }
                                if (InStr(A_LoopField239, Chr(125)) && readyToEnd1 == 1 && netsedCurly1 == 0 && insideBracket == 1) {
                                    //MsgBox, % A_LoopField
                                    eldLoopNestedBADlol = 1;
                                }
                                out4758686d86d86d86578991a += A_LoopField239 + Chr(10);
                            }
                            if (inTarget == 1 && dontSaveStr != 1 && fixLoopLokingForNum != 1 && insdeAnestedLoopBAD != 1) {
                                ALoopField = A_LoopField239;
                                // Replace "A_Index" with or without a following digit with "A_Index" + out1z
                            ...
                            if ((inTarget == 1 && InStr(A_LoopField239, Chr(125)) && readyToEnd == 1 && netsedCurly == 0 && weAreDoneHereCurly == 0 && dontSaveStr != 1 && insdeAnestedLoopBAD != 1)) {
                                //MsgBox, % A_LoopField
                                weAreDoneHereCurly = 1;
                                inTarget = 0;
                                endBracketDOntPutThere = 1;
                ...
                for (size_t A_Index240 = 0; A_Index240 < items240.size() + 0; A_Index240++) {
                    ...
                    ignore = 0;
                    if (SubStr(Trim(A_LoopField240), 1, 4) == "for ") {
                        if (hold == 1 && holdText == A_LoopField240) {
                            ignore = 1;
                        } else {
                            holdText = A_LoopField240;
                            hold = 1;
                        }
                    }
                    if (!ignore) {
                        out4758686d86dgt8r754444444 += A_LoopField240 + Chr(10);
                    }
                }
    ...
    if (COUNT_programmingBlock_HTVMsyntax != 0) {
        ...
        for (int A_Index242 = 0; A_Index242 < COUNT_programmingBlock_HTVMsyntax + 0; A_Index242++) {
            ADD_ALL_programmingBlock_HTVMsyntax += programmingBlock_HTVMsyntax[A_Index242] + Chr(10);
        }
        ...
        isNotHTVMfile2 = 1;
        for (int A_Index243 = 0; A_Index243 < COUNT_programmingBlock_HTVMsyntax + 0; A_Index243++) {
            htCode = StrReplace(htCode, "programmingBlock_HTVMsyntax-programmingBlock_HTVMsyntax-AA" + STR(A_Index243 + 1) + "AA", compiler(programmingBlock_HTVMsyntax[A_Index243], allInstructionFile, "full", langToConvertToParam));
        }
        isNotHTVMfile2 = 0;
    }
    //;;;;;;;;;;;;;;;;;;;;;;;;;;;
    ...
    if (langToConvertTo == langFileExtension_2) {
        ...
        if (useEnd_2 == "on") {
            ...
            for (size_t A_Index244 = 0; A_Index244 < items244.size() + 0; A_Index244++) {
                ...
                if (Trim(A_LoopField244) == keyWordCurlyBraceClose_2) {
                    fixLuaAndRuby += StrReplace(A_LoopField244, keyWordCurlyBraceClose_2, Trim(keyWordEnd_2)) + Chr(10);
                }
                else if (SubStrLastChars(A_LoopField244, 2) == " " + keyWordCurlyBraceOpen_2) {
                    fixLuaAndRuby += StringTrimRight(A_LoopField244, 2) + Chr(10);
                } else {
                    fixLuaAndRuby += A_LoopField244 + Chr(10);
                }
            }
        ...
        if (langToConvertTo == "lua" || langToConvertTo == "rb" || langToConvertTo == langFileExtension_2) {
            if (langToConvertTo == langFileExtension_2 && useEnd_2 == "on") {
                ...
                for (size_t A_Index245 = 0; A_Index245 < items245.size() + 0; A_Index245++) {
                    ...
                    if (Trim(A_LoopField245) == "}") {
                        fixLuaAndRuby += StrReplace(A_LoopField245, "}", fixLuaAndRubyHELP) + Chr(10);
                    }
                    else if (SubStrLastChars(A_LoopField245, 2) == " {") {
                        fixLuaAndRuby += StringTrimRight(A_LoopField245, 2) + Chr(10);
                    } else {
                        fixLuaAndRuby += A_LoopField245 + Chr(10);
                    }
                }
                htCode = StringTrimRight(fixLuaAndRuby, 1);
            } else {
                ...
                for (size_t A_Index246 = 0; A_Index246 < items246.size() + 0; A_Index246++) {
                    ...
                    if (Trim(A_LoopField246) == "}") {
                        fixLuaAndRuby += StrReplace(A_LoopField246, "}", fixLuaAndRubyHELP) + Chr(10);
                    }
                    else if (SubStrLastChars(A_LoopField246, 2) == " {") {
                        fixLuaAndRuby += StringTrimRight(A_LoopField246, 2) + Chr(10);
                    } else {
                        fixLuaAndRuby += A_LoopField246 + Chr(10);
                    }
                }
                htCode = StringTrimRight(fixLuaAndRuby, 1);
            }
    ...
    if (langToConvertTo == langFileExtension_2 && useEnd_2 == "on") {
        ...
        for (size_t A_Index247 = 0; A_Index247 < items247.size() + 0; A_Index247++) {
            ...
            HTVM_Append(nextWordEndFix, A_LoopField247);
        }
        ...
        for (size_t A_Index248 = 0; A_Index248 < items248.size() + 0; A_Index248++) {
            ...
            if (Trim(A_LoopField248) == Trim(keyWordEnd_2) && SubStr(Trim(nextWordEndFix[A_Index248 + 1]), 1, StrLen(Trim(keyWordElseIf_2) + " ")) == Trim(keyWordElseIf_2) + " " || Trim(A_LoopField248) == Trim(keyWordEnd_2) && (Trim(nextWordEndFix[A_Index248 + 1]) == Trim(keyWordElse_2) || Trim(nextWordEndFix[A_Index248 + 1]) == Trim(keyWordElse_2 + ":"))) {
                nextWordEndFixOut += "";
            } else {
    ...
    if (langToConvertTo == "lua") {
        ...
        for (size_t A_Index249 = 0; A_Index249 < items249.size() + 0; A_Index249++) {
            ...
            HTVM_Append(nextWordEndFix, A_LoopField249);
        }
        HTVM_Append(nextWordEndFix, " ");
        ...
        for (size_t A_Index250 = 0; A_Index250 < items250.size() + 0; A_Index250++) {
            ...
            if (Trim(A_LoopField250) == "end" && SubStr(Trim(nextWordEndFix[A_Index250 + 1]), 1, StrLen("elseif ")) == "elseif " || Trim(A_LoopField250) == "end" && Trim(nextWordEndFix[A_Index250 + 1]) == "else") {
                nextWordEndFixOut += "";
            } else {
    ...
    if (langToConvertTo == "rb") {
        ...
        for (size_t A_Index251 = 0; A_Index251 < items251.size() + 0; A_Index251++) {
            ...
            HTVM_Append(nextWordEndFix, A_LoopField251);
        }
        HTVM_Append(nextWordEndFix, " ");
        ...
        for (size_t A_Index252 = 0; A_Index252 < items252.size() + 0; A_Index252++) {
            ...
            if (Trim(A_LoopField252) == "end" && SubStr(Trim(nextWordEndFix[A_Index252 + 1]), 1, StrLen("elsif ")) == "elsif " || Trim(A_LoopField252) == "end" && Trim(nextWordEndFix[A_Index252 + 1]) == "else") {
                nextWordEndFixOut += "";
            } else {
    ...
    if (langToConvertTo == "rb") {
        ...
        for (size_t A_Index253 = 0; A_Index253 < items253.size() + 0; A_Index253++) {
            ...
            HTVM_Append(nextWordEndFixRB, A_LoopField253);
        }
        ...
        for (size_t A_Index254 = 0; A_Index254 < items254.size() + 0; A_Index254++) {
            ...
            if (Trim(A_LoopField254) == "end" && SubStr(Trim(nextWordEndFixRB[A_Index254 + 1]), 1, StrLen("rescue ")) == "rescue " || Trim(A_LoopField254) == "end" && Trim(nextWordEndFixRB[A_Index254 + 1]) == "ensure") {
                nextWordEndFixOut += "";
            } else {
                nextWordEndFixOut += A_LoopField254 + Chr(10);
            }
        }
        htCode = StringTrimRight(nextWordEndFixOut, 1);
    }
    if (langToConvertTo == langFileExtension_2 && useEnd_2 == "on") {
        std::vector<std::string> items255 = LoopParseFunc(htCode, "\n", "\r");
        for (size_t A_Index255 = 0; A_Index255 < items255.size() + 0; A_Index255++) {
            std::string A_LoopField255 = items255[A_Index255 - 0];
            HTVM_Append(nextWordEndFixRB, A_LoopField255);
        }
        HTVM_Append(nextWordEndFixRB, " ");
        ...
        for (size_t A_Index256 = 0; A_Index256 < items256.size() + 0; A_Index256++) {
            ...
            if (Trim(A_LoopField256) == Trim(keyWordEnd_2) && SubStr(Trim(nextWordEndFixRB[A_Index256 + 1]), 1, StrLen(Trim(keyWordCatch_2) + " ")) == Trim(keyWordCatch_2) + " " || Trim(A_LoopField256) == Trim(keyWordEnd_2) && Trim(nextWordEndFixRB[A_Index256 + 1]) == Trim(keyWordFinally_2) || Trim(nextWordEndFixRB[A_Index256 + 1]) == Trim(keyWordFinally_2 + ":")) {
                nextWordEndFixOut += "";
            } else {
                nextWordEndFixOut += A_LoopField256 + Chr(10);
            }
        }
        htCode = StringTrimRight(nextWordEndFixOut, 1);
    }
    ...
    if (langToConvertTo == "ahk") {
        ...
        for (size_t A_Index257 = 0; A_Index257 < items257.size() + 0; A_Index257++) {
            ...
            if (SubStr(Trim(A_LoopField257), 1, StrLen("Loop, Parse, ")) == "Loop, Parse, ") {
                nextWordEndFixOut += StrReplace(A_LoopField257, "{", Chr(10) + "{" + Chr(10)) + Chr(10);
            } else {
                nextWordEndFixOut += A_LoopField257 + Chr(10);
            }
        }
        htCode = StringTrimRight(nextWordEndFixOut, 1);
    }
    if (langToConvertTo == "go") {
        ...
        for (size_t A_Index258 = 0; A_Index258 < items258.size() + 0; A_Index258++) {
            ...
            HTVM_Append(fixGoManGoIsSoAnnoyingBroooFurure, A_LoopField258);
        }
        ...
        for (size_t A_Index259 = 0; A_Index259 < items259.size() + 0; A_Index259++) {
            ...
            if (Trim(A_LoopField259) == "}" && SubStr(Trim(fixGoManGoIsSoAnnoyingBroooFurure[A_Index259 + 1]), 1, StrLen("else if ")) == "else if ") {
                fixGoManGoIsSoAnnoyingBroooSkip = 1;
                fixGoManGoIsSoAnnoyingBroooCount = 0;
                std::vector<std::string> items260 = LoopParseFunc(fixGoManGoIsSoAnnoyingBroooFurure[A_Index259 + 1]);
                ...
                for (int A_Index261 = 0; A_Index261 < fixGoManGoIsSoAnnoyingBroooCount + 0; A_Index261++) {
                    fixGoManGoIsSoAnnoyingBroooCountSpaceses += " ";
                }
                fixGoManGoIsSoAnnoyingBrooo += fixGoManGoIsSoAnnoyingBroooCountSpaceses + "} " + Trim(fixGoManGoIsSoAnnoyingBroooFurure[A_Index259 + 1]) + Chr(10);
            } else {
                if (fixGoManGoIsSoAnnoyingBroooSkip == 0) {
                    fixGoManGoIsSoAnnoyingBrooo += A_LoopField259 + Chr(10);
                }
    ...
    for (int A_Index262 = 0; A_Index262 < HTVM_Size(commentsBugFix) + 0; A_Index262++) {
        htCode = StrReplace(htCode, "HTVM--cnavisdofbuvsesdivdidufg79wregwewaeosd8ges7dfdftuawdiHTVMv2yerheyziberlasduci6syiq--AA" + STR(A_Index262) + "AA", commentsBugFix[A_Index262]);
    }
    //;;;;;;;;;;;;;;;;;;;;;;;;; fix here
    //;;;;;;;;;;;;;;;;;;;;;;;;; fix here
    //;;;;;;;;;;;;;;;;;;;;;;;;; fix here
    if (COUNT_programmingBlock_InTheTranspiledLang != 0) {
        for (int A_Index263 = 0; A_Index263 < COUNT_programmingBlock_InTheTranspiledLang + 0; A_Index263++) {
            htCode = StrReplace(htCode, "programmingBlock_InTheTranspiledLang-programmingBlock_InTheTranspiledLang-AA" + STR(A_Index263 + 1) + "AA", programmingBlock_InTheTranspiledLang[A_Index263]);
        }
    }
    if (langToConvertTo == "cpp") {
        if (COUNT_programmingBlock_CPP != 0) {
            for (int A_Index264 = 0; A_Index264 < COUNT_programmingBlock_CPP + 0; A_Index264++) {
                htCode = StrReplace(htCode, "programmingBlock_CPP-programmingBlock_CPP-AA" + STR(A_Index264 + 1) + "AA", programmingBlock_CPP[A_Index264]);
            }
        }
    } else {
        if (COUNT_programmingBlock_CPP != 0) {
            for (int A_Index265 = 0; A_Index265 < COUNT_programmingBlock_CPP + 0; A_Index265++) {
                if (langToConvertTo == langFileExtension_2) {
                    htCode = StrReplace(htCode, "programmingBlock_CPP-programmingBlock_CPP-AA" + STR(A_Index265 + 1) + "AA", keyWordCodeInTheTranspiledLangStartCPP_2 + Chr(10) + programmingBlock_CPP[A_Index265] + Chr(10) + keyWordCodeInTheTranspiledLangEndCPP_2);
                } else {
                    htCode = StrReplace(htCode, "programmingBlock_CPP-programmingBlock_CPP-AA" + STR(A_Index265 + 1) + "AA", Chr(10));
                }
            }
        }
    }
    if (langToConvertTo == "py") {
        if (COUNT_programmingBlock_PY != 0) {
            for (int A_Index266 = 0; A_Index266 < COUNT_programmingBlock_PY + 0; A_Index266++) {
                htCode = StrReplace(htCode, "programmingBlock_PY-programmingBlock_PY-AA" + STR(A_Index266 + 1) + "AA", programmingBlock_PY[A_Index266]);
            }
        }
    } else {
        if (COUNT_programmingBlock_PY != 0) {
            for (int A_Index267 = 0; A_Index267 < COUNT_programmingBlock_PY + 0; A_Index267++) {
                if (langToConvertTo == langFileExtension_2) {
                    htCode = StrReplace(htCode, "programmingBlock_PY-programmingBlock_PY-AA" + STR(A_Index267 + 1) + "AA", keyWordCodeInTheTranspiledLangStartPY_2 + Chr(10) + programmingBlock_PY[A_Index267] + Chr(10) + keyWordCodeInTheTranspiledLangEndPY_2);
                } else {
                    htCode = StrReplace(htCode, "programmingBlock_PY-programmingBlock_PY-AA" + STR(A_Index267 + 1) + "AA", Chr(10));
                }
            }
        }
    }
    if (langToConvertTo == "js") {
        if (COUNT_programmingBlock_JS != 0) {
            for (int A_Index268 = 0; A_Index268 < COUNT_programmingBlock_JS + 0; A_Index268++) {
                htCode = StrReplace(htCode, "programmingBlock_JS-programmingBlock_JS-AA" + STR(A_Index268 + 1) + "AA", programmingBlock_JS[A_Index268]);
            }
        }
    } else {
        if (COUNT_programmingBlock_JS != 0) {
            for (int A_Index269 = 0; A_Index269 < COUNT_programmingBlock_JS + 0; A_Index269++) {
                if (langToConvertTo == langFileExtension_2) {
                    htCode = StrReplace(htCode, "programmingBlock_JS-programmingBlock_JS-AA" + STR(A_Index269 + 1) + "AA", keyWordCodeInTheTranspiledLangStartJS_2 + Chr(10) + programmingBlock_JS[A_Index269] + Chr(10) + keyWordCodeInTheTranspiledLangEndJS_2);
                } else {
                    htCode = StrReplace(htCode, "programmingBlock_JS-programmingBlock_JS-AA" + STR(A_Index269 + 1) + "AA", Chr(10));
                }
            }
        }
    }
    if (langToConvertTo == "go") {
        if (COUNT_programmingBlock_GO != 0) {
            for (int A_Index270 = 0; A_Index270 < COUNT_programmingBlock_GO + 0; A_Index270++) {
                htCode = StrReplace(htCode, "programmingBlock_GO-programmingBlock_GO-AA" + STR(A_Index270 + 1) + "AA", programmingBlock_GO[A_Index270]);
            }
        }
    } else {
        if (COUNT_programmingBlock_GO != 0) {
            for (int A_Index271 = 0; A_Index271 < COUNT_programmingBlock_GO + 0; A_Index271++) {
                if (langToConvertTo == langFileExtension_2) {
                    htCode = StrReplace(htCode, "programmingBlock_GO-programmingBlock_GO-AA" + STR(A_Index271 + 1) + "AA", keyWordCodeInTheTranspiledLangStartGO_2 + Chr(10) + programmingBlock_GO[A_Index271] + Chr(10) + keyWordCodeInTheTranspiledLangEndGO_2);
                } else {
                    htCode = StrReplace(htCode, "programmingBlock_GO-programmingBlock_GO-AA" + STR(A_Index271 + 1) + "AA", Chr(10));
                }
            }
        }
    }
    if (langToConvertTo == "lua") {
        if (COUNT_programmingBlock_LUA != 0) {
            for (int A_Index272 = 0; A_Index272 < COUNT_programmingBlock_LUA + 0; A_Index272++) {
                htCode = StrReplace(htCode, "programmingBlock_LUA-programmingBlock_LUA-AA" + STR(A_Index272 + 1) + "AA", programmingBlock_LUA[A_Index272]);
            }
        }
    } else {
        if (COUNT_programmingBlock_LUA != 0) {
            for (int A_Index273 = 0; A_Index273 < COUNT_programmingBlock_LUA + 0; A_Index273++) {
                if (langToConvertTo == langFileExtension_2) {
                    htCode = StrReplace(htCode, "programmingBlock_LUA-programmingBlock_LUA-AA" + STR(A_Index273 + 1) + "AA", keyWordCodeInTheTranspiledLangStartLUA_2 + Chr(10) + programmingBlock_LUA[A_Index273] + Chr(10) + keyWordCodeInTheTranspiledLangEndLUA_2);
                } else {
                    htCode = StrReplace(htCode, "programmingBlock_LUA-programmingBlock_LUA-AA" + STR(A_Index273 + 1) + "AA", Chr(10));
                }
            }
        }
    }
    if (langToConvertTo == "cs") {
        if (COUNT_programmingBlock_CS != 0) {
            for (int A_Index274 = 0; A_Index274 < COUNT_programmingBlock_CS + 0; A_Index274++) {
                htCode = StrReplace(htCode, "programmingBlock_CS-programmingBlock_CS-AA" + STR(A_Index274 + 1) + "AA", programmingBlock_CS[A_Index274]);
            }
        }
    } else {
        if (COUNT_programmingBlock_CS != 0) {
            for (int A_Index275 = 0; A_Index275 < COUNT_programmingBlock_CS + 0; A_Index275++) {
                if (langToConvertTo == langFileExtension_2) {
                    htCode = StrReplace(htCode, "programmingBlock_CS-programmingBlock_CS-AA" + STR(A_Index275 + 1) + "AA", keyWordCodeInTheTranspiledLangStartCS_2 + Chr(10) + programmingBlock_CS[A_Index275] + Chr(10) + keyWordCodeInTheTranspiledLangEndCS_2);
                } else {
                    htCode = StrReplace(htCode, "programmingBlock_CS-programmingBlock_CS-AA" + STR(A_Index275 + 1) + "AA", Chr(10));
                }
            }
        }
    }
    if (langToConvertTo == "java") {
        if (COUNT_programmingBlock_JAVA != 0) {
            for (int A_Index276 = 0; A_Index276 < COUNT_programmingBlock_JAVA + 0; A_Index276++) {
                htCode = StrReplace(htCode, "programmingBlock_JAVA-programmingBlock_JAVA-AA" + STR(A_Index276 + 1) + "AA", programmingBlock_JAVA[A_Index276]);
            }
        }
    } else {
        if (COUNT_programmingBlock_JAVA != 0) {
            for (int A_Index277 = 0; A_Index277 < COUNT_programmingBlock_JAVA + 0; A_Index277++) {
                if (langToConvertTo == langFileExtension_2) {
                    htCode = StrReplace(htCode, "programmingBlock_JAVA-programmingBlock_JAVA-AA" + STR(A_Index277 + 1) + "AA", keyWordCodeInTheTranspiledLangStartJAVA_2 + Chr(10) + programmingBlock_JAVA[A_Index277] + Chr(10) + keyWordCodeInTheTranspiledLangEndJAVA_2);
                } else {
                    htCode = StrReplace(htCode, "programmingBlock_JAVA-programmingBlock_JAVA-AA" + STR(A_Index277 + 1) + "AA", Chr(10));
                }
            }
        }
    }
    if (langToConvertTo == "kt") {
        if (COUNT_programmingBlock_KT != 0) {
            for (int A_Index278 = 0; A_Index278 < COUNT_programmingBlock_KT + 0; A_Index278++) {
                htCode = StrReplace(htCode, "programmingBlock_KT-programmingBlock_KT-AA" + STR(A_Index278 + 1) + "AA", programmingBlock_KT[A_Index278]);
            }
        }
    } else {
        if (COUNT_programmingBlock_KT != 0) {
            for (int A_Index279 = 0; A_Index279 < COUNT_programmingBlock_KT + 0; A_Index279++) {
                if (langToConvertTo == langFileExtension_2) {
                    htCode = StrReplace(htCode, "programmingBlock_KT-programmingBlock_KT-AA" + STR(A_Index279 + 1) + "AA", keyWordCodeInTheTranspiledLangStartKT_2 + Chr(10) + programmingBlock_KT[A_Index279] + Chr(10) + keyWordCodeInTheTranspiledLangEndKT_2);
                } else {
                    htCode = StrReplace(htCode, "programmingBlock_KT-programmingBlock_KT-AA" + STR(A_Index279 + 1) + "AA", Chr(10));
                }
            }
        }
    }
    if (langToConvertTo == "rb") {
        if (COUNT_programmingBlock_RB != 0) {
            for (int A_Index280 = 0; A_Index280 < COUNT_programmingBlock_RB + 0; A_Index280++) {
                htCode = StrReplace(htCode, "programmingBlock_RB-programmingBlock_RB-AA" + STR(A_Index280 + 1) + "AA", programmingBlock_RB[A_Index280]);
            }
        }
    } else {
        if (COUNT_programmingBlock_RB != 0) {
            for (int A_Index281 = 0; A_Index281 < COUNT_programmingBlock_RB + 0; A_Index281++) {
                if (langToConvertTo == langFileExtension_2) {
                    htCode = StrReplace(htCode, "programmingBlock_RB-programmingBlock_RB-AA" + STR(A_Index281 + 1) + "AA", keyWordCodeInTheTranspiledLangStartRB_2 + Chr(10) + programmingBlock_RB[A_Index281] + Chr(10) + keyWordCodeInTheTranspiledLangEndRB_2);
                } else {
                    htCode = StrReplace(htCode, "programmingBlock_RB-programmingBlock_RB-AA" + STR(A_Index281 + 1) + "AA", Chr(10));
                }
            }
        }
    }
    if (langToConvertTo == "nim") {
        if (COUNT_programmingBlock_NIM != 0) {
            for (int A_Index282 = 0; A_Index282 < COUNT_programmingBlock_NIM + 0; A_Index282++) {
                htCode = StrReplace(htCode, "programmingBlock_NIM-programmingBlock_NIM-AA" + STR(A_Index282 + 1) + "AA", programmingBlock_NIM[A_Index282]);
            }
        }
    } else {
        if (COUNT_programmingBlock_NIM != 0) {
            for (int A_Index283 = 0; A_Index283 < COUNT_programmingBlock_NIM + 0; A_Index283++) {
                if (langToConvertTo == langFileExtension_2) {
                    htCode = StrReplace(htCode, "programmingBlock_NIM-programmingBlock_NIM-AA" + STR(A_Index283 + 1) + "AA", keyWordCodeInTheTranspiledLangStartNIM_2 + Chr(10) + programmingBlock_NIM[A_Index283] + Chr(10) + keyWordCodeInTheTranspiledLangEndNIM_2);
                } else {
                    htCode = StrReplace(htCode, "programmingBlock_NIM-programmingBlock_NIM-AA" + STR(A_Index283 + 1) + "AA", Chr(10));
                }
            }
        }
    }
    if (langToConvertTo == "ahk") {
        if (COUNT_programmingBlock_AHK != 0) {
            for (int A_Index284 = 0; A_Index284 < COUNT_programmingBlock_AHK + 0; A_Index284++) {
                htCode = StrReplace(htCode, "programmingBlock_AHK-programmingBlock_AHK-AA" + STR(A_Index284 + 1) + "AA", programmingBlock_AHK[A_Index284]);
            }
        }
    } else {
        if (COUNT_programmingBlock_AHK != 0) {
            for (int A_Index285 = 0; A_Index285 < COUNT_programmingBlock_AHK + 0; A_Index285++) {
                if (langToConvertTo == langFileExtension_2) {
                    htCode = StrReplace(htCode, "programmingBlock_AHK-programmingBlock_AHK-AA" + STR(A_Index285 + 1) + "AA", keyWordCodeInTheTranspiledLangStartAHK_2 + Chr(10) + programmingBlock_AHK[A_Index285] + Chr(10) + keyWordCodeInTheTranspiledLangEndAHK_2);
                } else {
                    htCode = StrReplace(htCode, "programmingBlock_AHK-programmingBlock_AHK-AA" + STR(A_Index285 + 1) + "AA", Chr(10));
                }
            }
        }
    }
    if (langToConvertTo == "swift") {
        if (COUNT_programmingBlock_SWIFT != 0) {
            for (int A_Index286 = 0; A_Index286 < COUNT_programmingBlock_SWIFT + 0; A_Index286++) {
                htCode = StrReplace(htCode, "programmingBlock_SWIFT-programmingBlock_SWIFT-AA" + STR(A_Index286 + 1) + "AA", programmingBlock_SWIFT[A_Index286]);
            }
        }
    } else {
        if (COUNT_programmingBlock_SWIFT != 0) {
            for (int A_Index287 = 0; A_Index287 < COUNT_programmingBlock_SWIFT + 0; A_Index287++) {
                if (langToConvertTo == langFileExtension_2) {
                    htCode = StrReplace(htCode, "programmingBlock_SWIFT-programmingBlock_SWIFT-AA" + STR(A_Index287 + 1) + "AA", keyWordCodeInTheTranspiledLangStartSWIFT_2 + Chr(10) + programmingBlock_SWIFT[A_Index287] + Chr(10) + keyWordCodeInTheTranspiledLangEndSWIFT_2);
                } else {
                    htCode = StrReplace(htCode, "programmingBlock_SWIFT-programmingBlock_SWIFT-AA" + STR(A_Index287 + 1) + "AA", Chr(10));
                }
            }
        }
    }
    if (langToConvertTo == "dart") {
        if (COUNT_programmingBlock_DART != 0) {
            for (int A_Index288 = 0; A_Index288 < COUNT_programmingBlock_DART + 0; A_Index288++) {
                htCode = StrReplace(htCode, "programmingBlock_DART-programmingBlock_DART-AA" + STR(A_Index288 + 1) + "AA", programmingBlock_DART[A_Index288]);
            }
        }
    } else {
        if (COUNT_programmingBlock_DART != 0) {
            for (int A_Index289 = 0; A_Index289 < COUNT_programmingBlock_DART + 0; A_Index289++) {
                if (langToConvertTo == langFileExtension_2) {
                    htCode = StrReplace(htCode, "programmingBlock_DART-programmingBlock_DART-AA" + STR(A_Index289 + 1) + "AA", keyWordCodeInTheTranspiledLangStartDART_2 + Chr(10) + programmingBlock_DART[A_Index289] + Chr(10) + keyWordCodeInTheTranspiledLangEndDART_2);
                } else {
                    htCode = StrReplace(htCode, "programmingBlock_DART-programmingBlock_DART-AA" + STR(A_Index289 + 1) + "AA", Chr(10));
                }
            }
        }
    }
    if (langToConvertTo == "ts") {
        if (COUNT_programmingBlock_TS != 0) {
            for (int A_Index290 = 0; A_Index290 < COUNT_programmingBlock_TS + 0; A_Index290++) {
                htCode = StrReplace(htCode, "programmingBlock_TS-programmingBlock_TS-AA" + STR(A_Index290 + 1) + "AA", programmingBlock_TS[A_Index290]);
            }
        }
    } else {
        if (COUNT_programmingBlock_TS != 0) {
            for (int A_Index291 = 0; A_Index291 < COUNT_programmingBlock_TS + 0; A_Index291++) {
                if (langToConvertTo == langFileExtension_2) {
                    htCode = StrReplace(htCode, "programmingBlock_TS-programmingBlock_TS-AA" + STR(A_Index291 + 1) + "AA", keyWordCodeInTheTranspiledLangStartTS_2 + Chr(10) + programmingBlock_TS[A_Index291] + Chr(10) + keyWordCodeInTheTranspiledLangEndTS_2);
                } else {
                    htCode = StrReplace(htCode, "programmingBlock_TS-programmingBlock_TS-AA" + STR(A_Index291 + 1) + "AA", Chr(10));
                }
            }
        }
    }
    if (langToConvertTo == "groovy") {
        if (COUNT_programmingBlock_GROOVY != 0) {
            for (int A_Index292 = 0; A_Index292 < COUNT_programmingBlock_GROOVY + 0; A_Index292++) {
                htCode = StrReplace(htCode, "programmingBlock_GROOVY-programmingBlock_GROOVY-AA" + STR(A_Index292 + 1) + "AA", programmingBlock_GROOVY[A_Index292]);
            }
        }
    } else {
        if (COUNT_programmingBlock_GROOVY != 0) {
            for (int A_Index293 = 0; A_Index293 < COUNT_programmingBlock_GROOVY + 0; A_Index293++) {
                if (langToConvertTo == langFileExtension_2) {
                    htCode = StrReplace(htCode, "programmingBlock_GROOVY-programmingBlock_GROOVY-AA" + STR(A_Index293 + 1) + "AA", keyWordCodeInTheTranspiledLangStartGROOVY_2 + Chr(10) + programmingBlock_GROOVY[A_Index293] + Chr(10) + keyWordCodeInTheTranspiledLangEndGROOVY_2);
                } else {
                    htCode = StrReplace(htCode, "programmingBlock_GROOVY-programmingBlock_GROOVY-AA" + STR(A_Index293 + 1) + "AA", Chr(10));
                }
            }
        }
    }
    ...
    if (langToConvertTo == langFileExtension_2) {
        if (COUNT_programmingBlock_HTVM != 0) {
            for (int A_Index294 = 0; A_Index294 < COUNT_programmingBlock_HTVM + 0; A_Index294++) {
                htCode = StrReplace(htCode, "programmingBlock_HTVM-programmingBlock_HTVM-AA" + STR(A_Index294 + 1) + "AA", programmingBlock_HTVM[A_Index294]);
            }
        }
    } else {
        if (COUNT_programmingBlock_HTVM != 0) {
            for (int A_Index295 = 0; A_Index295 < COUNT_programmingBlock_HTVM + 0; A_Index295++) {
                htCode = StrReplace(htCode, "programmingBlock_HTVM-programmingBlock_HTVM-AA" + STR(A_Index295 + 1) + "AA", Chr(10));
            }
        }
    }
    if (langToConvertTo != langFileExtension_2) {
        ...
        ALoopField = "";
        str23 = "";
        std::vector<std::string> items296 = LoopParseFunc(htCode, "\n", "\r");
        for (size_t A_Index296 = 0; A_Index296 < items296.size() + 0; A_Index296++) {
            std::string A_LoopField296 = items296[A_Index296 - 0];
            ALoopField = A_LoopField296;
            if (InStr(A_LoopField296, "fihuiuuhuuhtheidFor--asds")) {
                    ...
                    if (theIdNumOfThe34 == A_Index297 + 1) {
                        if (langToConvertTo == langFileExtension_2) {
                            if (keyWordEscpaeChar_2 == "\\" && keyWordEscpaeChar != "\\") {
                                ALoopField = StrReplace(ALoopField, "fihuiuuhuuhtheidFor--asds" + str21 + "as--" + str21 + "theuhturtyphoutr--" + Chr(65) + Chr(65) + STR(A_Index297 + 1) + Chr(65) + Chr(65), StrReplace(StrReplace(fString(theIdNumOfThe34theVar[A_Index297 + 1]), "\\", "\\\\"), keyWordEscpaeChar, keyWordEscpaeChar_2) + Chr(34));
                            } else {
                                ALoopField = StrReplace(ALoopField, "fihuiuuhuuhtheidFor--asds" + str21 + "as--" + str21 + "theuhturtyphoutr--" + Chr(65) + Chr(65) + STR(A_Index297 + 1) + Chr(65) + Chr(65), StrReplace(fString(theIdNumOfThe34theVar[A_Index297 + 1]), keyWordEscpaeChar, keyWordEscpaeChar_2) + Chr(34));
                            }
                        } else {
                            ALoopField = StrReplace(ALoopField, "fihuiuuhuuhtheidFor--asds" + str21 + "as--" + str21 + "theuhturtyphoutr--" + Chr(65) + Chr(65) + STR(A_Index297 + 1) + Chr(65) + Chr(65), StrReplace(fString(theIdNumOfThe34theVar[A_Index297 + 1]), keyWordEscpaeChar, "\\") + Chr(34));
                        }
                    } else {
                        if (langToConvertTo == langFileExtension_2) {
                            if (keyWordEscpaeChar_2 == "\\" && keyWordEscpaeChar != "\\") {
                                ALoopField = StrReplace(ALoopField, "fihuiuuhuuhtheidFor--asds" + str21 + "as--" + str21 + "theuhturtyphoutr--" + Chr(65) + Chr(65) + STR(A_Index297 + 1) + Chr(65) + Chr(65), StrReplace(StrReplace(fString(theIdNumOfThe34theVar[A_Index297 + 1]), "\\", "\\\\"), keyWordEscpaeChar, keyWordEscpaeChar_2));
                            } else {
                                ALoopField = StrReplace(ALoopField, "fihuiuuhuuhtheidFor--asds" + str21 + "as--" + str21 + "theuhturtyphoutr--" + Chr(65) + Chr(65) + STR(A_Index297 + 1) + Chr(65) + Chr(65), StrReplace(fString(theIdNumOfThe34theVar[A_Index297 + 1]), keyWordEscpaeChar, keyWordEscpaeChar_2));
                            }
                        } else {
                            ALoopField = StrReplace(ALoopField, "fihuiuuhuuhtheidFor--asds" + str21 + "as--" + str21 + "theuhturtyphoutr--" + Chr(65) + Chr(65) + STR(A_Index297 + 1) + Chr(65) + Chr(65), StrReplace(fString(theIdNumOfThe34theVar[A_Index297 + 1]), keyWordEscpaeChar, "\\"));
                        }
                    }
                }
            }
    ...
    int correctLang = 0;
    std::vector<std::string> items298 = LoopParseFunc(instructions, "\n", "\r");
    for (size_t A_Index298 = 0; A_Index298 < items298.size() + 0; A_Index298++) {
        std::string A_LoopField298 = items298[A_Index298 - 0];
        if (Trim(A_LoopField298) == "funcEND======================funcEND==============") {
            areWeInAFuncFromInstructions = 0;
            areWeInAFuncFromInstructionsLineNum = 0;
        ...
        if (areWeInAFuncFromInstructions == 1) {
            if (areWeInAFuncFromInstructionsLineNum == 1) {
                // name of the func
                funcLangHolder = StringTrimLeft(A_LoopField298, 5);
            ...
            if (areWeInAFuncFromInstructionsLineNum == 2) {
                // name of the func
                funcNameHolder = StringTrimLeft(A_LoopField298, 5);
                if (correctLang == 1 && InStr(htCode, Trim(funcNameHolder))) {
            ...
            if (areWeInAFuncFromInstructionsLineNum == 3) {
                // all libs
                funcLibsHolder = StringTrimLeft(A_LoopField298, 5);
                if (correctLang == 1 && InStr(htCode, Trim(funcNameHolder))) {
            ...
            if (areWeInAFuncFromInstructionsLineNum == 4) {
                // func description
                funcDescriptionHolder = StringTrimLeft(A_LoopField298, 12);
                if (correctLang == 1 && InStr(htCode, Trim(funcNameHolder))) {
            ...
            if (areWeInAFuncFromInstructionsLineNum >= 5) {
                // the full func
                if (correctLang == 1 && InStr(htCode, Trim(funcNameHolder))) {
                    funcFuncHolder += A_LoopField298 + Chr(10);
                }
            }
            //MsgBox, % A_LoopField
            areWeInAFuncFromInstructionsLineNum++;
        }
        if (Trim(A_LoopField298) == "func======================func==============") {
    ...
    if (!(HTVM_Size(allFuncNames) <= 0)) {
        for (int A_Index299 = 0; A_Index299 < HTVM_Size(allFuncNames) + 0; A_Index299++) {
            if (InStr(htCode, allFuncNames[A_Index299]) + "(") {
                //MsgBox, % allFuncNames[A_Index]
                allFuncsToPutAtTop += allFuncs[A_Index299] + Chr(10);
                if (Trim(allFuncLibs[A_Index299]) != "null") {
                    allLibsToPutAtTop += allFuncLibs[A_Index299] + "|";
                }
            }
        }
        allLibsToPutAtTop = StringTrimRight(allLibsToPutAtTop, 1);
        std::string allLibsToPutAtTopTEMP = "";
        std::vector<std::string> items300 = LoopParseFunc(allLibsToPutAtTop, "|");
        for (size_t A_Index300 = 0; A_Index300 < items300.size() + 0; A_Index300++) {
            std::string A_LoopField300 = items300[A_Index300 - 0];
            allLibsToPutAtTopTEMP += A_LoopField300 + Chr(10);
        }
    ...
    if (langToConvertTo != langFileExtension_2) {
        for (int A_Index301 = 0; A_Index301 < HTVM_Size(ospDic1) + 0; A_Index301++) {
            str00 = ospDic1[A_Index301];
            if (langToConvertTo == "rb") {
    ...
    if (size_ospDic != 0) {
        for (int A_Index302 = 0; A_Index302 < size_ospDic + 0; A_Index302++) {
            HTVM_Pop(ospDic);
        }
    }
    //;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;
    ...
    if (size_ospDic1 != 0) {
        for (int A_Index303 = 0; A_Index303 < size_ospDic1 + 0; A_Index303++) {
            HTVM_Pop(ospDic1);
        }
    }
    //;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;
    ...
    if (size_ospDic2 != 0) {
        for (int A_Index304 = 0; A_Index304 < size_ospDic2 + 0; A_Index304++) {
            HTVM_Pop(ospDic2);
        }
    }
    //;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;
    ...
    if (size_htCodeFixRubyGlobalVars_EXPOSED != 0) {
        for (int A_Index305 = 0; A_Index305 < size_htCodeFixRubyGlobalVars_EXPOSED + 0; A_Index305++) {
    ...
    for (int A_Index306 = 0; A_Index306 < theIdNumOfThe34 + 0; A_Index306++) {
        if (theIdNumOfThe34 == A_Index306 + 1) {
            if (langToConvertTo == langFileExtension_2) {
                if (keyWordEscpaeChar_2 == "\\" && keyWordEscpaeChar != "\\") {
                    htCode = StrReplace(htCode, "ihuiuuhuuhtheidFor--asds" + str21 + "as--" + str21 + "theuhturtyphoutr--" + Chr(65) + Chr(65) + STR(A_Index306 + 1) + Chr(65) + Chr(65), StrReplace(StrReplace(theIdNumOfThe34theVar[A_Index306 + 1], "\\", "\\\\"), keyWordEscpaeChar, keyWordEscpaeChar_2) + Chr(34));
                } else {
                    htCode = StrReplace(htCode, "ihuiuuhuuhtheidFor--asds" + str21 + "as--" + str21 + "theuhturtyphoutr--" + Chr(65) + Chr(65) + STR(A_Index306 + 1) + Chr(65) + Chr(65), StrReplace(theIdNumOfThe34theVar[A_Index306 + 1], keyWordEscpaeChar, keyWordEscpaeChar_2) + Chr(34));
                }
            } else {
                htCode = StrReplace(htCode, "ihuiuuhuuhtheidFor--asds" + str21 + "as--" + str21 + "theuhturtyphoutr--" + Chr(65) + Chr(65) + STR(A_Index306 + 1) + Chr(65) + Chr(65), StrReplace(theIdNumOfThe34theVar[A_Index306 + 1], keyWordEscpaeChar, "\\") + Chr(34));
            }
        } else {
            if (langToConvertTo == langFileExtension_2) {
                if (keyWordEscpaeChar_2 == "\\" && keyWordEscpaeChar != "\\") {
                    htCode = StrReplace(htCode, "ihuiuuhuuhtheidFor--asds" + str21 + "as--" + str21 + "theuhturtyphoutr--" + Chr(65) + Chr(65) + STR(A_Index306 + 1) + Chr(65) + Chr(65), StrReplace(StrReplace(theIdNumOfThe34theVar[A_Index306 + 1], "\\", "\\\\"), keyWordEscpaeChar, keyWordEscpaeChar_2));
                } else {
                    htCode = StrReplace(htCode, "ihuiuuhuuhtheidFor--asds" + str21 + "as--" + str21 + "theuhturtyphoutr--" + Chr(65) + Chr(65) + STR(A_Index306 + 1) + Chr(65) + Chr(65), StrReplace(theIdNumOfThe34theVar[A_Index306 + 1], keyWordEscpaeChar, keyWordEscpaeChar_2));
                }
            } else {
                htCode = StrReplace(htCode, "ihuiuuhuuhtheidFor--asds" + str21 + "as--" + str21 + "theuhturtyphoutr--" + Chr(65) + Chr(65) + STR(A_Index306 + 1) + Chr(65) + Chr(65), StrReplace(theIdNumOfThe34theVar[A_Index306 + 1], keyWordEscpaeChar, "\\"));
            }
        }
    }
    if (langToConvertTo == langFileExtension_2) {
    ...
    if (isNotHTVMfile2 == 0) {
        ...
        if (langToConvertTo == langFileExtension_2) {
            for (int A_Index307 = 0; A_Index307 < HTVM_Size(weAreInMLSarr) + 0; A_Index307++) {
                htCode = StrReplace(htCode, "aejsoydoxubviycbdgsut7eoy-ihsxbvhowadesdio6376e7wuwyuau--szd--AA" + STR(A_Index307) + "AA", weAreInMLSarr[A_Index307]);
            }
    ...
    if (size_weAreInMLSarr != 0) {
        for (int A_Index308 = 0; A_Index308 < size_weAreInMLSarr + 0; A_Index308++) {
...
void HTVMv2() {
    ...
    if (HTVM_getLang_HTVM() == "cpp" || HTVM_getLang_HTVM() == "py") {
        ...
        std::vector<std::string> items310 = LoopParseFunc(allArgs, "\n", "\r");
        for (size_t A_Index310 = 0; A_Index310 < items310.size() + 0; A_Index310++) {
            std::string A_LoopField310 = items310[A_Index310 - 0];
            if (A_Index310 == 0) {
                numOfParams++;
                argCODE = FileRead(Trim(A_LoopField310));
                argCODEfile = Trim(A_LoopField310);
            }
            else if (A_Index310 == 1) {
                numOfParams++;
                argHTVMinstr = Trim(A_LoopField310);
            }
            else if (A_Index310 == 2) {
                numOfParams++;
                argLangTo = Trim(A_LoopField310);
            } else {
                numOfParams++;
                HTVM_Append(argHTVMinstrMORE, Trim(A_LoopField310));
            }
        }
        //print("===============123431234===========start=====")
        for (int A_Index311 = 0; A_Index311 < HTVM_Size(argHTVMinstrMORE) + 0; A_Index311++) {
