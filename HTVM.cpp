...
std::string extraFlexableFuncCallsHelp(std::string line) {
    ...
    std::string temp = "";
    std::vector<std::string> items71 = LoopParseFunc(line, ".");
    ...
    for (int A_Index72 = 0; A_Index72 < HTVM_Size(allFuncsDot) + 0; A_Index72++) {
        if (HTVM_Size(allFuncsDot) == A_Index72 + 1) {
            ...
            for (int A_Index73 = 0; A_Index73 < endParentesesToPut + 1 + 0; A_Index73++) {
                out += ")";
            }
        } else {
            endParentesesToPut++;
...
std::string splitFunctionCalls(std::string line) {
    ...
    if (InStr(line, "[")) {
        doseBracketsExist = 1;
        std::vector<std::string> items75 = LoopParseFunc(line);
    ...
    int seenDot = 0;
    std::vector<std::string> items78 = LoopParseFunc(line);
    ...
    for (int A_Index79 = 0; A_Index79 < HTVM_Size(funcCalls) + 0; A_Index79++) {
    ...
    if (doWeAddParentisiesLater != 0) {
        for (int A_Index80 = 0; A_Index80 < doWeAddParentisiesLater + 0; A_Index80++) {
            arrToStrSTR = "(" + arrToStrSTR + ")";
        }
    }
    if (doseBracketsExist == 1) {
        for (int A_Index81 = 0; A_Index81 < HTVM_Size(saveDoseBracketsExist) + 0; A_Index81++) {
    ...
    if (doseFuncAtTheBegggging == 1) {
        for (int A_Index82 = 0; A_Index82 < HTVM_Size(FuncAtTheBegggging) + 0; A_Index82++) {
            arrToStrSTR = FuncAtTheBegggging[A_Index82] + arrToStrSTR;
        }
        for (int A_Index83 = 0; A_Index83 < doWeAddParentisiesLater2 + 0; A_Index83++) {
            arrToStrSTR = arrToStrSTR + ")";
        }
    }
    return arrToStrSTR;
...
std::string extraFlexableFuncCalls(std::string line, std::string availableFuncsInHTVMInHTVM) {
    ...
    int parenthesesDepth = 0;
    int seenDot = 0;
    std::vector<std::string> items84 = LoopParseFunc(line);
    ...
    out = "";
    int i = 0;
    for (int A_Index85 = 0; A_Index85 < HTVM_Size(funcCalls) + 0; A_Index85++) {
        i = A_Index85;
        isFuncExist = 0;
        std::vector<std::string> items86 = LoopParseFunc(availableFuncsInHTVMInHTVM, "\n", "\r");
...
std::string expressionParserTranspiler(std::string expression) {
    ...
    // osp
    if (langToConvertTo != langFileExtension_2) {
        for (int A_Index87 = 0; A_Index87 < HTVM_Size(ospDic1) + 0; A_Index87++) {
            str00 = ospDic1[A_Index87];
            expression = StrReplace(expression, StrReplace(str00, "_", ".") + ".", str00 + "_");
        }
        for (int A_Index88 = 0; A_Index88 < HTVM_Size(ospDic2) + 0; A_Index88++) {
    ...
    for (int A_Index89 = 0; A_Index89 < 29 + 0; A_Index89++) {
...
std::string FixRubyGlobalVars(std::string line) {
    ...
    std::vector<std::string> items90 = LoopParseFunc(line, "\n", "\r");
    ...
    for (int A_Index91 = 0; A_Index91 < HTVM_Size(htCodeFixRubyGlobalVars) + 0; A_Index91++) {
...
std::string propHELP(std::string line, std::string lineOspHelpLine) {
    ...
    std::string ALoopField = "";
    std::vector<std::string> items92 = LoopParseFunc(line, "\n", "\r");
    for (size_t A_Index92 = 0; A_Index92 < items92.size() + 0; A_Index92++) {
        ...
        if (InStr(A_LoopField92, " " + keyWordAssign + " ") || InStr(A_LoopField92, " " + keyWordAssignAdd + " ") || InStr(A_LoopField92, " " + keyWordAssignConcat + " ") || InStr(A_LoopField92, " " + keyWordAssignSub + " ") || InStr(A_LoopField92, " " + keyWordAssignMul + " ") || InStr(A_LoopField92, " " + keyWordAssignDiv + " ") || InStr(A_LoopField92, " " + keyWordAssignMod + " ") || InStr(A_LoopField92, " " + keyWordAssignShiftLeft + " ") || InStr(A_LoopField92, " " + keyWordAssignShiftRight + " ") || InStr(A_LoopField92, " " + keyWordLogicalAssignShiftRight + " ") || InStr(A_LoopField92, " " + keyWordAssignBitAnd + " ") || InStr(A_LoopField92, " " + keyWordAssignBitOr + " ") || InStr(A_LoopField92, " " + keyWordAssignBitXor + " ") && lineDone == 0) {
            ...
            std::vector<std::string> items99 = LoopParseFunc(str1, " ");
...
std::string guiParserStep4(std::string option) {
    ...
    if (InStr(option, ":")) {
        ...
        //print(option1)
        //print("=-=-=-=-=-=-=-=-=-=-::::::::::::::::::=-=-=-=-=-=-=-=-=-=-=-=-=")
        for (int A_Index100 = 0; A_Index100 < HTVM_Size(aliases_colon_id) + 0; A_Index100++) {
            if (StrLower(option0) == StrLower(aliases_colon_id[A_Index100])) {
                option0 = canonical_id;
                break;
            }
        }
        for (int A_Index101 = 0; A_Index101 < HTVM_Size(aliases_colon_parentId) + 0; A_Index101++) {
            if (StrLower(option0) == StrLower(aliases_colon_parentId[A_Index101])) {
                option0 = canonical_parentId;
                break;
            }
        }
        for (int A_Index102 = 0; A_Index102 < HTVM_Size(aliases_colon_x) + 0; A_Index102++) {
        ...
        for (int A_Index103 = 0; A_Index103 < HTVM_Size(aliases_colon_y) + 0; A_Index103++) {
        ...
        for (int A_Index104 = 0; A_Index104 < HTVM_Size(aliases_colon_width) + 0; A_Index104++) {
        ...
        for (int A_Index105 = 0; A_Index105 < HTVM_Size(aliases_colon_height) + 0; A_Index105++) {
        ...
        for (int A_Index106 = 0; A_Index106 < HTVM_Size(aliases_colon_zIndex) + 0; A_Index106++) {
            if (StrLower(option0) == StrLower(aliases_colon_zIndex[A_Index106])) {
                option0 = canonical_zIndex;
                break;
            }
        }
        for (int A_Index107 = 0; A_Index107 < HTVM_Size(aliases_colon_value) + 0; A_Index107++) {
            if (StrLower(option0) == StrLower(aliases_colon_value[A_Index107])) {
                option0 = canonical_value;
                break;
            }
        }
        for (int A_Index108 = 0; A_Index108 < HTVM_Size(aliases_colon_placeholder) + 0; A_Index108++) {
            if (StrLower(option0) == StrLower(aliases_colon_placeholder[A_Index108])) {
                option0 = canonical_placeholder;
                break;
            }
        }
        for (int A_Index109 = 0; A_Index109 < HTVM_Size(aliases_colon_callback) + 0; A_Index109++) {
            if (StrLower(option0) == StrLower(aliases_colon_callback[A_Index109])) {
                option0 = canonical_callback;
                break;
            }
        }
        for (int A_Index110 = 0; A_Index110 < HTVM_Size(aliases_colon_backgroundColor) + 0; A_Index110++) {
        ...
        for (int A_Index111 = 0; A_Index111 < HTVM_Size(aliases_colon_color) + 0; A_Index111++) {
        ...
        for (int A_Index112 = 0; A_Index112 < HTVM_Size(aliases_colon_rounding) + 0; A_Index112++) {
            if (StrLower(option0) == StrLower(aliases_colon_rounding[A_Index112])) {
                option0 = canonical_rounding;
                break;
            }
        }
        for (int A_Index113 = 0; A_Index113 < HTVM_Size(aliases_colon_border) + 0; A_Index113++) {
            if (StrLower(option0) == StrLower(aliases_colon_border[A_Index113])) {
                option0 = canonical_border;
                break;
            }
        }
        for (int A_Index114 = 0; A_Index114 < HTVM_Size(aliases_colon_rawCss) + 0; A_Index114++) {
            if (StrLower(option0) == StrLower(aliases_colon_rawCss[A_Index114])) {
                option0 = canonical_rawCss;
                break;
            }
        }
        for (int A_Index115 = 0; A_Index115 < HTVM_Size(aliases_colon_size) + 0; A_Index115++) {
            if (StrLower(option0) == StrLower(aliases_colon_size[A_Index115])) {
                option0 = canonical_size;
                break;
            }
        }
        for (int A_Index116 = 0; A_Index116 < HTVM_Size(aliases_colon_isEnabled) + 0; A_Index116++) {
            if (StrLower(option0) == StrLower(aliases_colon_isEnabled[A_Index116])) {
                option0 = canonical_isEnabled;
                break;
            }
        }
        for (int A_Index117 = 0; A_Index117 < HTVM_Size(aliases_colon_isVisible) + 0; A_Index117++) {
            if (StrLower(option0) == StrLower(aliases_colon_isVisible[A_Index117])) {
                option0 = canonical_isVisible;
                break;
            }
        }
        for (int A_Index118 = 0; A_Index118 < HTVM_Size(aliases_colon_isDeleting) + 0; A_Index118++) {
        ...
        //print(option1)
        //print("=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=")
        for (int A_Index119 = 0; A_Index119 < HTVM_Size(aliases_single_id) + 0; A_Index119++) {
            if (StrLower(option0) == StrLower(aliases_single_id[A_Index119])) {
                option0 = canonical_id;
                break;
            }
        }
        for (int A_Index120 = 0; A_Index120 < HTVM_Size(aliases_single_parentId) + 0; A_Index120++) {
            if (StrLower(option0) == StrLower(aliases_single_parentId[A_Index120])) {
                option0 = canonical_parentId;
                break;
            }
        }
        for (int A_Index121 = 0; A_Index121 < HTVM_Size(aliases_single_x) + 0; A_Index121++) {
        ...
        for (int A_Index122 = 0; A_Index122 < HTVM_Size(aliases_single_y) + 0; A_Index122++) {
        ...
        for (int A_Index123 = 0; A_Index123 < HTVM_Size(aliases_single_width) + 0; A_Index123++) {
        ...
        for (int A_Index124 = 0; A_Index124 < HTVM_Size(aliases_single_height) + 0; A_Index124++) {
        ...
        for (int A_Index125 = 0; A_Index125 < HTVM_Size(aliases_single_zIndex) + 0; A_Index125++) {
            if (StrLower(option0) == StrLower(aliases_single_zIndex[A_Index125])) {
                option0 = canonical_zIndex;
                break;
            }
        }
        for (int A_Index126 = 0; A_Index126 < HTVM_Size(aliases_single_value) + 0; A_Index126++) {
            if (StrLower(option0) == StrLower(aliases_single_value[A_Index126])) {
                option0 = canonical_value;
                break;
            }
        }
        for (int A_Index127 = 0; A_Index127 < HTVM_Size(aliases_single_placeholder) + 0; A_Index127++) {
            if (StrLower(option0) == StrLower(aliases_single_placeholder[A_Index127])) {
                option0 = canonical_placeholder;
                break;
            }
        }
        for (int A_Index128 = 0; A_Index128 < HTVM_Size(aliases_single_callback) + 0; A_Index128++) {
            if (StrLower(option0) == StrLower(aliases_single_callback[A_Index128])) {
                option0 = canonical_callback;
                break;
            }
        }
        for (int A_Index129 = 0; A_Index129 < HTVM_Size(aliases_single_backgroundColor) + 0; A_Index129++) {
        ...
        for (int A_Index130 = 0; A_Index130 < HTVM_Size(aliases_single_color) + 0; A_Index130++) {
        ...
        for (int A_Index131 = 0; A_Index131 < HTVM_Size(aliases_single_rounding) + 0; A_Index131++) {
            if (StrLower(option0) == StrLower(aliases_single_rounding[A_Index131])) {
                option0 = canonical_rounding;
                break;
            }
        }
        for (int A_Index132 = 0; A_Index132 < HTVM_Size(aliases_single_border) + 0; A_Index132++) {
            if (StrLower(option0) == StrLower(aliases_single_border[A_Index132])) {
                option0 = canonical_border;
                break;
            }
        }
        for (int A_Index133 = 0; A_Index133 < HTVM_Size(aliases_single_rawCss) + 0; A_Index133++) {
            if (StrLower(option0) == StrLower(aliases_single_rawCss[A_Index133])) {
                option0 = canonical_rawCss;
                break;
            }
        }
        for (int A_Index134 = 0; A_Index134 < HTVM_Size(aliases_single_size) + 0; A_Index134++) {
            if (StrLower(option0) == StrLower(aliases_single_size[A_Index134])) {
                option0 = canonical_size;
                break;
            }
        }
        for (int A_Index135 = 0; A_Index135 < HTVM_Size(aliases_single_isEnabled) + 0; A_Index135++) {
            if (StrLower(option0) == StrLower(aliases_single_isEnabled[A_Index135])) {
                option0 = canonical_isEnabled;
                break;
            }
        }
        for (int A_Index136 = 0; A_Index136 < HTVM_Size(aliases_single_isVisible) + 0; A_Index136++) {
            if (StrLower(option0) == StrLower(aliases_single_isVisible[A_Index136])) {
                option0 = canonical_isVisible;
                break;
            }
        }
        for (int A_Index137 = 0; A_Index137 < HTVM_Size(aliases_single_isDeleting) + 0; A_Index137++) {
...
std::string guiParserStep3(std::string guiCode) {
    ...
    std::vector<std::string> items138 = LoopParseFunc(guiCode, "\n", "\r");
    for (size_t A_Index138 = 0; A_Index138 < items138.size() + 0; A_Index138++) {
        ...
        if (SubStr(Trim(StrLower(A_LoopField138)), 1, 4) == "gui " || RegExMatch(StrLower(Trim(A_LoopField138)), "^gui[-_][A-Za-z0-9_]+(?:[-_][A-Za-z0-9_]+)*(?=[, ]|$)")) {
            isElement = 0;
            whatElement = "";
            std::vector<std::string> items139 = LoopParseFunc(A_LoopField138, " ");
            ...
            found_isVisible = 0;
            found_isDeleting = 0;
            std::vector<std::string> items140 = LoopParseFunc(temp1, ",");
            ...
            isElement = 0;
            whatElement = "";
            std::vector<std::string> items141 = LoopParseFunc(A_LoopField138, " ");
            ...
            found_isVisible = 0;
            found_isDeleting = 0;
            std::vector<std::string> items142 = LoopParseFunc(temp1, ",");
...
std::string guiParser(std::string guiCode) {
    ...
    HTVM_Append(lookAhead, " ");
    std::vector<std::string> items148 = LoopParseFunc(guiCode, "\n", "\r");
...
std::string initializeBackendAndGUIInteractionWithLabelSubroutineLikeButConvertsToFunction(std::string codeToConvert) {
    ...
    std::vector<std::string> items149 = LoopParseFunc(codeToConvert, "\n", "\r");
    for (size_t A_Index149 = 0; A_Index149 < items149.size() + 0; A_Index149++) {
        ...
        if (RegExMatch(Trim(A_LoopField149), "^[a-zA-Z_][a-zA-Z0-9_]*:$") && isLineAconstruct(Trim(A_LoopField149)) == false) {
            ...
            temp1 = "";
            std::vector<std::string> items150 = LoopParseFunc(Trim(A_LoopField149), ",");
            ...
            // HTVM_init("fileName")
            temp1 = "";
            std::vector<std::string> items151 = LoopParseFunc(Trim(A_LoopField149), ",");
            ...
            temp1 = "";
            std::vector<std::string> items152 = LoopParseFunc(Trim(A_LoopField149), ",");
...
std::string FixDoubleInThe4langs(std::string line) {
    ...
    std::vector<std::string> items155 = LoopParseFunc(line, "\n", "\r");
    for (size_t A_Index155 = 0; A_Index155 < items155.size() + 0; A_Index155++) {
        std::string A_LoopField155 = items155[A_Index155 - 0];
        if (RegExMatch(A_LoopField155, "\\bdouble\\b") || RegExMatch(A_LoopField155, "\\bDouble\\b")) {
            FixDoubleInThe4langsOUT += RegExReplace(A_LoopField155, "(?<![A-Za-z0-9_])(-?\\d+\\.\\d+)f(?![A-Za-z0-9_])", "$1") + Chr(10);
        } else {
            FixDoubleInThe4langsOUT += A_LoopField155 + Chr(10);
        }
    }
...
std::string handleMultiLineStrings(std::string code) {
    ...
    std::vector<std::string> items156 = LoopParseFunc(code, "\n", "\r");
    for (size_t A_Index156 = 0; A_Index156 < items156.size() + 0; A_Index156++) {
        std::string A_LoopField156 = items156[A_Index156 - 0];
        if (SubStrLastChars(Trim(A_LoopField156), 4) == " mls" || SubStrLastChars(Trim(A_LoopField156), 5) == " fmls") {
            didWeFindOrWeOut = 1;
        }
    }
    ...
    for (size_t A_Index158 = 0; A_Index158 < items158.size() + 0; A_Index158++) {
        ...
        if (langToConvertTo == langFileExtension_2) {
            ...
            if (weAreIn == 1) {
                ...
                weAreInMLSarrTemp += A_LoopField158;
            }
            if (SubStrLastChars(Trim(A_LoopField158), 4) == " mls" || SubStrLastChars(Trim(A_LoopField158), 5) == " fmls") {
                weAreIn = 1;
                out += A_LoopField158 + Chr(10);
            }
            if (weAreIn == 0) {
                out += A_LoopField158 + Chr(10);
            }
            ...
            if (weAreIn == 1 && Trim(A_LoopField158) == "HTVM-newLineAHw4y9w730q8237w4e8srhgesw0s8rgw04e5g08sgr0e8srdgt--YOU-BETERNOTUSETHISAH--aesdfssz") {
                if (lookAhead[A_Index158 + 1] == "HTVM-newLineAHw4y9w730q8237w4e8srhgesw0s8rgw04e5g08sgr0e8srdgt--YOU-BETERNOTUSETHISAH--aesdfssz") {
                    out += keyWordEscpaeChar + "n";
                }
            }
            if (weAreIn == 1 && Trim(A_LoopField158) != "HTVM-newLineAHw4y9w730q8237w4e8srhgesw0s8rgw04e5g08sgr0e8srdgt--YOU-BETERNOTUSETHISAH--aesdfssz") {
                out += A_LoopField158 + keyWordEscpaeChar + "n";
            }
            if (SubStrLastChars(Trim(A_LoopField158), 4) == " mls" || SubStrLastChars(Trim(A_LoopField158), 5) == " fmls") {
                weAreIn = 1;
                if (SubStrLastChars(Trim(A_LoopField158), 5) == " fmls") {
                    out += TrimR(StringTrimRight(TrimR(A_LoopField158), 3)) + Chr(34);
                } else {
                    out += TrimR(StringTrimRight(TrimR(A_LoopField158), 3)) + " " + Chr(34);
                }
            }
            if (weAreIn == 0) {
                if (Trim(A_LoopField158) != "mle") {
                    out += A_LoopField158 + Chr(10);
                }
            }
            //;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;
...
std::vector<std::string> slidingWinFixCommentsRmFUNC(std::string line) {
    ...
    std::vector<std::string> items159 = LoopParseFunc(line);
    for (size_t A_Index159 = 0; A_Index159 < items159.size() + 0; A_Index159++) {
        std::string A_LoopField159 = items159[A_Index159 - 0];
        HTVM_Append(slidingWinFixCommentsRmFFFF, A_LoopField159);
    }
    for (int A_Index160 = 0; A_Index160 < StrLen(keyWordComment + keyWordComment) + 2 + 0; A_Index160++) {
...
std::string fixIfElseIfMultiLineStatementHELP(std::string aloopfield) {
    std::string out = "";
    int count = 0;
    std::vector<std::string> items161 = LoopParseFunc(aloopfield);
...
std::string fixIfElseIfMultiLineStatement(std::string someHTVMcode) {
    int doWeExitInsta = 1;
    std::vector<std::string> items162 = LoopParseFunc(someHTVMcode, "\n", "\r");
    for (size_t A_Index162 = 0; A_Index162 < items162.size() + 0; A_Index162++) {
        std::string A_LoopField162 = items162[A_Index162 - 0];
        if (SubStr(StrLower(Trim(A_LoopField162)), 1, StrLen(keyWordIF)) == keyWordIF) {
            doWeExitInsta = 0;
        }
        else if (SubStr(StrLower(Trim(A_LoopField162)), 1, StrLen(keyWordWhileLoop)) == keyWordWhileLoop) {
            doWeExitInsta = 0;
        }
        else if (SubStr(StrLower(Trim(A_LoopField162)), 1, StrLen(keyWordElseIf)) == keyWordElseIf) {
            doWeExitInsta = 0;
        }
    }
    if (doWeExitInsta == 1) {
    ...
    for (int A_Index163 = 0; A_Index163 < 10000 + 0; A_Index163++) {
        out2 = out;
        out = "";
        doWeExit = false;
        std::vector<std::string> items164 = LoopParseFunc(out2, "\n", "\r");
        for (size_t A_Index164 = 0; A_Index164 < items164.size() + 0; A_Index164++) {
            std::string A_LoopField164 = items164[A_Index164 - 0];
            if (SubStr(StrLower(Trim(A_LoopField164)), 1, StrLen(keyWordIF)) == keyWordIF) {
                doWeJustExit = 1;
                if (fixIfElseIfMultiLineStatementHELP(A_LoopField164) == "") {
                    out += Trim(A_LoopField164) + " ";
                    doWeExit = false;
                    doWeExitHELP--;
                } else {
                    doWeExit = true;
                    out += Trim(A_LoopField164) + Chr(10);
                }
            }
            else if (SubStr(StrLower(Trim(A_LoopField164)), 1, StrLen(keyWordWhileLoop)) == keyWordWhileLoop) {
                doWeJustExit = 1;
                if (fixIfElseIfMultiLineStatementHELP(A_LoopField164) == "") {
                    out += Trim(A_LoopField164) + " ";
                    doWeExit = false;
            ...
            else if (SubStr(StrLower(Trim(A_LoopField164)), 1, StrLen(keyWordElseIf)) == keyWordElseIf) {
                doWeJustExit = 1;
                if (fixIfElseIfMultiLineStatementHELP(A_LoopField164) == "") {
                    out += Trim(A_LoopField164) + " ";
                    doWeExit = false;
...
bool doseHaveInclude(std::string TheCodeThatMightHaveInclude) {
    std::vector<std::string> items165 = LoopParseFunc(TheCodeThatMightHaveInclude, "\n", "\r");
    for (size_t A_Index165 = 0; A_Index165 < items165.size() + 0; A_Index165++) {
        std::string A_LoopField165 = items165[A_Index165 - 0];
        if (SubStr(StrLower(Trim(A_LoopField165)), 1, StrLen(StrLower(keyWordInclude + " "))) == StrLower(keyWordInclude + " ")) {
            return true;
        }
    }
    return false;
...
std::string compiler(std::string htCode, std::string allInstructionFile, std::string mode, std::string langToConvertToParam = "") {
    ...
    std::vector<std::string> items166 = LoopParseFunc(allInstructionFile, "\n", "\r");
    for (size_t A_Index166 = 0; A_Index166 < items166.size() + 0; A_Index166++) {
        std::string A_LoopField166 = items166[A_Index166 - 0];
        if (A_Index166 == 0) {
            langToConvertTo = Trim(A_LoopField166);
        }
        if (A_Index166 == 1) {
            langFileExtension = Trim(A_LoopField166);
        }
        if (A_Index166 == 2) {
            commands = Trim(A_LoopField166);
        }
        if (A_Index166 == 3) {
            keyWordAlliance = Trim(A_LoopField166) + " ";
        }
        if (A_Index166 == 4) {
            keyWordCrew = Trim(A_LoopField166) + " ";
        }
        if (A_Index166 == 5) {
            keyWordProc = Trim(A_LoopField166) + " ";
        }
        if (A_Index166 == 6) {
            keyWordStruct = Trim(A_LoopField166) + " ";
        }
        if (A_Index166 == 7) {
            keyWordProp = Trim(A_LoopField166) + " ";
        }
        if (A_Index166 == 8) {
            keyWordThis = Trim(A_LoopField166);
        }
        if (A_Index166 == 9) {
            keyWordInclude = Trim(A_LoopField166);
        }
        if (A_Index166 == 10) {
            keyWordCodeInTheTranspiledLangStart = Trim(A_LoopField166);
        }
        if (A_Index166 == 11) {
            keyWordCodeInTheTranspiledLangEnd = Trim(A_LoopField166);
        }
        if (A_Index166 == 12) {
            keyWordCodeInTheTranspiledLangStartCPP = Trim(A_LoopField166);
        }
        if (A_Index166 == 13) {
            keyWordCodeInTheTranspiledLangEndCPP = Trim(A_LoopField166);
        }
        if (A_Index166 == 14) {
            keyWordCodeInTheTranspiledLangStartPY = Trim(A_LoopField166);
        }
        if (A_Index166 == 15) {
            keyWordCodeInTheTranspiledLangEndPY = Trim(A_LoopField166);
        }
        if (A_Index166 == 16) {
            keyWordCodeInTheTranspiledLangStartJS = Trim(A_LoopField166);
        }
        if (A_Index166 == 17) {
            keyWordCodeInTheTranspiledLangEndJS = Trim(A_LoopField166);
        }
        if (A_Index166 == 18) {
            keyWordCodeInTheTranspiledLangStartGO = Trim(A_LoopField166);
        }
        if (A_Index166 == 19) {
            keyWordCodeInTheTranspiledLangEndGO = Trim(A_LoopField166);
        }
        if (A_Index166 == 20) {
            keyWordCodeInTheTranspiledLangStartLUA = Trim(A_LoopField166);
        }
        if (A_Index166 == 21) {
            keyWordCodeInTheTranspiledLangEndLUA = Trim(A_LoopField166);
        }
        if (A_Index166 == 22) {
            keyWordCodeInTheTranspiledLangStartCS = Trim(A_LoopField166);
        }
        if (A_Index166 == 23) {
            keyWordCodeInTheTranspiledLangEndCS = Trim(A_LoopField166);
        }
        if (A_Index166 == 24) {
            keyWordCodeInTheTranspiledLangStartJAVA = Trim(A_LoopField166);
        }
        if (A_Index166 == 25) {
            keyWordCodeInTheTranspiledLangEndJAVA = Trim(A_LoopField166);
        }
        if (A_Index166 == 26) {
            keyWordCodeInTheTranspiledLangStartKT = Trim(A_LoopField166);
        }
        if (A_Index166 == 27) {
            keyWordCodeInTheTranspiledLangEndKT = Trim(A_LoopField166);
        }
        if (A_Index166 == 28) {
            keyWordCodeInTheTranspiledLangStartRB = Trim(A_LoopField166);
        }
        if (A_Index166 == 29) {
            keyWordCodeInTheTranspiledLangEndRB = Trim(A_LoopField166);
        }
        if (A_Index166 == 30) {
            keyWordCodeInTheTranspiledLangStartNIM = Trim(A_LoopField166);
        }
        if (A_Index166 == 31) {
            keyWordCodeInTheTranspiledLangEndNIM = Trim(A_LoopField166);
        }
        if (A_Index166 == 32) {
            keyWordCodeInTheTranspiledLangStartAHK = Trim(A_LoopField166);
        }
        if (A_Index166 == 33) {
            keyWordCodeInTheTranspiledLangEndAHK = Trim(A_LoopField166);
        }
        if (A_Index166 == 34) {
            keyWordCodeInTheTranspiledLangStartSWIFT = Trim(A_LoopField166);
        }
        if (A_Index166 == 35) {
            keyWordCodeInTheTranspiledLangEndSWIFT = Trim(A_LoopField166);
        }
        if (A_Index166 == 36) {
            keyWordCodeInTheTranspiledLangStartDART = Trim(A_LoopField166);
        }
        if (A_Index166 == 37) {
            keyWordCodeInTheTranspiledLangEndDART = Trim(A_LoopField166);
        }
        if (A_Index166 == 38) {
            keyWordCodeInTheTranspiledLangStartTS = Trim(A_LoopField166);
        }
        if (A_Index166 == 39) {
            keyWordCodeInTheTranspiledLangEndTS = Trim(A_LoopField166);
        }
        if (A_Index166 == 40) {
            keyWordCodeInTheTranspiledLangStartGROOVY = Trim(A_LoopField166);
        }
        if (A_Index166 == 41) {
            keyWordCodeInTheTranspiledLangEndGROOVY = Trim(A_LoopField166);
        }
        if (A_Index166 == 42) {
            keyWordCodeInTheTranspiledLangStartHTVM = Trim(A_LoopField166);
        }
        if (A_Index166 == 43) {
            keyWordCodeInTheTranspiledLangEndHTVM = Trim(A_LoopField166);
        }
        if (A_Index166 == 44) {
            keyWordCodeInHTVMstart = Trim(A_LoopField166);
        }
        if (A_Index166 == 45) {
            keyWordCodeInHTVMend = Trim(A_LoopField166);
        }
        if (A_Index166 == 46) {
            keyWordCurlyBraceOpen = Trim(A_LoopField166);
        }
        if (A_Index166 == 47) {
            keyWordCurlyBraceClose = Trim(A_LoopField166);
        }
        if (A_Index166 == 48) {
            keyWordNull = Trim(A_LoopField166);
        }
        if (A_Index166 == 49) {
            keyWordTrue = Trim(A_LoopField166);
        }
        if (A_Index166 == 50) {
            keyWordFalse = Trim(A_LoopField166);
        }
        if (A_Index166 == 51) {
            keyWordVoid = Trim(A_LoopField166);
        }
        if (A_Index166 == 52) {
            keyWordDouble = Trim(A_LoopField166);
        }
        if (A_Index166 == 53) {
            keyWordChar = Trim(A_LoopField166);
        }
        if (A_Index166 == 54) {
            keyWordUint8 = Trim(A_LoopField166);
        }
        if (A_Index166 == 55) {
            keyWordUint16 = Trim(A_LoopField166);
        }
        if (A_Index166 == 56) {
            keyWordUint32 = Trim(A_LoopField166);
        }
        if (A_Index166 == 57) {
            keyWordUint64 = Trim(A_LoopField166);
        }
        if (A_Index166 == 58) {
            keyWordINT = Trim(A_LoopField166);
        }
        if (A_Index166 == 59) {
            keyWordSTR = Trim(A_LoopField166);
        }
        if (A_Index166 == 60) {
            keyWordBOOL = Trim(A_LoopField166);
        }
        if (A_Index166 == 61) {
            keyWordFLOAT = Trim(A_LoopField166);
        }
        if (A_Index166 == 62) {
            keyWordINT8 = Trim(A_LoopField166);
        }
        if (A_Index166 == 63) {
            keyWordINT16 = Trim(A_LoopField166);
        }
        if (A_Index166 == 64) {
            keyWordINT32 = Trim(A_LoopField166);
        }
        if (A_Index166 == 65) {
            keyWordINT64 = Trim(A_LoopField166);
        }
        if (A_Index166 == 66) {
            keyWordIF = Trim(A_LoopField166) + " ";
        }
        if (A_Index166 == 67) {
            keyWordElseIf = Trim(A_LoopField166) + " ";
        }
        if (A_Index166 == 68) {
            keyWordElse = Trim(A_LoopField166);
        }
        if (A_Index166 == 69) {
            keyWordWhileLoop = Trim(A_LoopField166) + " ";
        }
        if (A_Index166 == 70) {
            keyWordLoopInfinite = Trim(A_LoopField166);
        }
        if (A_Index166 == 71) {
            keyWordLoop = Trim(A_LoopField166) + " ";
        }
        if (A_Index166 == 72) {
            keyWordLoopParse = Trim(A_LoopField166) + " ";
        }
        if (A_Index166 == 73) {
            keyWordContinue = Trim(A_LoopField166);
        }
        if (A_Index166 == 74) {
            keyWordBreak = Trim(A_LoopField166);
        }
        if (A_Index166 == 75) {
            keyWordFunc = Trim(A_LoopField166) + " ";
        }
        if (A_Index166 == 76) {
            keyWordAwait = Trim(A_LoopField166) + " ";
        }
        if (A_Index166 == 77) {
            keyWordAsync = Trim(A_LoopField166) + " ";
        }
        if (A_Index166 == 78) {
            keyWordThrow = Trim(A_LoopField166) + " ";
        }
        if (A_Index166 == 79) {
            keyWordErrorMsg = Trim(A_LoopField166);
        }
        if (A_Index166 == 80) {
            keyWordTry = Trim(A_LoopField166);
        }
        if (A_Index166 == 81) {
            keyWordCatch = Trim(A_LoopField166) + " ";
        }
        if (A_Index166 == 82) {
            keyWordFinally = Trim(A_LoopField166);
        }
        if (A_Index166 == 83) {
            keyWordReturnStatement = Trim(A_LoopField166) + " ";
        }
        if (A_Index166 == 84) {
            keyWordArrayAppend = Trim(A_LoopField166);
        }
        if (A_Index166 == 85) {
            keyWordArrayPop = Trim(A_LoopField166);
        }
        if (A_Index166 == 86) {
            keyWordArraySize = Trim(A_LoopField166);
        }
        if (A_Index166 == 87) {
            keyWordArrayInsert = Trim(A_LoopField166);
        }
        if (A_Index166 == 88) {
            keyWordArrayRemove = Trim(A_LoopField166);
        }
        if (A_Index166 == 89) {
            keyWordArrayIndexOf = Trim(A_LoopField166);
        }
        if (A_Index166 == 90) {
            keyWordArrayDefinition = Trim(A_LoopField166);
        }
        if (A_Index166 == 91) {
            keyWordArrayOfIntegersDefinition = Trim(A_LoopField166);
        }
        if (A_Index166 == 92) {
            keyWordArrayOfStringsDefinition = Trim(A_LoopField166);
        }
        if (A_Index166 == 93) {
            keyWordArrayOfFloatingPointNumbersDefinition = Trim(A_LoopField166);
        }
        if (A_Index166 == 94) {
            keyWordArrayOfBooleansDefinition = Trim(A_LoopField166);
        }
        if (A_Index166 == 95) {
            keyWordVar = Trim(A_LoopField166) + " ";
        }
        if (A_Index166 == 96) {
            keyWordLet = Trim(A_LoopField166) + " ";
        }
        if (A_Index166 == 97) {
            keyWordConst = Trim(A_LoopField166) + " ";
        }
        if (A_Index166 == 98) {
            keyWordEnd = Trim(A_LoopField166);
        }
        if (A_Index166 == 99) {
            keyWordGlobal = Trim(A_LoopField166) + " ";
        }
        if (A_Index166 == 100) {
            keyWordComment = Trim(A_LoopField166);
        }
        if (A_Index166 == 101) {
            keyWordCommentOpenMultiLine = Trim(A_LoopField166);
        }
        if (A_Index166 == 102) {
            keyWordCommentCloseMultiLine = Trim(A_LoopField166);
        }
        if (A_Index166 == 103) {
            keyWordEscpaeChar = Trim(A_LoopField166);
        }
        if (A_Index166 == 104) {
            keyWordMainLabel = Trim(A_LoopField166);
        }
        if (A_Index166 == 105) {
            keyWordConcat = Trim(A_LoopField166);
        }
        if (A_Index166 == 106) {
            keyWordAdd = Trim(A_LoopField166);
        }
        if (A_Index166 == 107) {
            keyWordSub = Trim(A_LoopField166);
        }
        if (A_Index166 == 108) {
            keyWordMul = Trim(A_LoopField166);
        }
        if (A_Index166 == 109) {
            keyWordDiv = Trim(A_LoopField166);
        }
        if (A_Index166 == 110) {
            keyWordMod = Trim(A_LoopField166);
        }
        if (A_Index166 == 111) {
            keyWordExp = Trim(A_LoopField166);
        }
        if (A_Index166 == 112) {
            keyWordEqual = Trim(A_LoopField166);
        }
        if (A_Index166 == 113) {
            keyWordStrictEqual = Trim(A_LoopField166);
        }
        if (A_Index166 == 114) {
            keyWordNotEqual = Trim(A_LoopField166);
        }
        if (A_Index166 == 115) {
            keyWordGreater = Trim(A_LoopField166);
        }
        if (A_Index166 == 116) {
            keyWordLess = Trim(A_LoopField166);
        }
        if (A_Index166 == 117) {
            keyWordGreaterEqual = Trim(A_LoopField166);
        }
        if (A_Index166 == 118) {
            keyWordLessEqual = Trim(A_LoopField166);
        }
        if (A_Index166 == 119) {
            keyWordAnd = Trim(A_LoopField166);
        }
        if (A_Index166 == 120) {
            keyWordOr = Trim(A_LoopField166);
        }
        if (A_Index166 == 121) {
            keyWordNot = Trim(A_LoopField166);
        }
        if (A_Index166 == 122) {
            keyWordBitAnd = Trim(A_LoopField166);
        }
        if (A_Index166 == 123) {
            keyWordBitOr = Trim(A_LoopField166);
        }
        if (A_Index166 == 124) {
            keyWordBitXor = Trim(A_LoopField166);
        }
        if (A_Index166 == 125) {
            keyWordBitNot = Trim(A_LoopField166);
        }
        if (A_Index166 == 126) {
            keyWordShiftLeft = Trim(A_LoopField166);
        }
        if (A_Index166 == 127) {
            keyWordShiftRight = Trim(A_LoopField166);
        }
        if (A_Index166 == 128) {
            keyWordShiftUnsignedRight = Trim(A_LoopField166);
        }
        if (A_Index166 == 129) {
            keyWordAssign = Trim(A_LoopField166);
        }
        if (A_Index166 == 130) {
            keyWordAssignAdd = Trim(A_LoopField166);
        }
        if (A_Index166 == 131) {
            keyWordAssignConcat = Trim(A_LoopField166);
        }
        if (A_Index166 == 132) {
            keyWordAssignSub = Trim(A_LoopField166);
        }
        if (A_Index166 == 133) {
            keyWordAssignMul = Trim(A_LoopField166);
        }
        if (A_Index166 == 134) {
            keyWordAssignDiv = Trim(A_LoopField166);
        }
        if (A_Index166 == 135) {
            keyWordAssignMod = Trim(A_LoopField166);
        }
        if (A_Index166 == 136) {
            keyWordAssignShiftLeft = Trim(A_LoopField166);
        }
        if (A_Index166 == 137) {
            keyWordAssignShiftRight = Trim(A_LoopField166);
        }
        if (A_Index166 == 138) {
            keyWordLogicalAssignShiftRight = Trim(A_LoopField166);
        }
        if (A_Index166 == 139) {
            keyWordAssignBitAnd = Trim(A_LoopField166);
        }
        if (A_Index166 == 140) {
            keyWordAssignBitOr = Trim(A_LoopField166);
        }
        if (A_Index166 == 141) {
            keyWordAssignBitXor = Trim(A_LoopField166);
        }
        if (A_Index166 == 142) {
            keyWordTernary1 = Trim(A_LoopField166);
        }
        if (A_Index166 == 143) {
            keyWordTernary2 = Trim(A_LoopField166);
        }
        if (A_Index166 == 144) {
            keyWordInc = Trim(A_LoopField166);
        }
        if (A_Index166 == 145) {
            keyWordDec = Trim(A_LoopField166);
        }
        if (A_Index166 == 146) {
            AHKlikeLoopsIndexedAt = Trim(A_LoopField166);
        }
        if (A_Index166 == 147) {
            keyWordAIndex = Trim(A_LoopField166);
        }
        if (A_Index166 == 148) {
            keyWordALoopField = Trim(A_LoopField166);
        }
        if (A_Index166 == 149) {
            useCurlyBraces = Trim(A_LoopField166);
        }
        if (A_Index166 == 150) {
            useEnd = Trim(A_LoopField166);
        }
        if (A_Index166 == 151) {
            useSemicolon = Trim(A_LoopField166);
        }
        if (A_Index166 == 152) {
            useParentheses = Trim(A_LoopField166);
        }
        if (A_Index166 == 153) {
            usePrefixTypeForTypeDefinition = Trim(A_LoopField166);
        }
        if (A_Index166 == 154) {
            usePostfixTypeForTypeDefinition = Trim(A_LoopField166);
        }
        if (A_Index166 == 155) {
            usePythonicColonSyntax = Trim(A_LoopField166);
        }
        if (A_Index166 == 156) {
            useCurlyBracesSyntaxForArrayDef = Trim(A_LoopField166);
        }
        if (A_Index166 == 157) {
            useInJavaScriptAlwaysUseVar = Trim(A_LoopField166);
        }
        if (A_Index166 == 158) {
            useJavaScriptInAfullHTMLfile = Trim(A_LoopField166);
        }
        if (A_Index166 == 159) {
            useJavaScriptAmainFuncDef = Trim(A_LoopField166);
        }
        if (A_Index166 == 160) {
            useJavaScriptAllFuncsAreAsync = Trim(A_LoopField166);
        }
        if (A_Index166 == 161) {
            useJavaScriptAlwaysTripleEqual = Trim(A_LoopField166);
        }
    }
    if (HTVM_Size(argHTVMinstrMORE) > 0) {
        ...
        std::vector<std::string> items167 = LoopParseFunc(allInstructionFile_2, "\n", "\r");
        for (size_t A_Index167 = 0; A_Index167 < items167.size() + 0; A_Index167++) {
            std::string A_LoopField167 = items167[A_Index167 - 0];
            if (A_Index167 == 0) {
                langToConvertTo_2 = Trim(A_LoopField167);
            }
            if (A_Index167 == 1) {
                langFileExtension_2 = Trim(A_LoopField167);
            }
            if (A_Index167 == 2) {
                commands_2 = Trim(A_LoopField167);
            }
            if (A_Index167 == 3) {
                keyWordAlliance_2 = Trim(A_LoopField167) + " ";
            }
            if (A_Index167 == 4) {
                keyWordCrew_2 = Trim(A_LoopField167) + " ";
            }
            if (A_Index167 == 5) {
                keyWordProc_2 = Trim(A_LoopField167) + " ";
            }
            if (A_Index167 == 6) {
                keyWordStruct_2 = Trim(A_LoopField167) + " ";
            }
            if (A_Index167 == 7) {
                keyWordProp_2 = Trim(A_LoopField167) + " ";
            }
            if (A_Index167 == 8) {
                keyWordThis_2 = Trim(A_LoopField167);
            }
            if (A_Index167 == 9) {
                keyWordInclude_2 = Trim(A_LoopField167);
            }
            if (A_Index167 == 10) {
                keyWordCodeInTheTranspiledLangStart_2 = Trim(A_LoopField167);
            }
            if (A_Index167 == 11) {
                keyWordCodeInTheTranspiledLangEnd_2 = Trim(A_LoopField167);
            }
            if (A_Index167 == 12) {
                keyWordCodeInTheTranspiledLangStartCPP_2 = Trim(A_LoopField167);
            }
            if (A_Index167 == 13) {
                keyWordCodeInTheTranspiledLangEndCPP_2 = Trim(A_LoopField167);
            }
            if (A_Index167 == 14) {
                keyWordCodeInTheTranspiledLangStartPY_2 = Trim(A_LoopField167);
            }
            if (A_Index167 == 15) {
                keyWordCodeInTheTranspiledLangEndPY_2 = Trim(A_LoopField167);
            }
            if (A_Index167 == 16) {
                keyWordCodeInTheTranspiledLangStartJS_2 = Trim(A_LoopField167);
            }
            if (A_Index167 == 17) {
                keyWordCodeInTheTranspiledLangEndJS_2 = Trim(A_LoopField167);
            }
            if (A_Index167 == 18) {
                keyWordCodeInTheTranspiledLangStartGO_2 = Trim(A_LoopField167);
            }
            if (A_Index167 == 19) {
                keyWordCodeInTheTranspiledLangEndGO_2 = Trim(A_LoopField167);
            }
            if (A_Index167 == 20) {
                keyWordCodeInTheTranspiledLangStartLUA_2 = Trim(A_LoopField167);
            }
            if (A_Index167 == 21) {
                keyWordCodeInTheTranspiledLangEndLUA_2 = Trim(A_LoopField167);
            }
            if (A_Index167 == 22) {
                keyWordCodeInTheTranspiledLangStartCS_2 = Trim(A_LoopField167);
            }
            if (A_Index167 == 23) {
                keyWordCodeInTheTranspiledLangEndCS_2 = Trim(A_LoopField167);
            }
            if (A_Index167 == 24) {
                keyWordCodeInTheTranspiledLangStartJAVA_2 = Trim(A_LoopField167);
            }
            if (A_Index167 == 25) {
                keyWordCodeInTheTranspiledLangEndJAVA_2 = Trim(A_LoopField167);
            }
            if (A_Index167 == 26) {
                keyWordCodeInTheTranspiledLangStartKT_2 = Trim(A_LoopField167);
            }
            if (A_Index167 == 27) {
                keyWordCodeInTheTranspiledLangEndKT_2 = Trim(A_LoopField167);
            }
            if (A_Index167 == 28) {
                keyWordCodeInTheTranspiledLangStartRB_2 = Trim(A_LoopField167);
            }
            if (A_Index167 == 29) {
                keyWordCodeInTheTranspiledLangEndRB_2 = Trim(A_LoopField167);
            }
            if (A_Index167 == 30) {
                keyWordCodeInTheTranspiledLangStartNIM_2 = Trim(A_LoopField167);
            }
            if (A_Index167 == 31) {
                keyWordCodeInTheTranspiledLangEndNIM_2 = Trim(A_LoopField167);
            }
            if (A_Index167 == 32) {
                keyWordCodeInTheTranspiledLangStartAHK_2 = Trim(A_LoopField167);
            }
            if (A_Index167 == 33) {
                keyWordCodeInTheTranspiledLangEndAHK_2 = Trim(A_LoopField167);
            }
            if (A_Index167 == 34) {
                keyWordCodeInTheTranspiledLangStartSWIFT_2 = Trim(A_LoopField167);
            }
            if (A_Index167 == 35) {
                keyWordCodeInTheTranspiledLangEndSWIFT_2 = Trim(A_LoopField167);
            }
            if (A_Index167 == 36) {
                keyWordCodeInTheTranspiledLangStartDART_2 = Trim(A_LoopField167);
            }
            if (A_Index167 == 37) {
                keyWordCodeInTheTranspiledLangEndDART_2 = Trim(A_LoopField167);
            }
            if (A_Index167 == 38) {
                keyWordCodeInTheTranspiledLangStartTS_2 = Trim(A_LoopField167);
            }
            if (A_Index167 == 39) {
                keyWordCodeInTheTranspiledLangEndTS_2 = Trim(A_LoopField167);
            }
            if (A_Index167 == 40) {
                keyWordCodeInTheTranspiledLangStartGROOVY_2 = Trim(A_LoopField167);
            }
            if (A_Index167 == 41) {
                keyWordCodeInTheTranspiledLangEndGROOVY_2 = Trim(A_LoopField167);
            }
            if (A_Index167 == 42) {
                keyWordCodeInTheTranspiledLangStartHTVM_2 = Trim(A_LoopField167);
            }
            if (A_Index167 == 43) {
                keyWordCodeInTheTranspiledLangEndHTVM_2 = Trim(A_LoopField167);
            }
            if (A_Index167 == 44) {
                keyWordCodeInHTVMstart_2 = Trim(A_LoopField167);
            }
            if (A_Index167 == 45) {
                keyWordCodeInHTVMend_2 = Trim(A_LoopField167);
            }
            if (A_Index167 == 46) {
                keyWordCurlyBraceOpen_2 = Trim(A_LoopField167);
            }
            if (A_Index167 == 47) {
                keyWordCurlyBraceClose_2 = Trim(A_LoopField167);
            }
            if (A_Index167 == 48) {
                keyWordNull_2 = Trim(A_LoopField167);
            }
            if (A_Index167 == 49) {
                keyWordTrue_2 = Trim(A_LoopField167);
            }
            if (A_Index167 == 50) {
                keyWordFalse_2 = Trim(A_LoopField167);
            }
            if (A_Index167 == 51) {
                keyWordVoid_2 = Trim(A_LoopField167);
            }
            if (A_Index167 == 52) {
                keyWordDouble_2 = Trim(A_LoopField167);
            }
            if (A_Index167 == 53) {
                keyWordChar_2 = Trim(A_LoopField167);
            }
            if (A_Index167 == 54) {
                keyWordUint8_2 = Trim(A_LoopField167);
            }
            if (A_Index167 == 55) {
                keyWordUint16_2 = Trim(A_LoopField167);
            }
            if (A_Index167 == 56) {
                keyWordUint32_2 = Trim(A_LoopField167);
            }
            if (A_Index167 == 57) {
                keyWordUint64_2 = Trim(A_LoopField167);
            }
            if (A_Index167 == 58) {
                keyWordINT_2 = Trim(A_LoopField167);
            }
            if (A_Index167 == 59) {
                keyWordSTR_2 = Trim(A_LoopField167);
            }
            if (A_Index167 == 60) {
                keyWordBOOL_2 = Trim(A_LoopField167);
            }
            if (A_Index167 == 61) {
                keyWordFLOAT_2 = Trim(A_LoopField167);
            }
            if (A_Index167 == 62) {
                keyWordINT8_2 = Trim(A_LoopField167);
            }
            if (A_Index167 == 63) {
                keyWordINT16_2 = Trim(A_LoopField167);
            }
            if (A_Index167 == 64) {
                keyWordINT32_2 = Trim(A_LoopField167);
            }
            if (A_Index167 == 65) {
                keyWordINT64_2 = Trim(A_LoopField167);
            }
            if (A_Index167 == 66) {
                keyWordIF_2 = Trim(A_LoopField167) + " ";
            }
            if (A_Index167 == 67) {
                keyWordElseIf_2 = Trim(A_LoopField167) + " ";
            }
            if (A_Index167 == 68) {
                keyWordElse_2 = Trim(A_LoopField167);
            }
            if (A_Index167 == 69) {
                keyWordWhileLoop_2 = Trim(A_LoopField167) + " ";
            }
            if (A_Index167 == 70) {
                keyWordLoopInfinite_2 = Trim(A_LoopField167);
            }
            if (A_Index167 == 71) {
                keyWordLoop_2 = Trim(A_LoopField167) + " ";
            }
            if (A_Index167 == 72) {
                keyWordLoopParse_2 = Trim(A_LoopField167) + " ";
            }
            if (A_Index167 == 73) {
                keyWordContinue_2 = Trim(A_LoopField167);
            }
            if (A_Index167 == 74) {
                keyWordBreak_2 = Trim(A_LoopField167);
            }
            if (A_Index167 == 75) {
                keyWordFunc_2 = Trim(A_LoopField167) + " ";
            }
            if (A_Index167 == 76) {
                keyWordAwait_2 = Trim(A_LoopField167) + " ";
            }
            if (A_Index167 == 77) {
                keyWordAsync_2 = Trim(A_LoopField167) + " ";
            }
            if (A_Index167 == 78) {
                keyWordThrow_2 = Trim(A_LoopField167) + " ";
            }
            if (A_Index167 == 79) {
                keyWordErrorMsg_2 = Trim(A_LoopField167);
            }
            if (A_Index167 == 80) {
                keyWordTry_2 = Trim(A_LoopField167);
            }
            if (A_Index167 == 81) {
                keyWordCatch_2 = Trim(A_LoopField167) + " ";
            }
            if (A_Index167 == 82) {
                keyWordFinally_2 = Trim(A_LoopField167);
            }
            if (A_Index167 == 83) {
                keyWordReturnStatement_2 = Trim(A_LoopField167) + " ";
            }
            if (A_Index167 == 84) {
                keyWordArrayAppend_2 = Trim(A_LoopField167);
            }
            if (A_Index167 == 85) {
                keyWordArrayPop_2 = Trim(A_LoopField167);
            }
            if (A_Index167 == 86) {
                keyWordArraySize_2 = Trim(A_LoopField167);
            }
            if (A_Index167 == 87) {
                keyWordArrayInsert_2 = Trim(A_LoopField167);
            }
            if (A_Index167 == 88) {
                keyWordArrayRemove_2 = Trim(A_LoopField167);
            }
            if (A_Index167 == 89) {
                keyWordArrayIndexOf_2 = Trim(A_LoopField167);
            }
            if (A_Index167 == 90) {
                keyWordArrayDefinition_2 = Trim(A_LoopField167);
            }
            if (A_Index167 == 91) {
                keyWordArrayOfIntegersDefinition_2 = Trim(A_LoopField167);
            }
            if (A_Index167 == 92) {
                keyWordArrayOfStringsDefinition_2 = Trim(A_LoopField167);
            }
            if (A_Index167 == 93) {
                keyWordArrayOfFloatingPointNumbersDefinition_2 = Trim(A_LoopField167);
            }
            if (A_Index167 == 94) {
                keyWordArrayOfBooleansDefinition_2 = Trim(A_LoopField167);
            }
            if (A_Index167 == 95) {
                keyWordVar_2 = Trim(A_LoopField167);
            }
            if (A_Index167 == 96) {
                keyWordLet_2 = Trim(A_LoopField167);
            }
            if (A_Index167 == 97) {
                keyWordConst_2 = Trim(A_LoopField167);
            }
            if (A_Index167 == 98) {
                keyWordEnd_2 = Trim(A_LoopField167);
            }
            if (A_Index167 == 99) {
                keyWordGlobal_2 = Trim(A_LoopField167) + " ";
            }
            if (A_Index167 == 100) {
                keyWordComment_2 = Trim(A_LoopField167);
            }
            if (A_Index167 == 101) {
                keyWordCommentOpenMultiLine_2 = Trim(A_LoopField167);
            }
            if (A_Index167 == 102) {
                keyWordCommentCloseMultiLine_2 = Trim(A_LoopField167);
            }
            if (A_Index167 == 103) {
                keyWordEscpaeChar_2 = Trim(A_LoopField167);
            }
            if (A_Index167 == 104) {
                keyWordMainLabel_2 = Trim(A_LoopField167);
            }
            if (A_Index167 == 105) {
                keyWordConcat_2 = Trim(A_LoopField167);
            }
            if (A_Index167 == 106) {
                keyWordAdd_2 = Trim(A_LoopField167);
            }
            if (A_Index167 == 107) {
                keyWordSub_2 = Trim(A_LoopField167);
            }
            if (A_Index167 == 108) {
                keyWordMul_2 = Trim(A_LoopField167);
            }
            if (A_Index167 == 109) {
                keyWordDiv_2 = Trim(A_LoopField167);
            }
            if (A_Index167 == 110) {
                keyWordMod_2 = Trim(A_LoopField167);
            }
            if (A_Index167 == 111) {
                keyWordExp_2 = Trim(A_LoopField167);
            }
            if (A_Index167 == 112) {
                keyWordEqual_2 = Trim(A_LoopField167);
            }
            if (A_Index167 == 113) {
                keyWordStrictEqual_2 = Trim(A_LoopField167);
            }
            if (A_Index167 == 114) {
                keyWordNotEqual_2 = Trim(A_LoopField167);
            }
            if (A_Index167 == 115) {
                keyWordGreater_2 = Trim(A_LoopField167);
            }
            if (A_Index167 == 116) {
                keyWordLess_2 = Trim(A_LoopField167);
            }
            if (A_Index167 == 117) {
                keyWordGreaterEqual_2 = Trim(A_LoopField167);
            }
            if (A_Index167 == 118) {
                keyWordLessEqual_2 = Trim(A_LoopField167);
            }
            if (A_Index167 == 119) {
                keyWordAnd_2 = Trim(A_LoopField167);
            }
            if (A_Index167 == 120) {
                keyWordOr_2 = Trim(A_LoopField167);
            }
            if (A_Index167 == 121) {
                keyWordNot_2 = Trim(A_LoopField167);
            }
            if (A_Index167 == 122) {
                keyWordBitAnd_2 = Trim(A_LoopField167);
            }
            if (A_Index167 == 123) {
                keyWordBitOr_2 = Trim(A_LoopField167);
            }
            if (A_Index167 == 124) {
                keyWordBitXor_2 = Trim(A_LoopField167);
            }
            if (A_Index167 == 125) {
                keyWordBitNot_2 = Trim(A_LoopField167);
            }
            if (A_Index167 == 126) {
                keyWordShiftLeft_2 = Trim(A_LoopField167);
            }
            if (A_Index167 == 127) {
                keyWordShiftRight_2 = Trim(A_LoopField167);
            }
            if (A_Index167 == 128) {
                keyWordShiftUnsignedRight_2 = Trim(A_LoopField167);
            }
            if (A_Index167 == 129) {
                keyWordAssign_2 = Trim(A_LoopField167);
            }
            if (A_Index167 == 130) {
                keyWordAssignAdd_2 = Trim(A_LoopField167);
            }
            if (A_Index167 == 131) {
                keyWordAssignConcat_2 = Trim(A_LoopField167);
            }
            if (A_Index167 == 132) {
                keyWordAssignSub_2 = Trim(A_LoopField167);
            }
            if (A_Index167 == 133) {
                keyWordAssignMul_2 = Trim(A_LoopField167);
            }
            if (A_Index167 == 134) {
                keyWordAssignDiv_2 = Trim(A_LoopField167);
            }
            if (A_Index167 == 135) {
                keyWordAssignMod_2 = Trim(A_LoopField167);
            }
            if (A_Index167 == 136) {
                keyWordAssignShiftLeft_2 = Trim(A_LoopField167);
            }
            if (A_Index167 == 137) {
                keyWordAssignShiftRight_2 = Trim(A_LoopField167);
            }
            if (A_Index167 == 138) {
                keyWordLogicalAssignShiftRight_2 = Trim(A_LoopField167);
            }
            if (A_Index167 == 139) {
                keyWordAssignBitAnd_2 = Trim(A_LoopField167);
            }
            if (A_Index167 == 140) {
                keyWordAssignBitOr_2 = Trim(A_LoopField167);
            }
            if (A_Index167 == 141) {
                keyWordAssignBitXor_2 = Trim(A_LoopField167);
            }
            if (A_Index167 == 142) {
                keyWordTernary1_2 = Trim(A_LoopField167);
            }
            if (A_Index167 == 143) {
                keyWordTernary2_2 = Trim(A_LoopField167);
            }
            if (A_Index167 == 144) {
                keyWordInc_2 = Trim(A_LoopField167);
            }
            if (A_Index167 == 145) {
                keyWordDec_2 = Trim(A_LoopField167);
            }
            if (A_Index167 == 146) {
                AHKlikeLoopsIndexedAt_2 = Trim(A_LoopField167);
            }
            if (A_Index167 == 147) {
                keyWordAIndex_2 = Trim(A_LoopField167);
            }
            if (A_Index167 == 148) {
                keyWordALoopField_2 = Trim(A_LoopField167);
            }
            if (A_Index167 == 149) {
                useCurlyBraces_2 = Trim(A_LoopField167);
            }
            if (A_Index167 == 150) {
                useEnd_2 = Trim(A_LoopField167);
            }
            if (A_Index167 == 151) {
                useSemicolon_2 = Trim(A_LoopField167);
            }
            if (A_Index167 == 152) {
                useParentheses_2 = Trim(A_LoopField167);
            }
            if (A_Index167 == 153) {
                usePrefixTypeForTypeDefinition_2 = Trim(A_LoopField167);
            }
            if (A_Index167 == 154) {
                usePostfixTypeForTypeDefinition_2 = Trim(A_LoopField167);
            }
            if (A_Index167 == 155) {
                usePythonicColonSyntax_2 = Trim(A_LoopField167);
            }
            if (A_Index167 == 156) {
                useCurlyBracesSyntaxForArrayDef_2 = Trim(A_LoopField167);
            }
            if (A_Index167 == 157) {
                useInJavaScriptAlwaysUseVar_2 = Trim(A_LoopField167);
            }
            if (A_Index167 == 158) {
                useJavaScriptInAfullHTMLfile_2 = Trim(A_LoopField167);
            }
            if (A_Index167 == 159) {
                useJavaScriptAmainFuncDef_2 = Trim(A_LoopField167);
            }
            if (A_Index167 == 160) {
                useJavaScriptAllFuncsAreAsync_2 = Trim(A_LoopField167);
            }
            if (A_Index167 == 161) {
                useJavaScriptAlwaysTripleEqual_2 = Trim(A_LoopField167);
            }
        }
    }
    ...
    */
    ...
    std::vector<std::string> items168 = LoopParseFunc(allInstructionFile, "\n", "\r");
    for (size_t A_Index168 = 0; A_Index168 < items168.size() + 0; A_Index168++) {
        std::string A_LoopField168 = items168[A_Index168 - 0];
        if (RegExMatch(Trim(A_LoopField168), "^name: [A-Za-z0-9_]+")) {
            availableFuncsInHTVMInHTVM += Trim(StrSplit(Trim(A_LoopField168), "name: ", 2)) + Chr(10);
        }
    }
    ...
    std::vector<std::string> items169 = LoopParseFunc(code, "\n", "\r");
    for (size_t A_Index169 = 0; A_Index169 < items169.size() + 0; A_Index169++) {
        std::string A_LoopField169 = items169[A_Index169 - 0];
        if (Trim(StrLower(A_LoopField169)) == StrLower(keyWordCodeInTheTranspiledLangEnd)) {
        ...
        else if (Trim(StrLower(A_LoopField169)) == StrLower(keyWordCodeInTheTranspiledLangEndCPP)) {
        ...
        else if (Trim(StrLower(A_LoopField169)) == StrLower(keyWordCodeInTheTranspiledLangEndPY)) {
        ...
        else if (Trim(StrLower(A_LoopField169)) == StrLower(keyWordCodeInTheTranspiledLangEndJS)) {
        ...
        else if (Trim(StrLower(A_LoopField169)) == StrLower(keyWordCodeInTheTranspiledLangEndGO)) {
        ...
        else if (Trim(StrLower(A_LoopField169)) == StrLower(keyWordCodeInTheTranspiledLangEndLUA)) {
        ...
        else if (Trim(StrLower(A_LoopField169)) == StrLower(keyWordCodeInTheTranspiledLangEndCS)) {
        ...
        else if (Trim(StrLower(A_LoopField169)) == StrLower(keyWordCodeInTheTranspiledLangEndJAVA)) {
        ...
        else if (Trim(StrLower(A_LoopField169)) == StrLower(keyWordCodeInTheTranspiledLangEndKT)) {
        ...
        else if (Trim(StrLower(A_LoopField169)) == StrLower(keyWordCodeInTheTranspiledLangEndRB)) {
        ...
        else if (Trim(StrLower(A_LoopField169)) == StrLower(keyWordCodeInTheTranspiledLangEndNIM)) {
        ...
        else if (Trim(StrLower(A_LoopField169)) == StrLower(keyWordCodeInTheTranspiledLangEndAHK)) {
        ...
        else if (Trim(StrLower(A_LoopField169)) == StrLower(keyWordCodeInTheTranspiledLangEndSWIFT)) {
        ...
        else if (Trim(StrLower(A_LoopField169)) == StrLower(keyWordCodeInTheTranspiledLangEndDART)) {
        ...
        else if (Trim(StrLower(A_LoopField169)) == StrLower(keyWordCodeInTheTranspiledLangEndTS)) {
        ...
        else if (Trim(StrLower(A_LoopField169)) == StrLower(keyWordCodeInTheTranspiledLangEndGROOVY)) {
        ...
        else if (Trim(StrLower(A_LoopField169)) == StrLower(keyWordCodeInTheTranspiledLangEndHTVM)) {
        ...
        else if (Trim(StrLower(A_LoopField169)) == StrLower(keyWordCodeInHTVMend)) {
            //;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;
            //;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;
            ...
            holdTempDataProgrammingBlockThenPutInArr += A_LoopField169 + Chr(10);
        }
        else if (Trim(StrLower(A_LoopField169)) == StrLower(keyWordCodeInTheTranspiledLangStart)) {
            holdTempDataProgrammingBlockThenPutInArr = "";
            inProgarmmingBlock = 1;
        }
        else if (Trim(StrLower(A_LoopField169)) == StrLower(keyWordCodeInTheTranspiledLangStartCPP)) {
            holdTempDataProgrammingBlockThenPutInArr = "";
            inProgarmmingBlock = 1;
        }
        else if (Trim(StrLower(A_LoopField169)) == StrLower(keyWordCodeInTheTranspiledLangStartPY)) {
            holdTempDataProgrammingBlockThenPutInArr = "";
            inProgarmmingBlock = 1;
        }
        else if (Trim(StrLower(A_LoopField169)) == StrLower(keyWordCodeInTheTranspiledLangStartJS)) {
            holdTempDataProgrammingBlockThenPutInArr = "";
            inProgarmmingBlock = 1;
        }
        else if (Trim(StrLower(A_LoopField169)) == StrLower(keyWordCodeInTheTranspiledLangStartGO)) {
            holdTempDataProgrammingBlockThenPutInArr = "";
            inProgarmmingBlock = 1;
        }
        else if (Trim(StrLower(A_LoopField169)) == StrLower(keyWordCodeInTheTranspiledLangStartLUA)) {
            holdTempDataProgrammingBlockThenPutInArr = "";
            inProgarmmingBlock = 1;
        }
        else if (Trim(StrLower(A_LoopField169)) == StrLower(keyWordCodeInTheTranspiledLangStartCS)) {
            holdTempDataProgrammingBlockThenPutInArr = "";
            inProgarmmingBlock = 1;
        }
        else if (Trim(StrLower(A_LoopField169)) == StrLower(keyWordCodeInTheTranspiledLangStartJAVA)) {
            holdTempDataProgrammingBlockThenPutInArr = "";
            inProgarmmingBlock = 1;
        }
        else if (Trim(StrLower(A_LoopField169)) == StrLower(keyWordCodeInTheTranspiledLangStartKT)) {
            holdTempDataProgrammingBlockThenPutInArr = "";
            inProgarmmingBlock = 1;
        }
        else if (Trim(StrLower(A_LoopField169)) == StrLower(keyWordCodeInTheTranspiledLangStartRB)) {
            holdTempDataProgrammingBlockThenPutInArr = "";
            inProgarmmingBlock = 1;
        }
        else if (Trim(StrLower(A_LoopField169)) == StrLower(keyWordCodeInTheTranspiledLangStartNIM)) {
            holdTempDataProgrammingBlockThenPutInArr = "";
            inProgarmmingBlock = 1;
        }
        else if (Trim(StrLower(A_LoopField169)) == StrLower(keyWordCodeInTheTranspiledLangStartAHK)) {
            holdTempDataProgrammingBlockThenPutInArr = "";
            inProgarmmingBlock = 1;
        }
        else if (Trim(StrLower(A_LoopField169)) == StrLower(keyWordCodeInTheTranspiledLangStartSWIFT)) {
            holdTempDataProgrammingBlockThenPutInArr = "";
            inProgarmmingBlock = 1;
        }
        else if (Trim(StrLower(A_LoopField169)) == StrLower(keyWordCodeInTheTranspiledLangStartDART)) {
            holdTempDataProgrammingBlockThenPutInArr = "";
            inProgarmmingBlock = 1;
        }
        else if (Trim(StrLower(A_LoopField169)) == StrLower(keyWordCodeInTheTranspiledLangStartTS)) {
            holdTempDataProgrammingBlockThenPutInArr = "";
            inProgarmmingBlock = 1;
        }
        else if (Trim(StrLower(A_LoopField169)) == StrLower(keyWordCodeInTheTranspiledLangStartGROOVY)) {
            holdTempDataProgrammingBlockThenPutInArr = "";
            inProgarmmingBlock = 1;
        }
        else if (Trim(StrLower(A_LoopField169)) == StrLower(keyWordCodeInTheTranspiledLangStartHTVM)) {
            holdTempDataProgrammingBlockThenPutInArr = "";
            inProgarmmingBlock = 1;
        }
        else if (Trim(StrLower(A_LoopField169)) == StrLower(keyWordCodeInHTVMstart)) {
            //;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;
            //;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;
            holdTempDataProgrammingBlockThenPutInArr = "";
            inProgarmmingBlock = 1;
            ...
            programmingBlocksTemp += A_LoopField169 + Chr(10);
        }
    }
    ...
    if (isNotHTVMfile == 0) {
        if (langToConvertTo != langFileExtension_2) {
            ...
            if (doseHaveInclude(code)) {
                ...
                for (int A_Index170 = 0; A_Index170 < 10000 + 0; A_Index170++) {
                    ...
                    for (size_t A_Index171 = 0; A_Index171 < items171.size() + 0; A_Index171++) {
                        ...
                        if (SubStr(StrLower(Trim(currentLine)), 1, StrLen(keyWordInclude + " ")) == StrLower(keyWordInclude + " ")) {
                            ...
                            for (int A_Index172 = 0; A_Index172 < HTVM_Size(includedFilePaths) + 0; A_Index172++) {
                                if (filePathToInclude == includedFilePaths[A_Index172]) {
        ...
        inProgarmmingBlock = 0;
        holdTempDataProgrammingBlockThenPutInArr = "";
        std::vector<std::string> items173 = LoopParseFunc(code, "\n", "\r");
        for (size_t A_Index173 = 0; A_Index173 < items173.size() + 0; A_Index173++) {
            std::string A_LoopField173 = items173[A_Index173 - 0];
            if (Trim(StrLower(A_LoopField173)) == StrLower(keyWordCodeInTheTranspiledLangEnd)) {
                COUNT_programmingBlock_InTheTranspiledLang++;
                holdTempDataProgrammingBlockThenPutInArr = StringTrimRight(holdTempDataProgrammingBlockThenPutInArr, 1);
                programmingBlocksTemp += "programmingBlock_InTheTranspiledLang-programmingBlock_InTheTranspiledLang-AA" + STR(COUNT_programmingBlock_InTheTranspiledLang) + "AA\n";
                HTVM_Append(programmingBlock_InTheTranspiledLang, Chr(10) + holdTempDataProgrammingBlockThenPutInArr);
            ...
            else if (Trim(StrLower(A_LoopField173)) == StrLower(keyWordCodeInTheTranspiledLangEndCPP)) {
                COUNT_programmingBlock_CPP++;
                holdTempDataProgrammingBlockThenPutInArr = StringTrimRight(holdTempDataProgrammingBlockThenPutInArr, 1);
                programmingBlocksTemp += "programmingBlock_CPP-programmingBlock_CPP-AA" + STR(COUNT_programmingBlock_CPP) + "AA\n";
                HTVM_Append(programmingBlock_CPP, Chr(10) + holdTempDataProgrammingBlockThenPutInArr);
            ...
            else if (Trim(StrLower(A_LoopField173)) == StrLower(keyWordCodeInTheTranspiledLangEndPY)) {
                COUNT_programmingBlock_PY++;
                holdTempDataProgrammingBlockThenPutInArr = StringTrimRight(holdTempDataProgrammingBlockThenPutInArr, 1);
                programmingBlocksTemp += "programmingBlock_PY-programmingBlock_PY-AA" + STR(COUNT_programmingBlock_PY) + "AA\n";
                HTVM_Append(programmingBlock_PY, Chr(10) + holdTempDataProgrammingBlockThenPutInArr);
            ...
            else if (Trim(StrLower(A_LoopField173)) == StrLower(keyWordCodeInTheTranspiledLangEndJS)) {
                COUNT_programmingBlock_JS++;
                holdTempDataProgrammingBlockThenPutInArr = StringTrimRight(holdTempDataProgrammingBlockThenPutInArr, 1);
                programmingBlocksTemp += "programmingBlock_JS-programmingBlock_JS-AA" + STR(COUNT_programmingBlock_JS) + "AA\n";
                HTVM_Append(programmingBlock_JS, Chr(10) + holdTempDataProgrammingBlockThenPutInArr);
            ...
            else if (Trim(StrLower(A_LoopField173)) == StrLower(keyWordCodeInTheTranspiledLangEndGO)) {
                COUNT_programmingBlock_GO++;
                holdTempDataProgrammingBlockThenPutInArr = StringTrimRight(holdTempDataProgrammingBlockThenPutInArr, 1);
                programmingBlocksTemp += "programmingBlock_GO-programmingBlock_GO-AA" + STR(COUNT_programmingBlock_GO) + "AA\n";
                HTVM_Append(programmingBlock_GO, Chr(10) + holdTempDataProgrammingBlockThenPutInArr);
            ...
            else if (Trim(StrLower(A_LoopField173)) == StrLower(keyWordCodeInTheTranspiledLangEndLUA)) {
                COUNT_programmingBlock_LUA++;
                holdTempDataProgrammingBlockThenPutInArr = StringTrimRight(holdTempDataProgrammingBlockThenPutInArr, 1);
                programmingBlocksTemp += "programmingBlock_LUA-programmingBlock_LUA-AA" + STR(COUNT_programmingBlock_LUA) + "AA\n";
                HTVM_Append(programmingBlock_LUA, Chr(10) + holdTempDataProgrammingBlockThenPutInArr);
            ...
            else if (Trim(StrLower(A_LoopField173)) == StrLower(keyWordCodeInTheTranspiledLangEndCS)) {
                COUNT_programmingBlock_CS++;
                holdTempDataProgrammingBlockThenPutInArr = StringTrimRight(holdTempDataProgrammingBlockThenPutInArr, 1);
                programmingBlocksTemp += "programmingBlock_CS-programmingBlock_CS-AA" + STR(COUNT_programmingBlock_CS) + "AA\n";
                HTVM_Append(programmingBlock_CS, Chr(10) + holdTempDataProgrammingBlockThenPutInArr);
            ...
            else if (Trim(StrLower(A_LoopField173)) == StrLower(keyWordCodeInTheTranspiledLangEndJAVA)) {
                COUNT_programmingBlock_JAVA++;
                holdTempDataProgrammingBlockThenPutInArr = StringTrimRight(holdTempDataProgrammingBlockThenPutInArr, 1);
                programmingBlocksTemp += "programmingBlock_JAVA-programmingBlock_JAVA-AA" + STR(COUNT_programmingBlock_JAVA) + "AA\n";
                HTVM_Append(programmingBlock_JAVA, Chr(10) + holdTempDataProgrammingBlockThenPutInArr);
            ...
            else if (Trim(StrLower(A_LoopField173)) == StrLower(keyWordCodeInTheTranspiledLangEndKT)) {
                COUNT_programmingBlock_KT++;
                holdTempDataProgrammingBlockThenPutInArr = StringTrimRight(holdTempDataProgrammingBlockThenPutInArr, 1);
                programmingBlocksTemp += "programmingBlock_KT-programmingBlock_KT-AA" + STR(COUNT_programmingBlock_KT) + "AA\n";
                HTVM_Append(programmingBlock_KT, Chr(10) + holdTempDataProgrammingBlockThenPutInArr);
            ...
            else if (Trim(StrLower(A_LoopField173)) == StrLower(keyWordCodeInTheTranspiledLangEndRB)) {
                COUNT_programmingBlock_RB++;
                holdTempDataProgrammingBlockThenPutInArr = StringTrimRight(holdTempDataProgrammingBlockThenPutInArr, 1);
                programmingBlocksTemp += "programmingBlock_RB-programmingBlock_RB-AA" + STR(COUNT_programmingBlock_RB) + "AA\n";
                HTVM_Append(programmingBlock_RB, Chr(10) + holdTempDataProgrammingBlockThenPutInArr);
            ...
            else if (Trim(StrLower(A_LoopField173)) == StrLower(keyWordCodeInTheTranspiledLangEndNIM)) {
                COUNT_programmingBlock_NIM++;
                holdTempDataProgrammingBlockThenPutInArr = StringTrimRight(holdTempDataProgrammingBlockThenPutInArr, 1);
                programmingBlocksTemp += "programmingBlock_NIM-programmingBlock_NIM-AA" + STR(COUNT_programmingBlock_NIM) + "AA\n";
                HTVM_Append(programmingBlock_NIM, Chr(10) + holdTempDataProgrammingBlockThenPutInArr);
            ...
            else if (Trim(StrLower(A_LoopField173)) == StrLower(keyWordCodeInTheTranspiledLangEndAHK)) {
                COUNT_programmingBlock_AHK++;
                holdTempDataProgrammingBlockThenPutInArr = StringTrimRight(holdTempDataProgrammingBlockThenPutInArr, 1);
                programmingBlocksTemp += "programmingBlock_AHK-programmingBlock_AHK-AA" + STR(COUNT_programmingBlock_AHK) + "AA\n";
                HTVM_Append(programmingBlock_AHK, Chr(10) + holdTempDataProgrammingBlockThenPutInArr);
            ...
            else if (Trim(StrLower(A_LoopField173)) == StrLower(keyWordCodeInTheTranspiledLangEndSWIFT)) {
                COUNT_programmingBlock_SWIFT++;
                holdTempDataProgrammingBlockThenPutInArr = StringTrimRight(holdTempDataProgrammingBlockThenPutInArr, 1);
                programmingBlocksTemp += "programmingBlock_SWIFT-programmingBlock_SWIFT-AA" + STR(COUNT_programmingBlock_SWIFT) + "AA\n";
                HTVM_Append(programmingBlock_SWIFT, Chr(10) + holdTempDataProgrammingBlockThenPutInArr);
            ...
            else if (Trim(StrLower(A_LoopField173)) == StrLower(keyWordCodeInTheTranspiledLangEndDART)) {
                COUNT_programmingBlock_DART++;
                holdTempDataProgrammingBlockThenPutInArr = StringTrimRight(holdTempDataProgrammingBlockThenPutInArr, 1);
                programmingBlocksTemp += "programmingBlock_DART-programmingBlock_DART-AA" + STR(COUNT_programmingBlock_DART) + "AA\n";
                HTVM_Append(programmingBlock_DART, Chr(10) + holdTempDataProgrammingBlockThenPutInArr);
            ...
            else if (Trim(StrLower(A_LoopField173)) == StrLower(keyWordCodeInTheTranspiledLangEndTS)) {
                COUNT_programmingBlock_TS++;
                holdTempDataProgrammingBlockThenPutInArr = StringTrimRight(holdTempDataProgrammingBlockThenPutInArr, 1);
                programmingBlocksTemp += "programmingBlock_TS-programmingBlock_TS-AA" + STR(COUNT_programmingBlock_TS) + "AA\n";
                HTVM_Append(programmingBlock_TS, Chr(10) + holdTempDataProgrammingBlockThenPutInArr);
            ...
            else if (Trim(StrLower(A_LoopField173)) == StrLower(keyWordCodeInTheTranspiledLangEndGROOVY)) {
                COUNT_programmingBlock_GROOVY++;
                holdTempDataProgrammingBlockThenPutInArr = StringTrimRight(holdTempDataProgrammingBlockThenPutInArr, 1);
                programmingBlocksTemp += "programmingBlock_GROOVY-programmingBlock_GROOVY-AA" + STR(COUNT_programmingBlock_GROOVY) + "AA\n";
                HTVM_Append(programmingBlock_GROOVY, Chr(10) + holdTempDataProgrammingBlockThenPutInArr);
            ...
            else if (Trim(StrLower(A_LoopField173)) == StrLower(keyWordCodeInTheTranspiledLangEndHTVM)) {
                COUNT_programmingBlock_HTVM++;
                holdTempDataProgrammingBlockThenPutInArr = StringTrimRight(holdTempDataProgrammingBlockThenPutInArr, 1);
                programmingBlocksTemp += "programmingBlock_HTVM-programmingBlock_HTVM-AA" + STR(COUNT_programmingBlock_HTVM) + "AA\n";
                HTVM_Append(programmingBlock_HTVM, Chr(10) + holdTempDataProgrammingBlockThenPutInArr);
            ...
            else if (Trim(StrLower(A_LoopField173)) == StrLower(keyWordCodeInHTVMend)) {
                //;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;
                //;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;
                COUNT_programmingBlock_HTVMsyntax++;
                holdTempDataProgrammingBlockThenPutInArr = StringTrimRight(holdTempDataProgrammingBlockThenPutInArr, 1);
                ...
                holdTempDataProgrammingBlockThenPutInArr += A_LoopField173 + Chr(10);
            }
            else if (Trim(StrLower(A_LoopField173)) == StrLower(keyWordCodeInTheTranspiledLangStart)) {
                holdTempDataProgrammingBlockThenPutInArr = "";
                inProgarmmingBlock = 1;
            }
            else if (Trim(StrLower(A_LoopField173)) == StrLower(keyWordCodeInTheTranspiledLangStartCPP)) {
                holdTempDataProgrammingBlockThenPutInArr = "";
                inProgarmmingBlock = 1;
            }
            else if (Trim(StrLower(A_LoopField173)) == StrLower(keyWordCodeInTheTranspiledLangStartPY)) {
                holdTempDataProgrammingBlockThenPutInArr = "";
                inProgarmmingBlock = 1;
            }
            else if (Trim(StrLower(A_LoopField173)) == StrLower(keyWordCodeInTheTranspiledLangStartJS)) {
                holdTempDataProgrammingBlockThenPutInArr = "";
                inProgarmmingBlock = 1;
            }
            else if (Trim(StrLower(A_LoopField173)) == StrLower(keyWordCodeInTheTranspiledLangStartGO)) {
                holdTempDataProgrammingBlockThenPutInArr = "";
                inProgarmmingBlock = 1;
            }
            else if (Trim(StrLower(A_LoopField173)) == StrLower(keyWordCodeInTheTranspiledLangStartLUA)) {
                holdTempDataProgrammingBlockThenPutInArr = "";
                inProgarmmingBlock = 1;
            }
            else if (Trim(StrLower(A_LoopField173)) == StrLower(keyWordCodeInTheTranspiledLangStartCS)) {
                holdTempDataProgrammingBlockThenPutInArr = "";
                inProgarmmingBlock = 1;
            }
            else if (Trim(StrLower(A_LoopField173)) == StrLower(keyWordCodeInTheTranspiledLangStartJAVA)) {
                holdTempDataProgrammingBlockThenPutInArr = "";
                inProgarmmingBlock = 1;
            }
            else if (Trim(StrLower(A_LoopField173)) == StrLower(keyWordCodeInTheTranspiledLangStartKT)) {
                holdTempDataProgrammingBlockThenPutInArr = "";
                inProgarmmingBlock = 1;
            }
            else if (Trim(StrLower(A_LoopField173)) == StrLower(keyWordCodeInTheTranspiledLangStartRB)) {
                holdTempDataProgrammingBlockThenPutInArr = "";
                inProgarmmingBlock = 1;
            }
            else if (Trim(StrLower(A_LoopField173)) == StrLower(keyWordCodeInTheTranspiledLangStartNIM)) {
                holdTempDataProgrammingBlockThenPutInArr = "";
                inProgarmmingBlock = 1;
            }
            else if (Trim(StrLower(A_LoopField173)) == StrLower(keyWordCodeInTheTranspiledLangStartAHK)) {
                holdTempDataProgrammingBlockThenPutInArr = "";
                inProgarmmingBlock = 1;
            }
            else if (Trim(StrLower(A_LoopField173)) == StrLower(keyWordCodeInTheTranspiledLangStartSWIFT)) {
                holdTempDataProgrammingBlockThenPutInArr = "";
                inProgarmmingBlock = 1;
            }
            else if (Trim(StrLower(A_LoopField173)) == StrLower(keyWordCodeInTheTranspiledLangStartDART)) {
                holdTempDataProgrammingBlockThenPutInArr = "";
                inProgarmmingBlock = 1;
            }
            else if (Trim(StrLower(A_LoopField173)) == StrLower(keyWordCodeInTheTranspiledLangStartTS)) {
                holdTempDataProgrammingBlockThenPutInArr = "";
                inProgarmmingBlock = 1;
            }
            else if (Trim(StrLower(A_LoopField173)) == StrLower(keyWordCodeInTheTranspiledLangStartGROOVY)) {
                holdTempDataProgrammingBlockThenPutInArr = "";
                inProgarmmingBlock = 1;
            }
            else if (Trim(StrLower(A_LoopField173)) == StrLower(keyWordCodeInTheTranspiledLangStartHTVM)) {
                holdTempDataProgrammingBlockThenPutInArr = "";
                inProgarmmingBlock = 1;
            }
            else if (Trim(StrLower(A_LoopField173)) == StrLower(keyWordCodeInHTVMstart)) {
                //;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;
                //;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;
                holdTempDataProgrammingBlockThenPutInArr = "";
                inProgarmmingBlock = 1;
                ...
                programmingBlocksTemp += A_LoopField173 + Chr(10);
            }
        }
        code = StringTrimRight(programmingBlocksTemp, 1);
        //;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;; ----------------
    ...
    if (isNotHTVMfile == 0) {
        //;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;
        //;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;
        std::vector<std::string> items174 = LoopParseFunc(code, "\n", "\r");
        for (size_t A_Index174 = 0; A_Index174 < items174.size() + 0; A_Index174++) {
            std::string A_LoopField174 = items174[A_Index174 - 0];
            if (SubStr(Trim(A_LoopField174), 1, StrLen(keyWordComment)) == keyWordComment) {
                commentsBugFixHold = StringTrimLeft(Trim(A_LoopField174), StrLen(keyWordComment));
                std::vector<std::string> items175 = LoopParseFunc(A_LoopField174);
                for (size_t A_Index175 = 0; A_Index175 < items175.size() + 0; A_Index175++) {
                    std::string A_LoopField175 = items175[A_Index175 - 0];
                    if (InStr(A_LoopField175, " ") || InStr(A_LoopField175, Chr(9))) {
                        commentsBugFixHold1 = A_LoopField175;
                    } else {
                        break;
                    }
                }
                ...
                commentsBugFixOut += A_LoopField174 + Chr(10);
            }
        }
        ...
        //print("============programmingBlock_CPP==================")
        //print("============programmingBlock_CPP==================")
        std::vector<std::string> items176 = LoopParseFunc(code);
        ...
        std::vector<std::string> items177 = LoopParseFunc(code);
        for (size_t A_Index177 = 0; A_Index177 < items177.size() + 0; A_Index177++) {
            std::string A_LoopField177 = items177[A_Index177 - 0];
            theIdNumOfThe34theVar[A_Index177] = theIdNumOfThe34theVar[A_Index177] + Chr(34);
            HTVM_Append(getAllCharForTheFurtureSoIcanAddEscapeChar, A_LoopField177);
        }
        ...
        std::vector<std::string> items178 = LoopParseFunc(code);
        for (size_t A_Index178 = 0; A_Index178 < items178.size() + 0; A_Index178++) {
            std::string A_LoopField178 = items178[A_Index178 - 0];
            if (A_LoopField178 == keyWordEscpaeChar && getAllCharForTheFurtureSoIcanAddEscapeChar[A_Index178 + 1] == Chr(34)) {
                ...
                if (fixOutFixDoubleQuotesInsideDoubleQuotesFIXok != 1) {
                    OutFixDoubleQuotesInsideDoubleQuotes += A_LoopField178;
                } else {
        ...
        if (keyWordEscpaeChar == Chr(92) && langToConvertTo != langFileExtension_2) {
            std::vector<std::string> items179 = LoopParseFunc(code);
            for (size_t A_Index179 = 0; A_Index179 < items179.size() + 0; A_Index179++) {
                std::string A_LoopField179 = items179[A_Index179 - 0];
                if (A_LoopField179 == Chr(34)) {
                    areWEinSome34sNum++;
                }
                if (areWEinSome34sNum == 1) {
                    if (A_LoopField179 != Chr(34)) {
                        if (A_LoopField179 == keyWordEscpaeChar && langToConvertTo != langFileExtension_2) {
                            theIdNumOfThe34theVar[theIdNumOfThe34] = theIdNumOfThe34theVar[theIdNumOfThe34] + Chr(92);
                        } else {
                            theIdNumOfThe34theVar[theIdNumOfThe34] = theIdNumOfThe34theVar[theIdNumOfThe34] + A_LoopField179;
                        }
                    } else {
                        theIdNumOfThe34++;
                ...
                if (areWEinSome34sNum == 2 || areWEinSome34sNum == 0) {
                    if (A_LoopField179 != Chr(34)) {
                        htCodeOUT754754 += A_LoopField179;
                    }
                    areWEinSome34sNum = 0;
                }
            }
        } else {
            std::vector<std::string> items180 = LoopParseFunc(code);
            for (size_t A_Index180 = 0; A_Index180 < items180.size() + 0; A_Index180++) {
                std::string A_LoopField180 = items180[A_Index180 - 0];
                if (A_LoopField180 == Chr(34)) {
                    areWEinSome34sNum++;
                }
                if (areWEinSome34sNum == 1) {
                    if (A_LoopField180 != Chr(34)) {
                        if (A_LoopField180 == keyWordEscpaeChar && keyWordEscpaeChar == getAllCharForTheFurtureSoIcanAddEscapeChar[A_Index180 + 1] && langToConvertTo != langFileExtension_2) {
                        ...
                        else if (A_LoopField180 == keyWordEscpaeChar && langToConvertTo != langFileExtension_2) {
                            ...
                            theIdNumOfThe34theVar[theIdNumOfThe34] = theIdNumOfThe34theVar[theIdNumOfThe34] + A_LoopField180;
                        }
                    } else {
                        theIdNumOfThe34++;
                        htCodeOUT754754 += "ihuiuuhuuhtheidFor--asds" + str21 + "as--" + str21 + "theuhturtyphoutr--" + Chr(65) + Chr(65) + STR(theIdNumOfThe34) + Chr(65) + Chr(65);
                ...
                if (areWEinSome34sNum == 2 || areWEinSome34sNum == 0) {
                    if (A_LoopField180 != Chr(34)) {
                        htCodeOUT754754 += A_LoopField180;
                    }
                    areWEinSome34sNum = 0;
                }
            }
        ...
        for (int A_Index181 = 0; A_Index181 < theIdNumOfThe34 + 0; A_Index181++) {
            theIdNumOfThe34theVar[A_Index181] = theIdNumOfThe34theVar[A_Index181] + Chr(34);
        }
        ...
        for (size_t A_Index182 = 0; A_Index182 < items182.size() + 0; A_Index182++) {
            ...
            if (InStr(A_LoopField182, " " + keyWordComment) != false && SubStr(Trim(A_LoopField182), 1, StrLen(keyWordComment)) != keyWordComment) {
                posForRemoveCommentsOnTheEndOfTheLine = 0;
                slidingWinFixCommentsRmI = 0;
                slidingWinFixCommentsRm = slidingWinFixCommentsRmFUNC(A_LoopField182);
                for (int A_Index183 = 0; A_Index183 < HTVM_Size(slidingWinFixCommentsRm) + 0; A_Index183++) {
                    slidingWinFixCommentsRmI = A_Index183;
                    slidingWinFixCommentsRmHold = "";
                    for (int A_Index184 = 0; A_Index184 < StrLen(" " + keyWordComment) + 0; A_Index184++) {
                        slidingWinFixCommentsRmHold += slidingWinFixCommentsRm[slidingWinFixCommentsRmI + A_Index184];
                    }
                    if (Trim(slidingWinFixCommentsRmHold) == keyWordComment) {
                        posForRemoveCommentsOnTheEndOfTheLine = A_Index183;
                        break;
                    }
                }
                if (posForRemoveCommentsOnTheEndOfTheLine > 0) {
                    // Keep only the part before the comment
                    newStringOutCode += SubStr(A_LoopField182, 1, posForRemoveCommentsOnTheEndOfTheLine) + Chr(10);
                } else {
                    // Keep the whole line as it is
                    newStringOutCode += A_LoopField182 + Chr(10);
                }
            } else {
                newStringOutCode += A_LoopField182 + Chr(10);
            }
        ...
        if (langToConvertTo != langFileExtension_2) {
            ...
            for (size_t A_Index185 = 0; A_Index185 < items185.size() + 0; A_Index185++) {
                ...
                if (SubStr(Trim(A_LoopField185), 1, 1) != ";") {
                    semiColonForOneLinerOut += StrReplace(A_LoopField185, ";", Chr(10)) + Chr(10);
                } else {
                    semiColonForOneLinerOut += A_LoopField185 + Chr(10);
                }
            }
        ...
        if (useCurlyBraces == "on") {
            ...
            str1 = "";
            std::vector<std::string> items186 = LoopParseFunc(code, "\n", "\r");
            for (size_t A_Index186 = 0; A_Index186 < items186.size() + 0; A_Index186++) {
                std::string A_LoopField186 = items186[A_Index186 - 0];
                str1 = "";
                if (InStr(A_LoopField186, keyWordCurlyBraceOpen) || InStr(A_LoopField186, keyWordCurlyBraceClose)) {
                    if (SubStr(Trim(A_LoopField186), 1, StrLen(Trim(keyWordComment))) == Trim(keyWordComment) && isLineAconstruct(Trim(A_LoopField186)) == false) {
                        fixSomeCurlyBraceForExratFlexability += A_LoopField186 + Chr(10);
                    } else {
                        if (InStr(A_LoopField186, Trim(keyWordArrayDefinition) + " " + keyWordAssign + " ") || InStr(A_LoopField186, Trim(keyWordArrayOfIntegersDefinition) + " " + keyWordAssign + " ") || InStr(A_LoopField186, Trim(keyWordArrayOfStringsDefinition) + " " + keyWordAssign + " ") || InStr(A_LoopField186, Trim(keyWordArrayOfFloatingPointNumbersDefinition) + " " + keyWordAssign + " ") || InStr(A_LoopField186, Trim(keyWordArrayOfBooleansDefinition) + " " + keyWordAssign + " ") && useCurlyBracesSyntaxForArrayDef == "on") {
                            fixSomeCurlyBraceForExratFlexability += A_LoopField186 + Chr(10);
                        } else {
                            str1 = StrReplace(A_LoopField186, keyWordCurlyBraceOpen, "{");
            ...
            for (size_t A_Index187 = 0; A_Index187 < items187.size() + 0; A_Index187++) {
                ...
                fixSomeCurlyBraceForExratFlexabilityTriHELP += Trim(A_LoopField187) + Chr(10);
            }
            ...
            if (keyWordCurlyBraceOpen != "{") {
                ...
                for (size_t A_Index188 = 0; A_Index188 < items188.size() + 0; A_Index188++) {
                    std::string A_LoopField188 = items188[A_Index188 - 0];
                    htCodeOutFixINT2 = 0;
                    if (isLineAconstruct(Trim(A_LoopField188)) == true || Trim(A_LoopField188) == keyWordCurlyBraceOpen) {
                        htCodeOutFixINT2 = 1;
                    }
                    htCodeOutFixINT = 0;
                    std::vector<std::string> items189 = LoopParseFunc(A_LoopField188, " ");
                    ...
                    for (size_t A_Index190 = 0; A_Index190 < items190.size() + 0; A_Index190++) {
                        ...
                        if (htCodeOutFixINT == A_Index190 && htCodeOutFixINT2 == 1) {
                            htCodeOutFixINT2 = 0;
                            htCodeOutFixEnd += HTVMmatchStrRrplace(A_LoopField190, keyWordCurlyBraceOpen, "{") + " ";
                        } else {
                            htCodeOutFixEnd += A_LoopField190 + " ";
                        }
                    }
            ...
            if (keyWordCurlyBraceClose != "}") {
                std::vector<std::string> items191 = LoopParseFunc(code, "\n", "\r");
                for (size_t A_Index191 = 0; A_Index191 < items191.size() + 0; A_Index191++) {
                    std::string A_LoopField191 = items191[A_Index191 - 0];
                    htCodeOutFixINT2 = 0;
                    if (isLineAconstruct(Trim(A_LoopField191)) == true || Trim(A_LoopField191) == keyWordCurlyBraceClose) {
                        htCodeOutFixINT2 = 1;
                    }
                    std::vector<std::string> items192 = LoopParseFunc(A_LoopField191, " ");
                    for (size_t A_Index192 = 0; A_Index192 < items192.size() + 0; A_Index192++) {
                        std::string A_LoopField192 = items192[A_Index192 - 0];
                        if (htCodeOutFixINT2 == 1 || Trim(A_LoopField192) == keyWordCurlyBraceClose) {
                            htCodeOutFixINT2 = 0;
                            htCodeOutFixEnd += HTVMmatchStrRrplace(A_LoopField192, keyWordCurlyBraceClose, "}") + " ";
                        } else {
        ...
        if (useCurlyBraces == "on") {
            ...
            for (size_t A_Index193 = 0; A_Index193 < items193.size() + 0; A_Index193++) {
                ...
                HTVM_Append(addNoCurlyFixWhenNextLineAfterConstructLookAhead, Trim(A_LoopField193));
            }
            ...
            for (size_t A_Index194 = 0; A_Index194 < items194.size() + 0; A_Index194++) {
                ...
                if (isLineAconstruct(Trim(A_LoopField194)) && addNoCurlyFixWhenNextLineAfterConstructLookAhead[A_Index194 + 1] != "{") {
                    int5 = 1;
                    addNoCurlyFixWhenNextLineAfterConstructOut += A_LoopField194 + Chr(10) + "{" + Chr(10);
                } else {
                    if (int5 == 1) {
                        int5 = 0;
                        addNoCurlyFixWhenNextLineAfterConstructOut += A_LoopField194 + Chr(10) + "}" + Chr(10);
                    } else {
                        addNoCurlyFixWhenNextLineAfterConstructOut += A_LoopField194 + Chr(10);
                    }
                }
            }
        ...
        if (useEnd == "on") {
            ...
            for (size_t A_Index195 = 0; A_Index195 < items195.size() + 0; A_Index195++) {
                ...
                if (StrLower(Trim(A_LoopField195)) == StrLower(keyWordEnd)) {
                    htCodeOutFixEnd += "}\n";
                }
                else if (SubStr(StrLower(Trim(A_LoopField195)), 1, StrLen(StrLower(keyWordIF))) == StrLower(keyWordIF) && isLineAconstruct(Trim(A_LoopField195)) == true) {
                    htCodeOutFixEnd += A_LoopField195 + "\n{\n";
                }
                else if (SubStr(StrLower(Trim(A_LoopField195)), 1, StrLen(StrLower(keyWordWhileLoop))) == StrLower(keyWordWhileLoop) && isLineAconstruct(Trim(A_LoopField195)) == true) {
                    htCodeOutFixEnd += A_LoopField195 + "\n{\n";
                }
                else if (SubStr(StrLower(Trim(A_LoopField195)), 1, StrLen(StrLower(keyWordElseIf))) == StrLower(keyWordElseIf) && isLineAconstruct(Trim(A_LoopField195)) == true) {
                    htCodeOutFixEnd += "\n}\n" + A_LoopField195 + "\n{\n";
                }
                else if (SubStr(StrLower(Trim(A_LoopField195)), 1, StrLen(StrLower(keyWordElse))) == StrLower(keyWordElse) && isLineAconstruct(Trim(A_LoopField195)) == true) {
                    htCodeOutFixEnd += "\n}\n" + A_LoopField195 + "\n{\n";
                }
                else if (SubStr(StrLower(Trim(A_LoopField195)), 1, StrLen(StrLower(keyWordCatch))) == StrLower(keyWordCatch) && isLineAconstruct(Trim(A_LoopField195)) == true) {
                    htCodeOutFixEnd += "\n}\n" + A_LoopField195 + "\n{\n";
                }
                else if (SubStr(StrLower(Trim(A_LoopField195)), 1, StrLen(StrLower(keyWordFinally))) == StrLower(keyWordFinally) && isLineAconstruct(Trim(A_LoopField195)) == true) {
                    htCodeOutFixEnd += "\n}\n" + A_LoopField195 + "\n{\n";
                }
                else if (isLineAconstruct(Trim(A_LoopField195)) == true) {
                    htCodeOutFixEnd += A_LoopField195 + "\n{\n";
                } else {
                    htCodeOutFixEnd += A_LoopField195 + Chr(10);
                }
            }
            code = StringTrimRight(htCodeOutFixEnd, 1);
        }
        if (usePythonicColonSyntax == "off") {
            if (useEnd == "off" && useCurlyBraces == "off") {
                ...
                for (size_t A_Index196 = 0; A_Index196 < items196.size() + 0; A_Index196++) {
                    ...
                    if (isLineAconstruct(Trim(A_LoopField196)) == true) {
                        htCodeOutFixEnd += A_LoopField196 + ":\n";
                    } else {
                        htCodeOutFixEnd += A_LoopField196 + Chr(10);
                    }
                }
                code = StringTrimRight(htCodeOutFixEnd, 1);
            }
        ...
        outTrimCode = "";
        std::vector<std::string> items197 = LoopParseFunc(code, "\n", "\r");
        for (size_t A_Index197 = 0; A_Index197 < items197.size() + 0; A_Index197++) {
            std::string A_LoopField197 = items197[A_Index197 - 0];
            outTrimCode += Trim(A_LoopField197) + Chr(10);
        }
        code = StringTrimRight(outTrimCode, 1);
        // for converting c++ to js and py
        //code := StrReplace(code, "{}", "[]")
        std::string outCodeFixBraces = "";
        for (int A_Index198 = 0; A_Index198 < 2 + 0; A_Index198++) {
            outCodeFixBraces = "";
            ...
            for (size_t A_Index199 = 0; A_Index199 < items199.size() + 0; A_Index199++) {
                ...
                if (InStr(A_LoopField199, Trim(keyWordArrayDefinition) + " " + keyWordAssign + " ") || InStr(A_LoopField199, Trim(keyWordArrayOfIntegersDefinition) + " " + keyWordAssign + " ") || InStr(A_LoopField199, Trim(keyWordArrayOfStringsDefinition) + " " + keyWordAssign + " ") || InStr(A_LoopField199, Trim(keyWordArrayOfFloatingPointNumbersDefinition) + " " + keyWordAssign + " ") || InStr(A_LoopField199, Trim(keyWordArrayOfBooleansDefinition) + " " + keyWordAssign + " ") && useCurlyBracesSyntaxForArrayDef == "on") {
                    if (SubStr(StrLower(A_LoopField199), 1, StrLen(StrLower(keyWordFunc))) == StrLower(keyWordFunc) || SubStr(StrLower(A_LoopField199), 1, StrLen(StrLower(keyWordAsync) + StrLower(keyWordFunc))) == StrLower(keyWordAsync) + StrLower(keyWordFunc)) {
                        if (InStr(Trim(A_LoopField199), "{") && Trim(A_LoopField199) != "{" && SubStr(Trim(A_LoopField199), 1, StrLen(Trim(keyWordComment))) != Trim(keyWordComment)) {
                            outCodeFixBraces += Trim(StrReplace(Trim(A_LoopField199), "{", "")) + "\n{\n";
                        }
                        else if (InStr(Trim(A_LoopField199), "}") && Trim(A_LoopField199) != "}" && SubStr(Trim(A_LoopField199), 1, StrLen(Trim(keyWordComment))) != Trim(keyWordComment)) {
                            outCodeFixBraces += "}\n" + Trim(StrReplace(Trim(A_LoopField199), "}", "")) + Chr(10);
                        } else {
                            outCodeFixBraces += Trim(A_LoopField199) + Chr(10);
                        }
                    } else {
                        outCodeFixBraces += Trim(A_LoopField199) + Chr(10);
                    }
        ...
        if (usePythonicColonSyntax == "on" || useCurlyBraces == "off" && useEnd == "off") {
            ...
            for (size_t A_Index200 = 0; A_Index200 < items200.size() + 0; A_Index200++) {
                std::string A_LoopField200 = items200[A_Index200 - 0];
                if (isLineAconstruct(Trim(A_LoopField200)) == true) {
                    htCodeOutFixEnd += StringTrimRight(A_LoopField200, 1) + Chr(10);
                } else {
                    htCodeOutFixEnd += A_LoopField200 + Chr(10);
                }
            }
            code = StringTrimRight(htCodeOutFixEnd, 1);
        }
        ...
        if (isNotHTVMfile2 == 0) {
            ...
            for (size_t A_Index201 = 0; A_Index201 < items201.size() + 0; A_Index201++) {
                std::string A_LoopField201 = items201[A_Index201 - 0];
                if (StrLower(Trim(A_LoopField201)) == StrLower(keyWordMainLabel)) {
                    didWeUseMainLabel = 1;
                    if (langToConvertTo == langFileExtension_2) {
                    ...
                    codeOutFixAndAddMainFunc += A_LoopField201 + Chr(10);
                }
            }
        ...
        for (size_t A_Index202 = 0; A_Index202 < items202.size() + 0; A_Index202++) {
            ...
            HTVM_Append(lookIntoTheNextLineForFuncWhitNoKeyWord, A_LoopField202);
        }
        ...
        for (size_t A_Index203 = 0; A_Index203 < items203.size() + 0; A_Index203++) {
            ...
            lineDone = 0;
            if (SubStr(A_LoopField203, 1, StrLen(keyWordComment)) == keyWordComment) {
                lineDone = 1;
                str1 = StringTrimLeft(A_LoopField203, StrLen(keyWordComment));
                if (langToConvertTo == langFileExtension_2) {
            ...
            else if (InStr(A_LoopField203, "guiInit(") || InStr(A_LoopField203, "guiAdd(") || InStr(A_LoopField203, "guiAddElement(") || InStr(A_LoopField203, "guiControl(") || InStr(A_LoopField203, "HTVM_init(") || InStr(A_LoopField203, "HTVM_register(") || InStr(A_LoopField203, "HTVM_custom_port(") && lineDone == 0) {
                lineDone = 1;
                if (InStr(A_LoopField203, "HTVM_init(")) {
                    htCode += Trim(A_LoopField203) + Chr(10);
                } else {
                    htCode += Trim(expressionParserTranspiler(Trim(A_LoopField203))) + Chr(10);
                }
            }
            else if (Trim(A_LoopField203) == keyWordCommentOpenMultiLine) {
                lineDone = 1;
                if (langToConvertTo == langFileExtension_2) {
            ...
            else if (Trim(A_LoopField203) == keyWordCommentCloseMultiLine) {
                lineDone = 1;
                if (langToConvertTo == langFileExtension_2) {
            ...
            else if (InStr(A_LoopField203, "[][][][][] ")) {
                lineDone = 1;
                str1 = StrSplit(A_LoopField203, " ", 2);
            ...
            else if (SubStr(StrLower(Trim(A_LoopField203)), 1, StrLen(StrLower(keyWordStruct))) == StrLower(keyWordStruct)) {
                lineDone = 1;
                str0 = Trim(StringTrimLeft(A_LoopField203, StrLen(keyWordStruct)));
                if (langToConvertTo == langFileExtension_2) {
                    htCode += Trim(keyWordStruct_2) + " " + str0 + Chr(10);
                }
            }
            else if (SubStr(StrLower(Trim(A_LoopField203)), 1, StrLen(StrLower(keyWordAlliance))) == StrLower(keyWordAlliance)) {
                lineDone = 1;
                str0 = Trim(StringTrimLeft(A_LoopField203, StrLen(keyWordAlliance)));
                if (langToConvertTo == langFileExtension_2) {
                    htCode += Trim(keyWordAlliance_2) + " " + str0 + Chr(10);
                }
            }
            else if (SubStr(StrLower(Trim(A_LoopField203)), 1, StrLen(StrLower(keyWordCrew))) == StrLower(keyWordCrew)) {
                lineDone = 1;
                str0 = Trim(StringTrimLeft(A_LoopField203, StrLen(keyWordCrew)));
                if (langToConvertTo == langFileExtension_2) {
                    htCode += Trim(keyWordCrew_2) + " " + str0 + Chr(10);
                }
            }
            else if (InStr(A_LoopField203, "[end][end][end][end][end] ")) {
                lineDone = 1;
                if (langToConvertTo == langFileExtension_2) {
                    htCode += "}" + Chr(10);
                }
            }
            else if (SubStr(StrLower(Trim(A_LoopField203)), 1, StrLen(StrLower(keyWordProp))) == StrLower(keyWordProp)) {
                lineDone = 1;
                str0 = Trim(StringTrimLeft(A_LoopField203, StrLen(keyWordProp)));
            ...
            else if (SubStr(StrLower(Trim(A_LoopField203)), 1, StrLen(StrLower(keyWordProc))) == StrLower(keyWordProc)) {
                lineDone = 1;
                str0 = Trim(StringTrimLeft(A_LoopField203, StrLen(keyWordProc)));
                str1 = Trim(StrSplit(str0, "-----------------------HTVM-OSP-HTVM-OBJ--------------", 1));
                str20 = Trim(StrSplit(str0, "-----------------------HTVM-OSP-HTVM-OBJ--------------", 2));
            ...
            else if (SubStr(StrLower(Trim(A_LoopField203)), 1, StrLen(StrLower(keyWordIF))) == StrLower(keyWordIF) && SubStr(StrLower(Trim(A_LoopField203)), 1, StrLen(StrLower(keyWordElseIf))) != StrLower(keyWordElseIf)) {
                lineDone = 1;
                str1 = Trim(StringTrimLeft(A_LoopField203, StrLen(keyWordIF)));
            ...
            else if (SubStr(StrLower(Trim(A_LoopField203)), 1, StrLen(StrLower(keyWordElseIf))) == StrLower(keyWordElseIf) && SubStr(StrLower(Trim(A_LoopField203)), 1, StrLen(StrLower(keyWordIF))) != StrLower(keyWordIF)) {
                lineDone = 1;
                str1 = Trim(StringTrimLeft(A_LoopField203, StrLen(keyWordElseIf)));
                str1 = Trim(expressionParserTranspiler("(" + str1 + ")"));
                str1 = StringTrimRight(str1, 1);
                str1 = StringTrimLeft(str1, 1);
                if (langToConvertTo == langFileExtension_2) {
            ...
            else if (StrLower(Trim(A_LoopField203)) == StrLower(keyWordElse) || StrLower(Trim(A_LoopField203)) == StrLower(keyWordElse + ":")) {
                lineDone = 1;
                if (langToConvertTo == langFileExtension_2) {
                    if (usePythonicColonSyntax_2 == "on") {
            ...
            else if (SubStr(StrLower(Trim(A_LoopField203)), 1, StrLen(StrLower(keyWordWhileLoop))) == StrLower(keyWordWhileLoop)) {
                lineDone = 1;
                str1 = Trim(StringTrimLeft(A_LoopField203, StrLen(keyWordWhileLoop)));
                str1 = Trim(expressionParserTranspiler("(" + str1 + ")"));
                str1 = StringTrimRight(str1, 1);
                str1 = StringTrimLeft(str1, 1);
                if (langToConvertTo == langFileExtension_2) {
            ...
            else if (StrLower(Trim(A_LoopField203)) == StrLower(keyWordTry) || StrLower(Trim(A_LoopField203)) == StrLower(keyWordTry + ":")) {
                lineDone = 1;
                if (langToConvertTo == langFileExtension_2) {
                    if (usePythonicColonSyntax_2 == "on") {
                        htCode += keyWordTry_2 + ":\n";
            ...
            else if (SubStr(StrLower(Trim(A_LoopField203)), 1, StrLen(StrLower(keyWordCatch))) == StrLower(keyWordCatch)) {
                lineDone = 1;
                str1 = Trim(StrReplace(StrReplace(StringTrimLeft(A_LoopField203, StrLen(keyWordCatch)), "(", ""), ")", ""));
            ...
            else if (StrLower(Trim(A_LoopField203)) == StrLower(keyWordFinally) || StrLower(Trim(A_LoopField203)) == StrLower(keyWordFinally + ":")) {
                lineDone = 1;
                if (langToConvertTo == langFileExtension_2) {
                    if (usePythonicColonSyntax_2 == "on") {
            ...
            else if (SubStr(StrLower(Trim(A_LoopField203)), 1, StrLen(StrLower(keyWordThrow))) == StrLower(keyWordThrow)) {
                lineDone = 1;
                str1 = Trim(A_LoopField203);
                if (SubStrLastChars(str1, 1) == ";") {
                    str1 = StringTrimRight(str1, 1);
                }
            ...
            else if (StrLower(A_LoopField203) == StrLower(keyWordBreak) || StrLower(A_LoopField203) == StrLower(keyWordBreak + ";")) {
                lineDone = 1;
                if (langToConvertTo == langFileExtension_2) {
                    if (useSemicolon_2 == "on") {
            ...
            else if (StrLower(A_LoopField203) == StrLower(Trim(keyWordReturnStatement)) || StrLower(A_LoopField203) == StrLower(Trim(keyWordReturnStatement) + ";")) {
                lineDone = 1;
                if (langToConvertTo == langFileExtension_2) {
                    if (useSemicolon_2 == "on") {
            ...
            else if (SubStr(Trim(StrLower(A_LoopField203)), 1, StrLen(StrLower(keyWordReturnStatement))) == StrLower(keyWordReturnStatement)) {
                lineDone = 1;
                str1 = Trim(expressionParserTranspiler(Trim(StringTrimLeft(A_LoopField203, StrLen(keyWordReturnStatement)))));
                if (SubStrLastChars(str1, 1) == ";") {
                    str1 = StringTrimRight(str1, 1);
                }
                if (langToConvertTo == langFileExtension_2) {
            ...
            else if (StrLower(A_LoopField203) == StrLower(keyWordContinue) || StrLower(A_LoopField203) == StrLower(keyWordContinue + ";")) {
                lineDone = 1;
                if (langToConvertTo == langFileExtension_2) {
                    if (useSemicolon_2 == "on") {
            ...
            else if (StrLower(A_LoopField203) == "::" + keyWordContinue || StrLower(A_LoopField203) == "::" + keyWordContinue + ";") {
                lineDone = 1;
                if (langToConvertTo == langFileExtension_2) {
                    htCode += "::" + keyWordContinue_2 + Chr(10);
                }
            ...
            else if (StrLower(A_LoopField203) == StrLower(keyWordLoopInfinite) || StrLower(A_LoopField203) == StrLower(keyWordLoopInfinite + ":")) {
                // infinity loops
            ...
            else if (SubStr(Trim(StrLower(A_LoopField203)), 1, StrLen(StrLower(keyWordLoop))) == StrLower(keyWordLoop) && SubStr(Trim(StrLower(A_LoopField203)), 1, StrLen(StrLower(keyWordLoop)) + 2) != StrLower(keyWordLoop) + "% " && SubStr(Trim(StrLower(A_LoopField203)), 1, StrLen(StrLower(keyWordLoopParse))) != StrLower(keyWordLoopParse)) {
                out2 = StringTrimLeft(A_LoopField203, StrLen(keyWordLoop));
            ...
            else if (SubStr(Trim(StrLower(A_LoopField203)), 1, StrLen(StrLower(keyWordLoop)) + 2) == StrLower(keyWordLoop) + "% ") {
                howMany_fixAindexInGoUnused++;
                out2 = StringTrimLeft(A_LoopField203, StrLen(keyWordLoop) + 2);
                out2 = StrReplace(out2, ":", "");
                //MsgBox % out2
                //MsgBox, % out2
                out2 = Trim(out2);
            ...
            else if (SubStr(StrLower(A_LoopField203), 1, StrLen(StrLower(keyWordLoopParse))) == StrLower(keyWordLoopParse)) {
                howMany_fixAindexInGoUnused++;
                var1 = StringTrimLeft(A_LoopField203, StrLen(keyWordLoopParse));
                out2 = StrReplace(out2, ":", "");
                lineDone = 1;
            ...
            else if (SubStrLastChars(Trim(A_LoopField203), StrLen(keyWordInc)) == keyWordInc || SubStrLastChars(Trim(A_LoopField203), StrLen(keyWordInc + ";")) == keyWordInc + ";" && lineDone == 0) {
                if (useSemicolon == "on" || SubStrLastChars(Trim(A_LoopField203), 1) == ";") {
                    str1 = Trim(StringTrimRight(Trim(A_LoopField203), StrLen(keyWordInc + ";")));
                } else {
                    str1 = Trim(StringTrimRight(Trim(A_LoopField203), StrLen(keyWordInc)));
                }
                str1 = Trim(expressionParserTranspiler(str1));
            ...
            else if (SubStrLastChars(Trim(A_LoopField203), StrLen(keyWordDec)) == keyWordDec || SubStrLastChars(Trim(A_LoopField203), StrLen(keyWordDec + ";")) == keyWordDec + ";" && lineDone == 0) {
                if (useSemicolon == "on" || SubStrLastChars(Trim(A_LoopField203), 1) == ";") {
                    str1 = Trim(StringTrimRight(Trim(A_LoopField203), StrLen(keyWordDec + ";")));
                } else {
                    str1 = Trim(StringTrimRight(Trim(A_LoopField203), StrLen(keyWordDec)));
                }
                str1 = Trim(expressionParserTranspiler(str1));
            ...
            else if (SubStr(StrLower(Trim(A_LoopField203)), 1, StrLen(StrLower(keyWordInclude + " "))) == StrLower(keyWordInclude + " ") && langToConvertTo == langFileExtension_2) {
                htCode += keyWordInclude_2 + " " + Trim(StringTrimLeft(Trim(A_LoopField203), StrLen(StrLower(keyWordInclude) + " "))) + Chr(10);
            }
            else if (StrLower(A_LoopField203) == StrLower(keyWordGlobal) || StrLower(A_LoopField203) == StrLower(keyWordGlobal + ";")) {
                if (langToConvertTo == "ahk") {
                    htCode += "global\n";
                }
            }
            else if (SubStr(StrLower(A_LoopField203), 1, StrLen(StrLower(keyWordAwait))) == StrLower(keyWordAwait)) {
                str1 = StringTrimLeft(A_LoopField203, StrLen(keyWordAwait));
                //MsgBox, % A_LoopField
                lineDone = 1;
            ...
            else if (SubStr(StrLower(A_LoopField203), 1, StrLen(StrLower(keyWordGlobal))) == StrLower(keyWordGlobal)) {
                str1 = StringTrimLeft(A_LoopField203, StrLen(keyWordGlobal));
                //MsgBox, % A_LoopField
            ...
            else if (SubStr(StrLower(A_LoopField203), 1, StrLen(keyWordFunc)) == StrLower(keyWordFunc) || SubStr(StrLower(A_LoopField203), 1, StrLen(StrLower(keyWordAsync) + StrLower(keyWordFunc))) == StrLower(keyWordAsync) + StrLower(keyWordFunc) && InStr(StrLower(A_LoopField203), " main(") == false && lineDone == 0) {
                lineDone = 1;
                if (SubStr(StrLower(A_LoopField203), 1, StrLen(StrLower(keyWordAsync) + StrLower(keyWordFunc))) == StrLower(keyWordAsync) + StrLower(keyWordFunc)) {
                    // is async
                    int1 = 1;
                    str1 = Trim(StringTrimLeft(A_LoopField203, StrLen(StrLower(keyWordAsync + keyWordFunc))));
                } else {
                    // is async
                    int1 = 0;
                    str1 = Trim(StringTrimLeft(A_LoopField203, StrLen(StrLower(keyWordFunc))));
                }
                if (langToConvertTo != langFileExtension_2 && langToConvertTo != "cpp" && langToConvertTo != "go" && langToConvertTo != "cs" && langToConvertTo != "java" && langToConvertTo != "kt" && langToConvertTo != "nim" && langToConvertTo != "swift" && langToConvertTo != "dart" && langToConvertTo != "ts") {
                    if (str1 != "") {
                        str4 = "";
                        std::vector<std::string> items205 = LoopParseFunc(str1, ",");
                        for (size_t A_Index205 = 0; A_Index205 < items205.size() + 0; A_Index205++) {
                            std::string A_LoopField205 = items205[A_Index205 - 0];
                            if (InStr(A_LoopField205, " := ")) {
                                if (InStr(Trim(StrSplit(A_LoopField205, " := ", 1)), " ") != true) {
                                    str4 += "optanal-HTVM-opt-parmsNOtWorking-theirewIs-a-bug-Iusfbudfbuoeshfuisbav=-fwaegs-awedsf-dd-sfgc " + A_LoopField205 + ", ";
                                } else {
                                    str4 += A_LoopField205 + ", ";
                                }
                            }
                            else if (InStr(A_LoopField205, " = ")) {
                                if (InStr(Trim(StrSplit(A_LoopField205, " = ", 1)), " ") != true) {
                                    str4 += "optanal-HTVM-opt-parmsNOtWorking-theirewIs-a-bug-Iusfbudfbuoeshfuisbav=-fwaegs-awedsf-dd-sfgc " + A_LoopField205 + ", ";
                                } else {
                                    str4 += A_LoopField205 + ", ";
                            ...
                            else if (InStr(A_LoopField205, " " + Trim(keyWordAssign) + " ")) {
                                if (InStr(Trim(StrSplit(A_LoopField205, " " + Trim(keyWordAssign) + " ", 1)), " ") != true) {
                                    str4 += "optanal-HTVM-opt-parmsNOtWorking-theirewIs-a-bug-Iusfbudfbuoeshfuisbav=-fwaegs-awedsf-dd-sfgc " + A_LoopField205 + ", ";
                                } else {
                                    str4 += A_LoopField205 + ", ";
                                }
                ...
                if (Trim(str1) == "") {
                    ...
                    str19 = "";
                    str20 = "";
                    std::vector<std::string> items206 = LoopParseFunc(str1, ",");
                    for (size_t A_Index206 = 0; A_Index206 < items206.size() + 0; A_Index206++) {
                        ...
                        if (InStr(Trim(A_LoopField206), " ")) {
                            // there is space aka there is a type
                            if (InStr(A_LoopField206, " " + Trim(keyWordAssign) + " ") == false) {
                                if (usePrefixTypeForTypeDefinition == "on") {
                                    // normal
                                    str6 = Trim(A_LoopField206);
                                ...
                                // optanal param
                                if (InStr(Trim(StrSplit(Trim(A_LoopField206), " " + Trim(keyWordAssign) + " ", 1)), " ")) {
                                    // there is a type
                                    if (usePrefixTypeForTypeDefinition == "on") {
                                        // normal
                                        str6 = Trim(StrSplit(Trim(A_LoopField206), " " + Trim(keyWordAssign) + " ", 1));
                                        str9 = Trim(expressionParserTranspiler(Trim(StrSplit(Trim(A_LoopField206), " " + Trim(keyWordAssign) + " ", 2))));
                                        str7 = getTheLastWord(str6);
                                        str8 = getFuncTypeConvert(Trim(StringTrimRight(str6, StrLen(Trim(str7)) + 1)));
                                        //;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;
                                        //;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;
                                    ...
                                    // there is no type
                                    str8 = Trim(StrSplit(Trim(A_LoopField206), " " + Trim(keyWordAssign) + " ", 1));
                                    str9 = Trim(expressionParserTranspiler(Trim(StrSplit(Trim(A_LoopField206), " " + Trim(keyWordAssign) + " ", 2))));
                                    // str10
                                    // skipLeftCuleyForFuncPLS := 1
                                    //;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;
                            ...
                            str5 += Trim(A_LoopField206) + Chr(10);
                        }
                    }
                    //print(str5)
                    //print(str10)
                    ...
                    if (Trim(str5) != "" && Trim(str10) != "") {
                        ...
                        if (langToConvertTo == "lua") {
                            std::vector<std::string> items209 = LoopParseFunc(str12, ",");
                            for (size_t A_Index209 = 0; A_Index209 < items209.size() + 0; A_Index209++) {
                                std::string A_LoopField209 = items209[A_Index209 - 0];
                                str14 += Trim(StrSplit(Trim(A_LoopField209), " = ", 1)) + ", ";
                            }
                            str14 = StringTrimRight(str14, 2);
                            str13 += "{" + Chr(10);
                            std::vector<std::string> items210 = LoopParseFunc(str12, ",");
                            for (size_t A_Index210 = 0; A_Index210 < items210.size() + 0; A_Index210++) {
                                std::string A_LoopField210 = items210[A_Index210 - 0];
                                str13 += Trim(StrSplit(Trim(A_LoopField210), " = ", 1)) + " = " + Trim(StrSplit(Trim(A_LoopField210), " = ", 1)) + " or " + Trim(StrSplit(Trim(A_LoopField210), " = ", 2)) + Chr(10);
                            }
                            str12 = str14;
                        }
                        if (langToConvertTo == "go") {
                            ...
                            str13 += "{" + Chr(10) + " ";
                            std::vector<std::string> items211 = LoopParseFunc(str12, ",");
                            for (size_t A_Index211 = 0; A_Index211 < items211.size() + 0; A_Index211++) {
                                std::string A_LoopField211 = items211[A_Index211 - 0];
                                str16 = Trim(StrSplit(Trim(StrSplit(Trim(A_LoopField211), " = ", 1)), " ", 1));
                                str17 = Trim(StrSplit(Trim(A_LoopField211), " = ", 2));
                                str13 += str16 + " := " + str17 + Chr(10);
                            }
                            std::vector<std::string> items212 = LoopParseFunc(str12, ",");
                            for (size_t A_Index212 = 0; A_Index212 < items212.size() + 0; A_Index212++) {
                                std::string A_LoopField212 = items212[A_Index212 - 0];
                                str16 = Trim(StrSplit(Trim(StrSplit(Trim(A_LoopField212), " = ", 1)), " ", 1));
                                str18 = Trim(StrSplit(Trim(StrSplit(Trim(A_LoopField212), " = ", 1)), " ", 2));
                                // str16 = var name
                                // str18 = var type
                                str13 += "if len(__HTVM_V2_TO_GO_optionalParams__) " + Chr(62) + " " + STR(A_Index212) + Chr(10) + "{" + Chr(10) + str16 + " = __HTVM_V2_TO_GO_optionalParams__[" + STR(A_Index212) + "].(" + str18 + ")" + Chr(10) + "}" + Chr(10);
                            }
                            str12 = str14;
                        }
                        if (langToConvertTo == "java") {
                            ...
                            std::vector<std::string> items215 = LoopParseFunc(str12, ",");
                            for (size_t A_Index215 = 0; A_Index215 < items215.size() + 0; A_Index215++) {
                                std::string A_LoopField215 = items215[A_Index215 - 0];
                                str16 = Trim(StrSplit(Trim(StrSplit(Trim(A_LoopField215), " = ", 1)), " ", 2));
                                str17 = Trim(StrSplit(Trim(StrSplit(Trim(A_LoopField215), " = ", 1)), " ", 1));
                                // str16 = var name
                                // str17 = type name
                                str13 += "if (__HTVM_V2_TO_JAVA_optionalParams__.length " + Chr(62) + " " + STR(A_Index215) + ") " + str16 + " = (" + str17 + ") __HTVM_V2_TO_JAVA_optionalParams__[" + STR(A_Index215) + "];" + Chr(10);
                            }
                            str12 = str14;
                        }
                        str11 += str12;
                    ...
                    if (Trim(str5) == "" && Trim(str10) != "") {
                        ...
                        if (langToConvertTo == "lua") {
                            std::vector<std::string> items218 = LoopParseFunc(str11, ",");
                            for (size_t A_Index218 = 0; A_Index218 < items218.size() + 0; A_Index218++) {
                                std::string A_LoopField218 = items218[A_Index218 - 0];
                                str14 += Trim(StrSplit(Trim(A_LoopField218), " = ", 1)) + ", ";
                            }
                            str14 = StringTrimRight(str14, 2);
                            str13 += "{" + Chr(10);
                            std::vector<std::string> items219 = LoopParseFunc(str11, ",");
                            for (size_t A_Index219 = 0; A_Index219 < items219.size() + 0; A_Index219++) {
                                std::string A_LoopField219 = items219[A_Index219 - 0];
                                str13 += Trim(StrSplit(Trim(A_LoopField219), " = ", 1)) + " = " + Trim(StrSplit(Trim(A_LoopField219), " = ", 1)) + " or " + Trim(StrSplit(Trim(A_LoopField219), " = ", 2)) + Chr(10);
                            }
                            str11 = str14;
                        }
                        if (langToConvertTo == "go") {
                            str14 = "__HTVM_V2_TO_GO_optionalParams__ ...interface{}";
                            str13 += "{" + Chr(10) + " ";
                            std::vector<std::string> items220 = LoopParseFunc(str11, ",");
                            for (size_t A_Index220 = 0; A_Index220 < items220.size() + 0; A_Index220++) {
                                std::string A_LoopField220 = items220[A_Index220 - 0];
                                str16 = Trim(StrSplit(Trim(StrSplit(Trim(A_LoopField220), " = ", 1)), " ", 1));
                                str17 = Trim(StrSplit(Trim(A_LoopField220), " = ", 2));
                                str13 += str16 + " := " + str17 + Chr(10);
                            }
                            std::vector<std::string> items221 = LoopParseFunc(str11, ",");
                            for (size_t A_Index221 = 0; A_Index221 < items221.size() + 0; A_Index221++) {
                                std::string A_LoopField221 = items221[A_Index221 - 0];
                                str16 = Trim(StrSplit(Trim(StrSplit(Trim(A_LoopField221), " = ", 1)), " ", 1));
                                str18 = Trim(StrSplit(Trim(StrSplit(Trim(A_LoopField221), " = ", 1)), " ", 2));
                                // str16 = var name
                                // str18 = var type
                                str13 += "if len(__HTVM_V2_TO_GO_optionalParams__) " + Chr(62) + " " + STR(A_Index221) + Chr(10) + "{" + Chr(10) + str16 + " = __HTVM_V2_TO_GO_optionalParams__[" + STR(A_Index221) + "].(" + str18 + ")" + Chr(10) + "}" + Chr(10);
                            }
                            str11 = str14;
                        }
                        if (langToConvertTo == "java") {
                            ...
                            std::vector<std::string> items224 = LoopParseFunc(str11, ",");
                            for (size_t A_Index224 = 0; A_Index224 < items224.size() + 0; A_Index224++) {
                                std::string A_LoopField224 = items224[A_Index224 - 0];
                                str16 = Trim(StrSplit(Trim(StrSplit(Trim(A_LoopField224), " = ", 1)), " ", 2));
                                str17 = Trim(StrSplit(Trim(StrSplit(Trim(A_LoopField224), " = ", 1)), " ", 1));
                                // str16 = var name
                                // str17 = type name
                                str13 += "if (__HTVM_V2_TO_JAVA_optionalParams__.length " + Chr(62) + " " + STR(A_Index224) + ") " + str16 + " = (" + str17 + ") __HTVM_V2_TO_JAVA_optionalParams__[" + STR(A_Index224) + "];" + Chr(10);
                            }
                            str11 = str14;
                        }
                    }
            ...
            else if (InStr(StrLower(A_LoopField203), " main(") && lineDone == 0) {
                lineDone = 1;
                javaMainFuncSeen = 1;
                csMainFuncSeen = 1;
                htCode += A_LoopField203 + Chr(10);
            }
            else if (InStr(A_LoopField203, " " + keyWordAssign + " ") || InStr(A_LoopField203, " " + keyWordAssignAdd + " ") || InStr(A_LoopField203, " " + keyWordAssignConcat + " ") || InStr(A_LoopField203, " " + keyWordAssignSub + " ") || InStr(A_LoopField203, " " + keyWordAssignMul + " ") || InStr(A_LoopField203, " " + keyWordAssignDiv + " ") || InStr(A_LoopField203, " " + keyWordAssignMod + " ") || InStr(A_LoopField203, " " + keyWordAssignShiftLeft + " ") || InStr(A_LoopField203, " " + keyWordAssignShiftRight + " ") || InStr(A_LoopField203, " " + keyWordLogicalAssignShiftRight + " ") || InStr(A_LoopField203, " " + keyWordAssignBitAnd + " ") || InStr(A_LoopField203, " " + keyWordAssignBitOr + " ") || InStr(A_LoopField203, " " + keyWordAssignBitXor + " ") && lineDone == 0) {
                allVarsSoWeDontReDecVars_FIX_TOGGLE = 1;
                //keyWordAssign
                //keyWordAssignAdd
                //keyWordAssignConcat
                ...
                //allVarsSoWeDontReDecVars_FIX_int64
                //allVarsSoWeDontReDecVars_FIX_float
                if (InStr(A_LoopField203, " " + keyWordAssign + " ")) {
                    str1 = StrSplit(A_LoopField203, " " + keyWordAssign + " ", 1);
                    str6 = StrSplit(A_LoopField203, " " + keyWordAssign + " ", 2);
                    str10 = keyWordAssign;
                    str11 = keyWordAssign_2;
                    if (langToConvertTo == "ahk") {
                        str12 = ":=";
                ...
                else if (InStr(A_LoopField203, " " + keyWordAssignAdd + " ")) {
                    str1 = StrSplit(A_LoopField203, " " + keyWordAssignAdd + " ", 1);
                    str6 = StrSplit(A_LoopField203, " " + keyWordAssignAdd + " ", 2);
                    str10 = keyWordAssignAdd;
                    str11 = keyWordAssignAdd_2;
                    str12 = "+=";
                }
                else if (InStr(A_LoopField203, " " + keyWordAssignConcat + " ")) {
                    str1 = StrSplit(A_LoopField203, " " + keyWordAssignConcat + " ", 1);
                    str6 = StrSplit(A_LoopField203, " " + keyWordAssignConcat + " ", 2);
                    str10 = keyWordAssignConcat;
                    str11 = keyWordAssignConcat_2;
                    if (langToConvertTo == "ahk") {
                        str12 = ".=";
                ...
                else if (InStr(A_LoopField203, " " + keyWordAssignSub + " ")) {
                    str1 = StrSplit(A_LoopField203, " " + keyWordAssignSub + " ", 1);
                    str6 = StrSplit(A_LoopField203, " " + keyWordAssignSub + " ", 2);
                    str10 = keyWordAssignSub;
                    str11 = keyWordAssignSub_2;
                    str12 = "-=";
                }
                else if (InStr(A_LoopField203, " " + keyWordAssignMul + " ")) {
                    str1 = StrSplit(A_LoopField203, " " + keyWordAssignMul + " ", 1);
                    str6 = StrSplit(A_LoopField203, " " + keyWordAssignMul + " ", 2);
                    str10 = keyWordAssignMul;
                    str11 = keyWordAssignMul_2;
                    str12 = "*=";
                }
                else if (InStr(A_LoopField203, " " + keyWordAssignDiv + " ")) {
                    str1 = StrSplit(A_LoopField203, " " + keyWordAssignDiv + " ", 1);
                    str6 = StrSplit(A_LoopField203, " " + keyWordAssignDiv + " ", 2);
                    str10 = keyWordAssignDiv;
                    str11 = keyWordAssignDiv_2;
                    str12 = "/=";
                }
                else if (InStr(A_LoopField203, " " + keyWordAssignMod + " ")) {
                    str1 = StrSplit(A_LoopField203, " " + keyWordAssignMod + " ", 1);
                    str6 = StrSplit(A_LoopField203, " " + keyWordAssignMod + " ", 2);
                    str10 = keyWordAssignMod;
                    str11 = keyWordAssignMod_2;
                    str12 = "%=";
                }
                else if (InStr(A_LoopField203, " " + keyWordAssignShiftLeft + " ")) {
                    str1 = StrSplit(A_LoopField203, " " + keyWordAssignShiftLeft + " ", 1);
                    str6 = StrSplit(A_LoopField203, " " + keyWordAssignShiftLeft + " ", 2);
                    str10 = keyWordAssignShiftLeft;
                    str11 = keyWordAssignShiftLeft_2;
                    str12 = "<<=";
                }
                else if (InStr(A_LoopField203, " " + keyWordAssignShiftRight + " ")) {
                    str1 = StrSplit(A_LoopField203, " " + keyWordAssignShiftRight + " ", 1);
                    str6 = StrSplit(A_LoopField203, " " + keyWordAssignShiftRight + " ", 2);
                    str10 = keyWordAssignShiftRight;
                    str11 = keyWordAssignShiftRight_2;
                    str12 = ">>=";
                }
                else if (InStr(A_LoopField203, " " + keyWordLogicalAssignShiftRight + " ")) {
                    str1 = StrSplit(A_LoopField203, " " + keyWordLogicalAssignShiftRight + " ", 1);
                    str6 = StrSplit(A_LoopField203, " " + keyWordLogicalAssignShiftRight + " ", 2);
                    str10 = keyWordLogicalAssignShiftRight;
                    str11 = keyWordLogicalAssignShiftRight_2;
                    str12 = ">>>=";
                }
                else if (InStr(A_LoopField203, " " + keyWordAssignBitAnd + " ")) {
                    str1 = StrSplit(A_LoopField203, " " + keyWordAssignBitAnd + " ", 1);
                    str6 = StrSplit(A_LoopField203, " " + keyWordAssignBitAnd + " ", 2);
                    str10 = keyWordAssignBitAnd;
                    str11 = keyWordAssignBitAnd_2;
                    str12 = "&=";
                }
                else if (InStr(A_LoopField203, " " + keyWordAssignBitOr + " ")) {
                    str1 = StrSplit(A_LoopField203, " " + keyWordAssignBitOr + " ", 1);
                    str6 = StrSplit(A_LoopField203, " " + keyWordAssignBitOr + " ", 2);
                    str10 = keyWordAssignBitOr;
                    str11 = keyWordAssignBitOr_2;
                    str12 = "|=";
                }
                else if (InStr(A_LoopField203, " " + keyWordAssignBitXor + " ")) {
                    str1 = StrSplit(A_LoopField203, " " + keyWordAssignBitXor + " ", 1);
                    str6 = StrSplit(A_LoopField203, " " + keyWordAssignBitXor + " ", 2);
                    str10 = keyWordAssignBitXor;
                    str11 = keyWordAssignBitXor_2;
                    str12 = "^=";
                }
                ...
                if (InStr(ALoopField, " ") == false || SubStrLastChars(ALoopField, 1) == "]") {
                    ...
                    if (usePostfixTypeForTypeDefinition == "off") {
                        lineDone = 1;
                        str1 = Trim(StringTrimLeft(Trim(str1), StrLen(keyWordConst)));
                        std::vector<std::string> items225 = LoopParseFunc(str1, " ");
                    ...
                    if (usePostfixTypeForTypeDefinition == "off") {
                        str1 = Trim(StringTrimLeft(Trim(str1), StrLen(keyWordVar)));
                        std::vector<std::string> items226 = LoopParseFunc(str1, " ");
                    ...
                    if (usePostfixTypeForTypeDefinition == "off") {
                        str1 = Trim(StringTrimLeft(Trim(str1), StrLen(keyWordLet)));
                        std::vector<std::string> items227 = LoopParseFunc(str1, " ");
                    ...
                    lineDone = 1;
                    std::vector<std::string> items228 = LoopParseFunc(str1, " ");
            ...
            else if (SubStr(Trim(A_LoopField203), 1, StrLen(keyWordVar)) == keyWordVar && lineDone == 0) {
                allVarsSoWeDontReDecVars_FIX_TOGGLE = 1;
                if (usePostfixTypeForTypeDefinition == "off") {
                    lineDone = 1;
                    str1 = Trim(StringTrimLeft(Trim(A_LoopField203), StrLen(keyWordVar)));
                    if (SubStrLastChars(str1, 1) == ";") {
                        str1 = StringTrimRight(str1, 1);
                    }
                    std::vector<std::string> items229 = LoopParseFunc(str1, " ");
            ...
            else if (SubStr(Trim(A_LoopField203), 1, StrLen(keyWordLet)) == keyWordLet && lineDone == 0) {
                allVarsSoWeDontReDecVars_FIX_TOGGLE = 1;
                if (usePostfixTypeForTypeDefinition == "off") {
                    lineDone = 1;
                    str1 = Trim(StringTrimLeft(Trim(A_LoopField203), StrLen(keyWordLet)));
                    if (SubStrLastChars(str1, 1) == ";") {
                        str1 = StringTrimRight(str1, 1);
                    }
                    std::vector<std::string> items230 = LoopParseFunc(str1, " ");
            ...
            else if (SubStr(Trim(A_LoopField203), 1, StrLen(keyWordDouble + " ")) == keyWordDouble + " " || SubStr(Trim(A_LoopField203), 1, StrLen(keyWordChar + " ")) == keyWordChar + " " || SubStr(Trim(A_LoopField203), 1, StrLen(keyWordUint8 + " ")) == keyWordUint8 + " " || SubStr(Trim(A_LoopField203), 1, StrLen(keyWordUint16 + " ")) == keyWordUint16 + " " || SubStr(Trim(A_LoopField203), 1, StrLen(keyWordUint32 + " ")) == keyWordUint32 + " " || SubStr(Trim(A_LoopField203), 1, StrLen(keyWordUint64 + " ")) == keyWordUint64 + " " || SubStr(Trim(A_LoopField203), 1, StrLen(keyWordINT + " ")) == keyWordINT + " " || SubStr(Trim(A_LoopField203), 1, StrLen(keyWordSTR + " ")) == keyWordSTR + " " || SubStr(Trim(A_LoopField203), 1, StrLen(keyWordBOOL + " ")) == keyWordBOOL + " " || SubStr(Trim(A_LoopField203), 1, StrLen(keyWordFLOAT + " ")) == keyWordFLOAT + " " || SubStr(Trim(A_LoopField203), 1, StrLen(keyWordINT8 + " ")) == keyWordINT8 + " " || SubStr(Trim(A_LoopField203), 1, StrLen(keyWordINT16 + " ")) == keyWordINT16 + " " || SubStr(Trim(A_LoopField203), 1, StrLen(keyWordINT32 + " ")) == keyWordINT32 + " " || SubStr(Trim(A_LoopField203), 1, StrLen(keyWordINT64 + " ")) == keyWordINT64 + " " || SubStr(Trim(A_LoopField203), 1, StrLen(keyWordArrayDefinition + " ")) == keyWordArrayDefinition + " " || SubStr(Trim(A_LoopField203), 1, StrLen(keyWordArrayOfIntegersDefinition + " ")) == keyWordArrayOfIntegersDefinition + " " || SubStr(Trim(A_LoopField203), 1, StrLen(keyWordArrayOfStringsDefinition + " ")) == keyWordArrayOfStringsDefinition + " " || SubStr(Trim(A_LoopField203), 1, StrLen(keyWordArrayOfFloatingPointNumbersDefinition + " ")) == keyWordArrayOfFloatingPointNumbersDefinition + " " || SubStr(Trim(A_LoopField203), 1, StrLen(keyWordArrayOfBooleansDefinition + " ")) == keyWordArrayOfBooleansDefinition + " " && usePrefixTypeForTypeDefinition == "on" && lineDone == 0) {
                lineDone = 1;
                allVarsSoWeDontReDecVars_FIX_TOGGLE = 1;
                str1 = Trim(A_LoopField203);
                if (SubStrLastChars(str1, 1) == ";") {
                ...
                std::vector<std::string> items231 = LoopParseFunc(str1, " ");
            ...
            else if (RegExMatch(Trim(A_LoopField203), "^[a-zA-Z_][a-zA-Z0-9_]*: [^:]*;?$") && usePostfixTypeForTypeDefinition == "on" && lineDone == 0) {
                lineDone = 1;
                allVarsSoWeDontReDecVars_FIX_TOGGLE = 1;
                str1 = Trim(A_LoopField203);
                if (SubStrLastChars(str1, 1) == ";") {
            ...
            else if (RegExMatch(Trim(A_LoopField203), "^[a-zA-Z0-9_\\.]+\\(") && !(InStr(StrLower(A_LoopField203), " main(")) && lineDone == 0) {
                lineDone = 1;
                str1 = Trim(A_LoopField203);
                if (SubStrLastChars(str1, 1) == ";") {
                    str1 = StringTrimRight(str1, 1);
            ...
            else if (RegExMatch(A_LoopField203, "\\.[a-zA-Z_][a-zA-Z0-9_]*") && lineDone == 0) {
                lineDone = 1;
                str1 = Trim(A_LoopField203);
                if (SubStrLastChars(str1, 1) == ";") {
                    str1 = StringTrimRight(str1, 1);
            ...
            else if (KeyWordsCommands(A_LoopField203, "check", commands, langToConvertTo) == "true" && lineDone == 0) {
                lineDone = 1;
                if (langToConvertTo == langFileExtension_2) {
                    htCode += A_LoopField203 + Chr(10);
                } else {
                    out_KeyWordsCommands = KeyWordsCommands(A_LoopField203, "transpile", commands, langToConvertTo);
                ...
                if (lineDone != 1) {
                    if (skipLeftCuleyForFuncPLS != 1) {
                        if (SubStr(Trim(StrLower(A_LoopField203)), 1, 1) == Chr(125)) {
                            htCode += Chr(125) + Chr(10);
                        } else {
                            if (htCodeAcurlyBraceAddSomeVrasFixLP == 1 && SubStr(Trim(StrLower(A_LoopField203)), 1, 1) == Chr(123)) {
                                htCodeAcurlyBraceAddSomeVrasFixLP = 0;
                                htCode += A_LoopField203 + Chr(10) + theFixTextLoopLP + Chr(10);
                            } else {
                                if (htCodeAcurlyBraceAddSomeVrasFixNL == 1 && SubStr(Trim(StrLower(A_LoopField203)), 1, 1) == Chr(123)) {
                                    htCodeAcurlyBraceAddSomeVrasFixNL = 0;
                                    htCode += A_LoopField203 + Chr(10) + theFixTextLoopNL + Chr(10);
                                } else {
                                    htCode += A_LoopField203 + Chr(10);
                                }
                            }
        ...
        if (langToConvertTo != "ahk" && langToConvertTo != langFileExtension_2) {
            ...
            if (haveWeEverUsedAloop == 1) {
                ...
                std::vector<std::string> items232 = LoopParseFunc(htCodeLoopfixa, "\n", "\r");
                for (size_t A_Index232 = 0; A_Index232 < items232.size() + 0; A_Index232++) {
                    std::string A_LoopField232 = items232[A_Index232 - 0];
                    sstr123 = A_LoopField232;
                    fixLoopLokingFor = A_LoopField232;
                    ...
                    if (out1 == "nl") {
                        ...
                        std::vector<std::string> items233 = LoopParseFunc(htCode, "\n", "\r");
                        for (size_t A_Index233 = 0; A_Index233 < items233.size() + 0; A_Index233++) {
                            ...
                            if (InStr(A_LoopField233, fixLoopLokingFor) && insdeAnestedLoopBAD != 1) {
                                fixLoopLokingForNum = 1;
                                //MsgBox, do we came here 1
                            }
                            if (SubStr(Trim(A_LoopField233), 1, 4) == "for " && weAreDoneHereCurly != 1 && insdeAnestedLoopBAD != 1 && fixLoopLokingForNum == 1) {
                                s = StrSplit(A_LoopField233 ,"" + keyWordAIndex + "", 2);
                                out1z = s;
                                s = StrSplit(out1z, " ", 1);
                                out1z = Trim(s);
                                //MsgBox, % out1z
                                ...
                                //MsgBox, % A_LoopField
                                dontSaveStr = 1;
                                ALoopField = A_LoopField233;
                                DeleayOneCuzOfLoopParse = 1;
                                out4758686d86d86d86578991a += ALoopField + Chr(10);
                            }
                            if (inTarget == 1 && InStr(A_LoopField233, Chr(123)) && insdeAnestedLoopBAD != 1) {
                                insideBracket = 1;
                            }
                            if (insideBracket == 1 && InStr(A_LoopField233, Chr(123)) && insdeAnestedLoopBAD != 1) {
                                netsedCurly++;
                            }
                            if (insideBracket == 1 && InStr(A_LoopField233, Chr(125)) && insdeAnestedLoopBAD != 1) {
                                netsedCurly--;
                                readyToEnd = 1;
                            }
                            if (SubStr(Trim(A_LoopField233), 1, 4) == "for " && insdeAnestedLoopBAD != 1 && foundTheTopLoop >= 2) {
                            ...
                            if (insdeAnestedLoopBAD == 1) {
                                if (InStr(A_LoopField233, Chr(123))) {
                                    insideBracket1 = 1;
                                }
                                if (insideBracket1 == 1 && InStr(A_LoopField233, Chr(123))) {
                                    netsedCurly1++;
                                }
                                if (insideBracket1 == 1 && InStr(A_LoopField233, Chr(125))) {
                                    netsedCurly1--;
                                    readyToEnd1 = 1;
                                }
                                if (InStr(A_LoopField233, Chr(125)) && readyToEnd1 == 1 && netsedCurly1 == 0 && insideBracket == 1) {
                                    //MsgBox, % A_LoopField
                                    eldLoopNestedBADlol = 1;
                                }
                                out4758686d86d86d86578991a += A_LoopField233 + Chr(10);
                            }
                            ...
                            if (inTarget == 1 && InStr(A_LoopField233, Chr(125)) && readyToEnd == 1 && netsedCurly == 0 && weAreDoneHereCurly == 0 && dontSaveStr != 1 && insdeAnestedLoopBAD != 1) {
                                //MsgBox, % A_LoopField
                        ...
                        foundTheTopLoop = 0;
                        out4758686d86d86d86578991a = "";
                        std::vector<std::string> items234 = LoopParseFunc(htCode, "\n", "\r");
                        for (size_t A_Index234 = 0; A_Index234 < items234.size() + 0; A_Index234++) {
                            std::string A_LoopField234 = items234[A_Index234 - 0];
                            if (InStr(A_LoopField234 , fixLoopLokingFor) && insdeAnestedLoopBAD != 1) {
                                fixLoopLokingForNum = 1;
                                //MsgBox, do we came here 3
                            }
                            if (SubStr(Trim(A_LoopField234), 1, 4) == "for " && weAreDoneHereCurly != 1 && insdeAnestedLoopBAD != 1 && fixLoopLokingForNum == 1) {
                                s = StrSplit(A_LoopField234 ,"" + keyWordAIndex + "", 2);
                                out1z = s;
                                s = StrSplit(out1z, " ", 1);
                                ...
                                //MsgBox, % A_LoopField
                                dontSaveStr = 1;
                                ALoopField = A_LoopField234;
                                DeleayOneCuzOfLoopParse = 1;
                                out4758686d86d86d86578991a += ALoopField + Chr(10);
                            }
                            if (inTarget == 1 && InStr(A_LoopField234, Chr(123)) && insdeAnestedLoopBAD != 1) {
                                insideBracket = 1;
                            }
                            if (insideBracket == 1 && InStr(A_LoopField234, Chr(123)) && insdeAnestedLoopBAD != 1) {
                                netsedCurly++;
                            }
                            if (insideBracket == 1 && InStr(A_LoopField234, Chr(125)) && insdeAnestedLoopBAD != 1) {
                                netsedCurly--;
                                readyToEnd = 1;
                            }
                            if (SubStr(Trim(A_LoopField234), 1, 4) == "for " && insdeAnestedLoopBAD != 1 && foundTheTopLoop >= 2) {
                                insdeAnestedLoopBAD = 1;
                                insideBracket1 = 0;
                                netsedCurly1 = 0;
                            }
                            ...
                            if (insdeAnestedLoopBAD == 1) {
                                if (InStr(A_LoopField234, Chr(123))) {
                                    insideBracket1 = 1;
                                }
                                if (insideBracket1 == 1 && InStr(A_LoopField234, Chr(123))) {
                                    netsedCurly1++;
                                }
                                if (insideBracket1 == 1 && InStr(A_LoopField234, Chr(125))) {
                                    netsedCurly1--;
                                    readyToEnd1 = 1;
                                }
                                if (InStr(A_LoopField234, Chr(125)) && readyToEnd1 == 1 && netsedCurly1 == 0 && insideBracket == 1) {
                                    //MsgBox, % A_LoopField
                                    eldLoopNestedBADlol = 1;
                                }
                                out4758686d86d86d86578991a += A_LoopField234 + Chr(10);
                            }
                            if (inTarget == 1 && dontSaveStr != 1 && fixLoopLokingForNum != 1 && insdeAnestedLoopBAD != 1) {
                                ALoopField = A_LoopField234;
                                // Replace "A_Index" with or without a following digit with "A_Index" + out1z
                            ...
                            if ((inTarget == 1 && InStr(A_LoopField234, Chr(125)) && readyToEnd == 1 && netsedCurly == 0 && weAreDoneHereCurly == 0 && dontSaveStr != 1 && insdeAnestedLoopBAD != 1)) {
                                //MsgBox, % A_LoopField
                                weAreDoneHereCurly = 1;
                                inTarget = 0;
                                endBracketDOntPutThere = 1;
                ...
                hold = 0;
                std::vector<std::string> items235 = LoopParseFunc(htCode, "\n", "\r");
                for (size_t A_Index235 = 0; A_Index235 < items235.size() + 0; A_Index235++) {
                    std::string A_LoopField235 = items235[A_Index235 - 0];
                    ignore = 0;
                    if (SubStr(Trim(A_LoopField235), 1, 4) == "for ") {
                        if (hold == 1 && holdText == A_LoopField235) {
                            ignore = 1;
                        } else {
                            holdText = A_LoopField235;
                            hold = 1;
                        }
                    }
                    if (!ignore) {
                        out4758686d86dgt8r754444444 += A_LoopField235 + Chr(10);
                    }
                }
            ...
            htCodeOut1234565432 = "";
            std::vector<std::string> items236 = LoopParseFunc(htCode, "\n", "\r");
    ...
    if (COUNT_programmingBlock_HTVMsyntax != 0) {
        ...
        for (int A_Index237 = 0; A_Index237 < COUNT_programmingBlock_HTVMsyntax + 0; A_Index237++) {
            ADD_ALL_programmingBlock_HTVMsyntax += programmingBlock_HTVMsyntax[A_Index237] + Chr(10);
        }
        ...
        isNotHTVMfile2 = 1;
        for (int A_Index238 = 0; A_Index238 < COUNT_programmingBlock_HTVMsyntax + 0; A_Index238++) {
            htCode = StrReplace(htCode, "programmingBlock_HTVMsyntax-programmingBlock_HTVMsyntax-AA" + STR(A_Index238 + 1) + "AA", compiler(programmingBlock_HTVMsyntax[A_Index238], allInstructionFile, "full", langToConvertToParam));
        }
        isNotHTVMfile2 = 0;
    }
    //;;;;;;;;;;;;;;;;;;;;;;;;;;;
    ...
    if (langToConvertTo == langFileExtension_2) {
        ...
        if (useEnd_2 == "on") {
            std::vector<std::string> items239 = LoopParseFunc(htCode, "\n", "\r");
            for (size_t A_Index239 = 0; A_Index239 < items239.size() + 0; A_Index239++) {
                std::string A_LoopField239 = items239[A_Index239 - 0];
                if (Trim(A_LoopField239) == keyWordCurlyBraceClose_2) {
                    fixLuaAndRuby += StrReplace(A_LoopField239, keyWordCurlyBraceClose_2, Trim(keyWordEnd_2)) + Chr(10);
                }
                else if (SubStrLastChars(A_LoopField239, 2) == " " + keyWordCurlyBraceOpen_2) {
                    fixLuaAndRuby += StringTrimRight(A_LoopField239, 2) + Chr(10);
                } else {
                    fixLuaAndRuby += A_LoopField239 + Chr(10);
                }
            }
        ...
        if (langToConvertTo == "lua" || langToConvertTo == "rb" || langToConvertTo == langFileExtension_2) {
            if (langToConvertTo == langFileExtension_2 && useEnd_2 == "on") {
                ...
                std::vector<std::string> items240 = LoopParseFunc(htCode, "\n", "\r");
                for (size_t A_Index240 = 0; A_Index240 < items240.size() + 0; A_Index240++) {
                    std::string A_LoopField240 = items240[A_Index240 - 0];
                    if (Trim(A_LoopField240) == "}") {
                        fixLuaAndRuby += StrReplace(A_LoopField240, "}", fixLuaAndRubyHELP) + Chr(10);
                    }
                    else if (SubStrLastChars(A_LoopField240, 2) == " {") {
                        fixLuaAndRuby += StringTrimRight(A_LoopField240, 2) + Chr(10);
                    } else {
                        fixLuaAndRuby += A_LoopField240 + Chr(10);
                    }
                }
                htCode = StringTrimRight(fixLuaAndRuby, 1);
            } else {
                fixLuaAndRubyHELP = "end";
                std::vector<std::string> items241 = LoopParseFunc(htCode, "\n", "\r");
                for (size_t A_Index241 = 0; A_Index241 < items241.size() + 0; A_Index241++) {
                    std::string A_LoopField241 = items241[A_Index241 - 0];
                    if (Trim(A_LoopField241) == "}") {
                        fixLuaAndRuby += StrReplace(A_LoopField241, "}", fixLuaAndRubyHELP) + Chr(10);
                    }
                    else if (SubStrLastChars(A_LoopField241, 2) == " {") {
                        fixLuaAndRuby += StringTrimRight(A_LoopField241, 2) + Chr(10);
                    } else {
                        fixLuaAndRuby += A_LoopField241 + Chr(10);
                    }
                }
                htCode = StringTrimRight(fixLuaAndRuby, 1);
            }
    ...
    if (langToConvertTo == langFileExtension_2 && useEnd_2 == "on") {
        std::vector<std::string> items242 = LoopParseFunc(htCode, "\n", "\r");
        for (size_t A_Index242 = 0; A_Index242 < items242.size() + 0; A_Index242++) {
            std::string A_LoopField242 = items242[A_Index242 - 0];
            HTVM_Append(nextWordEndFix, A_LoopField242);
        }
        HTVM_Append(nextWordEndFix, " ");
        std::vector<std::string> items243 = LoopParseFunc(htCode, "\n", "\r");
        for (size_t A_Index243 = 0; A_Index243 < items243.size() + 0; A_Index243++) {
            std::string A_LoopField243 = items243[A_Index243 - 0];
            if (Trim(A_LoopField243) == Trim(keyWordEnd_2) && SubStr(Trim(nextWordEndFix[A_Index243 + 1]), 1, StrLen(Trim(keyWordElseIf_2) + " ")) == Trim(keyWordElseIf_2) + " " || Trim(A_LoopField243) == Trim(keyWordEnd_2) && (Trim(nextWordEndFix[A_Index243 + 1]) == Trim(keyWordElse_2) || Trim(nextWordEndFix[A_Index243 + 1]) == Trim(keyWordElse_2 + ":"))) {
                nextWordEndFixOut += "";
            } else {
                nextWordEndFixOut += A_LoopField243 + Chr(10);
            }
        }
        htCode = StringTrimRight(nextWordEndFixOut, 1);
    }
    if (langToConvertTo == "lua") {
        std::vector<std::string> items244 = LoopParseFunc(htCode, "\n", "\r");
        for (size_t A_Index244 = 0; A_Index244 < items244.size() + 0; A_Index244++) {
            std::string A_LoopField244 = items244[A_Index244 - 0];
            HTVM_Append(nextWordEndFix, A_LoopField244);
        }
        HTVM_Append(nextWordEndFix, " ");
        std::vector<std::string> items245 = LoopParseFunc(htCode, "\n", "\r");
        for (size_t A_Index245 = 0; A_Index245 < items245.size() + 0; A_Index245++) {
            std::string A_LoopField245 = items245[A_Index245 - 0];
            if (Trim(A_LoopField245) == "end" && SubStr(Trim(nextWordEndFix[A_Index245 + 1]), 1, StrLen("elseif ")) == "elseif " || Trim(A_LoopField245) == "end" && Trim(nextWordEndFix[A_Index245 + 1]) == "else") {
                nextWordEndFixOut += "";
            } else {
                nextWordEndFixOut += A_LoopField245 + Chr(10);
            }
        }
        htCode = StringTrimRight(nextWordEndFixOut, 1);
    }
    if (langToConvertTo == "rb") {
        std::vector<std::string> items246 = LoopParseFunc(htCode, "\n", "\r");
        for (size_t A_Index246 = 0; A_Index246 < items246.size() + 0; A_Index246++) {
            std::string A_LoopField246 = items246[A_Index246 - 0];
            HTVM_Append(nextWordEndFix, A_LoopField246);
        }
        HTVM_Append(nextWordEndFix, " ");
        std::vector<std::string> items247 = LoopParseFunc(htCode, "\n", "\r");
        for (size_t A_Index247 = 0; A_Index247 < items247.size() + 0; A_Index247++) {
            std::string A_LoopField247 = items247[A_Index247 - 0];
            if (Trim(A_LoopField247) == "end" && SubStr(Trim(nextWordEndFix[A_Index247 + 1]), 1, StrLen("elsif ")) == "elsif " || Trim(A_LoopField247) == "end" && Trim(nextWordEndFix[A_Index247 + 1]) == "else") {
                nextWordEndFixOut += "";
            } else {
                nextWordEndFixOut += A_LoopField247 + Chr(10);
            }
        }
        htCode = StringTrimRight(nextWordEndFixOut, 1);
    }
    ...
    nextWordEndFixOut = "";
    if (langToConvertTo == "rb") {
        std::vector<std::string> items248 = LoopParseFunc(htCode, "\n", "\r");
        for (size_t A_Index248 = 0; A_Index248 < items248.size() + 0; A_Index248++) {
            std::string A_LoopField248 = items248[A_Index248 - 0];
            HTVM_Append(nextWordEndFixRB, A_LoopField248);
        }
        HTVM_Append(nextWordEndFixRB, " ");
        std::vector<std::string> items249 = LoopParseFunc(htCode, "\n", "\r");
        for (size_t A_Index249 = 0; A_Index249 < items249.size() + 0; A_Index249++) {
            std::string A_LoopField249 = items249[A_Index249 - 0];
            if (Trim(A_LoopField249) == "end" && SubStr(Trim(nextWordEndFixRB[A_Index249 + 1]), 1, StrLen("rescue ")) == "rescue " || Trim(A_LoopField249) == "end" && Trim(nextWordEndFixRB[A_Index249 + 1]) == "ensure") {
                nextWordEndFixOut += "";
            } else {
    ...
    if (langToConvertTo == langFileExtension_2 && useEnd_2 == "on") {
        ...
        for (size_t A_Index250 = 0; A_Index250 < items250.size() + 0; A_Index250++) {
            ...
            HTVM_Append(nextWordEndFixRB, A_LoopField250);
        }
        HTVM_Append(nextWordEndFixRB, " ");
        ...
        for (size_t A_Index251 = 0; A_Index251 < items251.size() + 0; A_Index251++) {
            ...
            if (Trim(A_LoopField251) == Trim(keyWordEnd_2) && SubStr(Trim(nextWordEndFixRB[A_Index251 + 1]), 1, StrLen(Trim(keyWordCatch_2) + " ")) == Trim(keyWordCatch_2) + " " || Trim(A_LoopField251) == Trim(keyWordEnd_2) && Trim(nextWordEndFixRB[A_Index251 + 1]) == Trim(keyWordFinally_2) || Trim(nextWordEndFixRB[A_Index251 + 1]) == Trim(keyWordFinally_2 + ":")) {
                nextWordEndFixOut += "";
            } else {
                nextWordEndFixOut += A_LoopField251 + Chr(10);
            }
        }
        htCode = StringTrimRight(nextWordEndFixOut, 1);
    }
    ...
    if (langToConvertTo == "ahk") {
        ...
        for (size_t A_Index252 = 0; A_Index252 < items252.size() + 0; A_Index252++) {
            ...
            if (SubStr(Trim(A_LoopField252), 1, StrLen("Loop, Parse, ")) == "Loop, Parse, ") {
                nextWordEndFixOut += StrReplace(A_LoopField252, "{", Chr(10) + "{" + Chr(10)) + Chr(10);
            } else {
                nextWordEndFixOut += A_LoopField252 + Chr(10);
            }
        }
        htCode = StringTrimRight(nextWordEndFixOut, 1);
    }
    if (langToConvertTo == "go") {
        ...
        for (size_t A_Index253 = 0; A_Index253 < items253.size() + 0; A_Index253++) {
            ...
            HTVM_Append(fixGoManGoIsSoAnnoyingBroooFurure, A_LoopField253);
        }
        ...
        std::vector<std::string> items254 = LoopParseFunc(htCode, "\n", "\r");
        for (size_t A_Index254 = 0; A_Index254 < items254.size() + 0; A_Index254++) {
            std::string A_LoopField254 = items254[A_Index254 - 0];
            if (Trim(A_LoopField254) == "}" && SubStr(Trim(fixGoManGoIsSoAnnoyingBroooFurure[A_Index254 + 1]), 1, StrLen("else if ")) == "else if ") {
                fixGoManGoIsSoAnnoyingBroooSkip = 1;
                fixGoManGoIsSoAnnoyingBroooCount = 0;
                std::vector<std::string> items255 = LoopParseFunc(fixGoManGoIsSoAnnoyingBroooFurure[A_Index254 + 1]);
                for (size_t A_Index255 = 0; A_Index255 < items255.size() + 0; A_Index255++) {
                ...
                for (int A_Index256 = 0; A_Index256 < fixGoManGoIsSoAnnoyingBroooCount + 0; A_Index256++) {
                    fixGoManGoIsSoAnnoyingBroooCountSpaceses += " ";
                }
                fixGoManGoIsSoAnnoyingBrooo += fixGoManGoIsSoAnnoyingBroooCountSpaceses + "} " + Trim(fixGoManGoIsSoAnnoyingBroooFurure[A_Index254 + 1]) + Chr(10);
            } else {
                if (fixGoManGoIsSoAnnoyingBroooSkip == 0) {
                    fixGoManGoIsSoAnnoyingBrooo += A_LoopField254 + Chr(10);
                }
    ...
    for (int A_Index257 = 0; A_Index257 < HTVM_Size(commentsBugFix) + 0; A_Index257++) {
        htCode = StrReplace(htCode, "HTVM--cnavisdofbuvsesdivdidufg79wregwewaeosd8ges7dfdftuawdiHTVMv2yerheyziberlasduci6syiq--AA" + STR(A_Index257) + "AA", commentsBugFix[A_Index257]);
    }
    //;;;;;;;;;;;;;;;;;;;;;;;;; fix here
    //;;;;;;;;;;;;;;;;;;;;;;;;; fix here
    //;;;;;;;;;;;;;;;;;;;;;;;;; fix here
    if (COUNT_programmingBlock_InTheTranspiledLang != 0) {
        for (int A_Index258 = 0; A_Index258 < COUNT_programmingBlock_InTheTranspiledLang + 0; A_Index258++) {
            htCode = StrReplace(htCode, "programmingBlock_InTheTranspiledLang-programmingBlock_InTheTranspiledLang-AA" + STR(A_Index258 + 1) + "AA", programmingBlock_InTheTranspiledLang[A_Index258]);
        }
    }
    if (langToConvertTo == "cpp") {
        if (COUNT_programmingBlock_CPP != 0) {
            for (int A_Index259 = 0; A_Index259 < COUNT_programmingBlock_CPP + 0; A_Index259++) {
                htCode = StrReplace(htCode, "programmingBlock_CPP-programmingBlock_CPP-AA" + STR(A_Index259 + 1) + "AA", programmingBlock_CPP[A_Index259]);
            }
        }
    } else {
        if (COUNT_programmingBlock_CPP != 0) {
            for (int A_Index260 = 0; A_Index260 < COUNT_programmingBlock_CPP + 0; A_Index260++) {
                if (langToConvertTo == langFileExtension_2) {
                    htCode = StrReplace(htCode, "programmingBlock_CPP-programmingBlock_CPP-AA" + STR(A_Index260 + 1) + "AA", keyWordCodeInTheTranspiledLangStartCPP_2 + Chr(10) + programmingBlock_CPP[A_Index260] + Chr(10) + keyWordCodeInTheTranspiledLangEndCPP_2);
                } else {
                    htCode = StrReplace(htCode, "programmingBlock_CPP-programmingBlock_CPP-AA" + STR(A_Index260 + 1) + "AA", Chr(10));
                }
            }
        }
    }
    if (langToConvertTo == "py") {
        if (COUNT_programmingBlock_PY != 0) {
            for (int A_Index261 = 0; A_Index261 < COUNT_programmingBlock_PY + 0; A_Index261++) {
                htCode = StrReplace(htCode, "programmingBlock_PY-programmingBlock_PY-AA" + STR(A_Index261 + 1) + "AA", programmingBlock_PY[A_Index261]);
            }
        }
    } else {
        if (COUNT_programmingBlock_PY != 0) {
            for (int A_Index262 = 0; A_Index262 < COUNT_programmingBlock_PY + 0; A_Index262++) {
                if (langToConvertTo == langFileExtension_2) {
                    htCode = StrReplace(htCode, "programmingBlock_PY-programmingBlock_PY-AA" + STR(A_Index262 + 1) + "AA", keyWordCodeInTheTranspiledLangStartPY_2 + Chr(10) + programmingBlock_PY[A_Index262] + Chr(10) + keyWordCodeInTheTranspiledLangEndPY_2);
                } else {
                    htCode = StrReplace(htCode, "programmingBlock_PY-programmingBlock_PY-AA" + STR(A_Index262 + 1) + "AA", Chr(10));
                }
            }
        }
    }
    if (langToConvertTo == "js") {
        if (COUNT_programmingBlock_JS != 0) {
            for (int A_Index263 = 0; A_Index263 < COUNT_programmingBlock_JS + 0; A_Index263++) {
                htCode = StrReplace(htCode, "programmingBlock_JS-programmingBlock_JS-AA" + STR(A_Index263 + 1) + "AA", programmingBlock_JS[A_Index263]);
            }
        }
    } else {
        if (COUNT_programmingBlock_JS != 0) {
            for (int A_Index264 = 0; A_Index264 < COUNT_programmingBlock_JS + 0; A_Index264++) {
                if (langToConvertTo == langFileExtension_2) {
                    htCode = StrReplace(htCode, "programmingBlock_JS-programmingBlock_JS-AA" + STR(A_Index264 + 1) + "AA", keyWordCodeInTheTranspiledLangStartJS_2 + Chr(10) + programmingBlock_JS[A_Index264] + Chr(10) + keyWordCodeInTheTranspiledLangEndJS_2);
                } else {
                    htCode = StrReplace(htCode, "programmingBlock_JS-programmingBlock_JS-AA" + STR(A_Index264 + 1) + "AA", Chr(10));
                }
            }
        }
    }
    if (langToConvertTo == "go") {
        if (COUNT_programmingBlock_GO != 0) {
            for (int A_Index265 = 0; A_Index265 < COUNT_programmingBlock_GO + 0; A_Index265++) {
                htCode = StrReplace(htCode, "programmingBlock_GO-programmingBlock_GO-AA" + STR(A_Index265 + 1) + "AA", programmingBlock_GO[A_Index265]);
            }
        }
    } else {
        if (COUNT_programmingBlock_GO != 0) {
            for (int A_Index266 = 0; A_Index266 < COUNT_programmingBlock_GO + 0; A_Index266++) {
                if (langToConvertTo == langFileExtension_2) {
                    htCode = StrReplace(htCode, "programmingBlock_GO-programmingBlock_GO-AA" + STR(A_Index266 + 1) + "AA", keyWordCodeInTheTranspiledLangStartGO_2 + Chr(10) + programmingBlock_GO[A_Index266] + Chr(10) + keyWordCodeInTheTranspiledLangEndGO_2);
                } else {
                    htCode = StrReplace(htCode, "programmingBlock_GO-programmingBlock_GO-AA" + STR(A_Index266 + 1) + "AA", Chr(10));
                }
            }
        }
    }
    if (langToConvertTo == "lua") {
        if (COUNT_programmingBlock_LUA != 0) {
            for (int A_Index267 = 0; A_Index267 < COUNT_programmingBlock_LUA + 0; A_Index267++) {
                htCode = StrReplace(htCode, "programmingBlock_LUA-programmingBlock_LUA-AA" + STR(A_Index267 + 1) + "AA", programmingBlock_LUA[A_Index267]);
            }
        }
    } else {
        if (COUNT_programmingBlock_LUA != 0) {
            for (int A_Index268 = 0; A_Index268 < COUNT_programmingBlock_LUA + 0; A_Index268++) {
                if (langToConvertTo == langFileExtension_2) {
                    htCode = StrReplace(htCode, "programmingBlock_LUA-programmingBlock_LUA-AA" + STR(A_Index268 + 1) + "AA", keyWordCodeInTheTranspiledLangStartLUA_2 + Chr(10) + programmingBlock_LUA[A_Index268] + Chr(10) + keyWordCodeInTheTranspiledLangEndLUA_2);
                } else {
                    htCode = StrReplace(htCode, "programmingBlock_LUA-programmingBlock_LUA-AA" + STR(A_Index268 + 1) + "AA", Chr(10));
                }
            }
        }
    }
    if (langToConvertTo == "cs") {
        if (COUNT_programmingBlock_CS != 0) {
            for (int A_Index269 = 0; A_Index269 < COUNT_programmingBlock_CS + 0; A_Index269++) {
                htCode = StrReplace(htCode, "programmingBlock_CS-programmingBlock_CS-AA" + STR(A_Index269 + 1) + "AA", programmingBlock_CS[A_Index269]);
            }
        }
    } else {
        if (COUNT_programmingBlock_CS != 0) {
            for (int A_Index270 = 0; A_Index270 < COUNT_programmingBlock_CS + 0; A_Index270++) {
                if (langToConvertTo == langFileExtension_2) {
                    htCode = StrReplace(htCode, "programmingBlock_CS-programmingBlock_CS-AA" + STR(A_Index270 + 1) + "AA", keyWordCodeInTheTranspiledLangStartCS_2 + Chr(10) + programmingBlock_CS[A_Index270] + Chr(10) + keyWordCodeInTheTranspiledLangEndCS_2);
                } else {
                    htCode = StrReplace(htCode, "programmingBlock_CS-programmingBlock_CS-AA" + STR(A_Index270 + 1) + "AA", Chr(10));
                }
            }
        }
    }
    if (langToConvertTo == "java") {
        if (COUNT_programmingBlock_JAVA != 0) {
            for (int A_Index271 = 0; A_Index271 < COUNT_programmingBlock_JAVA + 0; A_Index271++) {
                htCode = StrReplace(htCode, "programmingBlock_JAVA-programmingBlock_JAVA-AA" + STR(A_Index271 + 1) + "AA", programmingBlock_JAVA[A_Index271]);
            }
        }
    } else {
        if (COUNT_programmingBlock_JAVA != 0) {
            for (int A_Index272 = 0; A_Index272 < COUNT_programmingBlock_JAVA + 0; A_Index272++) {
                if (langToConvertTo == langFileExtension_2) {
                    htCode = StrReplace(htCode, "programmingBlock_JAVA-programmingBlock_JAVA-AA" + STR(A_Index272 + 1) + "AA", keyWordCodeInTheTranspiledLangStartJAVA_2 + Chr(10) + programmingBlock_JAVA[A_Index272] + Chr(10) + keyWordCodeInTheTranspiledLangEndJAVA_2);
                } else {
                    htCode = StrReplace(htCode, "programmingBlock_JAVA-programmingBlock_JAVA-AA" + STR(A_Index272 + 1) + "AA", Chr(10));
                }
            }
        }
    }
    if (langToConvertTo == "kt") {
        if (COUNT_programmingBlock_KT != 0) {
            for (int A_Index273 = 0; A_Index273 < COUNT_programmingBlock_KT + 0; A_Index273++) {
                htCode = StrReplace(htCode, "programmingBlock_KT-programmingBlock_KT-AA" + STR(A_Index273 + 1) + "AA", programmingBlock_KT[A_Index273]);
            }
        }
    } else {
        if (COUNT_programmingBlock_KT != 0) {
            for (int A_Index274 = 0; A_Index274 < COUNT_programmingBlock_KT + 0; A_Index274++) {
                if (langToConvertTo == langFileExtension_2) {
                    htCode = StrReplace(htCode, "programmingBlock_KT-programmingBlock_KT-AA" + STR(A_Index274 + 1) + "AA", keyWordCodeInTheTranspiledLangStartKT_2 + Chr(10) + programmingBlock_KT[A_Index274] + Chr(10) + keyWordCodeInTheTranspiledLangEndKT_2);
                } else {
                    htCode = StrReplace(htCode, "programmingBlock_KT-programmingBlock_KT-AA" + STR(A_Index274 + 1) + "AA", Chr(10));
                }
            }
        }
    }
    if (langToConvertTo == "rb") {
        if (COUNT_programmingBlock_RB != 0) {
            for (int A_Index275 = 0; A_Index275 < COUNT_programmingBlock_RB + 0; A_Index275++) {
                htCode = StrReplace(htCode, "programmingBlock_RB-programmingBlock_RB-AA" + STR(A_Index275 + 1) + "AA", programmingBlock_RB[A_Index275]);
            }
        }
    } else {
        if (COUNT_programmingBlock_RB != 0) {
            for (int A_Index276 = 0; A_Index276 < COUNT_programmingBlock_RB + 0; A_Index276++) {
                if (langToConvertTo == langFileExtension_2) {
                    htCode = StrReplace(htCode, "programmingBlock_RB-programmingBlock_RB-AA" + STR(A_Index276 + 1) + "AA", keyWordCodeInTheTranspiledLangStartRB_2 + Chr(10) + programmingBlock_RB[A_Index276] + Chr(10) + keyWordCodeInTheTranspiledLangEndRB_2);
                } else {
                    htCode = StrReplace(htCode, "programmingBlock_RB-programmingBlock_RB-AA" + STR(A_Index276 + 1) + "AA", Chr(10));
                }
            }
        }
    }
    if (langToConvertTo == "nim") {
        if (COUNT_programmingBlock_NIM != 0) {
            for (int A_Index277 = 0; A_Index277 < COUNT_programmingBlock_NIM + 0; A_Index277++) {
                htCode = StrReplace(htCode, "programmingBlock_NIM-programmingBlock_NIM-AA" + STR(A_Index277 + 1) + "AA", programmingBlock_NIM[A_Index277]);
            }
        }
    } else {
        if (COUNT_programmingBlock_NIM != 0) {
            for (int A_Index278 = 0; A_Index278 < COUNT_programmingBlock_NIM + 0; A_Index278++) {
                if (langToConvertTo == langFileExtension_2) {
                    htCode = StrReplace(htCode, "programmingBlock_NIM-programmingBlock_NIM-AA" + STR(A_Index278 + 1) + "AA", keyWordCodeInTheTranspiledLangStartNIM_2 + Chr(10) + programmingBlock_NIM[A_Index278] + Chr(10) + keyWordCodeInTheTranspiledLangEndNIM_2);
                } else {
                    htCode = StrReplace(htCode, "programmingBlock_NIM-programmingBlock_NIM-AA" + STR(A_Index278 + 1) + "AA", Chr(10));
                }
            }
        }
    }
    if (langToConvertTo == "ahk") {
        if (COUNT_programmingBlock_AHK != 0) {
            for (int A_Index279 = 0; A_Index279 < COUNT_programmingBlock_AHK + 0; A_Index279++) {
                htCode = StrReplace(htCode, "programmingBlock_AHK-programmingBlock_AHK-AA" + STR(A_Index279 + 1) + "AA", programmingBlock_AHK[A_Index279]);
            }
        }
    } else {
        if (COUNT_programmingBlock_AHK != 0) {
            for (int A_Index280 = 0; A_Index280 < COUNT_programmingBlock_AHK + 0; A_Index280++) {
                if (langToConvertTo == langFileExtension_2) {
                    htCode = StrReplace(htCode, "programmingBlock_AHK-programmingBlock_AHK-AA" + STR(A_Index280 + 1) + "AA", keyWordCodeInTheTranspiledLangStartAHK_2 + Chr(10) + programmingBlock_AHK[A_Index280] + Chr(10) + keyWordCodeInTheTranspiledLangEndAHK_2);
                } else {
                    htCode = StrReplace(htCode, "programmingBlock_AHK-programmingBlock_AHK-AA" + STR(A_Index280 + 1) + "AA", Chr(10));
                }
            }
        }
    }
    if (langToConvertTo == "swift") {
        if (COUNT_programmingBlock_SWIFT != 0) {
            for (int A_Index281 = 0; A_Index281 < COUNT_programmingBlock_SWIFT + 0; A_Index281++) {
                htCode = StrReplace(htCode, "programmingBlock_SWIFT-programmingBlock_SWIFT-AA" + STR(A_Index281 + 1) + "AA", programmingBlock_SWIFT[A_Index281]);
            }
        }
    } else {
        if (COUNT_programmingBlock_SWIFT != 0) {
            for (int A_Index282 = 0; A_Index282 < COUNT_programmingBlock_SWIFT + 0; A_Index282++) {
                if (langToConvertTo == langFileExtension_2) {
                    htCode = StrReplace(htCode, "programmingBlock_SWIFT-programmingBlock_SWIFT-AA" + STR(A_Index282 + 1) + "AA", keyWordCodeInTheTranspiledLangStartSWIFT_2 + Chr(10) + programmingBlock_SWIFT[A_Index282] + Chr(10) + keyWordCodeInTheTranspiledLangEndSWIFT_2);
                } else {
                    htCode = StrReplace(htCode, "programmingBlock_SWIFT-programmingBlock_SWIFT-AA" + STR(A_Index282 + 1) + "AA", Chr(10));
                }
            }
        }
    }
    if (langToConvertTo == "dart") {
        if (COUNT_programmingBlock_DART != 0) {
            for (int A_Index283 = 0; A_Index283 < COUNT_programmingBlock_DART + 0; A_Index283++) {
                htCode = StrReplace(htCode, "programmingBlock_DART-programmingBlock_DART-AA" + STR(A_Index283 + 1) + "AA", programmingBlock_DART[A_Index283]);
            }
        }
    } else {
        if (COUNT_programmingBlock_DART != 0) {
            for (int A_Index284 = 0; A_Index284 < COUNT_programmingBlock_DART + 0; A_Index284++) {
                if (langToConvertTo == langFileExtension_2) {
                    htCode = StrReplace(htCode, "programmingBlock_DART-programmingBlock_DART-AA" + STR(A_Index284 + 1) + "AA", keyWordCodeInTheTranspiledLangStartDART_2 + Chr(10) + programmingBlock_DART[A_Index284] + Chr(10) + keyWordCodeInTheTranspiledLangEndDART_2);
                } else {
                    htCode = StrReplace(htCode, "programmingBlock_DART-programmingBlock_DART-AA" + STR(A_Index284 + 1) + "AA", Chr(10));
                }
            }
        }
    }
    if (langToConvertTo == "ts") {
        if (COUNT_programmingBlock_TS != 0) {
            for (int A_Index285 = 0; A_Index285 < COUNT_programmingBlock_TS + 0; A_Index285++) {
                htCode = StrReplace(htCode, "programmingBlock_TS-programmingBlock_TS-AA" + STR(A_Index285 + 1) + "AA", programmingBlock_TS[A_Index285]);
            }
        }
    } else {
        if (COUNT_programmingBlock_TS != 0) {
            for (int A_Index286 = 0; A_Index286 < COUNT_programmingBlock_TS + 0; A_Index286++) {
                if (langToConvertTo == langFileExtension_2) {
                    htCode = StrReplace(htCode, "programmingBlock_TS-programmingBlock_TS-AA" + STR(A_Index286 + 1) + "AA", keyWordCodeInTheTranspiledLangStartTS_2 + Chr(10) + programmingBlock_TS[A_Index286] + Chr(10) + keyWordCodeInTheTranspiledLangEndTS_2);
                } else {
                    htCode = StrReplace(htCode, "programmingBlock_TS-programmingBlock_TS-AA" + STR(A_Index286 + 1) + "AA", Chr(10));
                }
            }
        }
    }
    if (langToConvertTo == "groovy") {
        if (COUNT_programmingBlock_GROOVY != 0) {
            for (int A_Index287 = 0; A_Index287 < COUNT_programmingBlock_GROOVY + 0; A_Index287++) {
                htCode = StrReplace(htCode, "programmingBlock_GROOVY-programmingBlock_GROOVY-AA" + STR(A_Index287 + 1) + "AA", programmingBlock_GROOVY[A_Index287]);
            }
        }
    } else {
        if (COUNT_programmingBlock_GROOVY != 0) {
            for (int A_Index288 = 0; A_Index288 < COUNT_programmingBlock_GROOVY + 0; A_Index288++) {
                if (langToConvertTo == langFileExtension_2) {
                    htCode = StrReplace(htCode, "programmingBlock_GROOVY-programmingBlock_GROOVY-AA" + STR(A_Index288 + 1) + "AA", keyWordCodeInTheTranspiledLangStartGROOVY_2 + Chr(10) + programmingBlock_GROOVY[A_Index288] + Chr(10) + keyWordCodeInTheTranspiledLangEndGROOVY_2);
                } else {
                    htCode = StrReplace(htCode, "programmingBlock_GROOVY-programmingBlock_GROOVY-AA" + STR(A_Index288 + 1) + "AA", Chr(10));
                }
            }
        }
    }
    ...
    if (langToConvertTo == langFileExtension_2) {
        if (COUNT_programmingBlock_HTVM != 0) {
            for (int A_Index289 = 0; A_Index289 < COUNT_programmingBlock_HTVM + 0; A_Index289++) {
                htCode = StrReplace(htCode, "programmingBlock_HTVM-programmingBlock_HTVM-AA" + STR(A_Index289 + 1) + "AA", programmingBlock_HTVM[A_Index289]);
            }
        }
    } else {
        if (COUNT_programmingBlock_HTVM != 0) {
            for (int A_Index290 = 0; A_Index290 < COUNT_programmingBlock_HTVM + 0; A_Index290++) {
                htCode = StrReplace(htCode, "programmingBlock_HTVM-programmingBlock_HTVM-AA" + STR(A_Index290 + 1) + "AA", Chr(10));
            }
        }
    }
    if (langToConvertTo != langFileExtension_2) {
        ...
        for (int A_Index291 = 0; A_Index291 < theIdNumOfThe34 + 0; A_Index291++) {
            if (theIdNumOfThe34 == A_Index291 + 1) {
                if (langToConvertTo == langFileExtension_2) {
                    if (keyWordEscpaeChar_2 == "\\" && keyWordEscpaeChar != "\\") {
                        htCode = StrReplace(htCode, "fihuiuuhuuhtheidFor--asds" + str21 + "as--" + str21 + "theuhturtyphoutr--" + Chr(65) + Chr(65) + STR(A_Index291 + 1) + Chr(65) + Chr(65), StrReplace(StrReplace(fString(theIdNumOfThe34theVar[A_Index291 + 1]), "\\", "\\\\"), keyWordEscpaeChar, keyWordEscpaeChar_2) + Chr(34));
                    } else {
                        htCode = StrReplace(htCode, "fihuiuuhuuhtheidFor--asds" + str21 + "as--" + str21 + "theuhturtyphoutr--" + Chr(65) + Chr(65) + STR(A_Index291 + 1) + Chr(65) + Chr(65), StrReplace(fString(theIdNumOfThe34theVar[A_Index291 + 1]), keyWordEscpaeChar, keyWordEscpaeChar_2) + Chr(34));
                    }
                } else {
                    htCode = StrReplace(htCode, "fihuiuuhuuhtheidFor--asds" + str21 + "as--" + str21 + "theuhturtyphoutr--" + Chr(65) + Chr(65) + STR(A_Index291 + 1) + Chr(65) + Chr(65), StrReplace(fString(theIdNumOfThe34theVar[A_Index291 + 1]), keyWordEscpaeChar, "\\") + Chr(34));
                }
            } else {
                if (langToConvertTo == langFileExtension_2) {
                    if (keyWordEscpaeChar_2 == "\\" && keyWordEscpaeChar != "\\") {
                        htCode = StrReplace(htCode, "fihuiuuhuuhtheidFor--asds" + str21 + "as--" + str21 + "theuhturtyphoutr--" + Chr(65) + Chr(65) + STR(A_Index291 + 1) + Chr(65) + Chr(65), StrReplace(StrReplace(fString(theIdNumOfThe34theVar[A_Index291 + 1]), "\\", "\\\\"), keyWordEscpaeChar, keyWordEscpaeChar_2));
                    } else {
                        htCode = StrReplace(htCode, "fihuiuuhuuhtheidFor--asds" + str21 + "as--" + str21 + "theuhturtyphoutr--" + Chr(65) + Chr(65) + STR(A_Index291 + 1) + Chr(65) + Chr(65), StrReplace(fString(theIdNumOfThe34theVar[A_Index291 + 1]), keyWordEscpaeChar, keyWordEscpaeChar_2));
                    }
                } else {
                    htCode = StrReplace(htCode, "fihuiuuhuuhtheidFor--asds" + str21 + "as--" + str21 + "theuhturtyphoutr--" + Chr(65) + Chr(65) + STR(A_Index291 + 1) + Chr(65) + Chr(65), StrReplace(fString(theIdNumOfThe34theVar[A_Index291 + 1]), keyWordEscpaeChar, "\\"));
                }
            }
        }
    }
    ...
    int correctLang = 0;
    std::vector<std::string> items292 = LoopParseFunc(instructions, "\n", "\r");
    for (size_t A_Index292 = 0; A_Index292 < items292.size() + 0; A_Index292++) {
        std::string A_LoopField292 = items292[A_Index292 - 0];
        if (Trim(A_LoopField292) == "funcEND======================funcEND==============") {
            areWeInAFuncFromInstructions = 0;
            areWeInAFuncFromInstructionsLineNum = 0;
        ...
        if (areWeInAFuncFromInstructions == 1) {
            if (areWeInAFuncFromInstructionsLineNum == 1) {
                // name of the func
                funcLangHolder = StringTrimLeft(A_LoopField292, 5);
            ...
            if (areWeInAFuncFromInstructionsLineNum == 2) {
                // name of the func
                funcNameHolder = StringTrimLeft(A_LoopField292, 5);
                if (correctLang == 1 && InStr(htCode, Trim(funcNameHolder))) {
            ...
            if (areWeInAFuncFromInstructionsLineNum == 3) {
                // all libs
                funcLibsHolder = StringTrimLeft(A_LoopField292, 5);
                if (correctLang == 1 && InStr(htCode, Trim(funcNameHolder))) {
            ...
            if (areWeInAFuncFromInstructionsLineNum == 4) {
                // func description
                funcDescriptionHolder = StringTrimLeft(A_LoopField292, 12);
                if (correctLang == 1 && InStr(htCode, Trim(funcNameHolder))) {
            ...
            if (areWeInAFuncFromInstructionsLineNum >= 5) {
                // the full func
                if (correctLang == 1 && InStr(htCode, Trim(funcNameHolder))) {
                    funcFuncHolder += A_LoopField292 + Chr(10);
                }
            }
            //MsgBox, % A_LoopField
            areWeInAFuncFromInstructionsLineNum++;
        }
        if (Trim(A_LoopField292) == "func======================func==============") {
    ...
    if (!(HTVM_Size(allFuncNames) <= 0)) {
        for (int A_Index293 = 0; A_Index293 < HTVM_Size(allFuncNames) + 0; A_Index293++) {
            if (InStr(htCode, allFuncNames[A_Index293]) + "(") {
                //MsgBox, % allFuncNames[A_Index]
                allFuncsToPutAtTop += allFuncs[A_Index293] + Chr(10);
                if (Trim(allFuncLibs[A_Index293]) != "null") {
                    allLibsToPutAtTop += allFuncLibs[A_Index293] + "|";
                }
            }
        }
        allLibsToPutAtTop = StringTrimRight(allLibsToPutAtTop, 1);
        std::string allLibsToPutAtTopTEMP = "";
        std::vector<std::string> items294 = LoopParseFunc(allLibsToPutAtTop, "|");
        for (size_t A_Index294 = 0; A_Index294 < items294.size() + 0; A_Index294++) {
            std::string A_LoopField294 = items294[A_Index294 - 0];
            allLibsToPutAtTopTEMP += A_LoopField294 + Chr(10);
        }
    ...
    if (langToConvertTo != langFileExtension_2) {
        for (int A_Index295 = 0; A_Index295 < HTVM_Size(ospDic1) + 0; A_Index295++) {
            str00 = ospDic1[A_Index295];
            if (langToConvertTo == "rb") {
    ...
    if (size_ospDic != 0) {
        for (int A_Index296 = 0; A_Index296 < size_ospDic + 0; A_Index296++) {
            HTVM_Pop(ospDic);
        }
    }
    //;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;
    ...
    if (size_ospDic1 != 0) {
        for (int A_Index297 = 0; A_Index297 < size_ospDic1 + 0; A_Index297++) {
            HTVM_Pop(ospDic1);
        }
    }
    //;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;
    ...
    if (size_ospDic2 != 0) {
        for (int A_Index298 = 0; A_Index298 < size_ospDic2 + 0; A_Index298++) {
            HTVM_Pop(ospDic2);
        }
    }
    availableFuncsInHTVMInHTVM = "";
    saveAllArrayVarNamesSoWeCanDoAfix = "";
    for (int A_Index299 = 0; A_Index299 < theIdNumOfThe34 + 0; A_Index299++) {
        if (theIdNumOfThe34 == A_Index299 + 1) {
            if (langToConvertTo == langFileExtension_2) {
                if (keyWordEscpaeChar_2 == "\\" && keyWordEscpaeChar != "\\") {
                    htCode = StrReplace(htCode, "ihuiuuhuuhtheidFor--asds" + str21 + "as--" + str21 + "theuhturtyphoutr--" + Chr(65) + Chr(65) + STR(A_Index299 + 1) + Chr(65) + Chr(65), StrReplace(StrReplace(theIdNumOfThe34theVar[A_Index299 + 1], "\\", "\\\\"), keyWordEscpaeChar, keyWordEscpaeChar_2) + Chr(34));
                } else {
                    htCode = StrReplace(htCode, "ihuiuuhuuhtheidFor--asds" + str21 + "as--" + str21 + "theuhturtyphoutr--" + Chr(65) + Chr(65) + STR(A_Index299 + 1) + Chr(65) + Chr(65), StrReplace(theIdNumOfThe34theVar[A_Index299 + 1], keyWordEscpaeChar, keyWordEscpaeChar_2) + Chr(34));
                }
            } else {
                htCode = StrReplace(htCode, "ihuiuuhuuhtheidFor--asds" + str21 + "as--" + str21 + "theuhturtyphoutr--" + Chr(65) + Chr(65) + STR(A_Index299 + 1) + Chr(65) + Chr(65), StrReplace(theIdNumOfThe34theVar[A_Index299 + 1], keyWordEscpaeChar, "\\") + Chr(34));
            }
        } else {
            if (langToConvertTo == langFileExtension_2) {
                if (keyWordEscpaeChar_2 == "\\" && keyWordEscpaeChar != "\\") {
                    htCode = StrReplace(htCode, "ihuiuuhuuhtheidFor--asds" + str21 + "as--" + str21 + "theuhturtyphoutr--" + Chr(65) + Chr(65) + STR(A_Index299 + 1) + Chr(65) + Chr(65), StrReplace(StrReplace(theIdNumOfThe34theVar[A_Index299 + 1], "\\", "\\\\"), keyWordEscpaeChar, keyWordEscpaeChar_2));
                } else {
                    htCode = StrReplace(htCode, "ihuiuuhuuhtheidFor--asds" + str21 + "as--" + str21 + "theuhturtyphoutr--" + Chr(65) + Chr(65) + STR(A_Index299 + 1) + Chr(65) + Chr(65), StrReplace(theIdNumOfThe34theVar[A_Index299 + 1], keyWordEscpaeChar, keyWordEscpaeChar_2));
                }
            } else {
                htCode = StrReplace(htCode, "ihuiuuhuuhtheidFor--asds" + str21 + "as--" + str21 + "theuhturtyphoutr--" + Chr(65) + Chr(65) + STR(A_Index299 + 1) + Chr(65) + Chr(65), StrReplace(theIdNumOfThe34theVar[A_Index299 + 1], keyWordEscpaeChar, "\\"));
            }
        }
    }
    if (langToConvertTo == langFileExtension_2) {
    ...
    if (isNotHTVMfile2 == 0) {
        ...
        if (langToConvertTo == langFileExtension_2) {
            for (int A_Index300 = 0; A_Index300 < HTVM_Size(weAreInMLSarr) + 0; A_Index300++) {
                htCode = StrReplace(htCode, "aejsoydoxubviycbdgsut7eoy-ihsxbvhowadesdio6376e7wuwyuau--szd--AA" + STR(A_Index300) + "AA", weAreInMLSarr[A_Index300]);
            }
    ...
    if (size_weAreInMLSarr != 0) {
        for (int A_Index301 = 0; A_Index301 < size_weAreInMLSarr + 0; A_Index301++) {
...
void HTVMv2() {
    ...
    std::vector<std::string> items302 = LoopParseFunc(str0);
    for (size_t A_Index302 = 0; A_Index302 < items302.size() + 0; A_Index302++) {
        std::string A_LoopField302 = items302[A_Index302 - 0];
        str00 = Trim(A_LoopField302);
    ...
    if (HTVM_getLang_HTVM() == "cpp" || HTVM_getLang_HTVM() == "py") {
        ...
        std::vector<std::string> items303 = LoopParseFunc(allArgs, "\n", "\r");
        for (size_t A_Index303 = 0; A_Index303 < items303.size() + 0; A_Index303++) {
            std::string A_LoopField303 = items303[A_Index303 - 0];
            if (A_Index303 == 0) {
                numOfParams++;
                argCODE = FileRead(Trim(A_LoopField303));
                argCODEfile = Trim(A_LoopField303);
            }
            else if (A_Index303 == 1) {
                numOfParams++;
                argHTVMinstr = Trim(A_LoopField303);
            }
            else if (A_Index303 == 2) {
                numOfParams++;
                argLangTo = Trim(A_LoopField303);
            } else {
                numOfParams++;
                HTVM_Append(argHTVMinstrMORE, Trim(A_LoopField303));
            }
        }
        //print("===============123431234===========start=====")
        for (int A_Index304 = 0; A_Index304 < HTVM_Size(argHTVMinstrMORE) + 0; A_Index304++) {
