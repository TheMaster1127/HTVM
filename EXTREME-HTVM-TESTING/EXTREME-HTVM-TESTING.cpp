...
std::vector<std::string> GenerateRandomizedStates() {
    ...
    for (int A_Index1 = 0; A_Index1 < numStates + 0; A_Index1++) {
    ...
    for (int A_Index2 = 0; A_Index2 < HTVM_Size(remainingIndices) + 0; A_Index2++) {
        int currentIndexInLoop = A_Index2;
...
std::string GenerateGibberishName(int minLen, int maxLen) {
    ...
    for (int A_Index3 = 0; A_Index3 < (len - 1) + 0; A_Index3++) {
...
std::string GenerateOperatorGibberishName(std::string prefix, int minTotalLen, int maxTotalLen) {
    ...
    for (int A_Index4 = 0; A_Index4 < remainingLen + 0; A_Index4++) {
...
std::string GetRandomSane_Assign_Keyword() {
    std::vector<std::string> saneNames = {"assign", "set", "is", "becomes", "equals", "letBe", "store", "defineAs", "setTo", "make", "transfer", "copyTo", "initAs", "valueIs", "varIs", "dataIs", "bindTo", "linkTo", "holdVal", "giveVal", "acceptVal", "fixVal", "placeVal", "gets", "receives", "takes"};
    return saneNames[Random(0, HTVM_Size(saneNames) - 1)];
}
std::string GetRandomSane_AssignAdd_Keyword() {
    std::vector<std::string> saneNames = {"addAssign", "plusEquals", "incrementBy", "addTo", "sumAssign", "increaseBy", "accumulate", "appendNum", "augAssign", "combineAdd", "mergeSum", "growBy", "extendSum", "amassAdd", "PlusEq", "sumTo", "tallyUp"};
    return saneNames[Random(0, HTVM_Size(saneNames) - 1)];
}
std::string GetRandomSane_AssignConcat_Keyword() {
    std::vector<std::string> saneNames = {"concatAssign", "appendStr", "stringAdd", "dotEquals", "joinAssign", "textAppend", "mergeStr", "combineText", "chainStr", "linkText", "uniteStr", "fuseText", "weldStr", "CatEq", "stringJoin", "attachText"};
    return saneNames[Random(0, HTVM_Size(saneNames) - 1)];
}
std::string GetRandomSane_AssignSub_Keyword() {
    std::vector<std::string> saneNames = {"subAssign", "minusEquals", "decrementBy", "subFrom", "deductAssign", "decreaseBy", "reduceBy", "diminishBy", "shrinkBy", "lessenBy", "withdrawFrom", "MinusEq", "takeAway", "deductFrom"};
    return saneNames[Random(0, HTVM_Size(saneNames) - 1)];
}
std::string GetRandomSane_AssignMul_Keyword() {
    std::vector<std::string> saneNames = {"mulAssign", "timesEquals", "multiplyBy", "scaleBy", "productAssign", "factorAssign", "amplifyBy", "magnifyBy", "replicateBy", "TimesEq", "MultEq", "productIs"};
    return saneNames[Random(0, HTVM_Size(saneNames) - 1)];
}
std::string GetRandomSane_AssignDiv_Keyword() {
    std::vector<std::string> saneNames = {"divAssign", "divideEquals", "divideBy", "ratioAssign", "quotientAssign", "shareBy", "splitBy", "partitionBy", "DivEq", "SlashEq", "quotientIs"};
    return saneNames[Random(0, HTVM_Size(saneNames) - 1)];
}
std::string GetRandomSane_AssignMod_Keyword() {
    std::vector<std::string> saneNames = {"modAssign", "moduloEquals", "remainderAssign", "modBy", "cycleAssign", "wrapAssign", "ModEq", "RemEq", "PercentEq", "moduloIs"};
    return saneNames[Random(0, HTVM_Size(saneNames) - 1)];
}
std::string GetRandomSane_AssignShiftLeft_Keyword() {
    std::vector<std::string> saneNames = {"shiftLAssign", "lshEquals", "bitShiftLStore", "moveBitsLSet", "slideLStore", "LShiftEq", "LeftShiftAssign", "LSLset", "bitSlideL"};
    return saneNames[Random(0, HTVM_Size(saneNames) - 1)];
}
std::string GetRandomSane_AssignShiftRight_Keyword() {
    std::vector<std::string> saneNames = {"shiftRAssign", "rshEquals", "bitShiftRStore", "moveBitsRSet", "slideRStore", "RShiftEq", "RightShiftAssign", "RSRset", "bitSlideR"};
    return saneNames[Random(0, HTVM_Size(saneNames) - 1)];
}
std::string GetRandomSane_AssignUShiftRight_Keyword() {
    std::vector<std::string> saneNames = {"uShiftRAssign", "urshEquals", "logicShiftRStore", "zeroFillRSet", "positiveRStore", "URShiftEq", "UShiftAssign", "USRset", "logicSlideR"};
    return saneNames[Random(0, HTVM_Size(saneNames) - 1)];
}
std::string GetRandomSane_AssignBitAnd_Keyword() {
    std::vector<std::string> saneNames = {"bitAndAssign", "andEquals", "maskAssign", "intersectStore", "filterAndSet", "BitAndEq", "AmpersandEq", "bitwiseAndSet"};
    return saneNames[Random(0, HTVM_Size(saneNames) - 1)];
}
std::string GetRandomSane_AssignBitOr_Keyword() {
    std::vector<std::string> saneNames = {"bitOrAssign", "orEquals", "setBitsStore", "unionOrSet", "includeOrStore", "BitOrEq", "PipeEq", "bitwiseOrSet"};
    return saneNames[Random(0, HTVM_Size(saneNames) - 1)];
}
std::string GetRandomSane_AssignBitXor_Keyword() {
    std::vector<std::string> saneNames = {"bitXorAssign", "xorEquals", "toggleBitsStore", "exclusiveOrSet", "flipBitsStore", "BitXorEq", "CaretEq", "bitwiseXorSet"};
    return saneNames[Random(0, HTVM_Size(saneNames) - 1)];
}
...
int main(int argc, char* argv[]) {
    ...
    for (int A_Index5 = 0; A_Index5 < HTVM_Size(assignmentOpsGenerated) + 0; A_Index5++) {
    ...
    for (int A_Index10 = 0; A_Index10 < HTVM_Size(outArrFromStates) + 0; A_Index10++) {
